...
/* If user want to init OPTEE from u-boot */
#ifdef BOOT_AFTER_UBOOT
static uint32_t bl32_not_init = 1;
extern int32_t (*bl32_init)(void);
#endif
/*******************************************************************************
...
#ifdef BOOT_AFTER_UBOOT
		/*
		 * If OPTEE exist and not be initialized, init it now.
		 */
		if ((bl32_init != NULL) && (bl32_not_init)) {
			bl32_not_init = 0;
