...
#define TSIDEN		(ULL(1) << 10)
#define SMEN		(ULL(1) << 11)
#define CLUSTERPMUEN	(ULL(1) << 12)

uintptr_t rdistif_base_addrs[PLATFORM_CORE_COUNT] = {
	(GICR_BASE + 0x00000),	/* GICR Base address of CPU0 */
	(GICR_BASE + 0x20000),	/* GICR Base address of CPU1 */
	(GICR_BASE + 0x40000),	/* GICR Base address of CPU2 */
	(GICR_BASE + 0x60000),	/* GICR Base address of CPU3 */
	(GICR_BASE + 0x80000),	/* GICR Base address of CPU4 */
	(GICR_BASE + 0xA0000),	/* GICR Base address of CPU5 */
	(GICR_BASE + 0xC0000),	/* GICR Base address of CPU6 */
	(GICR_BASE + 0xE0000),	/* GICR Base address of CPU7 */
//	0U			/* Zero Termination */
};
static unsigned int nvt_mpidr_to_core_pos(u_register_t mpidr)
{
	return plat_core_pos_by_mpidr(mpidr);
}

const gicv3_driver_data_t gicv3_data = {
	.gicd_base = GICD_BASE,
	.gicr_base = 0,//GICR_BASE,
	.rdistif_num = PLATFORM_CORE_COUNT,
	.rdistif_base_addrs = rdistif_base_addrs,
	.mpidr_to_core_pos = nvt_mpidr_to_core_pos,
};
...
void bl31_plat_arch_setup(void)
{
	...
	/* enable clusterpm/sm/tsid to el1 */
	actlr = read_actlr_el3();
	actlr |= (TSIDEN | SMEN | CLUSTERPMUEN);
	write_actlr_el3(actlr);

	actlr = read_actlr_el2();
	actlr |= (TSIDEN | SMEN | CLUSTERPMUEN);
