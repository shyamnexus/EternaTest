...
static __aligned(CACHE_LINE_ALIGN) uint8_t hash_tmp_buf[HASH_TMP_BUF_SIZE];

static const struct crypto_hash_ops nvt_hash_ops;

static struct nvt_hash_ctx *to_hash_ctx(struct crypto_hash_ctx *ctx)
{
	assert(ctx && ctx->ops == &nvt_hash_ops);

	return container_of(ctx, struct nvt_hash_ctx, ctx);
}

static TEE_Result nvt_hash_init(struct crypto_hash_ctx *ctx)
{
	struct nvt_hash_ctx *hc = to_hash_ctx(ctx);
...
static TEE_Result nvt_hw_hash_trigger(struct crypto_hash_ctx *ctx, uint8_t *data, size_t len, size_t pad_len)
{
	struct nvt_hash_ctx *hc = to_hash_ctx(ctx);
...
static TEE_Result nvt_hash_update(struct crypto_hash_ctx *ctx, const uint8_t *data, size_t len)
{
	struct nvt_hash_ctx *hc = to_hash_ctx(ctx);
	size_t   align_len;
	int      ret;
	...
	if (left && len >= fill) {
		...
		if ((ret = nvt_hw_hash_trigger(ctx, (uint8_t *)hc->sha.buf, 64, 0)) != 0)
			return ret;
	...
	if (len >= 64) {
		align_len = len & (~0x3f);
		if((ret = nvt_hw_hash_trigger(ctx, (uint8_t *)data, align_len, 0)) != 0)
			return ret;
...
static TEE_Result nvt_hash_final(struct crypto_hash_ctx *ctx, uint8_t *digest, size_t len)
{
	struct nvt_hash_ctx *hc = to_hash_ctx(ctx);
	size_t left;
	...
	left = hc->sha.total & 0x3F;
	nvt_hw_hash_trigger(ctx, (uint8_t *)hc->sha.buf, (UINT32)left, hc->sha.total);
	memcpy(digest, hc->sha.state, len);
	return TEE_SUCCESS;
}

static void nvt_hash_free_ctx(struct crypto_hash_ctx *ctx)
{
	free(to_hash_ctx(ctx));
}

static void nvt_hash_copy_state(struct crypto_hash_ctx *dst_ctx, struct crypto_hash_ctx *src_ctx)
{
	struct nvt_hash_ctx *src = to_hash_ctx(src_ctx);
	struct nvt_hash_ctx *dst = to_hash_ctx(dst_ctx);
...
static const struct crypto_hash_ops nvt_hash_ops = {
	.init       = nvt_hash_init,
	.update     = nvt_hash_update,
	.final      = nvt_hash_final,
	.free_ctx   = nvt_hash_free_ctx,
	.copy_state = nvt_hash_copy_state,
};

static TEE_Result nvt_hash_alloc_ctx(struct crypto_hash_ctx **ctx_ret, int hash_mode)
{
	struct nvt_hash_ctx *ctx = NULL;

	ctx = calloc(1, sizeof(*ctx));
	if (!ctx)
		return TEE_ERROR_OUT_OF_MEMORY;

	ctx->ctx.ops = &nvt_hash_ops;
	if (hash_mode == HASH_MODE_SHA1) {
...
static TEE_Result nvt_hashdrv_alloc_ctx(struct crypto_hash_ctx **ctx, uint32_t algo)
{
	TEE_Result res = TEE_ERROR_NOT_IMPLEMENTED;

	switch (algo) {
	case TEE_ALG_SHA1:
		res = nvt_hash_alloc_ctx(ctx, HASH_MODE_SHA1);
		break;
	case TEE_ALG_SHA256:
		res = nvt_hash_alloc_ctx(ctx, HASH_MODE_SHA256);
		break;
	default:
...
static TEE_Result nvt_hash_drv_init(void)
{
	return drvcrypt_register_hash(&nvt_hashdrv_alloc_ctx);
}
driver_init_late(nvt_hash_drv_init);
