...
/* Interface Functions                                                         */
/*-----------------------------------------------------------------------------*/
static void rtos_sem_add_waitlist_p(SEM_CELL_T   *p_semcell, VK_TASK_HANDLE task_hdl)
{
	int i;
...
static void rtos_sem_del_waitlist_p(SEM_CELL_T   *p_semcell, VK_TASK_HANDLE task_hdl)
{
	int i;
	SEM_WAIT_TSK_INFO *p_waitsk;
...
static int _vos_sem_wait(ID semid, int timeout_tick, int interruptible)
{
	...
	if (!((-1 == timeout_tick) && (0 == interruptible))) {
		DBG_ERR("Only support vos_sem_wait\r\n");
		return E_PAR;
	}
#endif
...
void vos_sem_dump(int (*dump)(const char *fmt, ...), int level)
{
	...
	for (i = 0; i < gsemnum; i++) {
		...
		if (0 == p_semcell->own_tsk) {
			...
			DBG_DUMP("Semaphore[%.3d, %32s] -> * (Max: %.2d, Cur: %.2d), Owner: tee%d\r\n",
			     i, p_semcell->name, p_semcell->maxsem, p_semcell->isemcnt, p_semcell->own_tsk);
		}
		if (p_semcell->waitsk_count > 0) {
			...
			for (tsk_i = 0; tsk_i < WAIT_TASK_NUM_MAX; tsk_i++, p_waitsk++) {
				...
				if (0 != p_waitsk->task_hdl) {
					DBG_DUMP("  Task[%.3d, tee%d], sleep_time = %lld ms\r\n",
						     task_id, p_waitsk->task_hdl, sleep_time_ms);
				}
