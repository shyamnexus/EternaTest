...
// Types Declarations
typedef unsigned int vk_spinlock_t;
typedef unsigned int vk_raw_spinlock_t;

// Interface Function Prototype
#define __VK_SPIN_LOCK_INITIALIZER(lock)    SPINLOCK_UNLOCK
#define __VK_SPIN_LOCK_UNLOCKED(lock)       __VK_SPIN_LOCK_INITIALIZER(lock)
#define VK_DEFINE_SPINLOCK(x)               vk_spinlock_t x = __VK_SPIN_LOCK_UNLOCKED(x)
#define VK_DEFINE_RAW_SPINLOCK(x)           raw_spinlock_t x = __VK_SPIN_LOCK_UNLOCKED(x)

#define vk_spin_lock_init(lock)             do { *(lock) = __VK_SPIN_LOCK_UNLOCKED(lock); } while (0)
#define vk_spin_lock                        cpu_spin_lock
#define vk_spin_unlock                      cpu_spin_unlock
#define vk_spin_trylock                     cpu_spin_trylock

#define vk_spin_lock_irqsave(lock, flags)   do { flags = cpu_spin_lock_xsave(lock); } while (0)
#define vk_spin_unlock_irqrestore           cpu_spin_unlock_xrestore
