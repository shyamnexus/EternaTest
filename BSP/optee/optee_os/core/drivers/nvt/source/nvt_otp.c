...
#if defined(_BSP_NA51102_) || defined(_BSP_NA51103_) || defined(_BSP_NS02201_)
	OUTREG32(KEY_MANAGER_TIMING_CONFIG_ADDRESS, uiReg);
#else
	uiclk_sel = pll_get_clock_rate(PLL_CLKSEL_APB);

	if (uiclk_sel == PLL_CLKSEL_APB_120) {
		DBG_DUMP("120MHz\r\n");
		uiReg = OTP_120_TIMING;
		OUTREG32(KEY_MANAGER_TIMING_CONFIG_ADDRESS, uiReg);
	}
#ifndef _BSP_NA51089_
	else if (uiclk_sel == PLL_CLKSEL_APB_150) {
		DBG_DUMP("150MHz\r\n");
		uiReg = OTP_150_TIMING;
		OUTREG32(KEY_MANAGER_TIMING_CONFIG_ADDRESS, uiReg);
	}
#endif
#endif
...
static INT32 key_write_data(UINT32 addr, UINT32 data)
{
	UINT32  ui_bits;
	UINT32  ui_data;
#if defined(_BSP_NS02201_) || defined(_BSP_NA51103_)
...
#else
		if (key_program_bit(addr, ui_bits, DBL_BIT_EN) != E_OK)
#endif
		{
...
BOOL quary_secure_boot(SECUREBOOT_STATUS scu_status)
{
	...
	case SECUREBOOT_DSP1_DISABLE:
		if ((sec & OTP_DSP1_DISABLE) == OTP_DSP1_DISABLE) {
			result = TRUE;
		}
		break;

	case SECUREBOOT_DSP2_DISABLE:
		if ((sec & OTP_DSP2_DISABLE) == OTP_DSP2_DISABLE) {
			result = TRUE;
		}
		break;
	...
	case SECUREBOOT_MCU_DISABLE:
		if ((sec2 & OTP_MCU_DISABLE) == OTP_MCU_DISABLE) {
			result = TRUE;
		}
		break;
#endif
...
#define OTP_REVISION_STR     "1.00.002"
#elif defined(_BSP_NS02201_)
#define OTP_MAJOR            00
#define OTP_MINOR            003
...
#endif
#if defined(_BSP_NA51102_)
static CHAR    OTPVersion[] CYGBLD_ATTRIB_SECTION(".otp.version." __xstring(1) "." __xstring(OTP_MAJOR)"."__xstring(OTP_MINOR)) = "NT9853X_OTP#"OTP_REVISION_STR;
#elif defined(_BSP_NA51103_)
...
#else
static CHAR    OTPVersion[] CYGBLD_ATTRIB_SECTION(".otp.version." __xstring(1) "." __xstring(OTP_MAJOR)"."__xstring(OTP_MINOR)) = "NT9869X_OTP#"OTP_REVISION_STR;
#endif
