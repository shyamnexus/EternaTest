...
#define ARB2_SETREG(ofs,value)          arb_set_reg(DDR_ARB_2, (ofs), (value))
#define ARB2_GETREG(ofs)                arb_get_reg(DDR_ARB_2, (ofs))

#define PROT2_SETREG(ofs,value)         arb_set_reg(DDR_ARB_2, 0x4300+(ofs),(value))
#define PROT2_GETREG(ofs)               arb_get_reg(DDR_ARB_2, 0x4300+(ofs))
...
static void prot_set_reg(DDR_ARB id, int prot_id, UINT32 offset, REGVALUE value)
{
	if (id == DDR_ARB_1) {
		if (prot_id < 6) {
			PROT_SETREG(offset + prot_id * 0x100, value);
		} else {
			DBG_ERR("invalid protect set %d\r\n", prot_id);
		}
	} else {
...
static vaddr_t arb_base_addr(DDR_ARB ddr_id)
{
	...
	if (cpu_mmu_enabled()) {
		...
		if (ddr_id == DDR_ARB_1) {
			...
			return IOADDR_DDR_ARB2_REG_BASE;
		}
									...
									uint32_t allow_ns_device_access)
{
	...
	if (protect_attr.protect_rgn_attr.starting_addr >= DRAM2_STARTING_PHYS_ADDRESS) {
		...
		if ((protect_attr.protect_rgn_attr.starting_addr + protect_attr.protect_rgn_attr.size) >= DRAM2_STARTING_PHYS_ADDRESS) {
