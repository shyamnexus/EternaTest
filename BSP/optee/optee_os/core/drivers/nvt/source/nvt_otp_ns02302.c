...
void otp_init(void)
{
	...
	DBG_DUMP("otp_reg_base= 0x%x chip_id[0x%08x]\r\n", (int)otp_reg_base, chip_id);
...
BOOL enable_secure_boot(SECUREBOOT_STATUS scu_status)
{
	...
	case SECUREBOOT_SECURE_EN:
		if ((sec & (OTP_HW_SECURE_EN | OTP_FW_SECURE_EN)) != 0x0) {
			...
			if(quary_secure_boot(SECUREBOOT_SECURE_EN)) {
				result = TRUE;
			} else {
				result = FALSE;
			}
	...
	case SECUREBOOT_DATA_AREA_ENCRYPT:
		if ((sec & OTP_DATA_ENCRYPT_EN) != 0x0) {
			...
			if(quary_secure_boot(SECUREBOOT_DATA_AREA_ENCRYPT)) {
				result = TRUE;
			} else {
				result = FALSE;
			}
...
UINT32 otp_key_manager(UINT32 rowAddress)
{
	...
	OTP_RSTN_DISABLE();
	if(chip_id == CHIP_NS02402) {
		uiData = local_load_addr(((rowAddress & 0x3F) | (DBL_BIT_EN << 11)), otp_reg_base);
	} else {
...
static CHAR    OTPVersion[] = "NT98538_OTP#"OTP_REVISION_STR;
