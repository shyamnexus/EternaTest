...
#if defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
...
{
#if defined(_BSP_NA51102_)
	return local_store_addr((rol | (col << 6) | (DBL_BIT_EN << 11)), otp_reg_base);
#elif defined(_BSP_NA51103_)
	return local_store_addr((rol | (col << 5) | (DBL_BIT_EN << 11)), otp_reg_base);
#else
...
//	UINT32 i;
	...
	DBG_DUMP("              is_new_key_rule()=%d\r\n", is_new_key_rule());


#if 0
	for (i = 16; i < 32; i += 4) {
		DBG_DUMP("key=0x%08x, 0x%08x, 0x%08x, 0x%08x\r\n", (int)otp_key_manager(i), (int)otp_key_manager(i + 1), (int)otp_key_manager(i + 2), (int)otp_key_manager(i + 3));
	}
#endif
}
