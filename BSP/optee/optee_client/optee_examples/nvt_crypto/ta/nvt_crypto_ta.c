...
{
	switch (param) {
	case TA_ALGO_AES_ECB:
		*algo = TEE_ALG_AES_ECB_NOPAD;
		return TEE_SUCCESS;
	case TA_ALGO_AES_CBC:
		*algo = TEE_ALG_AES_CBC_NOPAD;
		return TEE_SUCCESS;
	case TA_ALGO_AES_CFB:
		*algo = TEE_ALG_AES_CFB;
		return TEE_SUCCESS;
	case TA_ALGO_AES_OFB:
		*algo = TEE_ALG_AES_OFB;
		return TEE_SUCCESS;
	case TA_ALGO_AES_CTR:
		*algo = TEE_ALG_AES_CTR;
		return TEE_SUCCESS;
	case TA_ALGO_AES_GCM:
...
/*
 * Process command TA_AES_CMD_PREPARE. API in nvt_crypto_ta.h
 *
					...
					TEE_Param params[4])
{
	...
	case TA_CRYPT_CMD_DIGEST_UPDATE:
		return ta_entry_digest_update(param_types, params);
	case TA_CRYPT_CMD_DIGEST_DO_FINAL:
		return ta_entry_digest_do_final(param_types, params);
	default:
