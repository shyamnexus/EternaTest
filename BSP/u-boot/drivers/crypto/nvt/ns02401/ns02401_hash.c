...
#define NVT_HASH_DMA_36BIT_SUPPORT      
...
static void nvt_hash_platform_init(void)
{
	...
	/* Hash clock select, 0:pll13(400MHz) 1:fix(320MHz) */
	tmp = IO_REG_RD(IOADDR_CG_REG_BASE, 0x20);
	tmp &= ~(0x3<<22);
	IO_REG_WR(IOADDR_CG_REG_BASE, 0x20, tmp);

	/* Hash master clock enable */
	tmp = IO_REG_RD(IOADDR_CG_REG_BASE, 0x74);
	tmp |= (0x1<<19);
	IO_REG_WR(IOADDR_CG_REG_BASE, 0x74, tmp);

	/* Hash program clock enable */
	tmp = IO_REG_RD(IOADDR_CG_REG_BASE, 0x150);
	tmp |= (0x1<<28);
	IO_REG_WR(IOADDR_CG_REG_BASE, 0x150, tmp);
...
static void nvt_hash_platform_exit(void)
{
	...
	/* Hash reset enable */
	tmp = IO_REG_RD(IOADDR_CG_REG_BASE, 0x98);
	tmp &= ~(0x1<<27);
	IO_REG_WR(IOADDR_CG_REG_BASE, 0x98, tmp);
