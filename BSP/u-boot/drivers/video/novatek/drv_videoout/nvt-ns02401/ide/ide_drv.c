...
irqreturn_t nvt_ide3_drv_isr(int irq, void *dev_id);
/*===========================================================================*/
/* Define                                                                    */
/*===========================================================================*/
...
#if (FPGA_3LCD == 1)
	/* register IRQ here*/
	if (request_irq(pmodule_info->iinterrupt_id[1], nvt_ide2_drv_isr, IRQF_TRIGGER_HIGH, "IDE2_INT", pmodule_info)) {
		nvt_dbg(ERR, "failed to register an IRQ Int:%d\n", pmodule_info->iinterrupt_id[1]);
		i_ret = -ENODEV;
		goto FAIL_FREE_IRQ;
	}
	/* register IRQ here*/
	if (request_irq(pmodule_info->iinterrupt_id[2], nvt_ide3_drv_isr, IRQF_TRIGGER_HIGH, "IDE3_INT", pmodule_info)) {
...
void nvt_ide_drv_do_tasklet(unsigned long data)
{
	ide_platform_ist(LCD_ID_1, data);
}
