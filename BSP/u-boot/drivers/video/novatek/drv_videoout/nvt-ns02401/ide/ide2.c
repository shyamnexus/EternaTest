...
void idec_set_rgbd_swap(IDE_ID id, BOOL b_en)
{
	...
	reg.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS);
	...
	idec_set_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS, reg.reg);
}
...
/**
    ...
    @param uidevice the display device for display
			...
			- @b DISPLAY_DEVICE_HDMI_24BIT:HDMI YUV420
			- @b DISPLAY_DEVICE_HDMI_24BIT:HDMI YUV444
			- @b DISPLAY_DEVICE_HDMI_16BIT:HDMI YUV422
...
void idec_set_device(IDE_ID id, IDE_DEVICE_TYPE uidevice)
{
	...
	ide_chk_range(uidevice, DISPLAY_DEVICE_CASIO2G, DISPLAY_DEVICE_RGB_16BIT);
...
/**
    ...
    @return void
*/
void idec_set_dram_out_format(IDE_ID id, UINT8 ui_fmt)
{
	idec_set_wb_fmt(id,ui_fmt);
}
/**
    Set ide pixel order.
...
/**
    Set ide DDR mode select.

    Enable/Disable ide Output bus DDR mode.

    @param[in] id   ide ID
    @param[in] b_en
		- @b TRUE:     Enable DDR mode
		- @b FALSE:    Disable DDR mode.
...
{
	idec_set_sw_reset(id, b_en);
}

/**
    Enable/Disable ide AXI.

    ide module AXI Enable/Disable.
...
/*	T_IDE_AXI_REG1 reg;
	...
	reg.reg = idec_get_reg(id, IDE_AXI_REG1_OFS);
	...
	idec_set_reg(id, IDE_AXI_REG1_OFS, reg.reg);*/
}

void idec_set_axi_wrp_outstanding(IDE_ID id, UINT32 wrp_os)
{
	idec_set_reg(id, 0x82c, wrp_os);
}
...
/**
    Get ide output to dram's format

    Get ide output to dram's format during dram out path

    @param[in]  id          ide ID
    @return is_yuv422   yuv422 or 420
		- @b FALSE  : 420
		- @b TRUE   : 422
*/
...
UINT32 idec_get_window_en(IDE_ID id)
{
	...
	if (idec_get_ch_en(id, LCD_CH_1)) {
		ui_win_en |= DISPLAY_OSD1_EN;
	}
	if (idec_get_ch_en(id, LCD_CH_0)) {
		ui_win_en |= DISPLAY_VIDEO1_EN;
	}
	if (idec_get_ch_en(id, LCD_CH_2)) {
...
{
	idec_set_ch_en(id, LCD_CH_0, 0);
	idec_set_ch_en(id, LCD_CH_1, 0);
	idec_set_ch_en(id, LCD_CH_2, 0);
}
...
{
	idec_set_all_window_dis(id);
	if (ui_wins & DISPLAY_OSD1_EN) {
		idec_set_ch_en(id, LCD_CH_1, 1);
	}
	if (ui_wins & DISPLAY_VIDEO1_EN) {
		idec_set_ch_en(id, LCD_CH_0, 1);
	}
	if (ui_wins & DISPLAY_VIDEO2_EN) {
		idec_set_ch_en(id, LCD_CH_2, 1);
	}
...
{
	UINT32 reg_offset;
	T_LCD_CURSOR_POS_REG reg;

	reg_offset = LCD_CURSOR_POS_REG_OFS;
	reg.reg = idec_get_reg(id, reg_offset);
	reg.bit.cursor_en = ui_en;	
...
{
	T_LCD_P2I_CTRL_REG reg;

	reg.reg = idec_get_reg(id, LCD_P2I_CTRL_REG_OFS);

	reg.bit.vpi_en = ui_en;

	idec_set_reg(id, LCD_P2I_CTRL_REG_OFS, reg.reg);
}
...
void idec_set_vpi_coef(IDE_ID id, UINT8 ui_coef0, UINT8 ui_coef1)
{
	...
	reg.bit.pif_coef0 = ui_coef0;
	reg.bit.pif_coef1 = ui_coef1;
...
BOOL idec_get_hwcur_en(IDE_ID id)
{
	...
	reg.reg = idec_get_reg(id, LCD_CURSOR_POS_REG_OFS);

	return reg.bit.cursor_en;
}


/**
    Set HW cursor read/write access
...
/**
    Get HW cursor read/write access
...
void idec_set_hwcur_pos(IDE_ID id, UINT16 ui_x, UINT16 ui_y)
{
	UINT32 reg_offset;
	T_LCD_CURSOR_POS_REG reg;
	...
	reg.bit.cursor_sx = ui_x & 0xFFF;
	reg.bit.cursor_sy = ui_y & 0xFFF;
...
{
	UINT32 reg_offset;
	T_LCD_CURSOR_BUFFER_DIM_REG reg;

	reg_offset = LCD_CURSOR_BUFFER_DIM_REG_OFS;
	reg.reg = idec_get_reg(id, reg_offset);

	reg.bit.cursor_bw = ui_bw;
	reg.bit.cursor_bh = ui_bh;
...
void idec_get_hwcur_buf_dim(IDE_ID id, UINT8 *ui_bw, UINT8 *ui_bh)
{
	...
	*ui_bw = reg.bit.cursor_bw;
	*ui_bh = reg.bit.cursor_bh;
}

void idec_get_hwcur_pos(IDE_ID id, UINT16 *ui_x, UINT16 *ui_y)
{
	T_LCD_CURSOR_POS_REG reg;

	reg.reg = idec_get_reg(id, LCD_CURSOR_POS_REG_OFS);

	*ui_x = reg.bit.cursor_sx;
	*ui_y = reg.bit.cursor_sy;
...
{
	UINT32 reg_offset;
	T_LCD_CURSOR_ADDR_REG reg;
	T_LCD_CURSOR_HADDR_REG reg2;

	reg_offset = LCD_CURSOR_ADDR_REG_OFS;
	reg.reg = idec_get_reg(id, reg_offset);
	reg.bit.cursor_ba = (UINT32)ide_platform_va2pa(id, ui_addr);
	idec_set_reg(id, reg_offset, reg.reg);

	reg_offset = LCD_CURSOR_HADDR_REG_OFS;
	reg2.reg = idec_get_reg(id, reg_offset);
	reg2.bit.cursor_hba = (UINT32)(ide_platform_va2pa(id,ui_addr)>>32);
...
void idec_get_hwcur_addr(IDE_ID id, IDEDATA_TYPE *ui_addr)
{
	...
	reg.reg = idec_get_reg(id, LCD_CURSOR_ADDR_REG_OFS);

	*ui_addr = reg.bit.cursor_ba;
...
{
	UINT32 reg_offset;
	T_LCD_CURSOR_HADDR_REG reg;

	reg_offset = LCD_CURSOR_HADDR_REG_OFS;
	reg.reg = idec_get_reg(id, reg_offset);

	*ui_addr = reg.bit.cursor_hba;
}
...
void idec_get_vpi_coef(IDE_ID id, UINT8 *ui_coef0, UINT8 *ui_coef1)
{
	...
	*ui_coef0 = reg.bit.pif_coef0;
	*ui_coef1 = reg.bit.pif_coef1;
}
...
{
	UINT32 reg_offset;
	T_LCD_PAL_RAM_REG reg;

	reg_offset = LCD_PAL_RAM_REG_OFS+ui_entry*4;
	reg.reg = idec_get_reg(id, reg_offset);
	reg.bit.palette_entry0 = ui_palette & 0xFFFF;
	reg.bit.palette_entry1 = (ui_palette>>16) & 0xFFFF;
	//printk("idec_set_pal: ofs = 0x%x val = 0x%08x\r\n",(unsigned int)reg_offset,(unsigned int)reg.reg);
	idec_set_reg(id, reg_offset, reg.reg);
}
...
/**
    ...
    @param[in] id   ide ID
    @param[in] ui_entry      OSD palette entry(0~14)
...
void idec_get_pal(IDE_ID id, UINT32 ui_entry, UINT32 *ui_palette)
{
	...
	reg_offset = LCD_PAL_RAM_REG_OFS+ui_entry*4;
	reg.reg = idec_get_reg(id, reg_offset);
	*ui_palette = reg.bit.palette_entry0 | (reg.bit.palette_entry1 << 16);

}

void idec_get_pal_shadow(IDE_ID id, UINT32 ui_entry, UINT32 *ui_palette)
{

}

/**
    Set the color of the specific entry of HW cursor.

    @param[in] id   ide ID
    @param[in] ui_entry      HW cursor color entry(0~14)
    @param[in] ui_palette    The palette content.
		 - ui_palette[7~0]:      Y value of the color.
		 - ui_palette[15~8]:     Cb value of the color.
		 - ui_palette[23~16]:    Cr value of the color.
...
/**
    Get the palette of the specific entry of HW cursor.

    @param[in] id   ide ID
    @param[in] ui_entry      HW cursor color entry(0~511)
    @param[in] ui_palette    The palette content.
		 - ui_palette[7~0]:      Y value of the color.
		 - ui_palette[15~8]:     Cb value of the color.
		 - ui_palette[23~16]:    Cr value of the color.
...
/**
    Set the color and blending operation of the specific entry of HW cursor font 64x64.

    @param[in] id   ide ID
    @param[in] ui_entry      HW cursor entry(0~511)
    @param[in] ui_font       HW cursor font value.
...
/**
    Get the color and blending operation of the specific entry of HW cursor font 64x64.
...
{
	T_LCD_BG_PARM00_REG reg;
	reg.reg = idec_get_reg(id, LCD_BG_PARM00_REG_OFS);
	reg.bit.bg_hs = ui_bg_hs;
	reg.bit.bg_vs = ui_bg_vs;
	idec_set_reg(id, LCD_BG_PARM00_REG_OFS, reg.reg);
}
...
void idec_get_background_res(IDE_ID id, UINT32 *ui_bg_hs, UINT32 *ui_bg_vs)
{
	...
	*ui_bg_hs = reg.bit.bg_hs;
	*ui_bg_vs = reg.bit.bg_vs;
}
...
{
	T_LCD_BG_PARM01_REG reg;
	T_LCD_BG_PARM02_REG reg2;
	reg.reg = idec_get_reg(id, LCD_BG_PARM01_REG_OFS);
	reg.bit.bg_y = ui_color_y;
	idec_set_reg(id, LCD_BG_PARM01_REG_OFS, reg.reg);

	reg2.reg = idec_get_reg(id, LCD_BG_PARM02_REG_OFS);
	reg2.bit.bg_cb = ui_color_cb;
	reg2.bit.bg_cr = ui_color_cr;
	idec_set_reg(id, LCD_BG_PARM02_REG_OFS, reg2.reg);
...
void idec_get_background(IDE_ID id, UINT8 *ui_color_y, UINT8 *ui_color_cb, UINT8 *ui_color_cr)
{
	...
	reg.reg = idec_get_reg(id, LCD_BG_PARM01_REG_OFS);
	reg2.reg = idec_get_reg(id, LCD_BG_PARM02_REG_OFS);
	*ui_color_y = reg.bit.bg_y ;
	*ui_color_cb = reg2.bit.bg_cb;
	*ui_color_cr = reg2.bit.bg_cr;
}
...
{
	T_LCD_SUB_PIXEL_REG  reg;


	reg.reg = idec_get_reg(id, LCD_SUB_PIXEL_REG_OFS);
	if (b_odd_line) {
		reg.bit.odd_r = b_r;
		reg.bit.odd_g = b_g;
		reg.bit.odd_b = b_b;
	...
	idec_set_reg(id, LCD_SUB_PIXEL_REG_OFS, reg.reg);
}
...
void idec_set_update(IDE_ID id,BOOL b_en)
{
	...
	reg1.reg = idec_get_reg(id, LCD_FUNC_REG_OFS);
	reg1.bit.update = b_en;
	idec_set_reg(id, LCD_FUNC_REG_OFS, reg1.reg);
}

void idec_set_vtc_en(IDE_ID id,BOOL b_en)
{
	T_LCD_TIMING_CTRL_PRM0_REG reg1;

	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS);
	reg1.bit.vtc_en = b_en;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS, reg1.reg);
}
...
void idec_set_csc3_en(IDE_ID id, BOOL b_en)
{
	...
	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS);
	reg1.bit.csc3_en = b_en;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS, reg1.reg);
}


void idec_set_rgb_gamma_en(IDE_ID id, BOOL b_en)
{
	T_LCD_TIMING_CTRL_PRM0_REG reg1;

	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS);
	reg1.bit.rgb_gamma_en = b_en;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM0_REG_OFS, reg1.reg);
}
...
{
	T_LCD_TIMING_CTRL_PRM2_REG reg1;
	T_LCD_TIMING_CTRL_PRM3_REG reg2;
	T_LCD_FRM_RESET_REG 	   reg3;


	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM2_REG_OFS);
	reg1.bit.hsynct = ui_hsynct;
	reg1.bit.htotal = ui_htotal;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM2_REG_OFS, reg1.reg);

	reg2.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM3_REG_OFS);
	reg2.bit.hvldst = ui_hvalidst;
	reg2.bit.hvlded = ui_hvalided;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM3_REG_OFS, reg2.reg);

	
	reg3.reg = idec_get_reg(id, LCD_FRM_RESET_REG_OFS);
	reg3.bit.frst_cnt = (ui_htotal*4)&0xffff;
	idec_set_reg(id, LCD_FRM_RESET_REG_OFS, reg3.reg);	
...
{
	T_LCD_TIMING_CTRL_PRM4_REG reg4;
	T_LCD_TIMING_CTRL_PRM6_REG reg6;
	T_LCD_TIMING_CTRL_PRM5_REG reg5;

	reg4.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS);
	reg4.bit.vsynct = ui_vsynct;
	reg4.bit.vtotal = ui_vtotal;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS, reg4.reg);

	reg5.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM5_REG_OFS);
	reg5.bit.oddvvldst = ui_odd_vvalidst;
	reg5.bit.oddvvlded = ui_odd_vvalided;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM5_REG_OFS, reg5.reg);

	reg6.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM6_REG_OFS);
	reg6.bit.evenvvldst = ui_even_vvalidst;
	reg6.bit.evenvvlded = ui_even_vvalided;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM6_REG_OFS, reg6.reg);
}
...
void idec_get_ver_timing(IDE_ID id, UINT32 *pui_vsynct, UINT32 *pui_vtotal, UINT32 *pui_odd_vvalidst, UINT32 *pui_odd_vvalided, UINT32 *pui_even_vvalidst, UINT32 *pui_even_vvalided)
{
	...
	reg4.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS);
	*pui_vsynct = reg4.bit.vsynct;
	*pui_vtotal = reg4.bit.vtotal;

	reg5.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM5_REG_OFS);
	*pui_odd_vvalidst = reg5.bit.oddvvldst;
	*pui_odd_vvalided = reg5.bit.oddvvlded;

	reg6.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM6_REG_OFS);
	*pui_even_vvalidst = reg6.bit.evenvvldst;
	*pui_even_vvalided = reg6.bit.evenvvlded;
}
...
{
	T_LCD_TIMING_CTRL_PRM5_REG reg4;

	reg4.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM5_REG_OFS);

	reg4.bit.inter_load_mode = b2field;

	idec_set_reg(id, LCD_TIMING_CTRL_PRM5_REG_OFS, reg4.reg);
}
...
{
	T_LCD_TIMING_CTRL_PRM2_REG reg1;
	T_LCD_TIMING_CTRL_PRM4_REG reg3;

	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM2_REG_OFS);
	reg1.bit.hsdelay = ui_hs_delay;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM2_REG_OFS, reg1.reg);

	reg3.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS);
	reg3.bit.vsdelay = ui_vs_delay;
	idec_set_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS, reg3.reg);
}
...
void idec_get_sync_delay(IDE_ID id, UINT8 *ui_hs_delay, UINT8 *ui_vs_delay)
{
	...
	reg1.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM2_REG_OFS);
	*ui_hs_delay = reg1.bit.hsdelay;


	reg3.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM4_REG_OFS);
	*ui_vs_delay = reg3.bit.vsdelay;
}
...
{
	T_LCD_TIMING_CTRL_PRM6_REG reg;

	reg.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM6_REG_OFS);
	if (b_inter) {
		reg.bit.display_interlace = 1;
	} else {
		reg.bit.display_interlace = 0;
	}
	idec_set_reg(id, LCD_TIMING_CTRL_PRM6_REG_OFS, reg.reg);
}
...
BOOL idec_get_interlace(IDE_ID id)
{
	...
	return reg.bit.display_interlace;
}
...
{
	T_LCD_DIGI_TIMING_PRM0_REG reg1;
	T_LCD_DIGI_TIMING_PRM1_REG reg2;
	T_LCD_DIGI_TIMING_PRM2_REG reg3;

	reg1.reg = idec_get_reg(id, LCD_DIGI_TIMING_PRM0_REG_OFS);
	reg2.reg = idec_get_reg(id, LCD_DIGI_TIMING_PRM1_REG_OFS);
	reg3.reg = idec_get_reg(id, LCD_DIGI_TIMING_PRM2_REG_OFS);
	...
	idec_set_reg(id, LCD_DIGI_TIMING_PRM0_REG_OFS, reg1.reg);
	idec_set_reg(id, LCD_DIGI_TIMING_PRM1_REG_OFS, reg2.reg);
	idec_set_reg(id, LCD_DIGI_TIMING_PRM2_REG_OFS, reg3.reg);
}
...
void idec_get_digital_timing(IDE_ID id, UINT32 *ui_codd_blk_st, UINT32 *ui_codd_blk_ed, UINT32 *ui_ceven_blk_st, UINT32 *ui_ceven_blk_ed, UINT32 *ui_cfid_st, UINT32 *ui_cfid_ed)
{
	...
	*ui_codd_blk_st = reg1.bit.coddblkst;
	*ui_codd_blk_ed = reg1.bit.coddblked;
	*ui_ceven_blk_st = reg2.bit.cevenblkst;
	*ui_ceven_blk_ed = reg2.bit.cevenblked;
	*ui_cfid_st = reg3.bit.cfidst;
	*ui_cfid_ed = reg3.bit.cfided;
}
...
void idec_set_gamma_en(IDE_ID id, BOOL b_en)
{
	...
	if (b_en) {
		reg.bit.rgb_gamma_en = 1;
	} else {
		reg.bit.rgb_gamma_en = 0;
	}
...
void idec_fill_gamma(IDE_ID id, UINT8 *uipgamma)
{
	/*    UINT32 i;
	    ...
	    for(i=0; i<=LCD_GAMMA_LEVEL; i+=4)
	    {
			reg.reg = 0;
			reg.bit.gamma0 = uipgamma[i];
			if(i < LCD_GAMMA_LEVEL)
			{
...
void idec_fill_rgb_gamma(IDE_ID id, UINT8 *uiprgamma, UINT8 *uipggamma, UINT8 *uipbgamma)
{
	...
	for (i = 0; i <= LCD_GAMMA_LEVEL; i += 4) {
		...
		if (uiprgamma != NULL) {
			...
			idec_set_reg(id, LCD_R_GAMMA_PRM0_REG_OFS + i, reg.reg);
		}
		if (uipggamma != NULL) {
			reg.reg = 0;
			...
			idec_set_reg(id, LCD_G_GAMMA_PRM0_REG_OFS + i, reg.reg);
		}
		if (uipbgamma != NULL) {
			reg.reg = 0;
			...
			idec_set_reg(id, LCD_B_GAMMA_PRM0_REG_OFS + i, reg.reg);
		}
...
void idec_get_rgb_gamma(IDE_ID id, UINT8 *uiprgamma, UINT8 *uipggamma, UINT8 *uipbgamma)
{
	...
	for (i = 0; i <= LCD_GAMMA_LEVEL; i += 4) {
		if (uiprgamma != NULL) {
			...
			reg.reg = idec_get_reg(id, LCD_R_GAMMA_PRM0_REG_OFS + i);
		...
		if (uipggamma != NULL) {
			...
			reg.reg = idec_get_reg(id, LCD_G_GAMMA_PRM0_REG_OFS + i);
		...
		if (uipbgamma != NULL) {
			...
			reg.reg = idec_get_reg(id, LCD_B_GAMMA_PRM0_REG_OFS + i);
...
{
	/*    T_LCD_TIMING_CTRL_PRM1_REG reg;

	    reg.reg = idec_get_reg(id, LCD_TIMING_CTRL_PRM1_REG_OFS);
	    reg.bit.ctrst = ui_ctrst;
	    idec_set_reg(id, LCD_TIMING_CTRL_PRM1_REG_OFS, reg.reg);*/
}
...
{
	/*    T_LCD_TIMING_CTRL_PRM0_REG2 reg;
...
{
	T_LCD_INT_STS_REG reg;

	reg.reg = idec_get_reg(id, LCD_INT_STS_REG_OFS);

	return reg.reg & LCD_INTEN_MSK;
}
...
{
	T_LCD_INT_CLR_REG reg;

	reg.reg = idec_get_reg(id, LCD_INT_CLR_REG_OFS);

	reg.reg &= ~LCD_INTEN_MSK;

	reg.reg |= ui_int_status & LCD_INTEN_MSK;

	idec_set_reg(id, LCD_INT_CLR_REG_OFS, reg.reg);
}
...
{
/*
	T_LCD_INT_EN_REG reg;

	reg.reg = idec_get_reg(id, LCD_INT_EN_REG_OFS);

	reg.reg &= LCD_INTEN_MSK;

	//set irqen and don't clear irqsts
	reg.reg |= (ui_int_en & LCD_INTEN_MSK);
	idec_set_reg(id, LCD_INT_EN_REG_OFS, reg.reg);
	*/
}
...
void idec_set_csc_coef(IDE_ID id, CSC_ID cscid, UINT16 *ui_pcoef)
{
	idec_set_csc_clip(id, cscid, ui_pcoef[0], ui_pcoef[1]);
	idec_set_csc_in_offset(id, cscid, ui_pcoef[2], ui_pcoef[3], ui_pcoef[4]);
	idec_set_csc_out_offset(id, cscid, ui_pcoef[5], ui_pcoef[6], ui_pcoef[7]);
	idec_set_csc_coef_ctrl(id, cscid, ui_pcoef[8], ui_pcoef[9], ui_pcoef[10], ui_pcoef[11]);
	idec_set_csc_coef_ctrl2(id, cscid,ui_pcoef[12], ui_pcoef[13], ui_pcoef[14], ui_pcoef[15]);
	idec_set_csc_coef_ctrl3(id, cscid,ui_pcoef[16]);
}
...
/**
    ...
    @return void
*/
void idec_set_icst_coef(IDE_ID id, UINT16 *ui_pcoef)
{
	idec_set_csc_coef(id, LCD_CSC_CSC1, ui_pcoef);
}
...
void idec_set_fd_all_en(IDE_ID id, UINT32 ui_en)
{
	...
	reg.reg = idec_get_reg(id, LCD_FD_EN_REG_OFS);

	reg.reg |= ui_en;

	idec_set_reg(id, LCD_FD_EN_REG_OFS, reg.reg);
}
/**
    Set ide FD disable
...
void idec_set_fd_win_pos(IDE_ID id, IDE_FD_NUM ui_num, UINT32 ui_x, UINT32 ui_y)
{
	...
	reg.reg = idec_get_reg(id, LCD_FD0_ATTR0_REG_OFS + i);

	reg.bit.fd_x = ui_x;
	reg.bit.fd_y = ui_y;

	idec_set_reg(id, LCD_FD0_ATTR0_REG_OFS + i, reg.reg);
}
...
void idec_set_fd_win_dim(IDE_ID id, IDE_FD_NUM ui_num, UINT32 ui_win_w, UINT32 ui_win_h)
{
	...
	i = (idec_chkfd_num(id, ui_num) * 3) * 4;
	reg.reg = idec_get_reg(id, LCD_FD0_ATTR1_REG_OFS + i);

	reg.bit.fd_w = ui_win_w;
	reg.bit.fd_h = ui_win_h;

	idec_set_reg(id, LCD_FD0_ATTR1_REG_OFS + i, reg.reg);
}
...
void idec_get_fd_win_dim(IDE_ID id, IDE_FD_NUM ui_num, UINT32 *ui_win_w, UINT32 *ui_win_h)
{
	...
	*ui_win_w = reg.bit.fd_w;
	*ui_win_h = reg.bit.fd_h;
}
...
void idec_set_fd_win_bord(IDE_ID id, IDE_FD_NUM ui_num, UINT32 ui_bor_h, UINT32 ui_bor_v)
{
	...
	reg1.reg = idec_get_reg(id, LCD_FD0_ATTR1_REG_OFS + i);

	reg1.bit.border_v = ui_bor_v;

	idec_set_reg(id, LCD_FD0_ATTR1_REG_OFS + i, reg1.reg);
}
...
void idec_set_fd_color(IDE_ID id, IDE_FD_NUM ui_num, UINT8 ui_y, UINT8 ui_cb, UINT8 ui_cr)
{
	...
	reg.reg = idec_get_reg(id, LCD_FD0_ATTR2_REG_OFS + i);

	reg.bit.fd_y = ui_y;
	reg.bit.fd_cb = ui_cb;
	reg.bit.fd_cr = ui_cr;

	idec_set_reg(id, LCD_FD0_ATTR2_REG_OFS + i, reg.reg);
}
...
{
	T_LCD_LINE_MON_CTL_REG reg;

	reg.reg = idec_get_reg(id, LCD_LINE_MON_CTL1_REG_OFS);
	reg.bit.mon_line = ui_idx;
	idec_set_reg(id, LCD_LINE_MON_CTL1_REG_OFS, reg.reg);
}
...
void idec_set_line_mon2(IDE_ID id, UINT32 ui_idx)
{
	...
	reg.reg = idec_get_reg(id, LCD_LINE_MON_CTL2_REG_OFS);
	reg.bit.mon_line = ui_idx;
	idec_set_reg(id, LCD_LINE_MON_CTL2_REG_OFS, reg.reg);

}

void idec_get_frame_cnt(IDE_ID id, UINT32 *ui_frame_cnt)
{
	T_LCD_IMG_CTRL_REG reg;

	reg.reg = idec_get_reg(id, LCD_IMG_CTRL_REG_OFS);
	*ui_frame_cnt = reg.bit.frame_cnt;
}
...
BOOL idec_compare_reg_default_value(void)
{
	...
	for (ui_index = 0; ui_index < (sizeof(ide_reg_default) / sizeof(IDE_REG_DEFAULT)); ui_index++) {
		if (INW(IOADDR_LCD_REG_BASE + ide_reg_default[ui_index].ui_offset) != ide_reg_default[ui_index].ui_value) {
			DBG_ERR("Register (0x%.4X) default value 0x%.8X isn't 0x%.8X\r\n",
					(unsigned int)ide_reg_default[ui_index].ui_offset,
					(unsigned int)INW(IOADDR_LCD_REG_BASE + ide_reg_default[ui_index].ui_offset),
