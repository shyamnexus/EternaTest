...
UINT32              hdmi_ctrl_option[HDMICFG_OPTION_ALL] = {TRUE,  TRUE, FALSE, FALSE, FALSE};
#else
// FORCE_RGB / FORCE_UNDERSCAN / VERIFY_VID /FORCE_YUV / TIMING_FACTOR_EN
UINT32              hdmi_ctrl_option[HDMICFG_OPTION_ALL] = {TRUE, TRUE, FALSE, FALSE, FALSE};
#endif
...
//VK_TASK_HANDLE hdmi_tsk_hdl = NULL;
//static SEM_HANDLE SEMID_HDMI;
//static SEM_HANDLE SEMID_HDMI_HPD;
...
static hdmi_state_t hdmi_state = STATE_NONE;
static int hdmi_hpd_status;
...
#if 0
THREAD_RETTYPE hdmi_polling_tsk(void *param)
{
	int status;
	...
	do {		
		vos_sem_wait(SEMID_HDMI_HPD);

		if (hdmi_state & HDMI_THREAD_STOP){
			hdmi_state &= ~HDMI_THREAD_STOP;
			THREAD_RETURN(0);
		}
	
		/* check hotplug */
		status = hdmitx_check_hotplug();
			
		if (hdmi_state & HDMI_OPEN) {
			if (status != hdmi_hpd_status) {
				...
				hdmi_state |= HPD_CHANGE;
				hdmi_hpd_status = status;
			}
		}
loop:		
		vos_sem_sig(SEMID_HDMI_HPD);
		DELAY_MS(5);
#ifdef __KERNEL__
...
{
	/*
	vos_flag_create(&FLG_ID_HDMI, NULL, "FLG_ID_HDMI");
	vos_sem_create(&SEMID_HDMI, 1, "SEMID_HDMI");
	vos_sem_create(&SEMID_HDMI_HPD, 1, "SEMID_HDMI_HPD");
	hdmi_tsk_hdl = vos_task_create(hdmi_polling_tsk, NULL, "hdmi_polling_tsk", 10, 4096);
	vos_task_resume(hdmi_tsk_hdl);
	*/
}
...
{
	/*
	vos_flag_destroy(FLG_ID_HDMI);
	vos_sem_destroy(SEMID_HDMI);
	vos_sem_destroy(SEMID_HDMI_HPD);
	vos_task_destroy(hdmi_tsk_hdl);
	*/
}
...
//void (void)
irqreturn_t hdmi_isr(int irq, void *devid)
{
	T_HDMI_INTSTS_REG    reg_int_sts;
...
static void hdmitx_validate_rtrim(void)
{
	...
	if (code == -33) {
	    ...
	    printf("[%d] data = NULL\n", EFUSE_TRIM_DATA_HDMI_H_PHY_RESISTOR_PRI);
	    return ;
	} else {
	...
	if (clock_rate >  297000000){//HDMI2.0
		...
		if (code == -33) {
		    printf("[%d] data = NULL\n", EFUSE_TRIM_DATA_HDMI_2_0_H_PHY_VL_TRIM_PRI);
		    return ;
		} else {
		    is_found = extract_trim_valid(code, (u32 *)&trim);
		    if (is_found) {
		...
		if (code == -33) {
		    printf("[%d] data = NULL\n", EFUSE_TRIM_DATA_HDMI_1_4_H_PHY_VL_TRIM_PRI);
		    return ;
		} else {
		    is_found = extract_trim_valid(code, (u32 *)&trim);
		    if (is_found) {
...
static void hdmitx_init_phy(void)
{
	...
	if (clock_rate < 50000000) {
		...
		hdmitx_write_phy(0x9C, 0x12);
		hdmitx_write_phy(0x7B, 0xFF);
		hdmitx_write_phy(0x9A, 0x15);
		hdmitx_write_phy(0x7A, 0x0F);
		hdmitx_write_phy(0x7A, 0xFF);	
	}
	DBG_WRN("=================================\n");
	DBG_WRN("clock_rate  = [%d Hz]\n", clock_rate);
...
// 530 new power down flow todo.
	...
	if (b_pd) {
		...
		hdmitx_read_phy(0x08,&value);
		hdmitx_write_phy(0x08, value&0xFFFFFFF7);

		hdmitx_read_phy(0x05,&value);
		hdmitx_write_phy(0x05, value&0xFFFFFFFE);

		hdmitx_read_phy(0x04,&value);
		hdmitx_write_phy(0x04, value&0xFFFFFFFE);		

		hdmitx_read_phy(0x09,&value);
		hdmitx_write_phy(0x09, value&0xFFFFFFFC);
	} else {
		hdmitx_write_phy(0x09, 0x87);
	}
#endif
...
void _hdmitx_enableclk_platform(int id){
	CG_SETREG(0x8C,CG_GETREG(0x8C)|0x20000000);
}

void _hdmitx_disableclk_platform(int id){
	CG_SETREG(0x8C,CG_GETREG(0x8C)&0xDFFFFFFF);
}
...
*/
static void hdmitx_hpd_pad_control(void)
{
 	PAD_SETREG(0x10,PAD_GETREG(0x10)&0xFFFFFF3F);
	DELAY_MS(5);
}
...
static UINT32 hdmitx_config_video(HDMI_VIDEOID vid_code)
{
	...
	case HDMI_4096X2160P60: {       /* VID102*/
			...
			if (hdmi_ctrl_option[HDMICFG_OPTION_FORCE_YUV] & 0x1) {
				...
				mode_fmt = (HDMI_INTF_YCBCR422 << 5);
			} else if (hdmi_ctrl_option[HDMICFG_OPTION_FORCE_YUV] & 0x4) {
...
static void hdmitx_scdc_dump(void)
{
	...
	for (i = 0; i < 256; i++) {
		...
		if(hdmitx_scdc_read(i, &data) == E_OK){
...
ER hdmitx_open(void)
{
	...
	DBG_WRN("[uboot]hdmitx_open: version [%s]\r\n", HDMI_VERSION);
#endif	
	...
	if (!hdmi_init) {
		hdmi_init = TRUE;
		hdmitx_hpd_pad_control();
		hdmi_hpd_status = hdmitx_check_hotplug();
#if defined(__FREERTOS)
	...
	//pll_enable_apb_clock(HDMI_PCLK);
   // pll_disable_apb_reset(HDMI_PRSTN);
#endif  

#if defined(__FREERTOS)
	// clear the interrupt flag
	//clr_flg(FLG_ID_HDMI, FLGPTN_HDMI);
	//request_irq(INT_ID_HDMI, hdmi_isr, IRQF_TRIGGER_HIGH, "hdmi", 0);//INT_ID_HDMI//94forHDMI2
#endif 
...
void hdmitx_set_config(HDMI_CONFIG_ID config_id, UINT32 ui_config)
{
	...
	case HDMI_CONFIG_ID_FORCE_YUV420: {
			if(ui_config > 0)
				hdmi_ctrl_option[HDMICFG_OPTION_FORCE_YUV] |= 0x4;
			else
				hdmi_ctrl_option[HDMICFG_OPTION_FORCE_YUV] &= ~0x4;
		}
		return;
	...
	case HDMI_CONFIG_ID_TIMING_FACTOR_EN: {
			if(ui_config > 0)
				hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] = 0x1;
			else
				hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] = 0x0;		
		}
		return;		
			...
			if (ui_config > 0) {
				...
				if (hdmitx_get_hdmi_2_vsdb_cap() & HDMI2VSDB_SCDC_PRESENT) {
					...
					if(clock_rate > 340000000){ //when TMDS > 340Mhz need 1:40 rate and scramble on
						...
						do {
							DELAY_MS(5);
							if (hdmitx_scdc_read(0x21, &data) != E_OK) {
					...
					do {
						DELAY_MS(5);
						if (hdmitx_scdc_read(0x40, &data) != E_OK) {
...
*/
	...
	case HDMI_CONFIG_ID_SCRAMBLE_EN: {
			...
			if (hdmitx_scdc_read(0x20,  &value2) != E_OK){
...
UINT32 hdmitx_get_config(HDMI_CONFIG_ID config_id)
{
	...
	case HDMI_CONFIG_ID_FORCE_YUV422: {
		...
		break;
	case HDMI_CONFIG_ID_FORCE_YUV420: {
			ret = (hdmi_ctrl_option[HDMICFG_OPTION_FORCE_YUV] & 0x4) > 0;
		}
		break;		
...
void hdmitx_hotplug_off(void)
{
	hdmitx_set_config(HDMI_CONFIG_ID_OE, FALSE);

	hdmitx_set_config(HDMI_CONFIG_ID_SCDC_ENABLE,    FALSE);
...
*/
void hdmitx_set_pg_pattern(UINT32 ui_pat_id, UINT32 pxl_rpt_cnt)
{
	T_HDMI_PG0_REG  reg_pg0;
...
#if 0//def __KERNEL__
EXPORT_SYMBOL(hdmitx_open);
EXPORT_SYMBOL(hdmitx_is_opened);
EXPORT_SYMBOL(hdmitx_close);
EXPORT_SYMBOL(hdmitx_set_config);
EXPORT_SYMBOL(hdmitx_get_config);
EXPORT_SYMBOL(hdmitx_check_video_stable);
EXPORT_SYMBOL(hdmitx_check_hotplug);
#endif

//VOS_MODULE_VERSION(hdmitx, 1, 00, 005, 00);
