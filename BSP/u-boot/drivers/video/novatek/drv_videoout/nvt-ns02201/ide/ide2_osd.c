...
{
	idec_set_ch_en(id, LCD_CH_1, b_en);
}
...
BOOL idec_get_o1_en(IDE_ID id)
{
	...
	return idec_get_ch_en(id, LCD_CH_1);
}
...
{
	idec_set_ch_yaddr(id, LCD_CH_1, ui_addr);
}
...
{
	idec_set_ch_uvaddr(id, LCD_CH_1, ui_addr);
}
...
{
	idec_set_ch_dim(id, LCD_CH_1,ui_bw, ui_bh, ui_lineoffset, ui_lineoffset);
}
...
{
	idec_set_ch_flip(id, LCD_CH_1, b_l2r, b_t2b);
}
...
{
	idec_get_ch_yaddr(id, LCD_CH_1, ui_addr);
}
...
{
	idec_get_ch_uvaddr(id, LCD_CH_1, ui_addr);
}
...
{
	idec_get_ch_dim(id, LCD_CH_1,pui_bw, pui_bh, pui_lineoffset);
}
...
{
	idec_get_ch_flip(id, LCD_CH_1, pb_l2r, pb_t2b);
}
...
{
	idec_set_ch_sdi_sc_dim1(id,LCD_CH_1,ui_win_w,ui_win_h);
}
...
{
	idec_set_mixer_ctrl1(id,1,ui_x,ui_y);
}
...
void idec_set_o1_fmt(IDE_ID id, IDE_COLOR_FORMAT ui_fmt)
{
	...
	idec_set_ch_fmt(id, LCD_CH_1, ui_fmt);

}

/**
    Set ide OSD1 RLD enable/disable.

    @param[in] id   ide ID
    @param[in] b_en enable/disable RLD for the specific window
		- @b TRUE:Enable
		- @b FALSE:Disable.
...
{
	idec_get_ch_sc_dim(id, LCD_CH_1,ui_win_w,ui_win_h);
}
...
{
	idec_get_ch_fmt(id, LCD_CH_1, ui_fmt);
}

/**
    Get ide OSD1 Horizontal scaling method
...
/**
    Get ide OSD1 Vertical scaling method
...
/**
    Get ide OSD1 RLD enable/disable.
...
{
	CH_MODE mode;
	idec_get_ch_mode(id, LCD_CH_1, &mode);
...
{
	idec_get_ch_rld_ctrl(id, LCD_CH_1, ui_cnt, ui_len);
}

void idec_get_o1_rld_readsize(IDE_ID id, UINT32 *ui_size)
{
	idec_get_ch_rld_readsize(id, LCD_CH_1,ui_size);
}

void idec_get_o1_rld_alpha_readsize(IDE_ID id, UINT32 *ui_size)
{
	idec_get_ch_rld_alpha_readsize(id, LCD_CH_1,ui_size);
}
#if 0
/**
...
{
	idec_set_ch_colorkey_en(id, LCD_CH_1, b_en);
}
/**
...
{
	*b_en = idec_get_ch_colorkey_en(id, LCD_CH_1);
}
/**
...
/**
    ...
    @return void
*/
void idec_set_osd1_colorkey(IDE_ID id, UINT32 ui_key_low, UINT32 ui_key_high)
{
	idec_set_ch_colorkey(id, LCD_CH_1, ui_key_low, ui_key_high);
}
/**
    ...
    @return void
*/
void idec_get_osd1_colorkey(IDE_ID id, UINT32 *ui_key_low, UINT32 *ui_key_high)
{
	idec_get_ch_colorkey(id, LCD_CH_1, ui_key_low, ui_key_high);
}
