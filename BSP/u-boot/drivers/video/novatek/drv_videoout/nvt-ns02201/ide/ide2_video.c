...
{
	idec_set_ch_en(id, LCD_CH_0, b_en);
}
...
{
	idec_set_ch_en(id, LCD_CH_2, b_en);
}
...
{
	return idec_get_ch_en(id, LCD_CH_0);
}
...
{
	return idec_get_ch_en(id, LCD_CH_2);
}
...
{
	idec_set_ch_yaddr(id, LCD_CH_0, ui_y_addr);
	idec_set_ch_uvaddr(id, LCD_CH_0, ui_cb_addr);
}
...
{
	idec_set_ch_dim(id, LCD_CH_0,ui_bw, ui_bh, ui_lineoffset, ui_lineoffset);
}
...
{
	idec_set_ch_flip(id, LCD_CH_0, b_l2r, b_t2b);
}
...
{
	idec_set_ch_yaddr(id, LCD_CH_2, ui_y_addr);
	idec_set_ch_uvaddr(id, LCD_CH_2, ui_cb_addr);
}
...
{
	idec_set_ch_dim(id, LCD_CH_2,ui_bw, ui_bh, ui_lineoffset, ui_lineoffset);
}
...
{
	idec_set_ch_flip(id, LCD_CH_2, b_l2r, b_t2b);
}
...
{
	idec_get_ch_yaddr(id, LCD_CH_0, pui_y_addr);
	idec_get_ch_uvaddr(id, LCD_CH_0, pui_cb_addr);
}
...
{
	idec_get_ch_dim(id, LCD_CH_0,pui_bw, pui_bh, pui_lineoffset);
}
...
{
	idec_get_ch_flip(id, LCD_CH_0, pb_l2r, pb_t2b);
}
...
{
	idec_get_ch_yaddr(id, LCD_CH_2, pui_y_addr);
	idec_get_ch_uvaddr(id, LCD_CH_2, pui_cb_addr);
}
...
{
	idec_get_ch_dim(id, LCD_CH_2,pui_bw, pui_bh, pui_lineoffset);
...
{
	idec_get_ch_flip(id, LCD_CH_2, pb_l2r, pb_t2b);
}
...
{
	idec_set_ch_sdi_sc_dim1(id,LCD_CH_0,ui_win_w,ui_win_h);
}
...
{
	idec_set_mixer_ctrl1(id,0,ui_x,ui_y);
}
...
{
	if (ui_fmt != COLOR_YCC420P&&
		ui_fmt != COLOR_YCC422P) {
		DBG_ERR("Unsupported Video1 format %d\r\n", (int)ui_fmt);
		return;
	}

	idec_set_ch_fmt(id, LCD_CH_0, ui_fmt);
}
...
{
	idec_set_ch_sc_dim(id,LCD_CH_2,ui_win_w,ui_win_h);
}
...
{
	idec_set_mixer_ctrl1(id,2,ui_x,ui_y);
}
...
void idec_set_v2_fmt(IDE_ID id, IDE_COLOR_FORMAT ui_fmt)
{
	if (ui_fmt != COLOR_RGB565 &&
		...
		DBG_ERR("Unsupported Video2 format %d\r\n", (int)ui_fmt);
		return;
	}
	idec_set_ch_fmt(id, LCD_CH_2, ui_fmt);
...
{
	idec_get_ch_sc_dim(id, LCD_CH_0,ui_win_w,ui_win_h);
}
...
{
	idec_get_mixer_ctrl1(id,2,ui_x,ui_y);
}
...
{
	idec_get_ch_fmt(id, LCD_CH_0, ui_fmt);
}

/**
    Get ide Video1 horizontal scaling method
...
/**
    Get ide Video2 horizontal scaling method
...
{
	idec_get_ch_sc_dim(id, LCD_CH_2,ui_win_w,ui_win_h);
}
...
{
	idec_get_mixer_ctrl1(id, LCD_CH_2,ui_x,ui_y);
}
...
{
	idec_get_ch_fmt(id, LCD_CH_2, ui_fmt);
}
...
{
	//idec_set_ch_colorkey(id, LCD_CH_0, ui_ck_y, ui_ck_cb, ui_ck_cr);
}

void idec_set_video_colorkey_high(IDE_ID id, UINT8 ui_ck_y, UINT8 ui_ck_cb, UINT8 ui_ck_cr)
{
	//idec_set_ch_colorkey(id, LCD_CH_0, ui_ck_y, ui_ck_cb, ui_ck_cr);
}
...
/**
    ...
    @return void
*/
void idec_get_video_colorkey(IDE_ID id, CH_ID chid, UINT32 *ui_ck_low, UINT32 *ui_ck_high)
{
	idec_get_ch_colorkey(id, LCD_CH_0, ui_ck_low, ui_ck_high);
}
...
{
	if(yccen)
		idec_set_ch_mode(id, LCD_CH_0, LCD_CH_MODE_YCC);
	else
		idec_set_ch_mode(id, LCD_CH_0, LCD_CH_MODE_RAW);
...
{
	CH_MODE mode;
	idec_get_ch_mode(id, LCD_CH_0, &mode);
...
{
	if(yccen)
		idec_set_ch_mode(id, LCD_CH_2, LCD_CH_MODE_YCC);
