...
#define RTL822X_VND1_SERDES_OPTION_MODE_HISGMII_SGMII   1
...
static int rtl8221b_config(struct phy_device *phydev)
{
	...
	reg = phy_read_mmd(phydev, MDIO_MMD_VEND1, 0x697a);
	reg &= ~RTL822X_VND1_SERDES_OPTION_MODE_MASK;
	reg |= RTL822X_VND1_SERDES_OPTION_MODE_HISGMII_SGMII;
	ret = phy_write_mmd(phydev, MDIO_MMD_VEND1, 0x697a, reg);
	if (ret < 0) {
		return ret;
	}

	/*  reg = phy_read_mmd(phydev, MDIO_MMD_AN, 0x20);
	    reg |= 0x20;
	    ret = phy_write_mmd(phydev, MDIO_MMD_AN, 0x20, reg);
	    if (ret < 0)
	        return ret;
	*/
	genphy_config_aneg(phydev);
	/*  reg = phy_read_mmd(phydev, MDIO_MMD_AN, MDIO_CTRL1);
	    ...
	    ret = phy_write_mmd(phydev, MDIO_MMD_AN, MDIO_CTRL1, reg);
	*/
	return 0;
}
...
static struct phy_driver RTL8221B_driver = {
	.name = "RealTek RTL8221B-VB-CG 2.5Gbps PHY",
	.uid = 0x1cc849,
	.mask = 0xffffff,
	.features = PHY_GBIT_FEATURES,
