 ...
 * ns02301:
 *    NOVATEK's NS02301 chip. This configuration uses an AXI master/DMA bus, an
...
#ifdef CONFIG_TARGET_NS02301_A64
...
typedef enum {
	...
	PIN_ETH_CFG_ETH_RMII_1     = 0x1,          ///< ETH_RMII_1   (P_GPIO[0..5] P_GPIO[8])
	PIN_ETH_CFG_ETH_LED_1      = 0x10,         ///< ETH_LED_1    (D_GPIO[0] D_GPIO[1])
	PIN_ETH_CFG_ETH_LED_2      = 0x20,         ///< ETH_LED_2    (D_GPIO[5] D_GPIO[6])
	PIN_ETH_CFG_ETH_EXTPHYCLK  = 0x100,        ///< ETH_EXTPHYCLK  RMII_1(P_GPIO[7]

	PIN_ETH_CFG_ETH_MDIO_1     = 0x1000,       ///< ETH_MDIO_1    (P_GPIO[9] P_GPIO[10])
} PIN_ETH_CFG;
...
static void mac0_clk_en(phy_interface_t phy_intf)
{
	...
	*/
	...
	reg = readl(IOADDR_CG_REG_BASE + 0x84);
	reg |= (1 << 29); // realse eth0 rst
	writel(reg, IOADDR_CG_REG_BASE + 0x84);
...
static void extphy0_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out, int pin_num)
{
	...
	if (phy_clk > 0) {
		reg = readl(IOADDR_CG_REG_BASE + 0x78); // CG ETH_EXT_PHY_CLKEN
		reg |= (1 << 24);
		writel(reg, IOADDR_CG_REG_BASE + 0x78);

		if (pin_num == 1) {
			reg = readl(IOADDR_TOP_REG_BASE + 0x24); // TOP ETH_EXT_PHY_CLK
			reg |= (1 << 20);
			writel(reg, IOADDR_TOP_REG_BASE + 0x24);

			reg = readl(IOADDR_TOP_REG_BASE + 0x24); // TOP ETH_MDIO_MUX
			reg |= (1 << 16);
			writel(reg, IOADDR_TOP_REG_BASE + 0x24);

			reg = readl(IOADDR_TOP_REG_BASE + 0xA8); // TOP EXT PHY GPIO FUNCTION NORMAL
			reg &= ~(1 << 7);
			writel(reg, IOADDR_TOP_REG_BASE + 0xA8);
		} else if (pin_num == 2) {
			printf("NOT SUPPORT RMII_2 or RGMII_2");
#if 0
	...
	if (pin_num == 1) {
		// reset external phy
		gpio_request(P_GPIO(11), "PHY0_RST");
...
static void mac0_pinmux_en(phy_interface_t phy_intf, int pin_num)
{
	...
	case PHY_INTERFACE_MODE_RMII:
		// set pad driving
		if (pin_num == 1) {
			reg = readl(IOADDR_PAD_REG_BASE + 0x2C);

			// PHYCLK 25m
			reg &= ~(0x3 << 14);
			reg |= (PAD_DRIVING_LV << 14);
			writel(reg, IOADDR_PAD_REG_BASE + 0x2C);

			reg = readl(IOADDR_PAD_REG_BASE + 0x2C);
			// TXCTL TXD0 TXD1
			reg &= ~((0x3 << 4) | (0x3 << 2) | (0x3 << 0));
			reg |= (PAD_DRIVING_LV << 4) | (PAD_DRIVING_LV << 2) | (PAD_DRIVING_LV << 0);
			writel(reg, IOADDR_PAD_REG_BASE + 0x2C);

			reg = readl(IOADDR_PAD_REG_BASE + 0x2C);
			// RX CLK 50M
			reg &= ~(0x3 << 16);
			reg |= (PAD_DRIVING_LV << 16);
			writel(reg, IOADDR_PAD_REG_BASE + 0x2C);
			...
			//GPIO EN
			reg = readl(IOADDR_TOP_REG_BASE + 0xA8);
			reg &= ~(0x73F);
			writel(reg, IOADDR_TOP_REG_BASE + 0xA8);

		} else if (pin_num == 2) {
			printf("NOT SUPPORT RMII_2");
			break;
#if 0
...
//			writel(reg, IOADDR_TOP_REG_BASE + 0xB4);
		...
		break;
	case PHY_INTERFACE_MODE_RGMII:
		printf("NOT SUPPORT RGMII_1 RGMII_2");
#if 0
		// set pad driving
		if (pin_num == 1) {
			reg = readl(IOADDR_PAD_REG_BASE + 0x130);
...
U_BOOT_DRIVER(ns02301_eth_eqos) = {
	.name = "ns02301_eth_eqos",
	.id = UCLASS_ETH,
	.of_match = eqos_ids,
	.probe = eqos_probe,
	.remove = eqos_remove,
