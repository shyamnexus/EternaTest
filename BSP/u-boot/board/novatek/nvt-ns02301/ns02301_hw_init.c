...
#define CG_PLL_ENABLE_OFS 0x20 + 0x100000 // WDT in Pre-roll part
#define STBC_ENABLE_OFS 0x140
#define CG_RESET_OFS 0x28 + 0x100000 // WDT in Pre-roll part
...
void nvt_ivot_reset_cpu(void)
{
	...
	reg_value = CG_GETREG(CG_PLL_ENABLE_OFS);
	CG_SETREG(CG_PLL_ENABLE_OFS, reg_value | WDT_POS);

	WDT_SETREG(WDT_DATA_OFS, 0x0);
	WDT_SETREG(WDT_CTRL_OFS, 0x5A960102);

	udelay(80);

	WDT_SETREG(WDT_CTRL_OFS, 0x5A960103);
