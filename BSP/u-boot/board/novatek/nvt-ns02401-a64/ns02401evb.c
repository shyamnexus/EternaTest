...
#include <asm/arch/ns02401evb.h>
#include <linux/libfdt.h>
#include <fdt_support.h>

DECLARE_GLOBAL_DATA_PTR;
extern int nvt_mmc_init(int id);
extern int ns02401_eth_initialize(struct bd_info *bis);
const char *boardinfo[] = {
	"Board:"_CHIP_NAME_"EVB\n"
};

static struct mm_region evb_mem_map[] = {
	{
	    .virt = 0x0UL, /* For MAU1 */
		.phys = 0x0UL,
		.size = 0x200000000UL,
	    ...
	    .virt = 0x200000000UL, /* For MAU2 */
		.phys = 0x200000000UL,
		.size = 0x200000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_NORMAL) |
			PTE_BLOCK_INNER_SHARE
	}, {
	    .virt = 0x400000000UL, /* For Peripheral */
		.phys = 0x400000000UL,
		.size = 0x400000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_DEVICE_NGNRNE) |
			 PTE_BLOCK_NON_SHARE
	}, {
	    .virt = 0x800000000UL, /* For ACP for MAU1 */
		.phys = 0x800000000UL,
		.size = 0x200000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_NORMAL) |
			PTE_BLOCK_INNER_SHARE
	}, {
	    .virt = 0xA00000000UL, /* For ACP for MAU2 */
		.phys = 0xA00000000UL,
		.size = 0x200000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_NORMAL) |
			PTE_BLOCK_INNER_SHARE
	}, {
	    .virt = 0xC00000000UL, /* For ACP */
		.phys = 0xC00000000UL,
		.size = 0x400000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_NORMAL) |
			PTE_BLOCK_INNER_SHARE
	}, {
	    .virt = 0x1000000000UL, /* TBD*/
		.phys = 0x1000000000UL,
		.size = 0x7000000000UL,
		.attrs = PTE_BLOCK_MEMTYPE(MT_DEVICE_NGNRNE) |
			PTE_BLOCK_NON_SHARE
...
struct mm_region *mem_map = evb_mem_map;
