...
BOOL quary_secure_boot(SECUREBOOT_STATUS scu_status)
{
	...
	case SECUREBOOT_CN_ULTRA_OVR_CLK:
		if ((sec & OTP_CN_ULTRA_OVR_CLK) == OTP_CN_ULTRA_OVR_CLK) {
			result = TRUE;
		}
...
INT32 otp_get_pkg_version(void)
{
	...
	if (code == -33) {
		...
		printf("[UID] = 0x%08x\r\n", (int)trim);
		if(trim == OTP_336_TRIM) {
			pkgUID = NT98336_PKG;
		} else if(trim == OTP_633_TRIM) {
			pkgUID = NT98633_PKG;
		} else {
