...
def main(argv):
    ...
    if expected_leb != mtdcfg_leb:
        expected_leb_4K = int(ubi_max_leb_4K.get_ubifs_max_leb(partition_size))
        if expected_leb_4K != mtdcfg_leb:
            print("leb not matched. expected:2K {} or 4K {} vs mtdcfg:{} on {} partition size: 0x{:X}".format(expected_leb, expected_leb_4K, mtdcfg_leb, args.label, partition_size), file=sys.stderr)
            return -1
