...
void __init free_area_init(unsigned long *max_zone_pfn)
{
	...
	for (i = 0; i < MAX_NR_ZONES; i++) {
		...
		if (arch_zone_lowest_possible_pfn[i] ==
			...
			if (((u64)arch_zone_highest_possible_pfn[i] << PAGE_SHIFT) > ((u64)1 << 32))
				pr_warn("\033[0;31m" "Linux mapping > 4GB, system may be at risk, please contact Novatek to check the status\n" "\033[0m");
#endif
#endif
		}
	}
