...
#define DAI_ID      DAI_ID_3

#define IOBASE_DAI	0x2F0163000
#define IOIRQ_DAI	146
#define EAC_REG_BASE 0x2F0160000
...
static irqreturn_t dma_irq_handler(int irq, void *arg)
{
	...
	reg_status_rx.reg = DAI_GETREG(DAI_INTSTATUS_REG_OFS) & 0x40000;
...
static snd_pcm_uframes_t nvt_dma_pointer(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		curr = DAI_GETREG(DAI_RX1DMACURRENT_REG_OFS);
		start = DAI_GETREG(DAI_1CHRX1DMASTART_REG_OFS);
...
int dai_interrupt_id[MODULE_IRQ_NUM];
EXPORT_SYMBOL(dai_interrupt_id);
...
static int nvt_audio_probe(struct platform_device *pdev)
{
	...
	nvtdai_drv_data->dai_mmio = ioremap(IOBASE_DAI, 0x100);
	_DAI_REG_BASE_ADDR[0] = nvtdai_drv_data->dai_mmio;

	eac_mmio = ioremap(EAC_REG_BASE, 0x200);
	_EAC_REG_BASE_ADDR[0] = eac_mmio;
...
int __init nvt_alsapcm_module_init(void)
{
	...
	pdev = platform_device_alloc("nvt,alsa_plat",-1);
	if (!pdev)
	{
