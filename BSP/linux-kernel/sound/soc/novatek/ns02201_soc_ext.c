...
static int nvt_ext_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	return 0;
}

static int nvt_ext_set_dai_tdm_slot(struct snd_soc_dai *dai,
...
static int nvt_ext_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	return 0;
}
...
static const struct snd_soc_dai_ops nvt_ext_dai_ops = {
	...
	.mute_stream    = nvt_ext_mute,
	.set_fmt		= nvt_ext_set_dai_fmt,
	.set_tdm_slot   = nvt_ext_set_dai_tdm_slot,
};
...
{
	DBG_WRN( "nvt_ext_codec_device_remove\n");
...
int __init nvt_alsa_ext_module_init(void)
{
	...
	DBG_WRN("nvt_alsa_ext_module_init\r\n");

	iRet = platform_driver_register(&nvt_ext_codec_driver);
	if (iRet) {
		printk("nvt_alsa_ext_module_init platform_driver_register error\n");
	}

	pdev = platform_device_alloc("nvt,ext_codec",-1);
	if (!pdev)
	{
...
{
	DBG_WRN("nvt_alsa_ext_module_exit\r\n");
...
MODULE_DESCRIPTION("NOVATEK ALSA ext Codec Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:snd-soc-nvt-ext");
