...
void __iomem *_EAC_REG_BASE_ADDR[1];
...
static UINT32 ADC_STATE = 0;
static UINT32 DAC_STATE = 0;
static BOOL init_eac = FALSE;
...
void eac_init_dac(void){
	...
	EAC_SETREG(EAC_DACIPCTRL1_REG_OFS,  0x37F00000);
...
void eac_init(void)
{
	...
	// Set DAC DSM Maximum output swing
	EAC_SETREG(EAC_DACIPCTRL1_REG_OFS,  0x37F006F0); //0x37F006F0
...
void eac_set_phypower(UINT32 mode, BOOL b_en)
{
	...
	// for access ADC
	if (mode == 0) {
		// from none to ADC or both open
		if (b_en == TRUE) {
			// only ADC
			if (DAC_STATE == 0) {
				reg_phy_ctrl.bit.VOS_ADC 			= 2;

				reg_phy_ctrl.bit.PD_EXT_RP_ADC		= 0;
				reg_phy_ctrl.bit.PD_BIAS			= 0;
				reg_phy_ctrl.bit.CS_COM_CTL 		= 2;
				reg_phy_ctrl.bit.CS_PGA_ADC 		= 1;
				reg_phy_ctrl.bit.CS_REF_ADC 		= 1;
				reg_phy_ctrl.bit.CS_SDM_ADC 		= 3;
				reg_phy_ctrl.bit.CS_COM_DAC 		= 1;

				reg_phy_ctrl.bit.CS_LN_DAC	 		= 3;

				reg_phy_ctrl.bit.PD_ADAC_IU 		= 1;
				reg_phy_ctrl.bit.PD_ADAC_VCM 		= 1;
				reg_phy_ctrl.bit.PD_RC_ADAC 		= 0;
				reg_phy_ctrl.bit.ADAC_GATE 			= 0;
			...
			// both open
				...
				reg_phy_ctrl.bit.PD_ADAC_IU 		= 0;
				reg_phy_ctrl.bit.PD_ADAC_VCM 		= 0;
				reg_phy_ctrl.bit.PD_RC_ADAC 		= 0;
				reg_phy_ctrl.bit.ADAC_GATE 			= 1;
			...
			// to none
			if (DAC_STATE == 0) {
				reg_phy_ctrl.bit.VOS_ADC 			= 3;

				reg_phy_ctrl.bit.PD_EXT_RP_ADC		= 0;
				reg_phy_ctrl.bit.PD_BIAS			= 1;
