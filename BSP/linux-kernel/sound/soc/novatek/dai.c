...
void dai_set_tx_dma_para(int id, int ddr_id, UINT32 dma_channel, uintptr_t dma_start_addr, UINT32 dma_buffer_size)
{
	...
	T_DAI_TX1DMASTARTMSB_REG reg_dma_start_msb = {0};
	...
	//reg_dma_start.bit.TX1DMASTADR = phy_addr & 0xFFFFFFFF;
#elif defined(__FREERTOS)
...
#ifdef __KERNEL__
	reg_dma_start.bit.TX1DMASTADR = phy_addr & 0xFFFFFFFF; //dma_getPhyAddr(dma_start_addr);
	reg_dma_start_msb.bit.TX1DMASTADRMSB = phy_addr >> 32;
	...
	if (dma_channel == 0) {
        ...
        DAI_SETREG(DAI_TX1DMASTARTMSB_REG_OFS,reg_dma_start_msb.reg);
...
{
	T_DAI_1CHRX1DMASTART_REG   reg_dma_start = {0};
	T_DAI_1CHRX1DMASTARTMSB_REG   reg_dma_start_msb = {0};
	T_DAI_1CHRX1DMABUFSIZE_REG reg_dma_bufsz = {0};
#ifdef __KERNEL__
    UINT64 phy_addr;
#elif defined(__FREERTOS)
    //UINT64 phy_addr;
	...
	if (dma_channel == 0) {
        DAI_SETREG( DAI_1CHRX1DMABUFSIZE_REG_OFS, reg_dma_bufsz.reg);
        DAI_SETREG( DAI_1CHRX1DMASTARTMSB_REG_OFS,reg_dma_start_msb.reg);
        DAI_SETREG( DAI_1CHRX1DMASTART_REG_OFS,   reg_dma_start.reg);
	} else if (dma_channel == 1) {
