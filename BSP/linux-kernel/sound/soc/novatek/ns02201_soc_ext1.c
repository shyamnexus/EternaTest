...
{
	DBG_WRN( "nvt_ext1_codec_device_remove\n");
...
int __init nvt_alsa_ext_module_init(void)
{
	...
	DBG_WRN("nvt_alsa_ext1_module_init\r\n");
	iRet = platform_driver_register(&nvt_ext_codec_driver);
	if (iRet) {
		printk("nvt_alsa_ext1_module_init platform_driver_register error\n");
	}

	pdev = platform_device_alloc("nvt,ext1_codec",-1);
	if (!pdev)
	{
...
{
	DBG_WRN("nvt_alsa_ext1_module_exit\r\n");
...
MODULE_DESCRIPTION("NOVATEK ALSA ext1 Codec Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:snd-soc-nvt-ext1");
