...
#if defined(CONFIG_USB_NVTIVOT_HCD)
	/* Only U3 need the patch, for MT7601 warn reboot*/
	if(hcd->driver->flags&HCD_USB3 && udev->speed == USB_SPEED_FULL ) {
		retval = hub_port_reset(hub, port1, udev, delay, false);
		if (retval < 0)  /* error or disconnect */
			goto fail;
	}
	...
	if (retval) {
		...
		if (oldspeed == udev->speed) {
			if (hdev->devpath[0] == '0') {
				...
				if (test_bit(1, &bus_to_hcd(hdev->bus)->nvt_flag))
					clear_bit(1, &(bus_to_hcd(hdev->bus)->nvt_flag));
				else /* result reconnect if power gpio valid*/
					retval = -ENOTCONN;
			} else {
				set_bit(1, &hcd->modem_dongle);
			}
...
loop:
	...
	if (hub->hdev->parent ||
		...
		#if defined(CONFIG_USB_NVTIVOT_HCD)
		if (hdev->devpath[0] == '0') {
			dev_err(&port_dev->dev, "do port reset\n");
			bus_to_hcd(hdev->bus)->driver->port_nc(bus_to_hcd(hdev->bus));
		}
		#endif
	}
		...
		__must_hold(&port_dev->status_lock)
{
	...
	struct usb_device *hdev = hub->hdev;
	u16 portstatus, portchange;
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201) || defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NA51102) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	u32 count;
#endif
#if defined(CONFIG_USB_NVTIVOT_HCD) || defined(CONFIG_NVT_IVOT_PLAT_NS02201) || defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NA51102) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	struct usb_hcd *hcd = bus_to_hcd(hdev->bus);
#endif
	...
	if(hcd->driver->flags&HCD_USB3) {
		if (hub->hdev->speed == USB_SPEED_SUPER) {
			//Recovery
			if((portstatus & USB_PORT_STAT_LINK_STATE) == USB_SS_PORT_LS_RECOVERY) {
				hub_port_reset(hub, port1, NULL,
							HUB_BH_RESET_TIME, true);
			//	printk("[port reset]\r\n");
			}

			//[8..5]  Link on hot reset   9
			if((portstatus & USB_PORT_STAT_LINK_STATE) == USB_SS_PORT_LS_HOT_RESET) {
				count = 0;
				while(1) {
					if (hub_port_status(hub, port1, &portstatus, &portchange) < 0)
						return;
					...
					if(count >500){
						printk("Timeout #####portstatus = 0x%x\r\n",portstatus);
						break;
					}
				}
			}

			//[8..5]  Link on U0   debounce
			if((portstatus & USB_PORT_STAT_LINK_STATE) == USB_SS_PORT_LS_U0) {
			       count = 0;
				while(1)	{
					if (hub_port_status(hub, port1, &portstatus, &portchange) < 0)
						return;
