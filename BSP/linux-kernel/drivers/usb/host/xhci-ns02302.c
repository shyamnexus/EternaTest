...
int xhci_nvt_host_resume(struct usb_hcd *hcd)
{
	...
	temp = USB3CTRL_GETREG(0x14);//readl(regs + 0x14);
	temp &= 0xFFFFFDFF;
	temp |= BUF_RESP_MODE;
	USB3CTRL_SETREG(0x14, temp);//writel(temp, regs + 0x14);

	temp = USB3CTRL_GETREG(0x28);//readl(regs + 0x28);
	temp |= 0x1;
	USB3CTRL_SETREG(0x28, temp);//writel(temp, regs + 0x28);

	temp = USB3CTRL_GETREG(0x10);//readl(regs + 0x10);
	temp &= ~0x2;
	temp |= USB_VBUSVALID;
	//force_hs = 1;
	if (xhci->force_hs)
	...
	USB3CTRL_SETREG(0x10, temp);//writel(temp, regs + 0x10);
	udelay(100);

	//Open SE1 detect
	USB3CTRL_SETREG(0xC, 0x10);
                ...
                char *buf)
{
	...
	printk("============================================\n");
	printk("[BEST_S][U2] 0x%x = (0x%x)\n", (U3UTMI_PHY_BASE + 0x60*4), USB3U2PHY_GETREG(0x60));
	printk("[BEST_S][U2] 0x%x = (0x%x)\n", (U3UTMI_PHY_BASE + 0xC*4), USB3U2PHY_GETREG(0xC));
	printk("[BEST_S][U2] 0x%x = (0x%x)\n", (U3UTMI_PHY_BASE + 0xB*4), USB3U2PHY_GETREG(0xB));
	printk("[BEST_S][U2] 0x%x = (0x%x)\n", (U3UTMI_PHY_BASE + 0x63*4), USB3U2PHY_GETREG(0x63));
...
int xhci_nvt_host_init(struct usb_hcd *hcd)
{
	...
	ret = nvt_hwdump_sysfs_init(dev);
	if (ret < 0) {
		dev_err(dev, "create sysfs failed!\n");
	} else {
		printk("nvt_hwdump_sysfs_init pass\n");
	}
