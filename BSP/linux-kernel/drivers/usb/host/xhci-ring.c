...
static void xhci_handle_cmd_stop_ep(struct xhci_hcd *xhci, int slot_id,
		union xhci_trb *trb, struct xhci_event_cmd *event, bool fakecmd)
{
	unsigned int ep_index;
	struct xhci_ring *ep_ring;
	struct xhci_virt_ep *ep;
		...
		struct xhci_event_cmd *event)
{
	...
	cmd_dequeue_dma = xhci_trb_virt_to_dma(xhci->cmd_ring->deq_seg,
			cmd_trb) & 0xFFFFFFFF;
	cmd_dma &= 0xFFFFFFFF; //for cmd_dma != (u64)cmd_dequeue_dma,cmd_dequeue_dma only 32bit, so cmd_dma &=0xFFFFFFFF. For problem that cmd_dma may equal to 0xCxxxxxxxx over 32bit.
#else
	cmd_dequeue_dma = xhci_trb_virt_to_dma(xhci->cmd_ring->deq_seg,
			cmd_trb);
#endif
	...
	case TRB_STOP_RING:
		...
		if (!cmd->completion)
			xhci_handle_cmd_stop_ep(xhci, slot_id, cmd_trb, event, false);
		break;
	case TRB_NVT_STOP:
		if (!cmd->completion)
			xhci_handle_cmd_stop_ep(xhci, TRB_TO_SLOT_ID( le32_to_cpu(cmd_trb->generic.field[3])), cmd_trb, event, true);
		break;
	case TRB_SET_DEQ:
		WARN_ON(slot_id != TRB_TO_SLOT_ID(
				le32_to_cpu(cmd_trb->generic.field[3])));
...
irqreturn_t xhci_irq(struct usb_hcd *hcd)
{
	...
	u32 status;
	int event_loop = 0;
#if defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	tmpval = readl((volatile unsigned long *)(xhci->phy_reg + NVT_SE1_OFS));

	if ((tmpval & NVT_SE1_EN) && (tmpval & NVT_SE1_STS)) {
		writel(0, (volatile unsigned long *)(xhci->phy_reg + NVT_SE1_OFS));
		printk("SE1 abnormal signal!\n");
		hcd->driver->port_nc_se1(hcd);
		writel(NVT_SE1_EN, (volatile unsigned long *)(xhci->phy_reg + NVT_SE1_OFS));
		goto out;
	}
#endif
	...
	///check eveing again if need to deq
...
int xhci_queue_stop_endpoint_nvt(struct xhci_hcd *xhci, struct xhci_command *cmd,
				int slot_id, unsigned int ep_index, int suspend)
{
	u32 trb_slot_id = SLOT_ID_FOR_TRB(slot_id);
	u32 trb_ep_index = EP_ID_FOR_TRB(ep_index);
	u32 type = TRB_TYPE(TRB_NVT_STOP); //FAKE TRB_STOP_RING
	u32 trb_suspend = SUSPEND_PORT_FOR_TRB(suspend);
