...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
static void xhci_nvt_host_set_highaddr(struct xhci_hcd *xhci, dma_addr_t dma)
{
	unsigned long usbbase;
	unsigned long tmpval = 0;
	unsigned long high_addr = dma;

	high_addr = (high_addr >> 32) & 0xF;

	usbbase = xhci->phy_reg;
					       ...
					       gfp_t flags)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		dma = (dma_addr_t)nvtpcie_get_upstream_pa(xhci->ep_id, dma);
	}
#endif
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
		if (xhci->ep_id != CHIP_RC) {
			nvtpcie_chipid_t ret_chip;
			seg->dma = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, seg->dma, &ret_chip);
		}
#endif
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	if (xhci->ep_id != CHIP_RC) {
		ctx->dma = (dma_addr_t)nvtpcie_get_upstream_pa(xhci->ep_id, ctx->dma);
	}
#endif
			     ...
			     struct xhci_container_ctx *ctx)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		nvtpcie_chipid_t ret_chip;
		ctx->dma = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, ctx->dma, &ret_chip);
	}
#endif
		...
		struct xhci_stream_ctx *stream_ctx, dma_addr_t dma)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		nvtpcie_chipid_t ret_chip;
		dma = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, dma, &ret_chip);
	}
#endif
		...
		gfp_t mem_flags)
{
	...
	size_t size = sizeof(struct xhci_stream_ctx) * num_stream_ctxs;
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	struct xhci_stream_ctx *result;

	if (size > MEDIUM_STREAM_ARRAY_SIZE)
		result = dma_alloc_coherent(dev, size,
				dma, mem_flags);
	else if (size <= SMALL_STREAM_ARRAY_SIZE)
		result = dma_pool_alloc(xhci->small_streams_pool,
				mem_flags, dma);
	else
		result = dma_pool_alloc(xhci->medium_streams_pool,
				mem_flags, dma);
...
static int scratchpad_alloc(struct xhci_hcd *xhci, gfp_t flags)
{
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	if (xhci->ep_id != CHIP_RC) {
		xhci->scratchpad->sp_dma = (dma_addr_t)nvtpcie_get_upstream_pa(xhci->ep_id, xhci->scratchpad->sp_dma);
	}
#endif
 ...
 fail_sp4:
	for (i = i - 1; i >= 0; i--) {
		...
		if (xhci->ep_id != CHIP_RC) {
			nvtpcie_chipid_t ret_chip;
			xhci->scratchpad->sp_array[i] = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, xhci->scratchpad->sp_array[i], &ret_chip);
		}
#endif
...
static void scratchpad_free(struct xhci_hcd *xhci)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		nvtpcie_chipid_t ret_chip;
		xhci->scratchpad->sp_dma = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, xhci->scratchpad->sp_dma, &ret_chip);
	}
#endif
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	 if (xhci->ep_id != CHIP_RC)
		 erst->erst_dma_addr = (dma_addr_t)nvtpcie_get_upstream_pa(xhci->ep_id, erst->erst_dma_addr);
#endif
...
void xhci_free_erst(struct xhci_hcd *xhci, struct xhci_erst *erst)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		nvtpcie_chipid_t ret_chip;
		erst->erst_dma_addr = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, erst->erst_dma_addr, &ret_chip);
	}
#endif
	if (erst->entries)
		dma_free_coherent(dev, size,
...
void xhci_mem_cleanup(struct xhci_hcd *xhci)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		nvtpcie_chipid_t ret_chip;
		xhci->dcbaa->dma = (dma_addr_t)nvtpcie_get_unmapped_pa(xhci->ep_id, xhci->dcbaa->dma, &ret_chip);
	}
#endif
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	if (xhci->ep_id != CHIP_RC) {
		dma = (dma_addr_t)nvtpcie_get_upstream_pa(xhci->ep_id, dma);
		xhci_nvt_host_set_highaddr(xhci, dma);
	}
#endif
