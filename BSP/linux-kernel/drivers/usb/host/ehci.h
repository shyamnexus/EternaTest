...
#define	DEFAULT_I_TDPS		1024		/* some HCs can do less */
...
#if defined(CONFIG_PLAT_NOVATEK) || defined(CONFIG_NVT_IVOT_PLAT_NS02201) || defined(CONFIG_NVT_IVOT_PLAT_NA51102) || defined(CONFIG_NVT_IVOT_PLAT_NA51055)
	unsigned long         fuseData;
	...
	int                   patch_thread;
	int                   se_patch_thread;
	int                   se_patch_thread_ms;
	u32                   se_intr_counter;
	u32                   se_intr_threshold;
	u32                   patch_jstate;
	struct clk            *reset_clk;
	u8                    efuse_data[3];
	u8                    store_efuse_data[3];
	spinlock_t            host_lock;
	struct task_struct    *task_nvt_daemon;
	struct task_struct    *task_se_daemon;
	struct task_struct    *task_j_daemon;
	int                   ep_id;
	int                   power_gpio[2];
	unsigned long         usbbase;
	unsigned long         phyrstbase;
	unsigned long         vbusbase;
	int                   remove_thread;
	int                   u2_only;
#endif
