...
static void end_free_itds(struct ehci_hcd *ehci)
{
	...
	list_for_each_entry_safe(itd, n, &ehci->cached_itd_list, itd_list) {
		...
		if (CHIP_RC != ehci->ep_id) {
			...
			itd->itd_dma = (dma_addr_t)nvtpcie_get_unmapped_pa(ehci->ep_id, itd->itd_dma, &ret_chip);
		}
#endif
	...
	list_for_each_entry_safe(sitd, sn, &ehci->cached_sitd_list, sitd_list) {
		...
		if (CHIP_RC != ehci->ep_id) {
			nvtpcie_chipid_t ret_chip;
			sitd->sitd_dma = (dma_addr_t)nvtpcie_get_unmapped_pa(ehci->ep_id, sitd->sitd_dma, &ret_chip);
		}
#endif
