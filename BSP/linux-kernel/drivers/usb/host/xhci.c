...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02201) || defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NA51102)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
#include <linux/gpio.h>
static void nvtim_patch_usbhc(struct usb_hcd *hcd)
{
	struct xhci_hcd *xhci = hcd_to_xhci(hcd);
	u32 usb2_port, usb3_port;

#if defined(CONFIG_NVT_IVOT_PLAT_NS02201)|| defined(CONFIG_NVT_IVOT_PLAT_NA51090) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	if(xhci->standalone_pci)
		return;
#endif
	...
	if (xhci->power_gpio[0] > 0) {
		...
		usb2_port = readl(xhci->usb3_rhub.ports[0]->addr);
		usb3_port = readl(xhci->usb2_rhub.ports[0]->addr);

		/*if gpio setting is right, device should disconnect and u2/u3 port value should be the same */
		if(usb2_port != usb3_port) {
			printk("the power gpio is fake!\n");
			set_bit(1, &hcd->nvt_flag);
		}
...
static int nvtim_patch_usbhc3(struct usb_hcd *hcd)
{
	struct xhci_hcd *xhci = hcd_to_xhci(hcd);
...
static void nvtim_patch_usbhc_se1(struct usb_hcd *hcd)
{
	u32 portsc;
	struct xhci_hcd *xhci = hcd_to_xhci(hcd);
	struct xhci_hub *rhub;
	...
	portsc = readl(rhub->ports[0]->addr);

	portsc |= PORT_RESET;
	writel(portsc, rhub->ports[0]->addr);
...
static const struct hc_driver xhci_hc_driver = {
	...
	.port_nc3 =       nvtim_patch_usbhc3,
	#if defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NS02401)
	.port_nc_se1 =    nvtim_patch_usbhc_se1,
	#endif
#endif
};
