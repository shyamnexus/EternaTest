...
#include "nvt_crypto.h"

#define DRV_VERSION                         "1.01.02"

//#define NVT_CRYPTO_GCM_SOFTWARE_GHASH   1
//#define NVT_CRYPTO_PIO_SUPPORT          1
#define NVT_CRYPTO_DMA_TIMEOUT_DBG      1
#define NVT_CRYPTO_DMA_DESC_CV_CHECK    1
#define NVT_CRYPTO_DMA_36BIT_SUPPORT    1
#define NVT_CRYPTO_DMA_ALIGN_SIZE       4
#define NVT_CRYPTO_CACHE_ALIGN_SIZE     64          ///< platform cache line size => CA53:64 byte

#define ALIGN_UP(x, align_to)               (((x) + ((align_to)-1)) & ~((align_to)-1))
#define ALIGN_DN(x, align_to)               ((x) & ~((align_to)-1))

#define DMA_ADDR_LO(x)                      ((u32)((u64)(x)))
#define DMA_ADDR_HI(x)                      ((u32)(((u64)(x)) >> 32))

#define NVT_CRYPTO_QUEUE_LENGTH         10
#define NVT_CRYPTO_DEFAULT_TIMEOUT      3000        ///< 3 sec
#define NVT_CRYPTO_ALG_PRIORITY         1000

#ifdef NVT_CRYPTO_DMA_36BIT_SUPPORT
#define NVT_CRYPTO_DMA_ADDR_MASK        0xFFFFFFFFFULL  ///< DMA support address bit[35..0]
#else
#define NVT_CRYPTO_DMA_ADDR_MASK        0xFFFFFFFF      ///< DMA support address bit[31..0]
#endif

#define NVT_CRYPTO_EKEY_HDR_MAGIC       0x79656B65  ///< 'ekey'
#define NVT_CRYPTO_EKEY_OFS_MAX         20          ///< 0 ~ 19 word offset

typedef enum {
	NVT_CRYPTO_TBUF_ALLOC_NONE = 0,     ///< disable
	NVT_CRYPTO_TBUF_ALLOC_BUDDY,        ///< from kernel buddy system
	NVT_CRYPTO_TBUF_ALLOC_MAX
} NVT_CRYPTO_TBUF_ALLOC_T;

typedef enum {
	NVT_CRYPTO_ALIGN_MODE_DMA = 0,      ///< dma word alignment mode
	NVT_CRYPTO_ALIGN_MODE_CACHE,        ///< cache line alignment mode
	NVT_CRYPTO_ALIGN_MODE_MAX
} NVT_CRYPTO_ALIGN_MODE_T;

static int tbuf_alloc = NVT_CRYPTO_TBUF_ALLOC_NONE;
...
static int align_mode = NVT_CRYPTO_ALIGN_MODE_CACHE;
...
struct nvt_crypto_dma_desc {
	u32                              key[NVT_CRYPTO_MAX_KEY_SIZE / sizeof(u32)];  ///< crypto input key value
	u32                              iv[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)];    ///< crypto input IV value
	u32                              counter[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)]; ///< crypto input counter value in the CTR
	u32                              header_cfg;                                      ///< DMA descriptor header configuration
	u32                              reserved[3];                                     ///< reserve bytes
	u32                              cv[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)];    ///< current IV
	u32                              s0[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)];    ///< E(K,Y0) or S0 in the GCM
	u32                              ghash[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)]; ///< GHASH output data in the GCM
	struct nvt_crypto_dma_block  block[NVT_CRYPTO_MAX_DMA_BLOCK_NUM];         ///< DMA process blocks
} __attribute__((packed, aligned(4)));

#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
struct nvt_crypto_ghash_ctx {
...
struct nvt_crypto_ctx {
	struct nvt_crypto_dev        *dev;
	int                              keylen;
...
struct nvt_crypto_reqctx {
	NVT_CRYPTO_TYPE_T            type;
	NVT_CRYPTO_MODE_T            mode;
	NVT_CRYPTO_OPMODE_T          opmode;
	NVT_CRYPTO_KEY_SRC_T         key_src;
	u32                              get_s0;
	NVT_CRYPTO_CCM_TYPE_T        ccm_type;
};

struct nvt_crypto_gcm_reqctx {
	NVT_CRYPTO_TYPE_T            type;
	NVT_CRYPTO_MODE_T            mode;
	NVT_CRYPTO_OPMODE_T          opmode;
	NVT_CRYPTO_KEY_SRC_T         key_src;
...
#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
	struct nvt_crypto_ghash_ctx  ghash_ctx;
...
struct nvt_crypto_dma_ch {
	int                              idx;                                           ///< DMA channel index
	NVT_CRYPTO_STATE_T           state;                                         ///< DMA channel state

	dma_addr_t                       desc_paddr;                                    ///< DMA channel descriptor physical address
	struct nvt_crypto_dma_desc   *desc;                                         ///< DMA channel descriptor
	...
	u32                              iv[NVT_CRYPTO_MAX_IV_SIZE / sizeof(u32)];  ///< for store current request IV

	struct crypto_async_request      *req;                                          ///< asynchronous request
	struct nvt_crypto_ctx        *ctx;
...
struct nvt_crypto_dev {
	...
	struct nvt_crypto_dma_buf    dma_buf;
	struct nvt_crypto_dma_ch     dma_ch[NVT_CRYPTO_DMA_CH_MAX];
...
static struct nvt_crypto_dev *nvt_cdev = NULL;

#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
static const u8 nvt_crypto_aes_zeroes[AES_BLOCK_SIZE] __attribute__((aligned(4))) = {[0 ...(AES_BLOCK_SIZE - 1)] = 0};
#endif

static inline u32 nvt_crypto_read(struct nvt_crypto_dev *dd, u32 offset)
{
	return readl(dd->iobase + offset);
}

static inline void nvt_crypto_write(struct nvt_crypto_dev *dd, u32 offset, u32 value)
{
	writel(value, dd->iobase + offset);
}

static void nvt_crypto_reset(struct nvt_crypto_dev *dd)
{
	u32 value = 0;
	u32 cnt   = 0;

	/* disable crypto */
	nvt_crypto_write(dd, NVT_CRYPTO_CFG_REG, 0);

	/* set reset, hardware will auto clear */
	nvt_crypto_write(dd, NVT_CRYPTO_CFG_REG, 0x01);

	/* check reset done */
	while ((value = nvt_crypto_read(dd, NVT_CRYPTO_CFG_REG)) & 0x1) {
		if (cnt++ >= 100) {
			break;
		}
		udelay(1);
	...
	/* clear all status */
	nvt_crypto_write(dd, NVT_CRYPTO_INT_STS_REG, 0xFF1);
...
static int nvt_crypto_handle_req(struct nvt_crypto_dev *dev, struct crypto_async_request *req, int q_id)
{
	unsigned long flags;
	int err;
...
static void nvt_crypto_complete(struct nvt_crypto_dev *dev, NVT_CRYPTO_DMA_CH_T ch, int err)
{
	if (ch >= NVT_CRYPTO_DMA_CH_MAX) {
		return;
	}

	(dev->dma_ch[ch].req)->complete(dev->dma_ch[ch].req, err);
	dev->dma_ch[ch].req   = NULL;
	dev->dma_ch[ch].state = NVT_CRYPTO_STATE_IDLE;
}

static bool nvt_crypto_is_sg_aligned(struct scatterlist *sg, int align_size, int align_last, int chk_size, int mode)
{
	int count = 0;
	int leng_align_size;
	int addr_align_size;

	if (mode == NVT_CRYPTO_ALIGN_MODE_DMA) {
		leng_align_size = align_size;
		addr_align_size = NVT_CRYPTO_DMA_ALIGN_SIZE;
	} else {
		leng_align_size = NVT_CRYPTO_CACHE_ALIGN_SIZE;
		addr_align_size = NVT_CRYPTO_CACHE_ALIGN_SIZE;
	}

	while (sg && (count < chk_size)) {
		if (((sg_phys(sg) + sg->length) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
			return false;
			...
			if ((align_last == 0) && (mode == NVT_CRYPTO_ALIGN_MODE_DMA)) {
...
static void nvt_crypto_timeout_handler(struct timer_list *t)
{
	struct nvt_crypto_dma_ch *pdma = from_timer(pdma, t, timer);
	struct nvt_crypto_dev    *dev  = container_of(pdma, struct nvt_crypto_dev, dma_ch[pdma->idx]);
	unsigned long flags;
	int i;
#ifdef NVT_CRYPTO_DMA_TIMEOUT_DBG
	int j;
	volatile u32 *p_desc;
#endif
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
	...
	for (i = 0; i <= NVT_CRYPTO_KEY_READ_REG; i += 16) {
		dev_err(dev->dev, "%04x | %08x %08x %08x %08x\n",
				i,
				nvt_crypto_read(dev, i),
				nvt_crypto_read(dev, i + 4),
				nvt_crypto_read(dev, i + 8),
				nvt_crypto_read(dev, i + 12));
	}
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		...
		dev_err(dev->dev, "[DMA#%d] paddr:0x%08lx vaddr:0x%08lx\n", i, (uintptr_t)dev->dma_ch[i].desc_paddr, (uintptr_t)dev->dma_ch[i].desc);
		for (j = 0; j < (sizeof(struct nvt_crypto_dma_desc) / 4); j += 4) {
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		...
		if (crypto_tfm_alg_type((dev->dma_ch[i].req)->tfm) == CRYPTO_ALG_TYPE_AEAD) {
			struct nvt_crypto_ccm_reqctx *ccm_reqctx = aead_request_ctx(aead_request_cast(dev->dma_ch[i].req));
		...
		nvt_crypto_complete(dev, i, -EINVAL);
	}
...
static int nvt_crypto_trigger_efuse_key(u8 key_ofs, u8 key_cnt)
{
...
static void nvt_crypto_start(struct nvt_crypto_dev *dev)
{
	struct nvt_ekey_hdr          *ekey_hdr;
	struct skcipher_request          *sk_req;
	struct aead_request              *aead_req;
	struct crypto_skcipher           *cipher;
	struct nvt_crypto_ctx        *ctx;
	struct nvt_crypto_reqctx     *reqctx;
	struct nvt_crypto_ccm_reqctx *ccm_reqctx;
	struct nvt_crypto_gcm_reqctx *gcm_reqctx;
	volatile struct nvt_crypto_dma_desc *desc;
	struct nvt_crypto_dma_ch     *dma_ch;
	u8                               *iv;
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		...
		if (dma_ch->state != NVT_CRYPTO_STATE_START) {
			continue;
		}
		...
		if (!dma_ch->req) {
			...
			dma_ch->state = NVT_CRYPTO_STATE_IDLE;
			continue;
		}
		...
		if (crypto_tfm_alg_type(dma_ch->req->tfm) == CRYPTO_ALG_TYPE_AEAD) {
			...
			if (reqctx->ccm_type) {
				ccm_reqctx = (struct nvt_crypto_ccm_reqctx *)reqctx;
				if (ccm_reqctx->opmode == NVT_CRYPTO_OPMODE_CBC) {
				...
				gcm_reqctx = (struct nvt_crypto_gcm_reqctx *)reqctx;
				ass_sg     = aead_req->src;
			...
			sk_req     = skcipher_request_cast(dma_ch->req);
			cipher     = crypto_skcipher_reqtfm(sk_req);
			reqctx     = skcipher_request_ctx(sk_req);
			...
			dma_ch->ass_nbytes = 0;
			dma_ch->req_nbytes = sk_req->cryptlen;
		}
		...
		dst_dma_map = 0;
		err         = 0;
		align_last  = (reqctx->opmode == NVT_CRYPTO_OPMODE_GCM || reqctx->opmode == NVT_CRYPTO_OPMODE_CTR || reqctx->opmode == NVT_CRYPTO_OPMODE_CFB) ? 0 : 1;
		...
		if (dev->dbg_mode >= 2) {
			...
			dev_info(dev->dev, "DMA#%d Sg   => orignal src_nents :%u dst_nents:%u req_nbytes:%u auth_size:%u\n", i, (src_sg ? sg_nents(src_sg) : 0), (dst_sg ? sg_nents(dst_sg) : 0), (u32)dma_ch->req_nbytes, auth_size);
			dev_info(dev->dev, "DMA#%d Src  => orignal pa:0x%08lx va:0x%08lx size:%d\n", i, (src_sg ? (uintptr_t)sg_phys(src_sg) : 0), (src_sg ? (uintptr_t)sg_virt(src_sg) : 0), (src_sg ? src_sg->length : 0));
			dev_info(dev->dev, "DMA#%d Dst  => orignal pa:0x%08lx va:0x%08lx size:%d\n", i, (dst_sg ? (uintptr_t)sg_phys(dst_sg) : 0), (dst_sg ? (uintptr_t)sg_virt(dst_sg) : 0), (dst_sg ? dst_sg->length : 0));
		}
		...
		if (src_sg && dma_ch->req_nbytes) {
			...
			if (!nvt_crypto_is_sg_aligned(src_sg, ctx->block_size, align_last, dma_ch->req_nbytes, NVT_CRYPTO_ALIGN_MODE_DMA)) {
				if (dev->dbg_mode >= 1) {
				...
				if (dma_ch->tbuf_src && tbuf_size) {
					if (dma_ch->req_nbytes > tbuf_size) {
						if (reqctx->opmode == NVT_CRYPTO_OPMODE_GCM) {
						...
						if (dev->dbg_mode >= 3) {
							dev_info(dev->dev, "crypto dma#%d copy %u bytes from original offset 0 to temp buffer\n", i, (u32)dma_ch->req_nbytes);
						}
					...
					if ((((uintptr_t)page_to_phys(virt_to_page(src_pages)) + dma_ch->req_nbytes) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
						dev_err(dev->dev, "crypto dma#%d allocated source paddr:0x%08lx not support\n", i, (uintptr_t)page_to_phys(virt_to_page(src_pages)));
						err = -ENOMEM;
						goto error;
					}
		...
		if (dst_sg && dma_ch->req_nbytes) {
			...
			if (!nvt_crypto_is_sg_aligned(dst_sg, ctx->block_size, align_last, dma_ch->req_nbytes, align_mode) || !IS_ALIGNED(dma_ch->req_nbytes, 4)) {
				if (dev->dbg_mode >= 1) {
				...
				if (dma_ch->tbuf_dst && tbuf_size) {
					...
					if ((dma_ch->src_copied == 1) && IS_ALIGNED(dma_ch->req_nbytes, 4)) {
						...
						if ((((uintptr_t)page_to_phys(virt_to_page(dst_pages)) + ALIGN_UP(dma_ch->req_nbytes, 4)) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
							dev_err(dev->dev, "crypto dma#%d allocated destination paddr:0x%08lx not support\n", i, (uintptr_t)page_to_phys(virt_to_page(dst_pages)));
							err = -ENOMEM;
							goto error;
						}
		...
		if (ass_sg && dma_ch->ass_nbytes) {
			...
			if (!nvt_crypto_is_sg_aligned(ass_sg, ctx->block_size, align_last, dma_ch->ass_nbytes, NVT_CRYPTO_ALIGN_MODE_DMA)) {
				if (dev->dbg_mode >= 1) {
				...
				if (dma_ch->tbuf_ass && tbuf_size) {
					...
					if ((((uintptr_t)page_to_phys(virt_to_page(ass_pages)) + dma_ch->ass_nbytes) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
						dev_err(dev->dev, "crypto dma#%d allocated associated paddr:0x%08lx not support\n", i, (uintptr_t)page_to_phys(virt_to_page(ass_pages)));
						err = -ENOMEM;
						goto error;
					}
		...
		if (dev->dbg_mode >= 2) {
			dev_info(dev->dev, "DMA#%d Sg   => src_nents :%u dst_nents:%u req_nbytes:%u auth_size:%u\n", i, (dma_ch->sg_src ? sg_nents(dma_ch->sg_src) : 0), (dma_ch->sg_dst ? sg_nents(dma_ch->sg_dst) : 0), (u32)dma_ch->req_nbytes, auth_size);
			dev_info(dev->dev, "DMA#%d Sg   => src_copied:%d dst_copied:%d same:%d\n", i, dma_ch->src_copied, dma_ch->dst_copied, dma_ch->sg_same);
			dev_info(dev->dev, "DMA#%d Src  => pa:0x%08lx va:0x%08lx size:%d\n", i, (dma_ch->sg_src ? (uintptr_t)sg_dma_address(dma_ch->sg_src) : 0), (dma_ch->sg_src ? (uintptr_t)sg_virt(dma_ch->sg_src) : 0), (dma_ch->sg_src ? dma_ch->sg_src->length : 0));
			dev_info(dev->dev, "DMA#%d Dst  => pa:0x%08lx va:0x%08lx size:%d\n", i, (dma_ch->sg_dst ? (uintptr_t)sg_dma_address(dma_ch->sg_dst) : 0), (dma_ch->sg_dst ? (uintptr_t)sg_virt(dma_ch->sg_dst) : 0), (dma_ch->sg_dst ? dma_ch->sg_dst->length : 0));
			dev_info(dev->dev, "DMA#%d Ass  => ass_nents:%u ass_nbytes:%u ass_copied:%d\n", i, (dma_ch->sg_ass ? sg_nents_for_len(dma_ch->sg_ass, dma_ch->ass_nbytes) : 0), (u32)dma_ch->ass_nbytes, dma_ch->ass_copied);
			dev_info(dev->dev, "DMA#%d Ass  => pa:0x%08lx va:0x%08lx size:%d\n", i, (dma_ch->sg_ass ? (uintptr_t)sg_dma_address(dma_ch->sg_ass) : 0), (dma_ch->sg_ass ? (uintptr_t)sg_virt(dma_ch->sg_ass) : 0), (dma_ch->sg_ass ? dma_ch->sg_ass->length : 0));
			dev_info(dev->dev, "DMA#%d Desc => pa:0x%08lx va:0x%08lx\n", i, (uintptr_t)dma_ch->desc_paddr, (uintptr_t)dma_ch->desc);
		}
		...
		/* key header */
		ekey_hdr = (struct nvt_ekey_hdr *)ctx->key;

		if ((ekey_hdr->magic == NVT_CRYPTO_EKEY_HDR_MAGIC) &&
			(ekey_hdr->offset < NVT_CRYPTO_EKEY_OFS_MAX)   &&
			(ekey_hdr->rsvd[0] == 0)                           &&
			(ekey_hdr->rsvd[1] == 0)                           &&
			(ekey_hdr->rsvd[2] == 0)) {
			switch (reqctx->mode) {
			case NVT_CRYPTO_MODE_DES:
				key_cnt = 2;
				break;
			case NVT_CRYPTO_MODE_3DES:
				key_cnt = 6;
				break;
			case NVT_CRYPTO_MODE_AES_128:
				key_cnt = 4;
				break;
			case NVT_CRYPTO_MODE_AES_256:
				key_cnt = 8;
				break;
			default:
			...
			if (nvt_crypto_trigger_efuse_key(ekey_hdr->offset, key_cnt)) {
				dev_err(dev->dev, "DMA#%d Key  => key invalid(offset:%d)!\n", i, ekey_hdr->offset);
				err = -EINVAL;
				goto error;
			} else {
				reqctx->key_src = NVT_CRYPTO_KEY_SRC_MANAGAMENT;
				if (dev->dbg_mode >= 3) {
		...
		switch (reqctx->mode) {
		case NVT_CRYPTO_MODE_DES:       /* key => 64 bits, IV => 64 bits, data => 64 bits */
			/* set IV */
			if (iv && ivsize) {
			...
			/* set key */
			if (reqctx->key_src == NVT_CRYPTO_KEY_SRC_DESC0) {
			...
			break;
		case NVT_CRYPTO_MODE_3DES:      /* key => 192 bit, IV => 64 bits, data => 64 bits, DES-EDE3 */
			/* set IV */
			if (iv && ivsize) {
				desc->iv[0] = dma_ch->iv[0];
				desc->iv[1] = dma_ch->iv[1];
			...
			break;
		case NVT_CRYPTO_MODE_AES_128:   /* key => 128 bits, IV => 128 bits, data => 128 bits*/
			/* set IV */
			if (iv && ivsize) {
				desc->iv[0] = dma_ch->iv[0];
				desc->iv[1] = dma_ch->iv[1];
			...
			break;
		case NVT_CRYPTO_MODE_AES_256:   /* key => 256 bits, IV => 128 bits, data => 128 bits*/
			/* set IV */
			if (iv && ivsize) {
				desc->iv[0] = dma_ch->iv[0];
				desc->iv[1] = dma_ch->iv[1];
		...
		/* set conuter */
		if (reqctx->opmode == NVT_CRYPTO_OPMODE_CTR) {
			if (reqctx->mode == NVT_CRYPTO_MODE_DES || reqctx->mode == NVT_CRYPTO_MODE_3DES) {
				desc->counter[1] = 1;
			} else {
				desc->counter[3] = 1;
			}
		...
		if (dma_ch->sg_ass && dma_ch->ass_nbytes) {
			...
			dma_ch->ass_total   = 0;
			for (j = 0; j < NVT_CRYPTO_MAX_DMA_BLOCK_NUM; j++) {
				...
				desc->block[j].src_addr  = DMA_ADDR_LO(ALIGN_DN(sg_dma_address(dma_ch->sg_ass_work), 4));
				desc->block[j].dst_addr  = 0;
				desc->block[j].length    = dma_len;
				desc->block[j].block_cfg = 0;
#ifdef NVT_CRYPTO_DMA_36BIT_SUPPORT
				desc->block[j].block_cfg |= ((DMA_ADDR_HI(ALIGN_DN(sg_dma_address(dma_ch->sg_ass_work), 4)) & 0xf) << 16);
#endif
				block_num++;
		...
		if ((block_num == 0) && (reqctx->opmode == NVT_CRYPTO_OPMODE_GCM)) {
		...
		if (dma_ch->sg_src && dma_ch->sg_dst && dma_ch->req_nbytes) {
			...
			dma_ch->sg_dst_ofs  = 0;
			dma_ch->dst_total   = 0;
			for (j = block_num; j < NVT_CRYPTO_MAX_DMA_BLOCK_NUM; j++) {
				...
				/* DMA block config setting */
				desc->block[j].src_addr  = DMA_ADDR_LO(ALIGN_DN((sg_dma_address(dma_ch->sg_src_work) + dma_ch->sg_src_ofs), 4));
				desc->block[j].dst_addr  = DMA_ADDR_LO(ALIGN_DN((sg_dma_address(dma_ch->sg_dst_work) + dma_ch->sg_dst_ofs), 4));
				desc->block[j].length    = dma_len;
				desc->block[j].block_cfg = 0;
#ifdef NVT_CRYPTO_DMA_36BIT_SUPPORT
				desc->block[j].block_cfg |= ((DMA_ADDR_HI(ALIGN_DN((sg_dma_address(dma_ch->sg_src_work) + dma_ch->sg_src_ofs), 4)) & 0xf) << 16);
				desc->block[j].block_cfg |= ((DMA_ADDR_HI(ALIGN_DN((sg_dma_address(dma_ch->sg_dst_work) + dma_ch->sg_dst_ofs), 4)) & 0xf) << 24);
#endif
				block_num++;
...
#ifdef NVT_CRYPTO_DMA_36BIT_SUPPORT
				desc->block[j].block_cfg |= ((DMA_ADDR_HI(ALIGN_DN((sg_dma_address(dma_ch->sg_src_work) + dma_ch->sg_src_ofs), 4)) & 0xf) << 16);
				desc->block[j].block_cfg |= ((DMA_ADDR_HI(ALIGN_DN((sg_dma_address(dma_ch->sg_src_work) + dma_ch->sg_src_ofs), 4)) & 0xf) << 24);
#endif
				block_num++;
		...
		if ((reqctx->opmode == NVT_CRYPTO_OPMODE_GCM) && (dma_ch->src_total != dma_ch->req_nbytes)) {
		...
		if (block_num) {
...
#ifdef NVT_CRYPTO_DMA_DESC_CV_CHECK
			/* clear current cv */
			desc->cv[0] = 0;
			desc->cv[1] = 0;
			desc->cv[2] = 0;
			...
			dma_ch->state = NVT_CRYPTO_STATE_BUSY;
		} else {
			err = -EINVAL;
		}
...
error:
	...
	if (dma_enb) {
		/* set crypto enable */
		reg_value = nvt_crypto_read(dev, NVT_CRYPTO_CFG_REG);
		nvt_crypto_write(dev, NVT_CRYPTO_CFG_REG, (reg_value | 0x2));

		/*disable dma_dis */
		reg_value = nvt_crypto_read(dev, NVT_CRYPTO_AXIBUS_CTR_REG);
		reg_value = reg_value & (~(0x1 << 16));
		nvt_crypto_write(dev, NVT_CRYPTO_AXIBUS_CTR_REG, reg_value);

		/* set interrupt enable */
		reg_value = nvt_crypto_read(dev, NVT_CRYPTO_INT_ENB_REG);
		nvt_crypto_write(dev, NVT_CRYPTO_INT_ENB_REG, (reg_value | (dma_enb << 4)));

		/* clear interrupt status */
		nvt_crypto_write(dev, NVT_CRYPTO_INT_STS_REG, (dma_enb << 4));
		...
		for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
			...
			nvt_crypto_write(dev, NVT_CRYPTO_DMA0_ADDR_REG + (i * 4), DMA_ADDR_LO(dev->dma_ch[i].desc_paddr));
#ifdef NVT_CRYPTO_DMA_36BIT_SUPPORT
			nvt_crypto_write(dev, NVT_CRYPTO_DMA0_ADDR_H_REG + (i * 4), DMA_ADDR_HI(dev->dma_ch[i].desc_paddr));
#endif
		}

		/* set DMA channel enable */
		reg_value  = nvt_crypto_read(dev, NVT_CRYPTO_CTRL_REG);
		reg_value &= ~(0xf << 4);
		reg_value |= (dma_enb << 4);
		nvt_crypto_write(dev, NVT_CRYPTO_CTRL_REG, reg_value);
		...
		for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
			...
			mod_timer(&dev->dma_ch[i].timer, jiffies + msecs_to_jiffies(NVT_CRYPTO_DEFAULT_TIMEOUT));
		}
	}
...
#ifdef NVT_CRYPTO_PIO_SUPPORT
static int nvt_crypto_pio(struct nvt_crypto_dev *dev, NVT_CRYPTO_MODE_T mode, NVT_CRYPTO_TYPE_T type, const u32 *key, const u32 *src, u32 *dst)
{
#define NVT_CRYPTO_PIO_TIMEOUT  3000
	int ret     = 0;
	u32 cnt     = 0;
	u32 reg_value;
	u8  key_cnt;
	struct nvt_ekey_hdr *ekey_hdr = (struct nvt_ekey_hdr *)key;

	if (mode >= NVT_CRYPTO_MODE_MAX || type >= NVT_CRYPTO_TYPE_MAX || !key || !dst) {
		return -EINVAL;
	}
	...
	reg_value = nvt_crypto_read(dev, NVT_CRYPTO_CTRL_REG);
	while ((reg_value & 0x1) && (cnt++ < NVT_CRYPTO_PIO_TIMEOUT)) {
		udelay(1);
		reg_value = nvt_crypto_read(dev, NVT_CRYPTO_CTRL_REG);
	}
	...
	if ((ekey_hdr->magic == NVT_CRYPTO_EKEY_HDR_MAGIC) &&
		...
		switch (mode) {
		case NVT_CRYPTO_MODE_DES:
			nvt_crypto_write(dev, NVT_CRYPTO_KEY0_REG, key[0]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY1_REG, key[1]);
			if (dev->dbg_mode >= 3) {
		...
		case NVT_CRYPTO_MODE_3DES:
			nvt_crypto_write(dev, NVT_CRYPTO_KEY0_REG, key[0]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY1_REG, key[1]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY2_REG, key[2]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY3_REG, key[3]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY4_REG, key[4]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY5_REG, key[5]);
			if (dev->dbg_mode >= 3) {
		...
		case NVT_CRYPTO_MODE_AES_256:
			...
			nvt_crypto_write(dev, NVT_CRYPTO_KEY4_REG, key[4]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY5_REG, key[5]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY6_REG, key[6]);
			nvt_crypto_write(dev, NVT_CRYPTO_KEY7_REG, key[7]);
			if (dev->dbg_mode >= 3) {
	...
	/* set input data */
	if (!src) {
		nvt_crypto_write(dev, NVT_CRYPTO_IN0_REG, 0);
		nvt_crypto_write(dev, NVT_CRYPTO_IN1_REG, 0);
		nvt_crypto_write(dev, NVT_CRYPTO_IN2_REG, 0);
		nvt_crypto_write(dev, NVT_CRYPTO_IN3_REG, 0);
		if (dev->dbg_mode >= 3) {
			dev_info(dev->dev, "PIO In     => %08x %08x %08x %08x\n", 0, 0, 0, 0);
		}
	} else {
		if (mode == NVT_CRYPTO_MODE_DES || mode == NVT_CRYPTO_MODE_3DES) {
			nvt_crypto_write(dev, NVT_CRYPTO_IN0_REG, src[0]);
			nvt_crypto_write(dev, NVT_CRYPTO_IN1_REG, src[1]);
			nvt_crypto_write(dev, NVT_CRYPTO_IN2_REG, 0);
			nvt_crypto_write(dev, NVT_CRYPTO_IN3_REG, 0);
			if (dev->dbg_mode >= 3) {
				dev_info(dev->dev, "PIO In     => %08x %08x %08x %08x\n", src[0], src[1], 0, 0);
			...
			nvt_crypto_write(dev, NVT_CRYPTO_IN0_REG, src[0]);
			nvt_crypto_write(dev, NVT_CRYPTO_IN1_REG, src[1]);
			nvt_crypto_write(dev, NVT_CRYPTO_IN2_REG, src[2]);
			nvt_crypto_write(dev, NVT_CRYPTO_IN3_REG, src[3]);
			if (dev->dbg_mode >= 3) {
	...
	nvt_crypto_write(dev, NVT_CRYPTO_CFG_REG, reg_value);

	/* clear PIO status */
	nvt_crypto_write(dev, NVT_CRYPTO_INT_STS_REG, 0x1);

	/* trigger PIO mode */
	nvt_crypto_write(dev, NVT_CRYPTO_CTRL_REG, 0x1);

	/* polling status */
	cnt = 0;
	reg_value = nvt_crypto_read(dev, NVT_CRYPTO_INT_STS_REG);
	while (((reg_value & 0x1) == 0) && (cnt++ < NVT_CRYPTO_PIO_TIMEOUT)) {
		udelay(1);
		reg_value = nvt_crypto_read(dev, NVT_CRYPTO_INT_STS_REG);
	}
	...
	if (reg_value & 0x1) {
		...
		/* read output data */
		if (mode == NVT_CRYPTO_MODE_DES || mode == NVT_CRYPTO_MODE_3DES) {
			dst[0] = nvt_crypto_read(dev, NVT_CRYPTO_OUT0_REG);
			dst[1] = nvt_crypto_read(dev, NVT_CRYPTO_OUT1_REG);
			if (dev->dbg_mode >= 3) {
			...
			dst[0] = nvt_crypto_read(dev, NVT_CRYPTO_OUT0_REG);
			dst[1] = nvt_crypto_read(dev, NVT_CRYPTO_OUT1_REG);
			dst[2] = nvt_crypto_read(dev, NVT_CRYPTO_OUT2_REG);
			dst[3] = nvt_crypto_read(dev, NVT_CRYPTO_OUT3_REG);
			if (dev->dbg_mode >= 3) {
...
static int nvt_crypto_cra_init(struct crypto_skcipher *tfm)
{
	struct nvt_crypto_ctx *ctx = crypto_skcipher_ctx(tfm);
	const char *name = crypto_tfm_alg_name(&tfm->base);

	ctx->fallback_u.skcipher = crypto_alloc_skcipher(name, 0, CRYPTO_ALG_NEED_FALLBACK);
	...
	if (ctx->fallback_u.skcipher) {
		crypto_skcipher_set_reqsize(tfm, sizeof(struct skcipher_request) + crypto_skcipher_reqsize(ctx->fallback_u.skcipher));
	} else {
...
static void nvt_crypto_cra_exit(struct crypto_skcipher *tfm)
{
	struct nvt_crypto_ctx *ctx = crypto_skcipher_ctx(tfm);
...
static int nvt_crypto_cra_aes_gcm_init(struct crypto_aead *tfm)
{
	struct nvt_crypto_ctx *ctx     = crypto_aead_ctx(tfm);
	...
	if (ctx->fallback_u.aead) {
		crypto_aead_set_reqsize(tfm, max(sizeof(struct nvt_crypto_gcm_reqctx), (sizeof(struct aead_request) + crypto_aead_reqsize(ctx->fallback_u.aead))));
	} else {
		crypto_aead_set_reqsize(tfm, sizeof(struct nvt_crypto_gcm_reqctx));
	}

#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
#ifndef NVT_CRYPTO_PIO_SUPPORT
...
static void nvt_crypto_cra_aes_gcm_exit(struct crypto_aead *tfm)
{
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
...
static int nvt_crypto_cra_aes_ccm_init(struct crypto_aead *tfm)
{
	struct nvt_crypto_ctx *ctx     = crypto_aead_ctx(tfm);
	struct crypto_tfm         *org_tfm = crypto_aead_tfm(tfm);
	struct crypto_alg         *alg     = org_tfm->__crt_alg;
	...
	if (ctx->fallback_u.aead) {
		crypto_aead_set_reqsize(tfm, max(sizeof(struct nvt_crypto_ccm_reqctx), (sizeof(struct aead_request) + crypto_aead_reqsize(ctx->fallback_u.aead))));
	} else {
		crypto_aead_set_reqsize(tfm, sizeof(struct nvt_crypto_ccm_reqctx));
	}

	return 0;
}

static void nvt_crypto_cra_aes_ccm_exit(struct crypto_aead *tfm)
{
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
...
static int nvt_crypto_aes_do_fallback(struct skcipher_request *req, bool is_encrypt)
{
	struct crypto_skcipher    *cipher = crypto_skcipher_reqtfm(req);
	struct crypto_tfm         *tfm = crypto_skcipher_tfm(cipher);
	struct nvt_crypto_ctx *ctx = crypto_tfm_ctx(tfm);
...
static int nvt_crypto_aes_aead_do_fallback(struct aead_request *req, bool is_encrypt)
{
...
static int nvt_crypto_aes_setkey(struct crypto_skcipher *tfm, const u8 *key, unsigned int keylen)
{
	int err;
	struct nvt_crypto_ctx *ctx = crypto_skcipher_ctx(tfm);
	...
	if (unlikely(keylen != AES_KEYSIZE_128 && keylen != AES_KEYSIZE_256)) {
		...
		if (ctx->fallback_u.skcipher) {
			...
			crypto_skcipher_set_flags(ctx->fallback_u.skcipher, crypto_skcipher_get_flags(tfm)&CRYPTO_TFM_REQ_MASK);
			err = crypto_skcipher_setkey(ctx->fallback_u.skcipher, key, keylen);
			if (err) {
				crypto_skcipher_clear_flags(tfm, CRYPTO_TFM_REQ_MASK);
				crypto_skcipher_set_flags(tfm, crypto_skcipher_get_flags(ctx->fallback_u.skcipher)&CRYPTO_TFM_REQ_MASK);
				return err;
			}
		} else {
...
static void nvt_crypto_aes_gcm_copy_hash(struct aead_request *req)
{
	struct crypto_aead               *tfm      = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *pctx     = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx   = aead_request_ctx(req);
...
static int nvt_crypto_aes_gcm_verify_hash(struct aead_request *req)
{
	struct crypto_aead               *tfm       = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *pctx      = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx    = aead_request_ctx(req);
...
static void nvt_crypto_aes_gcm_encrypt_hash_done(struct aead_request *req, int err)
{
	if (!err) {
		nvt_crypto_aes_gcm_copy_hash(req);
	}

	aead_request_complete(req, err);
}

static void nvt_crypto_aes_gcm_decrypt_hash_done(struct aead_request *req, int err)
{
	struct crypto_aead        *tfm  = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx *pctx = crypto_aead_ctx(tfm);

	if (!err) {
		nvt_crypto_handle_req(pctx->dev, &req->base, 0);    ///< ghash complete then issue hardware to do gcm decryption
	} else {
		aead_request_complete(req, err);
	}
}

static int nvt_crypto_aes_gcm_hash_update(struct aead_request *req, crypto_completion_t compl, struct scatterlist *src, unsigned int len)
{
	struct crypto_aead        *tfm   = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx *pctx  = crypto_aead_ctx(tfm);
...
static int nvt_crypto_aes_gcm_hash_remain(struct aead_request *req, unsigned int remain, crypto_completion_t compl)
{
	struct crypto_aead               *tfm    = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *pctx   = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx = aead_request_ctx(req);
	...
	ahash_request_set_callback(ahreq, aead_request_flags(req), compl, req);
	sg_init_one(&reqctx->ghash_src, nvt_crypto_aes_zeroes, remain);
...
static void nvt_crypto_aes_gcm_hash_final_done(struct crypto_async_request *areq, int err)
{
...
static int nvt_crypto_aes_gcm_hash_final(struct aead_request *req)
{
	struct crypto_aead               *tfm       = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *pctx      = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx    = aead_request_ctx(req);
	...
	ahash_request_set_callback(ahreq, aead_request_flags(req), nvt_crypto_aes_gcm_hash_final_done, req);
...
{
	if (!err) {
		err = nvt_crypto_aes_gcm_hash_final(req);
...
static void nvt_crypto_aes_gcm_hash_len_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static int nvt_crypto_aes_gcm_hash_len(struct aead_request *req)
{
	struct crypto_aead               *tfm    = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *pctx   = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx = aead_request_ctx(req);
	struct ahash_request             *ahreq  = pctx->ghash_req;
	struct nvt_crypto_ghash_ctx  *gctx   = &reqctx->ghash_ctx;
	u128                             lengths;
	...
	ahash_request_set_callback(ahreq, aead_request_flags(req), nvt_crypto_aes_gcm_hash_len_done, req);
...
{
	if (!err) {
		err = nvt_crypto_aes_gcm_hash_len(req);
		if (err == -EINPROGRESS || err == -EBUSY) {
			return;
		}
	}
...
static void nvt_crypto_aes_gcm_hash_crypt_remain_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static void __gcm_hash_crypt_done(struct aead_request *req, int err)
{
	...
	if (!err) {
		...
		BUG_ON(!remain);
		err = nvt_crypto_aes_gcm_hash_remain(req, remain, nvt_crypto_aes_gcm_hash_crypt_remain_done);
		if (err == -EINPROGRESS || err == -EBUSY) {
			return;
		}
	}
...
static void nvt_crypto_aes_gcm_hash_crypt_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static void __gcm_hash_assoc_remain_done(struct aead_request *req, int err)
{
	...
	if (!err && gctx->cryptlen) {
		remain = __gcm_remain(gctx->cryptlen);
		compl = remain ? nvt_crypto_aes_gcm_hash_crypt_done : nvt_crypto_aes_gcm_hash_crypt_remain_done;
		err   = nvt_crypto_aes_gcm_hash_update(req, compl, gctx->src, gctx->cryptlen);
		if (err == -EINPROGRESS || err == -EBUSY) {
			return;
		}
	}
...
static void nvt_crypto_aes_gcm_hash_assoc_remain_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static void __gcm_hash_assoc_done(struct aead_request *req, int err)
{
	...
	if (!err) {
		...
		BUG_ON(!remain);
		err = nvt_crypto_aes_gcm_hash_remain(req, remain, nvt_crypto_aes_gcm_hash_assoc_remain_done);
		if (err == -EINPROGRESS || err == -EBUSY) {
			return;
		}
	}
...
static void nvt_crypto_aes_gcm_hash_assoc_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static void __gcm_hash_init_done(struct aead_request *req, int err)
{
	...
	if (!err && req->assoclen) {
		remain = __gcm_remain(req->assoclen);
		compl  = remain ? nvt_crypto_aes_gcm_hash_assoc_done : nvt_crypto_aes_gcm_hash_assoc_remain_done;
		err    = nvt_crypto_aes_gcm_hash_update(req, compl, req->src, req->assoclen);
		if (err == -EINPROGRESS || err == -EBUSY) {
			return;
		}
	}
...
static void nvt_crypto_aes_gcm_hash_init_done(struct crypto_async_request *areq, int err)
{
	struct aead_request *req = (struct aead_request *)areq->data;
...
static int nvt_crypto_aes_gcm_hash(struct aead_request *req, struct nvt_crypto_ctx *pctx)
{
	struct ahash_request             *ahreq  = pctx->ghash_req;
	struct nvt_crypto_gcm_reqctx *reqctx = aead_request_ctx(req);
	struct nvt_crypto_ghash_ctx  *gctx   = &reqctx->ghash_ctx;
	...
	/* hash init */
	ahash_request_set_callback(ahreq, aead_request_flags(req), nvt_crypto_aes_gcm_hash_init_done, req);
...
static int nvt_crypto_aes_gcm_setkey(struct crypto_aead *tfm, const u8 *key, unsigned int keylen)
{
	int err;
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
	...
	if (ctx->cipher) {
		...
		crypto_cipher_encrypt_one(ctx->cipher, (u8 *)ctx->key, nvt_crypto_aes_zeroes);
	}
#else
	/* use PIO mode to create gcm ghash key with zero pattern */
	memcpy(ctx->key, key, keylen);
	err = nvt_crypto_pio(nvt_cdev, ((keylen == AES_KEYSIZE_128) ? NVT_CRYPTO_MODE_AES_128 : NVT_CRYPTO_MODE_AES_256), NVT_CRYPTO_TYPE_ENCRYPT, ctx->key, NULL, ctx->key);
	if (err) {
		return err;
	}
#endif
...
static int nvt_crypto_aes_gcm_setauthsize(struct crypto_aead *tfm, unsigned int authsize)
{
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
...
static void nvt_crypto_aes_gcm_complete(struct nvt_crypto_dev *dev, NVT_CRYPTO_DMA_CH_T ch, int err)
{
	struct aead_request              *aead_req;
	struct nvt_crypto_gcm_reqctx *reqctx;
	...
	if (err) {
		...
		if (reqctx->type == NVT_CRYPTO_TYPE_ENCRYPT) {
			unsigned long flags;
			...
			err = nvt_crypto_aes_gcm_verify_hash(aead_req);
			aead_request_complete(aead_req, err);
		}
#else
		...
		if (reqctx->type == NVT_CRYPTO_TYPE_ENCRYPT) {
			...
			nvt_crypto_aes_gcm_copy_hash(aead_req);
		} else {
			err = nvt_crypto_aes_gcm_verify_hash(aead_req);
		}
		aead_request_complete(aead_req, err);
...
static void nvt_crypto_aes_ccm_complete(struct nvt_crypto_dev *dev, NVT_CRYPTO_DMA_CH_T ch, int err)
{
	struct crypto_aead               *tfm;
	struct aead_request              *aead_req;
	struct nvt_crypto_ccm_reqctx *reqctx;
	unsigned long flags;
	...
	if (err) {
		...
		if (reqctx->ccm_type == NVT_CRYPTO_CCM_TYPE_ENCRYPT) {
			if (reqctx->opmode == NVT_CRYPTO_OPMODE_CBC) {
				/* copy tag */
				...
				reqctx->type   = NVT_CRYPTO_TYPE_ENCRYPT;
				reqctx->opmode = NVT_CRYPTO_OPMODE_CTR;
				reqctx->get_s0 = 1;
				spin_lock_irqsave(&dev->queue_lock, flags);
		...
		} else if (reqctx->ccm_type == NVT_CRYPTO_CCM_TYPE_DECRYPT) {
			if (reqctx->opmode == NVT_CRYPTO_OPMODE_CTR) {
				/* copy S0 */
				memcpy(reqctx->auth_tag, (u8 *)(dev->dma_ch[ch].desc)->s0, AES_BLOCK_SIZE);
				...
				reqctx->type   = NVT_CRYPTO_TYPE_ENCRYPT;
				reqctx->opmode = NVT_CRYPTO_OPMODE_CBC;
				reqctx->get_s0 = 0;
				spin_lock_irqsave(&dev->queue_lock, flags);
...
static int nvt_crypto_aes_ccm_setkey(struct crypto_aead *tfm, const u8 *key, unsigned int keylen)
{
	int err;
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
...
static int nvt_crypto_aes_ccm_setauthsize(struct crypto_aead *tfm, unsigned int authsize)
{
	struct nvt_crypto_ctx *ctx = crypto_aead_ctx(tfm);
...
static void nvt_crypto_queue_tasklet(unsigned long data)
{
	struct nvt_crypto_dev   *dev = (struct nvt_crypto_dev *)data;
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		if (dev->dma_ch[i].state != NVT_CRYPTO_STATE_IDLE) {
			continue;
		}
		...
		dev->dma_ch[i].state = NVT_CRYPTO_STATE_START;
...
exit:
	...
	if (req_cnt) {
		nvt_crypto_start(dev);
	}
}

static void nvt_crypto_done_tasklet(unsigned long data)
{
	struct nvt_crypto_dev        *dev = (struct nvt_crypto_dev *)data;
	struct skcipher_request          *sk_req;
	struct aead_request              *aead_req;
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		...
		if (dma_ch->state != NVT_CRYPTO_STATE_DONE) {
			continue;
		}
		...
		if (crypto_tfm_alg_type(dma_ch->req->tfm) == CRYPTO_ALG_TYPE_AEAD) {
			...
			ivsize     = crypto_skcipher_ivsize(cipher);
			iv         = sk_req->iv;
			req_src_sg = sk_req->src;
			req_dst_sg = sk_req->dst;
		}
		align_chk = (reqctx->opmode == NVT_CRYPTO_OPMODE_GCM || reqctx->opmode == NVT_CRYPTO_OPMODE_CTR || reqctx->opmode == NVT_CRYPTO_OPMODE_CFB) ? 0 : 1;
		...
		if (!dma_ch->sg_src_work && (dma_ch->src_copied == 2)) {
			...
			if ((dma_ch->src_total < dma_ch->req_nbytes) && req_src_sg) {
				if (dma_ch->req_nbytes >= (dma_ch->src_total + tbuf_size)) {
					if (dev->dbg_mode >= 3) {
						dev_info(dev->dev, "crypto dma#%d copy %d bytes from original offset %u to temp buffer\n", i, tbuf_size, (u32)dma_ch->src_total);
					}
					scatterwalk_map_and_copy(dma_ch->tbuf_src, req_src_sg, dma_ch->src_total, tbuf_size, 0);
					sg_init_one(&dma_ch->sg_src_cpy, dma_ch->tbuf_src, tbuf_size);
				} else {
					if (dev->dbg_mode >= 3) {
						dev_info(dev->dev, "crypto dma#%d copy %d bytes from original offset %u to temp buffer\n", i, (int)(dma_ch->req_nbytes - dma_ch->src_total), (u32)dma_ch->src_total);
					}
		...
		if (reqctx->opmode != NVT_CRYPTO_OPMODE_ECB) {
			switch (reqctx->mode) {
			case NVT_CRYPTO_MODE_DES:
			case NVT_CRYPTO_MODE_3DES:
				for (j = 0; j < 100; j++) {
...
completed:
			...
			if (dma_ch->dst_copied == 1) {
				if (dev->dbg_mode >= 3) {
					dev_info(dev->dev, "crypto dma#%d copy %u bytes back to original buffer\n", i, (u32)dma_ch->dst_total);
				}
			...
			if (reqctx->ccm_type) {
				nvt_crypto_aes_ccm_complete(dev, i, err);
			} else if (reqctx->opmode == NVT_CRYPTO_OPMODE_GCM) {
				nvt_crypto_aes_gcm_complete(dev, i, err);
			} else {
				/* next IV copy back */
				if (iv && ivsize) {
...
#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
static void nvt_crypto_hash_tasklet(unsigned long data)
{
	struct nvt_crypto_dev        *dev = (struct nvt_crypto_dev *)data;
	struct crypto_aead               *tfm;
	...
	err = nvt_crypto_aes_gcm_hash(aead_req, ctx);
	if (err == -EINPROGRESS || err == -EBUSY) {
		return;
	}
	...
	if (!err) {
		if (reqctx->type == NVT_CRYPTO_TYPE_ENCRYPT) {
			...
			nvt_crypto_handle_req(dev, async_req, 0);
		}
	} else {
		aead_request_complete(aead_req, err);
	}
...
static void nvt_crypto_payload_tasklet(unsigned long data)
{
	struct nvt_crypto_dev        *dev = (struct nvt_crypto_dev *)data;
	struct crypto_aead               *tfm;
	struct crypto_async_request      *async_req;
...
static int nvt_crypto_aes_ecb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher       *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
	...
	if (unlikely(ctx->keylen != AES_KEYSIZE_128 && ctx->keylen != AES_KEYSIZE_256)) {
		return nvt_crypto_aes_do_fallback(req, 1);
	}
	...
	/* setup operation */
	reqctx->type     = NVT_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode     = (ctx->keylen == AES_KEYSIZE_256) ? NVT_CRYPTO_MODE_AES_256 : NVT_CRYPTO_MODE_AES_128;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_ECB;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
	reqctx->ccm_type = NVT_CRYPTO_CCM_TYPE_NONE;

	return nvt_crypto_handle_req(ctx->dev, &req->base, 0);
}

static int nvt_crypto_aes_ecb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher       *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
	...
	if (unlikely(ctx->keylen != AES_KEYSIZE_128 && ctx->keylen != AES_KEYSIZE_256)) {
		return nvt_crypto_aes_do_fallback(req, 0);
	}
	...
	/* setup operation */
	reqctx->type     = NVT_CRYPTO_TYPE_DECRYPT;
	reqctx->mode     = (ctx->keylen == AES_KEYSIZE_256) ? NVT_CRYPTO_MODE_AES_256 : NVT_CRYPTO_MODE_AES_128;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_ECB;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
...
static int nvt_crypto_aes_cbc_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_aes_cbc_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_aes_cfb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
	...
	reqctx->type     = NVT_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode     = (ctx->keylen == AES_KEYSIZE_256) ? NVT_CRYPTO_MODE_AES_256 : NVT_CRYPTO_MODE_AES_128;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_CFB;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
	reqctx->ccm_type = NVT_CRYPTO_CCM_TYPE_NONE;
...
static int nvt_crypto_aes_cfb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
#ifdef CONFIG_CRYPTO_OFB
static int nvt_crypto_aes_ofb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
	...
	reqctx->type     = NVT_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode     = (ctx->keylen == AES_KEYSIZE_256) ? NVT_CRYPTO_MODE_AES_256 : NVT_CRYPTO_MODE_AES_128;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_OFB;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
	reqctx->ccm_type = NVT_CRYPTO_CCM_TYPE_NONE;
...
static int nvt_crypto_aes_ofb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_aes_ctr_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_aes_ctr_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_aes_gcm_encrypt(struct aead_request *req)
{
	struct crypto_aead               *tfm    = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *ctx    = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx = aead_request_ctx(req);
	...
	if (unlikely(ctx->keylen != AES_KEYSIZE_128 && ctx->keylen != AES_KEYSIZE_256)) {
		return nvt_crypto_aes_aead_do_fallback(req, 1);
	}
	...
	reqctx->type     = NVT_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode     = (ctx->keylen == AES_KEYSIZE_256) ? NVT_CRYPTO_MODE_AES_256 : NVT_CRYPTO_MODE_AES_128;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_GCM;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
	reqctx->ccm_type = NVT_CRYPTO_CCM_TYPE_NONE;
	...
	reqctx->ghash_ctx.complete = nvt_crypto_aes_gcm_encrypt_hash_done;
#endif

	return nvt_crypto_handle_req(ctx->dev, &req->base, 0);
}

static int nvt_crypto_aes_gcm_decrypt(struct aead_request *req)
{
	struct crypto_aead               *tfm     = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *ctx     = crypto_aead_ctx(tfm);
	struct nvt_crypto_gcm_reqctx *reqctx  = aead_request_ctx(req);
	...
	if (unlikely(ctx->keylen != AES_KEYSIZE_128 && ctx->keylen != AES_KEYSIZE_256)) {
		return nvt_crypto_aes_aead_do_fallback(req, 0);
	}
	...
	reqctx->ghash_ctx.complete = nvt_crypto_aes_gcm_decrypt_hash_done;

	return nvt_crypto_handle_req(ctx->dev, &req->base, 1);  ///< 1 means to issue to do software ghash first
#else
	return nvt_crypto_handle_req(ctx->dev, &req->base, 0);
#endif
}

static int nvt_crypto_aes_ccm_encrypt(struct aead_request *req)
{
	struct crypto_aead               *tfm    = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *ctx    = crypto_aead_ctx(tfm);
	struct nvt_crypto_ccm_reqctx *reqctx = aead_request_ctx(req);
	int                              err;
	...
	reqctx->key_src      = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0       = 0;
	reqctx->ccm_type     = NVT_CRYPTO_CCM_TYPE_ENCRYPT;
	reqctx->sg_src       = scatterwalk_ffwd(reqctx->src, req->src, req->assoclen);
	reqctx->sg_dst       = (req->src == req->dst) ? reqctx->sg_src : scatterwalk_ffwd(reqctx->dst, req->dst, req->assoclen);
	...
	return nvt_crypto_handle_req(ctx->dev, &req->base, 2);
}

static int nvt_crypto_aes_ccm_decrypt(struct aead_request *req)
{
	struct crypto_aead               *tfm     = crypto_aead_reqtfm(req);
	struct nvt_crypto_ctx        *ctx     = crypto_aead_ctx(tfm);
	struct nvt_crypto_ccm_reqctx *reqctx  = aead_request_ctx(req);
	...
	reqctx->key_src      = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0       = 1;
	reqctx->ccm_type     = NVT_CRYPTO_CCM_TYPE_DECRYPT;
	reqctx->sg_src       = scatterwalk_ffwd(reqctx->src, req->src, req->assoclen);
	reqctx->sg_dst       = (req->src == req->dst) ? reqctx->sg_src : scatterwalk_ffwd(reqctx->dst, req->dst, req->assoclen);
...
static int nvt_crypto_des_setkey(struct crypto_skcipher *tfm, const u8 *key, unsigned int keylen)
{
	struct nvt_crypto_ctx *ctx = crypto_skcipher_ctx(tfm);
	int err;
...
static int nvt_crypto_des_ecb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
	...
	/* setup operation */
	reqctx->type     = NVT_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode     = (ctx->keylen == DES_KEY_SIZE) ? NVT_CRYPTO_MODE_DES : NVT_CRYPTO_MODE_3DES;
	reqctx->opmode   = NVT_CRYPTO_OPMODE_ECB;
	reqctx->key_src  = NVT_CRYPTO_KEY_SRC_DESC0;
	reqctx->get_s0   = 0;
	reqctx->ccm_type = NVT_CRYPTO_CCM_TYPE_NONE;
...
static int nvt_crypto_des_ecb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_cbc_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_cbc_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_cfb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_cfb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
#ifdef CONFIG_CRYPTO_OFB
static int nvt_crypto_des_ofb_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_ofb_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_ctr_encrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static int nvt_crypto_des_ctr_decrypt(struct skcipher_request *req)
{
	struct crypto_skcipher     *tfm    = crypto_skcipher_reqtfm(req);
	struct nvt_crypto_ctx    *ctx    = crypto_skcipher_ctx(tfm);
	struct nvt_crypto_reqctx *reqctx = skcipher_request_ctx(req);
...
static irqreturn_t nvt_crypto_irq(int irq, void *dev_id)
{
	int i, dma_done = 0;
	struct nvt_crypto_dev *dev = (struct nvt_crypto_dev *)dev_id;
	u32                       status;

	spin_lock(&dev->lock);

	/* read DMA status */
	status = nvt_crypto_read(dev, NVT_CRYPTO_INT_STS_REG);

	/* clear DMA status, not clear PIO status */
	nvt_crypto_write(dev, NVT_CRYPTO_INT_STS_REG, status & (~0x1));

	/* DMA channel transfer done */
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		if (status & (0x1 << (i + 4))) {
			del_timer(&dev->dma_ch[i].timer);
			dev->dma_ch[i].state = NVT_CRYPTO_STATE_DONE;
			dma_done++;
		}
	}
...
static struct skcipher_alg nvt_crypto_algs[] = {
	{
		.base.cra_name = "ecb(aes)",
		.base.cra_driver_name = "nvt-ecb-aes",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags = CRYPTO_ALG_TYPE_AKCIPHER | CRYPTO_ALG_ASYNC |
		CRYPTO_ALG_KERN_DRIVER_ONLY |
		CRYPTO_ALG_NEED_FALLBACK,
		.base.cra_blocksize = AES_BLOCK_SIZE,
		.base.cra_ctxsize = sizeof(struct nvt_crypto_ctx),
		.base.cra_alignmask = 0x3,

		.base.cra_module = THIS_MODULE,
		.init = nvt_crypto_cra_init,
		.exit = nvt_crypto_cra_exit,
		.min_keysize = AES_MIN_KEY_SIZE,
		.max_keysize = AES_MAX_KEY_SIZE,
		.ivsize = AES_BLOCK_SIZE,
		.setkey = nvt_crypto_aes_setkey,
		.encrypt = nvt_crypto_aes_ecb_encrypt,
		.decrypt = nvt_crypto_aes_ecb_decrypt,
	},
	{
		.base.cra_name = "cbc(aes)",
		.base.cra_driver_name = "nvt-cbc-aes",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		...
		.ivsize = AES_BLOCK_SIZE,
		.setkey = nvt_crypto_aes_setkey,
		.encrypt = nvt_crypto_aes_cbc_encrypt,
		.decrypt = nvt_crypto_aes_cbc_decrypt,

	},
	{
		.base.cra_name = "cfb(aes)",
		.base.cra_driver_name = "nvt-cfb-aes",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags = CRYPTO_ALG_ASYNC |
		CRYPTO_ALG_KERN_DRIVER_ONLY |
		CRYPTO_ALG_NEED_FALLBACK,
		...
		.ivsize = AES_BLOCK_SIZE,
		.setkey = nvt_crypto_aes_setkey,
		.encrypt = nvt_crypto_aes_cfb_encrypt,
		.decrypt = nvt_crypto_aes_cfb_decrypt,

	},
#ifdef CONFIG_CRYPTO_OFB
	{
		.base.cra_name = "ofb(aes)",
		.base.cra_driver_name = "nvt-ofb-aes",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags = CRYPTO_ALG_ASYNC |
		CRYPTO_ALG_KERN_DRIVER_ONLY |
		CRYPTO_ALG_NEED_FALLBACK,
		...
		.ivsize = AES_BLOCK_SIZE,
		.setkey = nvt_crypto_aes_setkey,
		.encrypt = nvt_crypto_aes_ofb_encrypt,
		.decrypt = nvt_crypto_aes_ofb_decrypt,

	},
#endif
	{
		.base.cra_name = "ctr(aes)",
		.base.cra_driver_name = "nvt-ctr-aes",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags = CRYPTO_ALG_ASYNC |
		CRYPTO_ALG_KERN_DRIVER_ONLY |
		CRYPTO_ALG_NEED_FALLBACK,
		...
		.ivsize = AES_BLOCK_SIZE,
		.setkey = nvt_crypto_aes_setkey,
		.encrypt = nvt_crypto_aes_ctr_encrypt,
		.decrypt = nvt_crypto_aes_ctr_decrypt,

	},
	{
		.base.cra_name = "ecb(des)",
		.base.cra_driver_name = "nvt-ecb-des",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		...
		.setkey = nvt_crypto_des_setkey,
		.encrypt = nvt_crypto_des_ecb_encrypt,
		.decrypt = nvt_crypto_des_ecb_decrypt,

	},
	{
		.base.cra_name = "cbc(des)",
		.base.cra_driver_name = "nvt-cbc-des",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		...
		.setkey = nvt_crypto_des_setkey,
		.encrypt = nvt_crypto_des_cbc_encrypt,
		.decrypt = nvt_crypto_des_cbc_decrypt,

	},
	{
		.base.cra_name = "cfb(des)",
		.base.cra_driver_name = "nvt-cfb-des",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		...
		.ivsize = DES_BLOCK_SIZE,
		.setkey = nvt_crypto_des_setkey,
		.encrypt = nvt_crypto_des_cfb_encrypt,
		.decrypt = nvt_crypto_des_cfb_decrypt,
	},
#ifdef CONFIG_CRYPTO_OFB
	{
		.base.cra_name = "ofb(des)",
		.base.cra_driver_name = "nvt-ofb-des",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		...
		.ivsize = DES_BLOCK_SIZE,
		.setkey = nvt_crypto_des_setkey,
		.encrypt = nvt_crypto_des_ofb_encrypt,
		.decrypt = nvt_crypto_des_ofb_decrypt,
	},
#endif
	{
		.base.cra_name = "ctr(des)",
		.base.cra_driver_name = "nvt-ctr-des",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
		...
		.ivsize = DES_BLOCK_SIZE,
		.setkey = nvt_crypto_des_setkey,
		.encrypt = nvt_crypto_des_ctr_encrypt,
		.decrypt = nvt_crypto_des_ctr_decrypt,

	},
	{
		.base.cra_name = "ecb(des3_ede)",
		.base.cra_driver_name = "nvt-ecb-des3",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
	...
	},
	{
		...
		.base.cra_driver_name = "nvt-cbc-des3",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
	...
	{
		.base.cra_name = "cfb(des3_ede)",
		.base.cra_driver_name = "nvt-cfb-des3",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
...
#ifdef CONFIG_CRYPTO_OFB
	{
		...
		.base.cra_driver_name = "nvt-ofb-des3",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
...
#endif
	{
		...
		.base.cra_driver_name = "nvt-ctr-des3",
		.base.cra_priority = NVT_CRYPTO_ALG_PRIORITY,
		.base.cra_flags =
		CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY,
		.base.cra_blocksize = DES_BLOCK_SIZE,
...
static struct aead_alg nvt_aead_algs[] = {
	{
		.base = {
			.cra_name           = "gcm(aes)",
			.cra_driver_name    = "nvt-gcm-aes",
			.cra_priority       = NVT_CRYPTO_ALG_PRIORITY,
			.cra_flags          = CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY | CRYPTO_ALG_NEED_FALLBACK,
			.cra_blocksize      = 1,
			.cra_ctxsize        = sizeof(struct nvt_crypto_ctx),
			.cra_alignmask      = 0x3,
			.cra_module         = THIS_MODULE,
		},
		.init        = nvt_crypto_cra_aes_gcm_init,
		.exit        = nvt_crypto_cra_aes_gcm_exit,
		.ivsize      = 12,
		.maxauthsize = AES_BLOCK_SIZE,
		.setkey      = nvt_crypto_aes_gcm_setkey,
		.setauthsize = nvt_crypto_aes_gcm_setauthsize,
		.encrypt     = nvt_crypto_aes_gcm_encrypt,
		.decrypt     = nvt_crypto_aes_gcm_decrypt,
	},
	{
		.base = {
			.cra_name           = "ccm(aes)",
			.cra_driver_name    = "nvt-ccm-aes",
			.cra_priority       = NVT_CRYPTO_ALG_PRIORITY,
			.cra_flags          = CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY | CRYPTO_ALG_NEED_FALLBACK,
			.cra_blocksize      = 1,
			.cra_ctxsize        = sizeof(struct nvt_crypto_ctx),
		...
		.init        = nvt_crypto_cra_aes_ccm_init,
		.exit        = nvt_crypto_cra_aes_ccm_exit,
		.ivsize      = AES_BLOCK_SIZE,
		.maxauthsize = AES_BLOCK_SIZE,
		.setkey      = nvt_crypto_aes_ccm_setkey,
		.setauthsize = nvt_crypto_aes_ccm_setauthsize,
		.encrypt     = nvt_crypto_aes_ccm_encrypt,
		.decrypt     = nvt_crypto_aes_ccm_decrypt,
	}
};

static int nvt_crypto_dma_buf_alloc(struct nvt_crypto_dev *dev)
{
	int ret = 0;
	...
	dev->dma_buf.vaddr = dma_alloc_coherent(dev->dev, sizeof(struct nvt_crypto_dma_desc) * NVT_CRYPTO_DMA_CH_MAX, &dev->dma_buf.paddr, GFP_DMA | GFP_KERNEL);

	if (!dev->dma_buf.vaddr) {
		ret = -ENOMEM;
		goto exit;
	}
	dev->dma_buf.size = sizeof(struct nvt_crypto_dma_desc) * NVT_CRYPTO_DMA_CH_MAX;

	/* check DMA descriptor buffer address */
	if (((dev->dma_buf.paddr + dev->dma_buf.size) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
		dev_err(dev->dev, "crypto allocated dma descriptor address:0x%08lx not support\n", (uintptr_t)dev->dma_buf.paddr);

		dma_free_coherent(dev->dev, dev->dma_buf.size, dev->dma_buf.vaddr, dev->dma_buf.paddr);
...
static void nvt_crypto_dma_buf_free(struct nvt_crypto_dev *dev)
{
	if (dev->dma_buf.vaddr) {
		dma_free_coherent(dev->dev, dev->dma_buf.size, dev->dma_buf.vaddr, dev->dma_buf.paddr);
...
static int nvt_crypto_proc_version_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "Version: %s\n", DRV_VERSION);
	return 0;
}

static int nvt_crypto_proc_dbg_mode_show(struct seq_file *sfile, void *v)
{
	struct nvt_crypto_dev *dev = (struct nvt_crypto_dev *)sfile->private;
...
static int nvt_crypto_proc_param_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "[1]TBuf_Alloc: %s (%d)\n", (tbuf_alloc ? "Buddy" : "None"), tbuf_size);
	seq_printf(sfile, "[2]Align_Mode: %s (%d)\n", (align_mode ? "CACHE_ALIGN" : "DMA_ALIGN"), (align_mode ? NVT_CRYPTO_CACHE_ALIGN_SIZE : NVT_CRYPTO_DMA_ALIGN_SIZE));

	return 0;
}

static ssize_t nvt_crypto_proc_dbg_mode_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	int mode;
	unsigned long flags;
	char value_str[32] = {'\0'};
...
static int nvt_crypto_proc_dump_reg_show(struct seq_file *sfile, void *v)
{
	int i;
	unsigned long flags;
	struct nvt_crypto_dev *dev = (struct nvt_crypto_dev *)sfile->private;
...
static ssize_t nvt_crypto_proc_param_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	int ret;
	...
	if (ret >= 2) {
		switch (param_id) {
		case 2: /* Align Mode */
			if ((param_value >= NVT_CRYPTO_ALIGN_MODE_DMA) && (param_value < NVT_CRYPTO_ALIGN_MODE_MAX)) {
...
static int nvt_crypto_proc_version_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_crypto_proc_version_show, PDE_DATA(file_inode(file)));
}

static int nvt_crypto_proc_dbg_mode_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_crypto_proc_dbg_mode_show, PDE_DATA(file_inode(file)));
}

static int nvt_crypto_proc_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_crypto_proc_param_show, PDE_DATA(file_inode(file)));
}

static int nvt_crypto_proc_dump_reg_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_crypto_proc_dump_reg_show, PDE_DATA(file_inode(file)));
}

static const struct proc_ops nvt_crypto_proc_version_ops = {
	.proc_open = nvt_crypto_proc_version_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops nvt_crypto_proc_dbg_mode_ops = {
	.proc_open = nvt_crypto_proc_dbg_mode_open,
	.proc_write = nvt_crypto_proc_dbg_mode_write,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops nvt_crypto_proc_param_ops = {
	.proc_open = nvt_crypto_proc_param_open,
	.proc_write = nvt_crypto_proc_param_write,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops nvt_crypto_proc_dump_reg_ops = {
	.proc_open = nvt_crypto_proc_dump_reg_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static void nvt_crypto_proc_remove(struct nvt_crypto_dev *dev)
{
	if (dev->proc.root) {
...
static int nvt_crypto_proc_init(struct nvt_crypto_dev *dev)
{
	int  ret = 0;
	...
	/* version */
	dev->proc.version = proc_create_data("version", S_IRUGO | S_IXUGO, dev->proc.root, &nvt_crypto_proc_version_ops, dev);
	...
	/* dbg_mode */
	dev->proc.dbg_mode = proc_create_data("dbg_mode", S_IRUGO | S_IXUGO, dev->proc.root, &nvt_crypto_proc_dbg_mode_ops, dev);
	...
	/* param */
	dev->proc.param = proc_create_data("param", S_IRUGO | S_IXUGO, dev->proc.root, &nvt_crypto_proc_param_ops, dev);
	...
	/* dump_reg */
	dev->proc.dump_reg = proc_create_data("dump_reg", S_IRUGO | S_IXUGO, dev->proc.root, &nvt_crypto_proc_dump_reg_ops, dev);
...
err:
	if (ret < 0) {
		nvt_crypto_proc_remove(dev);
	}

	return ret;
}

static int nvt_crypto_probe(struct platform_device *pdev)
{
	int    ret, i;
	struct resource           *res;
	struct nvt_crypto_dev *dd;
	const char *parent_clk_name = NULL;

	/* check tbuf allocator */
	if ((tbuf_alloc < 0) || (tbuf_alloc >= NVT_CRYPTO_TBUF_ALLOC_MAX)) {
		tbuf_alloc = NVT_CRYPTO_TBUF_ALLOC_NONE;
	}
	...
	dd = nvt_cdev = devm_kzalloc(&pdev->dev, sizeof(struct nvt_crypto_dev), GFP_KERNEL);
	if (!dd) {
		dev_err(&pdev->dev, "unable to alloc device data struct\n");
		return -ENOMEM;
	}

	ret = dma_set_mask_and_coherent(&pdev->dev, (u64)NVT_CRYPTO_DMA_ADDR_MASK);
	if (ret) {
	...
	crypto_init_queue(&dd->queue,         NVT_CRYPTO_QUEUE_LENGTH);
#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
	crypto_init_queue(&dd->hash_queue,    NVT_CRYPTO_QUEUE_LENGTH);
#endif
	crypto_init_queue(&dd->payload_queue, NVT_CRYPTO_QUEUE_LENGTH);
	...
	ret = devm_request_irq(&pdev->dev, dd->irq, nvt_crypto_irq, 0, dev_name(&pdev->dev), dd);
	if (ret) {
	...
	ret = nvt_crypto_dma_buf_alloc(dd);
	if (ret < 0) {
	...
	for (i = 0; i < NVT_CRYPTO_DMA_CH_MAX; i++) {
		dd->dma_ch[i].idx        = i;
		dd->dma_ch[i].desc_paddr = ((dma_addr_t)dd->dma_buf.paddr) + (sizeof(struct nvt_crypto_dma_desc) * i);
		dd->dma_ch[i].desc       = (struct nvt_crypto_dma_desc *)(((uintptr_t)dd->dma_buf.vaddr) + (sizeof(struct nvt_crypto_dma_desc) * i));
		...
		if (tbuf_size) {
			switch (tbuf_alloc) {
			case NVT_CRYPTO_TBUF_ALLOC_BUDDY:
				/* source tbuf */
				...
				} else if ((((uintptr_t)page_to_phys(virt_to_page(dd->dma_ch[i].tbuf_src)) + tbuf_size) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
				...
				} else if ((((uintptr_t)page_to_phys(virt_to_page(dd->dma_ch[i].tbuf_dst)) + tbuf_size) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
				...
				} else if ((((uintptr_t)page_to_phys(virt_to_page(dd->dma_ch[i].tbuf_ass)) + tbuf_size) & (~NVT_CRYPTO_DMA_ADDR_MASK))) {
				...
				break;
			case NVT_CRYPTO_TBUF_ALLOC_NONE:
			default:
				break;
			}
		}
		...
		timer_setup(&dd->dma_ch[i].timer, nvt_crypto_timeout_handler, 0);
	}

	/* init tasklet for device request */
	tasklet_init(&dd->queue_tasklet,   nvt_crypto_queue_tasklet, (unsigned long)dd);
	tasklet_init(&dd->done_tasklet,    nvt_crypto_done_tasklet, (unsigned long)dd);
#ifdef NVT_CRYPTO_GCM_SOFTWARE_GHASH
	tasklet_init(&dd->hash_tasklet,    nvt_crypto_hash_tasklet, (unsigned long)dd);
#endif
	tasklet_init(&dd->payload_tasklet, nvt_crypto_payload_tasklet, (unsigned long)dd);
	...
	for (i = 0; i < ARRAY_SIZE(nvt_crypto_algs); i++) {
		ret = crypto_register_skcipher(&nvt_crypto_algs[i]);
		if (ret) {
	...
	ret = crypto_register_aeads(nvt_aead_algs, ARRAY_SIZE(nvt_aead_algs));
	if (ret) {
...
err:
	...
	nvt_crypto_proc_remove(dd);
	clk_disable_unprepare(dd->clk);
	nvt_crypto_dma_buf_free(dd);
	return ret;
}

static int nvt_crypto_remove(struct platform_device *pdev)
{
	int i;
	struct nvt_crypto_dev *dev = platform_get_drvdata(pdev);
	...
	for (i = 0; i < ARRAY_SIZE(nvt_crypto_algs); i++) {
		crypto_unregister_skcipher(nvt_crypto_algs);
	}

	crypto_unregister_aeads(nvt_aead_algs, ARRAY_SIZE(nvt_aead_algs));
	...
	nvt_crypto_dma_buf_free(dev);

	return 0;
}

#ifdef CONFIG_OF
static const struct of_device_id nvt_crypto_of_match[] = {
	{ .compatible = "nvt,nvt_crypto" },
	{},
};
MODULE_DEVICE_TABLE(of, nvt_crypto_of_match);
#else
#define nvt_crypto_of_match     NULL
#endif

#ifdef CONFIG_PM
static int nvt_crypto_runtime_suspend(struct device *pdev)
{
	struct nvt_crypto_dev *dev = dev_get_drvdata(pdev);
...
static int nvt_crypto_runtime_resume(struct device *pdev)
{
	struct nvt_crypto_dev *dev = dev_get_drvdata(pdev);
	int ret;
...
static const struct dev_pm_ops nvt_crypto_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
	SET_RUNTIME_PM_OPS(nvt_crypto_runtime_suspend,
			   nvt_crypto_runtime_resume, NULL)
};
#endif

static struct platform_driver nvt_crypto_driver = {
	.probe  = nvt_crypto_probe,
	.remove = nvt_crypto_remove,
	.driver = {
		.name   = "nvt_crypto",
#ifdef CONFIG_PM
		.pm		= &nvt_crypto_pm_ops,
#endif
		.of_match_table = nvt_crypto_of_match,
	},
};

module_platform_driver(nvt_crypto_driver);
