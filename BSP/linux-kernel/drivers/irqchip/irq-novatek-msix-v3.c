...
#define NR_IRQ_PER_GROP		64
#define NR_HW_IRQ_TOTAL		(NR_IRQ_PER_GROP * NR_HW_IRQ_GROP_NUM)

#define MAX_INT_PER_EP		(256)

#define MSIX_PCIE_MATCH_ADDR		(0x7B00000000ULL)	/* MSIX-TX(on EP) output pcie address */
...
typedef enum {		// tricky: this enum must has the same definition as that in nvt-pci.h
...
static u64 local_msix_get_pending_grps(struct nvt_msix_irq_data *chip_data)
{
	u64 val;

	val = readl(chip_data->base + INTR_GROUP_PENDING2_OFS);
	val <<= 32;
...
//		printk("%s: HW GROUP %d, pending 0x%lx\r\n", __func__, n, pending_int[n]);
		if (pending_grps & (1<<bit)) {
			...
			while (pendings) {
				...
				virq = irq_find_mapping(chip_data->domain, bit * NR_HW_IRQ_GROP_NUM + pending_bit);

				/* Assign virq to handle irq isr */
				generic_handle_irq(virq);

				/* Clear Interrupt Source */
				ep_msix_clear_tx_irq(chip_data, bit * NR_HW_IRQ_GROP_NUM + pending_bit);
			     ...
			     irq_hw_number_t hwirq)
{
	...
	irq_set_chip_and_handler_name(irq, &nvt_msix_irq_chip, handle_fasteoi_irq, data->name);
...
//	struct pci_dev * p_root;
	...
	printk("%s: dev id 0x%x, exp 0x%x\r\n", __func__, ep->dev_id, PCI_DEV_ID_NS02401);
	if ((ep->dev_id == PCI_DEV_ID_NS02401) && (slot == 0)) {
...
//	struct pci_dev * p_root;
	...
	if ((ep->dev_id == PCI_DEV_ID_NS02401) && (slot == 1)) {
...
static int build_fake_ep_info(struct nvt_msix_irq_data *p_data)
{
	...
	p_data->fake_ep.ep_idx = 0;
	p_data->fake_ep.dev_id = PCI_DEV_ID_NS02401;
	p_data->fake_ep.apb_va = p_ep_apb;
	p_data->fake_ep.pci_dev = NULL;
	p_data->fake_ep.ep_out_apb_pa = p_data->pa_remote_apb;
	p_data->fake_ep.ep_out_apb_len = EP_APB_SIZE;
...
static int nvt_msix_probe(struct platform_device *pdev)
{
...
#endif
	printk("%s: EP count %d\r\n", __func__, nvtpcie_get_ep_count(PCI_DEV_ID_NS02401));
	...
	data->pa_pcie_top = NVT_PCIE1_BASE_PHYS;
	data->pa_remote_apb = 0x4800000000;
...
#else
//	nvt_pci_list_for_each_ep_drv_info(_get_ep_info);
	ep_count = nvtpcie_get_ep_count(PCI_DEV_ID_NS02401);	// this will get total EP count on both RC port0 and port1
#endif
//	memset(g_nvt_ep_data, 0, sizeof(g_nvt_ep_data));
//	nvt_pci_list_for_each_ep_drv_info(_get_ep_info);
...
//	printk("MSI BASE MACRO 0x%lx, PHY BASE 0x%lx\r\n", NVT_MSI_BASE_PHYS, NVT_PERIPHERAL_PHYS_BASE);
	...
	for (i=0; i<ep_count; i++) {
		...
		val = readl(data->ep_data[i+CHIP_EP0].apb_va + (NVT_CG_BASE_PHYS-NVT_PERIPHERAL_PHYS_BASE) + 0x74);
		val |= GENMASK(5, 5);		// MSI_CLKEN
		writel(val, data->ep_data[i+CHIP_EP0].apb_va + (NVT_CG_BASE_PHYS-NVT_PERIPHERAL_PHYS_BASE) + 0x74);
