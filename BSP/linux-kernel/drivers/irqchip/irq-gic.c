...
void nvt_gic_target_parsing(struct seq_file *m)
{
	unsigned int gic_irqs;
	...
	for (i = 8; i < DIV_ROUND_UP(gic_irqs, 4); i++, j+=4) {
		target_value = readl_relaxed(dist_base + GIC_DIST_TARGET + i * 4);
		seq_printf(m, "GIC_SPI %03d to core%d\t", j, calc_pow(target_value & 0xFF));
		seq_printf(m, "GIC_SPI %03d to core%d\t", j + 1, calc_pow((target_value >> 8) & 0xFF));
		seq_printf(m, "GIC_SPI %03d to core%d\t", j + 2, calc_pow((target_value >> 16) & 0xFF));
		seq_printf(m, "GIC_SPI %03d to core%d\t", j + 3, calc_pow((target_value >> 24) & 0xFF));
		seq_printf(m, "\n");
	}
}
EXPORT_SYMBOL(nvt_gic_target_parsing);

#endif /* CONFIG_NVT_AMP */

static void gic_dist_init(struct gic_chip_data *gic)
{
#ifndef CONFIG_NVT_AMP
	unsigned int i;
	u32 cpumask;
#endif
