...
struct list_head nvt_clk_gate_list;
...
void nvt_cg_base_remap(void __iomem *remap_base)
{
	...
	INIT_LIST_HEAD(&nvt_clk_gate_list);
	cg_base = remap_base;
}
						 ...
						 unsigned long pll_div_value, spinlock_t *cg_spinlock)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		if (p_pll_clk->pll_ratio > 0) {
			p_pll_clk->pll_fixed = false;
		} else {
			p_pll_clk->pll_fixed = true;
		}
		...
		list_add_tail(&p_pll_clk->list, &nvt_clk_pll_list);
									...
									spinlock_t *cg_spinlock)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		list_add_tail(&p_cgate_clk->list, &nvt_clk_gate_list);
								   ...
								   spinlock_t *cg_spinlock)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		list_add_tail(&p_cmux_clk->list, &nvt_clk_mux_list);
...
int nvt_spread_spectrum(struct clk *clock, unsigned int steps, unsigned long ssc_step, unsigned long ssc_period_value)
{
	...
	/* Setup period vlaue */
	CG_SETREG(pll->rate_reg_offset - SPREAD_SPECTRUM_REG2_MINUS_OFFSET, ssc_period_value);

	/* Setup MULFAC */
	CG_SETREG(pll->rate_reg_offset - SPREAD_SPECTRUM_REG1_MINUS_OFFSET, steps);

	/* Setup SET_SEL */
	reg_val = CG_GETREG(pll->rate_reg_offset - SPREAD_SPECTRUM_REG0_MINUS_OFFSET);
	reg_val &= ~(0x3 << SPREAD_SPECTRUM_STEP_OFFSET);
	reg_val |= (ssc_step << SPREAD_SPECTRUM_STEP_OFFSET);
	CG_SETREG(pll->rate_reg_offset - SPREAD_SPECTRUM_REG0_MINUS_OFFSET, reg_val);

	/* Set spread spectrum mode and dssc */
	reg_val &= ~((0x1 << SPREAD_SPECTRUM_NEW_MODE_OFFSET) | (0x1 << SPREAD_SPECTRUM_DSSC_OFFSET));
	reg_val |= (0x1 << SPREAD_SPECTRUM_DSSC_OFFSET);
...
EXPORT_SYMBOL(nvt_clkmux_pm_suspend);

void nvt_clkgate_pm_suspend(struct nvt_composite_gate_clk *gate)
{
	if (!console_suspend_enabled && (strstr(gate->name, "uart") != NULL))
		return;

	gate->do_autogating = nvt_composite_gate_clk_get_phase(&gate->cgate_hw);
	gate->is_enabled = nvt_composite_gate_clk_is_enabled(&gate->cgate_hw);

	if (gate->is_enabled) {
		nvt_composite_gate_clk_disable(&gate->cgate_hw);
		nvt_composite_gate_clk_unprepare(&gate->cgate_hw);
	}
}
EXPORT_SYMBOL(nvt_clkgate_pm_suspend);
...
EXPORT_SYMBOL(nvt_clkpll_pm_suspend);
...
EXPORT_SYMBOL(nvt_clkmux_pm_resume);

void nvt_clkgate_pm_resume(struct nvt_composite_gate_clk *gate)
{
	if (!console_suspend_enabled && (strstr(gate->name, "uart") != NULL))
		return;

	if (nvt_composite_gate_clk_set_phase(&gate->cgate_hw, gate->do_autogating))
		return;

	if (nvt_composite_gate_clk_set_rate(&gate->cgate_hw, gate->current_rate, gate->parent_rate))
		return;

	if (!gate->is_enabled)
		return;

	if (nvt_composite_gate_clk_prepare(&gate->cgate_hw))
		return;

	if (nvt_composite_gate_clk_enable(&gate->cgate_hw))
		return;
}
EXPORT_SYMBOL(nvt_clkgate_pm_resume);
...
EXPORT_SYMBOL(nvt_clkpll_pm_resume);

int __maybe_unused nvt_clk_pm_suspend(void)
{
	struct nvt_pll_clk *pll, *_list;
	struct nvt_composite_gate_clk *gate, *_list_gate;
	...
	list_for_each_entry_safe(gate, _list_gate, &nvt_clk_gate_list, list) {
		nvt_clkgate_pm_suspend(gate);
	}
...
void __maybe_unused nvt_clk_pm_resume(void)
{
	struct nvt_pll_clk *pll, *_list;
	struct nvt_composite_gate_clk *gate, *_list_gate;
	struct nvt_composite_mux_clk *mux, *_list_mux;
