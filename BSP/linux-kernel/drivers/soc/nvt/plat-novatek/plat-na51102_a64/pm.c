...
unsigned int nvt_pm_ddr_clock = 1600;
...
#define NVT_SRAM_BASE 0x2f0e00000
#define NVT_SRAM_SIZE 0x30000
...
int nvt_copy_sram(void)
{
	...
	if (nvt_get_suspend_mode()) {
		writel(1, virt_sram + 0x1f010);
	} else {
		writel(0, virt_sram + 0x1f010);
	}

	writel(nvt_pm_func_mode, virt_sram + 0x1f014);
	writel(nvt_pm_ddr_clock, virt_sram + 0x1f018);

	writeq(S3_MAGIC_NUMBER, virt_sram + 0x1f000);
	writeq(virt_to_phys(s3_data_area), virt_sram + 0x1f008);
	iounmap(virt_sram);
...
int nvt_pm_prepare(void)
{
	...
	//change for cpu use
	writel(readl(s3_cg_base + 0x10) & (~(1<<4)), s3_cg_base + 0x10);
