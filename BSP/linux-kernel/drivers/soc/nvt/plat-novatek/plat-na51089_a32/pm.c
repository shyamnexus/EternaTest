...
int nvt_pm_prepare(void)
{
	...
	//IFE_SHARED_SRAM_CLKSEL[26]: 0:SRAM for CPU R/W 1:SRAM for IFE
	//change for cpu use
	writel(readl(NVT_CG_BASE_VIRT+0x18) & ~(1<<26), NVT_CG_BASE_VIRT+0x18);
	writel(readl(NVT_SRAM_BASE_VIRT) & ~(1<<13), NVT_SRAM_BASE_VIRT);
	memcpy((void *)NVT_IFE_SRAM_BASE_VIRT, (void *)s2_51089_firmware, NVT_51089_S2_FIRMWARE_LEN);
	return ret;
}
