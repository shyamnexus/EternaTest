...
union ADC_ONESHOT_REG {
	...
	unsigned int bandgap_oneshot:1; // bandgap channel one-shot start
...
#if 0
//for mcu
//0x08 ADC interrupt control Register(MCU)
...
union ADC_INTCTRL_REG {
	...
	unsigned int bandgap_inten:1;   // bandgap channel data ready interrupt enable
	unsigned int reserved0:24;
	} bit;
};
#else
//for cpu
//0x8C ADC interrupt control Register(CPU0)
#define ADC_INTCTRL_REG_OFS       0x8C
union ADC_INTCTRL_REG {
	uint32_t reg;
	struct {
	unsigned int ain0_inten:1;      // ADC channel 0 data ready interrupt enable
...
union ADC_STATUS_REG {
	...
	unsigned int bandgap_datardy:1; // bandgap channel data ready status
	unsigned int reserved2:25;
	} bit;
};
#else
//for cpu
#define ADC_STATUS_REG_OFS        0x90
union ADC_STATUS_REG {
	uint32_t reg;
	struct {
	unsigned int ain0_datardy:1;    // ADC channel 0 data ready status
...
union ADC_DIV0_REG {
	uint32_t reg;
	struct {
	unsigned int ain0_div:12;        // ADC channel 0 divider
	unsigned int reserved0:4;
	unsigned int ain1_div:12;        // ADC channel 1 divider
	unsigned int reserved1:4;
	} bit;
};

//0x24 ADC divide Register 1
#define ADC_DIV1_REG_OFS          0x28
union ADC_DIV1_REG {
	uint32_t reg;
	struct {
	unsigned int ain2_div:12;        // ADC channel 2 divider
	unsigned int reserved0:4;
	unsigned int ain3_div:12;        // ADC channel 3 divider
	unsigned int reserved1:4;
	} bit;
};

//0x2C ADC divide Register 2
#define ADC_DIV2_REG_OFS          0x2C
union ADC_DIV2_REG {
	uint32_t reg;
	struct {
	unsigned int bandgap_div:12;        // bandap channel divider
...
union THERMAL_SENSOR_CONFIGURE_REG {
	...
	unsigned int reserved0:1;
	unsigned int ain_avg_id:3;      // average channel id
...
//0x84 bandgap channel data register
#define ADC_BANDGAP_DATA_REG_OFS     0x84
union ADC_BANDGAP_DATA_REG {
	uint32_t reg;
	struct {
	unsigned int bandgap_data:9;     // data
	unsigned int reserved0:23;
	} bit;
};
