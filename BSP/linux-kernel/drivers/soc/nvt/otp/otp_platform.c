...
void __iomem *IOADDR_CG_REG_BASE;
...
#include <plat/hardware.h>
void __iomem *IOADDR_RTC_REG_BASE;
#endif
...
#if defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NS02402)
#include <plat/top.h>
UINT32 efuse_ext_field = 0;
#endif
...
#endif
{
	...
	if(!is_create) {
		...
		if(nvt_get_chip_id() == CHIP_NS02302) {
			efuse_ext_field = EFUSE_EXT_FIELD;
		} else if(nvt_get_chip_id() == CHIP_NS02402) { 
			efuse_ext_field = EFUSE_EXT_FIELD_539A;
		} else {
			efuse_ext_field = 0;
		}
#endif		
	...
	if (!IOADDR_CG_REG_BASE) {
		IOADDR_CG_REG_BASE = ioremap(NVT_CG_BASE_PHYS, 0x100);
	}
	if (!IOADDR_TZPC_REG_BASE) {
		IOADDR_TZPC_REG_BASE = ioremap(NVT_TZPC_BASE_PHYS, 0x50);
	}
#elif defined(CONFIG_NVT_IVOT_PLAT_NS02201) || defined(CONFIG_NVT_IVOT_PLAT_NS02302) || defined(CONFIG_NVT_IVOT_PLAT_NS02301)
	if (!IOADDR_CG_REG_BASE) {
		IOADDR_CG_REG_BASE = ioremap(NVT_CG_BASE_PHYS, 0x100);
	}
	if (!IOADDR_TZPC_REG_BASE) {
...
#elif defined(CONFIG_NVT_IVOT_PLAT_NA51103)
	if(!IOADDR_RTC_REG_BASE) {
		IOADDR_RTC_REG_BASE = ioremap(NVT_RTC_BASE_PHYS, 0x100);
	}
#elif defined(CONFIG_NVT_IVOT_PLAT_NA51089)	
