...
#define loc_cpu(flags) \
	{ \
		...
		cc_hwlock_resource(CC_CORE_CA53_CORE1, CC_RESOURCE_TOP); \
	}

#define unl_cpu(flags) \
	{ \
		cc_hwunlock_resource(CC_CORE_CA53_CORE1, CC_RESOURCE_TOP); \
...
uint32_t nvt_get_chip_ver(void)
{
	...
	top_reg_addr = ioremap(top_pa, 0x10);
	...
	if (chip_ver == TOP_ROMVER_A) {
		...
		pr_info("force to Ver C\n");
		return CHIPVER_C;
	}
}

EXPORT_SYMBOL(nvt_get_chip_ver);
...
static int pinmux_config_sif(uint32_t config)
{
	if (config == PIN_SIF_CFG_NONE) {
		...
		if (config & PIN_SIF_CFG_SIF0_1) {
			...
			if (top_reg0.bit.DSI_PROT_EN == MUX_1) {
				pr_err("SIF0_3 conflict with (DSI_PROT_EN = 1)\r\n");
				return E_PAR;
			}
...
static int pinmux_config_misc(uint32_t config)
{
	if (config == PIN_MISC_CFG_NONE) {
		...
		if (config & PIN_MISC_CFG_MIPI_SEL_DSI) {
			if (top_reg0.bit.DSI_PROT_EN == MUX_0) {
				pr_err("MIPI_SEL_DSI conflict with (DSI_PROT_EN = 0)\r\n");
				return E_PAR;
			}
			...
			if (top_reg0.bit.DSI_PROT_EN == MUX_0) {
				pr_err("MIPI_SEL_CSI_TX conflict with (DSI_PROT_EN = 0)\r\n");
				return E_PAR;
			}
		...
		if (config & PIN_MISC_CFG_CPU_ICE) {
			...
			gpio_func_keep(DGPIO_9, 5, func_MISC);
		}
	}
...
int pinmux_parsing_i2c(uint32_t config)
{
	int ret = 0;
	PAD_PULL pad_pull;
	if (config == PIN_I2C_CFG_NONE) {
		...
		if (config & PIN_I2C_CFG_I2C_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO28, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C 1st FUNC_EN && PGPIO28 pull up => force PGPIO28 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO28, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO28 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO28, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO29, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C 1st FUNC_EN && PGPIO29 pull up => force PGPIO29 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO29, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO29 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO29, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2C_CFG_I2C2_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO30, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C2 1st FUNC_EN && PGPIO30 pull up => force PGPIO30 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO30, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO30 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO30, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO31, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C2 1st FUNC_EN && PGPIO31 pull up => force PGPIO31 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO31, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO31 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO31, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2C_CFG_I2C3_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO12, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C3 1st FUNC_EN && PGPIO12 pull up => force PGPIO12 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO12, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO12 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO12, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO13, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C3 1st FUNC_EN && PGPIO13 pull up => force PGPIO13 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO13, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO13 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO13, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C3_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO11, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C3 2nd FUNC_EN && SGPIO11 pull up => force SGPIO11 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO11, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO11 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO11, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_SGPIO12, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C3 2nd FUNC_EN && SGPIO12 pull up => force SGPIO12 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO12, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO12 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO12, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C3_3) {
			if (pad_get_pull_updown(PAD_PIN_HSIGPIO22, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C3 3rd FUNC_EN && HSIGPIO22 pull up => force HSIGPIO22 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_HSIGPIO22, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get HSIGPIO22 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_HSIGPIO22, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_HSIGPIO23, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C3 3rd FUNC_EN && HSIGPIO23 pull up => force HSIGPIO23 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_HSIGPIO23, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get HSIGPIO23 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_HSIGPIO23, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2C_CFG_I2C4_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO14, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C4 1st FUNC_EN && PGPIO14 pull up => force PGPIO14 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO14, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO14 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO14, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO15, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C4 1st FUNC_EN && PGPIO15 pull up => force PGPIO15 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO15, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO15 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO15, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C4_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO13, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C4 2nd FUNC_EN && SGPIO13 pull up => force SGPIO13 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO13, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO13 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO13, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_SGPIO14, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C4 2nd FUNC_EN && SGPIO14 pull up => force SGPIO14 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO14, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO14 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO14, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C4_3) {
			if (pad_get_pull_updown(PAD_PIN_DSIGPIO4, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C4 3rd FUNC_EN && DSIGPIO4 pull up => force DSIGPIO4 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_DSIGPIO4, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get DSIGPIO4 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_DSIGPIO4, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_DSIGPIO5, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C4 3rd FUNC_EN && DSIGPIO5 pull up => force DSIGPIO5 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_DSIGPIO5, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get DSIGPIO5 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_DSIGPIO5, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C4_4) {
			if (pad_get_pull_updown(PAD_PIN_HSIGPIO20, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C4 4th FUNC_EN && HSIGPIO20 pull up => force HSIGPIO20 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_HSIGPIO20, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get HSIGPIO20 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_HSIGPIO20, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_HSIGPIO21, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C4 4th FUNC_EN && HSIGPIO21 pull up => force HSIGPIO21 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_HSIGPIO21, PAD_NONE);
				}
			} else {
				pr_err("Get HSIGPIO21 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_HSIGPIO21, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2C_CFG_I2C5_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO16, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C5 1st FUNC_EN && PGPIO16 pull up => force PGPIO16 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO16, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO16 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO16, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO17, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C5 1st FUNC_EN && PGPIO17 pull up => force PGPIO17 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO17, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO17 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO17, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C5_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO15, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C5 2nd FUNC_EN && SGPIO15 pull up => force SGPIO15 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO15, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO15 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO15, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_SGPIO16, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C5 2nd FUNC_EN && SGPIO16 pull up => force SGPIO16 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO16, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO16 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO16, PAD_NONE);
			}
		}
		if (config & PIN_I2C_CFG_I2C5_3) {
			if (pad_get_pull_updown(PAD_PIN_CGPIO4, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get CGPIO4 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_CGPIO4, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_CGPIO5, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get CGPIO5 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_CGPIO5, PAD_NONE);
			}
		}
	}
...
int pinmux_parsing_i2cII(uint32_t config)
{
	int ret = 0;
	PAD_PULL pad_pull;
	if (config == PIN_I2CII_CFG_NONE) {
		...
		if (config & PIN_I2CII_CFG_I2C6_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO18, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C6 1st FUNC_EN && PGPIO18 pull up => force PGPIO18 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO18, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO18 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO18, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO19, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C6 1st FUNC_EN && PGPIO19 pull up => force PGPIO19 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO19, PAD_NONE);
				}
			} else {
		...
		if (config & PIN_I2CII_CFG_I2C6_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO23, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C6 2nd FUNC_EN && SGPIO23 pull up => force SGPIO23 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO23, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO23 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO23, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_SGPIO24, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C6 2nd FUNC_EN && SGPIO24 pull up => force SGPIO24 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO24, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO24 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO24, PAD_NONE);
			}
		}
		if (config & PIN_I2CII_CFG_I2C6_3) {
			if (pad_get_pull_updown(PAD_PIN_CGPIO6, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get CGPIO6 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_CGPIO6, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_CGPIO7, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C6 3rd FUNC_EN && CGPIO7 pull up => force CGPIO7 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_CGPIO7, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get CGPIO7 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_CGPIO7, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2CII_CFG_I2C7_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO20, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C7 1st FUNC_EN && PGPIO20 pull up => force PGPIO20 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO20, PAD_NONE);
				}
			} else {
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO21, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C7 1st FUNC_EN && PGPIO21 pull up => force PGPIO21 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO21, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO21 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO21, PAD_NONE);
			}
		}

		if (config & PIN_I2CII_CFG_I2C7_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO25, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C7 2nd FUNC_EN && SGPIO25 pull up => force SGPIO25 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO25, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO25 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO25, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_SGPIO26, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C7 2nd FUNC_EN && SGPIO26 pull up => force SGPIO26 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO26, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO26 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO26, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2CII_CFG_I2C8_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO22, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C8 1st FUNC_EN && PGPIO22 pull up => force PGPIO22 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO22, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO22 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO22, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO23, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C8 1st FUNC_EN && PGPIO23 pull up => force PGPIO23 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO23, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get PGPIO23 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO23, PAD_NONE);
			}
		}
		if (config & PIN_I2CII_CFG_I2C8_2) {
			if (pad_get_pull_updown(PAD_PIN_SGPIO7, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C8 2nd FUNC_EN && SGPIO7 pull up => force SGPIO7 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO7, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO7 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO7, PAD_NONE);
			}

			if (pad_get_pull_updown(PAD_PIN_SGPIO8, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C8 2nd FUNC_EN && SGPIO8 pull up => force SGPIO8 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_SGPIO8, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get SGPIO8 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_SGPIO8, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2CII_CFG_I2C9_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO24, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO24 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO24, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO25, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO25 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO25, PAD_NONE);
			}
		}
		if (config & PIN_I2CII_CFG_I2C9_2) {
			if (pad_get_pull_updown(PAD_PIN_LGPIO26, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("###I2C9 2nd FUNC_EN && LGPIO26 pull up => force LGPIO26 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_LGPIO26, PAD_NONE);
				}
			} else {
				ret++;
				pr_err("Get LGPIO26 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_LGPIO26, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_LGPIO27, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get LGPIO27 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_LGPIO27, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2CII_CFG_I2C10_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO26, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO26 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO26, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO27, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO27 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO27, PAD_NONE);
			}
		}
		if (config & PIN_I2CII_CFG_I2C10_2) {
			if (pad_get_pull_updown(PAD_PIN_DGPIO0, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get DGPIO0 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_DGPIO0, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_DGPIO1, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get DGPIO1 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_DGPIO1, PAD_NONE);
			}
		}
		...
		if (config & PIN_I2CII_CFG_I2C11_1) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO36, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO36 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO36, PAD_NONE);
			}
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO37, &pad_pull) == E_OK) {
				...
				ret++;
				pr_err("Get PGPIO37 Fail, force pull none\r\n");
				pad_set_pull_updown(PAD_PIN_PGPIO37, PAD_NONE);
			}
		}
	}
...
ER pinmux_init(struct nvt_pinctrl_info *info)
{
	...
	if (top_reg0.bit.DSI_PROT_EN == MUX_0) {
		void __iomem *dsi_reg_addr = NULL;
		void __iomem *csitx_reg_addr = NULL;

		dsi_reg_addr = ioremap(NVT_DSI_PHY_BASE_PHYS, 0x1000);
		if (dsi_reg_addr) {
			writel(0x2, dsi_reg_addr + 0x300);
			//pr_err("%s: set bypass [0x%lx = 0x%x]\n", __func__, NVT_DSI_PHY_BASE_PHYS + 0x300, readl(dsi_reg_addr + 0x300));
			iounmap(dsi_reg_addr);
		} else {
			pr_err("invalid dsi_reg_addr\n");
			return -ENOMEM;
		}

		csitx_reg_addr = ioremap(NVT_CSI_TX_BASE_PHYS, 0x1000);
		if (csitx_reg_addr) {
			writel(0x2, csitx_reg_addr + 0x300);
			//pr_err("%s: set bypass [0x%lx = 0x%x]\n", __func__, NVT_CSI_TX_BASE_PHYS + 0x300, readl(csitx_reg_addr + 0x300));
			iounmap(csitx_reg_addr);
		} else {
			pr_err("invalid csitx_reg_addr\n");
			return -ENOMEM;
		}
	}
