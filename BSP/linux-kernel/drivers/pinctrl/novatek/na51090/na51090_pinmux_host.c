...
EXPORT_SYMBOL(pinmux_gpio_parsing);
...
void pinmux_parsing(struct nvt_pinctrl_info *info)
{
	...
	if ((local_top_reg_pgpio0.bit.PGPIO_27 == GPIO_ID_EMUM_FUNC) && store_extclk2) {
	...
	if (local_top_reg_jgpio0.bit.JGPIO_5 == GPIO_ID_EMUM_FUNC) {
		value |= PIN_MISC_CFG_PCIE_RSTN;

		if (local_top_reg10.bit.PCIE_RESTN_RSE == 1) {
			value |= PIN_MISC_CFG_RSTN_HIGHZ;
		}
	}

	if ((local_top_reg10.bit.PCIE_REFCLK_SRC == 0) &&
		(local_top_reg10.bit.PCIE_REFCLK_PAD_OUT_EN == 1)) {
		value |= PIN_MISC_CFG_PCIE_REFCLK_OUTEN;
	} else {
	}
	...
	if (config == PIN_I2C_CFG_NONE) {
		...
		if (config & (PIN_I2C_CFG_CH_1ST_PINMUX | PIN_I2C_CFG_CH_2ND_PINMUX)) {
			if (config & PIN_I2C_CFG_CH_1ST_PINMUX) {
				...
				pad_set_pull_updown(PAD_PIN_DGPIO9, PAD_NONE);
		...
		if (config & (PIN_I2C_CFG_CH3_1ST_PINMUX | PIN_I2C_CFG_CH3_2ND_PINMUX | PIN_I2C_CFG_CH3_3RD_PINMUX | PIN_I2C_CFG_CH3_4TH_PINMUX)) {
			if (config & PIN_I2C_CFG_CH3_1ST_PINMUX) {
				...
				pad_set_pull_updown(PAD_PIN_PGPIO34, PAD_NONE);
				...
				pad_set_pull_updown(PAD_PIN_CGPIO10, PAD_NONE);
	...
	if (config == PIN_SSP_CFG_NONE) {
		...
		if (config & (PIN_SSP_CFG_CH3_1ST_MCLK | PIN_SSP_CFG_CH3_2ND_MCLK | PIN_SSP_CFG_CH3_3RD_MCLK)) {
			if (config & PIN_SSP_CFG_CH3_1ST_MCLK) {
				...
				if (top_reg11.bit.I2S2_MCLK == MUX_EN) {
					pr_err("PIN_SSP_CFG_CH3_2ND_MCLK conflict with I2S2_MCLK\r\n");
					return E_OBJ;
				}

				top_reg_pgpio1.bit.PGPIO_32 = GPIO_ID_EMUM_FUNC;
				top_reg11.bit.I2S2_MCLK = MUX_2;
				...
				if (top_reg11.bit.I2S2_MCLK == MUX_EN) {
					pr_err("PIN_SSP_CFG_CH3_3RD_MCLK conflict with I2S2_MCLK\r\n");
					return E_OBJ;
				}
				top_reg_jgpio0.bit.JGPIO_0 = GPIO_ID_EMUM_FUNC;
				top_reg11.bit.I2S2_MCLK = MUX_3;
...
ER pinmux_config_vcap(uint32_t config)
{
	if (config == PIN_VCAP_CFG_NONE) {
		...
		if (config & (PIN_VCAP_CFG_CAP4_1ST_PINMUX | PIN_VCAP_CFG_CAP4_CLK_1ST_PINMUX | PIN_VCAP_CFG_CAP4_CLK_2ND_PINMUX)) {
			...
			} else if (config & PIN_VCAP_CFG_CAP4_CLK_2ND_PINMUX) {
				top_reg_sgpio1.bit.SGPIO_36 = GPIO_ID_EMUM_FUNC;
				top_reg9.bit.CAP4_CLK = MUX_2;
			} else {
				top_reg9.bit.CAP4_CLK = MUX_DIS;
			}
		}
		//CAP5
		if (config & (PIN_VCAP_CFG_CAP5_1ST_PINMUX | PIN_VCAP_CFG_CAP5_CLK_1ST_PINMUX | PIN_VCAP_CFG_CAP5_CLK_2ND_PINMUX)) {
			...
			} else if (config & PIN_VCAP_CFG_CAP5_CLK_2ND_PINMUX) {
				top_reg_sgpio1.bit.SGPIO_48 = GPIO_ID_EMUM_FUNC;
				top_reg9.bit.CAP5_CLK = MUX_2;
			} else {
				top_reg9.bit.CAP5_CLK = MUX_DIS;
			}
		}
		//CAP6
		if (config & (PIN_VCAP_CFG_CAP6_1ST_PINMUX | PIN_VCAP_CFG_CAP6_CLK_1ST_PINMUX | PIN_VCAP_CFG_CAP6_CLK_2ND_PINMUX)) {
			...
			} else if (config & PIN_VCAP_CFG_CAP6_CLK_2ND_PINMUX) {
				top_reg_sgpio1.bit.SGPIO_60 = GPIO_ID_EMUM_FUNC;
				top_reg9.bit.CAP6_CLK = MUX_2;
			} else {
				top_reg9.bit.CAP6_CLK = MUX_DIS;
			}
		}
		//CAP7
		if (config & (PIN_VCAP_CFG_CAP7_1ST_PINMUX | PIN_VCAP_CFG_CAP7_CLK_1ST_PINMUX | PIN_VCAP_CFG_CAP7_CLK_2ND_PINMUX)) {
			...
			} else if (config & PIN_VCAP_CFG_CAP7_CLK_2ND_PINMUX) {
				top_reg_sgpio2.bit.SGPIO_72 = GPIO_ID_EMUM_FUNC;
				top_reg9.bit.CAP7_CLK = MUX_2;
			} else {
				top_reg9.bit.CAP7_CLK = MUX_DIS;
			}
		}
...
ER pinmux_config_eth(uint32_t config)
{
	if (config == PIN_ETH_CFG_NONE) {
		...
		if (config & PIN_ETH_CFG_RST_PINMUX) {
			pr_warn("PIN_ETH_CFG_RST_PINMUX is obsoleted, skip it...\r\n");
		}
		...
		if (config & PIN_ETH2_CFG_RST_PINMUX) {
			pr_warn("PIN_ETH2_CFG_RST_PINMUX is obsoleted, skip it...\r\n");
		}
...
ER pinmux_config_misc(uint32_t config)
{
	if (config == PIN_ETH_CFG_NONE) {
		...
		if (config & PIN_MISC_CFG_PCIE_RSTN) {
			top_reg_jgpio0.bit.JGPIO_5 = GPIO_ID_EMUM_FUNC;

			if (config & PIN_MISC_CFG_RSTN_HIGHZ) {
				top_reg10.bit.PCIE_RESTN_RSE = 1;
			} else {
				top_reg10.bit.PCIE_RESTN_RSE = 0;
			}
		}
		if (config & PIN_MISC_CFG_PCIE_REFCLK_OUTEN) {
			top_reg10.bit.PCIE_REFCLK_SRC = 0;		// from MPLL24
			top_reg10.bit.PCIE_REFCLK_PAD_OUT_EN = 1;	// output enable
		} else {
			top_reg10.bit.PCIE_REFCLK_SRC = 1;		// from REFCLK pad
			top_reg10.bit.PCIE_REFCLK_PAD_OUT_EN = 0;	// output disable (input)
		}
...
ER pinmux_parsing_i2c(uint32_t config)
{
	...
	if (config == PIN_I2C_CFG_NONE) {
		...
		if (config & (PIN_I2C_CFG_CH_1ST_PINMUX | PIN_I2C_CFG_CH_2ND_PINMUX)) {
			if (config & PIN_I2C_CFG_CH_1ST_PINMUX) {
				...
				if (pad_get_pull_updown(PAD_PIN_PGPIO13, &pad_pull) == E_OK) {
					pr_info("PAD_PIN_PGPIO13 = %d\n", pad_pull);
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C 1st FUNC_EN && PGPIO13 pull up => force PGPIO13 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_PGPIO13, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_DGPIO9, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C 2ND FUNC_EN && DGPIO9 pull up => force DGPIO9 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_DGPIO9, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_DGPIO10, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C 2ND FUNC_EN && DGPIO10 pull up => force DGPIO10 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_DGPIO10, PAD_NONE);
					}
				} else {
		...
		if (config & (PIN_I2C_CFG_CH2_1ST_PINMUX)) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO19, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C2 1st FUNC_EN && PGPIO19 pull up => force PGPIO19 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO19, PAD_NONE);
				}
			} else {
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO20, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C2 1st FUNC_EN && PGPIO20 pull up => force PGPIO20 = pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO20, PAD_NONE);
				}
			} else {
		...
		if (config & (PIN_I2C_CFG_CH3_1ST_PINMUX | PIN_I2C_CFG_CH3_2ND_PINMUX | PIN_I2C_CFG_CH3_3RD_PINMUX | PIN_I2C_CFG_CH3_4TH_PINMUX)) {
			if (config & PIN_I2C_CFG_CH3_1ST_PINMUX) {
				if (pad_get_pull_updown(PAD_PIN_PGPIO33, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 1st FUNC_EN && PGPIO33 pull up => force PGPIO33 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_PGPIO33, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_PGPIO34, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 1st FUNC_EN && PGPIO34 pull up => force PGPIO34 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_PGPIO34, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_JGPIO0, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 2nd FUNC_EN && JGPIO0 pull up => force JGPIO0 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_JGPIO0, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_JGPIO1, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 2nd FUNC_EN && JGPIO1 pull up => force JGPIO1 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_JGPIO1, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_PGPIO6, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 3rd FUNC_EN && PGPIO6 pull up => force PGPIO6 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_PGPIO6, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_PGPIO7, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 3rd FUNC_EN && PGPIO7 pull up => force PGPIO7 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_PGPIO7, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_CGPIO10, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 4th FUNC_EN && CGPIO10 pull up => force CGPIO10 = pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_CGPIO10, PAD_NONE);
					}
				} else {
				...
				if (pad_get_pull_updown(PAD_PIN_CGPIO11, &pad_pull) == E_OK) {
					if (pad_pull != PAD_NONE) {
						ret++;
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pr_err("!!!I2C3 4th FUNC_EN && CGPIO11 pull up => force CGPIO11 pull none!!!\n");
						pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
						pad_set_pull_updown(PAD_PIN_CGPIO11, PAD_NONE);
					}
				} else {
		...
		if (config & (PIN_I2C_CFG_HDMI_1ST_PINMUX)) {
			if (pad_get_pull_updown(PAD_PIN_PGPIO37, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C hdmi 1st FUNC_EN && PGPIO37 pull up => force PGPIO37 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO37, PAD_NONE);
				}
			} else {
			...
			if (pad_get_pull_updown(PAD_PIN_PGPIO38, &pad_pull) == E_OK) {
				if (pad_pull != PAD_NONE) {
					ret++;
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pr_err("!!!I2C hdmi 1st FUNC_EN && PGPIO38 pull up => force PGPIO38 pull none!!!\n");
					pr_err("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					pad_set_pull_updown(PAD_PIN_PGPIO38, PAD_NONE);
				}
			} else {
