...
ER pad_set_delaychain(PAD_DLY_STRUCT *del_sel)
{
	union PAD_LCDDLY_REG reg_data;
	unsigned long flags = 0;

	loc_cpu(&pad_lock, flags);
	reg_data.reg = PAD_GETREG(PAD_LCDDLY_REG_OFS);
	...
	reg_data.bit.lcd_inv_vga_hs = del_sel->vga_hs_inv;
	PAD_SETREG(PAD_LCDDLY_REG_OFS, reg_data.reg);
	unl_cpu(&pad_lock, flags);

	return E_OK;
}
EXPORT_SYMBOL(pad_set_delaychain);

void pad_get_delaychain(PAD_DLY_STRUCT *del_sel)
{
	union PAD_LCDDLY_REG reg_data;
...
EXPORT_SYMBOL(pad_get_delaychain);
