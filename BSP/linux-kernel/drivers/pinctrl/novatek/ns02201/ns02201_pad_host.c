...
ER pad_set_pull_updown(PAD_PIN pin, PAD_PULL pulltype)
{
	...
	/* On ns02201, the pullup value of DSIGPIO0~11 and AGPIO is 0x3, and the reserved value is 0x2 */
	if ((pin >= PAD_PIN_DSIGPIO0) && (pin <= PAD_PIN_DSIGPIO11)) {
...
ER pad_get_pull_updown(PAD_PIN pin, PAD_PULL *pulltype)
{
	...
	*pulltype = ((reg_data >> bit_offset) & 0x3);
	...
	/* On ns02201, the pullup value of DSIGPIO0~11 and AGPIO0 is 0x3, and the reserved value is 0x2 */
	if ((pin >= PAD_PIN_DSIGPIO0) && (pin <= PAD_PIN_DSIGPIO11)) {
		if (*pulltype == 0x3) {
			*pulltype = PAD_PULLUP;
...
ER pad_set_power(PAD_POWER_STRUCT *pad_power)
{
	...
	if (pad_power->pad_power_id == PAD_POWERID_MC0) {
		...
		if (pad_power->pad_power == PAD_1P8V) {
			// Set the magic number of SN2 (bootstrap[12] needs to be 1 before setting)
			loc_cpu(flags);
			...
			pr_err("%s: Not Existed PAD_POWER for SN2\r\n", __func__);
			return E_NOEXS;
		}
...
void pad_show(struct nvt_pinctrl_info *info)
{

	char *gpio_name[] = {"C_GPIO", "P_GPIO", "D_GPIO", "L_GPIO", "S_GPIO", "DSI_GPIO", "A_GPIO"};
	char *pad_pull_name[] = {"NONE", "PULLDOWN", "PULLUP", "STRONG_PULLUP"};
	char name[10];
	...
	PAD_DS pin_ds_end[7] = {PAD_DS_CGPIO25, PAD_DS_PGPIO46, PAD_DS_SYS_RST, PAD_DS_LGPIO30, PAD_DS_SGPIO31, PAD_DS_DSIGPIO13, PAD_DS_AGPIO3};
	PAD_DS pin_ds_base[7] = {PAD_DS_CGPIO_BASE, PAD_DS_PGPIO_BASE, PAD_DS_DGPIO_BASE, PAD_DS_LGPIO_BASE, PAD_DS_SGPIO_BASE, PAD_DS_DSIGPIO_BASE, PAD_DS_AGPIO_BASE};

	PAD_DS pin_end[7] = {PAD_PIN_CGPIO25, PAD_PIN_PGPIO46, PAD_PIN_SYS_RST, PAD_PIN_LGPIO30, PAD_PIN_SGPIO31, PAD_PIN_DSIGPIO13, PAD_PIN_AGPIO3};
	PAD_DS pin_base[7] = {PAD_CGPIO_BASE, PAD_PGPIO_BASE, PAD_DGPIO_BASE, PAD_LGPIO_BASE, PAD_SGPIO_BASE, PAD_DSIGPIO_BASE, PAD_AGPIO_BASE};
	...
	for(nr_pad=0; nr_pad <PAD_PIN_MAX_CNT; nr_pad++){
		...
		if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_CGPIO25) {
			strcpy(name, gpio_name[0]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[0])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_PGPIO46) {
			strcpy(name, gpio_name[1]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[1])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_SYS_RST) {
			strcpy(name, gpio_name[2]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[2])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_LGPIO30) {
			strcpy(name, gpio_name[3]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[3])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_SGPIO31) {
			strcpy(name, gpio_name[4]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[4])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_DSIGPIO13) {
			strcpy(name, gpio_name[5]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[5])/4;
		} else if (info->pad[nr_pad].pad_ds_pin <= PAD_DS_AGPIO3) {
			strcpy(name, gpio_name[6]);
			num = (info->pad[nr_pad].pad_ds_pin - pin_ds_base[6])/4;
		}else{
			strcpy(name, gpio_name[0]);
			num=0;
			pr_err("Can not serch pad pin.\r\n");
		}
		//pr_info("ds:%lx ,dir:%ld\r\n",info->pad[nr_pad].pad_ds_pin, info->pad[nr_pad].direction);
		pr_info("PIN %s%d : pull: %s, Driving:%d \r\n", name, num, pad_pull_name[info->pad[nr_pad].direction], (int)info->pad[nr_pad].driving);
	}
