...
typedef struct proc_pinmux {
	...
	struct proc_dir_entry *pproc_pad_entry;
} proc_pinmux_t;
 ...
 *=============================================================================
 */
static int nvt_pinmux_proc_pad_show(struct seq_file *sfile, void *v)
{
        struct nvt_pinctrl_info *info;

        info = kmalloc(sizeof(struct nvt_pinctrl_info), GFP_KERNEL);
        if (!info) {
                pr_err("nvt pad mem alloc fail\n");
                return -ENOMEM;
        }
...
static int nvt_pinmux_proc_pad_open(struct inode *inode, struct file *file)
{
        return single_open(file, nvt_pinmux_proc_pad_show, NULL);
}

static struct proc_ops proc_pad_fops = {
        .proc_open   = nvt_pinmux_proc_pad_open,
...
int nvt_pinmux_proc_init(void)
{
	...
	pentry = proc_create("pad_show", S_IRUGO | S_IXUGO, pmodule_root, &proc_pad_fops);
        if (pentry == NULL) {
                pr_err("failed to create proc pad!\n");
                ret = -EINVAL;
                goto remove_proc;
        }
	proc_pinmux.pproc_pad_entry = pentry;
