...
static int nvt_romblock_isbad(struct mtd_info *mtd, loff_t ofs)
{
	struct mtd_info *mtd_parent = mtd_get_master(mtd);

	ofs += mtd->part.offset;

	return mtd_parent->_block_isbad(mtd_parent, ofs);
}
			     ...
			     struct mtd_info *mtd)
{
	...
	mtd->_block_isbad = nvt_romblock_isbad;
...
static struct mtd_blktrans_ops romblock_tr = {
	.name = "romblock",
	.major = 240, // based on device.txt,  240-254 block	LOCAL/EXPERIMENTAL USE
	.part_bits = 0,
	.blksize = 512,
