...
#include <plat/top.h>
#define NAND_SETREG(info, ofs,value)	   OUTW((void *)info->mmio_base +(ofs),(value))
#define NAND_GETREG(info, ofs)		       INW((void *)info->mmio_base+(ofs))

#define  DUMP_NAND_DEBUG_MSG 			0
#define  NAND_DEBUG_MSG(fmt, args...) \
	if(DUMP_NAND_DEBUG_MSG) pr_err(fmt, ##args)
...
typedef enum
{
	NAND_SPI_SPARE_16_BYTE = 0x0,
	NAND_SPI_SPARE_32_BYTE,
	NAND_SPI_SPARE_0_BYTE,
...
typedef enum
{
	WR_SPP  = 0,	// single page program
	WR_QPP  = 1,	// quad page program
	WR_OPP  = 2,	// octal page program

	ENUM_DUMMY4WORD(NAND_SPI_NOR_WRITE_MODE)
} NAND_SPI_NOR_WRITE_MODE;
#define NVT_GET_WRITE_IO(mode)  mode?(4*mode):1
...
struct nvt_nand_flash {
	...
	uint32_t dtr_en;        	/* Set SDR or DTR read          */
	uint32_t indly;             /* Set input clk delay          */
