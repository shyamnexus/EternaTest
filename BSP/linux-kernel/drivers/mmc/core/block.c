...
#include <linux/of.h>
#include <linux/soc/nvt/nvt_ivot_emmc.h>
#endif
...
static int nvt_emmc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "part_type\t\t\tpart_name\t\t\tcard id\t\t\tstart_sector\t\tnum_sector\t\tpart_num\t\t\n");
	seq_printf(m, "===================================================================================================================================================\n");

	for (i = 0; i < nvt_ivot_emmc_parts->nr_parts; i++)
	{
		seq_printf(m, "%-20s\t\t%-20s\t\t%-20s\t%-5lld\t\t\t%-10lld\t\t%-10s\t\t\n", nvt_ivot_emmc_parts->part_info[i].part_type,					nvt_ivot_emmc_parts->part_info[i].name,
...
static const struct proc_ops nvt_emmc_fops = {
	.proc_open = nvt_emmc_open,
	.proc_read = seq_read,
};
...
static struct nvt_ivot_emmc_total_partitions* nvt_ivot_emmc_part_parsing(struct mmc_host *host)
{
	...
	for_each_of_allnodes_from(host->parent->of_node, pp) {
		...
		if (strncmp("partition_", of_node_full_name(pp), 10) == 0) {
			parts->part_info[i].part_type = of_node_full_name(pp);
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	/*
	   if (xa_load(&md->disk->part_tbl, 1)) {
	   isMBR = true;
	   }
	   */
	raw_partno = nvt_ivot_emmc_parts->rootfs_num;
	for (i = 0; i < nvt_ivot_emmc_parts->nr_parts; i++)
	{
		if (strncmp(nvt_ivot_emmc_parts->part_info[i].part_type, "partition_rootfs", 16) == 0) {
			partno++;
			/* Add native partition (Only raw partitions are needed to create block device nodes) */
			hd = nvt_add_partition(md->disk, partno, nvt_ivot_emmc_parts->part_info[i].start, nvt_ivot_emmc_parts->part_info[i].num, 0);
		} else {
			raw_partno++;
			hd = nvt_add_partition(md->disk, raw_partno, nvt_ivot_emmc_parts->part_info[i].start, nvt_ivot_emmc_parts->part_info[i].num, 0);
		}

		if (!hd) {
			pr_err("EMMC add partition fail\n");
			goto add_part_done;
		}
		...
		sprintf(nvt_ivot_emmc_parts->part_info[i].disk_name, "%sp%d", md->disk->disk_name, nvt_ivot_emmc_parts->part_info[i].partno);
		pr_debug("%s: Add emmc raw partition type:%-20s name:%-15s sector:%lld@%-10lld /dev/%s\n", __func__, 
				nvt_ivot_emmc_parts->part_info[i].part_type,
