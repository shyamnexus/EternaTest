/*
 * spi_nvt.c - Novatek SPI controller driver.
 *
 * Based on linux/drivers/spi/spi_nvt.c
 *
...
static void nvt_spi_set_npcs(struct nvt_spi* spi_bus, int val)
{
	...
	unsigned long retryTimes = 1000000;
	...
	if (retryTimes == 0) {
		printk("ERR %s: wait cs to target state failed\r\n", __func__);
	}
}
...
static int nvt_spi_setup(struct spi_device *spi)
{
	...
	if (retryTimes == 0) {
		printk("ERR %s: wait spi_en == 1 failed\r\n", __func__);
	}
...
static void spi_enable_dma(struct nvt_spi *spi_bus, bool dma_en)
{
	...
	if (nvt_get_chip_id() == CHIP_NS02402) {
		status_reg.reg |= (0x1 << 2);  //tx abort
		status_reg.reg |= (0x1 << 16); //rx abort
	} else {
...
/*
	...
	@return void
*/
static void spi_set_dma_rx_address(struct nvt_spi *spi_bus, dma_addr_t addr)
{
	if (nvt_get_chip_id() == CHIP_NS02402) {
		spi_setreg(spi_bus, 0x28, addr & 0xFFFFFFFF);
	}
}

static void spi_set_dma_tx_address(struct nvt_spi *spi_bus, dma_addr_t addr)
{
	if (nvt_get_chip_id() == CHIP_NS02402) {
		union SPI_DMA_STARTADDR_REG dma_addr_reg = {0};
		dma_addr_reg.bit.spi_dma_start_addr = addr & 0xFFFFFFFF;
		spi_setreg(spi_bus, SPI_DMA_STARTADDR_REG_OFS, dma_addr_reg.reg);
	}
}

static void spi_set_dma_address(struct nvt_spi *spi_bus, dma_addr_t addr)
{
	union SPI_DMA_STARTADDR_REG dma_addr_reg = {0};
	#ifdef SPI_DMA_STARTADDR_H_REG_OFS
	union SPI_DMA_STARTADDR_H_REG dma_addr_h_reg = {0};
	#endif
	...
	#ifdef SPI_DMA_STARTADDR_H_REG_OFS
	dma_addr_h_reg.bit.spi_dma_start_addr_h = (addr >> 32) & 0xF;
	spi_setreg(spi_bus, SPI_DMA_STARTADDR_H_REG_OFS, dma_addr_h_reg.reg);
	#endif
}
...
static void spi_set_dma_direction(struct nvt_spi *spi_bus, u32 dma_dir)
{
	...
	if (nvt_get_chip_id() == CHIP_NS02402) {
		if (dma_dir == SPI_DMA_DIR_SPITX) {
			dma_ctrl_reg.reg = 1;
		} else if (dma_dir == SPI_DMA_DIR_SPIRX) {
			dma_ctrl_reg.reg = 2;
		} else {
			dma_ctrl_reg.reg = 3;
		}
...
/*
	...
	@return void
*/
static void spi_set_dma_write(struct nvt_spi *dev, u8 *buffer, u32 word_cnt)
{
	// Write-back cache to DRAM
	dma_addr_t dma_handle;

	dma_handle = dma_map_single(dev->dev, buffer, word_cnt*4, DMA_TO_DEVICE);
	if (dma_mapping_error(dev->dev, dma_handle)) {
		pr_err("Error: map tx dma buffer failed\r\n");
		return;
	}
	dma_sync_single_for_device(dev->dev, dma_handle, word_cnt*4, DMA_TO_DEVICE);
	dma_unmap_single(dev->dev, dma_handle, word_cnt*4, DMA_TO_DEVICE);
	
	spi_enable_dma(dev, false);                   // disable DMA
	spi_set_transfer_count(dev, word_cnt);
	if (nvt_get_chip_id() == CHIP_NS02402) {
		spi_set_dma_tx_address(dev, dma_handle);
		spi_set_dma_direction(dev, SPI_DMA_DIR_SPITX);
	} else {
		spi_set_dma_address(dev, dma_handle);
...
/*
	...
	@return void
*/
static void spi_set_dma_read(struct nvt_spi *dev, u8 *buffer, u32 word_cnt)
{
	// Invalidate cache
	dma_addr_t dma_handle;
	
	dma_handle = dma_map_single(dev->dev, buffer, word_cnt*4, DMA_FROM_DEVICE);
	if (dma_mapping_error(dev->dev, dma_handle)) {
		pr_err("Error: map rx dma buffer failed\r\n");
		return;
	}
	dma_sync_single_for_device(dev->dev, dma_handle, word_cnt*4, DMA_FROM_DEVICE);
	dma_unmap_single(dev->dev, dma_handle, word_cnt*4, DMA_FROM_DEVICE);
	
	spi_enable_dma(dev, false);                   // disable DMA
	spi_set_transfer_count(dev, word_cnt);
	if (nvt_get_chip_id() == CHIP_NS02402) {
		spi_set_dma_rx_address(dev, dma_handle);
		spi_set_dma_direction(dev, SPI_DMA_DIR_SPIRX);
	} else {
		spi_set_dma_address(dev, dma_handle);
...
static void spi_set_dma_read_write(struct nvt_spi *dev, u8 *rx_buffer, u8 *tx_buffer, u32 word_cnt)
{
	// Invalidate cache
	dma_addr_t dma_rx_handle, dma_tx_handle;
	if (nvt_get_chip_id() == CHIP_NS02402) {
		dma_rx_handle = dma_map_single(dev->dev, rx_buffer, word_cnt*4, DMA_FROM_DEVICE);
		if (dma_mapping_error(dev->dev, dma_rx_handle)) {
			pr_err("Error: map rx dma buffer failed\r\n");
			return;
		}
		dma_sync_single_for_device(dev->dev, dma_rx_handle, word_cnt*4, DMA_FROM_DEVICE);
		dma_unmap_single(dev->dev, dma_rx_handle, word_cnt*4, DMA_FROM_DEVICE);

		dma_tx_handle = dma_map_single(dev->dev, tx_buffer, word_cnt*4, DMA_TO_DEVICE);
		if (dma_mapping_error(dev->dev, dma_tx_handle)) {
			pr_err("Error: map tx dma buffer failed\r\n");
			return;
		}
		dma_sync_single_for_device(dev->dev, dma_tx_handle, word_cnt*4, DMA_TO_DEVICE);
		dma_unmap_single(dev->dev, dma_tx_handle, word_cnt*4, DMA_TO_DEVICE);

		
		spi_enable_dma(dev, false);                   // disable DMA
		spi_set_transfer_count(dev, word_cnt);
		spi_set_dma_rx_address(dev, dma_rx_handle);
		spi_set_dma_tx_address(dev, dma_tx_handle);
		spi_set_dma_direction(dev, SPI_DMA_DIR_SPITXRX);
		spi_enable_dma(dev, true);
	} else {
		pr_err("Fullduplex dma not support in this platform\r\n");
	}
}

static void nvt_spi_transfer_polled(struct nvt_spi *dev,u32 count, const u8* tx_data, u8* rx_data)
{
	u32 val, packet_cnt;
...
static void nvt_spi_transfer_dma(struct nvt_spi *dev, u32 count, const u8 *tx_data, u8 *rx_data)
{
	u32 packet_cnt;

	if (dev->dma_support == 0) {
	...
	if (NULL != tx_data) {
		u32 byte_offset = ((uintptr_t)tx_data) & 0x03;
		if (byte_offset) {
		...
		u32 byte_offset = ((uintptr_t)rx_data) & 0x03;
		if (byte_offset) {
			u32 length = sizeof(u32) - byte_offset;
			length = (length > count) ? count : length;
	...
	while (count >= 4* packet_cnt) {
		u32 tr_count = count > (SPI_DMA_MAX_LENGTH&(~0x03)) ?  (SPI_DMA_MAX_LENGTH&(~0x03)) : (count&(~0x03));

		atomic_set(&dev->event, 1);
		if (nvt_get_chip_id() == CHIP_NS02402) {
			if (dev->dma_fullduplex) {
				if (NULL != tx_data && NULL != rx_data) {
					spi_set_dma_read_write(dev, (u8*)rx_data, (u8*)tx_data, tr_count/4);
				} else {
					pr_err("Enable fullduplex but given invalid tx or rx buffer!\r\n");
				}
			} else {
				if (NULL != tx_data)  {
					spi_set_dma_write(dev, (u8*)tx_data, tr_count/4);
				} else {
					spi_set_dma_read(dev, (u8*)rx_data, tr_count/4);
				}
			}
		} else {
			if (NULL != tx_data)  {
		...
		if (NULL != tx_data) {
			dma_addr_t dma_handle;
			dma_handle = dma_map_single(dev->dev, (void*)tx_data, tr_count, DMA_TO_DEVICE);
			if (dma_mapping_error(dev->dev, dma_handle)) {
				pr_err("Error: map tx dma buffer failed\r\n");
				return;
			}
			dma_sync_single_for_cpu(dev->dev, dma_handle, tr_count, DMA_TO_DEVICE);
			dma_unmap_single(dev->dev, dma_handle, tr_count, DMA_TO_DEVICE);
			tx_data += tr_count;
		}

		if (NULL != rx_data) {
			dma_addr_t dma_handle;
			dma_handle = dma_map_single(dev->dev, rx_data, tr_count, DMA_FROM_DEVICE);
			if (dma_mapping_error(dev->dev, dma_handle)) {
				pr_err("Error: map rx dma buffer failed\r\n");
				return;
			}
			dma_sync_single_for_cpu(dev->dev, dma_handle, tr_count, DMA_FROM_DEVICE);
			dma_unmap_single(dev->dev, dma_handle, tr_count, DMA_FROM_DEVICE);
			rx_data += tr_count;
		}
...
static int nvt_spi_probe(struct platform_device *dev)
{
	...
	//add dev pointer in nvt_spi sturcture
	c->dev = &(dev->dev);
	...
	ret = devm_request_irq(&dev->dev, c->irq, nvt_spi_irq, 0, dev->name, c);
	...
	if (!of_property_read_u32(dev->dev.of_node, "dma-support", &dma)) {
		c->dma_support = dma;
		if (nvt_get_chip_id() == CHIP_NS02402) {
			if (!of_property_read_u32(dev->dev.of_node, "dma-fullduplex", &fullduplex)) {
				c->dma_fullduplex = fullduplex;
			}
		}
	}
	...
	if (!of_property_read_u32(dev->dev.of_node, "latch-delay", &latch_delay)) {
		if (latch_delay == 1) {
			...
			printk("spi%d latch delay 2T\r\n", master->bus_num);
		}
		c->latch_clk_delay = latch_delay;
	}

	if (!of_property_read_u32(dev->dev.of_node, "sample-edge", &sample_edge)) {
		if (sample_edge == 1) {
			printk("spi%d sample edge rising edge\r\n", master->bus_num);
		} else if (sample_edge == 2) {
			printk("spi%d sample edge falling edge\r\n", master->bus_num);
		}
		c->sample_edge = sample_edge;
	}
	...
	pr_info("%s: SPI bus driver %s is registered successfully \n", __func__, dev->name);
	return 0;
...
{	
	...
	if (ret) {
		pr_err("spi: cannot stop workqueue\r\n");
		return -1;
	}
...
static int nvt_spi_resume(struct device *dev)
{
	...
	if (ret) {
		pr_err("spi: cannot resume workqueue\r\n");
		return -1;
	}

	return 0;
}
static const struct dev_pm_ops nvt_spi_pmops = {
	.suspend	= nvt_spi_suspend,
	.resume		= nvt_spi_resume,
};
#define NVTSPI_PMOPS &nvt_spi_pmops
#else
#define NVTSPI_PMOPS NULL
#endif /* CONFIG_PM */
