...
static void nvt_sata_phy_trim(struct phy *phy)
{
	...
	EFUSE_PARAM_DATA EFUSE_TRIM_DATA_SATA;

	if (sata_phy->idx == 1) {
		addr = 0x0;
		EFUSE_TRIM_DATA_SATA = EFUSE_SATA_1_TRIM_DATA;
	} else if (sata_phy->idx == 2) {
		addr = 0x5;
		EFUSE_TRIM_DATA_SATA = EFUSE_USB3_2_SATA2_TRIM_DATA;
	} else {
		pr_info("Invalid SATA index\r\n");
		return;
	}

#if IS_ENABLED(CONFIG_NVT_OTP_SUPPORT)
	trim = efuse_readParamOps(EFUSE_TRIM_DATA_SATA, &value);
#else
	trim = -1;
#endif
...
static int nvt_sata_phy_init(struct phy *phy)
{
	...
	void __iomem *cg_base = sata_phy->cg_base;
...
static int nvt_sata_phy_probe(struct platform_device *pdev)
{
	...
	/* We temporarily reset sata with hardcode*/
	res->start = NVT_CG_BASE_PHYS;
	res->end = NVT_CG_BASE_PHYS + 0x5000;
	...
	sata_phy->cg_base = devm_ioremap(dev, res->start, resource_size(res));
...
MODULE_DESCRIPTION("Novatek NS02201 SATA PHY driver");
