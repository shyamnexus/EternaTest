...
#ifdef CONFIG_NVT_FB_DMABUF
	struct fb_dmabuf_export dmabuf_exp;
#endif
	long ret = 0;
...
#ifdef CONFIG_NVT_FB_DMABUF
	case FBIOGET_DMABUF:
		if (copy_from_user(&dmabuf_exp, argp, sizeof(dmabuf_exp)))
			return -EFAULT;

		lock_fb_info(info);
		dmabuf_exp.fd = fb_get_dmabuf(info, dmabuf_exp.flags);
		unlock_fb_info(info);

		if (dmabuf_exp.fd < 0)
			return dmabuf_exp.fd;

		ret = copy_to_user(argp, &dmabuf_exp, sizeof(dmabuf_exp))
			? -EFAULT : 0;
		break;
#endif
	default:
