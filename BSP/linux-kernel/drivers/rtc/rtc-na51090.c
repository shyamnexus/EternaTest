...
static int nvt_rtc_chk_power(struct device *dev, int reset_date)
{
	...
	if (daykey_reg.bit.key & RTC_INT_KEY_POR) { // POR
        ...
        if(reset_date == 0){
            ...
            usr_data0 = (data_reg.bit.rtc_data0) & 0x00FF;

            if( usr_data0 != 0xAA){
                printk("%s: RTC init usr_data0 \r\n", __func__);
                ctrl_reg.reg = rtc_getreg(RTC_CTRL_REG_OFS);
        	    ctrl_reg.bit.cset = 0;
                ctrl_reg.bit.da0_sel = 1;
        	    rtc_setreg(RTC_CTRL_REG_OFS, ctrl_reg.reg);
            
                data_reg.reg = rtc_getreg(RTC_DATA_REG_OFS);
                usr_data0 = (data_reg.bit.rtc_data0) & 0xFF00;
                usr_data0 = usr_data0 | 0X00AA;
                data_reg.bit.rtc_data0 = usr_data0;
    ...
    /* Reset date 1970 when power lost & time invalid & data ! = 0x5555 */
    	...
    	ctrl_reg.bit.pwralarmday_sel = 1;
        ctrl_reg.bit.da0_sel = 1;
    	if (time_ret != 0 || data_ret != 0 ){ //rtc time is invalid & data is not 0xAAAA Reset date
            ctrl_reg.bit.day_sel = 1;
			ctrl_reg.bit.time_sel = 1;
        }
    	rtc_setreg(RTC_CTRL_REG_OFS, ctrl_reg.reg);
...
static int nvt_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (!wait_for_completion_timeout(&cset_completion, msecs_to_jiffies(1000))) {
        dev_err(dev, "RTC CSET timeout, plz check RTC 32K osc VCC_VRTC\n");
        return -ETXTBSY;
    }
...
static int nvt_rtc_probe(struct platform_device *pdev)
{
    ...
    _REGIOBASE = devm_ioremap_resource(dev, memres);
