...
#define AUDIO_OUT_ON 1

#if AUDIO_OUT_ON
#define AUDOUT_CKRATIO      256
#define AUDOUT_TDMCH        2
#define AUDOUT_OPMODE       1 //0 as slave, 1 as master
#define AUDOUT_SR           HD_AUDIO_SR_48000
#define AUDOUT_BIT          HD_AUDIO_BIT_WIDTH_16
#define AUDOUT_MODE         HD_AUDIO_SOUND_MODE_STEREO
...
HD_AUDIO_SOUND_MODE aud_rx_sound_mode = HD_AUDIO_SOUND_MODE_STEREO;
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[id].blk_cnt = 9;
	mem_cfg.pool_info[id].ddr_id = DDR_ID0;
#endif
...
static HD_RESULT set_cap_cfg2(HD_PATH_ID *p_audio_cap_ctrl, HD_AUDIO_SR sample_rate, HD_AUDIO_SOUND_MODE mode, UINT32 vol)
{
	...
	#if 1
	{
		...
		vendor_config.aud_init_cfg.pin_cfg.pinmux.audio_pinmux  = 0x11; //PIN_AUDIO_CFG_I2S_2ND_PINMUX | PIN_AUDIO_CFG_MCLK
		vendor_config.aud_init_cfg.i2s_cfg.bit_clk_ratio = AUDOUT_CKRATIO;
		vendor_config.aud_init_cfg.i2s_cfg.bit_width     = AUDOUT_BIT;
		vendor_config.aud_init_cfg.i2s_cfg.tdm_ch        = AUDOUT_TDMCH;
		vendor_config.aud_init_cfg.i2s_cfg.op_mode       = AUDOUT_OPMODE; // acap (rx) as slave
...
THREAD_DECLARE(uac_thread, arg)
{
	...
	/* config pattern name */
	snprintf(file_path_main, sizeof(file_path_main), "/mnt/sd/uac_bs_%d_%d_%d_pcm.dat", HD_AUDIO_BIT_WIDTH_16, HD_AUDIO_SOUND_MODE_STEREO, gUIUvacAudRxSampleRate[0]);
...
static HD_RESULT set_aout_cfg(HD_PATH_ID *p_audio_out_ctrl)
{
	...
	ret = hd_audioout_open(0, HD_AUDIOOUT_2_CTRL, &audio_out_ctrl); //open this for device control
	if (ret != HD_OK) {
		return ret;
	}

	/* set audio out maximum parameters */
	audio_cfg_param.out_max.sample_rate = AUDOUT_SR;
	...
	audio_driver_cfg_param.output = HD_AUDIOOUT_OUTPUT_HDMI;
...

	{
		...
		vendor_config.aud_init_cfg.i2s_cfg.bit_width     = AUDOUT_BIT;
		vendor_config.aud_init_cfg.i2s_cfg.tdm_ch        = AUDOUT_TDMCH;
		vendor_config.aud_init_cfg.i2s_cfg.op_mode       = AUDOUT_OPMODE;
...
static HD_RESULT set_aout_param(HD_PATH_ID audio_out_ctrl, HD_PATH_ID audio_out_path)
{
	...
	audio_out_out_param.sample_rate = AUDOUT_SR;
...
static HD_RESULT open_module3(AUDIO_CAPONLY *p_caponly)
{
	...
	if((ret = hd_audioout_open(HD_AUDIOOUT_2_IN_0, HD_AUDIOOUT_2_OUT_0, &p_caponly->aout_path)) != HD_OK)
		return ret;
...
static void *playback_thread(void *arg)
{
	...
	INT timestamp;
	AUDIO_CAPONLY *p_caponly = (AUDIO_CAPONLY *)arg;

	/* read test pattern */
	snprintf(filename, sizeof(filename), "/mnt/sd/audio_bs_%d_%d_%d_pcm.dat", AUDOUT_BIT, AUDOUT_MODE, AUDOUT_SR);
	...
	snprintf(filename, sizeof(filename), "/mnt/sd/audio_bs_%d_%d_%d_pcm.len", AUDOUT_BIT, AUDOUT_MODE, AUDOUT_SR);
	...
	while (1) {
		...
		bs_in_buf.sample_rate = AUDOUT_SR;

		/* push in buffer */
resend:
		ret = hd_audioout_push_in_buf(p_caponly->aout_path, &bs_in_buf, -1);
		if (ret != HD_OK) {
...
THREAD_DECLARE(process_thread, arg)
{
	...
	while (p_stream0->prc_exit2 == 0) {
		...
		//printf("enc_push ....\r\n");
		if (p_stream0->codec_type != HD_CODEC_TYPE_RAW && encode_start) {
...
#if AUDIO_OUT_ON
	/* set audfmt config to hdmi via vendor_videoout */
	{
		VENDOR_VIDEOOUT_HDMI_AUDIOFMT audfmt;
		if (AUDOUT_SR == HD_AUDIO_SR_48000) {
			audfmt = VENDOR_VIDEOOUT_HDMI_AUDIO48KHZ;
		} else if (AUDOUT_SR == HD_AUDIO_SR_44100) {
			audfmt = VENDOR_VIDEOOUT_HDMI_AUDIO44_1KHZ;
		} else if (AUDOUT_SR == HD_AUDIO_SR_32000) {
			audfmt = VENDOR_VIDEOOUT_HDMI_AUDIO32KHZ;
		} else {
			audfmt = VENDOR_VIDEOOUT_HDMI_AUDIO_NO_CHANGE;
			printf("audsr(%d) of hdmi is not supported\r\n", AUDOUT_SR);
		}
		ret = vendor_videoout_set(video_out_ctrl, VENDOR_VIDEOOUT_ITEM_HDMI_AUDFMT, (void *)audfmt);
	}
#endif
...
static HD_RESULT open_module(VIDEO_STREAM *p_stream, HD_DIM* p_proc_max_dim, UINT32 out_type)
{
	...
	if(1){
		VENDOR_VIDEOOUT_IN in = {0};
		in.queue_depth = 3;
		ret = vendor_videoout_set(p_stream->vout_path, VENDOR_VIDEOOUT_ITEM_PARAM_IN, &in);
		printf("VENDOR_VIDEOOUT_ITEM_PARAM_IN=%d\n", ret);
	}
...
static void app_main(int argc, char* argv[])
{
	...
	stream[0].hdmi_id = 95;//HD_VIDEOOUT_HDMI_3840X2160P30 ;//97: 3840x2160p60, 95: 3840x2160p30
	...
	ret = set_proc_param(stream[0].proc_main_path, &main_dim, HD_VIDEO_PXLFMT_YUV420, 2, 0);
	if (ret != HD_OK) {
	...
	//audio create capture thread
	THREAD_CREATE(out_thread_id, playback_thread, (void *)&au_cap, "playback_thread");
	if (0 == out_thread_id) {
		printf("create out thread failed");
		goto exit;
	}
	THREAD_RESUME(out_thread_id);
	//start audio capture module
...
#if AUDIO_OUT_ON
	THREAD_DESTROY(out_thread_id); // stop audio
#endif
