...
typedef struct _NVUSB_TRANS_INFO {
	...
	uintptr_t addr; // for NVUSB_TRANS_TYPE_READ
}NVUSB_TRANS_INFO;
...
static unsigned int check_sum(uintptr_t addr, unsigned int size)
{
...
static int interpret_get(NVUSB_TRANS_INFO *p_info)
{
	...
	p_info->sum = check_sum((uintptr_t)m_shm, p_info->data_size);
	return com_write((unsigned char *)p_info, sizeof(NVUSB_TRANS_INFO));
}
...
static int interpret_set(NVUSB_TRANS_INFO *p_info)
{
	...
	if (p_info->sum != check_sum((uintptr_t)m_shm, p_info->data_size)) {
...
static int interpret_read(NVUSB_TRANS_INFO *p_info)
{
	...
	p_info->sum = check_sum((uintptr_t)p_mem, p_info->data_size);
#else
	uintptr_t aligned_pa = ALIGN_FLOOR(p_info->addr, 4096);
	...
	p_info->sum = check_sum((uintptr_t)p_mem+ofs, p_info->data_size);
	hd_common_mem_munmap(p_mem, aligned_size);
#endif
	return com_write((unsigned char *)p_info, sizeof(NVUSB_TRANS_INFO));
}
