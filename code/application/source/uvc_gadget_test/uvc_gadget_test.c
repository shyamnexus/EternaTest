...
#if 1
void DumpMem(unsigned long Addr, unsigned int Size, unsigned int Alignment)
{
	unsigned int i;
	unsigned char *pBuf = (unsigned char *)Addr;

	printf("####0x%lX, %d #### ", Addr, Size);
	for (i = 0; i < Size; i++) {
...
uvc_video_set_format(struct uvc_device *dev)
{
	...
	printf("Setting format to 0x%08x %ux%u %d\n",
			   ...
			   int iframe, int iformat)
{
	...
	ctrl->dwMaxPayloadTransferSize = 2048;	/* TODO this should be filled by the
driver. */
...
uvc_events_process_data(struct uvc_device *dev, struct uvc_request_data *data)
{
	...
	case UVC_VS_PROBE_CONTROL:
		...
		target = &dev->probe;
		//DumpMem((unsigned long) &data->data, data->length, 16);
...
//printf("ctrl->bFormatIndex %d, bFrameIndex %d, dwFrameInterval %d\n", ctrl->bFormatIndex,ctrl->bFrameIndex, ctrl->dwFrameInterval);
//ctrl->bFormatIndex = 2;
//ctrl->bFrameIndex = 2;
//ctrl->dwFrameInterval = 50000000;
...
if (resp.length>0) {
	//DumpMem((unsigned long) &resp.data[0], resp.length, 16);
}
