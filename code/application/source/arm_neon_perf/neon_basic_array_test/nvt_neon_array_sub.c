...
int routine(uint8_t *arr, const uint8_t *num, uint32_t w, uint32_t h, uint32_t times);
...
void test(uint32_t w, uint32_t h, uint32_t times)
{
	...
	ret = routine(arr, &num, w, h, times);
	if (ret < 0) {
		printf("test error with ret=%d\n", ret);
		free(arr);
		return;
	}
	...
	clock_gettime(CLOCK_REALTIME, &end);
	diff = (end.tv_sec - start.tv_sec) * 1000000 + (end.tv_nsec - start.tv_nsec) / 1000;
	printf("subtracting %4ux%4u matrix with NEON:  %5u us\n", w, h, diff / times);
