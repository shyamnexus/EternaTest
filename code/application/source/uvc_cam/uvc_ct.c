	...
	switch(request)
	{
		...
		case SET_CUR: {
				...
				if (MAX_CAP_SIZE_W != uiWidth && uiHeight != MAX_CAP_SIZE_H) {
					...
					width = ALIGN_CEIL_8(MAX_CAP_SIZE_W*1000 / ZoomRatio);
					height = ALIGN_CEIL_8(MAX_CAP_SIZE_H*1000 / ZoomRatio);

					coor_x_c = (MAX_CAP_SIZE_W/2)-(width/2);
					coor_y_c = (MAX_CAP_SIZE_H/2)-(height/2);

					coor_x_d = (MAX_CAP_SIZE_W-width)/2/10;
					coor_y_d = (MAX_CAP_SIZE_H-height)/2/10;

					coor_x = (UINT32)(coor_x_c + (ptz_param.pan_idx*coor_x_d));
					coor_y = (UINT32)(coor_y_c - (ptz_param.tilt_idx*coor_y_d));

					//printf("zoom_idx: %d pData[0]:%d idx1: %d ZoomRatio: %d, %d x %d %d %d %d %d\r\n", ptz_param.zoom_idx, pData[0], idx1, ZoomRatio, uiWidth, uiHeight, width, height, coor_x, coor_y);
					...
					if (pData[0] == 0) {
						...
						if (last_zoom != 0) {
							...
							if (0) {//(stream[0].codec_type != HD_CODEC_TYPE_RAW) {
								...
								set_proc_cfg(&stream[0].proc_ctrl, &stream[0].cap_dim, en_slice, &stream[0].cap_dim);
							...
							/*else {
								...
								set_cap_param(stream[0].cap_path, &stream[0].cap_dim, stream[0].fps, &stream[0].cap_dim);
					...
					} else if ((coor_x + width < MAX_CAP_SIZE_W) && (coor_y + height < MAX_CAP_SIZE_H) && (width >= uiWidth) && (height >= uiHeight)) {
