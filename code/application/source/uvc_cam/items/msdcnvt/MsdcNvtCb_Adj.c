	...
	for (i = 0; i < nItem; i++) {
		...
		case VAR_TYPE_BOOL:
			*((BOOL *)pItem->pVar) = *((BOOL *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((BOOL *)pVar));
			break;
		case VAR_TYPE_UINT32:
			*((UINT32 *)pItem->pVar) = *((UINT32 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((UINT32 *)pVar));
			break;
		case VAR_TYPE_INT32:
			*((INT32 *)pItem->pVar) = *((INT32 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((INT32 *)pVar));
			break;
		case VAR_TYPE_UINT16:
			*((UINT16 *)pItem->pVar) = *((UINT16 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((UINT16 *)pVar));
			break;
		case VAR_TYPE_INT16:
			*((INT16 *)pItem->pVar) = *((INT16 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((INT16 *)pVar));
			break;
		case VAR_TYPE_UINT8:
			*((UINT8 *)pItem->pVar) = *((UINT8 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((UINT8 *)pVar));
			break;
		case VAR_TYPE_INT8:
			*((INT8 *)pItem->pVar) = *((INT8 *)pVar);
			printf("%s=%d,", pItem->Tag, (int)*((INT8 *)pVar));
			break;
		}
	}
	printf("\r\n");
