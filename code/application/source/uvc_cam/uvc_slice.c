...
#endif
static void yuv_slice_push_finalpart_to_usb(uintptr_t buf_va, HD_VIDEO_FRAME *pVideoFrame)
{
	UVAC_STRM_FRM         strmFrm = {0};
...
static int yuv_slice_push_to_usb(uintptr_t buf_va, HD_VIDEO_FRAME *pVideoFrame)
{
	UVC_SLICE_INFO        *pObj = &g_slice_info;
...
HD_RESULT yuv_frame_push_to_usb(uintptr_t buf_va, HD_VIDEO_FRAME *pVideoFrame)
{
	UVAC_STRM_FRM      strmFrm = {0};
	...
	if (stream->isYUV) {
		...
		strmFrm.size = pVideoFrame->dim.w * pVideoFrame->dim.h * 3 / 2;
	}
	strmFrm.pStrmHdr = 0;
	strmFrm.strmHdrSize = 0;
	strmFrm.va = buf_va;
	...
	{
		ER ret;
		ret = UVAC_WaitStrmDone(UVAC_STRM_VID);
		if (ret == E_OK) {
			break;
		}
	};
...
HD_RESULT yuv_push_to_usb(uintptr_t buf_va, HD_VIDEO_FRAME *pVideoFrame)
{
	UVC_SLICE_INFO        *pObj = &g_slice_info;
...
HD_RESULT mjpg_slice_push_to_usb(uintptr_t buf_va, HD_VIDEOENC_BS *p_bs)
{
	//UVC_SLICE_INFO     *pObj = &g_slice_info;
	UVAC_STRM_FRM      strmFrm = {0};
	UINT32             slice_cnt;
	...
	if (slice_cnt == 0) {
		...
		strmFrm.frame_type = UVAC_VIDEO_FRM_FINAL;
		//printf("slice_cnt =0x%08X, bs_size = %d\r\n", slice_cnt, strmFrm.size);

		#if 0
		frame_size += strmFrm.size;
		frame_cnt++;

		if (frame_cnt == stream->fps) {
			frame_cnt = 0;

			printf("AVG size = %03f\r\n", ((float)frame_size)/1024/1024/stream->fps);
	...
	//hd_common_mem_flush_cache((void *)ALIGN_FLOOR_4(strmFrm.va), strmFrm.size + (strmFrm.va - ALIGN_FLOOR_4(strmFrm.va)));
	//printf("slice_cnt =0x%08X, frame_type = %d, bs_addr = 0x%x bs_size = 0x%x\r\n", slice_cnt, strmFrm.frame_type, strmFrm.va, strmFrm.size);
	//printf("slice_cnt =0x%08X, bs_size = 0x%x\r\n", slice_cnt, strmFrm.size);
