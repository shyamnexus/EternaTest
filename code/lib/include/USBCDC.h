...
typedef enum _CDC_TYPE {
	CDC_TYPE_U2 = 0,
	CDC_TYPE_U3,
	ENUM_DUMMY4WORD(CDC_TYPE)
} CDC_TYPE;
...
typedef enum _CDC_CONFIG_ID {
	...
	CDC_CONFIG_USB_TYPE,                 ///> asynchronous write mode
...
*/
extern void CDC_SetConfig(CDC_CONFIG_ID ConfigID, ULONG Value);
...
/**
    Write data to USB host with timeout.
    ...
    @return
        - @b Actual transfered length.
*/
extern UINT32 CDC_WriteDataTimeout(UINT8 *pBuf, UINT32 *pBufSize, UINT32 timeout);
#endif
