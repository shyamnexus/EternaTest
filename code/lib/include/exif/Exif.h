...
typedef struct _EXIF_SETTAG {
	...
	uintptr_t uiTagDataAddr;     ///< [in] tag data addr
...
typedef struct _EXIF_GETTAG {
	...
	uintptr_t uiTagDataAddr; ///< [out] tag data address
...
typedef struct _MAKERNOTE_INFO {
	uintptr_t uiMakerNoteAddr;    ///< MakerNote address
...
*/
extern EXIF_HANDLE_ID  EXIF_GetHandleID(uintptr_t ExifDataAddr);
