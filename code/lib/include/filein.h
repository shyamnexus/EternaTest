...
typedef struct {
	...
	uintptr_t buf_end_addr;                             ///< read file buffer end addr
	UINT64    blk_size;                                 ///< file block size
	UINT32    tt_blknum;                                ///< total file block number
	uintptr_t uiThisAddr;
	uintptr_t uiDemuxAddr;
...
extern ER FileIn_SetParam(UINT32 Id, UINT32 Param, ULONG Value);
extern ER FileIn_GetParam(UINT32 Id, UINT32 Param, VOID *pValue);
