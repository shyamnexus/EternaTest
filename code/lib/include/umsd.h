...
typedef struct {
	...
	unsigned long    VendorCmdBuf;                                         ///< Vendor CBW buffer that device will receive from host for CBW buffer. Fixed 31 bytes.
	unsigned long    VendorCSWBuf;                                         ///< Vendor CSW buffer that device will return to host for CSW buffer, MSDC task should prepare this buffer for callback function, Fixed 13 bytes.
...
typedef BOOL (*MSDC_Verify_CB)(unsigned long pCmdBuf, unsigned long *pDataBuf);       ///< Callback for verify the Vendor Command is supported or not
...
typedef struct {
	...
	uintptr_t           uiMsdcBufAddr_va;                           ///< Buffer address for MSDC-Task operation.
	uintptr_t           uiMsdcBufAddr_pa;                           ///< Buffer address for MSDC-Task operation.
...
typedef struct {
	...
	ULONG 			(*GetConfig)(USBMSDC_CONFIG_ID ConfigID);
