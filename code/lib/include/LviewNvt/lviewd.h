...
*/
//@{
typedef int cyg_lviewd_getJpg(LVIEWD_ULONG *jpgAddr, int *jpgSize); ///< Callback function for get one jpg frame.
typedef void cyg_lviewd_notify(int status);                ///< Callback function for notify the lviewd status.
typedef void cyg_lviewd_hwmem_memcpy(LVIEWD_ULONG uiDst, LVIEWD_ULONG uiSrc, LVIEWD_U32 uiSize);///< Callback function for doing HW memcpy.
//@}
...
typedef struct{
    unsigned long           addr;           ///<   frame address
