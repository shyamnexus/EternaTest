...
*/
typedef struct {
	ER(*Probe)(uintptr_t addr, UINT32 size);                                           ///< Probe file type
	ER(*Initialize)(void);                                                          ///< Initailize file container object
	ER(*SetMemBuf)(uintptr_t startAddr, UINT32 size);                                  ///< Set memeory buffer to file container object
	ER(*ParseHeader)(uintptr_t hdrAddr, UINT32 hdrSize, void *pobj);                   ///< Parse header
	ER(*GetInfo)(UINT32 type, VOID *pparam1, VOID *pparam2, VOID *pparam3);   ///< Get information from container
	ER(*SetInfo)(UINT32 type, ULONG param1, ULONG param2, ULONG param3);         ///< Get information from container
	ER(*CustomizeFunc)(UINT32 type, void *pobj);                                    ///< customize function, if needed
	ER(*cbReadFile)(UINT8 *type, UINT64 pos, UINT32 size, uintptr_t addr);             ///< file read callback function
...
typedef struct {
	UINT32  checkID;
	ULONG   uiFileStartAddr;        ///< AVI file start address
...
typedef struct {
	ULONG  fileStartAddr;//in: file address
	UINT32 freeSize;     //in: avaliable size
	ULONG  yAddr;        //in: y starting addr
	UINT32 imageWidth;   //out: image width
	UINT32 imageHeight;  //out: image height
	ULONG  cbAddr;       //out: cb addr
	ULONG  crAddr;       //out: cr addr
