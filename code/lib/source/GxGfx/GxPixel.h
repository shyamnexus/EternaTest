...
/*
void PIXEL_2D_Begin(PIXEL_2D* p, IRECT* pRect);
...
#define PIXEL_2D_Check(p)                           if( (!((p)->Begin)) || (!((p)->BeginLine)) || (!((p)->BeginPixel)) || (!((p)->Read))   \
		|| (!((p)->Write)) || (!((p)->EndPixel)) || (!((p)->EndLine)) || (!((p)->End)) \
		|| (!((p)->WriteOffset)) ||(!((p)->GotoXY)) || (!((p)->LineBuf)) || (!((p)->PlaneBuf)) ) return GX_NULL_POINTER;
#ifdef __cplusplus
} //extern "C"
#endif
