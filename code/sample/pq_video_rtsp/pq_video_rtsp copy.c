...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim, UINT32 path)
{
	...
	UINT32 color_bar_width = 200;
	{
		...
		if (sensor_info[sensor_sel_1].sensor_num != SEN_SEL_PATGEN) {
			...
			// For GC5603, use actual sensor output size (2960), not cropped size
			if (sensor_sel_1 == SEN_SEL_GC5603) {
				video_in_param.dim.w = 2960;
				video_in_param.dim.h = 1664;
			} else {
	...
	//crop for GC5603 to align width to 32 for BNR
	{
		HD_VIDEOCAP_CROP video_crop_param = {0};

		if (sensor_sel_1 == SEN_SEL_GC5603) {
			// Crop 2960 to 2944 (center crop: remove 8 pixels from each side)
...
int main(int argc, char** argv)
{
	...
	if (sensor_info[sensor_sel_1].sensor_num == SEN_SEL_PATGEN) {
		...
		// Adjust for GC5603 cropped output (2960 -> 2944 for 32-alignment with BNR)
		if (sensor_sel_1 == SEN_SEL_GC5603) {
			vdo_size_w_1 = 2944;
		}
	}
