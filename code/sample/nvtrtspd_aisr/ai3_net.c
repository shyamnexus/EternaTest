...
//dump net result
#define DUMP_POSTPROC_INFO			0  // dump result buffer info after proc()
...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	// load label
	ret = _load_label((UINTPTR)p_net->out_class_labels, LABEL_LEN, p_net->net_cfg.label_filename);
	if (ret != HD_OK) {
...
UINTPTR need_to_free_pa = 0;//change p_net->out_mem[0].pa to do aisr
static HD_RESULT network_close(NET_PATH_ID net_path)
{
	...
	for (i = 0 ; i < p_net->net_info.out_buf_cnt; i++) {
		if(i==0)
			p_net->out_mem[0].pa = need_to_free_pa;
...
VIDEO_LIVEVIEW ai_stream[1] = {0}; //0: net proc
...
static HD_RESULT network_dump_out_buf(NET_PATH_ID net_path, VENDOR_AI3_BUF *p_outbuf)
{
	...
	FLOAT *p_outbuf_float = (FLOAT *)get_post_buf(size * sizeof(FLOAT));
	ret = vendor_ai_cpu_util_fixed2float((VOID *)p_outbuf->va, p_outbuf->fmt, p_outbuf_float, p_outbuf->scale_ratio, size, p_outbuf->zero_point);
...
#endif
		{
			CHAR dump_path[128] = "aisr/output_buf.bin";
...
static HD_RESULT allocate_buf_by_out_path_list(NET_PATH_ID net_path)
{
	...
	for (i = 0; i < p_net->net_info.out_buf_cnt; i++) {
		...
		if (ret != HD_OK) {
			...
			if(i==0)
				need_to_free_pa = p_net->out_mem[0].pa;
		}
...
HD_RESULT network_user_oneshot(VIDEO_LIVEVIEW *p_stream)
{
	HD_RESULT ret = HD_OK;
...
HD_RESULT set_image(VIDEO_LIVEVIEW *p_stream,HD_VIDEO_FRAME *input_frame,HD_VIDEO_FRAME *output_frame)
{
	HD_RESULT ret = HD_OK;
	NET_PROC *p_net = g_net + p_stream->net_path;

	NET_IN *p_net_in = g_in + p_stream->in_path;
	p_net_in->src_img.pa = input_frame->phy_addr[0]; 	
	p_net->out_mem[0].pa = output_frame->phy_addr[0];
	return ret;
}

HD_RESULT network_user_trigger(void)
{
	HD_RESULT ret = HD_OK;

	VIDEO_LIVEVIEW *p_stream = &ai_stream[0];
	NET_PROC *p_net = g_net + p_stream->net_path;
	UINT32 proc_id = p_net->proc_id;
    UINT32 job_priority = VENDOR_AI_JOB_PRI(0);
    UINT32 core_mask = VENDOR_AI_CORE_MASK_DEFAULT;
...
//	printf("net_path(%u), proc_id(%u) run ...\n", p_stream->net_path, proc_id);
	ret = set_buf_by_out_path_list(p_stream->net_path);
	if (HD_OK != ret) {
		printf("net_path(%u), proc_id(%u) set in_buf fail(%d) !!\n", p_stream->net_path, proc_id, ret);
		goto skip;
...
int do_main(void)
{
	...
	//net proc
	NET_PROC_CONFIG net_cfg = {
		.model_filename = "aisr/nvt_model_4x.bin",
		.label_filename = "aisr/labels.txt"
	};

	printf("\r\n\r\n");

	ai_stream[0].in_path = 0;
	ai_stream[0].net_path = 0;

	// init mem
	{
		// config common pool
		network_mem_config(ai_stream[0].net_path, &mem_cfg, &net_cfg);
	}
	...
	// set open config
	ret = input_set_config(ai_stream[0].in_path, &in_cfg);
	if (HD_OK != ret) {
		printf("in_path(%u) input_set_config fail=%d\n", ai_stream[0].in_path, ret);
		goto exit;
	}
	ret = network_set_config(ai_stream[0].net_path, &net_cfg);
	if (HD_OK != ret) {
		printf("net_path(%u) network_set_config fail=%d\n", ai_stream[0].net_path, ret);
		goto exit;
	}

	// open video_liveview modules
	ret = open_module(&ai_stream[0]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	...
	// start
	input_start(ai_stream[0].in_path);
	network_user_start(&ai_stream[0]);

	allocate_buf_by_out_path_list(ai_stream[0].net_path);
#if 0
#if DBG_PERF_TIME_UT2
...
#endif
	// stop
	input_stop(ai_stream[0].in_path);
	network_user_stop(&ai_stream[0]);

exit:

	// close video_liveview modules
	ret = close_module(&ai_stream[0]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
