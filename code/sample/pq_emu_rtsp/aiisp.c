...
#define AIISP_MODEL_NAME_1 "nvt_model_539A_frame_2_5_1_1_0"
#define AIISP_MODEL_NAME_2 "nvt_model_539A_frame_2_5_1_1_0_low"

CHAR ai_model_name[2][256] = {AIISP_MODEL_NAME_1, AIISP_MODEL_NAME_2};
...
static VIDEO_AIISP aiisp_stream[VIDEO_AIISP_MAX] = {0};
...
HD_RESULT network_get_ai_cb(VIDEO_AIISP *p_stream)
{
	HD_RESULT ret = HD_OK;
...
// external functions
//=============================================================================
HD_RESULT aiisp_get_model_name(UINT32 path, CHAR *ai_name)
{
	HD_RESULT ret = HD_OK;

	if (path == 1) {
		memcpy(ai_name, ai_model_name[1], sizeof(ai_model_name) / 2);
	} else {
		memcpy(ai_name, ai_model_name[0], sizeof(ai_model_name) / 2);
	}

	return ret;
}

UINT32 aiisp_get_proc_id(UINT32 path)
{
	if (path == 1) {
		return ((NET_PROC*)(g_net + aiisp_stream[0].second_net_path))->proc_id;
	} else {
		return ((NET_PROC*)(g_net + aiisp_stream[0].net_path))->proc_id;
	}
}

HD_RESULT aiisp_get_ai_cb(uintptr_t *ai_cb)
{
	HD_RESULT ret = HD_OK;

	ret = vendor_ai3_dev_get(VENDOR_AI3_CFG_AI_CB, ai_cb);
	if (ret != HD_OK) {
		printf("get ai cb fail (%d) \n", ret);
		return ret;
	}
...
HD_RESULT aiisp_set_model_name(UINT32 path, CHAR *ai_name)
{
	HD_RESULT ret = HD_OK;

	if (path == 1) {
		memcpy(ai_model_name[1], ai_name, sizeof(ai_model_name) / 2);
	} else {
		memcpy(ai_model_name[0], ai_name, sizeof(ai_model_name) / 2);
	}

	return ret;
}

HD_RESULT aiisp_init(void)
{
	HD_RESULT ret = HD_OK;
...
HD_RESULT aiisp_open(VIDEO_AIISP_ID aiisp_id)
{
	HD_RESULT ret = HD_OK;

	snprintf(aiisp_stream[aiisp_id].net_proc_cfg.model_filename, 256 - 1, "/mnt/sd/para/%s.bin", ai_model_name[0]);
	snprintf(aiisp_stream[aiisp_id].second_net_proc_cfg.model_filename, 256 - 1, "/mnt/sd/para/%s.bin", ai_model_name[1]);
	aiisp_stream[aiisp_id].net_path = aiisp_id * 2;
	aiisp_stream[aiisp_id].second_net_path = aiisp_id * 2 + 1;
	aiisp_stream[aiisp_id].in_path = 0;

	ret = network_mem_config(aiisp_stream[aiisp_id].net_path, &aiisp_stream[aiisp_id].net_proc_cfg);
	if (ret != HD_OK) {
		printf("network_mem_config stream0 fail = %d \n", ret);
		return ret;
	}

	ret = network_open(aiisp_stream[aiisp_id].net_path);
	if (ret != HD_OK) {
		printf("network_open net_path fail = %d \n", ret);
		return ret;
	}

	ret = network_mem_config(aiisp_stream[aiisp_id].second_net_path, &aiisp_stream[aiisp_id].second_net_proc_cfg);
	if (ret != HD_OK) {
		printf("network_mem_config second_net_path fail = %d \n", ret);
		return ret;
	}

	ret = network_open(aiisp_stream[aiisp_id].second_net_path);
	if (ret != HD_OK) {
		printf("network_open second_net_path fail = %d \n", ret);
		return ret;
	}
...
HD_RESULT aiisp_start(VIDEO_AIISP_ID aiisp_id)
{
	HD_RESULT ret = HD_OK;

	ret = network_start(aiisp_stream[aiisp_id].net_path);
	if (ret != HD_OK) {
		printf("ai start 0 fail = %d \n", ret);
	}

	ret |= network_start(aiisp_stream[aiisp_id].second_net_path);
	if (ret != HD_OK) {
		printf("ai start 1 fail = %d \n", ret);
	}

	return ret;
}

HD_RESULT aiisp_stop(VIDEO_AIISP_ID aiisp_id)
{
	HD_RESULT ret = HD_OK;

	ret = network_stop(aiisp_stream[aiisp_id].net_path);
	if (ret != HD_OK) {
		printf("ai stop 0 fail = %d \n", ret);
	}

	ret |= network_stop(aiisp_stream[aiisp_id].second_net_path);
	if (ret != HD_OK) {
...
HD_RESULT aiisp_close(VIDEO_AIISP_ID aiisp_id)
{
	HD_RESULT ret = HD_OK;

	ret = network_close(aiisp_stream[aiisp_id].net_path);
	if (ret != HD_OK) {
		printf("ai close 0 fail = %d \n", ret);
	}

	ret |= network_close(aiisp_stream[aiisp_id].second_net_path);
	if (ret != HD_OK) {
...
HD_RESULT aiisp_uninit(void)
{
	HD_RESULT ret = HD_OK;

	ret = input_uninit();
	if (ret != HD_OK) {
		printf("ai uninit 0 fail = %d \n", ret);
	}

	ret |= network_uninit();
	if (ret != HD_OK) {
		printf("ai uninit 1 fail = %d \n", ret);
	}
