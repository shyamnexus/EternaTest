...
#else
#define IME_POSTSHARPEN_ENABLE     0
#endif
...
static CHAR isp_cfg_name[100] = "isp_os04a10_0";
...
static UINT32 aiisp_mode = AI_MODE_SEL_DISABLE; //0: Disable; 1: AI Model 0; 2: AI Model 1
#endif
...
HD_RESULT plug_pq_emu_init(void)
{
	...
	ret = set_proc_param(emu_stream.proc_path, &main_dim);
	if (ret != HD_OK) {
	...
	ret = set_proc_param(emu_stream.proc_path_sub, &sub_dim);
	if (ret != HD_OK) {
	...
	#if (IME_POSTSHARPEN_ENABLE)
	ret = set_proc_param(emu_stream.proc_path_3dnr, &main_dim);
	if (ret != HD_OK) {
	...
	#if (VPE_FUN_ENABLE)
	if (vpe_enable) {
		ret = set_proc_param(emu_stream.proc_path2, &main_dim);
		if (ret != HD_OK) {
	...
	#if (AI_ENABLE)
	if (aiisp_flow_enable) {
		isp_enable_fpn(sen_vcap_id);

		isp_set_total_gain(init_total_gain);
...
MAIN(argc, argv)
{
	...
	while (trig) {
		...
		printf("----------------------------------------\n");
		#if (AI_ENABLE)
		printf("  70.  Set ai model (%s) \n", (aiisp_mode == AI_MODE_SEL_DISABLE) ? "disable" : (aiisp_mode == AI_MODE_SEL_0) ? "AI Model 0" : "AI Model 1");
		printf("  71.  Set ai flow eable (%s) \n", aiisp_flow_enable ? "enable" : "disable");
		printf("  72.  Set ai message print (%s) \n", isp_get_msg_print() ? "enable" : "disable");
		{
			CHAR file_name[64];

			ai_get_model_name1(file_name);
			printf("  73.  Set ai model name1 (%s) \n", file_name);
			ai_get_model_name2(file_name);
			printf("  74.  Set ai model name2 (%s) \n", file_name);
			isp_get_json_name(file_name);
			printf("  75.  Set aiisp json name (%s) \n", file_name);
			isp_get_fpn_name(file_name);
			printf("  76.  Set isp fpn name (%s) \n", file_name);
		}
		printf("----------------------------------------\n");
		#endif
		printf("   0.  Quit\n");
		...
		case 70:
			printf("Current %s \n", (aiisp_mode == AI_MODE_SEL_DISABLE) ? "disable" : (aiisp_mode == AI_MODE_SEL_0) ? "AI Model 0" : "AI Model 1");
			printf("Set ai model(0: disable; 1: AI Model 0; 2: AI Model 1)>> \n");
			aiisp_mode = (UINT32)get_choose_int();
			printf("Set to %s \n", (aiisp_mode == AI_MODE_SEL_DISABLE) ? "disable" : (aiisp_mode == AI_MODE_SEL_0) ? "AI Model 0" : "AI Model 1");
			ai_model_sel(aiisp_mode);
		break;
		...
		case 72:
			{
				BOOL enable = isp_get_msg_print();

				printf("Current %s \n", enable ? "enable" : "disable");
				printf("Set ai flow enable(0: disable; 1: enable)>> \n");
				enable = (UINT32)get_choose_int();
				isp_set_msg_print(enable);
				printf("Set to %s \n", enable ? "enable" : "disable");
			}
		break;
		...
		case 73:
			{
				...
				ai_get_model_name1(ai_model_name);
				...
				scanf("%s", ai_model_name);
				printf("Set to %s \n", ai_model_name);
				ai_set_model_name1(ai_model_name);
			}
		break;
		...
		case 74:
			{
				CHAR ai_model_name[64];
				ai_get_model_name2(ai_model_name);
				...
				scanf("%s", ai_model_name);
				printf("Set to %s \n", ai_model_name);
				ai_set_model_name2(ai_model_name);
			}
		break;

		case 75:
			{
				CHAR aiisp_json_name[64];
				isp_get_json_name(aiisp_json_name);
				...
				printf("Current %s \n", aiisp_json_name);
				printf("Set aiisp json name >> \n");
				scanf("%s", aiisp_json_name);
				printf("Set to %s \n", aiisp_json_name);
				isp_set_json_name(aiisp_json_name);
			}
		break;

		case 76:
			{
				CHAR isp_fpn_name[64];
				isp_get_fpn_name(isp_fpn_name);
				...
				printf("Current %s \n", isp_fpn_name);
				printf("Set isp fpn name >> \n");
				scanf("%s", isp_fpn_name);
				printf("Set to %s \n", isp_fpn_name);
				isp_set_fpn_name(isp_fpn_name);
			}
		break;
		#endif
