...
static int mem_cma_test(void)
{
	...
	if (vos_cpu_get_phy_addr(cma_info.vaddr) != cma_info.paddr) {
		DBG_ERR("phy_addr conv failed, 0x%X != 0x%X\r\n", vos_cpu_get_phy_addr(cma_info.vaddr), cma_info.paddr);
	}
...
int file_test(void)
{
	...
	DBG_DUMP("vos_file_open write fd = 0x%X\r\n", fd);
	...
	DBG_DUMP("vos_file_open read fd = 0x%X, read_buf 0x%X\r\n", fd, (unsigned int)read_buf);
