...
#define USER_2DLUT_SZ(w,h,lofs)   ( (w&0xFFF)|((h&0xFFF)<<12)|((lofs&0xFFF)<<24) )
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[i].blk_cnt = 16;
...
EXAMFUNC_ENTRY(hd_video_record_with_vsp_aiisp, argc, argv)
{
	HD_RESULT ret;
	INT key;
	...
	if (argc == 1) {
		...
		printf("  <folder_num>  : 0(/mnt/sd/vsp_scenes_0), 1(/mnt/sd/vsp_scenes_1), 2(sd/vsp_scenes_2)\r\n");
	...
	if (argc >= 7) {
		g_dec_mode = atoi(argv[7]);
		if (USE_PLAYBACK_FLOW == 0) {
			printf("Use dec mode need to set macro: USE_PLAYBACK_FLOW to 1");
			goto exit;
		}
		g_dec_type = enc_type;
		stream[0].dec_type= enc_type;
		stream[1].dec_type= enc_type;
		stream[2].dec_type= enc_type;
		stream[3].dec_type= enc_type;
	}
	if (argc >= 8) {
		g_folder_num = atoi(argv[8]);
	}
	if (argc >= 9) {
		vdo_br = atof(argv[9]) * 1024 * 1024;
	}
	...
	if (g_dec_mode) {
		...
		ret = pthread_create(&stream[0].feed_thread_id, NULL, feed_bs_thread_1, (void *)&stream[0]);
		if (ret < 0) {
			printf("create playback thread 1st failed");
			goto exit;
		}
		stream[0].flow_start = 1;
		// create decode_thread (push_in bitstream 2nd)
		ret = pthread_create(&stream[1].feed_thread_id, NULL, feed_bs_thread_2, (void *)&stream[1]);
		if (ret < 0) {
			printf("create playback thread 2nd failed");
			goto exit;
		}
		stream[1].flow_start = 1;
		// create decode_thread (push_in bitstream 3rd)
		ret = pthread_create(&stream[2].feed_thread_id, NULL, feed_bs_thread_3, (void *)&stream[2]);
		if (ret < 0) {
			printf("create playback thread 3rd failed");
			goto exit;
		}
		stream[2].flow_start = 1;
		// create decode_thread (push_in bitstream 4th)
		ret = pthread_create(&stream[3].feed_thread_id, NULL, feed_bs_thread_4, (void *)&stream[3]);
		if (ret < 0) {
			printf("create playback thread 4th failed");
			goto exit;
		}
		stream[3].flow_start = 1;
	}
