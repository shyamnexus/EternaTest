...
#define VOS_AUTO_TIMEOUT_PRINT  0  //for debug
#define VOS_AUTO_TIMEOUT_SEC    53 //prefer a prime number and avoid recording seconds (e.g. 30 sec, 60 sec)

#ifdef CONFIG_DEFAULT_HUNG_TASK_TIMEOUT
STATIC_ASSERT(VOS_AUTO_TIMEOUT_SEC < CONFIG_DEFAULT_HUNG_TASK_TIMEOUT); //prevent hung task warnings
STATIC_ASSERT(VOS_AUTO_TIMEOUT_SEC > 0);
#endif
...
void rtos_flag_init(unsigned long max_flag_num)
{
	...
	//coverity[missing_lock]: init function, only call once
	g_curflg_id = 1;
...
ER vos_flag_create(ID *p_flgid, T_CFLG *pk_cflg, char *name)
{
	...
	vk_init_waitqueue_head(&p_flagcell->st_waitq);
	...
	if (pk_cflg) {
		...
		//coverity[missing_lock]: other functions will not access before vos_flag_create
...
ER vos_flag_set(ID flgid, FLGPTN setptn)
{
	...
	vk_wake_up(&p_flagcell->st_waitq);
	unl_cpu(flags);
...
int _vos_flag_wait(PFLGPTN p_flgptn, ID flgid, FLGPTN waiptn, UINT wfmode, int timeout_tick, int interruptible)
{
	...
	do {
		if (interruptible) {
			 ...
			 */
			if (wfmode & TWF_ORW) {
				wait_ret = vk_wait_event_interruptible(p_flagcell->st_waitq, p_flagcell->st_bits & waiptn);
			} else {
				wait_ret = vk_wait_event_interruptible(p_flagcell->st_waitq, (p_flagcell->st_bits & waiptn) == waiptn);
			}
			if (0 == wait_ret) {
			 ...
			 */
			int auto_timeout_tick = msecs_to_jiffies(VOS_AUTO_TIMEOUT_SEC*1000);
			do {
				if (wfmode & TWF_ORW) {
					wait_ret = vk_wait_event_timeout(p_flagcell->st_waitq, p_flagcell->st_bits & waiptn, auto_timeout_tick);
				} else {
					wait_ret = vk_wait_event_timeout(p_flagcell->st_waitq, (p_flagcell->st_bits & waiptn) == waiptn, auto_timeout_tick);
				}
				#if VOS_AUTO_TIMEOUT_PRINT
				if (!wait_ret) {
					DBG_WRN("auto timeout: flgid %d, sec %d\r\n", flgid, VOS_AUTO_TIMEOUT_SEC);
				}
				#endif
			} while (0 == wait_ret);
			//loop until the condition is met, since the timeout is not requested by users
		} else {
			/*
			 * Refer to include/linux/wait.h
			 * wait_event_timeout
			 ...
			 */
			if (wfmode & TWF_ORW) {
				wait_ret = vk_wait_event_timeout(p_flagcell->st_waitq, p_flagcell->st_bits & waiptn, timeout_tick);
			} else {
				wait_ret = vk_wait_event_timeout(p_flagcell->st_waitq, (p_flagcell->st_bits & waiptn) == waiptn, timeout_tick);
			}
		}
...
#endif
{
	...
	p_flagcell = g_st_flag_ctrl.p_flags;
	for (i = 0; i < gflgnum; i++) {
		//coverity[missing_lock]: debug dump function, no need to lock
...
int _IOFUNC_FLAG_IOCMD_SET(unsigned long arg)
{
	...
	if (E_OK != vos_flag_set((ID)(ioarg.id&INT_MAX), (FLGPTN)ioarg.bits)) {
		return -EFAULT;
	}
...
int _IOFUNC_FLAG_IOCMD_CLR(unsigned long arg)
{
	...
	if (E_OK != vos_flag_clr((ID)(ioarg.id&INT_MAX), (FLGPTN)ioarg.bits)) {
		return -EFAULT;
	}
...
int _IOFUNC_FLAG_IOCMD_WAIT(unsigned long arg)
{
	...
	ret = _vos_flag_wait(&st_flg_ptn, (ID)(ioarg.id&INT_MAX), (FLGPTN)ioarg.bits, (UINT)ioarg.mode, tick, ioarg.interruptible);
	if (ret == E_RLWAI) {
...
int _IOFUNC_FLAG_IOCMD_CHK(unsigned long arg)
{
	...
	ioarg.bits = vos_flag_chk((ID)(ioarg.id&INT_MAX), (FLGPTN)ioarg.bits);
	if (copy_to_user((void *)arg, (void *)&ioarg, sizeof(VOS_FLAG_IOARG))) {
		return -EFAULT;
	}
...
int _IOFUNC_FLAG_IOCMD_DESTROY(unsigned long arg)
{
	...
	if (E_OK != vos_flag_destroy((ID)(ioarg.id&INT_MAX))) {
		return -EFAULT;
	}
