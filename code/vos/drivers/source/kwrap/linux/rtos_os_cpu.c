...
//Use cache_line_size() is better, but it will read register dynamically
STATIC_ASSERT(VOS_ALIGN_BYTES >= L1_CACHE_BYTES);
...
int vos_cpu_dcache_syncall_by_cpu(UINT cache_op_cpu_id)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0))
	fmem_cache_flush_all(cache_op_cpu_id);
	return 0;
#else
	DBG_ERR("not supported\r\n");
	return -1;
...
int _IOFUNC_CPU_IOCMD_SYNCALL_CPU(unsigned long arg)
{
	VOS_CPU_IOARG_SYNCALL_CPU ioarg = {0};

	if (copy_from_user((void *)&ioarg, (void *)arg, sizeof(VOS_CPU_IOARG_SYNCALL_CPU))) {
		DBG_ERR("copy_from_user failed\n");
		return -EFAULT;
	}

	if (0 != vos_cpu_dcache_syncall_by_cpu((UINT)ioarg.cpu_id)) {
		return -EPERM;
	}
...
EXPORT_SYMBOL(vos_cpu_dcache_syncall_by_cpu);
