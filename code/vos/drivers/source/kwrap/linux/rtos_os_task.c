...
#define get_task_state(p_tsk) (p_tsk)->state
#else
#define get_task_state(p_tsk) (p_tsk)->__state
#endif
...
int vos_task_hdl2id(VK_TASK_HANDLE vk_task_hdl)
{
	...
	int                         i;
	TASK_CELL_T        *p_tskcell;
	struct task_struct *task_hdl = (struct task_struct *)vk_task_hdl;
...
static BOOL vos_task_chk_valid_p(TASK_CELL_T   *p_tskcell)
{
	...
	task_state_str = get_task_state_str(get_task_state(p_tskcell->task_hdl));
...
int vos_task_get_name(VK_TASK_HANDLE vk_task_hdl, char *name, unsigned int len)
{
	...
	if (NULL == get_task_state_str(get_task_state(task_hdl))) {
		return -1;
	}
...
#define vos_setscheduler sched_setscheduler_nocheck
#else
#define SETPARAM_POLICY	-1
static int vos_sched_setscheduler_nocheck(struct task_struct *p, int policy, const struct sched_param *param)
{
...
#define vos_setscheduler vos_sched_setscheduler_nocheck
#endif
...
void vos_task_set_priority(VK_TASK_HANDLE task_hdl, int priority)
{
...
#if VOS_RT_LINUX
	if (priority & VK_TASK_RT_PRIO_FLAG) {
		priority &= (~VK_TASK_RT_PRIO_FLAG);
	} else if (priority) {
		DBG_IND("skip prio %d, task 0x%lx\r\n", priority, (ULONG)task_hdl);
		return;
	}
#endif
	...
	if (0 != vos_setscheduler(task_hdl, SCHED_RR, &param)) {
...
BOOL vos_task_chk_hdl_valid(VK_TASK_HANDLE task_hdl)
{
	...
	task_state_str = get_task_state_str(get_task_state((struct task_struct *)task_hdl));
	if (NULL == task_state_str) {
		return FALSE;
	}
	return TRUE;
...
VK_TASK_HANDLE vos_task_create(void *fp, void *parm, const char name[], int priority, int stksz)
{
...
#if VOS_RT_LINUX
	if (priority & VK_TASK_RT_PRIO_FLAG) {
		//focibly set priority in RT Linux, clear the flag
		priority &= (~VK_TASK_RT_PRIO_FLAG);
	} else if (priority) {
		//set priority without RT flag, reset priority to 0
		DBG_IND("skip prio %d, task %s\r\n", priority, name);
		priority = 0;
	}
#endif
	...
	return (VK_TASK_HANDLE)p_tskcell->task_hdl;
}
...
#endif
{
	...
	for (i = 0; i < gtasknum; i++) {
		...
		task_count++;
		task_state_str = get_task_state_str(get_task_state(task_hdl));
		if (NULL != task_state_str) {
			if (virt_addr_valid(&task_hdl->comm[0])) {
...
int _IOFUNC_TASK_IOCMD_REG_N_RUN(unsigned long arg)
{
	...
	p_hdl = vos_task_tid2hdl((int)(ioarg.tid&INT_MAX));
	if (NULL == p_hdl) {
		DBG_ERR("tid2hdl failed, tid %d\r\n", (int)(ioarg.tid&INT_MAX));
		return -EFAULT;
	}
...
EXPORT_SYMBOL(vos_task_should_stop);
