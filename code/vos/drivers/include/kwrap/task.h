...
#if VOS_RT_LINUX
#define VK_TASK_RT_PRIO_FLAG        (1 << 31)
#else
#define VK_TASK_RT_PRIO_FLAG        0
#endif
...
extern void                 vos_task_destroy(VK_TASK_HANDLE vk_task_hdl);
extern void                 vos_task_enter(void);
extern int                  vos_task_return(int rtn_val);
extern void                 vos_task_update_info(int wait_res, VK_TASK_HANDLE vk_task_hdl, ID wairesid, FLGPTN waiptn, UINT wfmode);
extern void                 vos_task_dump(int (*dump)(const char *fmt, ...));
extern int                  vos_task_hdl2id(VK_TASK_HANDLE vk_task_hdl);
extern VK_TASK_HANDLE       vos_task_id2hdl(int task_id);
extern void                 vos_task_dump_by_tskhdl(int (*dump)(const char *fmt, ...), VK_TASK_HANDLE vk_task_hdl);
extern BOOL                 vos_task_chk_hdl_valid(VK_TASK_HANDLE vk_task_hdl);
extern int                  vos_task_get_tid(void);
extern void                 vos_task_set_priority(VK_TASK_HANDLE vk_task_hdl, int priority); //Range from VK_TASK_LOWEST_PRIORITY to VK_TASK_HIGHEST_PRIORITY
extern VK_TASK_HANDLE       vos_task_get_handle(void);
extern int                  vos_task_resume(VK_TASK_HANDLE vk_task_hdl);
extern int                  vos_task_get_name(VK_TASK_HANDLE vk_task_hdl, char *name, unsigned int len);
extern int                  vos_task_should_stop(void);

extern void                 vos_dump_stack_backtrace(ULONG *info, ULONG h_stack_base, ULONG h_stack_size, UINT32 level);
extern void                 vos_dump_stack_content(ULONG h_stack_pointer, ULONG h_stack_base, ULONG h_stack_size, UINT32 level);
extern void                 vos_dump_stack(void);
extern void                 vos_show_stack(VK_TASK_HANDLE vk_task_hdl, UINT32 level);
...
#else // fix freertos v11.1.0 compiler error
#include <FreeRTOS.h>
...
#define THREAD_RESUME(handle)                   vos_task_resume(handle)
#define THREAD_ENTRY()                          vos_task_enter()
#define THREAD_SHOULD_STOP                      vos_task_should_stop()
#define THREAD_DESTROY(handle)                  vos_task_destroy(handle)
