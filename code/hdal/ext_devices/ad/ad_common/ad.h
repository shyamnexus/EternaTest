...
#define AD_MODULE_VERSION "1.01.016"
...
*******************************************************************************/
typedef enum {
	AD_DATA_RATE_MODE_SDR = 0,	// single data rate
	AD_DATA_RATE_MODE_DDR,		// double data rate
	AD_DATA_RATE_MODE_MAX,
} AD_DATA_RATE_MODE;
...
typedef struct {
	...
	AD_DATA_FMT fmt;
	AD_DATA_PRTCL protocal; // (ap) this mean user prefer setting, driver should decide by itself.
	AD_DATA_DEPTH depth;
	AD_DATA_RATE_MODE dr_mode; // (ap) this mean IC can support dr mode, driver can use less than it.
	AD_MUX_NUM mux_num; // (dtsi) driver should support this setting, or return error
	UINT32 clk_rate;
	UINT32 data_rate;

	// mipi
	UINT32 data_lane_num; // (ap) this mean the real HW data lanes, driver can use less than it.
	UINT32 virtual_ch;
...
typedef struct {
	...
	AD_MUX_NUM mux_num; //[out] parallel, should be the same as AD_VOUT setting
	UINT32 in_clock; //[out]
	UINT32 out_clock; //[out] parallel
	UINT32 data_rate; //[out]
	AD_MIPI_LANE_NUM mipi_lane_num; //[out] serial, this mean the real data lanes which driver used
	AD_DATA_PRTCL protocal; //[out] parallel, this mean the real protocol which driver used
	AD_DATA_RATE_MODE dr_mode; //[out] parallel, this mean the real dr mode which driver used
