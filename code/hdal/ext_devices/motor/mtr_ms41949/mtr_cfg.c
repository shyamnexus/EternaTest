...
//	(cfile->filp)->f_pos = cfile->offset;
	...
	n = (INT64)((p1 < p2) ? p1 : p2) - (INT64)line;  // calculate length of the line
	cfile->offset += (n + 1);  // record & move current

	//if (vos_file_lseek(cfile->filp, cfile->offset, SEEK_SET) == cfile->offset) {
	if (vos_file_lseek(cfile->filp, cfile->offset, 0) == cfile->offset) {
		// discard comments
...
INT32 mtr_cfg_get_field_str(char *section, const char *key_name, INT8 *buf, INT32 size, CFG_FILE_FMT *cfile)
{
	...
	cfile->offset = vos_file_lseek(cfile->filp, 0, 0);
