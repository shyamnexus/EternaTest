...
INT32 sen_spi_transfer(UINT32 id, struct spi_msg *msgs, INT32 num)
{
	...
	if (num == 2) { // read register
		if (msgs->len == 2) {
			spi_data = ((((UINT32)msgs->buf[1] << 8) | (UINT32)msgs->buf[0]) & 0xFFFF);
		} else if (msgs->len == 3) {
			spi_data = (((UINT32)msgs->buf[2] << 16) | ((UINT32)msgs->buf[1] << 8) | ((UINT32)msgs->buf[0]));
		} else if (msgs->len == 4) {
			spi_data = (((UINT32)msgs->buf[3] << 24) | ((UINT32)msgs->buf[2] << 16) | ((UINT32)msgs->buf[1] << 8) | ((UINT32)msgs->buf[0]));
		}
	} else {
	...
	if (num == 2) {    // read register
		if (unlikely(spi_read(sen_spi_device[id], &rd_addr, 1) != 0)) {
			printk("%s fail: spi_transfer not OK!\n", __FUNCTION__);
			return -1;
		}
		(msgs+1)->buf[0] = (UINT8)(rd_addr & 0xFF);
		//spi_write_then_read(sen_spi_device[id], &spi_data, msgs->len, &rd_addr, 1);
		//(msgs+1)->buf[0] = (UINT32)(rd_addr & 0xFF);	
