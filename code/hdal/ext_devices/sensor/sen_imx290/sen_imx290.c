...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx290, 1, 01, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_IMX290_MODULE_NAME     "sen_imx290"
#define SEN_MAX_MODE               10
#define MAX_VD_PERIOD              0x3FFFF
#define HCG_ENABLE                 99
#define BRL_1080P                  1109
#define SENSOR_ID                  0
...
static ER sen_get_min_shr_imx290(CTL_SEN_MODE mode, UINT32 *min_shr, UINT32 *min_exp);
//=============================================================================
// global variable
//=============================================================================
...
static SEN_POWER sen_power[CTL_SEN_ID_MAX] = {
	...
	{CTL_SEN_CLK_SEL_SIEMCLK_IGNORE, CTL_SEN_IGNORE, 0x43, 1, 1},
	#else 
...
static CTL_SENDRV_GET_MODE_BASIC_PARAM mode_basic_param[SEN_MAX_MODE] = {
	...
	},
	{
		...
		CTL_SEN_FMT_POGRESSIVE,
		{1945, 1104},
		{{12, 16, 1920, 1080}, {12, 20, 1920, 1080}, {12, 20, 1920, 1080}, {0, 0, 0, 0}},
		{1920, 1080},
		{0, 1100, 0, 1125},
		CTL_SEN_RATIO(16, 9),
		{1000, 4096000},
...
static CTL_SEN_CMD imx290_mode_10[] = {
	{0x3003, 1, { 0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},
	{0x3000, 1, {0x01, 0x00}}, //standby Master mode stop
	{SEN_CMD_DELAY, 1, { 20, 0x0}},
	{0x3002, 1, {0x01, 0x0}},  // Master mode stop
	{SEN_CMD_DELAY, 1, { 20, 0x0}},
	{0x3005, 1, { 0x00, 0x0}},
	...
	{0x3019, 1, { 0x04, 0x0}},
	{0x301c, 1, { 0x4c, 0x0}}, //HMAX
	{0x301d, 1, { 0x04, 0x0}},
	{0x3045, 1, { 0x05, 0x0}},
	{0x3046, 1, { 0x00, 0x0}},
	{0x304B, 1, { 0x0a, 0x0}},
	{0x305C, 1, { 0x18, 0x0}}, //INCK
	{0x305D, 1, { 0x03, 0x0}},
	{0x305E, 1, { 0x20, 0x0}},
	{0x305F, 1, { 0x01, 0x0}},
	{0x3070, 1, { 0x02, 0x0}},
	...
	{0x313D, 1, { 0x83, 0x0}},
	{0x3150, 1, { 0x03, 0x0}},
	{0x315E, 1, { 0x1A, 0x0}}, //INCKSEL5
	{0x3164, 1, { 0x1A, 0x0}}, //INCKSEL6
	{0x317C, 1, { 0x12, 0x0}}, //ADBIT2
	{0x317E, 1, { 0x00, 0x0}},
	{0x31EC, 1, { 0x37, 0x0}},
	{0x31A0, 1, { 0xfc, 0x0}},
	{0x31A1, 1, { 0x00, 0x0}},
	...
	{0x3021, 1, { 0x00, 0x0}},
	{0x3024, 1, { 0x5e, 0x0}}, //SHS2
	{0x3025, 1, { 0x03, 0x0}},
	{0x3028, 1, { 0x43, 0x0}}, //SHS3
	{0x3029, 1, { 0x04, 0x0}},
	{0x3030, 1, { 0x58, 0x0}}, //RHS1
	{0x3031, 1, { 0x03, 0x0}},
	{0x3034, 1, { 0x34, 0x0}}, //RHS2
	{0x3035, 1, { 0x04, 0x0}},
	{0x3014, 1, { 0x60, 0x0}}, //LEF gain set(0.0dB to 72.0dB/0.3dB)
	{0x30F0, 1, { 0x64, 0x0}}, //
	{0x30F2, 1, { 0x60, 0x0}}, //SEF1 gain set
	{0x30F4, 1, { 0x64, 0x0}}, //2
	{0x30F6, 1, { 0x60, 0x0}}, //SEF2 gain set
	{SEN_CMD_SETVD, 1, {0x00, 0x0}},	
	{SEN_CMD_PRESET, 1, {0x00, 0x0} },		
	{SEN_CMD_DIRECTION, 1, {0x00, 0x0}},	
	{0x3000, 1, {0x00, 0x0}},// operating
...
static void sen_pwr_ctrl_imx290(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb)
{
	...
	if (flag == CTL_SEN_PWR_CTRL_TURN_ON) {
		...
		if (sen_power[id].rst_pin != CTL_SEN_IGNORE) {
			#if defined(CONFIG_NVT_FPGA_EMULATION) || defined(_NVT_FPGA_)
			sen_power[id].rst_pin = 0x43; //P_GPIO 0x43
			#endif
...
static UINT32 sen_get_cmd_tab_imx290(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	...
	case CTL_SEN_MODE_10:
		*cmd_tab = imx290_mode_10;
		return sizeof(imx290_mode_10) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_imx290(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			DBG_DUMP(" id = %d, preset ctrl = %d, expt = %d %d, gain = %d %d \r\n", id, preset_ctrl[id].mode, sensor_ctrl.exp_time[0], sensor_ctrl.exp_time[1], sensor_ctrl.gain_ratio[0], sensor_ctrl.gain_ratio[1]);
...
static ER sen_get_min_shr_imx290(CTL_SEN_MODE mode, UINT32 *min_shr, UINT32 *min_exp)
{
    ...
    case CTL_SEN_MODE_9:    	    	    	    	   	
        ...
        *min_exp = 1;
        break;
    case CTL_SEN_MODE_2: // 2 frame hdr
        *min_shr = 0; //don't care
        *min_exp = 1; //short exp frame min exp = rhs1 - shs1(max = rhs1-2) - 1 = 5 - 3 -1 = 1
        break;        
    case CTL_SEN_MODE_10: // 3 frame hdr
        *min_shr = 0; //don't care
        *min_exp = 1; //short exp frame min exp = rhs2 - shs2(rhs2 -2) - 1 = 14 - 12 -1 = 1
        break;
...
static void sen_set_expt_imx290(CTL_SEN_ID id, void *param)
{
	...
	UINT32 shs1 = 0, shs2 = 0, shs3 = 0, rhs1 = 0, rhs2 = 0, t_row = 0;
	UINT32 fsc = 0, real_l_exp = 0, real_s_exp = 0;
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		sen_get_min_shr_imx290(cur_sen_mode[id], &min_shr, &min_exp_line);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (mode_basic_param[cur_sen_mode[id]].frame_num == 2) {			
			...
			if (5 > rhs1) {
				rhs1 = 5;
			} else if (rhs1 > (fsc - (2 * BRL_1080P) - 21)) {
				rhs1 = fsc - (2 * BRL_1080P) - 21;
			}
			...
			shs1 = rhs1 - real_s_exp - 1;
			
			//L
			shs2 = fsc - real_l_exp - 1;
			if ( rhs1 + 2 > shs2) {
				shs2 = rhs1 + 2;
			} else if ((fsc - 2) < shs2) {
				shs2 = fsc - 2;
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (mode_basic_param[cur_sen_mode[id]].frame_num == 2) {	
			...
			rt |= sen_write_reg_imx290(id, &cmd);		    	
			DBG_IND("shs1 %d (0x3020 0x%x 0x3021 0x%x 0x3022 0x%x)\r\n", shs1, shs1 & 0xFF, (shs1 >> 8) & 0xFF, (shs1 >> 16) & 0x03);
		...
		} else if (mode_basic_param[cur_sen_mode[id]].frame_num == 3) {
			//Set exposure line to sensor (rhs1)
			cmd = sen_set_cmd_info_imx290(0x3030, 1, rhs1 & 0xFF, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			cmd = sen_set_cmd_info_imx290(0x3031, 1, (rhs1 >> 8) & 0xFF, 0);
			...
			//Set exposure line to sensor (rhs2)
			cmd = sen_set_cmd_info_imx290(0x3034, 1, rhs2 & 0xFF, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			cmd = sen_set_cmd_info_imx290(0x3035, 1, (rhs2 >> 8) & 0xFF, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			cmd = sen_set_cmd_info_imx290(0x3036, 1, (rhs2 >> 16) & 0x03, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			DBG_IND("rhs2 %d (0x3030 0x%x 0x3031 0x%x 0x3032 0x%x)\r\n", rhs2, rhs2 & 0xFF, (rhs2 >> 8) & 0xFF, (rhs2 >> 16) & 0x03);
			...
			//Set exposure line to sensor (shs3)
			cmd = sen_set_cmd_info_imx290(0x3028, 1, shs3 & 0xFF, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			cmd = sen_set_cmd_info_imx290(0x3029, 1, (shs3 >> 8) & 0xFF, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			cmd = sen_set_cmd_info_imx290(0x302A, 1, (shs3 >> 16) &0x03, 0);
			rt |= sen_write_reg_imx290(id, &cmd);
			DBG_IND("shs3 %d (0x3024 0x%x 0x3025 0x%x 0x3026 0x%x)\r\n", shs3, shs3 & 0xff, (shs3 & 0xff00) >> 8, (shs3 & 0x30000) >> 16);
		}
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR){
		// Set exposure line to sensor (shs1)
		cmd = sen_set_cmd_info_imx290(0x3020, 1, shs1 & 0xFF, 0);
	...
	//Set exposure line to sensor (vmax)
...
static void sen_get_min_expt_imx290(CTL_SEN_ID id, void *param)
{
	...
	t_row = sen_calc_rowtime_imx290(id, cur_sen_mode[id]);
	sen_get_min_shr_imx290(cur_sen_mode[id], &min_shr, &min_exp_line);
	*min_exp_time = t_row * min_exp_line / 10 + 1; // if linear and shdr mode is different,the value need to separate.
}
...
static void sen_get_modesel_imx290(CTL_SENDRV_GET_MODESEL_PARAM *data)
{
	...
	if (data->if_type == CTL_SEN_IF_TYPE_MIPI) {
		if (CTL_SEN_DATALANE_2 == (data->data_lane)) {
			...
			if (data->mode_type_sel == CTL_SEN_MODE_STAGGER_HDR) {
				if (data->frame_num == 2) {
					if ((data->size.w <= 1920) && (data->size.h <= 1080)) {
						if (data->frame_rate <= 3000) {
							data->mode = CTL_SEN_MODE_2;
			...
			} else { //if (data->mode_type_sel == CTL_SEN_MODE_LINEAR) {
				if (data->frame_num == 1) {
					if ((data->size.w <= 1920) && (data->size.h <= 1080)) {
						if ((data->size.w <= 1280) && (data->size.h <= 720)) {
							if (data->frame_rate <= 6000) {
...
int sen_init_imx290(SENSOR_DTSI_INFO *info)
{
	...
	#if defined(CONFIG_NVT_FPGA_EMULATION) || defined(_NVT_FPGA_)
	sen_preset[0].expt_time = 30000;
	sen_preset[0].gain_ratio = 1000;
	DBG_DUMP("FPGA, path 0 of expt_time = %d, gain_ratio = %d \r\n", sen_preset[0].expt_time, sen_preset[0].gain_ratio);
	#endif
