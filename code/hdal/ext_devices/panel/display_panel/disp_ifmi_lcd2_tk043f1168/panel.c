...
{
#if 0
	*cmd_number = sizeof(t_cmd_standby_mi2) / sizeof(T_PANEL_CMD);
	return (T_PANEL_CMD *)t_cmd_standby_mi2;
#else
	return 0;
#endif
}
...
void dispdev_set_lcd_colour2(void)
{
	...
	dispdev_write_lcd_mi2(MICMD_CMD, 0x2D);
	for (i=0; i<=63; i++)
	{
		dispdev_write_lcd_mi2(MICMD_DATA, i*8);   
	}

	for (i=0; i<=63; i++)
	{
		dispdev_write_lcd_mi2(MICMD_DATA, i*4);   
	}
...
int panel_init2(void)
{
    ...
    DBG_DUMP("Hello, panel2: TK043F1168\n");
    return 0;
}
...
static int __init panel_init2(void)
{
	...
	p_disp_obj->dev_callback = &dispdev_get_lcd1_dev_obj;
    pr_info("Hello, panel2: TK043F1168\n");
    return 0;
}
