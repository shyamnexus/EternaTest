#ifndef __CTL_VIE_MODULE_MAIN_H__
#define __CTL_VIE_MODULE_MAIN_H__
#include <linux/cdev.h>
#include "ctl_vie_drv.h"

#define MODULE_MINOR_ID      0
#define MODULE_MINOR_COUNT   1
#define MODULE_NAME          "kflow_vie"

typedef struct ctl_vie_drv_info {
	MODULE_INFO module_info;

	struct class *pmodule_class;
	struct cdev cdev;
	dev_t dev_id;

	// proc entries
	struct proc_dir_entry *pproc_module_root;
	struct proc_dir_entry *pproc_help_entry;
	struct proc_dir_entry *pproc_cmd_entry;
	struct proc_dir_entry *pproc_info_entry;
	struct proc_dir_entry *pproc_buf_entry;
	struct proc_dir_entry *pproc_dbglevel_entry;

	struct seq_file *info_seq_file;
} CTL_VIE_DRV_INFO, *PCTL_VIE_DRV_INFO;


#endif
