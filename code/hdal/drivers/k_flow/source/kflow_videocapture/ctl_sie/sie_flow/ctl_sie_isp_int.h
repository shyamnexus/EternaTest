...
typedef enum {
	...
	CTL_SIE_IQ_FUNC_ROI_ACC     = 0x00000100, ///<
} CTL_SIE_ISP_IQ_FUNC;
...
typedef struct {
	ULONG src_addr;
	UINT16 *lum_1;	// pre gamma result
	UINT16 *lum_2;	// post-gamma result
	UINT16 *histogram;	// [NT96680] only SIE 1~4 support hist out
...
BOOL ctl_sie_isp_int_pull_img_out(UINT32 id, ULONG sie_header_addr);
void ctl_sie_isp_set_skip(UINT32 id, BOOL skip_en, UINT32 iq_func_off);
void ctl_sie_isp_rst_to_dft(UINT32 id);
INT32 ctl_sie_isp_get_roi_acc_rslt(CTL_SIE_ID id, void *data);
