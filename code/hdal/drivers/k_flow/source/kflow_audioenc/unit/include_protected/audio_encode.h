...
*/
typedef struct {
	ULONG  encAddr;    	///< in:codec needed memory
	UINT32 encSize;    	///< in:size for codec memory
	ULONG  rawAddr;    	///< in:encoding audio raw address
...
typedef struct {
	...
	ER(*GetInfo)(MP_AUDENC_GETINFO_TYPE type, ULONG *pparam1, ULONG *pparam2, ULONG *pparam3);   ///< get information from codec library
	ER(*SetInfo)(MP_AUDENC_SETINFO_TYPE type, ULONG param1, ULONG param2, ULONG param3);  ///< set parameters to codec library
	ER(*EncodeOne)(UINT32 type, ULONG outputAddr, UINT32 *pSize, MP_AUDENC_PARAM *ptr);  ///< encoding one audio frame
