...
// Global variable
//============================================================================
static ISF_AUDENC_DRV_INFO *p_drv_info_data;
extern UINT32 g_audenc_init[ISF_FLOW_MAX];
...
static struct proc_ops proc_info_fops = {
	.proc_open    = isf_audenc_proc_info_open,
...
static struct proc_ops proc_cmd_fops = {
	.proc_open    = isf_audenc_proc_cmd_open,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
	.proc_write   = isf_audenc_proc_cmd_write
};
#endif
...
//=============================================================================
static int isf_audenc_proc_dbglevel_show(struct seq_file *sfile, void *v)
{
	unsigned int dbg_lvl;

	dbg_lvl = get_hdal_flow_dbglevel(HDAL_FLOW_DBG_AENC);
	seq_printf(sfile, "0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\r\n");
	seq_printf(sfile, "aenc dbglevel:%d \r\n", dbg_lvl);
	return 0;
}

static int isf_audenc_proc_dbglevel_open(struct inode *inode, struct file *file)
{
	return single_open(file, isf_audenc_proc_dbglevel_show, NULL);
}

static ssize_t isf_audenc_proc_dbglevel_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *ppos)
{
	...
	printk("aenc dbglevel=%d\n", flow_dbg_lvl);
	set_hdal_flow_dbglevel(HDAL_FLOW_DBG_AENC, flow_dbg_lvl);
	return count;
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0)
static struct file_operations proc_dbglevel_fops = {
	.owner   = THIS_MODULE,
	.open    = isf_audenc_proc_dbglevel_open,
	.release = single_release,
	.read    = seq_read,
	.write   = isf_audenc_proc_dbglevel_write,
	.llseek  = seq_lseek,
};
#else
static struct proc_ops proc_dbglevel_fops = {
	.proc_open    = isf_audenc_proc_dbglevel_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_write   = isf_audenc_proc_dbglevel_write,
	.proc_lseek  = seq_lseek,
};
#endif
...
static struct proc_ops proc_help_fops = {
	.proc_open    = isf_audenc_proc_help_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
};
