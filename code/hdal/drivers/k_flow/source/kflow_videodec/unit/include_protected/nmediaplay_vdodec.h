...
typedef enum {
	...
	NMI_VDODEC_PARAM_H26X_DECODER_OBJ,    ///< media plugin h26x decoder obj
	NMI_VDODEC_PARAM_JPEG_Y_ONLY,
} NMI_VDODEC_PARAM;
...
*/
/*typedef struct {
	uintptr_t					uiBSAddr;						///< Bit-Stream address
...
typedef struct {
	uintptr_t                   uiBSAddr;                       ///< Bit-Stream address
	UINT32                      uiBSSize;                       ///< Bit-Stream size
	UINT32                      uiSliceIdx;                     ///< slice index
	uintptr_t                   uiRawAddr;                      ///< output raw addr (should use common buffer)
	...
	UINT64                      TimeStamp;                      ///< Timestamp
	ULONG                       uiCommBufBlkID;                 ///< user common buffer blkID
	BOOL                        bSliceDec;                      ///< slice decode enable
} NMI_VDODEC_BS_INFO, *PNMI_VDODEC_BS_INFO;

typedef struct {
	uintptr_t                   uiBSNXTAddr;                    ///< Bit-Stream address
...
typedef struct {
	...
	UINT32                      BsBufID;                        ///< Bs Buffer ID
	UINT32                      RawBufID;                       ///< Raw Buffer ID
	uintptr_t                   uiRawAddr;                      ///< Raw address
	UINT32                      uiRawSize;                      ///< Raw address
	uintptr_t                   uiYAddr;                        ///< Y address
	uintptr_t                   uiUVAddr;                       ///< UV address
	UINT32                      uiWidth;                        ///< [out] decoded image width
	UINT32                      uiHeight;                       ///< [out] decoded image height
...
*/
typedef struct {
	uintptr_t                   Y_Addr;                         ///< Image Y address
	uintptr_t                   UV_Addr;                        ///< Image UV address
...
typedef void (NMI_VDODEC_CB)(UINT32 event, ULONG param);
