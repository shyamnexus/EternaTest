...
// Global variable
//============================================================================
static ISF_VDODEC_DRV_INFO *p_drv_info_data;
extern UINT32 g_vdodec_init[ISF_FLOW_MAX];
...
static struct proc_ops proc_info_fops = {
	.proc_open    = isf_vdodec_proc_info_open,
...
static struct proc_ops proc_cmd_fops = {
	.proc_open    = isf_vdodec_proc_cmd_open,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
	.proc_write   = isf_vdodec_proc_cmd_write
};
...
static struct proc_ops proc_help_fops = {
	.proc_open    = isf_vdodec_proc_help_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
};
...
//=============================================================================
static int isf_vdodec_dbglevel_proc_show(struct seq_file *sfile, void *v)
{
	unsigned int dbg_lvl;

	dbg_lvl = get_hdal_flow_dbglevel(HDAL_FLOW_DBG_VDEC);
	seq_printf(sfile, "0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\r\n");
	seq_printf(sfile, "vdec dbglevel:%d \r\n", dbg_lvl);
	return 0;
}

static int isf_vdodec_dbglevel_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, isf_vdodec_dbglevel_proc_show, NULL);
}

static ssize_t isf_vdodec_dbglevel_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	printk("vdec dbglevel=%d\n", flow_dbg_lvl);
	set_hdal_flow_dbglevel(HDAL_FLOW_DBG_VDEC, flow_dbg_lvl);
	return count;
}

static struct proc_ops isf_vdodec_dbglevel_proc_fops = {
	.proc_open   = isf_vdodec_dbglevel_proc_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_write   = isf_vdodec_dbglevel_proc_write,
	.proc_lseek = seq_lseek,
};
...
int isf_vdodec_proc_init(ISF_VDODEC_DRV_INFO *p_drv_info)
{
	...
	// dbglevel
	pentry = proc_create("dbglevel", S_IRUGO | S_IXUGO, p_module_root, &isf_vdodec_dbglevel_proc_fops);
	if (pentry == NULL) {
