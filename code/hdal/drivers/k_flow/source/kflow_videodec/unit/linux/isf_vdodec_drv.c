...
#include "kflow_videodec/isf_vdodec_int.h"
...
int isf_vdodec_drv_suspend(ISF_VDODEC_INFO *pmodule_info)
{
	...
	/* Check status */
	{
		if (isf_vdodec_check_start_stream_for_suspend() != 0) {
			DBG_ERR("vdec has working streaming(status:start) while system is suspending\r\n");
			return -1;
		}
	}
