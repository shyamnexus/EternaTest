...
// Global variable
//============================================================================
static PISF_AUDOUT_DRV_INFO p_drv_info_data;
...
#else
static struct proc_ops isf_aout_cmd_proc_fops = {
	.proc_open    = isf_aout_cmd_proc_open,
	.proc_read    = seq_read,
	.proc_write   = isf_aout_cmd_proc_write,
...
//=============================================================================
static int isf_aout_dbglevel_proc_show(struct seq_file *sfile, void *v)
{
	unsigned int dbg_lvl;

	dbg_lvl = get_hdal_flow_dbglevel(HDAL_FLOW_DBG_AOUT);
	seq_printf(sfile, "0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\r\n");
	seq_printf(sfile, "aout dbglevel:%d \r\n", dbg_lvl);
	return 0;
}

static int isf_aout_dbglevel_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, isf_aout_dbglevel_proc_show, NULL);
}

static ssize_t isf_aout_dbglevel_proc_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *ppos)
{
	...
	printk("aout dbglevel=%d\n", flow_dbg_lvl);
	set_hdal_flow_dbglevel(HDAL_FLOW_DBG_AOUT, flow_dbg_lvl);
	return count;
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0)
static struct file_operations isf_aout_dbglevel_proc_fops = {
	.owner   = THIS_MODULE,
	.open    = isf_aout_dbglevel_proc_open,
	.release = single_release,
	.read    = seq_read,
	.write   = isf_aout_dbglevel_proc_write,
	.llseek  = seq_lseek,
};
#else
static struct proc_ops isf_aout_dbglevel_proc_fops = {
	.proc_open    = isf_aout_dbglevel_proc_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_write   = isf_aout_dbglevel_proc_write,
	.proc_lseek   = seq_lseek,
};
#endif
...
#else
static struct proc_ops isf_aout_help_proc_fops = {
	.proc_open    = isf_aout_help_proc_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
};
...
#else
static struct proc_ops isf_aout_info_proc_fops = {
	.proc_open    = isf_aout_info_proc_open,
	.proc_release = single_release,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
};
...
int isf_audout_proc_init(PISF_AUDOUT_DRV_INFO p_drv_info)
{
	...
	pentry = proc_create("dbglevel", S_IRUGO | S_IXUGO, p_module_root, &isf_aout_dbglevel_proc_fops);
	if (pentry == NULL) {
