...
int isf_audout_drv_suspend(ISF_AUDOUT_INFO *p_module_info)
{
	...
	/* Check status */
	{
		UINT32 status = WAVSTUD_STS_UNKNONWN;
		status |= wavstudio_get_config(WAVSTUD_CFG_GET_STATUS, WAVSTUD_ACT_PLAY, 0);
		status |= wavstudio_get_config(WAVSTUD_CFG_GET_STATUS, WAVSTUD_ACT_PLAY2, 0);
		if (status & WAVSTUD_STS_GOING) {
			DBG_ERR("aout has working streaming(status:start) while system is suspending\r\n");
			return -1;
		}
	}
