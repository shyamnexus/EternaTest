...
#define CTL_IPP_ISP_ID_NO_NEED	(0x0000FFFF)	// hdal use HD_ISP_DONT_CARE(0x0000FFFF) to notify that this ipp handle dont need isp info

typedef INT32(*CTL_IPP_ISP_SET_FP)(ISP_ID id, CTL_IPP_ISP_ITEM item, void *data, CTL_IPP_HANDLE *p_hdl);
typedef INT32(*CTL_IPP_ISP_GET_FP)(ISP_ID id, CTL_IPP_ISP_ITEM item, void *data, CTL_IPP_HANDLE *p_hdl);
...
typedef struct {
	UINT32 isp_id;
	UINT32 eng_bind_id[CTL_IPP_ENG_MAX];
	CTL_IPP_INFO_LIST_ITEM *p_ctrl_info[CTL_IPP_BIND_ISP_MAX];
	...
	CTL_IPP_ISP_BNR_STA cur_bnr_sta;

	// information storage used to output user
	CTL_IPP_ISP_ETH rdy_eth;
	CTL_IPP_ISP_3DNR_STA rdy_3dnr_sta;
	CTL_IPP_ISP_BNR_STA rdy_bnr_sta;
	CTL_IPP_ISP_PRE_VA_RST rdy_pre_va_rst;
	CTL_IPP_ISP_PRE_VA_INDEP_RST rdy_pre_va_indep_rst;
	...
	CTL_IPP_ISP_STRP_INFO stripe_info[CTL_IPP_BIND_ISP_MAX];
	CTL_IPP_ISP_FPN_INFO fpn_info;
	CTL_IPP_ISP_LOW_PWR_INFO low_pwr_info;
	CTL_IPP_ISP_INT_AIISP_PARAM aiisp_param;
...
INT32 ctl_ipp_isp_release_int_info(CTL_IPP_HANDLE *p_hdl, UINT32 id);
...
INT32 ctl_ipp_isp_event_cb_proc(ISP_ID id, ISP_EVENT evt, UINT32 raw_frame_cnt, CTL_IPP_ISP_CB_PARAM *cb_info);
...
INT32 ctl_ipp_isp_int_set_eng_id(CTL_IPP_HANDLE *p_hdl, UINT32 *p_eng_id);
...
typedef struct {
	...
	CTL_IPP_ISP_CB_PARAM cb_info;
} CTL_IPP_ISP_MSG_EVENT;

ER ctl_ipp_isp_msg_snd(UINT32 cmd, ULONG p1, ULONG p2, ULONG p3, CTL_IPP_ISP_CB_PARAM *cb_info);
ER ctl_ipp_isp_msg_rcv(UINT32 *cmd, ULONG *p1, ULONG *p2, ULONG *p3, CTL_IPP_ISP_CB_PARAM *cb_info);
...
void ctl_ipp_isp_process(UINT32 id, UINT32 event, UINT32 raw_frame_cnt, CTL_IPP_ISP_CB_PARAM *cb_info);
