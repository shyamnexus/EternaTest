...
#define CTL_IPP_HDL_SUBOUT_BUF_NUM 5	// subout buf queue depth
...
#define CTL_IPP_MSG_PROCESS_NNISP			0x00000005  //[0]: handle, [1]: ctrl info
...
ER ctl_ipp_msg_snd_imm(UINT32 cmd, ULONG p1, ULONG p2, ULONG p3, IPP_EVENT_FP drop_fp);
...
ER ctl_ipp_erase_nnisp_path_in_queue(CTL_IPP_HANDLE *p_hdl, UINT32 pid);
...
/**********************************/
#define CTL_IPP_HANDLE_NAME_MAX             (10)
...
typedef struct {
	...
	UINT32 verify_id;	// use to check if buf_info is invalid
...
typedef struct {
	...
	CTL_IPP_BUF_PRI_LOF_INFO shdr_subout[CTL_IPP_BUF_COMM_BLK_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO pre_va[CTL_IPP_BUF_VA_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO pre_va_dbg[CTL_IPP_BUF_DBG_NUM];
	...
	CTL_IPP_BUF_PRI_LOF_INFO bnr_mo[CTL_IPP_BUF_BNR_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO bnr_mo_dbg[CTL_IPP_BUF_DBG_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO bnr_sigma[CTL_IPP_BUF_COMM_BLK_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO bnr_gamma[CTL_IPP_BUF_COMM_BLK_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO bnr_sta[CTL_IPP_BUF_BNR_STA_NUM];
	CTL_IPP_BUF_PRI_LOF_INFO bnr_sta_dbg[CTL_IPP_BUF_DBG_NUM];
...
typedef enum {
	CTL_IPP_BUF_INFO_CNT_EVT_NEW = 0,
	CTL_IPP_BUF_INFO_CNT_EVT_NEW_FAILED,
	//CTL_IPP_BUF_INFO_CNT_EVT_PUSH,
	//CTL_IPP_BUF_INFO_CNT_EVT_PUSH_FAILED,
	CTL_IPP_BUF_INFO_CNT_EVT_LOCK,
	//CTL_IPP_BUF_INFO_CNT_EVT_LOCK_FAILED,
	CTL_IPP_BUF_INFO_CNT_EVT_UNLOCK,
	//CTL_IPP_BUF_INFO_CNT_EVT_UNLOCK_FAILED,
	CTL_IPP_BUF_INFO_CNT_EVT_UNLOCK_BY_OTHER,	// for subout buf that is unlock by posterior pipe
	CTL_IPP_BUF_INFO_CNT_EVT_MAX,
} CTL_IPP_BUF_INFO_CNT_EVT;
...
typedef struct _CTL_IPP_HANDLE {
	...
	*/
	...
	atomic_t ai_proc_cnt;															/* ai task process ok count */
	atomic_t ai_drop_cnt;															/* ai task drop count */
	atomic_t ai_frm_mode_buf_cnt[CTL_IPP_BUF_INFO_CNT_EVT_MAX];						/* ai frame mode buffer count */
	atomic_t aided_drop_cnt;														/* subout buf drop old cnt */
	atomic_t aided_invalid_cnt;														/* subout buf is not used (already be freed) */
	atomic_t aided_buf_cnt[CTL_IPP_BUF_INFO_CNT_EVT_MAX];							/* subout buf buffer count */
	atomic_t parse_err_cnt;															/* subout buf is invalid (may be used by others) */
	...
	UINT32 last_bnr_disable_frm;	/* latest frame number that disable bnr */
	...
	/* aided map subout metadata checking */
	UINT32 cur_subout_buf_idx;
	CTL_IPP_BUF_INFO *subout_buf[CTL_IPP_HDL_SUBOUT_BUF_NUM];
	UINT32 subout_verify_id[CTL_IPP_HDL_SUBOUT_BUF_NUM];

	CTL_IPP_BIND_ISP_INFO bind_isp;
} CTL_IPP_HANDLE;
...
UINT32 ctl_ipp_get_bind_hdl_by_ispid(UINT32 id, CTL_IPP_HANDLE **hdl_list);
...
void ctl_ipp_info_release_no_lock(CTL_IPP_INFO_LIST_ITEM *p_info);
...
INT32 ctl_ipp_process_nnisp(CTL_IPP_HANDLE *p_hdl, CTL_IPP_INFO_LIST_ITEM *p_ctrl_info);
