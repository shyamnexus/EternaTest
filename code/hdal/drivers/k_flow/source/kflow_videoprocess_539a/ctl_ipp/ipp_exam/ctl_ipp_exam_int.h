...
typedef struct {
	...
	UINT32 save_rst_frm_cnt;						// nodify the frame count that will be save as result
	UINT32 frame_cnt;								// vdoframe count
	...
	UINT32 prv_dir_in_done_num;						// previous dir_in_done_num
	UINT32 prv_dir_in_drop_num;						// previous dir_in_drop_num
	...
	UINT32 bind_hdl_id_dst[CTL_IPP_OUT_PATH_ID_MAX];// bind path output to another handle
	UINT32 bind_hdl_id_src;							// bind input frame from another handle
} CTL_IPP_EXAM_HDL;
...
extern INT32 ctl_ipp_exam_set_frame_cnt(UINT32 id, UINT32 fcnt);

extern UINT32 ctl_ipp_exam_get_frame_cnt(UINT32 id);
...
extern INT32 ctl_ipp_exam_trig_imm(UINT32 id);
