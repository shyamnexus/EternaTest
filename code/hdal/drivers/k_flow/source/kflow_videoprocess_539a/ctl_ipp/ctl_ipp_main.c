...
int __init nvt_ctl_ipp_init(void)
{
	...
	if (register_printout_notifier(ctl_ipp_dbg_panic_printout_handler) < 0) {
		CTL_IPP_DBG_ERR("register_printout_notifier fail\n");
	}
...
void __exit nvt_ctl_ipp_exit(void)
{
	...
	unregister_printout_notifier(ctl_ipp_dbg_panic_printout_handler);
}
