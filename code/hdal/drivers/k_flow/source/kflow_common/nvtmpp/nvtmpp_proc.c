...
static struct proc_ops nvtmpp_info_proc_fops = {
	.proc_open   = nvtmpp_info_proc_open,
...
static struct proc_ops nvtmpp_task_proc_fops = {
	.proc_open   = nvtmpp_task_proc_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_lseek = seq_lseek,
};
...
static struct proc_ops nvtmpp_sem_proc_fops = {
	.proc_open   = nvtmpp_sem_proc_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_lseek = seq_lseek,
};
...
// plug file-system
static const struct proc_ops nvtmpp_cmd_proc_fops = {
	.proc_open  = nvtmpp_cmd_proc_open,
	.proc_read  = seq_read,
	.proc_write   = nvtmpp_cmd_proc_write,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
};
...
static struct proc_ops nvtmpp_help_proc_fops = {
	.proc_open   = nvtmpp_help_proc_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_lseek = seq_lseek,
};
...
//=============================================================================
static int nvtmpp_dbglevel_proc_show(struct seq_file *sfile, void *v)
{
	unsigned int dbg_lvl;

	dbg_lvl = get_hdal_flow_dbglevel(HDAL_FLOW_DBG_COMM);
	seq_printf(sfile, "0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\r\n");
	seq_printf(sfile, "comm dbglevel:%d \r\n", dbg_lvl);
	return 0;
}

static int nvtmpp_dbglevel_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvtmpp_dbglevel_proc_show, NULL);
}

static ssize_t nvtmpp_dbglevel_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	printk("comm dbglevel=%d\n", flow_dbg_lvl);
	set_hdal_flow_dbglevel(HDAL_FLOW_DBG_COMM, flow_dbg_lvl);
	return count;
}

static struct proc_ops nvtmpp_dbglevel_proc_fops = {
	.proc_open   = nvtmpp_dbglevel_proc_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_write   = nvtmpp_dbglevel_proc_write,
	.proc_lseek = seq_lseek,
};
...
int nvtmpp_proc_init(PNVTMPP_DRV_INFO pdrv_info)
{
	...
	// dbglevel
	pentry = proc_create("dbglevel", S_IRUGO | S_IXUGO, pmodule_root, &nvtmpp_dbglevel_proc_fops);
	if (pentry == NULL) {
