...
static int isf_flow_drv_parse_maxpath_from_dts_p(ISF_FLOW_IOCTL_GET_MAX_PATH *p_max_path)
{
	...
	if (of_property_read_u32_array(dt_node, "vdocap_active_list", p_max_path->vdocap_active_list, 16)) {
	...
	if (of_property_read_u32(dt_node, "vdoprc_vpe_max_handle", &p_max_path->vdoprc_vpe_max_handle)) {
		//no config means use default
		p_max_path->vdoprc_vpe_max_handle = p_max_path->vdoprc_maxdevice*2;
	}
	if (of_property_read_u32(dt_node, "vdoprc_vpe_max_2dlut_sz", &p_max_path->vdoprc_vpe_max_2dlut_sz)) {
		//no config means use default
		p_max_path->vdoprc_vpe_max_2dlut_sz = 5;
	}
	if (of_property_read_u32(dt_node, "vdoprc_aiisp_maxdevice", &p_max_path->vdoprc_aiisp_maxdevice)) {
		p_max_path->vdoprc_aiisp_maxdevice = 0;
	}
	#if 0
	DBG_DUMP("vdocap_active_list = %d %d %d %d %d %d %d %d %d %d %d %d %d %d\r\n", p_max_path->vdocap_active_list[0], p_max_path->vdocap_active_list[1],
		      ...
		      p_max_path->vdocap_active_list[8], p_max_path->vdocap_active_list[9], p_max_path->vdocap_active_list[10],
		      p_max_path->vdocap_active_list[11], p_max_path->vdocap_active_list[12], p_max_path->vdocap_active_list[13]);
	...
	DBG_DUMP("vdoprc_vpe_max_handle = %d\r\n", p_max_path->vdoprc_vpe_max_handle);
	DBG_DUMP("vdoprc_vpe_max_2dlut_sz = %d\r\n", p_max_path->vdoprc_vpe_max_2dlut_sz);
	DBG_DUMP("vdoprc_aiisp_maxdevice = %dr\n", p_max_path->vdoprc_aiisp_maxdevice);
	#endif
	return 0;
}
...
int isf_flow_drv_ioctl(unsigned char minor, PMODULE_INFO pmodule_info, unsigned int cmd_id, unsigned long arg)
{
	...
	case ISF_FLOW_CMD_INIT: {
			ISF_FLOW_IOCTL_CMD ctrl_cmd = {0};
			ISF_FLOW_IOCTL_CMD *p_cmd  = 0;
			p_cmd  = &ctrl_cmd;

			if (unlikely(copy_from_user(p_cmd, argp, sizeof(ISF_FLOW_IOCTL_CMD)))) {
				DBG_ERR("copy_from_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_cmd, (ULONG)argp, (int)sizeof(ISF_FLOW_IOCTL_CMD));
			...
			if (unlikely(copy_to_user(argp, p_cmd, sizeof(ISF_FLOW_IOCTL_CMD)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)argp, (ULONG)p_cmd, (int)sizeof(ISF_FLOW_IOCTL_CMD));
	...
	case ISF_FLOW_CMD_OUT_WAI: {
			...
			if (unlikely(copy_to_user(argp, p_cmd, sizeof(ISF_FLOW_IOCTL_OUT_LOG)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)argp, (ULONG)p_cmd, (int)sizeof(ISF_FLOW_IOCTL_BIND_ITEM));
				return -EFAULT;
			}
		}
		break;
	...
	case ISF_FLOW_CMD_OUT_SIG: {
			...
			if (unlikely(copy_from_user(p_cmd, argp, sizeof(ISF_FLOW_IOCTL_OUT_LOG)))) {
				DBG_ERR("copy_from_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_cmd, (ULONG)argp, (int)sizeof(ISF_FLOW_IOCTL_OUT_LOG));
				return -EFAULT;
			}
	...
	case ISF_FLOW_CMD_SET_STATE: {
			...
			if (unlikely(copy_to_user(argp, p_cmd, sizeof(ISF_FLOW_IOCTL_STATE_ITEM)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)argp, (ULONG)p_cmd, (int)sizeof(ISF_FLOW_IOCTL_STATE_ITEM));
				p_cmd->rv = ISF_ERR_COPY_TO_USER;
				return -EFAULT;
			}
	...
	case ISF_FLOW_CMD_SET_PARAM: {
			...
			if(p_cmd->size > 0) {
				...
				if (unlikely(copy_from_user(p_struct, ((void __user *)p_cmd->value), p_cmd->size))) {
					DBG_ERR("copy_from_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_struct, (ULONG)p_cmd->value, p_cmd->size);
					kfree(p_struct);
					//p_struct = NULL;
					return -EFAULT;
				}
			...
			if (unlikely(copy_to_user(argp, p_cmd, sizeof(ISF_FLOW_IOCTL_PARAM_ITEM)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)argp, (ULONG)p_cmd, (int)sizeof(ISF_FLOW_IOCTL_PARAM_ITEM));
				p_cmd->rv = ISF_ERR_COPY_TO_USER;
				return -EFAULT;
			}
...

			{
				if(p_cmd->size > 0) {
					...
					if (unlikely(copy_to_user(((int __user *)p_cmd->value), p_struct, p_cmd->size))) {
						DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_cmd->value, (ULONG)p_struct, p_cmd->size);
						kfree(p_struct);
						//p_struct = NULL;
						return -EFAULT;
					}
					...
					p_cmd->rv = isf_unit_get_param(minor, p_cmd->dest, p_cmd->param, &value);
					p_cmd->value = value;
				}
			}
	...
	case ISF_FLOW_CMD_RELEASE_DATA: {
			ISF_DATA this_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM rel_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM *p_cmd  = 0;
			p_cmd  = &rel_data;

			if (unlikely(copy_from_user(p_cmd, argp, sizeof(ISF_FLOW_IOCTL_DATA_ITEM)))) {
				DBG_ERR("copy_from_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_cmd, (ULONG)argp, (int)sizeof(ISF_FLOW_IOCTL_DATA_ITEM));
				p_cmd->rv = ISF_ERR_COPY_FROM_USER;
				return -EFAULT;
			}
			if (unlikely(copy_from_user(&this_data, (ISF_DATA*)p_cmd->p_data, sizeof(ISF_DATA)))) {
				DBG_ERR("copy_from_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)&this_data, (ULONG)p_cmd->p_data, (int)sizeof(ISF_DATA));
				p_cmd->rv = ISF_ERR_COPY_FROM_USER;
				return -EFAULT;
			}

			DBG_IND("ioctl RELEASE_DATA(dest = %08x, p_data = %08lx)\r\n",
				p_cmd->dest, (ULONG)p_cmd->p_data);
			...
			//ret = -1;
			if (unlikely(copy_to_user(argp, p_cmd, sizeof(ISF_FLOW_IOCTL_DATA_ITEM)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)argp, (ULONG)p_cmd, (int)sizeof(ISF_FLOW_IOCTL_DATA_ITEM));
				p_cmd->rv = ISF_ERR_COPY_TO_USER;
				return -EFAULT;
			}
	...
	case ISF_FLOW_CMD_PUSH_DATA: {
			ISF_DATA this_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM push_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM *p_cmd  = 0;
			p_cmd  = &push_data;
			...
			DBG_IND("ioctl PUSH_DATA(dest = %08x, p_data = %08lx, async = %08x)\r\n",
				p_cmd->dest, (ULONG)p_cmd->p_data, p_cmd->async);
	...
	case ISF_FLOW_CMD_PULL_DATA: {
			ISF_DATA this_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM pull_data = {0};
			ISF_FLOW_IOCTL_DATA_ITEM *p_cmd  = 0;
			p_cmd  = &pull_data;
			...
			DBG_IND("ioctl PULL_DATA(dest = %08x, p_data = %08lx, async = %08x)\r\n",
				p_cmd->dest, (ULONG)p_cmd->p_data, p_cmd->async);
			...
			if (unlikely(copy_to_user((ISF_DATA*)p_cmd->p_data, &this_data, sizeof(ISF_DATA)))) {
				DBG_ERR("copy_to_user(0x%lX, 0x%lX, 0x%X)\r\n", (ULONG)p_cmd->p_data, (ULONG)&this_data, (int)sizeof(ISF_DATA));
				p_cmd->rv = ISF_ERR_COPY_TO_USER;
				return -EFAULT;
			}
