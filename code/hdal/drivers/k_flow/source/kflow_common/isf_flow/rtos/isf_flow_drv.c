...
static int isf_flow_drv_parse_maxpath_from_dts_p(ISF_FLOW_IOCTL_GET_MAX_PATH *p_max_path)
{
	...
	nodep = fdt_getprop(p_fdt, nodeoffset, "vdoprc_vpe_max_handle", &len);
	if (len == 0 || nodep == NULL) {
		//no config means use default
		p_max_path->vdoprc_vpe_max_handle = p_max_path->vdoprc_maxdevice*2;
	} else {
		p_data = (unsigned int *)nodep;
		p_max_path->vdoprc_vpe_max_handle = be32_to_cpu(p_data[0]);
	}
	nodep = fdt_getprop(p_fdt, nodeoffset, "vdoprc_vpe_max_2dlut_sz", &len);
	if (len == 0 || nodep == NULL) {
		//no config means use default
		p_max_path->vdoprc_vpe_max_2dlut_sz = 5;
	} else {
		p_data = (unsigned int *)nodep;
		p_max_path->vdoprc_vpe_max_2dlut_sz = be32_to_cpu(p_data[0]);
	}
	nodep = fdt_getprop(p_fdt, nodeoffset, "vdoprc_aiisp_maxdevice", &len);
	if (len == 0 || nodep == NULL) {
		p_max_path->vdoprc_aiisp_maxdevice = 0;
	} else {
		p_data = (unsigned int *)nodep;
		p_max_path->vdoprc_aiisp_maxdevice = be32_to_cpu(p_data[0]);
	}
	...
	DBG_DUMP("vdoprc_aiisp_maxdevice = %d\r\n", p_max_path->vdoprc_aiisp_maxdevice;
	#endif
	return 0;
}
	...
	case ISF_FLOW_CMD_GET_PARAM: {
...

			{
				if(p_cmd->size > 0) {
					...
					p_cmd->rv = isf_unit_get_param(0, p_cmd->dest, p_cmd->param, &(p_cmd->value));
				}
			}
