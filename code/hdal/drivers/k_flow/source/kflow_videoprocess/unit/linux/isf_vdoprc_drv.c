...
int isf_vdoprc_drv_init(PMODULE_INFO pmodule_info)
{
	...
	if (!isf_vdoprc_init(&init)) {
		VPRC_DBG_ERR("vdoprc_init() Failed!\r\n");
		return -ENODEV;
	}
...
#endif
#if (VDOPRC_MAX_NUM > 16)
	isf_reg_unit(ISF_UNIT_ID(VDOPRC,16), &isf_vdoprc16);
#endif
#if (VDOPRC_MAX_NUM > 17)
	isf_reg_unit(ISF_UNIT_ID(VDOPRC,17), &isf_vdoprc17);
#endif
#if (VDOPRC_MAX_NUM > 18)
	isf_reg_unit(ISF_UNIT_ID(VDOPRC,18), &isf_vdoprc18);
#endif
#if (VDOPRC_MAX_NUM > 19)
	isf_reg_unit(ISF_UNIT_ID(VDOPRC,19), &isf_vdoprc19);
#endif
