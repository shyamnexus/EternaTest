...
//============================================================================
#if defined(__LINUX)
int nvt_ctl_ise_proc_buffer_to_int(const char __user *buffer, size_t count, int *dst, int dst_cnt)
{
#define PARAM_LENGTH 100
	...
	if ((count > PARAM_LENGTH) || (count < 2)) {
		ctl_ise_dbg_err("cmd length(%zd) must <%d and >2\n", count, PARAM_LENGTH);
		return -1;
	}

	memset(buf, 0, PARAM_LENGTH);
	if (copy_from_user(buf, buffer, PARAM_LENGTH)) {
		ctl_ise_dbg_err("convert fail1\n");
		return -1;
	}
...
static struct proc_ops proc_cmd_fops = {
	.proc_open		= nvt_ctl_ise_proc_cmd_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
	.proc_write		= nvt_ctl_ise_proc_cmd_write
};
...
{
	seq_printf(sfile, "=====================================================================\n");
	seq_printf(sfile, " kflow ise message\n");
	seq_printf(sfile, "=====================================================================\n");
	return 0;
}
...
static struct proc_ops proc_help_fops = {
	.proc_open		= nvt_ctl_ise_proc_help_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static struct proc_ops proc_info_fops = {
	.proc_open		= nvt_ctl_ise_proc_info_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
//=============================================================================
static int nvt_ctl_ise_proc_dbg_lvl_show(struct seq_file *sfile, void *v)
{
	pdrv_info_data->info_seq_file = sfile;
	return 0;
}

static ssize_t nvt_ctl_ise_proc_dbg_lvl_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
#define DATA_SIZE 1
	int data[DATA_SIZE] = {0};

	if (nvt_ctl_ise_proc_buffer_to_int(buffer, count, &data[0], DATA_SIZE) != DATA_SIZE) {
		ctl_ise_dbg_err("input parameter error\n");
		return -1;
	}
	DBG_ERR("0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\n");
	ctl_ise_dbg_dump("kflow_ise dbglevel = %d\n", data[0]);
	ctl_ise_set_dbg_lvl(data[0]);

#undef DATA_SIZE
	return count;
}

static int nvt_ctl_ise_proc_dbg_lvl_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_ctl_ise_proc_dbg_lvl_show, NULL);
}

static struct proc_ops proc_dbg_lvl_fops = {
	.proc_open   	= nvt_ctl_ise_proc_dbg_lvl_open,
	.proc_release 	= single_release,
	.proc_read   	= seq_read,
	.proc_lseek 	= seq_lseek,
    .proc_write   	= nvt_ctl_ise_proc_dbg_lvl_write,
};
