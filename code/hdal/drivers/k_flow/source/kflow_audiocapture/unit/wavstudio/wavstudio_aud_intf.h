...
ER WavStud_AudDrvInit(WAVSTUD_ACT actType, PWAVSTUD_AUD_INFO audInfo, UINT32 audVol, ULONG fpEventCB);
ER WavStud_AudLBDrvInit(WAVSTUD_ACT actType, AUDIO_SR audSR, AUDIO_CH audCh, ULONG fpEventCB);
...
VOID WavStud_SetAudObj(WAVSTUD_ACT actType, KDRV_DEV_CHIP chip, KDRV_DEV_ENGINE engine);
KDRV_AUD_HANDLE WavStud_GetAudHdl(WAVSTUD_ACT actType);
KDRV_AUDIO_TRIGGER_TYPE WavStud_GetAudTrigType(WAVSTUD_ACT actType);
VOID WavStud_SetAudInfo(WAVSTUD_ACT actType, KDRV_AUD_HANDLE kdrv_hdl);
extern UINT32 (*WavStud_GetAudObj)(WAVSTUD_ACT actType);
