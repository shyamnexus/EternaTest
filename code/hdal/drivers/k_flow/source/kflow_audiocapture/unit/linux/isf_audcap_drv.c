...
int isf_audcap_drv_init(ISF_AUDCAP_INFO *p_module_info)
{
	...
	isf_reg_unit(ISF_UNIT_ID(AUDCAP,0), &isf_audcap0);
...
int isf_audcap_drv_suspend(ISF_AUDCAP_INFO *p_module_info)
{
	...
	/* Check status */
	{
		...
		status |= wavstudio_get_config(WAVSTUD_CFG_GET_STATUS, WAVSTUD_ACT_REC2, 0);
		if (status & WAVSTUD_STS_GOING) {
			DBG_ERR("acap has working streaming(status:start) while system is suspending\r\n");
			return -1;
		}
	}
