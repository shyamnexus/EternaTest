...
int _isf_vdoenc_alloc_unit(void)
{
	...
	uintptr_t buff_addr = 0;
	UINT32 vdoenc_maxpath = 0, buff_size = 0, i = 0;
	...
	if (vdoenc_maxpath > VDOENC_MAX_PATH_NUM) {
		DBG_WRN("dts max_path=%lu is larger than built-in max_path=%lu, set to built-in max_path=%lu!\r\n", (unsigned long)(vdoenc_maxpath), (unsigned long)((UINT32)VDOENC_MAX_PATH_NUM), (unsigned long)((UINT32)VDOENC_MAX_PATH_NUM));
		vdoenc_maxpath = VDOENC_MAX_PATH_NUM;
	}
	...
	buff_addr = (uintptr_t)ISF_VdoEnc_Unit_Addr;
...
int isf_vdoenc_drv_suspend(ISF_VDOENC_INFO *pmodule_info)
{
	...
	/* Check status */
	{
		if (isf_vdoenc_check_start_stream_for_suspend() != 0) {
			DBG_ERR("venc has working streaming(status:start) while system is suspending\r\n");
			return -1;
		}
	}
