...
typedef struct {
	...
	uintptr_t                   uiSnapshotAddr;         //in: snapshot addr
	UINT32                      uiSnapshotSize;         //in: snapshot size
	uintptr_t                   uiVCodecAddr;           //in: video codec addr
	UINT32                      uiVCodecSize;           //in: video codec size
	uintptr_t                   uiBSStart;              //in: video bs start addr
	uintptr_t                   uiBSEnd;                //in: video bs end addr
	uintptr_t                   uiBSMax;                //in: video bs reserved addr
	...
	uintptr_t                   uiRecBuffAddr[VDOENC_MAX_RECONFRM_NUM];       //in: video reconstruct buffer addr
	UINT32                      uiRecBuffSize;          //in: video reconstruct buffer size
	UINT32                      uiRecBuffNum;           //in: video reconstruct buffer number
	UINT32                      uiEncMaxByteRate;       //in: enc max byte rate
...
typedef struct {
	...
	MP_VDOENC_CVBR_INFO         CVbrInfo;               //in: cvbr info
	...
	MP_VDOJPGE_ROI_INFO         JpegRoiInfo;            //in: set ROI Info (max count: 11)
	...
	KDRV_VDOENC_SLICE_SPLIT     SliceSplitExtinfo;      //in: slice split info for 690
	...
	MP_VDOENC_WATERMARK         WatermarkInfo;          //in: set watermark Info (max count: 8)
	...
	uintptr_t                   uiDescAddr;             //out: desc addr
	...
	MP_VDOENC_LPM               Lpm;                        //in: set low power
	UINT32                      uiVidLowLatencyVprcSrcPath; //in: fastboot video yuv from which vproc path
#ifdef VDOENC_LL
	...
	UINT32                      uiVidFastbootYCCEnable;             //in: fastboot ycc enable
	UINT32                      uiVidFastbootYCCWidth;              //in: fastboot ycc width
	UINT32                      uiVidFastbootYCCHeight;             //in: fastboot ycc hdeight
	UINT32                      uiVidFastbootYCCYLoff;              //in: fastboot ycc y lineoffset
	UINT32                      uiVidFastbootYCCCLoff;              //in: fastboot ycc c lineoffset
	...
	BOOL                        bJpgUVCSliceEnc;            //in: jpg uvc slice encode
	NMI_VDOENC_META_ALLOC_INFO  *pMetaAllocInfo;            //in: pointer for meta alloc info
	MP_VDOENC_USER_DATA         UsrData;                    //in: set user data
	MP_VDOENC_BG_RDO            BgRDO;                      //in: set bg rdo
	MP_VDOENC_MOTION_DETECT     EmbeddedMD;                 //in: set embedded md
	BOOL                        bSetRowRC;                  //in: set row rc from user
	BOOL                        bSetAQ;                     //in: set aq from user
	BOOL                        bSliceInterrupt;            //in: set slice interrupt
	BOOL                        bSetJnd;                    //in: set jnd
	BOOL                        bSetRdo;                    //in: set rdo
	MP_VDOENC_SMART_ROI         SmartRoi;               //in: set smart roi
	MP_VDOENC_SMART_BBOX        SmartBBox;              //in: set smart bbox
	BOOL                        bSetSmartRoi;           //in: set smart roi from user
	BOOL                        bSetSmartBBox;          //in: set smart bbox from user
	MP_VDOENC_DYNAMIC_FR        DynamicFr;              //in: set dynamic fr
	MP_VDOENC_DYNAMIC_GOP       DynamicGOP;             //in: set dynamic gop
	BOOL                        bSetDynamicFr;          //in: set dynamic fr from user
	BOOL                        bSetDynamicGOP;         //in: set dynamic gop from user
	NMI_VDOENC_FBOOT_FRC        FrcInfo;
	MP_VDOENC_SEI_SIGNED_VDO    SeiSignedVdo;               //in: set sei signed vdo
	BOOL                        bSetSEISIGNEDVDO;           //in: set sei signed vdo from user
	UINT64                      uiEncTrigCount;
	UINT64                      uiEncCBCount;
	BOOL                        bSetLowPower;               //in: set low power from user
	BOOL                        bForceStop;
	BOOL                        bSkipBsQuickRollback;   //in: skip bs buffer quick rollback
} NMR_VDOENC_OBJ;
...
extern void             NMR_VdoEnc_GetDesc(UINT32 pathID, uintptr_t *pAddr, UINT32 *pSize);
...
extern void             nmr_vdoenc_dump_encinfo_in_file(UINT32 pathID, int (*dump)(const char *fmt, ...));
extern void             nmr_vdoenc_dump_rcinfo_in_file(UINT32 pathID, int (*dump)(const char *fmt, ...));
...
extern SEM_HANDLE _SECTION(".kercfg_data") NMR_VDOENC_TRIG_SLICE_ENC_SEM_ID[];
