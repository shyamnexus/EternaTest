...
#define NMR_VDOTRIG_YUVQ_MAX            4              ///< Video Raw Queue Maximum number
...
typedef struct {
	...
	UINT32                      Rear;                   ///< Rear pointer
	UINT32                      bFull;                  ///< Full flag
	ULONG                       Queue[NMR_VDOTRIG_JOBQ_MAX];
} NMR_VDOTRIG_WRAPBS_USRDATAQ, *PNMR_VDOTRIG_WRAPBS_USRDATAQ;
...
typedef struct {
...
#ifdef VDOENC_LL
	MP_VDOENC_PARAM 			SliceEncParam;
#endif
	NMR_VDOTRIG_YUVQ            rawQueue;
	NMR_VDOTRIG_SRCOUTQ         srcoutQueue;
#ifdef VDOENC_LL
	NMR_VDOTRIG_WRAPBS_USRDATAQ usrdataQueue;
#endif
	...
	//BisStream info
	uintptr_t                   BsStart;                ///< in:bitstream starting addr
	uintptr_t                   BsEnd;                  ///< in:bitstream end addr
	uintptr_t                   BsMax;                  ///< in:addr for bitstream rollback
	uintptr_t                   BsNow;                  ///< out: current bs addr
	UINT32                      BsSize;        		    ///< out: current bs size
	uintptr_t                   BsNow2;                 ///< out: current bs addr2
...
#ifdef VDOENC_LL
extern ER       NMR_VdoTrig_PutUsrData_WrapBS(UINT32 pathID, ULONG UsrDataAddr);
...
#ifdef VDOENC_LL
extern ER       NMR_VdoTrig_GetUsrData_WrapBS(UINT32 pathID, ULONG *pUsrDataAddr);
...
#ifdef VDOENC_LL
extern UINT32   NMR_VdoTrig_GetUsrDataCount_WrapBS(UINT32 pathID);
extern UINT32   NMR_VdoTrig_GetJobCount_WrapBS(void);
#endif
extern UINT32   NMR_VdoTrig_GetBSCount(UINT32 pathID);
extern BOOL     NMR_VdoTrig_CheckBS(UINT32 pathID, uintptr_t *uiAddr, UINT32 *uiSize, uintptr_t *uiAddr2, UINT32 *uiSize2, uintptr_t *uiStartAddr);
extern BOOL     NMR_VdoTrig_LockBS(UINT32 pathID, uintptr_t uiAddr, UINT32 uiSize);
extern BOOL     NMR_VdoTrig_UnlockBS(UINT32 pathID, uintptr_t uiAddr);
extern void     NMR_VdoTrig_Stop(UINT32 pathID);
extern void     nmr_vdotrig_release_all_yuvq(UINT32 pathID);
extern void     nmr_vdotrig_release_all_usrdataq(UINT32 pathID);
