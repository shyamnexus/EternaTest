...
typedef void (*MEDIAREC_3DNRCB)(UINT32 pathID, ULONG config);
typedef void (*MEDIAREC_ISPCB)(UINT32 path_id, ULONG config);
typedef void (*MEDIAREC_SPNCB)(UINT32 pathID, ULONG config);
#ifdef VDOENC_LL
...
typedef enum {
	...
	MP_VDOENC_ID_17,                        ///< video encoder ID 17
	MP_VDOENC_ID_18,                        ///< video encoder ID 18
	MP_VDOENC_ID_19,                        ///< video encoder ID 19
	MP_VDOENC_ID_20,                        ///< video encoder ID 20
	MP_VDOENC_ID_21,                        ///< video encoder ID 21
	MP_VDOENC_ID_22,                        ///< video encoder ID 22
	MP_VDOENC_ID_23,                        ///< video encoder ID 23
	MP_VDOENC_ID_24,                        ///< video encoder ID 24
	MP_VDOENC_ID_25,                        ///< video encoder ID 25
	MP_VDOENC_ID_26,                        ///< video encoder ID 26
	MP_VDOENC_ID_27,                        ///< video encoder ID 27
	MP_VDOENC_ID_28,                        ///< video encoder ID 28
	MP_VDOENC_ID_29,                        ///< video encoder ID 29
	MP_VDOENC_ID_30,                        ///< video encoder ID 30
	MP_VDOENC_ID_31,                        ///< video encoder ID 31
	MP_VDOENC_ID_32,                        ///< video encoder ID 32
...
typedef struct {
	...
	uintptr_t  uiYAddr;                                 ///< Y address
	uintptr_t  uiCbAddr;                                ///< Cb address
	uintptr_t  uiCrAddr;                                ///< Cr address
...
typedef struct  {
	...
	BOOL    slice_mode;
	UINT32  slice_row_num_ext[3];
	INT32   slice_i_idx;
} MP_VDOENC_SLICESPLIT_INFO;
...
typedef KDRV_VDOENC_CVBR MP_VDOENC_CVBR_INFO;
...
typedef KDRV_VDOENC_GRAY MP_VDOENC_GRAY;

typedef KDRV_VDOENC_H264_TILE_BUF_INFO MP_VDOENC_H264_TILE_BUF_INFO;

typedef KDRV_VDOENC_H264_TILE_MEM MP_VDOENC_H264_TILE_MEM;

typedef KDRV_VDOENC_H26X_MIN_BS_BUF_INFO MP_VDOENC_H26X_MIN_BS_BUF_INFO;

typedef KDRV_VDOENC_MIN_BS_RATIO_INFO MP_VDOENC_MIN_BS_RATIO_INFO;

typedef KDRV_VDOENC_USER_DATA MP_VDOENC_USER_DATA;

typedef KDRV_VDOENC_BG_RDO MP_VDOENC_BG_RDO;

typedef KDRV_VDOENC_WATERMARK MP_VDOENC_WATERMARK;

typedef KDRV_VDOENC_MOTION_DETECT MP_VDOENC_MOTION_DETECT;

typedef KDRV_VDOJPGE_ROI MP_VDOJPGE_ROI_INFO;

typedef KDRV_VDOENC_SMART_ROI MP_VDOENC_SMART_ROI;

typedef KDRV_VDOENC_SMART_BBOX MP_VDOENC_SMART_BBOX;

typedef KDRV_VDOENC_DYNAMIC_FR MP_VDOENC_DYNAMIC_FR;

typedef KDRV_VDOENC_DYNAMIC_GOP MP_VDOENC_DYNAMIC_GOP;

typedef KDRV_VDOENC_SEI_SIGNED_VDO MP_VDOENC_SEI_SIGNED_VDO;

typedef KDRV_VDOENC_LPM MP_VDOENC_LPM;
...
typedef KDRV_VDOJPGE_PARAM MP_VDOJPGE_PARAM;

typedef KDRV_VDOJPGE_RC MP_VDOJPGE_RC;

/**
    Encode callback structure
*/
typedef KDRV_VDOJPGE_RESULT MP_VDOJPGE_RESULT;
...
typedef struct {
	...
	ER(*GetInfo)(MP_VDOENC_GETINFO_TYPE type, VOID *pparam1, VOID *pparam2, VOID *pparam3);   ///< get info from codec library
	ER(*SetInfo)(MP_VDOENC_SETINFO_TYPE type, ULONG param1, ULONG param2, ULONG param3);  		///< set parameters to codec library
#ifdef VDOENC_LL
...
#else
	ER(*EncodeOne)(UINT32 type, uintptr_t outputAddr, UINT32 *pSize, MP_VDOENC_PARAM *ptr);  			///< encoding one video frame and wait ready
#endif
