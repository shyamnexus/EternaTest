...
// jpeg slice encode
#define VDOENC_MAX_SLICE_CNT    32
#define VDOENC_SLICE_HEIGHT     64
#define VDOENC_SLICE_HEIGHT_4K  128
...
typedef enum {
	...
	NMI_VDOENC_PARAM_JPG_FRAMERATE_BASE,         // encode framerate base for jpeg encode
	NMI_VDOENC_PARAM_JPG_FRAMERATE_INCR,         // encode framerate incr for jpeg encode
	...
	NMI_VDOENC_PARAM_YUV420_MB_MODE,
	NMI_VDOENC_PARAM_BR_TOLERANCE,
	NMI_VDOENC_PARAM_JPG_COLOR_TO_GRAY,
	NMI_VDOENC_PARAM_TILE_SPLIT_MODE,
	NMI_VDOENC_PARAM_H26X_LOW_POWER,
	NMI_VDOENC_PARAM_LOWLATENCY_VPRC_SRC_PATH,
	NMI_VDOENC_PARAM_JPG_UVC_SLICE_ENC,
	...
	NMI_VDOENC_PARAM_USER_DATA,
	NMI_VDOENC_PARAM_BS_RING,
	NMI_VDOENC_PARAM_FASTBOOT_YCC_INFO,
	NMI_VDOENC_PARAM_BG_RDO,
	NMI_VDOENC_PARAM_WATERMARK,
	NMI_VDOENC_PARAM_EMBEDDED_MD,
	NMI_VDOENC_PARAM_JPEG_ROI,
	NMI_VDOENC_PARAM_SLICE_INTERRUPT,
	NMI_VDOENC_PARAM_SMART_BBOX,
	NMI_VDOENC_PARAM_DYNAMIC_FR,
	NMI_VDOENC_PARAM_DYNAMIC_GOP,
	NMI_VDOENC_PARAM_H26X_SKIP_FRM,
	NMI_VDOENC_PARAM_FASTBOOT_FRC_INFO,
	NMI_VDOENC_PARAM_SEI_SIGNED_VDO,
	NMI_VDOENC_PARAM_H26X_PRIVACY,
	NMI_VDOENC_PARAM_JPG_RC_VBR_PRIORITY,
	NMI_VDOENC_PARAM_JPG_RC_MIN_FRAME_RATE,
	NMI_VDOENC_PARAM_FORCE_STOP,
	NMI_VDOENC_PARAM_SKIP_BS_QUICK_ROLLBACK,
} NMI_VDOENC_PARAM;
...
typedef struct {
	uintptr_t Addr;                             //memory buffer starting address
...
typedef struct {
	...
	uintptr_t Addr;            					//1st output bit-stream address
	UINT32 Size;            					//1st output bit-stream size
	uintptr_t Addr2;            					//2nd output bit-stream address
	UINT32 Size2;            					//2nd output bit-stream address
	uintptr_t RawYAddr;        					//raw y address
	...
	uintptr_t SrcOutYAddr;     					//src out y addr
	uintptr_t SrcOutUVAddr;						//src out uv addr
	...
	uintptr_t nalu_info_addr;                      //nalu_info_addr(va)
	...
	UINT32 bs_offset;                           ///< jpeg bs offset
	UINT32 sha_chksum[8];                       ///< sha256 checksum
	UINT32 enc_err;
...
typedef struct {
	...
	uintptr_t  y_addr;                          ///< y address
	uintptr_t  c_addr;                          ///< uv address
	UINT32  y_line_offset;                      ///< y line offset
	UINT32  c_line_offset;                      ///< uv line offset
	UINT32  quality;                            ///< jpeg quality
	uintptr_t  snap_addr;                       ///< bs addr
	UINT32  snap_size;                          ///< bs buffer size, encoded bs size
	UINT64  timestamp;                          ///< YUV timestamp
} NMI_VDOENC_YUV_SRCOUT;

typedef struct {
	UINT32 pathID;                              ///< pathID
	UINT32 slice_idx;                           ///< jpg slice encode index
	UINT32 buf_id;
	NMI_VDOENC_YUV_SRCOUT srcout_info;
} NMI_VDOENC_USR_DATA, *PNMI_VDOENC_USR_DATA;
#endif
...
#define NMI_VDOENC_WATERMARK_COUNT 8
typedef struct _NMI_VDOENC_WATERMARK{
	BOOL enable[NMI_VDOENC_WATERMARK_COUNT];
	UINT8 uv[NMI_VDOENC_WATERMARK_COUNT];
	UINT8 str[NMI_VDOENC_WATERMARK_COUNT];
	UINT32 left_top[NMI_VDOENC_WATERMARK_COUNT];
	UINT8 len[NMI_VDOENC_WATERMARK_COUNT];
	uintptr_t addr[NMI_VDOENC_WATERMARK_COUNT];
} NMI_VDOENC_WATERMARK;

#define NMI_VDOENC_SMART_ROI_MAX_NUM 	32
typedef struct {
	UINT32 x;
	UINT32 y;
} NMI_VDOENC_POSITION;
...
typedef struct {
	NMI_VDOENC_POSITION position[2];   ///< top left and bottom right position of bounding box
...
typedef struct {
	...
	UINT8 bbox_num;                                        	///< number of bounding boxes, range: NMI_VDOENC_SMART_ROI_MAX_NUM
	NMI_VDOENC_BBOX bbox[NMI_VDOENC_SMART_ROI_MAX_NUM];   ///< bounding boxes
	UINT64 timestamp;                                       ///< timestamp of bounding boxes
} NMI_VDOENC_SMART_BBOX;
...
} NMI_VDOENC_FBOOT_FRC;

typedef enum {
	NMI_VDOENC_SEI_SIGNED_VDO_ID_1 = 0,
	NMI_VDOENC_SEI_SIGNED_VDO_ID_2,
	NMI_VDOENC_SEI_SIGNED_VDO_ID_3,
	NMI_VDOENC_SEI_SIGNED_VDO_ID_4,
	NMI_VDOENC_SEI_SIGNED_VDO_ID_MAX,
	ENUM_DUMMY4WORD(NMI_VDOENC_SEI_SIGNED_VDO_ID)
} NMI_VDOENC_SEI_SIGNED_VDO_ID;

typedef struct {
	BOOL                enable;
	UINT32              machine_id[NMI_VDOENC_SEI_SIGNED_VDO_ID_MAX];
} NMI_VDOENC_SEI_SIGNED_VDO_CFG;

typedef struct _NMI_VDOENC_LOW_POWER{
	BOOL enable;
	UINT8 mode;
} NMI_VDOENC_LOW_POWER;

typedef UINT32 (NMI_VDOENC_CB)(UINT32 event, ULONG param);
typedef void (NMI_VDOENC_IMM_CB)(UINT32 type, ULONG p1, ULONG p2, ULONG p3);
