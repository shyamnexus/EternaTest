...
typedef struct {
	...
	NVTMPP_VB_POOL rtn;                                   ///< [out]:return value
...
typedef struct {
	NVTMPP_VB_BLK  blk;                                   ///< [in]: the block handle
	uintptr_t      rtn;                                   ///< [out]:return value
...
typedef struct {
	uintptr_t      blk_pa;                                ///< [in]:  the blk physical address
	NVTMPP_VB_POOL pool;                                  ///< [out]: the pool handle.
	UINT32         size;                                  ///< [out]: the blk size
} NVTMPP_IOC_VB_PA_TO_POOL_S;

typedef struct {
	uintptr_t       pa;                                    ///< [in]:  the physical address
	uintptr_t       va;                                    ///< [out]:  the kernel virtual address
} NVTMPP_IOC_VB_PA_TO_VA_S;

typedef struct {
	const void     *va;                                   ///< [in]  the user virtual memory
	uintptr_t       pa;                                   ///< [out] the physical memory
...
typedef struct {
	...
	UINT32         blk_cnt;                               ///< [in]: the block count of this pool
	NVTMPP_DDR     ddr;                                   ///< [in]: create pool in which DDR.
	uintptr_t      rtn;                                   ///< [out]:return value is the pa
} NVTMPP_IOC_VB_CREATE_FIXPOOL_S;


typedef struct {
	uintptr_t      phys_addr;                             ///< [in]: the physical addr of certain memory on fdt description
...
typedef struct {
	uintptr_t      va;                                    ///< [in]:  the kernel virtual address
	uintptr_t      pa;                                    ///< [out]: the physical address
} NVTMPP_IOC_VB_VA_TO_PA_S;


typedef struct {
	NVTMPP_DDR     ddr;                                   ///< [in]:  which DDR.
	uintptr_t      phys_addr;                             ///< [out]: the physical addr pool start
