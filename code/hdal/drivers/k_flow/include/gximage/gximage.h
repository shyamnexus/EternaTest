...
typedef struct {
	...
	UINT32 y1;
	UINT32 linewidth;
	UINT32 line_distance;	// line_distance = linewidth * sqrt(A * A + B * B)
} GXIMG_LINE_DESC, *PGXIMG_LINE_DESC;
/**
...
typedef struct _GXIMG_ENCODE_INFO {
	...
	ULONG      out_buf_addr;       ///< Out bitstream buffer starting address
...
/**
     ...
     @endcode
*/
extern ER gximg_init_buf(PVDO_FRAME p_img_buf, UINT32 width, UINT32 height, VDO_PXLFMT pxlfmt, UINT32 lineoff, uintptr_t addr, UINT32 available_size);

extern ER gximg_init_buf_h_align(PVDO_FRAME p_img_buf, UINT32 width, UINT32 height, UINT32 halign, VDO_PXLFMT pxlfmt, UINT32 lineoff, uintptr_t addr, UINT32 available_size);
...
/**
     ...
     @endcode
*/
extern ER gximg_init_buf_ex(PVDO_FRAME p_img_buf, UINT32 width, UINT32 height, VDO_PXLFMT pxlfmt, UINT32 lineoff[GXIMG_MAX_PLANE_NUM], uintptr_t pxladdr[GXIMG_MAX_PLANE_NUM]);

/**
     Init Image Buffer convert va API.

     User can use this API when the Y UV data address & lineoffset is already known in other image buffer structure.
     The Y UV data address may be not in sequence or continuous.
     convert va is for flush, not convert is for force pa and no flush
     ...
     @param[in] lineoff: The image pixel lineoffset of each plane.
     @param[in] pxladdr: The image raw buffer address of each plane.
     @param[in] convert_va: if convert va whe init buf.
     @return
         - @b E_OK:    Init Image buffer ok.
         - @b E_PAR:   Init Image buffer fail, input parameter error.
     ...
     @code
     {
         ...
         uiPxlAddr[0]    = StampBuffer.uiYAddr;
         uiPxlAddr[1]    = StampBuffer.uiUAddr;
         gximg_init_buf_conv(&SrcImg, uiWidth, uiStampDataHeight,VDO_PXLFMT_YUV420, uiLineOffset, uiPxlAddr,1);
     }
     @endcode
*/
extern ER gximg_init_buf_conv(PVDO_FRAME p_img_buf, UINT32 width, UINT32 height, VDO_PXLFMT pxlfmt, UINT32 lineoff[GXIMG_MAX_PLANE_NUM], uintptr_t pa[GXIMG_MAX_PLANE_NUM],UINT32 convert_va);
...
*/
extern ER gximg_get_buf_addr(PVDO_FRAME p_img_buf, uintptr_t *p_start_addr, uintptr_t *p_end_addr);
...
/**
     ...
     @return
         ...
         - @b E_PAR:   Input parameter error.
*/
extern ER gximg_copy_blend_data_ex(PVDO_FRAME p_src_img, IRECT    *p_src_region, PVDO_FRAME p_dst_img, IPOINT    *p_dst_location,PVDO_FRAME p_src2_img, UINT8 *p_alpha_plane);

extern ER gximg_copy_blend_data_ex_no_flush(PVDO_FRAME p_src_img, IRECT    *p_src_region, PVDO_FRAME p_dst_img, IPOINT    *p_dst_location,PVDO_FRAME p_src2_img, UINT8 *p_alpha_plane);
...
*/
extern ER gximg_rotate_data(PVDO_FRAME p_img_buf, uintptr_t dst_buff, UINT32 dst_buf_size, UINT32 rotate_dir, PVDO_FRAME p_out_img_buf);

extern ER gximg_rotate_data_no_flush(PVDO_FRAME p_img_buf, uintptr_t dst_buff, UINT32 dst_buf_size, UINT32 rotate_dir, PVDO_FRAME p_out_img_buf);
...
/**
     ...
     @return
         ...
         - @b E_PAR:   Input parameter error or destination buffer too small.
*/
extern ER gximg_rotate_data_ex(PVDO_FRAME p_img_buf, uintptr_t dst_buff, UINT32 dst_buf_size, UINT32 rotate_dir, PVDO_FRAME p_out_img_buf, GXIMG_RT_ENG engine);
...
*/
extern ER gximg_self_rotate(PVDO_FRAME p_img_buf, uintptr_t tmp_buff, UINT32 tmp_buf_size, UINT32 rotate_dir);
...
*/
extern ER gximg_color_transform(PVDO_FRAME p_src_img, PVDO_FRAME p_dst_img, uintptr_t tmp_buff, UINT32 tmp_buf_size);
...
*/
extern ER gximg_gray_to_yuv(PVDO_FRAME p_src_img, PVDO_FRAME p_dst_img, uintptr_t lookup_table[GXIMG_MAX_PLANE_NUM]);
...
/**
     draw slash line to the image.

     Supported input formats: \n
     GXIMG_PXLFMT_YUV420
     GXIMG_PXLFMT_Y8

     @param[in] p_line: The line description.
     @param[in] p_dst_img: The destionation image buffer.
     @param[in] color: The line color depend on formar, YUV should yuv color,ARGB is ARGB color.
     @param[in] no_flush: 0 flush img;1  not flush.
     ...
     @return
         ...
         - @b E_SYS:   HW engine or system error
*/
extern ER gximg_line_with_flush(GXIMG_LINE_DESC *p_line,PVDO_FRAME p_dst_img, UINT32 color,int no_flush);
...
/**
     setup up graphic's parameter to do data copy for link list operation

     build graphic's request, image_a and image_c for data copy

     @note caller should manually links image_a to request and image_c to image_a
	       Different requests should be manually linked together through request.p_next

     @param[in] p_src_img: The source image buffer.
     @param[in] p_src_region: The source buffer region for copying. GXIMG_REGION_MATCH_IMG means the region matches source image buffer.
     @param[in] p_dst_img: The destionation image buffer.
     @param[in] p_dst_location: The destionation image buffer location to copy to.
     @param[in] plane: y plane or uv plane
     @param[in] no_flush: if flush cpu cache is unneccessary
     @param[in] p_request: graphic's KDRV_GRPH_TRIGGER_PARAM request pointer
     @return
         - @b E_OK:    Copy data success.
         - @b E_PAR:   Input parameter error.
*/
extern ER gximg_setup_copy_request(PVDO_FRAME p_src_img, IRECT *p_src_region, PVDO_FRAME p_dst_img, IPOINT *p_dst_location, UINT32 plane, int no_flush, void *p_request);

/**
     run graphic request for link list operation

     @note p_request is recommended to be setup by gximg_setup_copy_request().
	       caller can also manually setup p_request but this requires solid graphic programming knowledge.

     @param[in] p_request: graphic request(KDRV_GRPH_TRIGGER_PARAM) to execute
     @param[in] engine: which graphic engine to execute the request
     @return
         - @b E_OK:    Copy data success.
         - @b E_PAR:   Input parameter error.
*/
extern ER gximg_execute_request(void *p_request, GXIMG_CP_ENG engine);

extern ER gximg_arithmetic(uintptr_t op1_va,uintptr_t op2_va,uintptr_t out_va,
           UINT32 operation,UINT32 bits,UINT32 size);
