...
#define AUDCAP_LIB_AGC  DISABLE

#define AUDCAP_MAX_OUT  20 ///< max count of output of this device (interface)
...
typedef struct {
	...
	ER(*stop)(void);
	ER(*apply)(ULONG, ULONG, ULONG, UINT32);
	void(*enable)(BOOL);
	UINT32(*getbuf)(UINT32);
	ER(*setbuf)(ULONG, UINT32);
} AUDCAP_AEC_OBJ, *PAUDCAP_AEC_OBJ;

typedef struct {
	ER(*open)(INT32, INT32);
	ER(*close)(void);
	ER(*apply)(ULONG, UINT32);
	void(*enable)(BOOL);
	UINT32(*getbuf)(INT32, INT32);
	void(*setbuf)(ULONG, ULONG);
