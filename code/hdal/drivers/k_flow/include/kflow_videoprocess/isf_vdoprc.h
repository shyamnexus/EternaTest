...
#define AI_FILTER_MAX  4
#define AI_EFFECT_ID_MAX 16

typedef struct {
    UINT32 enable;
    UINT32 proc_id;
    UINT32 effect2proc[AI_EFFECT_ID_MAX];
} VDOPRC_ISP_AI_FILTER;

typedef struct {
    VDOPRC_ISP_AI_FILTER filter[AI_FILTER_MAX];
} VDOPRC_ISP_AI;

typedef struct _VDOPRC_ISP_AI_MAP {
    UINT32 path_id; ///<  fill 0 ... 3
    UINT32 proc_id; ///< proc_id of AI model
} VDOPRC_ISP_AI_MAP;

typedef struct _VDOPRC_ISP_AI_EFFECT_MAP {
	UINT32 enable;                      ///< AI-ISP effect map enable
	UINT32 path_id;                     ///< AI-ISP path 0 ... 3
	UINT32 proc_id[AI_EFFECT_ID_MAX];   ///< proc_id of AI model for ISP effect 1 ... MAX
} VDOPRC_ISP_AI_EFFECT_MAP;

typedef struct _VDOPRC_ISP_AI_POSTPROC {
	UINT32 enable;
} VDOPRC_ISP_AI_POSTPROC;

typedef struct _VDOPRC_ISP_BIND_INFO {
...
} VDOPRC_ISP_BIND_INFO;

typedef struct {
	UINT32 cur_in_fps[VDOPRC_MAX_IN_NUM];       ///< current in fps
	UINT32 cur_out_fps[VDOPRC_MAX_OUT_NUM];     ///< current out fps
} VDOPRC_SYSINFO, *PVDOPRC_SYSINFO;
...
enum {
	...
	VDOPRC_PARAM_VSP_CFG,    ///< ISF_CTRL: vsp config
	VDOPRC_PARAM_EIS_FUNC, ///< ISF_OUTx: enable or disable EIS function
	VDOPRC_PARAM_DRE_CFG,    ///< ISF_CTRL: dre config
	VDOPRC_PARAM_VPE_CLEAR_WIN,  ///< ISF_CTRL: clear win config
    VDOPRC_PARAM_OUT_VNDCFG_FUNC,///< ISF_OUTx: . #VDOPRC_OFUNC_VND_XXX
	VDOPRC_PARAM_OUT_PRE_CROP, ///< ISF_OUTx: set ime pre-crop before scale
	VDOPRC_PARAM_AI_CB, ///< ISF_CTRL: set callback
	VDOPRC_PARAM_ISP_AI_START, ///< ISF_CTRL: set isp ai mapping start
	VDOPRC_PARAM_ISP_AI_STOP, ///< ISF_CTRL: set isp ai mapping stop
	VDOPRC_PARAM_ISP_CB, ///< ISF_CTRL: get ai to isp callback
	VDOPRC_PARAM_SPLIT_INFO, ///< ISF_OUTx: split information, for VPE only
	VDOPRC_PARAM_THERMAL_INFO,     ///< ISF_CTRL: thermal mode
	VDOPRC_PARAM_LCA_FUNC, ///< ISF_CTRL: enable or disable LCA function
	VDOPRC_PARAM_SYS_INFO,  ///< ISF_CTRL: sys info
	VDOPRC_PARAM_ISP_AI_EFFECT, ///< ISF_CTRL: set isp effect and ai model mapping
	VDOPRC_PARAM_ISP_AI_POSTPROC, ///< ISF_CTRL: notify that the posterior pipe (ex. pre -> *rawlite) had enable ai process. this info must set by VPROC because posterior pipe cannot inform prior pipe directly
	VDOPRC_PARAM_ISP_BIND_INFO, ///< ISF_CTRL: handle index in current isp_id group
	VDOPRC_PARAM_FBOOT_BUFFING,
...
#define VDOPRC_PIPE_VPE_LITE    0x000000F3 //do YUV-domain scale process by VPE LITE engine
#define VDOPRC_PIPE_VSP         0x000000F4 //
#define VDOPRC_PIPE_DRE         0x000000F5 //
...
#define VDOPRC_PIPE_RAWALL_LITE     (VDOPRC_PIPE_NOISE|VDOPRC_PIPE_WARP|VDOPRC_PIPE_COLOR|VDOPRC_PIPE_SCALE|VDOPRC_PIPE_OUTPROC)
...
#define VDOPRC_PIPE_RAWCOLOR    (VDOPRC_PIPE_INPROC|VDOPRC_PIPE_EFFECT|VDOPRC_PIPE_NOISE|VDOPRC_PIPE_WARP|VDOPRC_PIPE_COLOR)
#define VDOPRC_PIPE_RAWCOLOR_LITE   (VDOPRC_PIPE_NOISE|VDOPRC_PIPE_WARP|VDOPRC_PIPE_COLOR)
#define VDOPRC_PIPE_ISE    		VDOPRC_PIPE_YUVAUX //do YUV-domain scale process (ISE)
#define VDOPRC_PIPE_PRE             (VDOPRC_PIPE_INPROC|VDOPRC_PIPE_EFFECT)
...
#define VDOPRC_FUNC_VA_SUBOUT 		(0x00000100|0x00001000)  // CTL_IPP_FUNC_IPE_VA_SUBOUT|CTL_IPP_FUNC_IME_VA_SUBOUT
#elif defined(_BSP_NS02201_) || defined(_BSP_NS02301_) || defined(_BSP_NS02302_)
#define VDOPRC_FUNC_VA_SUBOUT 		(0x00000100|0x00001000|0x00002000)  // CTL_IPP_FUNC_IPE_VA_SUBOUT|CTL_IPP_FUNC_IME_VA_SUBOUT|CTL_IPP_FUNC_IFE_VA_SUBOUT
#else
#define VDOPRC_FUNC_VA_SUBOUT 		0x00000100
#endif
#define VDOPRC_FUNC_DIRECT_SCALEUP 	0x00000800 // CTL_IPP_FUNC_DIRECT_SCL_UP
//osg effect
#if defined(_BSP_NA51102_) || defined(_BSP_NS02201_) || defined(_BSP_NS02301_) || defined(_BSP_NS02302_)
#define VDOPRC_FUNC_DATASTAMP		0x00000000 //530 /690 not suport VDOPRC_FUNC_DATASTAMP
#else
...
#define VDOPRC_FUNC_LCA				0x00004000

//in~out distort correction
#if defined(_BSP_NS02201_) || defined(_BSP_NS02301_) || defined(_BSP_NS02302_)
#define VDOPRC_FUNC_GDC			0x00000000  //690 not support GDC
#else
#define VDOPRC_FUNC_GDC			0x00000400
#endif

#define VDOPRC_FUNC_PRE_VA_SUBOUT   0x00008000
#define VDOPRC_FUNC_BNR             0x00010000
#define VDOPRC_FUNC_BNR_STA         0x00020000
#define VDOPRC_FUNC_AIDED_MAP       0x00040000
#define VDOPRC_FUNC_FUSION_WEIGHT   0x00080000
...
typedef enum {
	VDOPRC_SPLIT_OFF,
	VDOPRC_SPLIT_LEFT_RIGHT,
	VDOPRC_SPLIT_UP_DOWN,
	VDOPRC_SPLIT_OP_MAX
} VDOPRC_SPLIT_OP;

typedef struct _VDOPRC_SPLIT_INFO
{
	VDOPRC_SPLIT_OP op;
...
extern ISF_UNIT isf_vdoprc16;
extern ISF_UNIT isf_vdoprc17;
extern ISF_UNIT isf_vdoprc18;
extern ISF_UNIT isf_vdoprc19;
