...
#include "ctl_pre_isp_539a.h"
#include "ctl_ife_isp_539a.h"
#include "ctl_ipe_isp_539a.h"
#include "ctl_ime_isp_539a.h"
...
#define CTL_IPP_ISP_PRE_VA_MAX_WINNUM	(8)
#define CTL_IPP_ISP_PRE_VA_INDEP_NUM	(5)
...
typedef enum {
	...
	CTL_IPP_ISP_FLOW_RAW_LITE,
	CTL_IPP_ISP_FLOW_RAWCOLOR_LITE,
	CTL_IPP_ISP_FLOW_DIRECT_PRE,
	CTL_IPP_ISP_FLOW_PRE_D2D,
...
/**
    PRE VA Window size ratio
*/
typedef struct {
	UINT32 ratio_base;
	USIZE winsz_ratio;											///< va window size of each window, max 1024x1024
	URECT indep_roi_ratio[CTL_IPP_ISP_PRE_VA_INDEP_NUM];		///< va indep-window size, max 1024x1024
} CTL_IPP_ISP_PRE_VA_WIN_SIZE_RATIO;
...
*/
typedef struct {
	UINT32 buf_lum[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g1_h[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g1_v[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g2_h[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g2_v[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g1_h_cnt[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g1_v_cnt[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g2_h_cnt[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
	UINT32 buf_g2_v_cnt[CTL_IPP_ISP_PRE_VA_MAX_WINNUM * CTL_IPP_ISP_PRE_VA_MAX_WINNUM];
} CTL_IPP_ISP_PRE_VA_RST;

/**
	PRE VA Independent window result
*/
typedef struct {
	UINT32 buf_lum[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g1_h[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g1_v[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g2_h[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g2_v[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g1_h_cnt[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g1_v_cnt[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g2_h_cnt[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
	UINT32 g2_v_cnt[CTL_IPP_ISP_PRE_VA_INDEP_NUM];
} CTL_IPP_ISP_PRE_VA_INDEP_RST;
...
typedef struct {
	...
	CTL_IPP_FLIP_TYPE flip;	// input image flip
...
typedef struct {
	...
	UINT32 version;										///< used to check ai / isp version matched
	UINT32 reserved_param_size;							///< size (bytes) of reserved param, must less than CTL_IPP_ISP_AIISP_RSV_PARAM_MAX
	ULONG reserved_param_viraddr;						///< virtual addr of reserved param, space size must less than UINT8[CTL_IPP_ISP_AIISP_RSV_PARAM_MAX]
} CTL_IPP_ISP_AIISP_PARAM;

/**
	aiisp internal info
*/
typedef struct {
	CTL_IPP_ISP_AIISP_PARAM params[CTL_IPP_AI_FILT_MAX];
	UINT8 update[CTL_IPP_AI_FILT_MAX];
	UINT8 reserved_param_data[CTL_IPP_AI_FILT_MAX][CTL_IPP_ISP_AIISP_RSV_PARAM_MAX];
} CTL_IPP_ISP_INT_AIISP_PARAM;

typedef enum {
	CTL_IPP_ISP_ITEM_FUNC_EN = 0,		///< [GET],		data_type: ISP_FUNC_EN
	CTL_IPP_ISP_ITEM_IOSIZE,			///< [GET], 	data_type: CTL_IPP_ISP_IOSIZE
	CTL_IPP_ISP_ITEM_PRE_IQ_PARAM,		///< [SET], 	data_type: CTL_PRE_ISP_IQ_ALL
	...
	CTL_IPP_ISP_ITEM_BNR_STA,			///< [GET],		data_type: CTL_IPP_ISP_BNR_STA
	CTL_IPP_ISP_ITEM_PRE_VA_WIN_SIZE,	///< [SET/GET], data_type: CTL_IPP_ISP_VA_WIN_SIZE_RATIO
	CTL_IPP_ISP_ITEM_PRE_VA_RST,		///< [GET], 	data_type: CTL_IPP_ISP_PRE_VA_RST
	CTL_IPP_ISP_ITEM_PRE_VA_INDEP_RST,	///< [GET], 	data_type: CTL_IPP_ISP_PRE_VA_INDEP_RST
	...
	CTL_IPP_ISP_ITEM_LOW_PWR_INFO,		///< [SET],		data_type: CTL_IPP_ISP_LOW_PWR_INFO
	CTL_IPP_ISP_ITEM_AIISP_PARAM,		///< [SET],		data_type: CTL_IPP_ISP_AIISP_PARAM
...
*/
typedef struct {
	UINT8 flow; // CTL_IPP_ISP_FLOW_TYPE
	UINT8 bind_num;
	UINT8 bind_id;
	ULONG reserved[31];
} CTL_IPP_ISP_CB_PARAM;
STATIC_ASSERT(sizeof(CTL_IPP_ISP_CB_PARAM) <= 0x100);
