...
typedef struct {
	...
	//539A add
	UINT8 edge_detect_en;							///< LCA edge detection
	UINT8 ref_center_en;							///< filter reference center
	UINT8 motion_status_en;						///< motion status enable
...
typedef struct {
	...
	UINT8 proc_location;						///< DBCS processing location
...
typedef struct {
	...
	UINT8 motion_en;								///< 539a add switch MATF
...
typedef struct {
	...
	//539A add
	UINT8 tf_y_set_en;  ///< spatial/temporal filter bypass or not
	UINT8 tf_c_set_en;
	...
	//539A add
	UINT8 dc_ratio0;    ///< nr pre-filtering dc ratio
	UINT8 dc_ratio1;
