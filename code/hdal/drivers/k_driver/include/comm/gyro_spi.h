...
extern ER spi_setConfig64(SPI_ID spiID, SPI_CONFIG_ID configID, ULONG configContext);
...
void __iomem* spi3_getRegAddr(void);
void spi_platform_init(SPI_LINUX_ACT_CB act_cb);
void spi_platform_uninit(void);
void spi3_isr(void);

extern void spi_setGyroBufAddr(SPI_ID spiID, SPI_CONFIG_ID configID, ULONG addr);

// Engineer usage
extern ULONG spi_getGyroStatus(SPI_ID spiID);
extern void spi_clrGyroStatus(SPI_ID spiID, ULONG uiSts);
extern ER spi_getGyroFifoCnt(SPI_ID spiID, ULONG *puiCnt);
extern ER spi_readGyroFifo(SPI_ID spiID, ULONG uiCnt, ULONG *pBuf);
//extern void spiTest_setPktDelay(SPI_ID spiID, ULONG uiDelay);
