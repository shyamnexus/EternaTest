...
typedef struct {
	ULONG  bs_start_addr;                               ///< [w] output bit-stream start address if enc space not enough
	ULONG  bs_addr_1;                                   ///< [w] 1st output bit-stream address
	UINT32 bs_size_1;                                   ///< [w] 1st output bit-stream size
	ULONG  bs_end_addr;                                 ///< [w] output bit-stream end address
	UINT32 interrupt;
	UINT32 timestamp;
} AUDCAP_BUILTIN_PARAM;

typedef struct {
	ULONG  addr;    							//BS buffer starting address
...
typedef void (*AUDCAP_BUILTIN_TRIGGER_CB)(ULONG, BOOL);
...
extern UINT32 audcap_builtin_get_param(UINT32 param, ULONG *value);
extern UINT32 audcap_builtin_set_param(UINT32 param, ULONG *value);
