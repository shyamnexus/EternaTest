...
#define KDRV_IPP_TOTAL_NUM  ( KDRV_IPP_CHIP_NUM * KDRV_IPP_ENG_NUM )
...
INT32 kdrv_ipp_open(void* node);
INT32 kdrv_ipp_close(void *p_node);

INT32 kdrv_ipp_set(void* node, UINT32 param_id, void *p_data);
INT32 kdrv_ipp_get(void* node, UINT32 param_id, void *p_data);

INT32 kdrv_ipp_trigger(void* node,  void *p_param, KDRV_CALLBACK_FUNC *p_cb_func, VOID *p_user_data);
INT32 kdrv_ipp_direct_drop_old_job(void* p_node, ULONG last_addr, ULONG job_owner);
...
INT32 kdrv_ipp_create_node(void** hdl_node, KDRV_IPP_CREATE_NODE_PARAM* p_param);
