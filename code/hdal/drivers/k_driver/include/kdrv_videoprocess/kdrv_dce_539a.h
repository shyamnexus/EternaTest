...
#define WDR_INPUT_YV_BLD_NUM    (9)
#define WDR_NEQ_TABLE_IDX_NUM   (32)
#define WDR_NEQ_TABLE_VAL_NUM   (65)
#define WDR_NEQ_TABLE_L_NUM     (65)
#define WDR_NEQ_TABLE_R_NUM     (17)
#define WDR_FBC_TH_NUM          (6)
...
typedef enum{
	KDRV_DCE_WDR_NORMAL_MODE = 0,
	KDRV_DCE_WDR_ANTI_HALO_MODE = 1,
}KDRV_DCE_WDR_MODE;

typedef enum{
	KDRV_DCE_ANTI_HALO_BLENDING = 0,
	KDRV_DCE_ANTI_HALO_ADAPTIVE = 1,
}KDRV_DCE_ANTI_HALO_OPT;
/**
    channel information
*/
...
typedef struct _KDRV_DCE_FOV_PARAM {
	...
	UINT8 fov_bnd_r;                   ///< Replace value, range [0~255]
	UINT8 fov_bnd_g;                   ///< Replace value, range [0~255]
	UINT8 fov_bnd_b;                   ///< Replace value, range [0~255]
} KDRV_DCE_FOV_PARAM;
...
*/
typedef struct {
	UINT16 left_table[WDR_NEQ_TABLE_L_NUM];     ///< non equatable table value [0~4095]
	UINT16 right_table[WDR_NEQ_TABLE_R_NUM];    ///< non equatable table value [0~4095]
} KDRV_WDR_NEQ_TABLE;
...
typedef struct {
	...
	//UINT8 bld_wt;                                 ///< blending of Y and localmax [0~8],0: Y, 8: local max //removed from 530
	UINT8 blend_lut[WDR_INPUT_BLD_NUM];             ///< blending of Y and raw [0~255], 0: Y, 255: raw
	UINT8 in_yv_blend_lut[WDR_INPUT_YV_BLD_NUM];    ///< lut of YV blending [0~63]
...
typedef struct {
	...
	KDRV_DCE_WDR_MODE      wdr_mode;           ///< wdr mode, 0:Normal mode, 1:Anti-halo mode
	KDRV_DCE_ANTI_HALO_OPT wdr_anti_halo_opt;  ///< wdr anti halo option, 0:blending, 1:adaptive
...
/**
    DCE structure - WDR FBC
*/
typedef struct {
	BOOL    fbc_en;//forward brigness compensation  ///<wdr fbc enable, 0:disable, 1:enable
	UINT8   fbc_ratio;                              ///<wdr fbc ratio [0~255]
	UINT16  fbc_th[WDR_FBC_TH_NUM];                 ///<wdr th [0~8191]
} KDRV_DEC_ENG_WDR_FBC;
...
typedef struct _KDRV_DCE_WDR_PARAM {
	...
	KDRV_DEC_ENG_WDR_FBC      fbc;              ///< wdr fbc
} KDRV_DCE_WDR_PARAM;
...
typedef struct _KDRV_DCE_CFA_SUBOUT_PARAM {
	...
	KDRV_DCE_BUF_ADDR  cfa_addr;                     ///< cfa subout addr
...
#define KDRV_DCE_IMG_MAX_STP_NUM	24
#define KDRV_DCE_STP_SET_MAX_NUM	24
...
typedef enum {
	...
	KDRV_DCE_INTERRUPT_2_PIX_FAR = 0x00000020,   // enable interrupt: DCE 2 pixel too far error
	...
	KDRV_DCE_INTERRUPT_ALL       = 0x00001FFF,   // enable interrupt: all
} KDRV_DCE_INTERRUPT;
...
typedef struct {
	...
	UINT32 inte_en;
	/* wdr para */
	KDRV_DCE_BUF_ADDR sub_in_addr;
	UINT8 sub_out_en;
	KDRV_DCE_BUF_ADDR sub_out_addr;
} KDRV_DCE_IO_CFG;
...
void kdrv_dce_dump_cfg_log(KDRV_DCE_JOB_CFG *p_cfg);
void kdrv_dce_dump_register(void);
void kdrv_dce_dump_register_log(void);
