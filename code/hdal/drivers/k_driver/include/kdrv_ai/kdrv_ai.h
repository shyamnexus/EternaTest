...
#define KDRV_AI_FCD_VLC_EN              ((UINT32)0x00000001)      ///< compression variable length encoding enable
#define KDRV_AI_FCD_QUANTI_EN           ((UINT32)0x00000002)      ///< compression quantization enable
#define KDRV_AI_FCD_SPARSE_EN           ((UINT32)0x00000004)      ///< compression sparse encoding enable
#define KDRV_AI_FCD_QUANTI_KMEANS_EN    ((UINT32)0x00000008)      ///< compression quantization kmeans enable
#define KDRV_AI_FCD_ALL_EN              ((UINT32)0x0000000f)      ///< compression all function enable

#define KDRV_AI_NEURAL_DECONV_EN        ((UINT32)0x00000001)      ///< neural network deconvolution enable
#define KDRV_AI_NEURAL_CONV_EN          ((UINT32)0x00000002)      ///< neural network convolution enable
#define KDRV_AI_NEURAL_NORM_EN          ((UINT32)0x00000004)      ///< neural network normalized enable
#define KDRV_AI_NEURAL_ELT_EN           ((UINT32)0x00000008)      ///< neural network eltwise enable
#define KDRV_AI_NEURAL_ACT_EN           ((UINT32)0x00000010)      ///< neural network activation enable
#define KDRV_AI_NEURAL_POOL_EN          ((UINT32)0x00000020)      ///< neural network pooling enable
#define KDRV_AI_NEURAL_PREACT_EN        ((UINT32)0x00000040)      ///< neural network pre-act enable
//#define KDRV_AI_NEURAL_ALL_EN           0x0000003f      ///< neural network all function enable
#define KDRV_AI_NEURAL_FUNC_CNT         ((UINT32)0x7)             ///< number of network function

#define KDRV_AI_FC_FULLY_EN             ((UINT32)0x00000001)      ///< fc fully connected enable
#define KDRV_AI_FC_ACT_EN               ((UINT32)0x00000002)      ///< fc activation enable
//#define KDRV_AI_FC_ALL_EN               0x00000003      ///< fc all function enable
#define KDRV_AI_FC_FUNC_CNT             ((UINT32)0x2)             ///< number of fc function


#define KDRV_AI_PREPROC_YUV2RGB_EN      ((UINT32)0x00000001)      ///< preprocessing from YUV to RGB enable
#define KDRV_AI_PREPROC_SUB_EN          ((UINT32)0x00000002)      ///< preprocessing mean subtraction enable
#define KDRV_AI_PREPROC_PAD_EN          ((UINT32)0x00000004)      ///< preprocessing padding enable
#define KDRV_AI_PREPROC_ROT_EN          ((UINT32)0x00000008)      ///< preprocessing rotate enable
#define KDRV_AI_PREPROC_FUNC_CNT        ((UINT32)0x4)
...
typedef struct _KDRV_AI_APP_HEAD {
	...
	uintptr_t parm_addr;            ///< model parameter starting address (pa)
	UINT32 parm_size;               ///< model parameter size
	uintptr_t stripe_head_addr;     ///< next head of stripe engine
} KDRV_AI_APP_HEAD;
//@}
...
typedef struct _KDRV_AI_FCD_KERPARM {
	...
	uintptr_t quanti_kmeans_addr;   ///< input quanti kmeans address
...
typedef struct _KDRV_AI_CONV_KERPARM {
	uintptr_t weight_addr;                      ///< input weight address
	uintptr_t bias_addr;                        ///< input bias address
...
typedef struct _KDRV_AI_BN_SCALE_KERPARM {
	uintptr_t bn_scale_addr;                    ///< packed with conv bias_addr
...
typedef struct _KDRV_AI_ELT_KERPARM {
	...
	uintptr_t addr;                             ///< eltwise input address
...
typedef struct _KDRV_AI_ROIPOOL_KERPARM {
	...
	uintptr_t roi_addr;                         ///< input roi address (top-left_x0, top-left_y0, bottom-right_x0, bottom-right_y0, ...)
...
typedef struct _KDRV_AI_SVM_KERPARM {
	...
	uintptr_t sv_addr;                          ///< input sv address
	uintptr_t alpha_addr;                       ///< input alpha address
...
typedef struct _KDRV_AI_FC_KERPARM {
	uintptr_t weight_addr;                      ///< input weighting address
	uintptr_t bias_addr;                        ///< input bias address
...
typedef struct _KDRV_AI_NEURAL_PARM {
	...
	uintptr_t out0_addr;                        ///< output0 address
	uintptr_t out1_addr;                        ///< output1 address
	uintptr_t in_interm_addr;                   ///< input intermediate address from convolution
	uintptr_t out_interm_addr;                  ///< output intermediate address from convolution
	uintptr_t tmp_buf_addr;                     ///< temporarily buffer address for intermediate output
...
typedef struct _KDRV_AI_FC_PARM {
	...
	uintptr_t in_addr;                          ///< input address
	uintptr_t out_addr;                         ///< output address
	uintptr_t in_interm_addr;                   ///< input intermediate address from inner product
	uintptr_t out_interm_addr;                  ///< output intermediate address from inner product
	INT8 in_interm_dma_en;                      ///< enable/disable input intermediate to dram
	INT8 out_interm_dma_en;                     ///< enable/disable ouput intermediate to dram
...
typedef struct _KDRV_AI_ANCHOR_PARM {
	...
	KDRV_AI_SIZE size;                          ///< input size
	uintptr_t in_addr;                          ///< input address
    uintptr_t w_addr;                           ///< weight address
    uintptr_t b_addr;                           ///< bias address
    uintptr_t tbl_addr;                         ///< exp table address
	uintptr_t out_addr;                         ///< output address
...
typedef struct _KDRV_AI_FC_INFO {
	uintptr_t input_parm_pa;
	uintptr_t input_parm_va;
	UINT32 input_parm_size;
	uintptr_t eng_parm_pa;
	uintptr_t eng_parm_va;
