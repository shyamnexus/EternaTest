...
typedef struct ETH_RX_DESC {
	...
	UINT32 vDummy[8];               // align to 64 byte here
...
// Exported functions
//
extern void ethDesc_initQueue(ETH_ID id,
							  ETHMAC_DMA_DESCRIPT_MODE_ENUM txQueueMode,
...
extern BOOL ethDesc_isTxFull(ETH_ID id);
extern BOOL ethDesc_isTxEmpty(ETH_ID id);
extern ER   ethDesc_insertTxFrame(ETH_ID id, ETH_FRAME_HEAD *pFrameHead, ETH_PAYLOAD_ENUM type, UINT32 uiLen, UINT8 *pData);
extern UINT32 ethDesc_getLastTxDescAddr(ETH_ID id);
extern ER   ethDesc_removeTxFrame(ETH_ID id);
extern ER   ethDesc_retrieveRxFrame(ETH_ID id, UINT32 uiBufSize, UINT32 *puiLen, UINT8 *pData);
