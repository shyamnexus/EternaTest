...
typedef struct _H264DEC_INIT{
    ...
    uintptr_t uiDecBufAddr;        ///< H.264 decoder internal buffer starting address
    UINT32 uiDecBufSize;        ///< H.264 decoder internal buffer size
    uintptr_t uiH264DescAddr;      ///< H.264 description data starting address
	...
	BOOL bFBCEn;				/// H.264 decoder FBC enable
}H264DEC_INIT, *PH264DEC_INIT;

typedef struct _H264DEC_INFO{
    uintptr_t uiSrcYAddr;     ///< Output (decoded) source image Y address
    uintptr_t uiSrcUVAddr;    ///< Output (decoded) source image UV address
    uintptr_t uiBSAddr;       ///< Input bit-stream address
...
typedef struct _H264DecMemInfo_{
	uintptr_t uiHdrBsAddr;		///< [input]  : decode sps/pps heeader biststream addr
...
extern INT32 h264Dec_setNextBsBuf(H26XDEC_VAR *pH264DecVar, uintptr_t uiBsAddr, UINT32 uiBsSize, UINT32 uiTotalBsSize);
...
extern uintptr_t h264Dec_getResYAddr(H26XDEC_VAR *pH264DecVar);
extern uintptr_t h264Dec_getResYAddr2(H26XDEC_VAR *pH264DecVar);
