...
#define H264ENC_NAL_MAXSIZE               (16384/16)
...
typedef struct _rrc_pic_t_ {
	...
	// 538 does not use, CWTSAO
	unsigned int	planned_stop_tile[5];
	unsigned int	planned_top_tile[5];
	unsigned int	planned_bot_tile[5];
	unsigned int	frm_coeff_tile[5];
	unsigned int	frm_cost_lsb_tile[5];
	unsigned int	frm_cost_msb_tile[5];
	unsigned int	frm_cmpx_lsb_tile[5];
	unsigned int	frm_cmpx_msb_tile[5];
	unsigned int	step_tile;
} rrc_pic_t;
...
typedef struct _lpm_t_ {
	...
	unsigned char	codec_md_ira_lpm_en;
} lpm_t;
...
typedef struct _osg_win_t_ {
	...
	osg_qpmap_t		qpmap;
	osg_colorkey_t	key;
	osg_mosaic_t	mosaic;  //for 0812_pat after
} osg_win_t;
...
typedef struct _osg_t_ {
	...
	unsigned char	mosaic_bitmap_blk_sz[2];
} osg_t;
...
typedef struct _jnd_t_ {
	...
	unsigned char	la_new;
	unsigned char	y_th1;
	unsigned char	y_th2;
	unsigned char	cm_th1;
	unsigned char	cm_th2;
	unsigned char	u_th1;
	unsigned char	u_th2;
	unsigned char	v_th1;
	unsigned char	v_th2;
...
typedef struct _lambda_t_ {
	int	adaptlambda_en;
	unsigned int lambda_mode_delta_motion;
	unsigned int lambda_mode_delta_static;
	unsigned int lambda_mode_delta_grad;
	unsigned int lambda_mode_grad_th;
} lambda_t;
#else
typedef struct _lambda_t_ {
	int	adaptlambda_en;
...
typedef struct _eskip_t_ {
	...
	unsigned char	codec_md_skip_en;
} eskip_t;
...
typedef struct _codec_md_t_ {
	unsigned char	enable;
	unsigned char	mode;
	char			dqp[2];
	unsigned char	gain;
	unsigned char	frm_ofs;
	unsigned char	q_ofs;
	unsigned char	q_step;
} codec_md_t;

typedef struct _codec_mf_t_ {
	unsigned char	enable;
...
typedef struct _codec_mdmf_t_ {
	codec_md_t md;
	codec_mf_t mf;
} codec_mdmf_t;

typedef struct _partial_frm_skip_t_ {
	unsigned char	enable;
	unsigned char	left;
	unsigned char	top;
	unsigned char	right;
...
typedef struct _h264_pat_t_ {
	...
	unsigned char	rnd_bs_buf;
	unsigned char	rnd_tile_bs_buf;
	unsigned char	rnd_sram_en;

	unsigned int	slice_number;

	unsigned int	bsdma_buf_size;
	uintptr_t		bsdma_buf_addr;
	unsigned int	bsdma_buf_size2;
	uintptr_t		bsdma_buf_addr2;
	...
	unsigned int	slice_hdr_buf_vlc_size;
	uintptr_t		slice_hdr_buf_vlc_addr;
	unsigned int	slice_hdr_len_vlc[H264ENC_NAL_MAXSIZE];
	unsigned int	slice_len_vlc[H264ENC_NAL_MAXSIZE];
	...
	unsigned int	mbqp_rotate;
	unsigned int	mbqp_offset;

	unsigned int	performance_check;
	unsigned int	performance_osg_range;
	unsigned int	sha_report[8];	// sha return

	uintptr_t		tmp_wtmk_va_addr[8];
	unsigned char	wtmk_file_read_flg;
	...
	unsigned int last_slice_idx;	// slc_end_int need to avoid appearing in last 50mb for 538

	uintptr_t		codec_md_tmp_addr;
	unsigned int	cmp_codec_md;
} h264_pat_t;
...
BOOL emu_h264_chk_one_pic(h26x_ctrl_t *p_ctrl, h26x_job_t *p_job, UINT32 interrupt, unsigned int rec_out_en);
...
BOOL emu_h264_set_nxt_ll_slice(h26x_job_t *p_job);
BOOL emu_h264_compare_bs_ll(h26x_job_t *p_job, UINT32 slice_idx_acl);
