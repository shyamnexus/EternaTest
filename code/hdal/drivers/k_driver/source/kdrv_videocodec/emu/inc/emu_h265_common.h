...
#define H265_LINEOFFSET_MAX 8192
...
typedef struct _seq_t_265_{
	...
	unsigned char srcrot2src;

	unsigned char lambda_delta_en;
	unsigned char turbo_mode_en;
} seq_t_265;
...
typedef struct _pic_t_265_{
	...
	unsigned int add_next_frm_num;
	unsigned char srcrot2src_rot;
} pic_t_265;
...
typedef struct _h265_perf_t_{
	...
	unsigned int i_frm_num;
	unsigned int max_i_frm_no;
	unsigned int cycle_p_frm;
	unsigned int bslen_p_frm;
	unsigned int p_frm_num;
	unsigned int max_p_frm_no;
} h265_perf_t;
