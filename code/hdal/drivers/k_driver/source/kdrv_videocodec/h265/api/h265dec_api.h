...
typedef struct _H265DEC_INIT {
	...
	uintptr_t uiDecBufAddr;        ///< H.265 decoder internal buffer starting address
	UINT32 uiDecBufSize;        ///< H.265 decoder internal buffer size
	uintptr_t uiH265DescAddr;      ///< H.265 description data starting address
...
typedef struct _H265DecMemInfo_{
	uintptr_t uiHdrBsAddr;		///< [input]  : decode vps/sps/pps header biststream addr
...
extern INT32 h265Dec_setNextBsBuf(H26XDEC_VAR *pH265DecVar, uintptr_t uiBsAddr, UINT32 uiBsSize, UINT32 uiTotalBsSize);
extern INT32 h265Dec_IsDecodeFinish(void);
extern INT32 h265Dec_queryMemSize(H265DecMemInfo *pMemInfo);
extern uintptr_t h265Dec_getResYAddr(H26XDEC_VAR *pH265DecVar);
