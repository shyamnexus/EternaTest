...
int kdrv_vdocdc_emode_get_enc_info(VENC_CH_INFO *p_info)
{
	...
	memcpy(p_info->kdrv_h26x_ver, H26XE_VER_STR, sizeof(H26XE_VER_STR));
	p_info->kdrv_builtin_ver = h26xEnc_getVersion();
...
int kdrv_vdocdc_emode_read_cfg(char file_name[64])
{
	...
	if ((c_file = vos_file_open(file_name, O_RDONLY, 0)) == (VOS_FILE)(-1)) {
...
int kdrv_vdocdc_emode_write_cfg(char file_name[64])
{
	...
	if ((c_file = vos_file_open(file_name, O_CREAT | O_WRONLY | O_SYNC, 0)) == (VOS_FILE)(-1)) {
		DBG_ERR("open file (%s) error\r\n", file_name);
		ret = -1;
		goto exit;
	}
