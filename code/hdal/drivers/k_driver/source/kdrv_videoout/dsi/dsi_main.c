...
static int nvt_dsi_probe(struct platform_device *pdev)
{
	...
	if (top_reg_addr) {
		if (!(readl(top_reg_addr) & (0x1 << 7))) {
			if (pdrv_info->module_info.io_addr[0]) {
				writel(0x2, pdrv_info->module_info.io_addr[0] + 0x300);
				nvt_dbg(IND, "Disable MIPI TX when DSI_PROT_EN BST[7] is 0, set bypass [0x%llx = 0x%x]\n", pdrv_info->presource[0]->start + 0x300, readl(pdrv_info->module_info.io_addr[0] + 0x300));
			} else {
				nvt_dbg(ERR, "invalid io_addr[0]\n");
			}
		...
		nvt_dbg(ERR, "invalid top_reg_addr\n");
