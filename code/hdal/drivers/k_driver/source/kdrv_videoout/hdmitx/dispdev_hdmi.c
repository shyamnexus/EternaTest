...
//#include "rtos_na51102/timer.h"
//#include "rtos_na51102/delay.h"
#include "plat/pll.h"
...
#if DISPDEVIOUSED
DISPDEV_IOCTRL   p_hdmi_io_control;
#endif
...
#if DISPDEVIOUSED
static DISPDEVHDMI_CTRL dispdevhdmi_control = {dispdev_get_hdmi_dev_obj, hdmitx_set_config, hdmitx_get_edid_info, hdmitx_get_video_ability, hdmitx_get_edid_raw_data, hdmitx_check_hotplug};
#endif
...
HDMI_MODE        g_dispdev_hdmi_mode = HDMI_MODE_YCBCR444;
HDMI_VIDEOID     g_dispdev_hdmi_vid = HDMI_720X480P60;
HDMI_AUDIOFMT    g_dispdev_aud_fmt  = HDMI_AUDIO48KHZ;
HDMI_SRCFMT		g_dispdev_src_fmt  = HDMI_IDE_0;
...
static void dispdev_init_hdmi_display(void)
{
	...
	if(hdmi_ctrl_option[HDMICFG_OPTION_RGB_RANGE] == 0x1){ // limited range
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_y_low   = 0x10;
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_y_up    = 0xEB;
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_cb_low  = 0x10;
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_cb_up   = 0xEB;
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_cr_low  = 0x10;
		dev_io_ctrl.SEL.SET_OUT_LIMIT.ui_cr_up   = 0xEB;
		DBG_WRN("Set limited range RGB out\r\n");
	} else {
...
static BOOL dispdev_chg_hdmi_clk_freq(BOOL b_native)
{
	...
	if (pll_clk != NULL) {
		...
		*/
		...
		case HDMI_720X480P240_16X9: {   /* VID57 */
				g_dispdev_src_freq = HDMI_PLL_DIV(216000000,hdmi_timing_factor);
			}
			break;
		...
		*/
		...
		case HDMI_640X480P60: {         /* VID1 */
				g_dispdev_src_freq = HDMI_PLL_DIV(252000000,hdmi_timing_factor);
			}
			break;
		...
		*/
		...
		case HDMI_4096X2160P60:{        /* VID102 */
				g_dispdev_src_freq = HDMI_PLL_DIV(594000000,hdmi_timing_factor);
			}
			break;


		case HDMI_1024X768P60: {
				g_dispdev_src_freq = HDMI_PLL_DIV(260000000,hdmi_timing_factor);
			}
			break;

		case HDMI_1600X1200P60: {
				g_dispdev_src_freq = HDMI_PLL_DIV(324000000,hdmi_timing_factor);
			}
			break;
		case HDMI_2560X1440P60: {
				g_dispdev_src_freq = HDMI_PLL_DIV(241500000,hdmi_timing_factor);
			}
			break;
		...
		if(g_dispdev_src_freq != cur_freq){
			DBG_WRN("Change pll source freq from [%d] to [%d]\r\n",cur_freq ,g_dispdev_src_freq);
		}
	}
	...
	cur_freq              = pll_get_pll_freq(src_pll) / 1000000;
	...
	*/
	...
	case HDMI_720X480P240_16X9:     /* VID57 */
		...
		if ((cur_freq != HDMI_PLL216) && (src_pll != PLL_ID_1)) {
			dispdev_disable_ide();
			if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
				pll_set_pll(src_pll,      HDMI_PLL215P78);
			} else {
				pll_set_pll(src_pll,      HDMI_PLL216);
			}
			disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll + 1), pll_get_pll_freq(src_pll) / 1000000));
		}
		break;
#endif
	/*
	...
	*/
	...
	case HDMI_640X480P60:          /* VID1 */
		...
		if ((cur_freq != HDMI_PLL252) && (src_pll != PLL_ID_1)) {
			dispdev_disable_ide();
			if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
				pll_set_pll(src_pll,      HDMI_PLL251P17);
			} else {
				pll_set_pll(src_pll,      HDMI_PLL252);
			}
			disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll + 1), pll_get_pll_freq(src_pll) / 1000000));
		}
#endif
	...
	*/
	...
	case HDMI_4096X2160P60: {       /* VID102*/
			if ((cur_freq != HDMI_PLL594) && (src_pll != PLL_ID_1)) {
				dispdev_disable_ide();
				if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
					pll_set_pll(src_pll,      HDMI_PLL593P4);
				} else {
					pll_set_pll(src_pll,      HDMI_PLL594);
				}
				disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll), pll_get_pll_freq(src_pll) / 1000000));
			}
		}
		break;
	...
	case HDMI_1024X768P60: {
			if ((cur_freq != HDMI_PLL260) && (src_pll != PLL_ID_1)) {
				dispdev_disable_ide();
				if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
					pll_set_pll(src_pll,      HDMI_PLL259P74);
				} else {
					pll_set_pll(src_pll,      HDMI_PLL260);
				}
				disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll), pll_get_pll_freq(src_pll) / 1000000));
			}
		}
	...
	case HDMI_1600X1200P60: {
			if ((cur_freq != HDMI_PLL324) && (src_pll != PLL_ID_1)) {
				dispdev_disable_ide();
				if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
					pll_set_pll(src_pll,      HDMI_PLL323P67);
				} else {
					pll_set_pll(src_pll,      HDMI_PLL324);
				}
				disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll), pll_get_pll_freq(src_pll) / 1000000));
			}
		}
	...
	case HDMI_2560X1440P60: {
			if ((cur_freq != HDMI_PLL241) && (src_pll != PLL_ID_1)) {
				dispdev_disable_ide();
				if(hdmi_ctrl_option[HDMICFG_OPTION_TIMING_DIV_1P001] == 0x1){
					pll_set_pll(src_pll,      HDMI_PLL241P25);
				} else {
					pll_set_pll(src_pll,      HDMI_PLL241);
				}
				disphdmi_debug(("DEVHDMI: Chg PLL%d to(%d)Mhz\r\n", (src_pll), pll_get_pll_freq(src_pll) / 1000000));
			}
		}
...
static ER dispdev_open_hdmi(void)
{
	...
	if (hdmitx_is_opened() == FALSE) {
		...
		/* check hot plug when open done*/
		if(hdmi_hpd_status){
			hdmi_state |= HPG_CONNECT;
		} else {
			hdmi_state &= ~HPG_CONNECT;
		}
