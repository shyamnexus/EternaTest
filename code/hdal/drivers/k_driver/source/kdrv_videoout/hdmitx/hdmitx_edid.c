...
#ifdef __KERNEL__
extern HDMI_I2C_OBJ hdmi_i2c_obj[1];
#else
...
static UINT32 hdmitx_parse_edid_header(void)
{
	...
	if ((hdmitx_ddc_read_byte(0x00, 0x00) == 0x00) && (hdmitx_ddc_read_byte(0x00, 0x07) == 0x00)) {
		for (i = 1; i < 7; i++) {
			if (hdmitx_ddc_read_byte(0x00, i) != 0xff) {
				hdmitx_edidmsg(("hdmitx_Parse_EDIDHeader: Non-Legal EDID Header! FAIL!\r\n"));
				return 1;
			}
		}
	} else {
	...
	hdmitx_edidmsg(("hdmitx_Parse_EDIDHeader: Legal EDID Header! ManID=0x%04X\r\n", gui_manufacturer_id));
	return 0;
...
static ER hdmitx_ddc_read(UINT32 addr, UINT32 segm, UINT32 offset, UINT32 cnt)
{
	...
	unsigned char   buf0[1], buf1[1], *buf2;
	int msg_cnt = 0;
	int retry_cnt = 10;
	int i;

	hdmitx_edidmsg(("DDC-Read:  addr=0x%X  segm=0x%X  offset=0x%X  cnt=0x%X\r\n", addr, segm, offset, cnt));

	if (segm != 0) {
		buf0[0] = segm;

		msgs[msg_cnt].addr  = (0x60>>1);
		msgs[msg_cnt].flags = 0;//w
		msgs[msg_cnt].len   = 1;
		msgs[msg_cnt].buf   = buf0;
		msg_cnt++;
	}

	buf1[0] = offset;
	msgs[msg_cnt].addr  = (addr>>1);
	msgs[msg_cnt].flags = 0;//w
	msgs[msg_cnt].len   = 1;
	msgs[msg_cnt].buf   = buf1;
	msg_cnt++;

	buf2 = (unsigned char *)kzalloc(cnt, GFP_KERNEL);
	if (!buf2) {
		DBG_ERR("buf2 alloc failed(%d)\r\n", cnt);
	}

	msgs[msg_cnt].addr  = (addr>>1);
	msgs[msg_cnt].flags = 1;//r
	msgs[msg_cnt].len   = cnt;
	msgs[msg_cnt].buf   = buf2;
	msg_cnt++;

    do {
		retry_cnt--;
	    if (i2c_transfer(hdmitx_i2c_info->iic_adapter, msgs, msg_cnt) != msg_cnt) {
			DBG_ERR("%s, retry = [%d] \n",__FUNCTION__,10-retry_cnt);
			if (retry_cnt == 0) {
		...
		if (hdmitx_check_hotplug() != TRUE) {
			DBG_ERR("hdmi cable is unplug while %s. \n",__FUNCTION__);
			hdmi_edid_error = 1;
			return -1;
		}
	...
	if ((segm == 0) && (offset == 0) && (cnt >= 0x80)) {
		hdmi_ext_blk = buf2[0x7E];
	}

	for(i=0;i<cnt;i+=4){
		hdmitx_edidmsg(("DDC_%d = 0x%02X 0x%02X 0x%02X 0x%02X \r\n", i, buf2[i], buf2[i+1], buf2[i+2], buf2[i+3]));
	}
	...
	while (ui_rest) {
		...
		//hdmitx_debug(("DDC_%d = 0x%02X 0x%02X \r\n",(cnt-ui_rest),I2cByte[0].uiValue,I2cByte[1].uiValue));
...
#ifdef __KERNEL__
	struct i2c_msg  msgs[3];
	unsigned char   buf0[1], buf1[1], buf2[1];
	int msg_cnt = 0, trycnt=0;
...
retry:
	...
	if (trycnt == 1){
		int rt;
		struct i2c_msg i2c_msg_cfg;
		UINT8 i2c_msg_cfg_buf[2];

		//DBG_ERR("NVT_I2C_AUTO_BUCLEAR gogo\r\n");
		i2c_msg_cfg_buf[0] = NVT_I2C_AUTO_BUCLEAR;
		i2c_msg_cfg_buf[1] = 1; // 0: DISABLE, 1: ENABLE
		i2c_msg_cfg.addr = msgs->addr;
		i2c_msg_cfg.flags = NVT_I2C_SET_CFG_FLG;
		i2c_msg_cfg.len = 2;
		i2c_msg_cfg.buf = i2c_msg_cfg_buf;
		rt = i2c_transfer(hdmitx_i2c_info->iic_adapter, &i2c_msg_cfg, 1);
		if (rt != 1) {
				DBG_ERR(" i2c_transfer fail. \r\n");
				return -1;
		}
		//DBG_ERR("NVT_I2C_SET_RST gogo\r\n");
		i2c_msg_cfg_buf[0] = NVT_I2C_SET_RST;
		i2c_msg_cfg_buf[1] = 1; // 0: DISABLE, 1: ENABLE
		i2c_msg_cfg.addr = msgs->addr;
		i2c_msg_cfg.flags = NVT_I2C_SET_CFG_FLG;
		i2c_msg_cfg.len = 2;
	...
	if (HDMI_RST_I2C) {
		...
		//DBG_ERR("NVT_I2C_SET_CTL_EN_IMM gogo\r\n");
		i2c_msg_cfg_buf[0] = NVT_I2C_SET_CTL_EN_IMM;
		i2c_msg_cfg_buf[1] = 0; // 0: DISABLE, 1: ENABLE
		i2c_msg_cfg.addr = msgs->addr;
		i2c_msg_cfg.flags = NVT_I2C_SET_CFG_FLG;
		i2c_msg_cfg.len = 2;
		i2c_msg_cfg.buf = i2c_msg_cfg_buf;
	...
	buf1[0] = offset;
	msgs[msg_cnt].addr  = (HDMI_DDCSLAVE_ADDR>>1);
	msgs[msg_cnt].flags = 0;//w
	msgs[msg_cnt].len   = 1;
	msgs[msg_cnt].buf   = buf1;
	msg_cnt++;
	...
	if (i2c_transfer(hdmitx_i2c_info->iic_adapter, msgs, msg_cnt) != msg_cnt) {
		DBG_ERR("%s fail: i2c_transfer not OK, trycnt %d\n", __FUNCTION__, trycnt);
		if(trycnt < 6) {
			goto retry;
		}
	}
	*pui_data = (UINT32)buf2[0];
	return E_OK;
#else
...
ER hdmitx_scdc_write(UINT32 offset, UINT32 data)
{
	...
	msgs[msg_cnt].addr  = (HDMI2_SCDC_SLAVEADDR>>1);
	msgs[msg_cnt].flags = 0;//w
	msgs[msg_cnt].len   = 2;
	msgs[msg_cnt].buf   = buf1;
	msg_cnt++;
	...
	if (hdmi_i2c_obj[hdmi_i2c_select].transmit(&I2cData) != I2C_STS_OK) {
		DBG_ERR("HDMI i2c_11 transmit err 1!\r\n");
		hdmi_i2c_obj[hdmi_i2c_select].unlock(hdmi_i2c_ses);
		return E_SYS;
	}
...
retry:
	...
	*pdata = (UINT32)buf2[0];
	...
	if (hdmi_i2c_obj[hdmi_i2c_select].transmit(&I2cData) != I2C_STS_OK) {
		DBG_ERR("i2c transmit err scdc 1!\r\n");
		hdmi_i2c_obj[hdmi_i2c_select].unlock(hdmi_i2c_ses);
		return E_SYS;
	}
	...
	if (hdmi_i2c_obj[hdmi_i2c_select].receive(&I2cData) != I2C_STS_OK) {
		DBG_ERR("i2c transmit err scdc 2!\r\n");
		hdmi_i2c_obj[hdmi_i2c_select].unlock(hdmi_i2c_ses);
		return E_SYS;
	}
...
void hdmitx_init_ddc(BOOL b_ddc_en)
{
	...
	if (b_ddc_en && (!skip_edid)) {
		...
		temp = hdmitx_ddc_read_byte2(0x00, 0x7E, &ext_blk); 
		if (temp != E_OK) {
...
void hdmitx_get_video_ability(UINT32 *p_len, PHDMI_VDOABI vdo_ability)
{
	...
	*p_len = (UINT32) g_svd_vid[HDMITX_EDIDNUM];
	...
	hdmitx_edidmsg(("hdmitx_get_video_ability: Length=%d  :\r\n", *p_len));
	for (i = 0; i < *p_len; i++) {
...
void hdmitx_get_edid_raw_data(UINT32 *len, UINT8 *data)
{
	int i;
	...
	for(i=0; i<(int)*len; i++){
		*(data+i) = hdmitx_ddc_read_byte(0x00,0x00+i);
	}
}
...
#ifdef __KERNEL__
EXPORT_SYMBOL(hdmitx_get_edid_info);
EXPORT_SYMBOL(hdmitx_verify_video_id);
EXPORT_SYMBOL(hdmitx_dump_info);
EXPORT_SYMBOL(hdmitx_get_video_ability);
EXPORT_SYMBOL(hdmitx_get_edid_raw_data);
EXPORT_SYMBOL(hdmitx_check_ddc);
#endif
