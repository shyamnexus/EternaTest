#ifndef __HDMITX_DBG_H_
#define __HDMITX_DBG_H_

#if defined(__LINUX)
#define vk_pr_warn printk
...
extern unsigned int hdmitx_debug_level;
#define nvt_dbg(level, fmt, args...)                \
	do {                                               \
if (unlikely(NVT_DBG_##level <= hdmitx_debug_level))    \
			vk_pr_warn("%s:" fmt, "hdmitx", ##args);   \
	} while (0)
#else
#define nvt_dbg(fmt, ...)
#endif
...
//#define HDMITX_API_DBG
#ifdef HDMITX_API_DBG
#define hdmitx_api  printk
#else
#define hdmitx_api(fmt, ...)
#endif
