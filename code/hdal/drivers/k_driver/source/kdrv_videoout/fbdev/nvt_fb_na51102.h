 ...
 *
 *  na51102 LCD Framebuffer Driver
 *
...
enum {
	NVT_PANDISPLAY_TYPE_LINUX = 0x00,                  ///< typical linux pan display behavior
	NVT_PANDISPLAY_TYPE_RLE   = 0x02,                  ///< every pand display action triggers rle encoding to a free pingpong buffer
	NVT_PANDISPLAY_TYPE_COPY  = 0x04,                  ///< every pand display action copies user buffer to a free pingpong buffer
};
...
struct nvt_fb_par {
	...
	int                     free_pingpong_idx;
	uintptr_t               pandisplay_buffer[2];
	unsigned int            pandisplay_type;
	int                     pandisplay_w;
	int                     pandisplay_h;
	int                     single_pingpong_size;
	unsigned int            format;
	int                     pandisplay_buffer_cnt;
	int                     pandisplay_copy_size;
};
