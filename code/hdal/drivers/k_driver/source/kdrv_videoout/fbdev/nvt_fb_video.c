...
int nvt_fb_video_probe(struct fb_info *fbinfo, unsigned int ide_id, unsigned int layer)
{
	...
	ret = display_obj_set_video2_addr(ide_id, (uintptr_t)fbinfo->screen_base, 1);
	if (ret) {
		printk(KERN_ERR "display_obj_set_video2_addr(%d,%lx) fail\n", ide_id, fbinfo->fix.smem_start);
		return ret;
	}
