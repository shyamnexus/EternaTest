		...
		if (cell > 0) {
			if (be32_to_cpu(cell[0]) > 0) {
                if (source == 0) {
                    ...
                    if (pll_set_driver_pll(PLL_ID_6, ((be32_to_cpu(cell[0])/1000000)*131072)/12)) {
                        DBG_ERR("current_rate %d error, using default\r\n", (int) be32_to_cpu(cell[0]));
                        return 1;
                    }
                } else if (source == 2) {
                    if (pll_set_driver_pll(PLL_ID_4, ((be32_to_cpu(cell[0])/1000000)*131072)/12)) {
                        DBG_ERR("current_rate %d error, using default\r\n", (int) be32_to_cpu(cell[0]));
                        return 1;
                    }
                } else if (source == 3) {
...
uintptr_t display_obj_platform_va2pa(uintptr_t addr)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
...
int display_obj_set_video2_addr(DISP_ID id, uintptr_t addr, bool load)
{
	PDISP_OBJ       p_disp_obj;
	DISPLAYER_PARAM disp_lyr;
	DISPBUFFORMAT format;
	...
	disp_lyr.SEL.SET_VDOBUFADDR.ui_addr_y0 = addr;
	disp_lyr.SEL.SET_VDOBUFADDR.ui_addr_cb0 = (addr + w*h);
	disp_lyr.SEL.SET_VDOBUFADDR.ui_addr_cr0 = (addr + w*h);
...
int display_obj_get_video2_param(DISP_ID id, struct fb_var_screeninfo *var, struct fb_fix_screeninfo *fix)
{
	...
	if (var != NULL) {
		...
		var->right_margin = h_total - h_valid_ed;
		var->hsync_len = h_sync + 1;
		var->left_margin = h_valid_st - var->hsync_len;
		...
		var->lower_margin = v_total - v_valid_odded;
		var->vsync_len = v_sync + 1;
		var->upper_margin = v_valid_oddst - var->vsync_len;

		DBG_IND("upper_margin: %d, lower_margin: %d, vsync_len: %d\r\n", (int)var->upper_margin, (int)var->lower_margin, (int)var->vsync_len);

		pxclk = ide_platform_get_iffreq((IDE_ID)id);
		var->pixclock = 1000000000000000LLU/pxclk;
	
		DBG_IND("pixclock: %d ps\r\n", (int)var->pixclock);
	}
...
EXPORT_SYMBOL(display_obj_set_video2_addr);
