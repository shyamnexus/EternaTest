...
int nvt_dispobj_drv_lcd_open(PMODULE_INFO pmodule_info, unsigned int open)
{
	...
	top_addr = ioremap(0xf0010000, 0x200);
#else
...
void fill_video_block422p(unsigned int width, unsigned int height, unsigned int loff, UINT64 y_addr, UINT64 cb_addr)
{
...
void fill_osd_block(unsigned int width, unsigned int height, unsigned int loff, UINT64 rgb_addr)
{
