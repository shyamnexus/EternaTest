...
//=============================================================================
#ifdef DEBUG
unsigned int dsiphy_debug_level = NVT_DBG_FUNC; //NVT_DBG_WRN
module_param_named(dsiphy_debug_level, dsiphy_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(dsiphy_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id dsiphy_match_table[] = {
...
//=============================================================================
static int nvt_dsiphy_open(struct inode *inode, struct file *file);
static int nvt_dsiphy_release(struct inode *inode, struct file *file);
static long nvt_dsiphy_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_dsiphy_probe(struct platform_device *pdev);
static int nvt_dsiphy_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_dsiphy_resume(struct platform_device *pdev);
static int nvt_dsiphy_remove(struct platform_device *pdev);
int __init nvt_dsiphy_module_init(void);
void __exit nvt_dsiphy_module_exit(void);
...
static int nvt_dsiphy_open(struct inode *inode, struct file *file)
{
	DSIPHY_DRV_INFO *pdrv_info;

	nvt_dbg(IND, "%s\n", __func__);

	pdrv_info = container_of(inode->i_cdev, DSIPHY_DRV_INFO, cdev);

	pdrv_info = container_of(inode->i_cdev, DSIPHY_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_dsiphy_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_dsiphy_release(struct inode *inode, struct file *file)
{
	...
	nvt_dsiphy_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
	return 0;
}
...
static long nvt_dsiphy_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	return nvt_dsiphy_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_dsiphy_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_dsiphy_open,
	.release = nvt_dsiphy_release,
	.unlocked_ioctl = nvt_dsiphy_ioctl,
	.llseek  = no_llseek,
};

static int nvt_dsiphy_probe(struct platform_device *pdev)
{
	DSIPHY_DRV_INFO *pdrv_info;//info;
	...
	match = of_match_device(dsiphy_match_table, &pdev->dev);
	if (!match) {
		printk("Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(DSIPHY_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
	...
	//DBG_FUNC("pdrv_info->presource[0] = 0x%lx   \r\n",(ULONG) pdrv_info->presource[0]);
	for (ucloop = 0 ; ucloop < MODULE_REG_NUM ; ucloop++) {
	...
	cdev_init(&pdrv_info->cdev, &nvt_dsiphy_fops);
	...
	ret = nvt_dsiphy_proc_init(pdrv_info);
	if (ret) {
	...
	ret = nvt_dsiphy_drv_init(&pdrv_info->module_info);
...
FAIL_DRV_INIT:
#if !defined(CONFIG_NVT_SMALL_HDAL)
	nvt_dsiphy_proc_remove(pdrv_info);
...
static int nvt_dsiphy_remove(struct platform_device *pdev)
{
	...
	nvt_dsiphy_drv_remove(&pdrv_info->module_info);
...
static int nvt_dsiphy_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_dsiphy_drv_suspend(&pdrv_info->module_info);
...
static int nvt_dsiphy_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_dsiphy_drv_resume(&pdrv_info->module_info);

	nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_dsiphy_driver = {
	.driver = {
		.name   = "nvt_dsiphy",
		.owner = THIS_MODULE,
		.of_match_table = dsiphy_match_table,
	},
	.probe      = nvt_dsiphy_probe,
	.remove     = nvt_dsiphy_remove,
	.suspend = nvt_dsiphy_suspend,
	.resume = nvt_dsiphy_resume
};
...
int __init nvt_dsiphy_module_init(void)
{
	...
	ret = platform_driver_register(&nvt_dsiphy_driver);
...
void __exit nvt_dsiphy_module_exit(void)
{
	...
	platform_driver_unregister(&nvt_dsiphy_driver);

}

module_init(nvt_dsiphy_module_init);
module_exit(nvt_dsiphy_module_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("dsiphy driver");
MODULE_LICENSE("GPL");
