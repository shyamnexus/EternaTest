...
#include "gpenc_drv.h"
#include "gpenc_reg.h"
#include "gpenc_main.h"
#include "gpenc_proc.h"
#include "gpenc_dbg.h"
...
//=============================================================================
#ifdef DEBUG
unsigned int gpenc_debug_level = NVT_DBG_IND;
module_param_named(gpenc_debug_level, gpenc_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(gpenc_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id gpenc_match_table[] = {
...
//=============================================================================
static int nvt_gpenc_open(struct inode *inode, struct file *file);
static int nvt_gpenc_release(struct inode *inode, struct file *file);
static long nvt_gpenc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_gpenc_probe(struct platform_device *pdev);
static int nvt_gpenc_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_gpenc_resume(struct platform_device *pdev);
static int nvt_gpenc_remove(struct platform_device *pdev);
int __init nvt_gpenc_module_init(void);
void __exit nvt_gpenc_module_exit(void);
...
static int nvt_gpenc_open(struct inode *inode, struct file *file)
{
	GPENC_DRV_INFO *pdrv_info;

	pdrv_info = container_of(inode->i_cdev, GPENC_DRV_INFO, cdev);

	pdrv_info = container_of(inode->i_cdev, GPENC_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_gpenc_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_gpenc_release(struct inode *inode, struct file *file)
{
	...
	pdrv_info = container_of(inode->i_cdev, GPENC_DRV_INFO, cdev);
	nvt_gpenc_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
	return 0;
}
...
{
	struct inode *inode;
	PGPENC_DRV_INFO pdrv;

	inode = file_inode(filp);
	pdrv = filp->private_data;

	return nvt_gpenc_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_gpenc_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_gpenc_open,
	.release = nvt_gpenc_release,
	.unlocked_ioctl = nvt_gpenc_ioctl,
	.llseek  = no_llseek,
};
...
static int nvt_gpenc_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(gpenc_match_table, &pdev->dev);
	if (!match) {
		nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(GPENC_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		if (ucloop == 0)
			...
			pdrv_info->module_info.pclk[ucloop] = clk_get(&pdev->dev, "2f0870000.gpe");
		//nvt_dbg(IND, "clk_dev = %d\r\n", pdrv_info->module_info.pclk[ucloop]);
	...
	cdev_init(&pdrv_info->cdev, &nvt_gpenc_fops);
	...
	ret = nvt_gpenc_proc_init(pdrv_info);
	if (ret) {
	...
	ret = nvt_gpenc_drv_init(&pdrv_info->module_info);
...
FAIL_DRV_INIT:
#if !defined(CONFIG_NVT_SMALL_HDAL)
	nvt_gpenc_proc_remove(pdrv_info);
...
static int nvt_gpenc_remove(struct platform_device *pdev)
{
	...
	nvt_gpenc_drv_remove(&pdrv_info->module_info);
...
static int nvt_gpenc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_gpenc_drv_suspend(&pdrv_info->module_info);
...
static int nvt_gpenc_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_gpenc_drv_resume(&pdrv_info->module_info);

	nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_gpenc_driver = {
	.driver = {
				.name   = "nvt_gpenc",
				.owner = THIS_MODULE,
				.of_match_table = gpenc_match_table,
				},
	.probe      = nvt_gpenc_probe,
	.remove     = nvt_gpenc_remove,
	.suspend = nvt_gpenc_suspend,
	.resume = nvt_gpenc_resume
};
...
int __init nvt_gpenc_module_init(void)
{
	...
	nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_gpenc_driver);
...
{
	nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_gpenc_driver);
}

module_init(nvt_gpenc_module_init);
module_exit(nvt_gpenc_module_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("gpenc driver");
MODULE_LICENSE("GPL");
