#ifndef __CSI_TX_DBG_H_
#define __CSI_TX_DBG_H_
...
extern unsigned int csi_tx_debug_level;
#define nvt_dbg(level, fmt, args...)                \
	do {                                               \
		if (unlikely(NVT_DBG_##level <= csi_tx_debug_level))    \
...
#if 0
#define DBG_CSITX(fmt, args...) nvt_dbg(ERR, fmt, ##args)
#define DBG_CSITX_SET(fmt, args...) nvt_dbg(ERR, fmt, ##args)
#define DBG_CSITX_REG(fmt, args...)
#else
#define DBG_CSITX(fmt, args...)
#define DBG_CSITX_SET(fmt, args...)
#define DBG_CSITX_REG(fmt, args...)
#endif
