#ifndef __CSI_TX_DRV_H__
#define __CSI_TX_DRV_H__
#include <linux/io.h>
...
typedef struct module_info {
	struct completion csi_tx_completion;
	struct semaphore csi_tx_sem;
	struct clk *pclk[MODULE_CLK_NUM];
	struct tasklet_struct csi_tx_tasklet;
	void __iomem *io_addr[MODULE_REG_NUM];
	int iinterrupt_id[MODULE_IRQ_NUM];
	wait_queue_head_t csi_tx_wait_queue;
	spinlock_t csi_tx_spinlock;
...
int nvt_csi_tx_drv_init(PMODULE_INFO pmodule_info);
int nvt_csi_tx_drv_remove(PMODULE_INFO pmodule_info);
