...
// Global variable
//============================================================================
static PCSI_TX_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_csi_tx_api_write_reg          },
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_csi_tx_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_csi_tx_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_csi_tx_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_csi_tx_proc_cmd_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	int len = size;
	int ret = -EINVAL;
...
static struct proc_ops proc_cmd_fops = {
	.proc_open      = nvt_csi_tx_proc_cmd_open,
	.proc_read      = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release   = single_release,
	.proc_write     = nvt_csi_tx_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_csi_tx_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_csi_tx_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_csi_tx_proc_help_show, NULL);
}

static struct proc_ops proc_help_fops = {
	.proc_open      = nvt_csi_tx_proc_help_open,
	.proc_release   = single_release,
	.proc_read      = seq_read,
	.proc_lseek     = seq_lseek,
};

int nvt_csi_tx_proc_init(PCSI_TX_DRV_INFO pdrv_info)
{
	int ret = 0;
	struct proc_dir_entry *pmodule_root = NULL;
	struct proc_dir_entry *pentry = NULL;

	pmodule_root = proc_mkdir("csi_tx", NULL);
...
int nvt_csi_tx_proc_remove(PCSI_TX_DRV_INFO pdrv_info)
{
