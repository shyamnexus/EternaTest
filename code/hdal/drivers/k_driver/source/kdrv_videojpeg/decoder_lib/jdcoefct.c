	...
	if (need_full_buffer) {
		...
		for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
			...
			coef->whole_image[ci] = (*cinfo->mem->request_virt_barray)(
							...
							JPOOL_IMAGE,
							TRUE,
							(JDIMENSION) jround_up2((long) compptr->width_in_blocks, (long) compptr->h_samp_factor),
							(JDIMENSION) jround_up2((long) compptr->height_in_blocks, (long) compptr->v_samp_factor),
							(JDIMENSION) access_rows);
			if (NULL == coef->whole_image[ci])
				return -1;
		}
