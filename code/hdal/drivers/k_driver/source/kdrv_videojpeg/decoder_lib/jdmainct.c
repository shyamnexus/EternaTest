...
METHODDEF(int) process_data_simple_main
JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf,
     JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail));
METHODDEF(int) process_data_context_main
JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf,
     JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail));
	...
	JDIMENSION out_rows_avail)
{
	...
	if (! pmain->buffer_full) {
		if (!(*cinfo->coef->decompress_data)(cinfo, pmain->buffer)) {
			return -1;			/* suspension forced, can do nothing more */
		}
			  ...
			  JDIMENSION out_rows_avail)
{
	...
	case CTX_POSTPONED_ROW:
		...
		if (pmain->rowgroup_ctr < pmain->rowgroups_avail)
			return -1;			/* Need to suspend */
		pmain->context_state = CTX_PREPARE_FOR_IMCU;
		if (*out_row_ctr >= out_rows_avail)
			return -1;			/* Postprocessor exactly filled output buf */
	/*FALLTHROUGH*/
...
jinit_d_main_controller(j_decompress_ptr cinfo, boolean need_full_buffer)
{
	...
	if (cinfo->upsample->need_context_rows) {
		...
		if (alloc_funny_pointers(cinfo) < 0) /* Alloc space for xbuffer[] lists */
			return -1;
