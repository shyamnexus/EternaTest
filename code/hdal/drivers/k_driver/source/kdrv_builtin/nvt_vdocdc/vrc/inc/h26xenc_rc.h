...
typedef struct _rc_cb_{
    ...
    int (*h26xEnc_RcGetLog)(H26XEncRC *pRc, uintptr_t *log_addr);
}rc_cb;

rc_cb *rc_cb_init(void);
int h26xEnc_setRCSyncMaqStr(int value);
	...
	#endif
	#if SUPPORT_CVBR
	extern int h26xEnc_RcGetCVBRInfo(int chn, H26XEncCVBRInfo *pinfo);
	#endif
#endif
