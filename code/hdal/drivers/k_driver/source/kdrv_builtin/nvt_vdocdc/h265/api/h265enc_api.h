...
typedef struct _H265ENC_INIT_{
    ...
    BOOL            bTileEn;
	QLVL_TYPE       eQualityLvl;		///< Quality level for setting search range and tile config

    uintptr_t          uiEncBufAddr;       ///< H.265 encoder internal buffer starting address
	...
	BOOL			bGrayColorEn;   	/// < H.265 encode osd/mask color mode while frame gray enable
	...
	BOOL            bD2d1TileEn;        /// < H.265 d2d one tile control (0: disable, 1: enable)
	BOOL            bColMvEn;           /// < H.265 col mv control (0: disable, 1: enable)
	BOOL            bCustQpPriority;    /// < H.265 customized cu16 priority
	// NT98538 //
	BOOL			bTu4Disable;		/// < H.265 TU4 disable (0: enable tu4, 1: disable tu4)
	BOOL			bTu32Disable;		/// < H.265 TU32 disable (0: enable tu32, 1: disable tu4)
	BOOL			bImeGetIntraMV;		/// < H.265 IME get Intra mv (0: disable, 1: enable)
	BOOL			bDebreath;			/// < H.265 debreath (0: disable, 1: enable)
	BOOL			bImeAMVPMode;		/// < H.265 IME AMVP Mode (0: orignal, 1: get col_mv around current block)
	BOOL			bILFLpm;			/// < H.265 ILF low power (0: disable, 1: enable)
	// NT98539 //
	UINT8			ucAMVPMvCost;
	...
	uintptr_t		uiRecBufAddr[3];	///< common buffer of reconstruct frame buffer address.
	UINT8			ucTileNum;
	UINT16			usTileWidth[H265E_TILE_MAX];
	UINT32          uiLtrSavingBufBsSize; ///< 0: LTR need to mallocate REC buffer, > 0: LTR Saving Rec Mode, unit is byte
	H26X_XVR_APP	xvr_app;			///< xvr application
...
typedef struct _H265ENC_INFO_{
	...
	uintptr_t	uiSrcYAddr;				///< Source Y address
	uintptr_t	uiSrcCAddr;				///< Source UV address
	...
	BOOL		bSrcOutEn;
	UINT8		ucSrcOutMode;
	uintptr_t	uiSrcOutYAddr;			///< Source Out (3DNR) Y address
	uintptr_t	uiSrcOutCAddr;			///< Source Out (3DNR) UV address
	...
	uintptr_t	uiBsOutBufAddr;		    ///< Encoded bit-stream Phy address
	UINT32		uiBsOutBufSize;			///< Encoded bit-stream size
	uintptr_t	uiBsOutBufAddr2;		///< Encoded bit-stream Phy address
	UINT32		uiBsOutBufSize2;		///< Encoded bit-stream size
	UINT64		uiSrcTimeStamp;
	UINT32		uiTemporalId;
	uintptr_t	uiNalLenOutAddr;
	BOOL		bSkipFrmEn;
    UINT8       ucSkipFrmMode;          ///< 0: fast, 1: normal speed
	BOOL		bGetSeqHdrEn;			///< get sps and pps from uiBsOutBufAddr

    UINT32      uiPredBitss;
    uintptr_t   uiH26XBaseVa;
...
typedef struct _H265EncRdo_{
	...
	INT8  cost_bias_skip_bg;
	INT8  cost_bias_merge_bg;
	UINT8 cost_bias_shift;

	UINT8 ime_left_amvp_mode;
} H265EncRdo;
...
typedef struct _H265EncRmdCfg_{
    UINT8 rmd_cost_tweak_pl;
    UINT8 rmd_cost_tweak_dc;
    UINT8 ira_modebits_de;
} H265EncRmdCfg;

typedef struct _H265EncEarlySkipCfg_{
	UINT8  ucMDTh;
	UINT8  ucMVTh;
	UINT32 uiSkipCostTh[2];

	UINT8  ucMDLpmIraStopEn;
	BOOL   bMDEarlySkipEn;
} H265EncEarlySkipCfg;

extern INT32 h265Enc_setEarlySkipCfg(H26XENC_VAR *pVar, H265EncEarlySkipCfg *pEarlySkipCfg);

extern INT32 h265Enc_initEncoder(H265ENC_INIT *pInit, H26XENC_VAR *pVar, BOOL bPreInit);
...
extern INT32 h265Enc_setBGRdoCfg(H26XENC_VAR *pVar, INT8 bg_skip_bias, INT8 bg_merge_bias, UINT8 bg_shift);
...
extern void h265Enc_getSeqHdr(H26XENC_VAR *pVar, uintptr_t *pAddr, UINT32 *pSize);

extern UINT32 h265Enc_getNxtPicType(H26XENC_VAR *pVar);
extern void h265Enc_setNxtPicType(H26XENC_VAR *pVar, SLICE_TYPE slice_type);
extern UINT32 h265Enc_getPocNum(H26XENC_VAR *pVar);
extern UINT32 h265Enc_getGopNum(H26XENC_VAR *pVar);
extern void h265Enc_setGopNum(H26XENC_VAR *pVar, UINT32 gop);
extern UINT32 h265Enc_getFrmSkipFlag(H26XENC_VAR *pVar);
extern void h265Enc_setFrmSkipFlag(H26XENC_VAR *pVar, BOOL bFrmSkip);
...
extern UINT32 h265Enc_getEncInfo(H26XENC_VAR *pVar, H26XEncInfoCfg *pEncInfo);
#if H26X_SET_PROC_PARAM
extern int h265Enc_getRowRCStopFactor(void);
extern int h265Enc_setRowRCStopFactor(int value);
#endif
extern int h265Enc_setFroSync(int value);
extern int h265Enc_getFroSync(void);
extern int h265Enc_setLongStartCode(UINT32 lsc);
extern int h265Enc_setSEIBsChksumen(H26XENC_VAR *pVar, UINT32 enable);
extern INT32 h265Enc_setRmdCfg(H26XENC_VAR *pVar, H265EncRmdCfg *pRmd);
extern void h265Enc_setTileCfg(H265ENC_INIT *pInit, H265EncTileCfg *p_tile_cfg);
