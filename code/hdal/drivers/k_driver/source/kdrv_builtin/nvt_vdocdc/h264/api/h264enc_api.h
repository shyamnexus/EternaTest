...
typedef struct _H264ENC_INIT_ {
	...
	// Bitstream Buffer  //
	uintptr_t       uiEncBufAddr;       ///< H.264 encoder internal buffer starting address
	...
	BOOL			bSraqSaveDQPEn;		/// < H.264  SRAQ AVC save dqp enable
...
typedef struct _H264ENC_INFO_ {
	...
	uintptr_t	uiBsOutBufAddr;			///< Encoded bit-stream address
	UINT32		uiBsOutBufSize;			///< Encoded bit-stream size
	uintptr_t	uiBsOutBufAddr2;	    ///< Encoded bit-stream Phy address2
	UINT32		uiBsOutBufSize2;		///< Encoded bit-stream size2
...
extern INT32 h264Enc_initEncoder(H264ENC_INIT *pInit, H26XENC_VAR *pVar, BOOL bPreInit);
#if 0
...
extern INT32 h264Enc_setBGRdoCfg(H26XENC_VAR *pVar, INT8 bg_skip, UINT8 bg_shift);
...
extern void h264Enc_getSeqHdr(H26XENC_VAR *pVar, uintptr_t *pAddr, UINT32 *pSize);

extern UINT32 h264Enc_getNxtPicType(H26XENC_VAR *pVar);
extern void h264Enc_setNxtPicType(H26XENC_VAR *pVar, SLICE_TYPE slice_type);
extern UINT32 h264Enc_getPocNum(H26XENC_VAR *pVar);
extern UINT32 h264Enc_getGopNum(H26XENC_VAR *pVar);
extern void h264Enc_setGopNum(H26XENC_VAR *pVar, UINT32 gop);
extern UINT32 h264Enc_getFrmSkipFlag(H26XENC_VAR *pVar);
extern void h264Enc_setFrmSkipFlag(H26XENC_VAR *pVar, BOOL bFrmSkip);
...
extern UINT32 h264Enc_getEncInfo(H26XENC_VAR *pVar, H26XEncInfoCfg *pEncInfo);
...
extern int h264Enc_setYCoefCostTh(int value);
extern int h264Enc_getYCoefCostTh(void);
extern int h264Enc_setCCoefCostTh(int value);
extern int h264Enc_getCCoefCostTh(void);
#endif
...
#ifdef VDOCDC_EMU
extern INT32 h264Enc_setRdo_emu(H26XENC_VAR *pVar, H264EncRdo *pRdo);
#endif // VDOCDC_EMU
