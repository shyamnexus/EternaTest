	...
	for (i = 0; i < BRIDGE_MAX_OPT_CNT; i++) {
		if (mp_bridge->opts[i].tag == tag) {
			*p_val = mp_bridge->opts[i].val;
			...
			pr_warn("unable to find tag: 0x%08X\n", tag);
			return -1;
		}
	}
...
int kdrv_bridge_set_tag(unsigned int tag, unsigned int val)
{
	int i;
...
int kdrv_fdt_map(void)
{
	u32 regs[2] = {0};
	const char path[] = "/nvt_memory_cfg/fdt";
	struct device_node* of_node = of_find_node_by_path(path);

	if (!kdrv_builtin_is_fastboot()) {
		pr_err("unable to map fdt without fastboot mode\n");
		return 0;
	}

	if (mp_fdt) {
		pr_err("fdt has mapped\n");
		return 0;
	}
	...
	if (of_property_read_u32_array(of_node, "reg", regs, 2) != 0) {
		pr_err("unable to read %s/reg\n", path);
		return -1;
	}

	mp_fdt = (void *)ioremap_cache(regs[0], regs[1]);

	if (fdt_check_header(mp_fdt) != 0) {
		pr_err("invalid fdt\n");
		iounmap((void *)mp_fdt);
		mp_fdt = NULL;
		return -1;
