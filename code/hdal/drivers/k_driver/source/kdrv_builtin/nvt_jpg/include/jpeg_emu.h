...
#define SUPPORT_JE_CTG				1
#define SUPPORT_JE_MASK				1
#define SUPPORT_JPEG_ENGINE			1
#define JPGDEC_PAD					1
#define SUPPORT_JE_ROI				1

#define ROI_REGION					11
#define ROI_LEVEL					3
#define OSG_REGION					16		//538: 16, 567: 10
#define OSG_LAYER					1
#define MASK_REGION					16
#define MASK_LINE					4
#define MASK_PAL_NUM				16
#define OSG_PAL_TABLE				16		// YCT
...
#define EMU_JPEG_MIN_BS_LEN         (0x104) //260

#define JPEG_BS_SEG_NUM				2		/* bs segment number */
// #define JPEG_BS_SEG_LEN_MIN			0x104	/* bs length minimum (without header) */
...
typedef struct {
	...
	uintptr_t      uiRefBSBuf;

	// Source Y buffer
	uintptr_t      uiYBuf;

	// Source Y temp buffer for line offset != width
	uintptr_t      uiTempYBuf;

	// Source UV buffer
	uintptr_t      uiUVBuf;

	// DC out Y buffer
	uintptr_t      uiDCOutYBuf;

	// DC out UV buffer
	uintptr_t      uiDCOutUVBuf;
	...
	uintptr_t      uiUPlanarBuf;
	uintptr_t      uiVPlanarBuf;

	// Reference DC out Y buffer
	uintptr_t      uiRefDCOutYBuf;

	// Reference DC out UV buffer
	uintptr_t      uiRefDCOutUVBuf;

	// For reference DC otuput UV packed temp buffer (Temporary solution)
	// Patterns are UV planar, but 650 only support UV pbacked
	// In all supported formats, Y size always >= U+V size.
	// We use reference DC out Y buffer for U V planar buffer
	uintptr_t      uiRefDCOutUPlanarBuf;
	uintptr_t      uiRefDCOutVPlanarBuf;

#if SUPPORT_JE_OSG
	uintptr_t		uiOSGGraphBuf[OSG_LAYER][OSG_REGION];
#endif
...
#if (SUPPORT_JE_PAD | SUPPORT_JE_CTG)
typedef struct {
	UINT32	PAD_X;
	UINT32	PAD_Y;
	UINT32	CTG_SRC_EN;
...
#if SUPPORT_JE_OSG
typedef struct {
	UINT32		OsgGraphReg[2][16][9];					// 0x000 ~ 0x240*2
	UINT32		RESERVED[2];							// 0x480 ~ 0x488
	UINT32		OsgCstReg[3];							// 0x488 ~ 0x494
	UINT32		OsgPalReg[16];							// 0x494 ~ 0x4D8
	UINT32		OsgOverall;								// 0x4D4
...
typedef struct {
	// UINT32		MaskEnable;
	// UINT32		MaskCoefReg[MASK_REGION][MASK_LINE*2+1];
	// UINT32		MaskPalReg[MASK_PAL_NUM];
	// UINT32		MaskPointReg[MASK_REGION][MASK_LINE*2];
	// UINT32		MaskChkReg[MASK_REGION][MASK_LINE];
	// UINT32		MaskCoef2Reg[MASK_REGION][MASK_LINE];
	UINT32		MaskReg[417];
} EMU_JPEG_ENC_MASK_REG;
#endif
...
typedef struct {
	UINT32		PAD_X;
	UINT32		PAD_Y;
} EMU_JPGDEC_EXTRA_REG;
#endif

#if SUPPORT_JE_ROI
typedef struct {
	UINT32		RoiReg[ROI_REGION][ROI_LEVEL];
} EMU_JPEG_ENC_ROI_REG;
#endif
...
typedef enum {
	...
	EMU_JPEG_MCU_OSG_ALL = 21,
	EMU_JPEG_MCU_OSG_TEST,
	// ============= 530
	EMU_JPEG_MCU_CTG = 50,
	EMU_JPEG_MCU_MASK = 51,
	EMU_JPEG_MCU_MASK_OSG = 52,
	EMU_JPEG_MCU_MASK_OSG_PAD = 53,
	EMU_JPEG_MCU_MASK_OSG_PAD_CTG = 54,
	EMU_JPEG_MCU_PAD_OSG = 55,
	EMU_JPEG_MCU_YCC = 80,						//chip530: 56
	EMU_JPEG_MCU_MASK_OSG_YCC = 57,
	EMU_JPEG_MCU_MASK_OSG_PAD_YCC = 58,
	EMU_JPEG_MCU_MASK_OSG_PAD_CTG_YCC = 59,
	EMU_JPEG_MCU_PAD_OSG_YCC = 60,
	// ============= specail
	EMU_JPEG_MCU_OSG_SINGLE = 61,
	// ============= 690
	EMU_JPEG_MCU_PAD_CTG = 79,
	// EMU_JPEG_MCU_YCC_NEW = 80,
	// =============
	EMU_JPEG_MCU_ERR_SDE = 99,
	// ============= 538
	EMU_JPEG_MCU_NEWOSG2 = 112,

	EMU_JPEG_ROI_BASIC = 100,
	EMU_JPEG_ROI_SPECIAL = 101,
	EMU_JPEG_ROI_CORNER = 102,
	EMU_JPEG_ROI_PAD = 103,
	FEATURE_RESTART = 104,
	EMU_JPEG_OSG2_CST_ROI = 108,
	EMU_JPEG_OSG2_CHROMA_ROI = 109,	//chroma alpha=0, graph_type=all
	EMU_JPEG_OSGOLD_ALL = 110,
	EMU_JPEG_NEWOSG_ALL = 111,
	EMU_JPEG_OSG_SINGLE = 113,
	EMU_JPEG_NEWOLD_MOSAIC = 114,
	EMU_JPEG_OLDOSG_ROI_CTG = 115,
	EMU_JPEG_OSG1_MOSAIC_ROI_PAD_CTG = 116,
	EMU_JPEG_OSG2_CHROMA_ALPHA0 = 117,
	EMU_JPEG_YCC_LITE = 120,
	EMU_JPEG_OSGROI = 121,
	EMU_JPEG_YCC_OSGROI = 122,
	EMU_JPEG_YCCLITE_OSG1_PAD = 123,
	EMU_JPEG_YCCLITE_OSG2_PAD = 124,
	EMU_JPEG_YCCLITE2_ROI_NEWOLD_OSG_CTG = 125,
	EMU_JPEG_YCCLITE2_OSG2_ROI_PAD_CTG = 126,
} EMU_JPEG_MCU;
...
typedef struct {
	...
	// Source second bit stream buffer
	uintptr_t      uiBSSecBuf;

	// Source header buffer
	uintptr_t      uiHeaderBuf;
...
#define EMU_JPEG_DECMODE_NORMAL_PAD			17		// Normal Pad
#define EMU_JPEG_DECMODE_RESTART_PAD		18		// Restart Pad
#define EMU_JPEG_DECMODE_TRANSFORM			19		// Transform 422 to 420
#define EMU_JPEG_DECMODE_TRANSFORM_PAD		20		// Transform 422 to 420 + Pad
#define EMU_JPEG_DECMODE_TF_PAD_RST			21		// Transform 422 to 420 + Restart Pad
