...
void __iomem *_EAC_REG_BASE_ADDR[EAC_REG_NUM];
...
int nvt_eac_drv_init(EAC_MODULE_INFO *pmodule_info)
{
	...
	for (ucloop = 0 ; ucloop < EAC_REG_NUM; ucloop++) {
		_EAC_REG_BASE_ADDR[ucloop] = pmodule_info->io_addr[ucloop];
	...
	eac_clk = clk_get(NULL, "clk_eacadc");
	if (IS_ERR(eac_clk)) {
	...
	eac_clk = clk_get(NULL, "clk_eacdac");
	if (IS_ERR(eac_clk)) {
