...
/*===========================================================================*/
int event_flag = 0;
//void __iomem *_DAI_REG_BASE_ADDR[DAI_REG_NUM];
...
int nvt_dai_drv_init(DAI_INFO *pmodule_info)
{
    ...
    for (ucloop = 0 ; ucloop < DAI_REG_NUM; ucloop++) {
		// _DAI_REG_BASE_ADDR[ucloop] = pmodule_info->io_addr[ucloop];
		dai_reg_base[0][ucloop] = pmodule_info->io_addr[ucloop];
        
		//nvt_dbg(IND, "DAI[%d]-Addr = 0x%08X\n", ucloop, _DAI_REG_BASE_ADDR[ucloop]);

        DBG_FUNC("DAI[%d]-Addr = 0x%X\n", ucloop, dai_reg_base[0][ucloop]);
	}
