...
#define NVT_FAST_AI_FLOW                0 //1: enable ai fastboot flow .....  No more use ai in builtin, disable it. 2023/1/4
...
typedef struct _NN_GEN_MODE_CTRL {
	...
	uintptr_t stripe_inaddr[4];
	uintptr_t stripe_outaddr[8];
	INT32 output_buffsize[8];
#else
	INT32 in_buf_index[3];
	INT32 out_buf_index[2];
	uintptr_t stripe_inaddr[3];
	uintptr_t stripe_outaddr[2];
