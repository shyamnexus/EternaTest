...
#include "grph_builtin_platform.h"
#include "grph_builtin_int.h"
...
static ID v_flg_id[GRPH_ID_3+1];

static SEM_HANDLE *v_sem[GRPH_ID_3+1];
static SEM_HANDLE *v_sem_oc[GRPH_ID_3+1];

static SEM_HANDLE SEMID_GRAPHIC;
static SEM_HANDLE SEMID_GRAPHIC2;
static SEM_HANDLE SEMID_GRAPHIC3;

static SEM_HANDLE SEMID_GRAPHIC_OPEN_CLOSE;
static SEM_HANDLE SEMID_GRAPHIC2_OPEN_CLOSE;
static SEM_HANDLE SEMID_GRAPHIC3_OPEN_CLOSE;

static ID		FLG_ID_GRAPHIC;
static ID		FLG_ID_GRPH_IST;
static const FLGPTN	v_flag_ptn[GRPH_ID_3+1] = {FLGPTN_BIT(0), FLGPTN_BIT(1), FLGPTN_BIT(2)};

static vk_spinlock_t	v_grph_spin_locks[GRPH_ID_3+1];

static THREAD_HANDLE	grph_ist_tsk_id          = 0;

static const CG_EN v_clken[] = {GRAPH_CLK, GRAPH2_CLK, GRAPH3_CLK};
static UINT32 v_clksel[] = {PLL_CLKSEL_GRAPHIC_240, PLL_CLKSEL_GRAPHIC2_240, PLL_CLKSEL_GRAPHIC3_240};

UINT8* grph_ll_buffer[GRPH_ID_3 + 1];
...
uintptr_t IOADDR_GRAPHIC_REG_BASE;
uintptr_t IOADDR_GRAPHIC2_REG_BASE;
uintptr_t IOADDR_GRAPHIC3_REG_BASE;
...
static spinlock_t v_grph_spin_locks[GRPH_ID_3+1];

static struct tasklet_struct * v_p_timer_tasklet[GRPH_ID_3+1];

static struct clk *p_grph_clk[GRPH_ID_3+1];
...
static UINT32 v_grph_ist_event[GRPH_ID_3+1];

KDRV_CALLBACK_FUNC v_grph_callback[GRPH_ID_3+1];
...
{
#if defined(__FREERTOS)
	pll_enable_clock(v_clken[id]);
	//*(UINT32 *)0xf0020070 |= 0x18400000;
#else
...
void grph_platform_clk_set_freq(GRPH_ID id, UINT32 freq)
{
	...
	const UINT32 v_clk_rate[3][4] = {
		{
			PLL_CLKSEL_GRAPHIC_240,     PLL_CLKSEL_GRAPHIC_320,
			PLL_CLKSEL_GRAPHIC_480,     PLL_CLKSEL_GRAPHIC_PLL13
		},
		{
			PLL_CLKSEL_GRAPHIC2_240,    PLL_CLKSEL_GRAPHIC2_320,
			PLL_CLKSEL_GRAPHIC2_480,    PLL_CLKSEL_GRAPHIC2_PLL13
		},
		{
			PLL_CLKSEL_GRAPHIC3_240,    PLL_CLKSEL_GRAPHIC3_320,
			PLL_CLKSEL_GRAPHIC3_480,    PLL_CLKSEL_GRAPHIC3_PLL13
		}
	};
	...
	if (freq < 240) {
		...
		DBG_WRN("%d: input frequency %d round to 500MHz\r\n", id, (unsigned int)freq);
		freq = 3;
		if (pll_get_pll_enable(PLL_ID_13) == FALSE) {
			pll_set_pll_enable(PLL_ID_13, TRUE);
		}
	...
	else if (id == GRPH_ID_2)
		pll_set_clock_rate(PLL_CLKSEL_GRAPHIC2, v_clk_rate[id][freq]);
	else
		pll_set_clock_rate(PLL_CLKSEL_GRAPHIC3, v_clk_rate[id][freq]);
	...
	if (freq < 240) {
		...
		printk("%d: input frequency %d round to 500MHz\r\n", id, freq);
		parent_clk = clk_get(NULL, "pll13");
	}
...
void grph_platform_clk_get_freq(GRPH_ID id, UINT32 *p_freq)
{
	...
	if (id == GRPH_ID_1) {
		...
		case PLL_CLKSEL_GRAPHIC_PLL13:
			...
			*p_freq = 500;
#else
			*p_freq = pll_get_pll_freq(PLL_ID_13);
			*p_freq = ((*p_freq) + 500000) / 1000000;
#endif
			break;
		}
	} else if (id == GRPH_ID_2) {
	...
	} else {	// if (id == GRPH_ID_3)
		switch (pll_get_clock_rate(PLL_CLKSEL_GRAPHIC3)) {
		case PLL_CLKSEL_GRAPHIC3_240:
		default:
			*p_freq = 240;
			break;
		case PLL_CLKSEL_GRAPHIC3_320:
			*p_freq = 320;
			break;
		case PLL_CLKSEL_GRAPHIC3_480:
			*p_freq = 480;
			break;
		case PLL_CLKSEL_GRAPHIC3_PLL13:
#if defined(_NVT_FPGA_)
			*p_freq = 500;
#else
			*p_freq = pll_get_pll_freq(PLL_ID_13);
...
unsigned long grph_platform_spin_lock(GRPH_ID id)
{
#if defined(__FREERTOS)
	unsigned long flags;
...
void grph_platform_spin_unlock(GRPH_ID id, unsigned long flags)
{
#if defined(__FREERTOS)
...
void grph_platform_sram_enable(GRPH_ID id)
{
    if (id == GRPH_ID_1)
    	...
    	nvt_disable_sram_shutdown(GRAPH3_SD);
}
...
UINT32 grph_platform_dma_is_cacheable(uintptr_t addr)
{
	return 1;
}

UINT32 grph_platform_dma_flush_dev2mem_width_neq_loff(uintptr_t addr, UINT32 size)
{
#if defined(__FREERTOS)
...
UINT32 grph_platform_dma_flush_dev2mem(uintptr_t addr, UINT32 size)
{
#if defined(__FREERTOS)
...
UINT32 grph_platform_dma_post_flush_dev2mem(uintptr_t addr, UINT32 size)
{
#if defined(__FREERTOS)
	vos_cpu_dcache_sync((VOS_ADDR)addr, size, VOS_DMA_FROM_DEVICE_NON_ALIGN);
	return 0;
...
UINT32 grph_platform_dma_flush_mem2dev(uintptr_t addr, UINT32 size)
{
#if defined(__FREERTOS)
...
BOOL grph_platform_is_valid_va(uintptr_t addr)
{
#if defined(__FREERTOS)
...
phys_addr_t grph_platform_va2pa(uintptr_t addr)
{
#if defined(__FREERTOS)
...
void grph_platform_ist(void)
{
	...
	while (1) {
		FLGPTN ptn = 0;
		UINT32 flg = v_flag_ptn[0]|v_flag_ptn[1]|v_flag_ptn[2];
		UINT32 ist_events;
		...
		if (ptn & v_flag_ptn[2]) {
			spin_flags = grph_platform_spin_lock(GRPH_ID_3);
			ist_events = v_grph_ist_event[GRPH_ID_3];
			v_grph_ist_event[GRPH_ID_3] = 0;
			grph_platform_spin_unlock(GRPH_ID_3, spin_flags);

			grph_isr_bottom(GRPH_ID_3, ist_events);
		}
	}
...
void grph_platform_init(void)
{
	...
	grph_ll_buffer[2] = malloc(GRPH_LL_BUF_SIZE);
	...
	vk_spin_lock_init(&v_grph_spin_locks[GRPH_ID_3]);
	...
	request_irq(INT_ID_GRAPHIC3, (irq_handler_t)graph3_isr, IRQF_TRIGGER_HIGH, "grph3", 0);
}
#else
void grph_platform_init(MODULE_INFO *pmodule_info)
{
	IOADDR_GRAPHIC_REG_BASE = (uintptr_t)pmodule_info->io_addr[0];
	IOADDR_GRAPHIC2_REG_BASE = (uintptr_t)pmodule_info->io_addr[1];
	IOADDR_GRAPHIC3_REG_BASE = (uintptr_t)pmodule_info->io_addr[2];
	p_grph_clk[0] = pmodule_info->pclk[0];
	p_grph_clk[1] = pmodule_info->pclk[1];
	p_grph_clk[2] = pmodule_info->pclk[2];
	OS_CONFIG_FLAG(FLG_ID_GRAPHIC);
	SEM_CREATE(SEMID_GRAPHIC, 1);
	SEM_CREATE(SEMID_GRAPHIC2, 1);
	SEM_CREATE(SEMID_GRAPHIC3, 1);
	...
	grph_ll_buffer[2] = kmalloc(GRPH_LL_BUF_SIZE, GFP_KERNEL);
	...
	v_p_timer_tasklet[GRPH_ID_3] = &pmodule_info->xxx_tasklet[GRPH_ID_3];
	grph_jobs_init();
	...
	// coverity[side_effect_free]: spin_lock_init is kernel API, do not change
	spin_lock_init(&v_grph_spin_locks[GRPH_ID_3]);
}
#endif
