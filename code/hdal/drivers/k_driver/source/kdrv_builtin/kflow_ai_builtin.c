...
static ER _kflow_ai_builtin_get_fastboot_dt(KFLOW_AI_BUILTIN_MAP_MEM_PARM *p_mem)
{
	...
	printk("    kerl_parm:  pa(0x%lx) va(0x%lx) size(0x%x)\r\n", (unsigned long)p_mem->kerl_parm.pa, (unsigned long)p_mem->kerl_parm.va, p_mem->kerl_parm.size);
	printk("    user_model: pa(0x%lx) va(0x%lx) size(0x%x)\r\n", (unsigned long)p_mem->user_model.pa, (unsigned long)p_mem->user_model.va, p_mem->user_model.size);
	printk("    pd_buff:    pa(0x%lx) va(0x%lx) size(0x%x)\r\n", (unsigned long)p_mem->pd_buff.pa, (unsigned long)p_mem->pd_buff.va, p_mem->pd_buff.size);
	printk("    fboot_rslt: pa(0x%lx) va(0x%lx) size(0x%x)\r\n", (unsigned long)p_mem->rslt_buff.pa, (unsigned long)p_mem->rslt_buff.va, p_mem->rslt_buff.size);
	printk("    io_buff:    pa(0x%lx) va(0x%lx) size(0x%x)\r\n", (unsigned long)p_mem->io_buff.pa, (unsigned long)p_mem->io_buff.va, p_mem->io_buff.size);
	...
	if (p_mem->kerl_parm.pa == kerl_start.pa) {
		...
		printk("starting mem not match! kerl_parm.pa(0x%lx) kerl_start.pa(0x%lx)\r\n", (unsigned long)p_mem->kerl_parm.pa, (unsigned long)kerl_start.pa);
		return E_SYS;
	}
	...
	if (p_mem->io_buff.pa == kerl_start.pa) {
		...
		printk("starting mem not match! io_buff.pa(0x%lx) kerl_start.pa(0x%lx)\r\n", (unsigned long)p_mem->io_buff.pa, (unsigned long)kerl_start.pa);
		return E_SYS;
	}
	printk("mem_in_kerl: g_old_base_va(0x%lx) g_old_io_base_va(0x%lx)\r\n", (unsigned long)g_old_base_va, (unsigned long)g_old_io_base_va);
...
ER vendor_ais_get_net_info(NN_GEN_NET_INFO *p_info, uintptr_t net_addr)
{
	...
	p_mctrl = (NN_GEN_MODE_CTRL *)((uintptr_t)p_head + model_head_size);
#if CNN_528_PSW
	p_id_list = (UINT32 *)((uintptr_t)p_mctrl + mctrl_size);
#if CNN_25_MATLAB
	p_io_mem = (NN_IOMEM *)((uintptr_t)p_id_list + p_head->layer_id_list_size);
#endif
#else
...
void _kflow_ai_builtin_trig(KFLOW_AI_BUILTIN_FMD_INFO *p_info, UINT32 reserved)
{
	...
	if (p_info->out_img[KDRV_IPP_BUILTIN_PATH_ID_3].addr[0] == 0 || size == 0) {
		DBG_ERR("invalid input, addr(0x%lx) size(0x%x)\r\n",
			(unsigned long)p_info->out_img[KDRV_IPP_BUILTIN_PATH_ID_3].addr[0], size);
		return;
	}
...
BOOL kflow_ai_builtin_put_rslt(UINT32 proc_id, VENDOR_AIS_IMG_PARM *p_img_parm, PD_MEM *p_pd_mem)
{
	...
	if ((p_obj->front == p_obj->rear) && (p_obj->b_full == TRUE)) {
		...
		if (rslt_buf[p_obj->rear].va < rslt_start_va) {
			DBG_ERR("invalid rslt mem! rslt_buf[%u].va(0x%lx) > rslt_start_va(0x%lx)\r\n",
					p_obj->rear, (unsigned long)rslt_buf[p_obj->rear].va, (unsigned long)rslt_start_va);
			return FALSE;
		} else {
...
ER kflow_ai_builtin_init(KFLOW_AI_BUILTIN_INIT_INFO *info)
{
	...
	cfg.net_id = 0;
	kdrv_ai_builtin_config_flow(1); //config to new AI flow
	...
	if (kflow_ai_bt_obj.mem_parm.kerl_parm.va != info->blk_addr) {
		DBG_ERR("memory alloc not match, kerl_va(0x%lx) blk_addr(0x%lx), please check dtsi\r\n",
			(unsigned long)kflow_ai_bt_obj.mem_parm.kerl_parm.va, (unsigned long)info->blk_addr);
		return E_SYS;
	}
	...
	ret = -1;
	if (ret != 1) { // 1: OK, -1: NG
		DBG_ERR("check preload finish fail(%d), function not ready\r\n", ret);
		return ret;
	}
#endif
