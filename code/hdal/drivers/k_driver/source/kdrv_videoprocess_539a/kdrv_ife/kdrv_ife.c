...
static IFE_ENG_YUV2RGB_FMT kdrv_ife_typecast_yuv2rgb(KDRV_IPP_FMT fmt)
{
...
#if IFE_SSDRV_SUPPORT
	/*-----------motion limitation-----------*/
	if (iq_cfg->filt.motion_set.motion_en) {
		if (io_cfg->motion_type == 0) {
			if (kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.w, 8) != TRUE || kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.h, 8) != TRUE) {
				DBG_ERR("motion_type 1/64, but in_width/high not align to %d\r\n", 8);
				return E_PAR;
			}
		} else if (io_cfg->motion_type == 1) {
			if (kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.w, 4) != TRUE || kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.h, 4) != TRUE) {
				DBG_ERR("motion_type 1/16, but in_width/high not align to %d\r\n", 4);
				return E_PAR;
			}
		} else {
			if (kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.w, 2) != TRUE || kdrv_ife_chk_align(p_cfg->p_iocfg->in_frm.size.h, 2) != TRUE) {
				DBG_ERR("motion_type 1/4, but in_width/high not align to %d\r\n", 2);
				return E_PAR;
			}
		}
		if (p_cfg->p_iocfg->in_frm.size.w > MAX_IPP_STRIPE_SIZE) {
			DBG_ERR("ife motion map not support MST (in_width %d > %d)\r\n", (UINT32)p_cfg->p_iocfg->in_frm.size.w, MAX_IPP_STRIPE_SIZE);
			return E_PAR;
		}
	}
#endif
...
static INT32 kdrv_ife_int_cfg_all(KDRV_IFE_HANDLE *p_hdl, KDRV_IFE_JOB_CFG *p_cfg)
{
	...
	if(KDRV_IPP_FMT_CLASS(io_cfg->in_frm.fmt ) == KDRV_IPP_FMT_CLASS_YUV){
		...
		yuv_in_param.yuv2rgb_fmt = kdrv_ife_typecast_yuv2rgb(io_cfg->in_frm.fmt);
		...
		//ife_eng_set_cfa_pat_2_buf_reg(p_hdl->p_eng, kdrv_ife_typecast_pix(io_cfg->in_frm.pxl_fmt)); //cfa2 move to pre
	...
	// 539A add
	if (op_mode == IFE_OPMODE_IPP) {

		if(io_cfg->mode == KDRV_IPP_OPMODE_IFE2IME || io_cfg->mode == KDRV_IPP_OPMODE_IFE2IPE){
			io_cfg->in_sel = 0;
		} else
			io_cfg->in_sel = 1;
		io_cfg->out_sel = 1; // fixed to IPE
		ife_eng_set_in_src_sel_buf_reg(p_hdl->p_eng, io_cfg->in_sel);
		ife_eng_set_out_dst_sel_buf_reg(p_hdl->p_eng, io_cfg->out_sel);
	}
	...
	/*------------------- Set WDR Subimg either update, subout_on --------------------*/
	if(iq_cfg->update & KDRV_IFE_UPDATE_WDR || io_cfg->wdr_sub_out_en){
		...
		if (iq_cfg->filt.enable) {
			...
			if(KDRV_IPP_FMT_CLASS(io_cfg->in_frm.fmt ) == KDRV_IPP_FMT_CLASS_RAW || KDRV_IPP_FMT_CLASS(io_cfg->in_frm.fmt ) == KDRV_IPP_FMT_CLASS_NRX){
				...
				IFE_CLAMPSET clamp_weight = {0};
				{
					...
					ife_eng_set_2dnr_nlm_search_range_buf_reg(p_hdl->p_eng, iq_cfg->filt.range_sel); //539A add
				}

				//539A add
				{
					IFE_MOTIONSET motion_set = {0};


					UINT64 addr2 = io_cfg->in_frm.phyaddr[2];

					// motion input address
					if(iq_cfg->filt.motion_set.motion_en)
						ife_eng_set_dma_in_addr_2_buf_reg(p_hdl->p_eng, (UINT32)(addr2 & 0xFFFFFFFF), (UINT32)((addr2 >> 32) & 0xFFFFFFFF), io_cfg->in_frm.lofs[2]);

					motion_set.motion_en = iq_cfg->filt.motion_set.motion_en;
					motion_set.motion_symbol = iq_cfg->filt.motion_set.motion_symbol;

					motion_set.motion_type = io_cfg->motion_type;

					motion_set.static_symbol = iq_cfg->filt.motion_set.static_symbol;
					motion_set.transition_symbol = iq_cfg->filt.motion_set.transition_symbol;

					ife_eng_set_2dnr_motion_control_buf_reg(p_hdl->p_eng, motion_set);
					ife_eng_set_2dnr_motion_input_buf_reg(p_hdl->p_eng, io_cfg->motion_sel);
				}

				{
					IFE_RANGESETA range_set_a = {0};
					IFE_RANGESETA range_set_a1 = {0};
					IFE_RANGESETA range_set_a2 = {0};
					...
					ife_eng_set_2dnr_nlm_parameter0_buf_reg(p_hdl->p_eng, &range_set_a);

					//539A add
					range_set_a1.p_rngth_c0 = &iq_cfg->filt.rng_filt_r_1.a_th[0];
					range_set_a1.p_rnglut_c0 = &iq_cfg->filt.rng_filt_r_1.a_lut[0];
					range_set_a1.p_rngth_c1 = &iq_cfg->filt.rng_filt_gr_1.a_th[0];
					range_set_a1.p_rnglut_c1 = &iq_cfg->filt.rng_filt_gr_1.a_lut[0];
					if (kdrv_ife_typecast_bayer_fmt(io_cfg->in_frm.fmt) == IFE_BAYER_RGBIR) {
						range_set_a1.p_rngth_c2 = &iq_cfg->filt.rng_filt_ir_1.a_th[0];
						range_set_a1.p_rnglut_c2 = &iq_cfg->filt.rng_filt_ir_1.a_lut[0];
					} else {
						range_set_a1.p_rngth_c2 = &iq_cfg->filt.rng_filt_gb_1.a_th[0];
						range_set_a1.p_rnglut_c2 = &iq_cfg->filt.rng_filt_gb_1.a_lut[0];
					}
					range_set_a1.p_rngth_c3 = &iq_cfg->filt.rng_filt_b_1.a_th[0];
					range_set_a1.p_rnglut_c3 = &iq_cfg->filt.rng_filt_b_1.a_lut[0];

					range_set_a2.p_rngth_c0 = &iq_cfg->filt.rng_filt_r_2.a_th[0];
					range_set_a2.p_rnglut_c0 = &iq_cfg->filt.rng_filt_r_2.a_lut[0];
					range_set_a2.p_rngth_c1 = &iq_cfg->filt.rng_filt_gr_2.a_th[0];
					range_set_a2.p_rnglut_c1 = &iq_cfg->filt.rng_filt_gr_2.a_lut[0];
					if (kdrv_ife_typecast_bayer_fmt(io_cfg->in_frm.fmt) == IFE_BAYER_RGBIR) {
						range_set_a2.p_rngth_c2 = &iq_cfg->filt.rng_filt_ir_2.a_th[0];
						range_set_a2.p_rnglut_c2 = &iq_cfg->filt.rng_filt_ir_2.a_lut[0];
					} else {
						range_set_a2.p_rngth_c2 = &iq_cfg->filt.rng_filt_gb_2.a_th[0];
						range_set_a2.p_rnglut_c2 = &iq_cfg->filt.rng_filt_gb_2.a_lut[0];
					}
					range_set_a2.p_rngth_c3 = &iq_cfg->filt.rng_filt_b_2.a_th[0];
					range_set_a2.p_rnglut_c3 = &iq_cfg->filt.rng_filt_b_2.a_lut[0];

					ife_eng_set_2dnr_nlm_parameter1_buf_reg(p_hdl->p_eng, &range_set_a1);
					ife_eng_set_2dnr_nlm_parameter2_buf_reg(p_hdl->p_eng, &range_set_a2);
				}

				{
					IFE_RANGESETB range_set_b = {0};
					IFE_RANGESETB range_set_b1 = {0};
					IFE_RANGESETB range_set_b2 = {0};
					...
					ife_eng_set_2dnr_bilat_parameter0_buf_reg(p_hdl->p_eng, &range_set_b);

					//539A add
					range_set_b1.p_rngth_c0 = &iq_cfg->filt.rng_filt_r_1.b_th[0];
					range_set_b1.p_rnglut_c0 = &iq_cfg->filt.rng_filt_r_1.b_lut[0];
					range_set_b1.p_rngth_c1 = &iq_cfg->filt.rng_filt_gr_1.b_th[0];
					range_set_b1.p_rnglut_c1 = &iq_cfg->filt.rng_filt_gr_1.b_lut[0];
					if (kdrv_ife_typecast_bayer_fmt(io_cfg->in_frm.fmt) == IFE_BAYER_RGBIR) {
						range_set_b1.p_rngth_c2 = &iq_cfg->filt.rng_filt_ir_1.b_th[0];
						range_set_b1.p_rnglut_c2 = &iq_cfg->filt.rng_filt_ir_1.b_lut[0];
					} else {
						range_set_b1.p_rngth_c2 = &iq_cfg->filt.rng_filt_gb_1.b_th[0];
						range_set_b1.p_rnglut_c2 = &iq_cfg->filt.rng_filt_gb_1.b_lut[0];
					}
					range_set_b1.p_rngth_c3 = &iq_cfg->filt.rng_filt_b_1.b_th[0];
					range_set_b1.p_rnglut_c3 = &iq_cfg->filt.rng_filt_b_1.b_lut[0];

					range_set_b2.p_rngth_c0 = &iq_cfg->filt.rng_filt_r_2.b_th[0];
					range_set_b2.p_rnglut_c0 = &iq_cfg->filt.rng_filt_r_2.b_lut[0];
					range_set_b2.p_rngth_c1 = &iq_cfg->filt.rng_filt_gr_2.b_th[0];
					range_set_b2.p_rnglut_c1 = &iq_cfg->filt.rng_filt_gr_2.b_lut[0];
					if (kdrv_ife_typecast_bayer_fmt(io_cfg->in_frm.fmt) == IFE_BAYER_RGBIR) {
						range_set_b2.p_rngth_c2 = &iq_cfg->filt.rng_filt_ir_2.b_th[0];
						range_set_b2.p_rnglut_c2 = &iq_cfg->filt.rng_filt_ir_2.b_lut[0];
					} else {
						range_set_b2.p_rngth_c2 = &iq_cfg->filt.rng_filt_gb_2.b_th[0];
						range_set_b2.p_rnglut_c2 = &iq_cfg->filt.rng_filt_gb_2.b_lut[0];
					}
					range_set_b2.p_rngth_c3 = &iq_cfg->filt.rng_filt_b_2.b_th[0];
					range_set_b2.p_rnglut_c3 = &iq_cfg->filt.rng_filt_b_2.b_lut[0];

					ife_eng_set_2dnr_bilat_parameter1_buf_reg(p_hdl->p_eng, &range_set_b1);
					ife_eng_set_2dnr_bilat_parameter2_buf_reg(p_hdl->p_eng, &range_set_b2);
				}
	...
	// - Shutdown
	if (!iq_cfg->wdr.tonecurve_enable && !iq_cfg->wdr.wdr_enable && !io_cfg->wdr_sub_out_en && !iq_cfg->hist.hist_enable &&
		!iq_cfg->wdr.dither.wdr_dithering_en && !iq_cfg->wdr.fbc.fbc_en && !iq_cfg->wdr.outbld.outbld_en && !iq_cfg->wdr.gainctrl.gainctrl_en) {
		if (op_mode != IFE_OPMODE_ALL_DIRECT || (op_mode == IFE_OPMODE_ALL_DIRECT && io_cfg->in_job_num == 1)) { //direct only start job can shutdown
			ife_eng_set_wdr_shutdown_enable_buf_reg(p_hdl->p_eng, ENABLE);
			ife_eng_set_wdr_sram_shutdown_buf_reg(p_hdl->p_eng, ENABLE);
			ife_eng_set_wdr_bypass_mode_buf_reg(p_hdl->p_eng, ENABLE);
		}
	} else {
		ife_eng_set_wdr_shutdown_enable_buf_reg(p_hdl->p_eng, DISABLE);
		ife_eng_set_wdr_sram_shutdown_buf_reg(p_hdl->p_eng, DISABLE);
		ife_eng_set_wdr_bypass_mode_buf_reg(p_hdl->p_eng, DISABLE);
	}

	if (!iq_cfg->gbal.enable && !io_cfg->subisp_param.subisp_out_en) {
		if (op_mode != IFE_OPMODE_ALL_DIRECT || (op_mode == IFE_OPMODE_ALL_DIRECT && io_cfg->in_job_num == 1)) { //direct only start job can shutdown
			ife_eng_set_gbal_shutdown_enable_buf_reg(p_hdl->p_eng, ENABLE);
			ife_eng_set_gbal_sram_shutdown_buf_reg(p_hdl->p_eng, ENABLE);
		}
	} else {
		ife_eng_set_gbal_shutdown_enable_buf_reg(p_hdl->p_eng, DISABLE);
		ife_eng_set_gbal_sram_shutdown_buf_reg(p_hdl->p_eng, DISABLE);
	}

	if (!iq_cfg->filt.enable) {
		if (op_mode != IFE_OPMODE_ALL_DIRECT || (op_mode == IFE_OPMODE_ALL_DIRECT && io_cfg->in_job_num == 1)) { //direct only start job can shutdown
			ife_eng_set_2dnr_shutdown_enable_buf_reg(p_hdl->p_eng, ENABLE);
			ife_eng_set_2dnr_sram_shutdown_buf_reg(p_hdl->p_eng, ENABLE);
		}
	} else {
		ife_eng_set_2dnr_shutdown_enable_buf_reg(p_hdl->p_eng, DISABLE);
		ife_eng_set_2dnr_sram_shutdown_buf_reg(p_hdl->p_eng, DISABLE);
	}


	//ife_eng_write_hw_reg(p_hdl->p_eng, 0x10, 0x00200000);//for 539a checksum
...
static INT32 kdrv_ife_trig_ll(KDRV_IFE_HANDLE *p_hdl, void *p_data)
{
...

	{
		...
		ife_eng_set_dma_ll_addr_buf_reg(p_hdl->p_eng, (UINT32)(addr & 0xFFFFFFFF), (UINT32)( (addr>> 32) & 0xFFFFFFFF));
	}
...
{
#if IFE_SSDRV_SUPPORT
	//ife_eng_set_dmaloop_line_count_clear_buf_reg(p_hdl->p_eng); //move to PRE
#endif
	return E_OK;
}
...
{
#if (IFE_SSDRV_SUPPORT && IFE_VA_SUPPORT)
...
static INT32 kdrv_ife_query_motion_buf_size(UINT32 id, void *p_data)
{
#if IFE_SSDRV_SUPPORT
	KDRV_IFE_MOTION_BUF_SIZE_INFO  *p_info;
	IFE_MOTION_MAP_SIZE_INFO  p_eng_info  = {0};
	IFE_MOTION_TYPE_SEL eng_motion_type = 3;

	p_info = (KDRV_IFE_MOTION_BUF_SIZE_INFO *)p_data;

	if(p_info->motion_type == KDRV_IFE_MOTION_TYPE_1_64)
		eng_motion_type = IFE_MOTION_TYPE_64;
	else if(p_info->motion_type == KDRV_IFE_MOTION_TYPE_1_16)
		eng_motion_type = IFE_MOTION_TYPE_16;
	else if(p_info->motion_type == KDRV_IFE_MOTION_TYPE_1_4)
		eng_motion_type = IFE_MOTION_TYPE_4;
	else
		DBG_ERR("unknown motion_type\n");


	ife_eng_get_motion_buffer_size(&p_eng_info, eng_motion_type, p_info->pre_in_width, p_info->pre_in_height);

	p_info->get_motion_map_size = p_eng_info.get_motion_map_size;
	p_info->get_motion_map_lofs = p_eng_info.get_motion_map_lofs;
...
static KDRV_IFE_QUERY_ITEM g_kdrv_ife_query_tab[KDRV_IFE_QUERY_ID_MAX] =
{
	...
	{kdrv_ife_query_motion_buf_size,	"motion buffer size"}
};
