...
static int nvt_kdrv_ife_probe_get_resource(struct platform_device *p_dev, NVT_KDRV_IFE_DRV_INFO *p_drv_info, int idx)
{
	...
	/* get clock resource */
	if(idx == 0)
		dev_resource.p_mclk = clk_get(&p_dev->dev, "2f0340000.ife");
...
static int nvt_kdrv_ife_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (nvt_get_suspend_mode()) { /* 0: not power off, 1: power off */
		...
		rt = kdrv_ife_suspend(KDRV_CHIP0, KDRV_VIDEOPROCS_IFE_ENGINE0);
		if (rt < 0) {
...
static int nvt_kdrv_ife_resume(struct platform_device *pdev)
{
	...
	if (nvt_get_suspend_mode()) { /* 0: not power off, 1: power off */
		...
		rt = kdrv_ife_resume(KDRV_CHIP0, KDRV_VIDEOPROCS_IFE_ENGINE0);
		if (rt < 0) {
			DBG_ERR("%s suspend fail\n", match->compatible);
			return -EINVAL;
		}
