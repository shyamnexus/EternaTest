...
#define IPP_ENG_538_STANDARD_RATE	(450000000)
#define IPP_ENG_539A_STANDARD_RATE	(500000000)
#define IPP_ENG_539A_OVER_STANDARD_RATE	(600000000)
	...
	if (nvt_get_chip_id() == CHIP_NS02402){
		...
		if(clk > IPP_ENG_539A_OVER_STANDARD_RATE){
			if (efuse_check_available_extend(EFUSE_ABILITY_VPE_FREQ, IPP_ENG_539A_OVER_STANDARD_RATE) == FALSE){
				if (efuse_check_available_extend(EFUSE_ABILITY_VPE_FREQ, IPP_ENG_539A_STANDARD_RATE) == FALSE){
					DBG_ERR("%s clock_rate=%d over spec(MAX: %dHz)", eng_name, (int)clk, (int)(IPP_ENG_538_STANDARD_RATE));
					clk = IPP_ENG_538_STANDARD_RATE;
				}else{
					DBG_ERR("%s clock_rate=%d over spec(MAX: %dHz)", eng_name, (int)clk, (int)(IPP_ENG_539A_STANDARD_RATE));
					clk = IPP_ENG_539A_STANDARD_RATE;
				}
			}else{
				DBG_ERR("%s clock_rate=%d over spec(MAX: %dHz)", eng_name, (int)clk, (int)(IPP_ENG_539A_OVER_STANDARD_RATE));
				clk = IPP_ENG_539A_OVER_STANDARD_RATE;
			}

		} else if(clk > IPP_ENG_539A_STANDARD_RATE){
			if (efuse_check_available_extend(EFUSE_ABILITY_VPE_FREQ, IPP_ENG_539A_OVER_STANDARD_RATE) == FALSE){
				if (efuse_check_available_extend(EFUSE_ABILITY_VPE_FREQ, IPP_ENG_539A_STANDARD_RATE) == FALSE){
					DBG_ERR("%s clock_rate=%d over spec(MAX: %dHz)", eng_name, (int)clk, (int)(IPP_ENG_538_STANDARD_RATE));
					clk = IPP_ENG_538_STANDARD_RATE;
		...
		} else if(clk > IPP_ENG_538_STANDARD_RATE){
			if (efuse_check_available_extend(EFUSE_ABILITY_VPE_FREQ, IPP_ENG_539A_STANDARD_RATE) == FALSE){
				DBG_ERR("%s clock_rate=%d over spec(MAX: %dHz)", eng_name, (int)clk, (int)(IPP_ENG_538_STANDARD_RATE));
				clk = IPP_ENG_538_STANDARD_RATE;
			}
