...
extern int nvt_kdrv_pre_module_init(void);
extern int nvt_kdrv_pre_module_exit(void);
...
//extern int nvt_kdrv_dre_module_init(void);
//extern int nvt_kdrv_dre_module_exit(void);
...
int __init kdrv_videoprocess_init(void)
{
	...
	if (nvt_get_chip_id() != CHIP_NS02402) {
		pr_err("invalid chip id, insmod wrong kdrv_videoprocess ko?\n");
		return -EINVAL;
	}
#else
	...
	if ((ret=nvt_kdrv_pre_module_init()) !=0 ) { //539A add
		return ret;
	}
	//if ((ret=nvt_kdrv_dce_module_init()) !=0 ) { //538 removed
	//	return ret;
	//}
...
//	if ((ret=nvt_kdrv_dre_module_init()) !=0 ) {
//		return ret;
//	}
	return 0;
}
...
void __exit kdrv_videoprocess_exit(void)
{
	...
	nvt_kdrv_pre_module_exit(); //539A add
	//nvt_kdrv_dce_module_exit(); //538 removed
	//nvt_kdrv_dre_module_exit();
}
