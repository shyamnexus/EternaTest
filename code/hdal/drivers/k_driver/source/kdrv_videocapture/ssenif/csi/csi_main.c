...
extern void                 ssenif_hook_csi3(void *object);
extern void                 ssenif_hook_csi4(void *object);
//extern void                 ssenif_hook_csi5(void *object);
...
static int nvt_csi_probe(struct platform_device *pdev)
{
	...
	nvt_dbg(WRN, "%s Version: %s\n", CSI_DRV_NAME, CSI_DRV_MODULE_VERSION);
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		if (ucloop)
			snprintf(pstr1, 16, "%08llx.csi%d", pdrv_info->presource[ucloop]->start, ucloop+1);
		else
			snprintf(pstr1, 16, "%08llx.csi", pdrv_info->presource[ucloop]->start);
	...
	//Get hs-clock source
	for (ucloop = 0 ; ucloop < MODULE_CSI_HS_CLK_NUM; ucloop++) {
		char clk_hsclk[8][11] = {"csi_ck0d4", "csi_ck1d4", "csi_ck2d4", "csi_ck3d4", \
			"csi2_ck1d4", "csi3_ck2d4", "csi3_ck3d4", "csi4_ck3d4"};

		pdrv_info->module_info.hsclk[ucloop] = clk_get(&pdev->dev, clk_hsclk[ucloop]);

		if (IS_ERR(pdrv_info->module_info.hsclk[ucloop])) {
			nvt_dbg(ERR, "faile to get csi hs-clock source (%s)\n", clk_hsclk[ucloop]);
			ret = -ENODEV;
			goto FAIL_FREE_REMAP;
		}
	}
	...
	if (fast_boot != 0) {
		...
		if(NVT_API_CHK_CALL(csi) != TRUE){
		    nvt_dbg(ERR, "objver mismatch: %s\r\n", "CSI");
		}
	} else {
...
static int nvt_csi_remove(struct platform_device *pdev)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_CSI_HS_CLK_NUM; ucloop++)
		clk_put(pdrv_info->module_info.hsclk[ucloop]);
...
int __init nvt_csi_module_init(void)
{
	...
	ssenif_hook_csi3((void *) csi_get_drv_object(CSI_ID_CSI3));
	ssenif_hook_csi4((void *) csi_get_drv_object(CSI_ID_CSI4));
	//ssenif_hook_csi5((void *) csi_get_drv_object(CSI_ID_CSI5));
...
void __exit nvt_csi_module_exit(void)
{
	...
	ssenif_hook_csi3(NULL);
	ssenif_hook_csi4(NULL);
	//ssenif_hook_csi5(NULL);
