...
int nvt_csi_drv_init(MODULE_INFO *pmodule_info/*, struct platform_device *pdev, unsigned char uc_if*/)
{
	...
	csi_create_resource();
	csi_set_base_addr((uintptr_t)pmodule_info->io_addr[0]);

	csi2_create_resource();
	csi2_set_base_addr((uintptr_t)pmodule_info->io_addr[1]);

	csi3_create_resource();
	csi3_set_base_addr((uintptr_t)pmodule_info->io_addr[2]);

	csi4_create_resource();
	csi4_set_base_addr((uintptr_t)pmodule_info->io_addr[3]);
...
int nvt_csi_drv_remove(MODULE_INFO *pmodule_info)
{
	...
	csi3_release_resource();
	csi4_release_resource();
...
irqreturn_t nvt_csi_drv_isr(int irq, void *devid)
{
	...
	if (pmodule_info->signature != CSI_SIGNATURE) {
		...
		} else if (irq == pmodule_info->iinterrupt_id[2]) {
			//nvt_dbg(IND, "irq3 = %d\r\n", irq);
			if (csi3_isr_check()) {
				csi3_isr();
				ret = IRQ_HANDLED;
		...
		} else if (irq == pmodule_info->iinterrupt_id[3]) {
			//nvt_dbg(IND, "irq4 = %d\r\n", irq);
			if (csi4_isr_check()) {
				csi4_isr();
				ret = IRQ_HANDLED;
