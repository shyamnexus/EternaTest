...
#if 0
int nvt_senphy_eoc_thread(void *__test)
{
	senphy_eoc_scan();
	senphy_eoc_set_trim();
	return 0;
...
int nvt_senphy_eoc_scan_thread(void)
{
	eoc_thread = kthread_run(nvt_senphy_eoc_thread, NULL, "nvt_senphy_eoc_thread");
	if (IS_ERR(eoc_thread)) {
		DBG_ERR("%s:  thread open error\n", __func__);
		return -1;
	}
	return 0;
}
