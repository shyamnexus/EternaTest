...
extern void                 ssenif_hook_lvds3(void *object);
extern void                 ssenif_hook_lvds4(void *object);
...
static int nvt_lvds_probe(struct platform_device *pdev)
{
	...
	nvt_dbg(WRN, "%s Version: %s\n", LVDS_DRV_NAME, LVDS_DRV_MODULE_VERSION);
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		if (ucloop)
			snprintf(pstr1, 16, "%08llx.lvds%d", pdrv_info->presource[ucloop]->start, ucloop+1);
		else
			snprintf(pstr1, 16, "%08llx.lvds", pdrv_info->presource[ucloop]->start);
...
int __init nvt_lvds_module_init(void)
{
	...
	ssenif_hook_lvds3((void *) lvds_get_drv_object(LVDS_ID_LVDS3));
	ssenif_hook_lvds4((void *) lvds_get_drv_object(LVDS_ID_LVDS4));
...
void __exit nvt_lvds_module_exit(void)
{
	...
	ssenif_hook_lvds3(NULL);
	ssenif_hook_lvds4(NULL);
