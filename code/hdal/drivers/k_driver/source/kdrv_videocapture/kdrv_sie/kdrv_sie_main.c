#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/moduleparam.h>
#include <linux/platform_device.h>
#include <linux/fs.h>
#include <linux/interrupt.h>
#include <linux/vmalloc.h>
#include <linux/slab.h>
#include <linux/sched.h>
#include <linux/io.h>
#include <linux/of_device.h>
#include <linux/kdev_t.h>
#include <linux/clk.h>
#include <asm/signal.h>
#include <kwrap/dev.h>
#include "kdrv_sie_main.h"
#include "kdrv_sie_proc.h"
#include "kdrv_sie_int.h"
#include "kdrv_sie_dbg.h"
#include <linux/soc/nvt/nvtmem.h>


//=============================================================================
//Module parameter : Set module parameters when insert the module
//=============================================================================
#ifdef DEBUG
unsigned int kdrv_sie_debug_level = (NVT_DBG_IND | NVT_DBG_WRN | NVT_DBG_ERR);
module_param_named(kdrv_sie_debug_level, kdrv_sie_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(kdrv_sie_debug_level, "Debug message level");
#endif

//=============================================================================
// function declaration
//=============================================================================
int __init nvt_kdrv_sie_module_init(void);
void __exit nvt_kdrv_sie_module_exit(void);

#if 0
#endif
//=============================================================================
// function define
//=============================================================================

#if defined(_GROUP_KO_)
#undef __init
#undef __exit
#undef module_init
#undef module_exit
#define __init
#define __exit
#define module_init(x)
#define module_exit(x)
#endif



#if 0
#if 0
#endif
//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id kdrv_sie_match_table[] = {
	{   .compatible = MODULE_NAME_COMPTB},
	{}
};

//=============================================================================
// function define
//=============================================================================
static int nvt_kdrv_sie_open(struct inode *inode, struct file *file)
{
	return 0;
}

static int nvt_kdrv_sie_release(struct inode *inode, struct file *file)
{
	return 0;
}

static long nvt_kdrv_sie_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	return 0;
}

struct file_operations nvt_kdrv_sie_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_kdrv_sie_open,
	.release = nvt_kdrv_sie_release,
	.unlocked_ioctl = nvt_kdrv_sie_ioctl,
	.llseek  = no_llseek,
};

#if 0
#endif

static CHAR *clk_name[KDRV_SIE_ENG_COUNT] = {
	MODULE_NAME_CLK_DEV_1,
	MODULE_NAME_CLK_DEV_2,
	MODULE_NAME_CLK_DEV_3,
	MODULE_NAME_CLK_DEV_4,
	MODULE_NAME_CLK_DEV_5,
};

static CHAR *pclk_name[KDRV_SIE_ENG_COUNT] = {
	MODULE_NAME_PCLK_DEV_1,
	MODULE_NAME_PCLK_DEV_2,
	MODULE_NAME_PCLK_DEV_3,
	MODULE_NAME_PCLK_DEV_4,
	MODULE_NAME_PCLK_DEV_5
};

static CHAR *pxclk_name[KDRV_SIE_ENG_COUNT] = {
	MODULE_NAME_PXCLK_DEV_1,
	NULL,
	MODULE_NAME_PXCLK_DEV_3,
	NULL,
	MODULE_NAME_PXCLK_DEV_5
};

static CHAR *intclk_name[KDRV_SIE_ENG_COUNT] = {
	NULL,
	MODULE_NAME_INTCLK_DEV_2,
	NULL,
	MODULE_NAME_INTCLK_DEV_4,
	NULL
};


static CHAR *pxclkpad_name[KDRV_SIE_PXCLKPAD_COUNT] = {
	NULL,
	NULL,
	NULL,
	NULL,
	NULL
};

static CHAR *mclk_name[KDRV_SIE_MCLK_COUNT] = {
	MODULE_NAME_MCLK_1,
	MODULE_NAME_MCLK_2,
	MODULE_NAME_MCLK_3,
	MODULE_NAME_MCLK_4
};

static int nvt_kdrv_sie_probe_get_resource(struct platform_device *p_dev, KDRV_SIE_DRV_INFO *p_drv_info, int idx)
{
	KDRV_SIE_RESOURCE dev_resource;
	struct resource *p_resource;
	int ret = 0;
	UINT32 i;

	memset((void *)&dev_resource, 0, sizeof(KDRV_SIE_RESOURCE));

	/* register your own device in sysfs, and this will cause udev to create corresponding device node */
	p_drv_info->p_device[idx] = device_create(p_drv_info->pclass, NULL
									 , MKDEV(MAJOR(p_drv_info->dev_id), (idx + MINOR(p_drv_info->dev_id))), NULL
									 , MODULE_NAME"%d", idx);

	if (IS_ERR(p_drv_info->p_device[idx])) {
		DBG_ERR("failed in creating device%d.\n", (int)idx);
		ret = -ENODEV;
		goto FAIL_NONE;
	}

	/* get resource */
	p_resource = platform_get_resource(p_dev, IORESOURCE_MEM, idx);
	if (p_resource == NULL) {
		DBG_ERR("No IO memory resource defined:%d\n", (int)idx);
		ret = -ENODEV;
		goto FAIL_DEVICE_UNREG;
	}

	if (!request_mem_region(p_resource->start, resource_size(p_resource), p_dev->name)) {
		DBG_ERR("failed to request memory resource%d\n", (int)idx);
		ret = -ENODEV;
		goto FAIL_DEVICE_UNREG;
	}

	dev_resource.phy_io_base = p_resource->start;
	dev_resource.phy_io_size = resource_size(p_resource);
	dev_resource.p_io_addr = ioremap(dev_resource.phy_io_base, dev_resource.phy_io_size);
	if (dev_resource.p_io_addr == NULL) {
		DBG_ERR("ioremap() failed in module%d\n", (int)idx);
		ret = -ENODEV;
		goto FAIL_RELEASE_MEM;
	}

	/* get irq id */
	dev_resource.irq_id = platform_get_irq(p_dev, idx);
	if (dev_resource.irq_id < 0) {
		DBG_ERR("idx %d No IRQ resource defined\n", idx);
		ret = -ENODEV;
		goto FAIL_IOUNMAP;
	}

	/* get clock resource */
	// module clk
	dev_resource.clk = devm_clk_get(&p_dev->dev, clk_name[idx]);
	if (IS_ERR(dev_resource.clk)) {
		DBG_ERR("idx %d fail to get %s source\n", idx, clk_name[idx]);
		ret = -ENODEV;
		goto FAIL_IOUNMAP;
	}
	// pclk
	dev_resource.pclk = devm_clk_get(&p_dev->dev, pclk_name[idx]);
	if (IS_ERR(dev_resource.pclk)) {
		DBG_ERR("idx %d fail to get %s source\n", idx, pclk_name[idx]);
		ret = -ENODEV;
		goto FAIL_IOUNMAP;
	}
	// pxclk
	dev_resource.pxclk = devm_clk_get(&p_dev->dev, pxclk_name[idx]);
	if (IS_ERR(dev_resource.pxclk)) {
		DBG_ERR("idx %d fail to get %s source\n", idx, pxclk_name[idx]);
		ret = -ENODEV;
		goto FAIL_IOUNMAP;
	}
	// pxclk_pad
	for (i = 0; i < KDRV_SIE_PXCLKPAD_COUNT; i++) {
		if (pxclkpad_name[i] != NULL) {
			dev_resource.pxclk_pad[i] = devm_clk_get(&p_dev->dev, pxclkpad_name[i]);
			if (IS_ERR(dev_resource.pxclk_pad[i])) {
				DBG_ERR("idx %d fail to get %s source\n", i, pxclkpad_name[i]);
				ret = -ENODEV;
				goto FAIL_IOUNMAP;
			}
		}
	}
	// mclk
	for (i = 0; i < KDRV_SIE_MCLK_COUNT; i++) {
		dev_resource.mclk[i] = devm_clk_get(&p_dev->dev, mclk_name[i]);
		if (IS_ERR(dev_resource.mclk[i])) {
			DBG_ERR("idx %d fail to get %s source\n", i, mclk_name[i]);
			ret = -ENODEV;
			goto FAIL_IOUNMAP;
		}
	}

	// int clk
	if (intclk_name[idx] != NULL) {
		dev_resource.intclk = devm_clk_get(&p_dev->dev, intclk_name[idx]);
		if (IS_ERR(dev_resource.intclk)) {
			DBG_ERR("idx %d fail to get %s source\n", idx, intclk_name[idx]);
			ret = -ENODEV;
			goto FAIL_IOUNMAP;
		}
	}

	dev_resource.reg_addr_va 	= kdrv_sie_os_malloc_wrap(SIE_ENG_REG_NUM << 2);
	dev_resource.flag_addr_va 	= kdrv_sie_os_malloc_wrap(sizeof(UINT8) * SIE_ENG_REG_NUM);

	/* set resource */
	kdrv_sie_resource_set(&dev_resource, idx);

#if 0
	DBG_ERR("\r\n[dump info] - BEGIN\r\n");
	DBG_DUMP("[%-12s] %d \r\n", 	"idx", 			idx);
	DBG_DUMP("[%-12s] %d \r\n", 	"irq_id", 		dev_resource.irq_id);
	DBG_DUMP("[%-12s] %lx \r\n", 	"p_io_addr", 	(ULONG)dev_resource.p_io_addr);
	DBG_DUMP("[%-12s] %lx \r\n", 	"phy_io_base", 	(ULONG)dev_resource.phy_io_base);
	DBG_DUMP("[%-12s] %lx \r\n", 	"phy_io_size", 	(ULONG)dev_resource.phy_io_size);
	DBG_DUMP("[%-12s] %d \r\n", 	"phy_io_size", 	dev_resource.phy_io_size);
	DBG_DUMP("[%-12s] %lx \r\n", 	"reg_addr_va",  (ULONG)dev_resource.reg_addr_va);
	DBG_DUMP("[%-12s] %lx \r\n", 	"flag_addr_va", (ULONG)dev_resource.flag_addr_va);
	DBG_ERR("[dump info] - END\r\n");
#endif
	return ret;

FAIL_IOUNMAP:
	iounmap(dev_resource.p_io_addr);

FAIL_RELEASE_MEM:
	release_mem_region(p_resource->start, resource_size(p_resource));

FAIL_DEVICE_UNREG:
	device_unregister(p_drv_info->p_device[idx]);

FAIL_NONE:
	return ret;
}

static int nvt_kdrv_sie_remove_rls_resource(struct platform_device *p_dev, KDRV_SIE_DRV_INFO *p_drv_info, int idx)
{
	KDRV_SIE_RESOURCE *p_dev_resource;
	UINT32 i;

	p_dev_resource = kdrv_sie_resource_get(idx);
	if (p_dev_resource) {
		release_mem_region(p_dev_resource->phy_io_base, p_dev_resource->phy_io_size);
		iounmap(p_dev_resource->p_io_addr);
		if (p_dev_resource->clk) {
			devm_clk_put(&p_dev->dev, p_dev_resource->clk);
		}

		if (p_dev_resource->pclk) {
			devm_clk_put(&p_dev->dev, p_dev_resource->pclk);
		}

		if (p_dev_resource->pxclk) {
			devm_clk_put(&p_dev->dev, p_dev_resource->pxclk);
		}

		if (p_dev_resource->intclk) {
			devm_clk_put(&p_dev->dev, p_dev_resource->intclk);
		}

		for (i = 0; i < KDRV_SIE_PXCLKPAD_COUNT; i++) {
			if (p_dev_resource->pxclk_pad[i]) {
				devm_clk_put(&p_dev->dev, p_dev_resource->pxclk_pad[i]);
			}
		}

		for (i = 0; i < KDRV_SIE_MCLK_COUNT; i++) {
			if (p_dev_resource->mclk[i]) {
				devm_clk_put(&p_dev->dev, p_dev_resource->mclk[i]);
			}
		}

		if (p_dev_resource->flag_addr_va) {
			kdrv_sie_os_mfree_wrap(p_dev_resource->flag_addr_va);
		}

		if (p_dev_resource->reg_addr_va) {
			kdrv_sie_os_mfree_wrap(p_dev_resource->reg_addr_va);
		}
	}

	if (p_drv_info && p_drv_info->p_device[idx]) {
		device_unregister(p_drv_info->p_device[idx]);
	}
	return 0;
}
static int nvt_kdrv_sie_probe(struct platform_device *pdev)
{
	KDRV_SIE_DRV_INFO *pdrv_info = NULL;
	const struct of_device_id *match;
	int ret = 0;
	unsigned char ucloop;

	match = of_match_device(kdrv_sie_match_table, &pdev->dev);
	if (!match) {
		DBG_ERR("Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(KDRV_SIE_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
		DBG_ERR("failed to allocate memory\n");
		return -ENOMEM;
	}

	/* Dynamic to allocate Device ID */
	if (vos_alloc_chrdev_region(&pdrv_info->dev_id, KDRV_SIE_TOTAL_CH_COUNT, MODULE_NAME)) {
		DBG_ERR("Can't get device ID\n");
		ret = -ENODEV;
		goto FAIL_FREE_BUF;
	}

	/* Register character device for the volume */
	cdev_init(&pdrv_info->cdev, &nvt_kdrv_sie_fops);
	pdrv_info->cdev.owner = THIS_MODULE;

	if (cdev_add(&pdrv_info->cdev, pdrv_info->dev_id, KDRV_SIE_TOTAL_CH_COUNT)) {
		DBG_ERR("Can't add cdev\n");
		ret = -ENODEV;
		goto FAIL_CDEV;
	}

	pdrv_info->pclass = class_create(THIS_MODULE, MODULE_NAME_CLASS);
	if (IS_ERR(pdrv_info->pclass)) {
		DBG_ERR("failed in creating class.\n");
		ret = -ENODEV;
		goto FAIL_CDEV;
	}

	ret = nvt_kdrv_sie_proc_init(pdrv_info);
	if (ret) {
		DBG_ERR("failed in creating proc.\n");
		goto FAIL_CLASS;
	}

	/* alloc kdrv_sie_init memory */
	kdrv_sie_resource_init();

	/* request resources */
	for (ucloop = 0 ; ucloop < KDRV_SIE_TOTAL_CH_COUNT; ucloop++) {
		ret = nvt_kdrv_sie_probe_get_resource(pdev, pdrv_info, ucloop);
		if (ret) {
			while (ucloop > 0) {
				ucloop--;
				nvt_kdrv_sie_remove_rls_resource(pdev, pdrv_info, ucloop);
			}
			ret = -ENODEV;
			goto FAIL_PROC;
		}
	}
	platform_set_drvdata(pdev, pdrv_info);
	kdrv_sie_module_init();

	return ret;

FAIL_PROC:
	nvt_kdrv_sie_proc_remove(pdrv_info);

FAIL_CLASS:
	class_destroy(pdrv_info->pclass);

FAIL_CDEV:
	cdev_del(&pdrv_info->cdev);
	vos_unregister_chrdev_region(pdrv_info->dev_id, KDRV_SIE_MINOR_COUNT);

FAIL_FREE_BUF:
	if (pdrv_info != NULL) {
		kfree(pdrv_info);

		// coverity[assigned_pointer]
		pdrv_info = NULL;
	}

	return ret;
}

static int nvt_kdrv_sie_remove(struct platform_device *pdev)
{
	PKDRV_SIE_DRV_INFO pdrv_info = NULL;
	unsigned char ucloop;

	pdrv_info = platform_get_drvdata(pdev);

	kdrv_sie_module_uninit();

	for (ucloop = 0 ; ucloop < KDRV_SIE_TOTAL_CH_COUNT; ucloop++) {
		nvt_kdrv_sie_remove_rls_resource(pdev, pdrv_info, ucloop);
	}
	kdrv_sie_resource_uninit();



	nvt_kdrv_sie_proc_remove(pdrv_info);
	class_destroy(pdrv_info->pclass);
	cdev_del(&pdrv_info->cdev);
	vos_unregister_chrdev_region(pdrv_info->dev_id, KDRV_SIE_TOTAL_CH_COUNT);

	// coverity[check_after_deref]
	if (pdrv_info != NULL) {
		kfree(pdrv_info);

		// coverity[assigned_pointer]
		pdrv_info = NULL;
	}

	return 0;
}

static struct platform_driver nvt_kdrv_sie_driver = {
	.driver = {
		.name = MODULE_NAME,
		.of_match_table = kdrv_sie_match_table,
	},
	.probe = nvt_kdrv_sie_probe,
	.remove = nvt_kdrv_sie_remove,
};

int __init nvt_kdrv_sie_init(void)
{
	return platform_driver_register(&nvt_kdrv_sie_driver);
}

void __exit nvt_kdrv_sie_exit(void)
{
	platform_driver_unregister(&nvt_kdrv_sie_driver);
}
#else
KDRV_SIE_DRV_INFO *pdrv_info = NULL;
int __init nvt_kdrv_sie_init(void)
{
	int ret;

	pdrv_info = kzalloc(sizeof(KDRV_SIE_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
		kdrv_sie_dbg_err("allocate mem fail\n");
		return -ENOMEM;
	}

	ret = nvt_kdrv_sie_proc_init(pdrv_info);
	if(ret) {
		kdrv_sie_dbg_err("failed in creating proc.\n");
		goto FAIL_PROC;
	}

	kdrv_sie_init();

	return ret;

FAIL_PROC:
	nvt_kdrv_sie_proc_remove(pdrv_info);
	kfree(pdrv_info);
	pdrv_info = NULL;
	return 0;
}

void __exit nvt_kdrv_sie_exit(void)
{
	nvt_kdrv_sie_proc_remove(pdrv_info);
	kdrv_sie_uninit();

	kfree(pdrv_info);
	pdrv_info = NULL;
}
#endif

module_init(nvt_kdrv_sie_init);
module_exit(nvt_kdrv_sie_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("SIE Kdriver");
MODULE_LICENSE("GPL");
