...
typedef struct proc_cmd {
	...
	int (*execute)(PKDRV_SIE_MODULE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
static PKDRV_SIE_DRV_INFO pkdrv_info_data;
...
#if defined(__LINUX)
int nvt_kdrv_sie_proc_buffer_to_int(const char __user *buffer, size_t count, int *dst, int dst_cnt)
{
#define PARAM_LENGTH 100
	...
	if ((count > PARAM_LENGTH) || (count < 2)) {
		kdrv_sie_dbg_err("cmd length(%zd) must <%d and >2\n", count, PARAM_LENGTH);
		return -1;
	}

	memset(buf, 0, PARAM_LENGTH);
	if (copy_from_user(buf, buffer, PARAM_LENGTH)) {
		kdrv_sie_dbg_err("convert fail1\n");
		return -1;
	}
	...
	while (scan_pos < end_pos)
	{
		...
		//scan fail
		if (n == 0) {
			kdrv_sie_dbg_err("convert fail2\n");
			return -1;
		}
...
static struct proc_ops proc_info_fops = {
	.proc_open   	= nvt_kdrv_sie_proc_info_open,
...
static int nvt_kdrv_sie_proc_dbg_lvl_show(struct seq_file *sfile, void *v)
{
	pkdrv_info_data->info_seq_file = sfile;
	return 0;
}

static ssize_t nvt_kdrv_sie_proc_dbg_lvl_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
#define DATA_SIZE 1
	int data[DATA_SIZE] = {0};

	if (nvt_kdrv_sie_proc_buffer_to_int(buffer, count, &data[0], DATA_SIZE) != DATA_SIZE) {
		kdrv_sie_dbg_err("input parameter error\n");
		return -1;
	}
	kdrv_sie_dbg_dump("0: fatal, 1: err, 2: wrn, 3: msg, 4: ind, 5: func\n");
	kdrv_sie_dbg_dump("kdrv_sie dbglevel = %d\n", data[0]);
	kdrv_sie_dbg_set_dbg_level(data[0]);

#undef DATA_SIZE
	return count;
}

static int nvt_kdrv_sie_proc_dbg_lvl_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_kdrv_sie_proc_dbg_lvl_show, NULL);
}

static struct proc_ops proc_dbg_lvl_fops = {
	.proc_open   	= nvt_kdrv_sie_proc_dbg_lvl_open,
	.proc_release 	= single_release,
	.proc_read   	= seq_read,
	.proc_lseek 	= seq_lseek,
    .proc_write   	= nvt_kdrv_sie_proc_dbg_lvl_write,
};
...
static int nvt_kdrv_sie_proc_gating_show(struct seq_file *sfile, void *v)
{
	pkdrv_info_data->info_seq_file = sfile;
	return 0;
}

static ssize_t nvt_kdrv_sie_proc_gating_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
#define DATA_SIZE 1
	int data[DATA_SIZE] = {0};

	if (nvt_kdrv_sie_proc_buffer_to_int(buffer, count, &data[0], DATA_SIZE) != DATA_SIZE) {
		kdrv_sie_dbg_dump("input parameter error\n");
		return -1;
	}

	kdrv_sie_dbg_dump("0: disable, 1: enable\n");
	kdrv_sie_dbg_dump("gating_en = %d\n", data[0]);
...
static int nvt_kdrv_sie_proc_gating_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_kdrv_sie_proc_gating_show, NULL);
}

static struct proc_ops proc_gating_fops = {
	.proc_open   	= nvt_kdrv_sie_proc_gating_open,
	.proc_release 	= single_release,
	.proc_read   	= seq_read,
	.proc_lseek 	= seq_lseek,
    .proc_write   	= nvt_kdrv_sie_proc_gating_write,
};
...
static ssize_t nvt_kdrv_sie_proc_cmd(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	...
	if (len > (MAX_CMD_LENGTH - 1)) {
		kdrv_sie_dbg_ind("Command length is too long!\n");
		goto ERR_OUT;
	}
...
static struct proc_ops proc_cmd_fops = {
    .proc_open    	= nvt_kdrv_sie_proc_cmd_open,
    .proc_read    	= seq_read,
    .proc_lseek  	= seq_lseek,
    .proc_release 	= single_release,
    .proc_write   	= nvt_kdrv_sie_proc_cmd
};
...
static struct proc_ops proc_help_fops = {
    .proc_open   	= nvt_kdrv_sie_proc_help_open,
    .proc_release 	= single_release,
    .proc_read   	= seq_read,
    .proc_lseek 	= seq_lseek,
};
