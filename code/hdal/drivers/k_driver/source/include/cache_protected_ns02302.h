/*
    Novatek protected header file of NT96650's driver.

    The header file for Novatek protected APIs of NT96650's driver.

    @file       cache_protected.h
    @ingroup    mIDriver
    @note       For Novatek internal reference, don't export to agent or customer.

    Copyright   Novatek Microelectronics Corp. 2012.  All rights reserved.
*/

#ifndef _NVT_CACHE_PROTECTED_H
#define _NVT_CACHE_PROTECTED_H

#include <kwrap/nvt_type.h>
//#include "timer.h"
//#include "cc.h"



#define CA53_DCACHE_SIZE                32768
#define CA53_ICACHE_SIZE                32768
#define CACHE_FLUSHALL_BOUNDARY         (CA53_DCACHE_SIZE<<1)

#define CACHE_SET_NUMBER_SHIFT          5           //64 bytes per set

extern void     cpu_invalidateICacheAll(void);
extern void     cpu_invalidateICache(ULONG addr);
extern void     cpu_invalidateICacheBlock(ULONG start, ULONG end);
extern void     cpu_invalidateDCacheBlock(ULONG start, ULONG end);
extern void     cpu_invalidateDCache(ULONG addr);
extern void     cpu_invalidateDCacheAll(void);
extern void     cpu_cleanDCache(ULONG addr);
extern void     cpu_cleanDCacheBlock(ULONG start, ULONG end);
extern void     cpu_cleanInvalidateDCacheAll(void);
extern void     cpu_cleanInvalidateDCache(ULONG addr);
extern void     cpu_cleanInvalidateDCacheBlock(ULONG start, ULONG end);
extern void     cpu_initCacheConfiguration(void);
extern void     cpu_showCacheConfiguration(void);
extern void 	cpu_v7_outer_cache_disable(void);
extern void 	cpu_v7_outer_cache_enable(void);
extern void 	cpu_invalidateDCacheL1(void);
extern void 	cpu_invalidateDCacheL2(void);


extern int 		icache_status(void);
extern int 		dcache_status(void);
extern void 	icache_enable(void);
extern void 	icache_disable(void);
extern void 	dcache_enable(void);
extern void 	dcache_disable(void);

#endif

