...
typedef enum {
	...
	NAND_PRIMARY_ECC_BCH,                       	//<BCH Code ECC
...
typedef enum {
	NANDCTRL_SPIFLASH_SPARE_16_BYTE = 0x0,
	NANDCTRL_SPIFLASH_SPARE_32_BYTE,
	NANDCTRL_SPIFLASH_SPARE_0_BYTE,

	ENUM_DUMMY4WORD(NAND_SPARE_SIZE_SEL)
} NAND_SPARE_SIZE_SEL;

typedef enum {
	NANDCTRL_SPIFLASH_SPARE_ALIGN_SECTOR = 0x0,     //  3-byte user data and  13-byte ecc code per sector
	NANDCTRL_SPIFLASH_SPARE_ALIGN_PAGE,             // 12-byte user data and  52-byte ecc code per 2k-page
	                                                // 24-byte user data and 104-byte ecc code per 4k-page

	ENUM_DUMMY4WORD(NAND_SPARE_ALIGN_SEL)
} NAND_SPARE_ALIGN_SEL;

typedef enum {
	NANDCTRL_SPIFLASH_USE_1_BIT = 0x0,
	NANDCTRL_SPIFLASH_USE_2_BIT,
	NANDCTRL_SPIFLASH_USE_4_BIT,
	NANDCTRL_SPIFLASH_USE_8_BIT,

	ENUM_DUMMY4WORD(NAND_SPINAND_BIT_SEL)
} NAND_SPINAND_BIT_SEL;

typedef enum {
	NANDCTRL_MULTIPAGE_NORMAL = 0x0,  // Normal mode, [(0x13)-(poll sts)-(read)] loop
	NANDCTRL_MULTIPAGE_SEQUENTAIL,    // Sequential cache read (MXIC, GD)
	NANDCTRL_MULTIPAGE_WB_CONT,       // Winbond Continuous read, cmd-addr-[page loop] (on-die ecc)
	NANDCTRL_MULTIPAGE_WB_SEQ,        // Winbond Sequential read, cmd-addr-[(page-oob) loop] (host ecc)

	ENUM_DUMMY4WORD(NAND_MULTI_PAGE_OPTION)
} NAND_MULTI_PAGE_OPTION;
...
extern void             emu_dump_write(uintptr_t uiData, UINT32 uiSize);
