...
/* Global variable                                                           */
/*===========================================================================*/
uintptr_t _USB_CHRGDET_REG_BASE_ADDR[MODULE_REG_NUM];
...
int nvt_usb_chrgdet_drv_init(USB_CHRGDET_MODULE_INFO *pmodule_info)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_REG_NUM; ucloop++) {
		_USB_CHRGDET_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_addr[ucloop];
		usb_chrgdet_api("USB_CHRGDET[%d]-Addr = 0x%08lX\n", ucloop, _USB_CHRGDET_REG_BASE_ADDR[ucloop]);
	}
