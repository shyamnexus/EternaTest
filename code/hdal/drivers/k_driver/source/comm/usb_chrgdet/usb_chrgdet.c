...
extern uintptr_t _USB_CHRGDET_REG_BASE_ADDR[MODULE_REG_NUM];
extern PUSB_CHRGDET_DRV_INFO pusb_chrgdet_drv_info_data;

#define USB_REG_ADDR(ofs)	(_USB_CHRGDET_REG_BASE_ADDR[0]+(ofs))
#define USB_SETREG(ofs, value)	OUTW(USB_REG_ADDR(ofs), (value))
#define USB_GETREG(ofs)			INW(USB_REG_ADDR(ofs))

#define USB_PHY_ADDR(ofs)	(_USB_CHRGDET_REG_BASE_ADDR[1]+(ofs<<2))
#define USB2PHY_SETREG(ofs, value)	OUTW(USB_PHY_ADDR(ofs), (value))
#define USB2PHY_GETREG(ofs)			INW(USB_PHY_ADDR(ofs))
...
INT32 __usb_chrgdet_set(USB_CHRGDET_PORT usb_port, USB_CHRGDET_PARAM_ID param_id, VOID *p_param)
{
	...
	case USB_CHRGDET_TYPE_STD: {
		...
		if(p_param!=NULL)
			usb_chrgdet_standard[ucloop] = 1;
		else
			usb_chrgdet_standard[ucloop] = 0;
	} break;
