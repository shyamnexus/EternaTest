...
#define SPI3_SETREG(ofs,value)      OUTW(spi3_getRegAddr() + ofs,(value))
#define SPI3_GETREG(ofs)            INW(spi3_getRegAddr() + ofs)
...
//#define SPI_GYRO_QUEUE_DEPTH        (SPI_GYRO_TRANSFER_MAX_PIO * SPI_GYRO_TRSLEN_MAX * 3+1)
...
typedef struct {
	...
	ULONG  uiReg[SPI_GYRO_TRS_INT_REG_NUM];
...
typedef struct {
    ULONG uiBufAddr;       //current buffer addr (for dma mode or pio mode)
    ULONG uiNextBufAddr;   //next buffer addr for dma mode (for dma mode only)
