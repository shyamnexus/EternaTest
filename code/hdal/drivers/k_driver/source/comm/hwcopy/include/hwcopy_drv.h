#ifndef __HWCOPY_DRV_H__
#define __HWCOPY_DRV_H__
#include <linux/io.h>
...
typedef struct module_info {
	struct completion hwcopy_completion;
	struct semaphore hwcopy_sem;
	struct clk *pclk[MODULE_CLK_NUM];
	struct tasklet_struct hwcopy_tasklet[MODULE_IRQ_NUM];
	void __iomem *io_addr[MODULE_REG_NUM];
	int iinterrupt_id[MODULE_IRQ_NUM];
	wait_queue_head_t hwcopy_wait_queue;
	spinlock_t hwcopy_spinlock;
	int chip_idx;
...
int nvt_hwcopy_drv_init(PMODULE_INFO pmodule_info);
int nvt_hwcopy_drv_remove(PMODULE_INFO pmodule_info);
