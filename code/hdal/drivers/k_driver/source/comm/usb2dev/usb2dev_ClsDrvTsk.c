...
void usb_fifoempty_interrupt(void);
...
void usb_fifoempty_interrupt(void)
{
	...
	UINT32 epbusy_enint_bit;

	/* check vdma cmplt and fifo empty interrupt */
	epbusy.reg = USB_GETREG(USB_DEVVMDA_CMPLT_EMP_FXINT_REG_OFS);
	epbusymask.reg = USB_GETREG(USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG_OFS);

	epbusy_enint_bit = (~epbusymask.bit.VDMA_CMPLT_EMP_FXINTMASK) & 0xFFFF;
	// need wait correspond fifo empty for making sure ep free
	if (epbusy_enint_bit) {
		// if interrupt was caused by vdma cmplt and fifo empty
		if ((epbusy.bit.VDMA_CMPLT_EMP_FXINT & epbusy_enint_bit) == epbusy_enint_bit) {
			USB_SETREG(USB_DEVVMDA_CMPLT_EMP_FXINT_REG_OFS, epbusy.reg);
			set_flg(FLG_ID_USBTSK, FLGPTN_USB_EP_FREE);
		}
	}
