...
static void ReqGetDescriptor(void)
{
	...
	if (DescType == USB_DEVICE_DESCRIPTOR_TYPE) {
		...
		pDataAddr = (uintptr_t *)pUSBMng->p_dev_desc;
	}
	//----- configuration descriptor ----------------
		...
		if (DescIdx >= pUSBMng->num_of_configurations) {
			...
			pDataAddr   = (uintptr_t *)pUSBMng->p_config_desc;
	...
	//----- string descriptor ------------------------
		...
		pDataAddr = (uintptr_t *)pUSBMng->p_string_desc[DescIdx];
	}
	//full speed only device report as it is USB 1.1
	...
	//----- Device_Qualifier descriptor ------------------------
		...
		pDataAddr = (uintptr_t *)pUSBMng->p_dev_quali_desc;
	}
	//-----Other_Speed_Configuration ------------------------
		...
		pDataAddr = (uintptr_t *)(gUSBManage.p_config_others_desc);
		//pDscptr = (UINT8 *)(pUSBMng->p_config_desc);
