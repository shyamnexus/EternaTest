...
irqreturn_t usb_isr(int irq, void *devid)
{
	...
	/* fifo empty interrupt check */
	usb_fifoempty_interrupt();
...
void usb_initFOTG200(void)
{
	...
	USB_SETREG(USB_DEVINTMASKGROUP1_REG_OFS, 0xFFFFFFFF);

	// enable response mode
	axi_moni.reg = USB_GETREG(USB_AXI_MONI_REG_OFS);
	axi_moni.bit.BUF_RESP_MODE = 1;
	USB_SETREG(USB_AXI_MONI_REG_OFS, axi_moni.reg);

	// enable axi monitor bus
	axi_moni.reg = USB_GETREG(USB_AXI_MONI_REG_OFS);
	axi_moni.bit.MONI_CH_DIS = 0;
	USB_SETREG(USB_AXI_MONI_REG_OFS, axi_moni.reg);
...
static ER usb_openInternal(BOOL bChkCharger, UINT32 uiDebounce)
{
...
#ifdef CONFIG_UVC_SUSPEND
	UINT32                      pcontext=0;
	static BOOL                 first_resume = 0;
	T_USB_PHYTSTSELECT_REG      devplug;
	...
	USB_DELAY_US_POLL(64);

	devplug.reg = USB_GETREG(USB_PHYTSTSELECT_REG_OFS);
	pcontext = !devplug.bit.UNPLUG;
#endif
...
#else
{
	...
	loc_cpu(flags);
	iowrite32( (ioread32((void *)(0xF0580140)) & ~(0x1<<4)), (void *)(0xF0580140));
	iowrite32( (ioread32((void *)(0xF0580094)) & ~(0x1<<25)), (void *)(0xF0580094));
	iowrite32( (ioread32((void *)(0xF0580094)) |  (0x1<<25)), (void *)(0xF0580094));
	iowrite32( (ioread32((void *)(0xF0580140)) |  (0x1<<4)), (void *)(0xF0580140));
	unl_cpu(flags);
}
...
void usb2dev_power_on_init(BOOL bHighSpeed)
{
	   ...
	   */
	{
		...
		#if 1//_USB2DEV_TODO_
		{
			...
			if(result >= 0) {
				DBG_DUMP("[530] USB = 0x%04x\r\n", (int)data);

				if ((TRIM_RESINT_LOWER <= (data&0x1F)) && ((data&0x1F) <= TRIM_RESINT_UPPER)) {//check if trim_resint fall in valid range
					RESINT = data&0x1F;
					DBG_DUMP("[530]]USB Trim data(12K) = 0x%04x\r\n", RESINT);
				} else {
					DBG_ERR("[530] data is out of valid range\n");
				}
			} else {
				DBG_ERR("[530] data = NULL\n");
			}

		}
		#endif
		//select PHY internal rsel value
		temp = U3PHY_GETREG(0x684);
		temp &= ~(0x80);
		U3PHY_SETREG(0x684, temp);

		//r45 trim setting
		U3PHY_SETREG(0x680, 0x40 + RESINT);
		U3PHY_SETREG(0x68C, 0x60);

		//best setting
		U3PHY_SETREG(0x158, 0x74);
		U3UTMIPHY_SETREG(0x24, 0xB0);
		USBPHY_SETREG(0x24, 0xB0);
		U3UTMIPHY_SETREG(0x48, 0x31);
		USBPHY_SETREG(0x48, 0x31);

		//r45 cali
		U3UTMIPHY_SETREG(0x144, 0x20);
		U3UTMIPHY_SETREG(0x144, 0x00);
	...
	//DBG_DUMP("exit\r\n");
		...
		T_USB_DEVEPMAP1_4_REG   DevMapReg;
		USB_EP  EPn;

		DBG_DUMP("%s: Resume Wakup\r\n", __func__);
		...
		for(EPn = USB_EP1; EPn <= USB_EP15; EPn++) {
			...
			if (EPn <= USB_EP4) {
				DevMapReg.reg = USB_GETREG(USB_DEVEPMAP1_4_REG_OFS);
			} else if (EPn <= USB_EP8) {
				DevMapReg.reg = USB_GETREG(USB_DEVEPMAP5_8_REG_OFS);
			} else if (EPn <= USB_EP12) {
				DevMapReg.reg = USB_GETREG(USB_DEVEPMAP9_12_REG_OFS);
			} else {
				DevMapReg.reg = USB_GETREG(USB_DEVEPMAP13_15_REG_OFS);
			}

			DevMapReg.reg = (DevMapReg.reg >> (((EPn-1)&0x3)*8))&0xFF;

			if (DevMapReg.bit.FNO_IEP1 != 0xF) {
				gFIFOInMap[DevMapReg.bit.FNO_IEP1] = EPn;
				gEPMap[EPn-1] = DevMapReg.bit.FNO_IEP1;
			} else if (DevMapReg.bit.FNO_OEP1 != 0xF) {
				gFIFOOutMap[DevMapReg.bit.FNO_OEP1] = EPn;
				gEPMap[EPn-1] = DevMapReg.bit.FNO_OEP1;
			}
		}
...
/*
    ...
    @return
        - @b E_OK: success
*/
ER usb_writePhyReg(uintptr_t uiOffset, UINT32 uiValue)
{
#ifdef _FPGA_EMULATION_
#if (_FPGA_EMULATION_ == 1)
	I2C_DATA    I2cData;
...
#else
	// Real board
	USBPHY_SETREG(uiOffset * 4, uiValue);
...
/*
    ...
    @return
        - @b E_OK: success
*/
ER usb_readPhyReg(uintptr_t uiOffset, UINT32 *puiValue)
{
#ifdef _FPGA_EMULATION_
#if (_FPGA_EMULATION_ == 1)
	I2C_DATA    I2cData;
...
#else
	// Real board
	*puiValue = USBPHY_GETREG(uiOffset * 4);
#endif
#endif
