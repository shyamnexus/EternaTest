...
void usb_FIFO_EPxCfg(void)
{
	...
	BOOL    bHS_En;
	//T_USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG    epbusymask;
	//UINT32 enMask = 0;
	...
	USB_SETREG(USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG_OFS,     0xFFFF);
	...
	for (EPi = 0 ; EPi < (USB_EP_MAX - 1) ; EPi++) {
		...
		if (pEnpCfg->enable) {
...

			{
				...
				// In UVC case, enable vdma cmplt and fifo empty interrupt for all fifos of IN Endpoint
				//if (gUSBManage.p_string_desc[0] == (USB_STRING_DESC *)&U2UvacStrDesc0) {
				/* todo
				   if ((EPi ==0) || (EPi==1)) {
				   if (pEnpCfg->direction == EP_DIR_IN) {
				   enMask = 1 << gEPMap[EPi];
				   epbusymask.reg = USB_GETREG(USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG_OFS);
				   epbusymask.reg &= ~(enMask);
				   USB_SETREG(USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG_OFS, epbusymask.reg);
				   }
				   }
				   */
...
ER usb2dev_read_endpoint(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen)
{
	...
	dma_flushReadCache((uintptr_t)pBuffer, *pDMALen);
...
ER usb2dev_set_ep_read(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen)
{
	...
	if (EPn == USB_EP0) {
		...
		// set Dma Addr
		//if ((((uintptr_t)pBuffer)&0xFFF00000) == 0xF0700000)
		//	devVDMACXF2.reg = (uintptr_t)pBuffer;
		//else
			devVDMACXF2.reg = dma_getPhyAddr2((uintptr_t)pBuffer);

		USB_SETREG(USB_DEVVIRTUAL_DMACXF2_REG_OFS, devVDMACXF2.reg);

		dma_flushReadCache2((uintptr_t)pBuffer, *pDMALen);
		dma_flushWriteCache2((uintptr_t)pBuffer, *pDMALen);
		...
		// set Dma Addr
		//if ((((uintptr_t)pBuffer)&0xFFF00000) == 0xF0700000)
		//	devDMACtl2.reg = (uintptr_t)pBuffer;
		//else
			devDMACtl2.reg = dma_getPhyAddr((uintptr_t)pBuffer);
		USB_SETREG(USB_DEVDMACTRL2_REG_OFS, devDMACtl2.reg);

		//dma_flushWriteCache((uintptr_t)pBuffer, *pDMALen);
		dma_flushReadCache((uintptr_t)pBuffer, *pDMALen);
	...
	if (fifonum < USB_FIFO_MAX) {
		...
		if (fifonum < USB_FIFO4) {
			...
			if ((((uintptr_t)pBuffer)&0xFFF00000) == 0xF0700000)
				...
				devVDMACXF2.reg = dma_getPhyAddr((uintptr_t)pBuffer);
...
ER usb2dev_set_ep_write(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen)
{
	...
	if (fifonum < USB_FIFO_MAX) {
		...
		if (fifonum < USB_FIFO4) {
			...
			if (USB_GETREG(USB_DEVVIRTUAL_DMAFIFO0_PAR2_REG_OFS + fifonum * 0x8) == 0x0) { //no any dma executed before this dma, so no necessary for checking EP busy or not.
			} else {
...
BOOL usb2dev_check_ep_busy(USB_EP EPn)
{
	...
	T_USB_DEVDMACTRL1_REG           devDMACtl1;
#endif
	//T_USB_DEVVMDA_CMPLT_EMP_FXINT_REG    epbusy;
	T_USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG    epbusymask;
	FLGPTN                        flags;
	UINT32 epbusy_enint_mask;

	fifonum = usb_getFifoFromEp(EPn);
	// fifo mask(uiMask) for EPn, for check fifo empty or not
	if (usb2dev_is_highspeed_enabled()) {
		uiBufNum = gUSBManage.ep_config_hs[EPn - 1].blk_num;
	...
	// check if EPn' fifos have enabled 'vdma cmplt emp interrupt' 
	epbusymask.reg = USB_GETREG(USB_DEVVMDA_CMPLT_EMP_FXINTMASK_REG_OFS);
	epbusy_enint_mask = (~epbusymask.reg)&0xFFFF;

	if (((epbusy_enint_mask & (1<<gEPMap[EPn - 1])) != epbusy_enint_mask) || (epbusy_enint_mask == 0)) { // [ORI] software check ep free.
		fifoStsReg.reg = USB_GETREG(USB_DEVCXCFGFIFOSTATUS_REG_OFS);
		fifoSts2Reg.reg = USB_GETREG(USB_DEVIDLECNT_REG_OFS);
	...
	} else { // [NEW] check ep free by interrupt.
		wai_flg(&flags, FLG_ID_USBTSK, FLGPTN_USB_EP_FREE, TWF_CLR | TWF_ORW);
		return FALSE;
	}
}
