...
/* Global variable                                                           */
/*===========================================================================*/
uintptr_t _USB2DEV_REG_BASE_ADDR[MODULE_REG_NUM];
uintptr_t _USB2DEV_PHY_REG_BASE_ADDR[MODULE_REG_NUM];
uintptr_t _USB3_PHY_REG_BASE_ADDR[MODULE_REG_NUM];
uintptr_t _USB3_UTMI_PHY_REG_BASE_ADDR[MODULE_REG_NUM];
uintptr_t _USB3_CTRL_REG_BASE_ADDR[MODULE_REG_NUM];
...
int nvt_usb2dev_drv_init(USB2DEV_MODULE_INFO *pmodule_info)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_REG_NUM; ucloop++) {
		_USB2DEV_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_addr[ucloop];
		usb2dev_api("USB2DEV[%d]-Addr = 0x%08lX\n", ucloop, _USB2DEV_REG_BASE_ADDR[ucloop]);

		_USB2DEV_PHY_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_phy_addr[ucloop];
                usb2dev_api("USB2DEV phy[%d]-Addr = 0x%08lX\n", ucloop, _USB2DEV_PHY_REG_BASE_ADDR[ucloop]);


		if (ucloop == 0) { //below items for two-port u2 phy use only
			_USB3_PHY_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_u3phy_addr[ucloop];
			usb2dev_api("USB3 phy[%d]-Addr = 0x%08lX\n", ucloop, _USB3_PHY_REG_BASE_ADDR[ucloop]);

			_USB3_UTMI_PHY_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_u3utmi_phy_addr[ucloop];
			usb2dev_api("USB3 UTMI phy[%d]-Addr = 0x%08lX\n", ucloop, _USB3_UTMI_PHY_REG_BASE_ADDR[ucloop]);

			_USB3_CTRL_REG_BASE_ADDR[ucloop] = (uintptr_t)pmodule_info->io_u3_ctrl_addr[ucloop];
			usb2dev_api("USB3 controller[%d]-Addr = 0x%08lX\n", ucloop, _USB3_CTRL_REG_BASE_ADDR[ucloop]);
		}
	}
...
/* initial clock here */
        for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
			...
			if (U3CTL_GETREG(0x28) & 0x80) {
				//printk("nvt_usb2dev: u2 clk already reset by other driver, 0x28 is 0x%x\n", U3CTL_GETREG(0x28));
			} else {
				clk_prepare_enable(pmodule_info->pclk[ucloop]);
				clk_disable_unprepare(pmodule_info->pclk[ucloop]);
				U3CTL_SETREG(0x28, 0x80);
			}
		}
        }
...
int nvt_usb2dev_drv_ioctl(unsigned char uc_if, USB2DEV_MODULE_INFO *pmodule_info, unsigned int ui_cmd, unsigned long ul_arg)
{
	...
	case USB2DEV_IOC_GET_EVENT: {
		...
		ret = vos_flag_wait_interruptible(&uiFlag, FLG_ID_USBTSK, FLGUSB_USER_EVENT_SET, TWF_ORW|TWF_CLR);
		if (ret) {
			ret = -ERESTARTSYS;
			break;
		}
	...
	case USB2DEV_IOC_WAIT_EP_DONE: {
		...
		if (!ret) {
			...
			if (!ret) {
				if ((data_rw.epn <= (USB_EP_MAX -1))) {
					usb2dev_wait_ep_done(data_rw.epn, &dma_len);

					ret = copy_to_user((void __user *)data_rw.dma_length, &dma_len, sizeof(UINT32));
					if (ret != 0)
						DBG_ERR("copy_to_user\n");
				} else {
					DBG_ERR("USB2DEV_IOC_WAIT_EP_DONE EP out of range\n");
				}
