...
static int nvt_usb2dev_probe(struct platform_device *pdev)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_REG_NUM ; ucloop++) {
		...
		/* phy(port1) addr*/
		pdrv_info->module_info.io_phy_addr[ucloop] = ioremap_nocache(USB_PHY_ADDR_1, USB_PHY_SIZE_1);
		usb2dev_api("phy start 0x%08lX  mapped to 0x%08lX\n", (uintptr_t)USB_PHY_ADDR_1, (uintptr_t)pdrv_info->module_info.io_phy_addr[ucloop]);
		if (pdrv_info->module_info.io_phy_addr[ucloop] == NULL) {
			printk("ioremap() failed in module%d\n", ucloop);
			ret = -ENODEV;
			goto FAIL_FREE_RES;
		}

		if (ucloop == 0) { //below items for two-port u2 phy use only
			/* u3 phy addr*/
			pdrv_info->module_info.io_u3phy_addr[ucloop] = ioremap_nocache(USB3_PHY_ADDR_1, USB3_PHY_SIZE_1);
			usb2dev_api("phy start 0x%08lX  mapped to 0x%08lX\n", (uintptr_t)USB3_PHY_ADDR_1, (uintptr_t)pdrv_info->module_info.io_u3phy_addr[ucloop]);
			if (pdrv_info->module_info.io_u3phy_addr[ucloop] == NULL) {
				printk("ioremap() failed in module%d\n", ucloop);
				ret = -ENODEV;
				goto FAIL_FREE_RES;
			}

			/* u3 utmi(port0) phy addr*/
			pdrv_info->module_info.io_u3utmi_phy_addr[ucloop] = ioremap_nocache(USB3_UTMI_PHY_ADDR_1, USB3_UTMI_PHY_SIZE_1);
			usb2dev_api("phy start 0x%08lX  mapped to 0x%08lX\n", (uintptr_t)USB3_UTMI_PHY_ADDR_1, (uintptr_t)pdrv_info->module_info.io_u3utmi_phy_addr[ucloop]);
			if (pdrv_info->module_info.io_u3utmi_phy_addr[ucloop] == NULL) {
				printk("ioremap() failed in module%d\n", ucloop);
				ret = -ENODEV;
				goto FAIL_FREE_RES;
			}

			/* u3 controller(port0) addr*/
			pdrv_info->module_info.io_u3_ctrl_addr[ucloop] = ioremap_nocache(USB3_CTRL_ADDR_1, USB3_CTRL_SIZE_1);
			usb2dev_api("u3 controller start 0x%08lX  mapped to 0x%08lX\n", (uintptr_t)USB3_CTRL_ADDR_1, (uintptr_t)pdrv_info->module_info.io_u3_ctrl_addr[ucloop]);
			if (pdrv_info->module_info.io_u3_ctrl_addr[ucloop] == NULL) {
				printk("ioremap() failed in module%d\n", ucloop);
				ret = -ENODEV;
				goto FAIL_FREE_RES;
			}
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		pdrv_info->module_info.pclk[ucloop] = clk_get(NULL, "u2phy");
...
FAIL_FREE_REMAP:
	for (ucloop = 0 ; ucloop < MODULE_REG_NUM ; ucloop++) {
		...
		iounmap(pdrv_info->module_info.io_phy_addr[ucloop]);

		if (ucloop == 0) {
			iounmap(pdrv_info->module_info.io_u3phy_addr[ucloop]);
			iounmap(pdrv_info->module_info.io_u3utmi_phy_addr[ucloop]);
		}
	}
