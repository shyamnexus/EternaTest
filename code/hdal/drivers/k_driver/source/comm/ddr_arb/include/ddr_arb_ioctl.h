#ifndef __MODULE_IOCTL_CMD_H_
#define __MODULE_IOCTL_CMD_H_

#include <linux/ioctl.h>
#include "kwrap/type.h"

#define DDR_ARB_IOC_VERSION 0x22051610

typedef struct _ARB_CHKSUM {
	unsigned int version;  ///< [IN] must be DDR_ARB_IOC_VERSION for check header version
	unsigned int ddr_id;   ///< [IN] 0 or 1 for DDR[0], or DDR[1]
	uintptr_t phy_addr;    ///< [IN] must be word alignment
	unsigned long len;     ///< [IN] must be word alignment
	unsigned short sum;    ///< [OUT] the result of sum
} ARB_CHKSUM;

typedef struct mon_info {
	int ch;
	int rw;
	int dram;
	UINT64 count;
} MON_INFO;

typedef struct cnt_info {
	int dram;
	UINT64 count;
} CNT_INFO;

//============================================================================
// IOCTL command
//============================================================================
#define DDR_ARB_IOC_COMMON_TYPE 'M'
#define DDR_ARB_IOC_CHKSUM    _IOWR(DDR_ARB_IOC_COMMON_TYPE, 1, void*)

#define DDR_IOC_MON_START     _IOWR(DDR_ARB_IOC_COMMON_TYPE, 2, void*)
#define DDR_IOC_MON_STOP      _IOWR(DDR_ARB_IOC_COMMON_TYPE, 3, void*)

#define DDR_IOC_RESET_CNT     _IOWR(DDR_ARB_IOC_COMMON_TYPE, 4, void*)
#define DDR_IOC_GET_CNT       _IOWR(DDR_ARB_IOC_COMMON_TYPE, 5, void*)

/* Add other command ID here*/


#endif
