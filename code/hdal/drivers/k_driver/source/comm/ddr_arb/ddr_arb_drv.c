...
int nvt_ddr_arb_drv_ioctl(unsigned char if_id, MODULE_INFO *pmodule_info, unsigned int cmd, unsigned long arg)
{
	...
	case DDR_IOC_RESET_CNT:
		/*call someone to start operation*/
		err = copy_from_user(&cnt_info, argp, sizeof(CNT_INFO));
        if(!err) {
    	 	dma_reset_data_count(cnt_info.dram);
		} else {
			nvt_dbg(ERR, "copy from user err\n");
		}
		break;
	case DDR_IOC_GET_CNT:
		/*call someone to start operation*/
		err = copy_from_user(&cnt_info, argp, sizeof(CNT_INFO));
        if(!err) {
    	 	cnt_info.count = dma_get_data_count(cnt_info.dram);

    	 	err = copy_to_user(argp, &cnt_info, sizeof(CNT_INFO));
		} else {
			nvt_dbg(ERR, "copy from user err\n");
		}
		break;
