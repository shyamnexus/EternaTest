...
void __iomem *IOADDR_DDR_HVY_REG_BASE;
void __iomem *IOADDR_DDR_HVY2_REG_BASE;
void __iomem *IOADDR_DDR_HVY3_REG_BASE;
void __iomem *IOADDR_DDR_QOS_REG_BASE;
void __iomem *IOADDR_LPV_REG_BASE;
void __iomem *IOADDR_PROT_REG_BASE;
...
ULONG ddr_arb_platform_spin_lock(void)
{
#if defined __UITRON || defined __ECOS
	loc_cpu();
	return 0;
...
void ddr_arb_platform_spin_unlock(ULONG flags)
{
#if defined __UITRON || defined __ECOS
	unl_cpu();
#else
...
ULONG ddr_arb_platform_va2pa(ULONG addr)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
			...
			if (nvt_heavyload_cfg.load_degree == DRAM_CONSUME_CH_DISABLE) {
				...
				*/
...
#endif
#if RANDOM_MODE
				hvy_param0.burst_len    = rand()%64 +1;
...
#if RANDOM_MODE
				hvy_param1.burst_len	= rand()%64 +1;
...
#if RANDOM_MODE
				hvy_param2.burst_len	= rand()%64 +1;
				...
				if (nvt_heavyload_cfg.load_degree == DRAM_CONSUME_EASY_LOADING) {
					...
					if (dma_enable_heavyload(0, DMA_HEAVY_LOAD_CH0, &hvy_param0) != E_OK) {
						DBG_ERR("Consume task open fail degree[%d]-1\r\n", (int)nvt_heavyload_cfg.load_degree);
						err = TRUE;
					}

					dma_trig_heavyload(0, 0x1);
				}
				// Normal
					...
					DBG_DUMP("*************ConsumeTsk Information*******************\r\n");
					if (dma_enable_heavyload(0, DMA_HEAVY_LOAD_CH0, &hvy_param1) != E_OK) {
						DBG_ERR("Consume task open fail degree[%d]-1\r\n", (int)nvt_heavyload_cfg.load_degree);
						err = TRUE;
					}
					if (dma_enable_heavyload(0, DMA_HEAVY_LOAD_CH1, &hvy_param1) != E_OK) {
						DBG_ERR("Consume task open fail degree[%d]-2\r\n", (int)nvt_heavyload_cfg.load_degree);
						err = TRUE;
					}

					dma_trig_heavyload(0, 0x3);
				}
				// Heavy
				else {
					DBG_DUMP("*   (@)Channel0 Burst Length============>[     %05d]*\r\n", (int)hvy_param0.burst_len);
					...
					if (dma_enable_heavyload(0, DMA_HEAVY_LOAD_CH2, &hvy_param2) != E_OK) {              // Enable maximun 3 channel at the same time
						DBG_ERR("Consume task open fail degree[%d]-3\r\n", (int)nvt_heavyload_cfg.load_degree);
						err = TRUE;
					}

					dma_trig_heavyload(0, 0x7);
				}
...
{
	IOADDR_DDR_ARB_REG_BASE = pmodule_info->io_addr[0];
	IOADDR_DDR_HVY_REG_BASE = pmodule_info->io_addr[1];
	IOADDR_DDR_HVY2_REG_BASE = pmodule_info->io_addr[2];
	IOADDR_DDR_HVY3_REG_BASE = pmodule_info->io_addr[3];
	IOADDR_LPV_REG_BASE = pmodule_info->io_addr[4];
	IOADDR_PROT_REG_BASE = pmodule_info->io_addr[5];
