...
static uintptr_t uvcp_output_address;
...
ER uvcp_trigger(BOOL eof, BOOL wait_break)
{
	...
	if(wait_break) {
		if (vos_flag_wait_timeout(&uiflag, FLG_ID_UVCP, FLGPTN_UVCP_BRK, TWF_ORW | TWF_CLR, vos_util_msec_to_tick(2000)) != E_OK) {
...
ER uvcp_wait_complete(void)
{
	...
	if (vos_flag_wait_timeout(&uiflag, FLG_ID_UVCP, FLGPTN_UVCP, TWF_ORW | TWF_CLR, vos_util_msec_to_tick(1000)) != E_OK) {
		DBG_ERR("uvcp_wait_complete timeout\r\n");
		return E_SYS;
	}
#endif
...
ER uvcp_wait_break(void)
{
	...
	if (vos_flag_wait_timeout(&uiflag, FLG_ID_UVCP, FLGPTN_UVCP_BRK, TWF_ORW | TWF_CLR, vos_util_msec_to_tick(1000)) != E_OK) {
...
void uvcp_config_dma(uintptr_t input_address, UINT32 input_size, uintptr_t output_address, UINT32 *output_size)
{
...
void uvcp_suspend(void)
{
	T_UVCP_OP_REG           regop;
	unsigned long   flags;

	//step1: wait uvcp stop
	loc_cpu(flags);
	regop.reg = UVCP_GETREG(UVCP_OP_REG_OFS);
	while (regop.bit.START) {
		regop.reg = UVCP_GETREG(UVCP_OP_REG_OFS);
	}
	unl_cpu(flags);

	//step2: close interrupt
	#if !UVCP_POLLING
	UVCP_SETREG(UVCP_INTEN_REG_OFS, 0x0);
	#endif
}

void uvcp_resume(void)
{
	//step1: re-enable interrupt
	#if !UVCP_POLLING
	UVCP_SETREG(UVCP_INTEN_REG_OFS, 0x3);
	#endif
}
