...
#define loc_cpu() vk_spin_lock_irqsave(&timer_spinlock, spin_flags)
#define unl_cpu() vk_spin_unlock_irqrestore(&timer_spinlock, spin_flags)
...
void __iomem *_REGIOBASE;
void __iomem *IOADDR_TIMER_REG_BASE;
...
#define TIMER_IST_FLG_EXIT_DONE (FLGPTN_BIT(20))
...
ULONG timer_platform_spin_lock(void)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
	loc_cpu();
	return 0;
...
void timer_platform_spin_unlock(ULONG flags)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
	unl_cpu();
#else
...
UINT32 timer_platform_dma_is_cacheable(uintptr_t addr)
{
#if defined __UITRON || defined __ECOS
...
UINT32 timer_platform_dma_flush_dev2mem_width_neq_loff(uintptr_t addr, UINT32 size)
{
#if defined __UITRON || defined __ECOS
...
*/
	{
		...
		fmem_dcache_sync((void*)address, size, DMA_BIDIRECTIONAL);
		return 0;
	}
#endif
}

UINT32 timer_platform_dma_flush_dev2mem(uintptr_t addr, UINT32 size)
{
#if defined __UITRON || defined __ECOS
...
UINT32 timer_platform_dma_flush_mem2dev(uintptr_t addr, UINT32 size)
{
#if defined __UITRON || defined __ECOS
...
BOOL timer_platform_is_valid_va(uintptr_t addr)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
#define IS_DRAM_ADDR(x)      (((x) < 0xC0000000UL)?1:0)
	return IS_DRAM_ADDR(addr);
#else
	if (fmem_lookup_pa(addr) == -1UL)
		return FALSE;
	else
		return TRUE;
#endif
...
UINT32 timer_platform_va2pa(uintptr_t addr)
{
#if defined __UITRON || defined __ECOS || defined __FREERTOS
...
//		printk("%s: ptn 0x%x\r\n", __func__, ptn);
	...
	set_flg(FLG_ID_TMR_IST, TIMER_IST_FLG_EXIT_DONE);
	THREAD_RETURN(0);
}
...
{
	_REGIOBASE = pmodule_info->io_addr[0];
	IOADDR_TIMER_REG_BASE = pmodule_info->io_addr[0];
...
void timer_platform_release_resource(void)
{
	...
	wai_flg(&ptn, FLG_ID_TMR_IST, TIMER_IST_FLG_EXIT_DONE, TWF_ORW | TWF_CLR);
#endif
	rel_flg(FLG_ID_TIMER);
