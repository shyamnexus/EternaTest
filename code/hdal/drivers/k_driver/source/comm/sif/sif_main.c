...
//=============================================================================
#ifdef DEBUG
unsigned int sif_debug_level = NVT_DBG_ERR;
...
#ifdef CONFIG_PM
static int nvt_sif_suspend(struct device *dev);
static int nvt_sif_resume(struct device *dev);
#endif
...
static int nvt_sif_suspend(struct device *dev)
{
	PSIF_DRV_INFO pdrv_info = dev_get_drvdata(dev);
	...
	if (!IS_ERR(&pdrv_info->module_info.pclk[0])) {
		if (__clk_is_enabled(pdrv_info->module_info.pclk[0])) {
			clk_disable_unprepare(pdrv_info->module_info.pclk[0]);
		}
	}
...
static int nvt_sif_resume(struct device *dev)
{
	...
	if (!IS_ERR(&pdrv_info->module_info.pclk[0])) {
		if (!__clk_is_enabled(pdrv_info->module_info.pclk[0])) {
			clk_prepare_enable(pdrv_info->module_info.pclk[0]);
		}
	}
...
static const struct dev_pm_ops nvt_sif_pmops = {
	.suspend	= nvt_sif_suspend,
	.resume		= nvt_sif_resume,
};
#define NVTSIF_PMOPS &nvt_sif_pmops
#else
#define NVTSIF_PMOPS NULL
#endif /* CONFIG_PM */
