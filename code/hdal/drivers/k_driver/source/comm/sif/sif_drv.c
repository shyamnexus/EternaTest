...
int nvt_sif_drv_init(MODULE_INFO* pmodule_info)
{
    ...
    sif_init((ULONG)pmodule_info->io_addr[0]);
...
int nvt_sif_drv_write_reg(PMODULE_INFO pmodule_info, unsigned long addr, unsigned long value)
{
	WRITE_REG(value, pmodule_info->io_addr[0] + addr);
	return 0;
}

int nvt_sif_drv_read_reg(PMODULE_INFO pmodule_info, unsigned long addr)
{
	return READ_REG(pmodule_info->io_addr[0] + addr);
}

int nvt_sif_drv_send_test_data(PMODULE_INFO pmodule_info, unsigned long test_ch, unsigned long test_data)
{	
	nvt_dbg(INFO, "Test send 32bits data\n");
	nvt_sif_channel_open(test_ch);

	nvt_sif_setConfig(test_ch, SIF_CONFIG_ID_LENGTH, 32);
	nvt_sif_send(test_ch, test_data, 0x0, 0x0);
	nvt_sif_waitTransferEnd(test_ch);

	nvt_sif_channel_close(test_ch);
