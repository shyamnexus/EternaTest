...
static struct proc_ops proc_summary_fops = {
	.proc_open   	= nvt_drvdump_proc_summary_open,
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0))
static struct proc_ops proc_efuse_check_available_fops = {
	.proc_open   	= nvt_check_available_proc_open,
	.proc_release 	= single_release,
	.proc_read   	= nvt_check_available_proc_cmd_read,
	.proc_lseek 	= seq_lseek,
	.proc_write  	= nvt_check_available_proc_cmd_write
};
#else
...
static int nvt_drvdump_remove(struct platform_device *pdev)
{
	...
	remove_proc_entry("nvt_info/drvdump/available", pmodule_root);
	remove_proc_entry("nvt_info/drvdump/info", pmodule_root);
	remove_proc_entry("nvt_info/drvdump", pmodule_root);
	return 0;
}
...
extern ULONG _ttb[];                       // symbol from lds file
...
static void drv_dump_mmu_info(void)
{
    ...
    printf("not support mmu dump for arm64\n");
#else
    ULONG i;
