...
typedef struct proc_cmd {
	...
	int (*execute)(PUSB3DEV_MODULE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
PUSB2DEV_DRV_INFO pusb3dev_drv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_usb3dev_api_write_reg          },
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_usb3dev_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
...
static int nvt_usb3dev_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");

	usb3dev_api("%s\n", __func__);

	return single_open(file, nvt_usb3dev_proc_cmd_show, &pusb3dev_drv_info_data->module_info);
}

static ssize_t nvt_usb3dev_proc_cmd_write(struct file *file, const char __user *buf,
	...
	if (strncmp(argv[0], "r", 2) == 0) {
		for (loop = 0 ; loop < NUM_OF_READ_CMD; loop++) {
			if (strncmp(argv[1], cmd_read_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_read_list[loop].execute(&pusb3dev_drv_info_data->module_info, ucargc - 2, &argv[2]);
				break;
			}
		}
		...
		for (loop = 0 ; loop < NUM_OF_WRITE_CMD ; loop++) {
			if (strncmp(argv[1], cmd_write_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_write_list[loop].execute(&pusb3dev_drv_info_data->module_info, ucargc - 2, &argv[2]);
				break;
			}
		}
...
static struct proc_ops proc_cmd_fops = {
	//.owner   = THIS_MODULE,
	.proc_open    = nvt_usb3dev_proc_cmd_open,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
	.proc_write   = nvt_usb3dev_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_usb3dev_proc_help_show(struct seq_file *sfile, void *v)
{
...
static int nvt_usb3dev_proc_help_open(struct inode *inode, struct file *file)
{
	usb3dev_api("%s\n", __func__);

	return single_open(file, nvt_usb3dev_proc_help_show, NULL);
}

static struct proc_ops proc_help_fops = {
	//.owner  = THIS_MODULE,
	.proc_open   = nvt_usb3dev_proc_help_open,
	.proc_release = single_release,
	.proc_read   = seq_read,
	.proc_lseek = seq_lseek,
};
...
int nvt_usb3dev_proc_init(PUSB2DEV_DRV_INFO pdrv_info)
{
	int ret = 0;
	...
	pmodule_root = proc_mkdir("usb3dev", NULL);
	...
	pusb3dev_drv_info_data = pdrv_info;
...
int nvt_usb3dev_proc_remove(PUSB2DEV_DRV_INFO pdrv_info)
{
	usb3dev_api("%s\n", __func__);
