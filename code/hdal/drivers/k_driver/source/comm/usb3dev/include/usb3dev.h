...
*/
typedef enum {
	U3DEV_CONFIG_ID_BUFFER_ADDR,		///< Driver internal needed buffer VA address.
	U3DEV_CONFIG_ID_BUFFER_ADDR_PA,		///< Driver internal needed buffer PA address.
	...
	U3DEV_CONFIG_ID_AUTO_FIFO_CLR,		///< Please use USB_EP as input parameter. Auto Clear EP FIFO when transfer trigger start.
	...
	U3DEV_CONFIG_ID_EPIN_INTSRC,		///< Select IN Endpoint interrupt source from Host IN Token or IN FIFO ready.
										///< Please use U3DEV_EPIN_INTSRC_FIFORDY(default) or U3DEV_EPIN_INTSRC_INTOKEN as input.
	U3DEV_CONFIG_ID_SUSPEND_DRAMOFF,	///< Suspend notifcation

	ENUM_DUMMY4WORD(U3DEV_CONFIG_ID)
} U3DEV_CONFIG_ID;

typedef enum {
	U3DEV_EP_CONFIG_ID_FIFO_NUM,
	U3DEV_EP_CONFIG_ID_PATCH2,
	ENUM_DUMMY4WORD(U3DEV_EP_CONFIG_ID)
} U3DEV_EP_CONFIG_ID;
...
*/
typedef enum {
	U3DEV_EPIN_INTSRC_FIFORDY,	///< IN Endpoint interrupt is issued when IN FIFO Ready.(default)
	U3DEV_EPIN_INTSRC_INTOKEN,	///< IN Endpoint interrupt is issued when IN Token received and FIFO Ready.

	ENUM_DUMMY4WORD(U3DEV_EPIN_INTSRC)
}U3DEV_EPIN_INTSRC;
...
*/
typedef enum {
	U3DEV_EPINFO_ID_BYTECOUNT,  ///< Get the current OUT EP FIFO byte count.
...
typedef enum {
	...
	U3DEV_CALLBACK_ID_SET_CONFIGURATION,	///< Callback function when set_configuration command success.
	U3DEV_CALLBACK_ID_TXFIFO_REMAP,			///< Callback function for TXFIFO Remapping.
	U3DEV_CALLBACK_ID_STD_UNKNOWN_REQ,      ///< usb std request with unknown request type event callback.

	U3DEV_CALLBACK_ID_SUSPEND,      ///< Callback function when enter suspend mode.
	U3DEV_CALLBACK_ID_RESUME,       ///< Callback function when resuming from the suspend state.

	U3DEV_CALLBACK_ID_U1_ENTRY,     ///< Callback function when enter U1 mode.
	U3DEV_CALLBACK_ID_U2_ENTRY,     ///< Callback function when enter U2 mode.
	U3DEV_CALLBACK_ID_U3_ENTRY,     ///< Callback function when enter U3 mode.
...
typedef enum {
	...
	U3DEV_DESC_ID_SS_STRING8,			///< Super Speed String Descriptor 8
	U3DEV_DESC_ID_SS_STRING9,			///< Super Speed String Descriptor 9
	U3DEV_DESC_ID_SS_STRING10,			///< Super Speed String Descriptor 10
	U3DEV_DESC_ID_SS_STRING11,			///< Super Speed String Descriptor 11
	...
	U3DEV_DESC_ID_HS_STRING8,			///< High Speed String Descriptor 8
	U3DEV_DESC_ID_HS_STRING9,			///< High Speed String Descriptor 9
	U3DEV_DESC_ID_HS_STRING10,			///< High Speed String Descriptor 10
	U3DEV_DESC_ID_HS_STRING11,			///< High Speed String Descriptor 11
	...
	U3DEV_DESC_ID_FS_STRING8,			///< Full Speed String Descriptor 8
	U3DEV_DESC_ID_FS_STRING9,			///< Full Speed String Descriptor 9
	U3DEV_DESC_ID_FS_STRING10,			///< Full Speed String Descriptor 10
	U3DEV_DESC_ID_FS_STRING11,			///< Full Speed String Descriptor 11
...
typedef enum {
	...
	U3DEV_INTRR_MAX,
	ENUM_DUMMY4WORD(U3DEV_INTRR)
}U3DEV_INTRR;


/**
  USB3 controller state

  @note For usb_setControllerState(), usb2dev_get_controller_state()
  */
typedef enum {
	USB3_CONTROLLER_STATE_NONE,			///< USB3 controller state is none
	USB3_CONTROLLER_STATE_RESET,		///< USB3 controller state is reset
	USB3_CONTROLLER_STATE_SUSPEND,		///< USB3 controller state is suspend
	ENUM_DUMMY4WORD(USB3_CONTROLLER_STATE)
} USB3_CONTROLLER_STATE;
...
*/
typedef struct {
	uintptr_t  uiBufAddr;                  ///< Transfer Buffer Address
...
/**
    ...
    @return void
*/
extern void             usb3dev_handleSetupResult(BOOL bStall, uintptr_t uiRetBufAddr, UINT32 uiRetSize);
...
/**
    ...
    @param[in] EPn          endpoint number. Only allow (USB_EP1 ~ USB_EP15)
...
//extern ER               usb3dev_writeEndpoint_pa(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen);
...
/**
    ...
    @param[in] EPn          ISOCHRONOUS endpoint number. Only allow (USB_EP1 ~ USB_EP15)
...
//extern ER               usb3dev_setIsoEPWrite_pa(USB_EP EPn, PU3DEV_ISODATA pIsoData);
...
//extern ER               usb3dev_readEndpoint_pa(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen);
...
/**
    ...
    @param[in] EPn IN endpoint number (USB_EP0~USB_EP15)
...
/**
    ...
    @param[in] EPn OUT endpoint number (USB_EP0~USB_EP15)
...
extern void             usb3dev_abortEndpoint(USB_EP EPn);

extern ER usb3dev_setEPConfig(USB_EP EPn, U3DEV_EP_CONFIG_ID CfgID, UINT32 uiConfig);

extern ER usb3dev_setInterruptEnable(U3DEV_INTRR Interrupt, BOOL bEnable);

extern void usb3dev_set_ep_stall(USB_EP EPn);

extern void usb3dev_set_tx0byte(USB_EP EPn);

extern ER usb3dev_waitWriteDone_timeout(USB_EP EPn, UINT32 *pDMALen, UINT32 timeout_ms);

extern ER usb3dev_writeEndpoint_timeout(USB_EP EPn, UINT8 *pBuffer, UINT32 *pDMALen, UINT32 timeout_ms);

extern void usb3_reset_ep_cfg(USB_EP EPn, UINT32 EpType, UINT32 MaxPktSz, UINT32 MaxBurst, UINT32 bInterval, BOOL isIN, UINT32 BwNum);

extern USB_CHARGER_STS usb3dev_check_charger(UINT32 value);

extern void usb3_detect_vbusgpio(int vbus_gpio_pin, int vbus_gpio_pol);

extern void usb3_vbus_polling(void);

extern void usb3_vbus_thread_remove(void);

extern void usb3_set_vbus_valid(void);

UINT32 usb3dev_gpio_getIntStatus(void);

/*
 * For ISO bufferable patch
 *
 * Provide two method to make sure isochronous type have enough data amount before transfer start
 *
 * */
extern ER               usb3dev_SetIsoBuff_HW(USB_EP EPn, BOOL bEnable);
extern ER               usb3dev_SetIsoBuff_SW(USB_EP EPn, BOOL bEnable);
#endif
