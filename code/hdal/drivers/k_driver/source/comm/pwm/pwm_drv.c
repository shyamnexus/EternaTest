...
static void nvt_pwm_store_reg(MODULE_INFO *pmodule_info)
{
	int i;

	for (i = 0; i < reg_sz; i++) {
		reg_val[i] = nvt_pwm_drv_read_reg(pmodule_info, i << 2);
	}
}

static void nvt_pwm_restore_reg(MODULE_INFO *pmodule_info)
{
	int i;
	...
	for (i = 0; i < reg_sz; i++) {
		if ( i != (0x100 >> 2))
			nvt_pwm_drv_write_reg(pmodule_info, i << 2, reg_val[i]);
	}
...
int nvt_pwm_drv_suspend(MODULE_INFO *pmodule_info)
{
	...
	nvt_pwm_store_reg(pmodule_info);

	for (idx = 0; idx < PWM_PWMCH_BITS; idx++) {
		if (pwm_pwm_is_en(1 << idx)) {
			pwm_en |= (0x1 << idx);
			clk_disable(pmodule_info->pclk[idx]);
			clk_unprepare(pmodule_info->pclk[idx]);
		}
	}
...
int nvt_pwm_drv_resume(MODULE_INFO *pmodule_info)
{
	...
	for (idx = 0; idx < PWM_PWMCH_BITS; idx++) {
		...
		if (pwm_en & (0x1 << idx)) {
			pwm_en &= ~(0x1 << idx);
			clk_prepare(pmodule_info->pclk[idx]);
			clk_enable(pmodule_info->pclk[idx]);
		}
	}

	nvt_pwm_restore_reg(pmodule_info);
