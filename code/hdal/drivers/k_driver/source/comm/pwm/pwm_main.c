...
static int nvt_pwm_suspend(struct device *dev);
static int nvt_pwm_resume(struct device *dev);
...
static int nvt_pwm_probe(struct platform_device *pdev)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		if (ucloop < 12) {
			sprintf(id, "%llx.pwm%d", pdrv_info->presource[0]->start, ucloop);
		} else {
			sprintf(id, "%llx.ccnt%d", pdrv_info->presource[0]->start, ucloop - 12);
		}
...
static const struct dev_pm_ops nvt_pwm_pm = {
