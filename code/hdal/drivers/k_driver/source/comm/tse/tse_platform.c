...
#endif
{
	...
	tse_reg_base = (TSEDATA_TYPE)pmodule_info->io_addr[0];
#endif
...
TSEDATA_TYPE tse_platform_get_reg_base(void)
{
#if (defined __UITRON || defined __ECOS)
...
UINT64 tse_platform_get_phy_addr(TSEDATA_TYPE addr)
{
	UINT64 rt;
#if (defined __FREERTOS)
	rt = sys_va2pa(addr).addr64;
#else
...
UINT32 tse_platform_cache_sync_dma_to_dev(TSEDATA_TYPE addr, UINT32 size)
{
	vos_cpu_dcache_sync(addr, size, VOS_DMA_TO_DEVICE);
	return 0;
}

UINT32 tse_platform_cache_sync_dma_from_dev(TSEDATA_TYPE addr, UINT32 size)
{
	vos_cpu_dcache_sync(addr, size, VOS_DMA_FROM_DEVICE);
	return 0;
}

void tse_platform_iclr_flg(void)
{
	vos_flag_iclr(tse_flg_id, FLGPTN_TSE);
}

void tse_platform_iset_flg(void)
{
...
{
	vos_flag_clr(tse_flg_id, FLGPTN_TSE);
}

void tse_platform_set_flg(void)
{
	vos_flag_set(tse_flg_id, FLGPTN_TSE);
}
