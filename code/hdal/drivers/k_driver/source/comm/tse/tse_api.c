...
static INT32 nvt_tse_set_const_verify(void* addr, UINT32 data_size, UINT32 data_val)
{
	UINT32 i;
	UINT32 *ptr32;
	...
	for (i = 0; i < (data_size >> 2); i ++) {
		if (ptr32[i] != data_val) {
			DBG_DUMP("data mismatch addr:0x%p (0x%.8x, 0x%.8x)\n", (void*)&ptr32[i], ptr32[i], data_val);
			return -1;
		}
	}
...
int nvt_tse_mem_set_test(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	...
	if ((out_handle == NULL) || (outbuf.vaddr == NULL)) {
		DBG_ERR("outbuf(0x%.8lx) alloc fail\n", outbuf.size);
		ret = -1;
		goto err;
	}
	DBG_DUMP("outbuf(0x%p, 0x%p, 0x%.8lx)\n", (void*)outbuf.vaddr, (void*)outbuf.paddr, outbuf.size);
	...
	tse_setConfig(HWCOPY_CFG_ID_DST_ADDR, (TSEDATA_TYPE)outbuf.vaddr);
...
static INT32 nvt_tse_copy_verify(void* src_addr, void* dst_addr, UINT32 size)
{
	UINT32 i;
	UINT8 *ptr_src;
	UINT8 *ptr_dst;
	...
	for (i = 0; i < size; i += 1) {
		if (ptr_src[i] != ptr_dst[i]) {
			DBG_ERR("data mismatch (0x%.8x, 0x%.8x) (0x%p, 0x%p)\n", ptr_src[i], ptr_dst[i], (void *)&ptr_dst[i], (void *)&ptr_dst[i]);
			return -1;
		}
	}
	DBG_DUMP("Compare PASS\n");
...
int nvt_tse_mem_cpy_test(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	...
	if ((in_handle == NULL) || (inbuf.vaddr == NULL)) {
		DBG_ERR("inbuf(0x%.8lx) alloc fail\n", inbuf.size);
		ret = -1;
		goto err;
	}
	DBG_DUMP("inbuf(0x%p, 0x%p, 0x%.8lx)\n", (void*)inbuf.vaddr, (void*)inbuf.paddr, inbuf.size);
	...
	if (outbuf.size < inbuf.size) {
		DBG_ERR("out size(0x%.8lx) < in size(0x%.8lx)\n", outbuf.size, inbuf.size);
		goto err;
	}
	...
	tse_setConfig(HWCOPY_CFG_ID_SRC_ADDR, (UINT64)inbuf.vaddr);
	tse_setConfig(HWCOPY_CFG_ID_DST_ADDR, (UINT64)outbuf.vaddr);
	...
	fmem_dcache_sync(outbuf.vaddr,  outbuf.size, DMA_FROM_DEVICE);
	ret = nvt_tse_copy_verify((void *)inbuf.vaddr, (void *)outbuf.vaddr, inbuf.size);
err:
	if (in_handle) {
...
static INT32 nvt_tse_mux_verify(void* DstAddr, void* SrcAddr, UINT32 SrcSize, TS_H_INFO header, UINT32 PayloadSize, UINT32 ts1_183_en)
{
	UINT8   *pDst8, *pSrc8;
	UINT32  seg, SrcLoc;
	UINT32  i, j, temp;
	...
	for (i = 0; i < seg; i++) {
		if (pDst8[i * 188] != temp) {
			DBG_ERR("SYNC Code Error! Addr=0x%p SEG=%d 0x%.8x!=0x%.8x\n", (void*)&pDst8[i * 188], i, pDst8[i * 188], temp);
			return -1;
		}
	}
	...
	for (i = 0; i < seg; i++) {
		if ((pDst8[(i * 188) + 1] & 0x1F) != temp) {
			DBG_ERR("^RPID Code Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 1], i);
			return -1;
		}
	}
	...
	for (i = 0; i < seg; i++) {
		if ((pDst8[(i * 188) + 1] & 0x80) != (temp << 7)) {
			DBG_ERR("^RTEI Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 1], i);
			return -1;
		}
	}
	...
	for (i = 0; i < seg; i++) {
		if ((pDst8[(i * 188) + 1] & 0x20) != (temp << 5)) {
			DBG_ERR("^RTP Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 1], i);
			return -1;
		}
	}
	...
	for (i = 0; i < seg; i++) {
		if ((pDst8[(i * 188) + 3] & 0xC0) != (temp << 6)) {
			DBG_ERR("^RSCRAMBLECTRL Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 3], i);
			return -1;
		}
	}
	...
	for (i = 0; i < seg; i++) {
		...
		if ((pDst8[(i * 188) + 3] & 0xF) != (temp & 0xF)) {
			DBG_ERR("^RCONT_COUNTER Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 3], i);
			return -1;
		}
	}

	if (((temp + 1) & 0xf) != tse_getConfig(TSMUX_CFG_ID_CON_CURR_CNT)) {
		DBG_ERR("^Rlast CONT_COUNTER Error! last=0x%d reg=0x%.8llx\n", temp, tse_getConfig(TSMUX_CFG_ID_CON_CURR_CNT));
		return -1;
	}
	...
	for (i = 0; i < seg; i++) {
		if (i == 0) {
			if ((pDst8[(i * 188) + 1] & 0x40) != (header.start_ind << 6)) {
				DBG_ERR("^RSTART_INDICATOR Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 1], i);
				return -1;
			}
		} else {
		...
		if ((pDst8[(i * 188) + 3] & 0x30) == 0x10) {
			...
			for (j = 0; j < PayloadSize; j++) {
				if (pDst8[(i * 188) + 4 + j] != pSrc8[SrcLoc + j]) {
					DBG_ERR("^RDATA Error! DstAddr=0x%p SrcAddr=0x%p  --", (void*)&pDst8[(i * 188) + 4 + j], (void*)&pSrc8[SrcLoc + j]);
			...
			if (segSize != 183) {
				...
				if (temp != pDst8[(i * 188) + 5]) {
					DBG_ERR("^RAPAPT FLAGS Error! Addr=0x%p SEG=%d\n", (void*)&pDst8[(i * 188) + 5], i);
					return -1;
				}
			}
			...
			if (StuffSize) {
				...
				for (j = 0; j < (UINT32)(StuffSize - 1); j++) {
					if (pDst8[(i * 188) + 6 + j] != temp) {
						DBG_ERR("^RSTUFF Error! DstAddr=0x%p\n", (void*)&pDst8[(i * 188) + 6 + j]);
						return -1;
					}
				}
				// Compare DATA
				for (j = 0; j < segSize; j++) {
					if (pDst8[(i * 188) + 5 + StuffSize + j] != pSrc8[SrcLoc + j]) {
						DBG_ERR("^RDATA Error! DstAddr=0x%p SrcAddr=0x%p\n", (void*)&pDst8[(i * 188) + 5 + StuffSize + j], (void*)&pSrc8[SrcLoc + j]);
						return -1;
					}
				}
			} else {
				...
				for (j = 0; j < segSize; j++) {
					if (pDst8[(i * 188) + 5 + j] != pSrc8[SrcLoc + j]) {
						DBG_ERR("^RDATA Error! DstAddr=0x%p SrcAddr=0x%p\n", (void*)&pDst8[(i * 188) + 5 + j], (void*)&pSrc8[SrcLoc + j]);
						return -1;
					}
				}
			}
...
static INT32 nvt_tse_demux_verify(void* addr, UINT32 size, UINT32 start_value)
{
	UINT32 i;
	UINT8 *ptr, data;
...
int nvt_tse_mux_demux_test(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	...
	tse_in_buf.addr = (UINT64)inbuf.vaddr;
	tse_in_buf.size = test_size;
	tse_in_buf.pnext = NULL;

	tse_out_buf.addr = (UINT64)outbuf.vaddr;
	...
	tse_setConfig(TSMUX_CFG_ID_SRC_INFO, (UINT64)&tse_in_buf);
	tse_setConfig(TSMUX_CFG_ID_DST_INFO, (UINT64)&tse_out_buf);
	...
	if (mux_out_size != tse_getConfig(TSMUX_CFG_ID_MUXING_LEN)) {
		DBG_ERR("size NG (0x%.8x, 0x%.8llx)\n", mux_out_size, tse_getConfig(TSMUX_CFG_ID_MUXING_LEN));
		ret = -1;
		goto err;
	} else {
		DBG_DUMP("size OK (0x%.8x, 0x%.8llx)\n", mux_out_size, tse_getConfig(TSMUX_CFG_ID_MUXING_LEN));
	}

	fmem_dcache_sync(outbuf.vaddr, outbuf.size, DMA_FROM_DEVICE);
	ret = nvt_tse_mux_verify((void*)tse_out_buf.addr, (void*)tse_in_buf.addr, tse_in_buf.size, header, payload, 0);
	tse_close();
	...
	tse_in_buf.addr = (UINT64)outbuf.vaddr;
	tse_in_buf.size = mux_out_size;
	tse_in_buf.pnext = NULL;

	tse_out_buf.addr = (UINT64)inbuf.vaddr;
	...
	tse_setConfig(TSDEMUX_CFG_ID_IN_INFO, (UINT64)&tse_in_buf);
	...
	tse_setConfig(TSDEMUX_CFG_ID_OUT0_INFO, (UINT64)&tse_out_buf);

	tse_start(TRUE, TSE_MODE_TSDEMUX);

	if (test_size != tse_getConfig(TSDEMUX_CFG_ID_OUT0_TOTAL_LEN)) {
		DBG_ERR("size0 NG (0x%.8x, 0x%.8llx)\r\n", test_size, tse_getConfig(TSDEMUX_CFG_ID_OUT0_TOTAL_LEN));
		ret = -1;
		goto err;
	} else {
		DBG_DUMP("size0 OK (0x%.8x, 0x%.8llx)\r\n", test_size, tse_getConfig(TSDEMUX_CFG_ID_OUT0_TOTAL_LEN));
	}
	ret = nvt_tse_demux_verify((void*)tse_out_buf.addr, test_size, 0);
