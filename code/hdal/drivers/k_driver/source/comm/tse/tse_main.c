...
#ifdef CONFIG_PM
static int nvt_tse_suspend(struct device *dev);
static int nvt_tse_resume(struct device *dev);
#endif
...
static int nvt_tse_suspend(struct device *dev)
{
	PTSE_DRV_INFO pdrv_info = dev_get_drvdata(dev);
...
static const struct dev_pm_ops nvt_tse_pmops = {
	.suspend	= nvt_tse_suspend,
	.resume		= nvt_tse_resume,
};
#define NVTTSE_PMOPS &nvt_tse_pmops
#else
#define NVTTSE_PMOPS NULL
#endif
