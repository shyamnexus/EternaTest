...
#ifdef CONFIG_PM
static int nvt_sdp_suspend(struct device *dev);
static int nvt_sdp_resume(struct device *dev);
#endif
...
static int nvt_sdp_suspend(struct device *dev)
{
	PSDP_DRV_INFO pdrv_info = dev_get_drvdata(dev);
...
static const struct dev_pm_ops nvt_sdp_pmops = {
	.suspend	= nvt_sdp_suspend,
	.resume		= nvt_sdp_resume,
};
#define NVTSDP_PMOPS &nvt_sdp_pmops
#else
#define NVTSDP_PMOPS NULL
#endif
