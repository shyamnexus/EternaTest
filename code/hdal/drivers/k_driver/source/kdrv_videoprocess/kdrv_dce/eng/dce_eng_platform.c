...
INT32 dce_eng_platform_set_clk_rate(DCE_ENG_HANDLE *p_eng)
{
	...
	if (dce_eng_platform_get_chip_id() == CHIP_NA51102) {
		if (source_clk == 240) {
	...
	if (dce_eng_platform_get_chip_id() == CHIP_NA51102) {
		if (source_clk == 240) {
			selected_clk = PLL_CLKSEL_DCE_240;
		} else if (source_clk == 320) {
			selected_clk = PLL_CLKSEL_DCE_320;
		} else if (source_clk == 480) {
			selected_clk = PLL_CLKSEL_DCE_480;
		} else {
			selected_clk = PLL_CLKSEL_DCE_PLL15;
	...
	pll_setClockRate(PLL_CLKSEL_DCE, selected_clk);
#else
#endif
...
{
	if(request_irq(p_eng->irq_id, dce_eng_platform_isr, IRQF_TRIGGER_HIGH, p_eng->name, p_eng) != 0){
		DBG_WRN("dce request_irq fail\n");
	}
}
