...
//#include "linux/soc/nvt/rcw_macro.h"  // modified for 530
...
#define KDRV_IME_ALIGN_FLOOR(value, base)  ((value) & ~((base)-1))                 ///< Align Floor
#define KDRV_IME_ALIGN_CEIL(value, base)   KDRV_IME_ALIGN_FLOOR((value) + ((base)-1), base)
...
typedef struct {
	...
	BOOL dir_start;			// 690 patch. used to check if current isr is the first frame start isr after direct start trigger. direct mode only
} KDRV_IME_HANDLE;
