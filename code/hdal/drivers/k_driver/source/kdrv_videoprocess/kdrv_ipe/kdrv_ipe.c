...
{
	if (io_cfg->mode == KDRV_IPP_OPMODE_D2D || io_cfg->mode == KDRV_IPP_OPMODE_IFE2IPE) {
		info->to_ime= FALSE;
	} else {
		info->to_ime = TRUE;
	}
	...
	if ((io_cfg->mode == KDRV_IPP_OPMODE_IFE2IME) || (io_cfg->mode == KDRV_IPP_OPMODE_DCE2IME) || (io_cfg->mode == KDRV_IPP_OPMODE_SIE2IME) || (io_cfg->mode == KDRV_IPP_OPMODE_IFE2IPE)) {
...
static INT32 kdrv_ipe_int_check_limit(KDRV_IPE_JOB_CFG *p_cfg, KDRV_IPP_PXLFMT pxl_fmt)
{
	...
	if (KDRV_IPP_FMT_CLASS(p_cfg->p_iocfg->in_frm.fmt) == KDRV_IPP_FMT_CLASS_YUV) {
		...
		if (fmt_ycc != KDRV_IPP_PXLFMT_YCC_FULL &&
			...
			DBG_ERR("unsupport PXL format 0x%.8x\r\n", (unsigned int)fmt_ycc);
			return E_PAR;
		}
	}
...
static INT32 kdrv_ipe_int_cfg_cc(KDRV_IPE_HANDLE *p_hdl, KDRV_IPE_JOB_CFG *p_cfg, KDRV_IPP_FRAME in_frm)
{
	...
	if (p_iq_cfg->update & KDRV_IPE_UPDATE_CC || p_iq_cfg->update & KDRV_IPE_UPDATE_CCM) {
		...
		if (p_iq_cfg->cc.enable) {
			...
			cc_para.cc_ofs_sel = CC_OFS_BYPASS; //tmp fix bypass due to ife always do YUV2RGB in YUV input
...
static INT32 kdrv_ipe_int_cfg_all(KDRV_IPE_HANDLE *p_hdl, KDRV_IPE_JOB_CFG *p_cfg)
{
	...
	if (p_io_cfg->mode == KDRV_IPP_OPMODE_D2D || p_io_cfg->mode == KDRV_IPP_OPMODE_IFE2IPE) {
	...
	rt |= kdrv_ipe_int_cfg_va(p_hdl, p_cfg, &single_out_info);
