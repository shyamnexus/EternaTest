...
extern BOOL _vendor_ai_net_debug_layer_write_file(CHAR *filepath, uintptr_t addr, UINT32 size);
extern HD_RESULT _vendor_ai_net_set_debug(UINT32 net_id, UINT32 debug_value);
extern HD_RESULT _vendor_ai_net_get_debug(UINT32 net_id, UINT32 *debug_value);

extern HD_RESULT perf_time_begin(UINT64 *ts_start);
extern HD_RESULT perf_time_end(UINT32 proc_id, VENDOR_AI3_PERF_TIME_CLASS_ID class_id, UINT64 ts_start, UINT32 *param);
#define PERF_TIME_BEGIN(ts_start) if((g_debug_value) & (KFLOW_AI_DBG_TIMELINE_ALL)) perf_time_begin(ts_start)
#define PERF_TIME_END(proc_id, class_id, ts_start ,param) if((g_debug_value) & (KFLOW_AI_DBG_TIMELINE_ALL)) perf_time_end(proc_id, class_id, ts_start, param)
