...
//=============================================================
#define __CLASS__ 				"[ai][lib_cpu][util]"
...
HD_RESULT vendor_ai_cpu_util_float2fixed(FLOAT *in_data, FLOAT in_scale_ratio, VOID *out_data, HD_VIDEO_PXLFMT out_fmt, INT32 data_size, INT32 zero_point)
{
	INT32 frac_bits = HD_VIDEO_PXLFMT_FRAC(out_fmt);
	UINT32 ai_pxlfmt = HD_VIDEO_PXLFMT_FMT(out_fmt);

	switch (ai_pxlfmt) {
		case HD_VIDEO_PXLFMT_AI_SINT8:
			{
                zero_point = (INT8)zero_point;
				floattoint8_1d_neon(in_data, out_data, data_size, frac_bits, in_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_UINT8:
			{
                zero_point = (UINT8)zero_point;
				floattouint8_1d_neon(in_data, out_data, data_size, frac_bits, in_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_SINT16:
			{
                zero_point = (INT16)zero_point;
				floattoint16_1d_neon(in_data, out_data, data_size, frac_bits, in_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_UINT16:
			{
                zero_point = (UINT16)zero_point;
				floattouint16_1d_neon(in_data, out_data, data_size, frac_bits, in_scale_ratio, zero_point);
			}
			break;
		default:
...
HD_RESULT vendor_ai_cpu_util_fixed2float(VOID *in_data, HD_VIDEO_PXLFMT in_fmt, FLOAT *out_data, FLOAT out_scale_ratio, INT32 data_size, INT32 zero_point)
{
	INT32 frac_bits = HD_VIDEO_PXLFMT_FRAC(in_fmt);
	UINT32 ai_pxlfmt = HD_VIDEO_PXLFMT_FMT(in_fmt);

	switch (ai_pxlfmt) {
		case HD_VIDEO_PXLFMT_AI_SINT8:
			{
                zero_point = (INT8)zero_point;
				int8tofloat_1d_neon(in_data, out_data, data_size, frac_bits, out_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_UINT8:
			{
                zero_point = (UINT8)zero_point;
				uint8tofloat_1d_neon(in_data, out_data, data_size, frac_bits, out_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_SINT16:
			{
                zero_point = (INT16)zero_point;
				int16tofloat_1d_neon(in_data, out_data, data_size, frac_bits, out_scale_ratio, zero_point);
			}
			break;
		case HD_VIDEO_PXLFMT_AI_UINT16:
			{
                zero_point = (UINT16)zero_point;
				uint16tofloat_1d_neon(in_data, out_data, data_size, frac_bits, out_scale_ratio, zero_point);
			}
			break;
		default:
			printf("vendor_ai_cpu_util_fixed2float: Not support for this input format(%lx), ai_pxlfmt(%#x)\n", (ULONG)in_fmt, ai_pxlfmt);
			break;
	}
	return HD_OK;
}
