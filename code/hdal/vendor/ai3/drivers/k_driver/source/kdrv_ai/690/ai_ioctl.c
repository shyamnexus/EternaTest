...
uintptr_t g_buf_info_pa[AI_SUPPORT_NET_MAX] ={0};
uintptr_t g_buf_info_va[AI_SUPPORT_NET_MAX] ={0};
UINT32 net_ll_size[AI_SUPPORT_NET_MAX] = {0};
uintptr_t net_ll_va[AI_SUPPORT_NET_MAX] = {0};
uintptr_t net_ll_pa[AI_SUPPORT_NET_MAX] = {0};
#else
void *g_buf_handle = NULL;
//struct nvt_fmem_mem_info_t g_buf_info = {0};
uintptr_t g_buf_info_pa =0;
uintptr_t g_buf_info_va =0;
#define MAX_LL_BUF_SIZE ((UINT32)0x2000000)
...
#if LL_SUPPORT_ROI
uintptr_t kdrv_ai_get_drv_base_va(UINT32 net_id)
{
#if LL_BUF_FROM_USR
	...
	return (uintptr_t)(g_buf_info.vaddr + net_id*AI_SINGLE_NET_LL_SIZE);
#endif
}

uintptr_t kdrv_ai_get_drv_base_pa(UINT32 net_id)
{
#if LL_BUF_FROM_USR
	...
	return (uintptr_t)(g_buf_info.paddr + net_id*AI_SINGLE_NET_LL_SIZE);
#endif
}
...
static BOOL nvt_ai_drv_is_mem_map(UINT32 net_id)
{
	...
	if ((p_mem->kerl_pa == 0) || (p_mem->kerl_va == 0) || (p_mem->user_pa == 0) || (p_mem->user_va == 0)) {
		nvt_dbg(ERR, "map mem user va=%p, user pa=%p, kernel va=%p, kernel pa=%p\r\n"
				, (void *)p_mem->user_va, (void *)p_mem->user_pa, (void *)p_mem->kerl_va, (void *)p_mem->kerl_pa);
		return FALSE;
	} else {
		return TRUE;
	}
...
static uintptr_t nvt_ai_drv_user2kerl_va(uintptr_t addr, UINT32 net_id)
{
	AI_DRV_MAP_MEM *p_mem;
	net_id = nvt_ai_chk_net_id(net_id);
	p_mem = &g_ai_drv_map_mem[net_id];
	...
	if (addr < p_mem->user_va) {
		nvt_dbg(ERR, "user va %p should > %p\r\n", (void *)addr, (void *)p_mem->user_va);
		return addr;
	}
	return addr - p_mem->user_va + p_mem->kerl_va;
}

static uintptr_t nvt_ai_drv_kerl2user_va(uintptr_t addr, UINT32 net_id)
{
	AI_DRV_MAP_MEM *p_mem;
	net_id = nvt_ai_chk_net_id(net_id);
	p_mem = &g_ai_drv_map_mem[net_id];
	...
	if (addr < p_mem->kerl_va) {
		nvt_dbg(ERR, "kernel va %p should > %p\r\n", (void *)addr, (void *)p_mem->kerl_va);
		return addr;
	}
...
void nvt_ai_drv_init_rtos(void)
{
	...
	nue_set_base_addr((uintptr_t)(0xF0C60000)); 
	nue2_set_base_addr((uintptr_t)(0xF0D50000));
	cnn_set_base_addr(0, (uintptr_t)(0xF0CB0000));
#if defined(_BSP_NA51102_)
#else
	cnn_set_base_addr(1, (uintptr_t)(0xf0d60000));
#endif
...
AI_IOC_SET_MODE_EXIT:
	...
	case AI_IOC_SET_APP:
		...
		ai_app_info.info.p_head = (KDRV_AI_APP_HEAD*)nvt_ai_drv_user2kerl_va((uintptr_t)ai_app_info.info.p_head, ai_app_info.net_id);
		if ((uintptr_t)ai_app_info.info.p_head == 0) {
			goto exit;
		}
...
AI_IOC_SET_APP_EXIT:
	...
	case AI_IOC_SET_LL:
		...
		ai_ll_info.info.p_head = (KDRV_AI_LL_HEAD*)nvt_ai_drv_user2kerl_va((uintptr_t)ai_ll_info.info.p_head, ai_ll_info.net_id);
		if ((uintptr_t)ai_ll_info.info.p_head == 0) {
			goto exit;
		}
...
AI_IOC_GET_MODE_EXIT:
	...
	case AI_IOC_GET_APP:
		...
		ai_app_info.info.p_head = (KDRV_AI_APP_HEAD*)nvt_ai_drv_kerl2user_va((uintptr_t)ai_app_info.info.p_head, ai_app_info.net_id);
		if ((uintptr_t)ai_app_info.info.p_head == 0) {
			goto exit;
		}
...
AI_IOC_GET_APP_EXIT:
	...
	case AI_IOC_GET_LL:
		...
		ai_ll_info.info.p_head = (KDRV_AI_LL_HEAD*)nvt_ai_drv_kerl2user_va((uintptr_t)ai_ll_info.info.p_head, ai_ll_info.net_id);
		if ((uintptr_t)ai_ll_info.info.p_head == 0) {
			goto exit;
		}
