...
static int nvt_ai_probe(struct platform_device *pdev)
{
	...
	tcm_va = (uintptr_t)ioremap(0x4f07f0040, 4);
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		if (ucloop < 7) {
			snprintf(pstr1, 16, "4%08x.ai", (UINT32)pdrv_info->presource[ucloop]->start);
		} else if (ucloop < 9) {
			// get ROU2/LSU2 part
			snprintf(pstr1, 16, "4%08x.ai", (UINT32)pdrv_info->presource[ucloop+2]->start);
		} else {
			// get additional conv auto gating
			snprintf(pstr1, 16, "4%08x.ai_c%d", (UINT32)pdrv_info->presource[0]->start, ucloop - 8);
		}

		ai_clk[ucloop] = clk_get(&pdev->dev, pstr1);
	...
	pdrv_info->pmodule_class = class_create("kdrv_ai");
