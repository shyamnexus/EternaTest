...
irqreturn_t nvt_ai_drv_isr(int irq, void *devid)
{
	...
	if (irq == pmodule_info->iinterrupt_id[3]) {
		CONV_ENG_HANDLE* p_handle = conv_eng_get_handle(0, 1);
		conv_eng_isr_hw_reg(p_handle);
	}
	
	if (irq == pmodule_info->iinterrupt_id[4]) {
		CONV_ENG_HANDLE* p_handle = conv_eng_get_handle(0, 2);
		conv_eng_isr_hw_reg(p_handle);
	}
	
	if (irq == pmodule_info->iinterrupt_id[5]) {
		CONV_ENG_HANDLE* p_handle = conv_eng_get_handle(0, 3);
		conv_eng_isr_hw_reg(p_handle);
	}
	...
	if (irq == pmodule_info->iinterrupt_id[11]) {
