...
INT32 nue2_eng_platform_set_clk_rate(NUE2_ENG_HANDLE *p_eng)
{
	...
	if (nue2_eng_platform_get_chip_id() == CHIP_NA51102) {
        if(source_clk >= 800) {
            parent_clk = clk_get(NULL, "pll29");
            if(source_clk > 800) {
                DBG_ERR("%s: input frequency %d nue2nd to 800\r\n", __func__, source_clk);
            }
            ...
            DBG_ERR("%s: illegal input frequency %u, set as 320\r\n", __func__, source_clk);
    		parent_clk = clk_get(NULL, "fix320m");
        }
	} else {
		parent_clk = clk_get(NULL, "pll29");
	...
	if (nue2_eng_platform_get_chip_id() == CHIP_NA51102) {
        if(source_clk >= 800) {
            ...
            selected_clock = PLL_CLKSEL_NUE2_PLL26;
            if(source_clk > 600) {
                DBG_ERR("%s: input frequency %d nue2nd to 600\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 480) {
