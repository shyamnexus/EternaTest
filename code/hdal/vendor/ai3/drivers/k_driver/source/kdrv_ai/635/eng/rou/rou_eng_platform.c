...
VOID rou_eng_platform_disable_sram_shutdown(ROU_ENG_HANDLE *p_eng)
{
    ...
    if(p_eng->eng_id == ROU_ID) {
        ...
        //nvt_disable_sram_shutdown(ROU2_SD); //TODO
    }
#elif defined (__FREERTOS)
    if(p_eng->eng_id == ROU_ID) {
	    nvt_disable_sram_shutdown(NUE_SD);
    } else if(p_eng->eng_id == ROU2_ID) {
        nvt_disable_sram_shutdown(ROU2_SD);
    }
#else
#endif
}
...
VOID rou_eng_platform_enable_sram_shutdown(ROU_ENG_HANDLE *p_eng)
{
	...
	if(p_eng->eng_id == ROU_ID) {
        ...
        //nvt_enable_sram_shutdown(ROU2_SD); //TODO
    }
#elif defined (__FREERTOS)
	if(p_eng->eng_id == ROU_ID) {
		nvt_enable_sram_shutdown(NUE_SD);
	} else if(p_eng->eng_id == ROU2_ID) {
        nvt_enable_sram_shutdown(ROU2_SD);
    }
#else
#endif
}
...
INT32 rou_eng_platform_set_clk_rate(ROU_ENG_HANDLE *p_eng)
{
	...
	if (p_eng->eng_id == ROU_ID) {
		...
		node = of_find_node_by_name(from, "clk_ai7");
	}
	if (node) {
	...
	if (rou_eng_platform_get_chip_id() == CHIP_NS02401) {
        if(source_clk >= 800) {
...
#elif defined (__FREERTOS )
	UINT32 selected_clock = p_eng->eng_id == ROU_ID ? PLL_CLKSEL_NUE30_480 : PLL_CLKSEL_ROU2_480;
	UINT32 source_clk;
	...
	if (rou_eng_platform_get_chip_id() == CHIP_NS02401) {
        if(source_clk >= 850) {
            selected_clock = p_eng->eng_id == ROU_ID ? PLL_CLKSEL_NUE30_PLL28 : PLL_CLKSEL_ROU2_PLL28;
            if(source_clk > 850) {
                DBG_ERR("%s: input frequency %d round to 850\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 800) {
            selected_clock = p_eng->eng_id == ROU_ID ? PLL_CLKSEL_NUE30_PLL29 : PLL_CLKSEL_ROU2_PLL29;
            if(source_clk > 800) {
                DBG_ERR("%s: input frequency %d round to 800\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 600) {
            selected_clock = p_eng->eng_id == ROU_ID ? PLL_CLKSEL_NUE30_PLL17 : PLL_CLKSEL_ROU2_PLL17;
            if(source_clk > 600) {
                DBG_ERR("%s: input frequency %d round to 600\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 480) {
            selected_clock = p_eng->eng_id == ROU_ID ? PLL_CLKSEL_NUE30_480 : PLL_CLKSEL_ROU2_480;
            if(source_clk > 480) {
                DBG_ERR("%s: input frequency %d round to 480\r\n", __func__, source_clk);
            }
        } else {
    ...
    if(p_eng->eng_id == ROU_ID) {
        ...
        pll_setClockRate(PLL_CLKSEL_ROU2, selected_clock);
    }
#else
#endif
