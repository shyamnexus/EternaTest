...
{
	const UINT32 NUE2_MAX_CLK_FREQ = 600;
	...
	if (nue2_eng_platform_get_chip_id() == CHIP_NS02302) {
		if(source_clk == 480) {
...
#elif defined (__FREERTOS )
	UINT32 selected_clock = PLL_CLKSEL_NUE2_PLL10;
	UINT32 source_clk;
	...
	if (nue2_eng_platform_get_chip_id() == CHIP_NS02302) {
        if(source_clk >= NUE2_MAX_CLK_FREQ) {
            selected_clock = PLL_CLKSEL_NUE2_PLL10;
            if(source_clk > NUE2_MAX_CLK_FREQ) {
                DBG_ERR("%s: input frequency %d nue2nd to 600\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 480) {
            selected_clock = PLL_CLKSEL_NUE2_480;
            if(source_clk > 480) {
                DBG_ERR("%s: input frequency %d nue2nd to 480\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 320) {
            selected_clock = PLL_CLKSEL_NUE2_320;
            if(source_clk > 320) {
                DBG_ERR("%s: input frequency %d nue2nd to 320\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 240) {
            selected_clock = PLL_CLKSEL_NUE2_240;
            if(source_clk > 240) {
                DBG_ERR("%s: input frequency %d nue2nd to 240\r\n", __func__, source_clk);
            }
        } else {
