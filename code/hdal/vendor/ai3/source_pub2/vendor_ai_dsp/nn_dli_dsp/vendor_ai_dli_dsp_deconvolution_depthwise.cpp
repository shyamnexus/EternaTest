/**
    @brief Source file of DLI dsp deconvolution_depthwise layer.

    @file vendor_ai_dli_dsp_deconvolution_depthwise.cpp

    @ingroup dli_dsp_deconvolution_depthwise
...
#include "vendor_ai_dli_dsp_deconvolution_depthwise.hpp"
...
// VOID dli_nvtnn_dsp_flush_cache_deconvolution_depthwise(NVTNN_DECONVOLUTION_PRMS *p_nvtnn_prms) {
...
static VOID dli_to_nvtnn_deconvolution_depthwise(NN_DLI_DECONVOLUTION_PARM *p_dli_parm, NVTNN_DECONVOLUTION_PRMS *p_nvtnn_prms)
{
	...
	this file weight shape:      [outchannel, inchannel, kernel_h,  kernel_w]
    */
	...
	p_nvtnn_prms->in_ofs.line_ofs          = p_nvtnn_prms->inshape.width;
	p_nvtnn_prms->in_ofs.channel_ofs       = p_nvtnn_prms->in_ofs.line_ofs * p_nvtnn_prms->inshape.height;
	p_nvtnn_prms->in_ofs.batch_ofs         = p_nvtnn_prms->in_ofs.channel_ofs * p_nvtnn_prms->inshape.channel;
	p_nvtnn_prms->weight_ofs.line_ofs      = p_nvtnn_prms->wtshape.width;
	p_nvtnn_prms->weight_ofs.channel_ofs   = p_nvtnn_prms->weight_ofs.line_ofs * p_nvtnn_prms->wtshape.height;
	p_nvtnn_prms->weight_ofs.batch_ofs     = p_nvtnn_prms->weight_ofs.channel_ofs * p_nvtnn_prms->wtshape.channel;
	...
	// dli_nvtnn_dsp_flush_cache_deconvolution_depthwise(p_nvtnn_prms);

	#if 0
	if(p_nvtnn_prms->outshape.channel!=p_nvtnn_prms->inshape.channel || p_nvtnn_prms->outshape.channel!=p_nvtnn_prms->group) {
		printf("error outchannel==inchannel==group in depthwise deconvolution\n");
		exit(-1);
	}
	printf("weight_info->data_va: %#x\n", weight_info->data_va);
	char out_file_dir1[200];
	sprintf(out_file_dir1, "input.txt");
	FILE *fp1 = fopen(out_file_dir1, "w");
	if (fp1) {
		for (int n = 0; n < 19*(27+1)*(27+1); n++) {
			fprintf(fp1, "%.9f\n", ((short *)(p_nvtnn_prms->in_addr))[n]/(float)(1<<10));
		}
		fclose(fp1);
	}
	printf("weight_info->data_va: %#x\n", weight_info->data_va);
	char out_file_dir2[200];
	sprintf(out_file_dir2, "weight.txt");
	FILE *fp2 = fopen(out_file_dir2, "w");
	if (fp2) {
		for (int n = 0; n < 19*16*16; n++) {
			fprintf(fp2, "%.9f\n", ((UINT8 *)(p_nvtnn_prms->weight_addr))[n]/(float)(1<<0));
		}
		fclose(fp2);
	}
...
HD_RESULT dli_dsp_deconvolution_depthwise_proc(NN_DLI_DECONVOLUTION_PARM *p_parm, NVTNN_SENDTO dsp_core)
{
	...
	// Convert parameters
	dli_to_nvtnn_deconvolution_depthwise(p_parm, &nvtnn_prms);
	HD_RESULT ret = nvtnndsp_op_proc(NN_DLI_DECONVOLUTION_DEPTHWISE, &nvtnn_prms, dsp_core);

	// char out_file_dir1[160];
	// sprintf(out_file_dir1, "output.txt");
	// FILE *fp1 = fopen(out_file_dir1, "w");
	// if (fp1) {
	// 	for (int n = 0; n < (19*224*224); n++) {
	// 		fprintf(fp1, "%.9f\n", ((short *)(nvtnn_prms.out_addr))[n]/(float)(1<<10));
	// 	}
	// 	fclose(fp1);
	// }
	// Execute dsp
