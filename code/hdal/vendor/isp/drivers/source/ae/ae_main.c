...
// global
//=============================================================================
INT32 ae_ioctl(INT32 cmd, ULONG arg, ULONG *buf_addr)
{
	INT32 ret = 0;
	UINT32 size;
	...
	if ((_IOC_DIR(cmd) == _IOC_READ) || (_IOC_DIR(cmd) == (_IOC_READ|_IOC_WRITE))) {
		...
		if (_IOC_NR(cmd) < AET_ITEM_NNSC_OFFSET) {
			ret = aet_api_get_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		} else {
			ret = aet_api_nnsc_get_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		}
		if (ret < 0) {
		...
		if (_IOC_NR(cmd) < AET_ITEM_NNSC_OFFSET) {
			ret = aet_api_set_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		} else {
			ret = aet_api_nnsc_set_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		}
		if (ret < 0) {
