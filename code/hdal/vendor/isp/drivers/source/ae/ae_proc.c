...
static UINT32 power_f = AE_ID_MAX_NUM;
...
static const struct proc_ops ae_proc_info_ops = {
	.proc_open    = ae_proc_info_open,
	.proc_read    = seq_read,
...
static s32 ae_proc_cmd_fusion_adjust_ratio(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	UINT32 adjust_ratio;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	adjust_ratio = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->adjust_ratio = adjust_ratio;

	ae_cmd_printf("id = %d, adjust_ratio= %d\n", id, ae_proc_cmd->adjust_ratio);

	ae_proc_cmd->cmd = AE_PROC_CMD_ADJUST_RATIO;
	return 0;
}

static s32 ae_proc_cmd_gamma_sel(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	UINT32 gamma_sel;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	gamma_sel = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->gamma_sel = gamma_sel;

	ae_cmd_printf("id = %d, gamma_sel= %d\n", id, ae_proc_cmd->gamma_sel);

	ae_proc_cmd->cmd = AE_PROC_CMD_GAMMA_SEL;
	return 0;
}
...
static s32 ae_proc_cmd_low_power_cnt(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	u32 low_power_cnt;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	ae_proc_cmd->low_power_cnt = low_power_cnt;

	// base 16
	ae_cmd_printf("id = %d, low_power_cnt = %d\n", id, ae_proc_cmd->low_power_cnt);

	ae_proc_cmd->cmd = AE_PROC_CMD_LOW_POWER_CNT;
...
static INT32 ae_proc_cmd_reload_cfg(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	...
	aet_api_set_cmd(AET_ITEM_RLD_CONFIG, (ULONG)&cfg_info);
...
static INT32 ae_proc_cmd_reload_dtsi(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	...
	aet_api_set_cmd(AET_ITEM_RLD_DTSI, (ULONG)&dtsi_info);
...
static INT32 ae_proc_cmd_low_power_info(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
...
static AE_PROC_CMD ae_proc_r_cmd_list[] = {
	...
	{ "low_power_info",     ae_proc_cmd_low_power_info,  "get ae low power info, param: [id]"}
};
...
static AE_PROC_CMD ae_proc_w_cmd_list[] = {
	...
	{ "gamma_sel",          ae_proc_cmd_gamma_sel,         "select fusion gamma, param: [id] [gamma_sel]"},
	{ "adj_ratio",          ae_proc_cmd_fusion_adjust_ratio,  "select fusion adjust ratio, param: [id] [adj_ratio]"},
	...
	{ "low_power_cnt",     	ae_proc_cmd_low_power_cnt,     "select low power, param: [id] [low_power_cnt]"}
};
...
#define LA_WIN_Y 32
	...
	if (g_cmd_msg.buf == NULL && la_f >= AE_ID_MAX_NUM && param_f >= AE_ID_MAX_NUM  && power_f >= AE_ID_MAX_NUM) {
		return -EINVAL;
	}
	...
	if (param_f < AE_ID_MAX_NUM) {
		...
		seq_printf(sfile, "%d %d\n", ae_param[param_f]->curve_gen_movie->hdr_ratio[0] >> 6, ae_param[param_f]->curve_gen_movie->hdr_ratio[1] >> 6);
		...
		//seq_printf(sfile, "\n");
		//seq_printf(sfile, "%d \n", ae_param[param_f]->shdr->metering_id);
		seq_printf(sfile, "\n");
	...
	if (power_f < AE_ID_MAX_NUM) {
		seq_printf(sfile, "id = %d\n", power_f);
		ae_msg_low_power(sfile, power_f);
	}
	power_f = AE_ID_MAX_NUM;
...
static const struct proc_ops ae_proc_command_ops = {
	.proc_open    = ae_proc_command_open,
	.proc_read    = seq_read,
	.proc_write   = ae_proc_command_write,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release,
};
...
//=============================================================================
unsigned int ae_debug_level = 3;

static INT32 ae_proc_dbglevel_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "0: fatal; 1: err; 2: wrn; 3: msg; 4: ind; 5: func \n");
	seq_printf(sfile, "ae_debug_level = %d \n", ae_debug_level);

	return 0;
}

static INT32 ae_proc_dbglevel_open(struct inode *inode, struct file *file)
{
	return single_open(file, ae_proc_dbglevel_show, PDE_DATA(inode));
}

static ssize_t ae_proc_dbglevel_write(struct file *file, const CHAR __user *buf, size_t size, loff_t *off)
{
	INT32 len = size;
	INT32 ret = -EINVAL;
	CHAR cmd_line[MAX_CMDLINE_LENGTH];
	...
	if ((len <= 1) || (len > MAX_CMDLINE_LENGTH)) {
		...
		if (copy_from_user(cmd_line, buf, len)) {
			...
			ae_debug_level = simple_strtoul(argv[0], NULL, 0);
			ae_cmd_printf("set dbglevel %d \n", ae_debug_level);
		}
	}
...
static const struct proc_ops ae_proc_dbglevel_ops = {
	.proc_open    = ae_proc_dbglevel_open,
	.proc_read    = seq_read,
	.proc_write   = ae_proc_dbglevel_write,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release
};
...
static INT32 ae_proc_help_show(struct seq_file *sfile, void *v)
{
	...
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_FCC             |                                                              \n", AE_DBG_FCC);
	...
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_AFD             |                                                              \n", AE_DBG_AFD);
...
static const struct proc_ops ae_proc_help_ops = {
	.proc_open    = ae_proc_help_open,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release,
};
...
static INT32 ae_proc_status_show(struct seq_file *sfile, void *v)
{
	AE_ID id;
	UINT32 i;
	UINT32 isp_gain[AE_ID_MAX_NUM] = {128, 128, 128, 128, 128, 128};
...
static const struct proc_ops ae_proc_status_ops = {
	.proc_open    = ae_proc_status_open,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release,
};
...
INT32 ae_proc_create()
{
	...
	// create "info" entry
	pentry = proc_create_data("info", S_IRUGO | S_IXUGO, root, &ae_proc_info_ops, NULL);
	if (pentry == NULL) {
	...
	pentry = proc_create_data("cmd", S_IRUGO | S_IXUGO, root, &ae_proc_command_ops, NULL);
	if (pentry == NULL) {
	...
	pentry = proc_create_data("dbglevel", S_IRUGO | S_IXUGO, root, &ae_proc_dbglevel_ops, NULL);
	if (pentry == NULL) {
	...
	// create "help" entry
	pentry = proc_create_data("help", S_IRUGO | S_IXUGO, root, &ae_proc_help_ops, NULL);
	if (pentry == NULL) {
	...
	// create "status" entry
	pentry = proc_create_data("status", S_IRUGO | S_IXUGO, root, &ae_proc_status_ops, NULL);
	if (pentry == NULL) {
