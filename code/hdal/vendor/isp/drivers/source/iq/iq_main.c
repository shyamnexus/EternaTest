...
UINT32 iq_3dcc_en = 1;
module_param_named(iq_3dcc_en, iq_3dcc_en, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(iq_3dcc_en, "En of iq 3dcc");
...
// global
//=============================================================================
INT32 iq_ioctl(INT32 cmd, ULONG arg, ULONG *buf_addr)
{
	INT32 ret = 0;
	UINT32 size;
	...
	if ((_IOC_DIR(cmd) == _IOC_READ) || (_IOC_DIR(cmd) == (_IOC_READ|_IOC_WRITE))) {
		...
		if (_IOC_NR(cmd) < IQT_ITEM_NNSC_OFFSET) {
			ret = iqt_api_get_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		} else {
			ret = iqt_api_nnsc_get_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		}
		if (ret < 0) {
		...
		if (_IOC_NR(cmd) < IQT_ITEM_NNSC_OFFSET) {
			ret = iqt_api_set_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		} else {
			ret = iqt_api_nnsc_set_cmd(_IOC_NR(cmd), (ULONG) buf_addr);
		}
		if (ret < 0) {
...
static INT32 __init iq_module_init(void)
{
	...
	if (nvt_get_chip_id() != CHIP_NS02402) {
		pr_err("invalid chip id, insmod wrong iq ko?\n");
		return -EINVAL;
	}
#else
	...
	iq_init_module(iq_id_list, iq_dpc_en, iq_ecs_en, iq_3dcc_en);
