...
#define ISP_IF_IPP_REG_EVENT        ISP_EVENT_IPP_PROCEND | ISP_EVENT_IPP_CFGSTART | ISP_EVENT_IPP_CFGSTART_IMM | ISP_EVENT_PARAM_RST | ISP_EVENT_CFGINIT
...
CTL_IPP_ISP_BNR_STA ctl_ipp_isp_bnr_sta;
...
static INT32 isp_api_cb_sie(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	if (sie_ret == CTL_SIE_E_OK) {
		...
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_FUNC_EN fail (%8X) (%d) (0x%X) \r\n", id, sie_ret, fun_en_tmp);
	}
	...
	if (sie_ret == CTL_SIE_E_OK) {
		...
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_MULTI_FRM_GRP fail (%8X) (%d) (0x%X) \r\n", id, sie_ret, frm_grp_tmp);
	}
	...
	if ((sie_ret == CTL_SIE_E_OK) && (src_id_tmp < SIE_ID_MAX_NUM)) {
		...
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_DUPL_SRC_ID fail (%8X) (%d) (%d) \r\n", id, sie_ret, src_id_tmp);
	}
	...
	if ((evt & ISP_EVENT_PARAM_RST) || (evt & ISP_EVENT_VCAP_VD)) {
		...
		if (evt & ISP_EVENT_PARAM_RST) {
			...
			PRINT_ISP(dbg_mode & ISP_DBG_SIE_CB, "sie rst,   id = %8X, frm = %d \r\n", id, frame_cnt);
		} else {
			ae_trig_obj.reset = FALSE;
			awb_trig_obj.reset = FALSE;
			iq_trig_obj.msg = ISP_TRIG_IQ_SIE;
			iq_trig_obj.sie.reset = FALSE;
			PRINT_ISP(dbg_mode & ISP_DBG_SIE_CB, "sie cb,    id = %8X, frm = %d \r\n", id, frame_cnt);
		}
...
static INT32 isp_api_cb_ipp(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	CTL_IPP_ISP_LOW_PWR_INFO low_pwr_info = {0};
	...
	low_pwr_info.level = low_power_lv;
	ctl_ipp_isp_set(id, CTL_IPP_ISP_ITEM_LOW_PWR_INFO, &low_pwr_info);
	if (evt == ISP_EVENT_CFGINIT){
		return 0;
	}
	...
	if (rt != E_OK) {
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_STATUS_INFO fail (%8X) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
	}
	...
	if (rt != E_OK) {
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_FUNC_EN fail, do not trigger (%8X) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
		return 0;
	}

	rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_STRIPE_INFO, &strp_info);
	if (rt != E_OK) {
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_STRIPE_INFO fail (%8X) (%d) (%d) (%d) \r\n", id, frame_cnt, strp_info.num, rt);
	}
	...
	case ISP_EVENT_IPP_CFGSTART_IMM:
		if (evt == ISP_EVENT_PARAM_RST) {
			...
			PRINT_ISP(dbg_mode & ISP_DBG_IPP_CB, "ipp rst,   id = %8X, frm = %d, func = 0x%X \r\n", id, frame_cnt, fun_en);
			...
			iq_trig_obj.ipp.ipp_capture = FALSE;
			if (ipp_info == 0) {
				PRINT_ISP(dbg_mode & ISP_DBG_IPP_CB, "ipp cb_M,  id = %8X, frm = %d, func = 0x%X \r\n", id, frame_cnt, fun_en);
			} else {
				PRINT_ISP(dbg_mode & ISP_DBG_IPP_CB, "ipp cb_S,  id = %8X, frm = %d, func = 0x%X \r\n", id, frame_cnt, fun_en);
			}
		} else if (evt == ISP_EVENT_IPP_CFGSTART_IMM) {
			iq_trig_obj.msg = ISP_TRIG_IQ_IPP_IMM;
			iq_trig_obj.ipp.reset = FALSE;
			iq_trig_obj.ipp.ipp_capture = TRUE;
			PRINT_ISP(dbg_mode & ISP_DBG_IPP_CB, "ipp cb_C,  id = %8X, frm = %d, func = 0x%X \r\n", id, frame_cnt, fun_en);
		}
		...
		if (fun_en & ISP_FUNC_EN_WDR) {
			...
			if (rt != E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_IFE_HIST_RST fail (%8X) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
			}
		}
		...
		if (fun_en & ISP_FUNC_EN_DEFOG) {
			...
			if (rt != E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_DEFOG_STCS fail (%8X) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
			}

			if (evt != ISP_EVENT_PARAM_RST) {
				rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_DEFOG_SUBOUT, &ipp_isp_defog_subout);
				if (rt != E_OK) {
					PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_DEFOG_SUBOUT fail (%8X) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
				}
		...
		if (pdev_info->isp_iq != NULL) {
			...
			iq_trig_obj.ipp.stripe_num = strp_info.num;
		...
		case ISP_EVENT_IPP_PROCEND:
			...
			if (fun_en & ISP_FUNC_EN_AF) {
				rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_PRE_VA_RST, pdev_info->ife_va_rslt[ipp_id]);
				rt |= ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_PRE_VA_INDEP_RST, pdev_info->ife_va_indep_rslt[ipp_id]);
				if (rt != E_OK) {
					PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_PRE_VA_RST fail (%d) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
				}
			}
...
ER isp_api_get_bnr_sta(ISP_ID id, ISP_BNR_STA_INFO *bnr_sta_info)
{
	...
	rt = ctl_ipp_isp_get(ipp_id_table[id], CTL_IPP_ISP_ITEM_BNR_STA, &ctl_ipp_isp_bnr_sta);

	if (rt == E_OK) {
		bnr_sta_info->info_vaild = TRUE;
		bnr_sta_info->enable = ctl_ipp_isp_bnr_sta.enable;
		bnr_sta_info->buf_addr = ctl_ipp_isp_bnr_sta.buf_addr;
		bnr_sta_info->buf_phyaddr = ctl_ipp_isp_bnr_sta.buf_phyaddr;
		bnr_sta_info->lofs = ctl_ipp_isp_bnr_sta.lofs;
	} else {
...
void isp_api_set_iq_param(ISP_ID id, ISP_IQ_ITEM isp_iq_item, void *param)
{
	...
	case ISP_IQ_ITEM_PRE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_PRE_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_PRE_IQ_PARAM, (CTL_PRE_ISP_IQ_ALL *)param);
			PRINT_ISP(isp_dbg_get_dbg_mode(isp_id) & ISP_DBG_IPP_SET, "ctl_ipp_isp_set (%X) PRE_IQ_PARAM \r\n", ipp_id_table[isp_id]);
		}
	break;

	case ISP_IQ_ITEM_AIISP_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_AIISP_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_AIISP_PARAM, (CTL_IPP_ISP_AIISP_PARAM *)param);
			PRINT_ISP(isp_dbg_get_dbg_mode(isp_id) & ISP_DBG_AIISP_SET, "ctl_ipp_isp_set (%X) AIISP_PARAM \r\n", ipp_id_table[isp_id]);
		}
	break;
	...
	case ISP_IQ_ITEM_PRE_VA_WIN_SIZE:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IFE_VA_WIN_SIZE)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_PRE_VA_WIN_SIZE, (CTL_IPP_ISP_PRE_VA_WIN_SIZE_RATIO *)param);
		}
	break;
