...
// define
//=============================================================================
#if defined(__KERNEL__) && defined(CONFIG_PWM_NVT)
...
ER isp_uti_enable_pwm(UINT32 pwm_id, UINT32 period, UINT32 duty_cycle, UINT32 polarity)
{
	...
	if (nvt_pwm_enable[pwm_id] == FALSE) {
		...
		if ((nvt_pwm_dev[pwm_id] == NULL) || (nvt_pwm_dev[pwm_id] == (void *)0xFFFFFFF0)) {
			PRINT_ISP_WRN(isp_dbg_get_dbg_mode(pwm_id) & ISP_DBG_WRN_MSG, "pwm request fail, id (%d), lable (%s), dev (0x%lX) \r\n", pwm_id, lable, (ULONG)nvt_pwm_dev[pwm_id]);
			return E_SYS;
		}
