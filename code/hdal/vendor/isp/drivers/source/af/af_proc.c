...
static const struct proc_ops af_proc_info_ops = {
	.proc_open    = af_proc_info_open,
	.proc_read    = seq_read,
...
static INT32 af_proc_get_cmd_enable(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	...
	if(af_param[id] == NULL){
		DBG_ERR("af id %u is not active", id);
		return 0;
	}
...
static INT32 af_proc_set_cmd_rld_cfg(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	...
	if(af_param[cfg_info.id] == NULL){
		DBG_ERR("af id %u is not active", cfg_info.id);
		return 0;
	}
	...
	cfg_info.path[AF_CFG_NAME_LENGTH-1] = '\0';
	aft_api_set_cmd(AFT_ITEM_RLD_CONFIG, (ULONG) &cfg_info);
...
static const struct proc_ops af_proc_command_ops = {
	.proc_open    = af_proc_command_open,
	.proc_read    = seq_read,
	.proc_write   = af_proc_command_write,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release,
};
...
//=============================================================================
unsigned int af_debug_level = 3;

static INT32 af_proc_dbglevel_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "0: fatal; 1: err; 2: wrn; 3: msg; 4: ind; 5: func \n");
	seq_printf(sfile, "af_debug_level = %d \n", af_debug_level);

	return 0;
}

static INT32 af_proc_dbglevel_open(struct inode *inode, struct file *file)
{
	return single_open(file, af_proc_dbglevel_show, PDE_DATA(inode));
}

static ssize_t af_proc_dbglevel_write(struct file *file, const CHAR __user *buf, size_t size, loff_t *off)
{
	INT32 len = size;
	INT32 ret = -EINVAL;
	CHAR cmd_line[MAX_CMDLINE_LENGTH];
	...
	if ((len <= 1) || (len > MAX_CMDLINE_LENGTH)) {
		...
		if (copy_from_user(cmd_line, buf, len)) {
			...
			if (argc != 1) {
				af_cmd_printf("wrong argument:%d", argc);
				return -EINVAL;
			}

			af_debug_level = simple_strtoul(argv[0], NULL, 0);
			af_cmd_printf("set dbglevel %d \n", af_debug_level);
		}
	}
...
static const struct proc_ops af_proc_dbglevel_ops = {
	.proc_open    = af_proc_dbglevel_open,
	.proc_read    = seq_read,
	.proc_write   = af_proc_dbglevel_write,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release
};
...
static const struct proc_ops af_proc_help_ops = {
	.proc_open    = af_proc_help_open,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
	.proc_release = single_release,
};
...
INT32 af_proc_create(void)
{
	...
	// create "info" entry
	pentry = proc_create_data("info", S_IRUGO | S_IXUGO, root, &af_proc_info_ops, NULL);
	if (pentry == NULL) {
	...
	// create "cmd" entry
	pentry = proc_create_data("cmd", S_IRUGO | S_IXUGO, root, &af_proc_command_ops, NULL);
	if (pentry == NULL) {
	...
	pentry = proc_create_data("dbglevel", S_IRUGO | S_IXUGO, root, &af_proc_dbglevel_ops, NULL);
	if (pentry == NULL) {
	...
	// create "help" entry
	pentry = proc_create_data("help", S_IRUGO | S_IXUGO, root, &af_proc_help_ops, NULL);
	if (pentry == NULL) {
