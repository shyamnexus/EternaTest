...
{
	memset(&iq_info->final_setting.pre, 0, sizeof(CTL_PRE_ISP_IQ_ALL));
...
static ER iq_flow_set_sie_param(IQALG_INFO *iq_info)
{
	...
	if ((iq_info->dpc_en == TRUE) && (iq_info->sie_tab_update & IQ_TBL_SIE_DPC)) {
		...
		iq_info->final_setting.sie.dpc_param->def_same_ch_enb = iq_info->final_sie.sie_dpc->def_same_ch_only_en;
	...
	if (iq_info->low_power_lv || reset_lv0) {
		UINT32 final_lv = 0;

		if (iq_info->low_power_lv) {
			reset_lv0 = TRUE;
		} else {
			reset_lv0 = FALSE;
		}
		...
		if (iq_info->dpc_en) {
			final_lv = (iq_info->low_power_param.dpc) ? iq_info->low_power_param.dpc : iq_info->low_power_lv;
			switch (final_lv) {
			case 0:
			default:
				iq_info->final_setting.sie.dpc_param->enable = iq_info->final_sie.sie_dpc->enable;
		...
		iq_info->final_setting.sie.ecs_param = &(iq_info->final_setting.sie_ecs_param);
		if (iq_info->ecs_en) {
			final_lv = (iq_info->low_power_param.ecs) ? iq_info->low_power_param.ecs : iq_info->low_power_lv;
			switch (final_lv) {
			case 0:
			default:
				iq_info->final_setting.sie.ecs_param->enable = iq_info->final_sie.sie_ecs->enable;
			...
			case 4:
			case 5:
				iq_info->final_setting.sie.ecs_param->enable = FALSE;
				break;
			};
		}
...
static ER iq_flow_set_ipp_setting(IQALG_INFO *iq_info)
{
	...
	iq_info->final_setting.ipp.pre_va_win_size = &(iq_info->final_ipp.pre_va_win_size);
	isp_api_set_iq_param(isp_id, ISP_IQ_ITEM_PRE_VA_WIN_SIZE, iq_info->final_setting.ipp.pre_va_win_size);
	...
	iq_info->final_setting.ipp.pre_fpn_info = &(iq_info->final_ipp.pre_fpn_info);
	isp_api_set_iq_param(isp_id, ISP_IQ_ITEM_PRE_FPN_INFO, iq_info->final_setting.ipp.pre_fpn_info);

	return E_OK;
}

static ER iq_flow_set_pre_param(IQALG_INFO *iq_info)
{
	UINT32 isp_id = (iq_info->ipp_trig_obj.ipp_info << 16) + iq_info->id;
	static BOOL reset_lv0 = FALSE;

	if (iq_info->final_ipp.ready_flag != TRUE) {
		PRINT_IQ_WRN(iq_dbg_get_dbg_mode(iq_info->id) & IQ_DBG_WRN_MSG, "PRE param (%d) not ready!! \r\n", iq_info->id);
		return E_PAR;
	}

	iq_info->final_setting.pre.p_nrs0 = &(iq_info->final_ipp.pre_nrs_0);
	iq_info->final_setting.pre.p_fcurve = &(iq_info->final_ipp.pre_fcurve);
	iq_info->final_setting.pre.p_fusion = &(iq_info->final_ipp.pre_fusion);
	iq_info->final_setting.pre.p_outl = &(iq_info->final_ipp.pre_outl);
	iq_info->final_setting.pre.p_cgain = &(iq_info->final_ipp.pre_cgain);
	iq_info->final_setting.pre.p_bnr = &(iq_info->final_ipp.pre_bnr);
	iq_info->final_setting.pre.p_va = &(iq_info->final_ipp.pre_va);
	iq_info->final_setting.pre.p_fpn = &(iq_info->final_ipp.pre_fpn);
	...
	if (iq_info->low_power_lv || reset_lv0) {
		...
		final_lv = (iq_info->low_power_param.va) ? iq_info->low_power_param.va : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.pre.p_va->enable = ctl_pre_va_init.enable;
			iq_info->final_setting.pre.p_va->indep_va_enable = ctl_pre_va_init.indep_va_enable;
			iq_info->final_setting.pre.p_va->va_out_grp1_2 = ctl_pre_va_init.va_out_grp1_2;
			iq_info->final_setting.pre.p_va->indep_win[0].enable = ctl_pre_va_init.indep_win[0].enable;
			iq_info->final_setting.pre.p_va->indep_win[1].enable = ctl_pre_va_init.indep_win[1].enable;
			iq_info->final_setting.pre.p_va->indep_win[2].enable = ctl_pre_va_init.indep_win[2].enable;
			iq_info->final_setting.pre.p_va->indep_win[3].enable = ctl_pre_va_init.indep_win[3].enable;
			iq_info->final_setting.pre.p_va->indep_win[4].enable = ctl_pre_va_init.indep_win[4].enable;
			// NOTE: vdetgh1_filter_sel = vdetgh1_filter_sel = 1, then VA_VERTICAL_FIR_EN = 0.
			break;
		case 1:
			iq_info->final_setting.pre.p_va->va_out_grp1_2 = FALSE;
			break;
		case 2:
			iq_info->final_setting.pre.p_va->va_out_grp1_2 = FALSE;
			break;
		...
		case 4:
		case 5:
			iq_info->final_setting.pre.p_va->enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_va_enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_win[0].enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_win[1].enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_win[2].enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_win[3].enable = FALSE;
			iq_info->final_setting.pre.p_va->indep_win[4].enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.outlier) ? iq_info->low_power_param.outlier : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		default:
			iq_info->final_setting.pre.p_outl->enable = iq_param[iq_info->id]->nr->outl_enable;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.pre.p_outl->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.bnr) ? iq_info->low_power_param.bnr : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		default:
			iq_info->final_setting.pre.p_bnr->bnr_enable = iq_param[iq_info->id]->bnr->enable;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.pre.p_bnr->bnr_enable = FALSE;
			break;
		};
	}

	isp_api_set_iq_param(isp_id, ISP_IQ_ITEM_PRE_PARAM, &iq_info->final_setting.pre);
	return E_OK;
}

static ER iq_flow_set_aiisp_param(IQALG_INFO *iq_info)
{
	UINT32 isp_id = (iq_info->ipp_trig_obj.ipp_info << 16) + iq_info->id;
	...
	iq_info->final_setting.aiisp = &(iq_info->final_ipp.aiisp);

	isp_api_set_iq_param(isp_id, ISP_IQ_ITEM_AIISP_PARAM, iq_info->final_setting.aiisp);
	return E_OK;
}
...
static ER iq_flow_set_ife_param(IQALG_INFO *iq_info)
{
	...
	if (iq_info->low_power_lv || reset_lv0) {
		...
		final_lv = (iq_info->low_power_param.gbal) ? iq_info->low_power_param.gbal : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		default:
			iq_info->final_setting.ife.p_gbal->enable = iq_param[iq_info->id]->nr->gbal_enable;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ife.p_gbal->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.nr) ? iq_info->low_power_param.nr : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ife.p_filt->enable = iq_param[iq_info->id]->nr->filter_enable;
			iq_info->final_setting.ife.p_filt->range_sel = ctl_ife_filter_init.range_sel;
			iq_info->final_setting.ife.p_filt->motion_set.motion_en = ctl_ife_filter_init.motion_set.motion_en;
			break;
		case 1:
		case 2:
			iq_info->final_setting.ife.p_filt->range_sel = 1;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ife.p_filt->enable = FALSE;
			iq_info->final_setting.ife.p_filt->motion_set.motion_en = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.wdr) ? iq_info->low_power_param.wdr : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ife.p_wdr->wdr_enable = iq_param[iq_info->id]->wdr->enable;
			iq_info->final_setting.ife.p_wdr->tonecurve_enable = ctl_ife_wdr_init.tonecurve_enable;
			iq_info->final_setting.ife.p_wdr->wdr_str.wdr_mode = ctl_ife_wdr_init.wdr_str.wdr_mode;
			iq_info->final_setting.ife.p_wdr->gainctrl.gainctrl_en = ctl_ife_wdr_init.gainctrl.gainctrl_en;
			iq_info->final_setting.ife.p_wdr->outbld.outbld_en = ctl_ife_wdr_init.outbld.outbld_en;
			iq_info->final_setting.ife.p_wdr->dither.wdr_dithering_en = ctl_ife_wdr_init.dither.wdr_dithering_en;
			iq_info->final_setting.ife.p_wdr->fbc.fbc_en = ctl_ife_wdr_init.fbc.fbc_en;
			iq_info->final_setting.ife.p_hist->hist_enable = ctl_ife_hist_init.hist_enable;
			// NOTE: IFE_WDR_SUBIMG_OUT_EN, IFE_WDR_BYPASS_MODE control by kflow ipp
			break;
		case 1:
		case 2:
			iq_info->final_setting.ife.p_wdr->wdr_str.wdr_mode = CTL_IFE_ISP_WDR_NORMAL_MODE;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ife.p_wdr->wdr_enable = FALSE;
			iq_info->final_setting.ife.p_wdr->tonecurve_enable = FALSE;
			iq_info->final_setting.ife.p_wdr->gainctrl.gainctrl_en = FALSE;
			iq_info->final_setting.ife.p_wdr->outbld.outbld_en = FALSE;
			iq_info->final_setting.ife.p_wdr->dither.wdr_dithering_en = FALSE;
			iq_info->final_setting.ife.p_wdr->fbc.fbc_en = FALSE;
			iq_info->final_setting.ife.p_hist->hist_enable = FALSE;
			break;
		};
	}
...
static ER iq_flow_set_ipe_param(IQALG_INFO *iq_info)
{
	UINT32 isp_id = (iq_info->ipp_trig_obj.ipp_info << 16) + iq_info->id;
	static BOOL reset_lv0 = FALSE;
	static BOOL _3dcc_enable_lv0 = FALSE, is_save_3dcc_enable_lv0 = FALSE;
	...
	if (iq_info->low_power_lv || reset_lv0) {
		...
		final_lv = (iq_info->low_power_param.rgblpf) ? iq_info->low_power_param.rgblpf : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_rgb_lpf->enable = ctl_ipe_rgblpf_init.enable;
			break;
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_rgb_lpf->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.pfr) ? iq_info->low_power_param.pfr : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_pfr->enable = iq_param[iq_info->id]->pfr->enable;
			iq_info->final_setting.ipe.p_pfr->color_wet_set[1].enable = iq_param[iq_info->id]->pfr->set1_en;
			iq_info->final_setting.ipe.p_pfr->color_wet_set[2].enable = iq_param[iq_info->id]->pfr->set2_en;
			iq_info->final_setting.ipe.p_pfr->color_wet_set[3].enable = iq_param[iq_info->id]->pfr->set3_en;
			break;
		case 1:
			iq_info->final_setting.ipe.p_pfr->color_wet_set[1].enable = FALSE;
			iq_info->final_setting.ipe.p_pfr->color_wet_set[2].enable = FALSE;
			iq_info->final_setting.ipe.p_pfr->color_wet_set[3].enable = FALSE;
			break;
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_pfr->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.edge) ? iq_info->low_power_param.edge : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_eext->motion_enable = ctl_ipe_eext_init.motion_enable;
			iq_info->final_setting.ipe.p_eext->edge_ker_mode = ctl_ipe_eext_init.edge_ker_mode;
			iq_info->final_setting.ipe.p_eext->edge_mode = ctl_ipe_eext_init.edge_mode;
			break;
		case 1:
			iq_info->final_setting.ipe.p_eext->edge_mode = 1;
			break;
		case 2:
			iq_info->final_setting.ipe.p_eext->motion_enable = FALSE;
			iq_info->final_setting.ipe.p_eext->edge_ker_mode = 1;
			iq_info->final_setting.ipe.p_eext->edge_mode = 1;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_eext->motion_enable = FALSE;
			iq_info->final_setting.ipe.p_eext->edge_ker_mode = 3;
			iq_info->final_setting.ipe.p_eext->edge_mode = 1;
			break;
		};
		
		final_lv = (iq_info->low_power_param.cfa) ? iq_info->low_power_param.cfa : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		default:
			iq_info->final_setting.ipe.p_cfa->cfa_enable = ctl_ipe_cfa_init.cfa_enable;
			iq_info->final_setting.ipe.p_cfa->cfa_fcs.fcs_enable = ctl_ipe_cfa_init.cfa_fcs.fcs_enable;
			// NOTE: CFA_SUBIMG_OUT_EN control by kflow ipp
			break;
		case 2:
		case 3:
		case 4:
			iq_info->final_setting.ipe.p_cfa->cfa_fcs.fcs_enable = FALSE;
			break;
		case 5:
			iq_info->final_setting.ipe.p_cfa->cfa_enable = FALSE;
			break;
		};

		iq_info->final_setting.ipe.p_3dcc = iq_info->final_ipp.ipe_3dcc;

		final_lv = (iq_info->low_power_param.cc) ? iq_info->low_power_param.cc : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_cc->enable = iq_param[iq_info->id]->ccm->enable;
			if (iq_info->_3dcc_en && is_save_3dcc_enable_lv0) {
				iq_info->final_setting.ipe.p_3dcc->enable = _3dcc_enable_lv0;
				is_save_3dcc_enable_lv0 = FALSE;
			}
			break;
		case 1:
		case 2:
		case 3:
			if (iq_info->_3dcc_en) {
				if (!is_save_3dcc_enable_lv0) {
					_3dcc_enable_lv0 = iq_info->final_setting.ipe.p_3dcc->enable;
					is_save_3dcc_enable_lv0 = TRUE;
				}
				iq_info->final_setting.ipe.p_3dcc->enable = FALSE;
			}
			break;
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_cc->enable = FALSE;
			if (iq_info->_3dcc_en) {
				if (!is_save_3dcc_enable_lv0) {
					_3dcc_enable_lv0 = iq_info->final_setting.ipe.p_3dcc->enable;
					is_save_3dcc_enable_lv0 = TRUE;
		...
		final_lv = (iq_info->low_power_param.y_curve) ? iq_info->low_power_param.y_curve : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_y_curve->enable = iq_param[iq_info->id]->ycurve->enable;
			break;
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_y_curve->enable = FALSE;
			break;
		};

		iq_info->final_setting.ipe.p_gamma = &(iq_info->final_ipp.ipe_gamma);

		final_lv = (iq_info->low_power_param.gamma) ? iq_info->low_power_param.gamma : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		...
		case 4:
		default:
			iq_info->final_setting.ipe.p_gamma->enable = iq_param[iq_info->id]->gamma->enable;
			break;
		case 5:
			iq_info->final_setting.ipe.p_gamma->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.defog_lce) ? iq_info->low_power_param.defog_lce : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		default:
			iq_info->final_setting.ipe.p_defog->enable = iq_param[iq_info->id]->defog->enable;
			iq_info->final_setting.ipe.p_defog->air_enable = ctl_ipe_defog_init.air_enable;
			iq_info->final_setting.ipe.p_lce->enable = iq_param[iq_info->id]->contrast->lce_enable;
			// NOTE: DEFOG_SUBIMG_OUT_EN control by kflow ipp
			break;
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_defog->enable = FALSE;
			iq_info->final_setting.ipe.p_defog->air_enable = FALSE;
			iq_info->final_setting.ipe.p_lce->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.cstp) ? iq_info->low_power_param.cstp : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ipe.p_cstp->enable = ctl_ipe_cstp_init.enable;
			break;
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_cstp->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.cctrl) ? iq_info->low_power_param.cctrl : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		default:
			iq_info->final_setting.ipe.p_cctrl->enable = ctl_ipe_cctrl_init.enable;
			break;
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_cctrl->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.cadjust) ? iq_info->low_power_param.cadjust : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		case 2:
		case 3:
		default:
			iq_info->final_setting.ipe.p_cadj_ee->enable = iq_param[iq_info->id]->edge->enable;
			iq_info->final_setting.ipe.p_cadj_yccon->enable = ctl_ipe_cadj_yccon_init.enable;
			iq_info->final_setting.ipe.p_cadj_cofs->enable = ctl_ipe_cadj_cofs_init.enable;
			iq_info->final_setting.ipe.p_cadj_rand->enable = ctl_ipe_cadj_rand_init.enable;
			iq_info->final_setting.ipe.p_cadj_hue->enable = ctl_ipe_cadj_hue_init.enable;
			iq_info->final_setting.ipe.p_cadj_fixth->enable = ctl_ipe_cadj_fixth_init.enable;
			iq_info->final_setting.ipe.p_cadj_mask->enable = ctl_ipe_cadj_mask_init.enable;
			break;
		case 4:
		case 5:
			iq_info->final_setting.ipe.p_cadj_ee->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_yccon->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_cofs->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_rand->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_hue->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_fixth->enable = FALSE;
			iq_info->final_setting.ipe.p_cadj_mask->enable = FALSE;
			break;
		};
	}
...
static ER iq_flow_set_ime_param(IQALG_INFO *iq_info)
{
	...
	if (iq_info->low_power_lv || reset_lv0) {
		...
		final_lv = (iq_info->low_power_param.lca) ? iq_info->low_power_param.lca : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ime.p_lca->rf.sr_ker_size_sel = ctl_ime_lca_init.rf.sr_ker_size_sel;
			iq_info->final_setting.ime.p_lca->rf.mr_ker_size_sel = ctl_ime_lca_init.rf.mr_ker_size_sel;
			iq_info->final_setting.ime.p_lca->edge_detect_en = ctl_ime_lca_init.edge_detect_en;
			iq_info->final_setting.ime.p_lca->ref_center_en = ctl_ime_lca_init.ref_center_en;
			iq_info->final_setting.ime.p_lca->motion_status_en = ctl_ime_lca_init.motion_status_en;
			// NOTE: LCA_EN control by kflow ipp
			break;
		case 1:
		case 2:
		case 3:
			iq_info->final_setting.ime.p_lca->rf.sr_ker_size_sel = (ctl_ime_lca_init.rf.sr_ker_size_sel < CTL_IME_ISP_KER_SIZE_11X9) ? ctl_ime_lca_init.rf.sr_ker_size_sel : CTL_IME_ISP_KER_SIZE_11X9;
			iq_info->final_setting.ime.p_lca->rf.mr_ker_size_sel = (ctl_ime_lca_init.rf.mr_ker_size_sel < CTL_IME_ISP_KER_SIZE_11X9) ? ctl_ime_lca_init.rf.mr_ker_size_sel : CTL_IME_ISP_KER_SIZE_11X9;
			iq_info->final_setting.ime.p_lca->edge_detect_en = FALSE;
			break;
		case 4:
		case 5:
			iq_info->final_setting.ime.p_lca->rf.sr_ker_size_sel = CTL_IME_ISP_KER_SIZE_3X3;
			iq_info->final_setting.ime.p_lca->rf.mr_ker_size_sel = CTL_IME_ISP_KER_SIZE_3X3;
			iq_info->final_setting.ime.p_lca->edge_detect_en = FALSE;
			iq_info->final_setting.ime.p_lca->ref_center_en = FALSE;
			iq_info->final_setting.ime.p_lca->motion_status_en = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param._3dnr) ? iq_info->low_power_param._3dnr : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		default:
			iq_info->final_setting.ime.p_tmnr->enable = iq_param[iq_info->id]->_3dnr->enable;
			iq_info->final_setting.ime.p_tmnr->me_param.motion_en = ctl_ime_tmnr_init.me_param.motion_en;
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_y_set_en = ctl_ime_tmnr_init.nr_param.tf_y_set_en;
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_c_set_en = ctl_ime_tmnr_init.nr_param.tf_c_set_en;
			break;
		case 1:
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_y_set_en = FALSE;
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_c_set_en = FALSE;
			break;
		case 2:
		case 3:
			iq_info->final_setting.ime.p_tmnr->me_param.motion_en = FALSE;
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_y_set_en = FALSE;
			iq_info->final_setting.ime.p_tmnr->nr_param.tf_c_set_en = FALSE;
			break;
		case 4:
		case 5:
			iq_info->final_setting.ime.p_tmnr->enable = FALSE;
			break;
		};

		final_lv = (iq_info->low_power_param.post_sharpen) ? iq_info->low_power_param.post_sharpen : iq_info->low_power_lv;
		switch (final_lv) {
		case 0:
		case 1:
		default:
			iq_info->final_setting.ime.p_sharpen->shp_en = iq_param[iq_info->id]->post_sharpen_1->enable;
			break;
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.ime.p_sharpen->shp_en = FALSE;
			break;
		};
	}
...
static ER iq_flow_set_enc_param(IQALG_INFO *iq_info)
{
	...
	if (iq_info->low_power_lv || reset_lv0) {
		...
		case 1:
		default:
			iq_info->final_setting.enc_sharpen->bEnable = iq_param[iq_info->id]->post_sharpen_2->enable;
			break;
		case 2:
		case 3:
		case 4:
		case 5:
			iq_info->final_setting.enc_sharpen->bEnable = FALSE;
			break;
		};
	}
...
INT32 iq_flow_init(UINT32 id)
{
	...
	iq_info->ai_aided_en = FALSE;
	iq_info->low_power_lv = 0;
	...
	if (iq_info->dpc_en == TRUE) {
		...
		iq_info->final_sie.sie_dpc->def_same_ch_only_en = FALSE;
	} else {
	...
	iq_info->final_ipp.pre_nrs_0 = ctl_pre_nrs_0_init;
	iq_info->final_ipp.pre_fcurve = ctl_pre_fcurve_init;
	iq_info->final_ipp.pre_fusion = ctl_pre_fusion_init;
	iq_info->final_ipp.pre_outl = ctl_pre_outl_init;
	iq_info->final_ipp.pre_va = ctl_pre_va_init;
	iq_info->final_ipp.pre_fpn = ctl_pre_fpn_init;
	iq_info->final_ipp.pre_bnr = ctl_pre_bnr_init;
	iq_info->final_ipp.pre_cgain = ctl_pre_cgain_init;

	iq_info->final_ipp.aiisp = ctl_aiisp_init;
	...
	if (iq_info->_3dcc_en == TRUE) {
		...
		iq_info->final_ipp.ipe_3dcc->mode = ctl_ipe_3dcc_init.mode;
	...
	iq_info->final_ipp.pre_va_win_size = ctrl_ipp_pre_va_win_size_init;
	...
	if (iq_sync_rt == E_OK) {
		...
		if (iq_init_info.param_addr != 0) {
			...
			// TODO: 539A IQ, implement BNR/AIISP here
			...
			if (iq_info->final_sie.sie_dpc != NULL) {
				...
				// TODO: 539A IQ, implement DPC here
			}
			...
			if (iq_info->final_ipp.ipe_3dcc != NULL) {
				...
				// TODO: 539A IQ, implement 3DCC here
			}
...
static void iq_flow_set_update(ISP_TRIG_MSG msg, IQALG_INFO *iq_info, IQ_PARAM_PTR *iq_param)
{
	...
	case ISP_TRIG_IQ_IPP:
		...
		if (update_all || (iqt_get_param_update(iq_info->id, IQT_ITEM_NR_PARAM) == TRUE)) {
			iq_info->final_ipp.pre_outl.enable = iq_param->nr->outl_enable;
			...
			iq_info->final_ipp.ime_dbcs.proc_location = iq_param->nr->dbcs_proc_location;
		...
		if (update_all || (iqt_get_param_update(iq_info->id, IQT_ITEM_RAW_VA_PARAM) == TRUE)) {
			iq_info->final_ipp.pre_va.fltr_sel.vdetgh1_filter_sel = iq_param->raw_va->g1_h_filter_sel;
			if (iq_param->raw_va->g1_h_filter_sel == IQ_VA_H_FILTER_FIR) {
				iq_info->final_ipp.pre_va.group_1.h_filt.symmetry = iq_param->raw_va->g1_fir_sym_sel;
				iq_info->final_ipp.pre_va.group_1.h_filt.tap_a = iq_param->raw_va->g1_fir_tap_a;
				iq_info->final_ipp.pre_va.group_1.h_filt.tap_b = iq_param->raw_va->g1_fir_tap_b;
				iq_info->final_ipp.pre_va.group_1.h_filt.tap_c = iq_param->raw_va->g1_fir_tap_c;
				iq_info->final_ipp.pre_va.group_1.h_filt.tap_d = iq_param->raw_va->g1_fir_tap_d;
				iq_info->final_ipp.pre_va.group_1.h_filt.div = iq_param->raw_va->g1_fir_div;
			} else {
				iq_info->final_ipp.pre_va.group_1.h_filt.symmetry = CTL_IFE_ISP_VA_FILTER_SYM_INVERSE;
				// @ iq_operation_iso
			}
			iq_info->final_ipp.pre_va.group_1.v_filt.symmetry = iq_param->raw_va->g1_fir_sym_sel;
			iq_info->final_ipp.pre_va.group_1.v_filt.tap_a = iq_param->raw_va->g1_fir_tap_a;
			iq_info->final_ipp.pre_va.group_1.v_filt.tap_b = iq_param->raw_va->g1_fir_tap_b;
			iq_info->final_ipp.pre_va.group_1.v_filt.tap_c = iq_param->raw_va->g1_fir_tap_c;
			iq_info->final_ipp.pre_va.group_1.v_filt.tap_d = iq_param->raw_va->g1_fir_tap_d;
			iq_info->final_ipp.pre_va.group_1.v_filt.div = iq_param->raw_va->g1_fir_div;
			iq_info->final_ipp.pre_va.fltr_sel.vdetgh2_filter_sel = iq_param->raw_va->g2_h_filter_sel;
			if (iq_param->raw_va->g2_h_filter_sel == IQ_VA_H_FILTER_FIR) {
				iq_info->final_ipp.pre_va.group_2.h_filt.symmetry = iq_param->raw_va->g2_fir_sym_sel;
				iq_info->final_ipp.pre_va.group_2.h_filt.tap_a = iq_param->raw_va->g2_fir_tap_a;
				iq_info->final_ipp.pre_va.group_2.h_filt.tap_b = iq_param->raw_va->g2_fir_tap_b;
				iq_info->final_ipp.pre_va.group_2.h_filt.tap_c = iq_param->raw_va->g2_fir_tap_c;
				iq_info->final_ipp.pre_va.group_2.h_filt.tap_d = iq_param->raw_va->g2_fir_tap_d;
				iq_info->final_ipp.pre_va.group_2.h_filt.div = iq_param->raw_va->g2_fir_div;
			} else {
				iq_info->final_ipp.pre_va.group_2.h_filt.symmetry = CTL_IFE_ISP_VA_FILTER_SYM_INVERSE;
				// @ iq_operation_iso
			}
			iq_info->final_ipp.pre_va.group_2.v_filt.symmetry = iq_param->raw_va->g2_fir_sym_sel;
			iq_info->final_ipp.pre_va.group_2.v_filt.tap_a = iq_param->raw_va->g2_fir_tap_a;
			iq_info->final_ipp.pre_va.group_2.v_filt.tap_b = iq_param->raw_va->g2_fir_tap_b;
			iq_info->final_ipp.pre_va.group_2.v_filt.tap_c = iq_param->raw_va->g2_fir_tap_c;
			iq_info->final_ipp.pre_va.group_2.v_filt.tap_d = iq_param->raw_va->g2_fir_tap_d;
			iq_info->final_ipp.pre_va.group_2.v_filt.div = iq_param->raw_va->g2_fir_div;
			iq_info->final_ipp.pre_va.ldg_para.ldg_low_th = iq_param->raw_va->ldg_low_th;
			iq_info->final_ipp.pre_va.ldg_para.ldg_high_th = iq_param->raw_va->ldg_high_th;
			iq_info->final_ipp.pre_va.ldg_para.ldg_low_gain = iq_param->raw_va->ldg_low_gain;
			iq_info->final_ipp.pre_va.ldg_para.ldg_high_gain = iq_param->raw_va->ldg_high_gain;
			iq_info->final_ipp.pre_va.ldg_para.ldg_low_slope = iq_param->raw_va->ldg_low_slope;
			iq_info->final_ipp.pre_va.ldg_para.ldg_high_slope = iq_param->raw_va->ldg_high_slope;
			iq_info->final_ipp.pre_va.energy_w = iq_param->raw_va->energy_w;
			iq_info->final_ipp.pre_va.win_cnt_out_sel = iq_param->raw_va->win_cnt_out_sel;
			iq_info->final_ipp.pre_va.high_luma_th = iq_param->raw_va->high_luma_th;
		...
		if (update_all || (iqt_get_param_update(iq_info->id, IQT_ITEM_SHDR_PARAM) == TRUE)) {
			iq_info->final_ipp.pre_nrs_0.enable = iq_param->shdr->nrs_enable;
			iq_info->final_ipp.pre_fusion.bld_cur.nor_sel = iq_param->shdr->fusion_nor_sel;
			iq_info->final_ipp.pre_fusion.bld_cur.l_nor_knee[0] = iq_param->shdr->fusion_l_nor_knee;
			iq_info->final_ipp.pre_fusion.bld_cur.l_nor_range = iq_param->shdr->fusion_l_nor_range;
			iq_info->final_ipp.pre_fusion.bld_cur.l_nor_slope = IQ_CLAMP(256 * 4096 / iq_param->shdr->fusion_l_nor_range, 0, 65535);
			// blend curve settings for S is calculated at iq_operation_shdr_fusion()
			iq_info->final_ipp.pre_fusion.bld_cur.dif_sel = iq_param->shdr->fusion_dif_sel;
			iq_info->final_ipp.pre_fusion.bld_cur.l_dif_knee[0] = iq_param->shdr->fusion_l_dif_knee;
			iq_info->final_ipp.pre_fusion.bld_cur.l_dif_range = iq_param->shdr->fusion_l_dif_range;
			iq_info->final_ipp.pre_fusion.bld_cur.l_dif_slope = IQ_CLAMP(256 * 4096 / iq_param->shdr->fusion_l_dif_range, 0, 65535);
			// blend curve settings for S is calculated at iq_operation_shdr_fusion()
			iq_info->final_ipp.pre_fusion.mc_para.lum_th = iq_param->shdr->fusion_lum_th;
			memcpy(iq_info->final_ipp.pre_fusion.mc_para.diff_w, iq_param->shdr->fusion_diff_w, sizeof(UINT8) * IQ_SHDR_FUSION_DIFF_W_NUM);
			iq_info->final_ipp.pre_fcurve.fcur_ctrl.y_mean_sel = iq_param->shdr->fcurve_y_mean_sel;
			iq_info->final_ipp.pre_fcurve.fcur_ctrl.yv_w = iq_param->shdr->fcurve_yv_w;
			memcpy(iq_info->final_ipp.pre_fcurve.y_weight.y_w_lut, iq_param->shdr->fcurve_y_w_lut, sizeof(UINT8) * IQ_SHDR_FCURVE_Y_W_NUM);
...
static void iq_flow_set_sync_param(ISP_TRIG_MSG msg, IQALG_INFO *iq_info, IQ_PARAM_PTR *iq_param)
{
	...
	URECT la_roi = {0};
	URECT pre_va_indep_roi[ISP_INDEP_VA_WIN_NUM] = {0};
	...
	case ISP_TRIG_IQ_IPP:
		if (iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_AF) {
			va_sync_rt = isp_dev_get_sync_item(iq_info->id, ISP_SYNC_SEL_CURRENT, ISP_SYNC_IFE_VA_INDEP_ROI, &pre_va_indep_roi[0]);
			if ((va_sync_rt == E_OK) && (pre_va_indep_roi[0].w > 0) && (pre_va_indep_roi[0].h > 0) && (pre_va_indep_roi[1].w > 0) && (pre_va_indep_roi[1].h > 0)
				&& (pre_va_indep_roi[2].w > 0) && (pre_va_indep_roi[2].h > 0) && (pre_va_indep_roi[3].w > 0) && (pre_va_indep_roi[3].h > 0) && (pre_va_indep_roi[4].w > 0) && (pre_va_indep_roi[4].h > 0)
				&& ((pre_va_indep_roi[0].x + pre_va_indep_roi[0].w) <= iq_info->final_ipp.pre_va_win_size.ratio_base) && ((pre_va_indep_roi[0].y + pre_va_indep_roi[0].h) <= iq_info->final_ipp.pre_va_win_size.ratio_base)
				&& ((pre_va_indep_roi[1].x + pre_va_indep_roi[1].w) <= iq_info->final_ipp.pre_va_win_size.ratio_base) && ((pre_va_indep_roi[1].y + pre_va_indep_roi[1].h) <= iq_info->final_ipp.pre_va_win_size.ratio_base)
				&& ((pre_va_indep_roi[2].x + pre_va_indep_roi[2].w) <= iq_info->final_ipp.pre_va_win_size.ratio_base) && ((pre_va_indep_roi[2].y + pre_va_indep_roi[2].h) <= iq_info->final_ipp.pre_va_win_size.ratio_base)
				&& ((pre_va_indep_roi[3].x + pre_va_indep_roi[3].w) <= iq_info->final_ipp.pre_va_win_size.ratio_base) && ((pre_va_indep_roi[3].y + pre_va_indep_roi[3].h) <= iq_info->final_ipp.pre_va_win_size.ratio_base)
				&& ((pre_va_indep_roi[4].x + pre_va_indep_roi[4].w) <= iq_info->final_ipp.pre_va_win_size.ratio_base) && ((pre_va_indep_roi[4].y + pre_va_indep_roi[4].h) <= iq_info->final_ipp.pre_va_win_size.ratio_base)) {
				iq_info->final_ipp.pre_va_win_size.indep_roi_ratio[0] = pre_va_indep_roi[0];
				iq_info->final_ipp.pre_va_win_size.indep_roi_ratio[1] = pre_va_indep_roi[1];
				iq_info->final_ipp.pre_va_win_size.indep_roi_ratio[2] = pre_va_indep_roi[2];
				iq_info->final_ipp.pre_va_win_size.indep_roi_ratio[3] = pre_va_indep_roi[3];
				iq_info->final_ipp.pre_va_win_size.indep_roi_ratio[4] = pre_va_indep_roi[4];
			} else {
				if (va_sync_rt != E_OK) {
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "PRE VA ROI not update!! \r\n");
				} else {
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "IQ(%d) sync pre_va_indep_roi is out of range!! \r\n", iq_info->id);
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "pre_va_indep_roi(0) (%d, %d, %d, %d)!! \r\n", pre_va_indep_roi[0].x, pre_va_indep_roi[0].y, pre_va_indep_roi[0].w, pre_va_indep_roi[0].h);
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "pre_va_indep_roi(1) (%d, %d, %d, %d)!! \r\n", pre_va_indep_roi[1].x, pre_va_indep_roi[1].y, pre_va_indep_roi[1].w, pre_va_indep_roi[1].h);
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "pre_va_indep_roi(2) (%d, %d, %d, %d)!! \r\n", pre_va_indep_roi[2].x, pre_va_indep_roi[2].y, pre_va_indep_roi[2].w, pre_va_indep_roi[2].h);
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "pre_va_indep_roi(3) (%d, %d, %d, %d)!! \r\n", pre_va_indep_roi[3].x, pre_va_indep_roi[3].y, pre_va_indep_roi[3].w, pre_va_indep_roi[3].h);
					PRINT_IQ(dbg_mode & IQ_DBG_SYNC, "pre_va_indep_roi(4) (%d, %d, %d, %d)!! \r\n", pre_va_indep_roi[4].x, pre_va_indep_roi[4].y, pre_va_indep_roi[4].w, pre_va_indep_roi[4].h);
				}
			}
		}
		break;
...
INT32 iq_flow_process(UINT32 id, ISP_TRIG_MSG msg, void *arg)
{
	...
	case ISP_TRIG_IQ_SIE:
		...
		// NOTE: Low power level setting
		iq_info->low_power_lv = iq_info->sie_trig_obj.low_power_lv;
		...
		if ((iq_info->sie_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_HDR) || (iq_info->sie_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_DCG_HDR) || (iq_info->sie_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_DCG_SHDR)) {
			...
			// SHDR
			iq_info->ob_mode = IQ_OB_PRE_F;
			iq_info->dg_mode = IQ_DG_SIE;
			iq_info->cg_mode = IQ_CG_PRE_F;
			iq_info->final_sie.sie_companding.enable = companding_en[iq_info->id];
		...
		if (iq_info->cg_mode_manual.manual_enable) {
			iq_info->cg_mode = iq_info->cg_mode_manual.manual_mode;
		}
	...
	case ISP_TRIG_IQ_IPP:
		...
		iq_info->ai_aided_en = (iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_AI_AIDED);

		// NOTE: Low power level setting
		iq_info->low_power_lv = iq_info->ipp_trig_obj.low_power_lv;
			...
			if (!is_update_shdr_param[id]) {
				if (((iq_info->ipp_flow_mode == IQ_FLOW_SHDR) && (iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_SHDR)) && (iqt_get_param_update(id, IQT_ITEM_SHDR_PARAM) == FALSE)) {
					...
					#endif
					{
						...
						memcpy(iq_param[iq_info->id]->shdr->fcurve_left_lut, ctl_pre_fcurve_init.fcurve_l.fcur_l_lut, sizeof(UINT32) * IQ_SHDR_FCURVE_LEFT_NUM);
						memcpy(iq_param[iq_info->id]->shdr->fcurve_right_lut, ctl_pre_fcurve_init.fcurve_r.fcur_r_lut, sizeof(UINT32) * IQ_SHDR_FCURVE_RIGHT_NUM);
						memcpy(iq_param[iq_info->id]->shdr->fcurve_end_lut, ctl_pre_fcurve_init.fcurve_end.fcur_end_lut, sizeof(UINT32) * IQ_SHDR_FCURVE_END_NUM);
					}
				}
			}

			iq_info->final_ipp.ife_hist.hist_enable = ctl_ife_hist_init.hist_enable;
			iq_info->final_ipp.pre_va.enable = ctl_pre_va_init.enable;
			iq_info->final_ipp.pre_va.indep_va_enable = ctl_pre_va_init.indep_va_enable;
		}
		...
		if ((iq_info->ipp_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_HDR) || (iq_info->ipp_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_DCG_HDR) || (iq_info->ipp_trig_obj.mode_type == ISP_SEN_MODE_BUILTIN_DCG_SHDR)) {
			...
			iq_info->dg_mode = IQ_DG_SIE;
			iq_info->cg_mode = IQ_CG_SIE;
			iq_info->final_ipp.pre_nrs_0.enable = FALSE;
			iq_info->final_ipp.pre_fcurve.enable = fcurve_en[iq_info->id];
			...
			iq_info->final_ipp.pre_nrs_0.enable = iq_param[iq_info->id]->shdr->nrs_enable;
			if ((iq_info->final_ipp.pre_fusion.fu_ctrl.mode == CTL_PRE_ISP_MODE_FUSION) && !iq_info->final_ipp.pre_fusion.dbg.enable) {
				iq_info->final_ipp.pre_fcurve.enable = TRUE;
			} else {
				iq_info->final_ipp.pre_fcurve.enable = fcurve_en[iq_info->id];
			}
		...
		// NOTE: Set VA enable
		if (iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_AF) {
			iq_info->final_ipp.pre_va.enable = TRUE;
			iq_info->final_ipp.pre_va.indep_va_enable = TRUE;
		} else {
			iq_info->final_ipp.pre_va.enable = FALSE;
			iq_info->final_ipp.pre_va.indep_va_enable = FALSE;
		}
		if (iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_WDR) {
			iq_info->final_ipp.ife_hist.hist_enable = TRUE;
		} else {
		...
		if (aiisp_aided_test_en && (iq_info->id == aiisp_aided_test_clone_id)) {
			...
			if (!(iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_BNR) || (iq_info->ipp_trig_obj.stripe_num != 1) || !(iq_info_id0->final_ipp.pre_bnr.bnr_enable) || !(iq_info->final_ipp.ife_filter.enable)) {
				iq_info->final_ipp.ife_filter.motion_set.motion_en = FALSE;
			} else {
				iq_info->final_ipp.ife_filter.motion_set.motion_en = TRUE;
			}
			if (!(iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_BNR) || (iq_info->ipp_trig_obj.stripe_num != 1) || !(iq_info_id0->final_ipp.pre_bnr.bnr_enable) || !(iq_info->final_ipp.ipe_cadj_ee.enable)) {
				iq_info->final_ipp.ipe_eext.motion_enable = FALSE;
			} else {
				iq_info->final_ipp.ipe_eext.motion_enable = TRUE;
			}
		} else {
			if (!(iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_BNR) || (iq_info->ipp_trig_obj.stripe_num != 1) || !(iq_info->final_ipp.pre_bnr.bnr_enable) || !(iq_info->final_ipp.ife_filter.enable)) {
				iq_info->final_ipp.ife_filter.motion_set.motion_en = FALSE;
			} else {
				iq_info->final_ipp.ife_filter.motion_set.motion_en = TRUE;
			}
			if (!(iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_BNR) || (iq_info->ipp_trig_obj.stripe_num != 1) || !(iq_info->final_ipp.pre_bnr.bnr_enable) || !(iq_info->final_ipp.ipe_cadj_ee.enable)) {
				iq_info->final_ipp.ipe_eext.motion_enable = FALSE;
			} else {
				iq_info->final_ipp.ipe_eext.motion_enable = TRUE;
			}
