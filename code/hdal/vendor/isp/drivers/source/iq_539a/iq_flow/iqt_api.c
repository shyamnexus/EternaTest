...
static void iqt_api_get_3dcc_param(ULONG addr)
{
	...
	if (iq_info->final_ipp.ipe_3dcc != NULL) {
		...
		data->_3dcc.mode = iq_info->final_ipp.ipe_3dcc->mode;
	}
}
...
static void iqt_api_get_dpc_param(ULONG addr)
{
	...
	if (iq_info->final_sie.sie_dpc != NULL) {
		...
		data->dpc.def_same_ch_only_en = iq_info->final_sie.sie_dpc->def_same_ch_only_en;
	}
}
...
static void iqt_api_get_shdr_mode(ULONG addr)
{
	...
	data->shdr_mode = iq_info->final_ipp.pre_fusion.fu_ctrl.mode;
}
...
static void iqt_api_get_bnr_param(ULONG addr)
{
	IQT_BNR_PARAM *data = (IQT_BNR_PARAM *)addr;
	
	memcpy(&(data->bnr), iq_param[data->id]->bnr, sizeof(IQ_BNR_PARAM));
}

static void iqt_api_get_low_power_param(ULONG addr)
{
	IQT_LOW_POWER_PARAM *data = (IQT_LOW_POWER_PARAM *)addr;
	ISP_MODULE *iq_module = iq_get_module();
	void *iq_private = iq_module->private;
	IQALG_INFO *iq_info;
	...
	memcpy(&(data->low_power), &(iq_info->low_power_param), sizeof(IQ_LOW_POWER_PARAM));
}
...
static void iqt_api_get_aiisp_param(ULONG addr)
{
	IQT_AIISP_PARAM *data = (IQT_AIISP_PARAM *)addr;

	memcpy(&(data->aiisp), iq_param[data->id]->aiisp, sizeof(IQ_AIISP_PARAM));
}

static void iqt_api_get_aiisp_custom_param(ULONG addr)
{
	IQT_AIISP_CUSTOM_PARAM *data = (IQT_AIISP_CUSTOM_PARAM *)addr;

	memcpy(&(data->aiisp_custom), iq_param[data->id]->aiisp_custom, sizeof(IQ_AIISP_CUSTOM_PARAM));
}
...
static void iqt_api_set_raw_va_param(ULONG addr)
{
	...
	// TODO: 539A IQ, not support IPE VA
...
static void iqt_api_set_3dcc_param(ULONG addr)
{
	...
	if (iq_info->final_ipp.ipe_3dcc != NULL) {
		...
		iq_info->final_ipp.ipe_3dcc->mode = data->_3dcc.mode;
...
static void iqt_api_set_dpc_param(ULONG addr)
{
	...
	if (iq_info->final_sie.sie_dpc != NULL) {
		...
		iq_info->final_sie.sie_dpc->def_same_ch_only_en = data->dpc.def_same_ch_only_en;
...
static void iqt_api_set_shdr_mode(ULONG addr)
{
	...
	iq_info->final_ipp.pre_fusion.fu_ctrl.mode = data->shdr_mode;
...
static void iqt_api_set_ob_mode_manual(ULONG addr)
{
	...
	if (data->ob_mode_manual.manual_mode == IQ_OB_IFE_F) {
...
static void iqt_api_set_bnr_param(ULONG addr)
{
	...
	memcpy(iq_param[data->id]->bnr, &(data->bnr), sizeof(IQ_BNR_PARAM));
	iqt_update[data->id][IQT_ITEM_BNR_PARAM] = TRUE;
}
...
static void iqt_api_set_low_power_param(ULONG addr)
{
	...
	memcpy(&(iq_info->low_power_param), &(data->low_power), sizeof(IQ_LOW_POWER_PARAM));
}

static void iqt_api_set_bnr_dbg(ULONG addr)
{
	IQT_BNR_DBG *data = (IQT_BNR_DBG *)addr;
	ISP_MODULE *iq_module = iq_get_module();
	void *iq_private = iq_module->private;
	IQALG_INFO *iq_info;

	iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(data->id));
	memcpy(&(iq_info->final_ipp.pre_bnr.bnr_debug_mode_en), &(data->enable), sizeof(BOOL));
	iqt_update[data->id][IQT_ITEM_BNR_DBG] = TRUE;
}
...
static void iqt_api_set_dg_mode_manual(ULONG addr)
{
	...
	if ((data->dg_mode_manual.manual_mode != IQ_DG_SIE) && (data->dg_mode_manual.manual_mode != IQ_DG_PRE) && (data->dg_mode_manual.manual_mode != IQ_DG_IFE) && (data->dg_mode_manual.manual_mode != IQ_DG_AI)) {
...
static void iqt_api_set_aiisp_param(ULONG addr)
{
	...
	memcpy(iq_param[data->id]->aiisp, &(data->aiisp), sizeof(IQ_AIISP_PARAM));
	iqt_update[data->id][IQT_ITEM_AIISP_PARAM] = TRUE;
}
...
static void iqt_api_set_aiisp_custom_param(ULONG addr)
{
	...
	memcpy(iq_param[data->id]->aiisp_custom, &(data->aiisp_custom), sizeof(IQ_AIISP_CUSTOM_PARAM));
	iqt_update[data->id][IQT_ITEM_AIISP_CUSTOM_PARAM] = TRUE;
}
...
static void iqt_api_set_cg_mode_manual(ULONG addr)
{
	...
	if ((data->cg_mode_manual.manual_mode != IQ_CG_SIE) && (data->cg_mode_manual.manual_mode != IQ_CG_PRE_F) && (data->cg_mode_manual.manual_mode != IQ_CG_PRE) && (data->cg_mode_manual.manual_mode != IQ_CG_IFE)) {
...
static void iqt_api_set_shdr_dbg(ULONG addr)
{
	...
	memcpy(&(iq_info->final_ipp.pre_fusion.dbg), &(data->shdr_dbg), sizeof(CTL_IFE_ISP_FUSION_DBG));
