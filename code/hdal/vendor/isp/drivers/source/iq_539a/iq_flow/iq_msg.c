...
void iq_msg_sie(IQALG_INFO *iq_info)
{
	...
	if ((iq_info->sie_tab_update & IQ_TBL_SIE_DPC) && (dpc != NULL)) {
		...
		if (dpc_param->enable) {
			...
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_SIE, dpc_param->def_same_ch_only_en);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_SIE, "\r\n");
	}
	...
	if (dgain != NULL) {
		...
		if (dgain->enable_stcs) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_SIE, dgain->gain_stcs);
		}
...
void iq_msg_pre(IQALG_INFO *iq_info)
{
	UINT32 dbg_mode = iq_dbg_get_dbg_mode(iq_info->id);

	CTL_PRE_ISP_NRS *nrs_0 = iq_info->final_setting.pre.p_nrs0;
	CTL_PRE_ISP_FCURVE *fcurve = iq_info->final_setting.pre.p_fcurve;
	CTL_PRE_ISP_FUSION *fusion = iq_info->final_setting.pre.p_fusion;
	CTL_PRE_ISP_CGAIN *cgain = iq_info->final_setting.pre.p_cgain;
	CTL_PRE_ISP_OUTL *outl = iq_info->final_setting.pre.p_outl;
	CTL_PRE_ISP_VA *va = iq_info->final_setting.pre.p_va;
	CTL_PRE_ISP_FPN *fpn = iq_info->final_setting.pre.p_fpn;
	CTL_PRE_ISP_BNR *bnr = iq_info->final_setting.pre.p_bnr;
	CTL_IPP_ISP_PRE_VA_WIN_SIZE_RATIO *va_win_size = iq_info->final_setting.ipp.pre_va_win_size;

	if (nrs_0 != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, nrs_0->enable);
		if (nrs_0->enable) {
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, nrs_0->str, CTL_PRE_ISP_NRS_STR_NUM);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "nrs_0 does not need to update!! \r\n");
	}

	if (fcurve != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fcurve->enable);
		if (fcurve->enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcur_ctrl.y_mean_sel);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcur_ctrl.yv_w);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcur_ctrl.ev_fmt);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fcurve->y_weight.y_w_lut, CTL_PRE_ISP_FCURVE_Y_W_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcurve_l.fcur_l_lut, CTL_PRE_ISP_FCURVE_L_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcurve_r.fcur_r_lut, CTL_PRE_ISP_FCURVE_R_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fcurve->fcurve_end.fcur_end_lut, CTL_PRE_ISP_FCURVE_END_NUM);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "fcurve does not need to update!! \r\n");
	}

	if (fusion != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_ctrl.y_mean_sel);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_ctrl.mode);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_ctrl.ev_ratio);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.nor_sel);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.dif_sel);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_nor_knee, CTL_PRE_ISP_FUSION_BLD_CUR_KNEE_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_nor_range);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_nor_slope);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_nor_knee, CTL_PRE_ISP_FUSION_BLD_CUR_KNEE_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_nor_range);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_nor_slope);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_dif_knee, CTL_PRE_ISP_FUSION_BLD_CUR_KNEE_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_dif_range);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.l_dif_slope);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_dif_knee, CTL_PRE_ISP_FUSION_BLD_CUR_KNEE_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_dif_range);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->bld_cur.s_dif_slope);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->mc_para.lum_th);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->mc_para.diff_ratio);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->mc_para.diff_w, CTL_PRE_ISP_FUSION_MC_DIFF_W_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->mc_para.dwd);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->dk_sat.th, CTL_PRE_ISP_FUSION_DARK_SAT_NUM);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->dk_sat.step, CTL_PRE_ISP_FUSION_DARK_SAT_NUM);
		PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fusion->dk_sat.low_bound, CTL_PRE_ISP_FUSION_DARK_SAT_NUM);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.enable);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.bit_field);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_s[CTL_PRE_ISP_CGAIN_CH_R]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_s[CTL_PRE_ISP_CGAIN_CH_GR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_s[CTL_PRE_ISP_CGAIN_CH_GB]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_s[CTL_PRE_ISP_CGAIN_CH_B]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_s[CTL_PRE_ISP_CGAIN_CH_IR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_s[CTL_PRE_ISP_CGAIN_CH_R]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_s[CTL_PRE_ISP_CGAIN_CH_GR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_s[CTL_PRE_ISP_CGAIN_CH_GR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_s[CTL_PRE_ISP_CGAIN_CH_B]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_s[CTL_PRE_ISP_CGAIN_CH_IR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_l[CTL_PRE_ISP_CGAIN_CH_R]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_l[CTL_PRE_ISP_CGAIN_CH_GR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_l[CTL_PRE_ISP_CGAIN_CH_GB]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_l[CTL_PRE_ISP_CGAIN_CH_B]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcgain_l[CTL_PRE_ISP_CGAIN_CH_IR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_l[CTL_PRE_ISP_CGAIN_CH_R]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_l[CTL_PRE_ISP_CGAIN_CH_GR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_l[CTL_PRE_ISP_CGAIN_CH_GB]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_l[CTL_PRE_ISP_CGAIN_CH_B]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->fu_cgain.fcofs_l[CTL_PRE_ISP_CGAIN_CH_IR]);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->dbg.enable);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fusion->dbg.mode);
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "fusion does not need to update!! \r\n");
	}

	if (cgain != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->enable);
		if (cgain->enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->inv);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->hinv);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->bit_field);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->mask);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cgain_r);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cgain_gr);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cgain_gb);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cgain_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cgain_ir);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cofs_r);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cofs_gr);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cofs_gb);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cofs_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, cgain->cofs_ir);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "cgain does not need to update!! \r\n");
	}

	if (outl != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->enable);
		if (outl->enable) {
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, outl->bright_th, CTL_PRE_ISP_OUTL_BRI_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, outl->dark_th, CTL_PRE_ISP_OUTL_DARK_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, outl->outl_cnt, CTL_PRE_ISP_OUTL_CNT_NUM);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->outl_weight);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->dark_ofs);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->bright_ofs);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->outl_comp_mode);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->ord_rng_bri);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->ord_rng_dark);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->ord_protect_th);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->ord_blend_w);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, outl->ord_bri_w, CTL_PRE_ISP_OUTL_ORD_W_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, outl->ord_dark_w, CTL_PRE_ISP_OUTL_ORD_W_NUM);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->outl_rgbir_rb_w);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, outl->ord_rgbir_rb_w);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "outl does not need to update!! \r\n");
	}

	if (va != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->enable)
		if (va->enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_va_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->gamma_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.symmetry);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.filter_size);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.tap_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.tap_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.tap_c);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.tap_d);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.div);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.th_l);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.h_filt.th_u);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.symmetry);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.filter_size);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.tap_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.tap_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.tap_c);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.tap_d);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.div);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.th_l);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.v_filt.th_u);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.symmetry_iir2);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.symmetry_iir3);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir1_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir1_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.iir1_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.iir2_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir2_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir2_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir2_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir2_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.iir2_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.iir3_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir3_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir3_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir3_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.tap_iir3_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.iir_filt.iir3_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_1.count_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.symmetry);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.filter_size);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.tap_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.tap_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.tap_c);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.tap_d);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.div);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.th_l);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.h_filt.th_u);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.symmetry);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.filter_size);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.tap_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.tap_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.tap_c);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.tap_d);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.div);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.th_l);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.v_filt.th_u);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.symmetry_iir2);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.symmetry_iir3);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir1_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir1_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.iir1_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.iir2_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir2_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir2_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir2_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir2_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.iir2_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.iir3_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir3_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir3_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir3_e);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.tap_iir3_f);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.iir_filt.iir3_shift_bit);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->group_2.count_enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->fltr_sel.vdetgh1_filter_sel);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->fltr_sel.vdetgh2_filter_sel);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->va_out_grp1_2);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->win_num.w);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->win_num.h);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_win[0].enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_win[1].enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_win[2].enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_win[3].enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->indep_win[4].enable);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_low_th);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_high_th);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_low_gain);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_high_gain);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_low_slope);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->ldg_para.ldg_high_slope);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->pre_filter_mode);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, va->va_gamma_lut,CTL_PRE_ISP_VA_GAMMA_LEN);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->win_cnt_out_sel);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->high_luma_th);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va->energy_w);
			if (va_win_size != NULL) {
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->ratio_base);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->winsz_ratio.w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->winsz_ratio.h);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[0].x);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[0].y);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[0].w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[0].h);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[1].x);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[1].y);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[1].w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[1].h);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[2].x);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[2].y);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[2].w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[2].h);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[3].x);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[3].y);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[3].w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[3].h);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[4].x);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[4].y);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[4].w);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, va_win_size->indep_roi_ratio[4].h);
			}
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "va does not need to update!! \r\n");
	}

	if (fpn != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_en);
		if (fpn->fpn_en) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_range);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_r, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_gr, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_gb, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_b, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cgain_ir, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cofs_r, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cofs_gr, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cofs_gb, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cofs_b, CTL_PRE_ISP_FPN_PATH_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, fpn->fpn_cofs_ir, CTL_PRE_ISP_FPN_PATH_MAX);
			if (iq_info->final_setting.ipp.pre_fpn_info != NULL) {
				PRINT_IQ_VAR_HEX(dbg_mode & IQ_DBG_P_PRE, iq_info->final_setting.ipp.pre_fpn_info->buf_phyaddr);
				PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, iq_info->final_setting.ipp.pre_fpn_info->buf_size);
				PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
			} else {
				PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "pre_cent does not need to update!! \r\n");
			}
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "fpn_param does not need to update!! \r\n");
	}

	if (bnr != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_enable);
		if (bnr->bnr_enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_debug_mode_en);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_prefilter_str);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_err_comp.err_comp_en);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_err_comp.err_comp_l, CTL_PRE_ISP_BNR_ERR_COMP_L_MAX);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_err_comp.err_comp_r, CTL_PRE_ISP_BNR_ERR_COMP_R_MAX);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_err_comp.err_sft);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_np_coef.coef_a);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->bnr_np_coef.coef_b);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->ds_th.downsample_th1);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->ds_th.downsample_th2);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->fusion_str.static_region);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->fusion_str.transision_region);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->fusion_str.motion_region);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->residue_th);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, bnr->md.md_th_l, CTL_PRE_ISP_BNR_MD_TH_L);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_PRE, bnr->md.md_th_r, CTL_PRE_ISP_BNR_MD_TH_R);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->md.md_base);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->md.md_k1);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_PRE, bnr->md.md_k2);
		}
		PRINT_IQ(dbg_mode & IQ_DBG_P_IME, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_PRE, "bnr_param does not need to update!! \r\n");
	}

	iq_dbg_clr_dbg_mode(iq_info->id, IQ_DBG_P_PRE);
}
...
void iq_msg_ife(IQALG_INFO *iq_info)
{
	...
	if (filt != NULL) {
		...
		if (filt->enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->spatial.ife_filt_mode);
			...
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_1.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_1.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_1.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_1.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_1.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_1.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_1.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_1.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_1.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_1.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_1.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_1.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_1.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_1.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_1.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_1.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_1.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_1.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_1.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_1.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_2.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_2.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_2.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_r_2.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_2.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_2.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_2.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gr_2.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_2.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_2.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_2.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_gb_2.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_2.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_2.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_2.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_b_2.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_2.a_th, CTL_IFE_ISP_RANGE_A_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_2.a_lut, CTL_IFE_ISP_RANGE_A_LUT_SIZE);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_2.b_th, CTL_IFE_ISP_RANGE_B_TH_NUM);
			PRINT_IQ_ARR(dbg_mode & IQ_DBG_P_IFE, filt->rng_filt_ir_2.b_lut, CTL_IFE_ISP_RANGE_B_LUT_SIZE);
			...
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->range_sel);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->motion_set.motion_en);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->motion_set.static_symbol);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->motion_set.transition_symbol);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IFE, filt->motion_set.motion_symbol);
		}
...
void iq_msg_aiisp(IQALG_INFO *iq_info)
{
	UINT32 dbg_mode = iq_dbg_get_dbg_mode(iq_info->id);
	CTL_IPP_ISP_AIISP_PARAM *aiisp = iq_info->final_setting.aiisp;
	UINT32 i;

	if (aiisp != NULL) {
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->version);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->enable);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->path_id);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->effect);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->param_num);
		for (i = 0; i < aiisp->param_num; i++) {
			PRINT_IQ(dbg_mode & IQ_DBG_P_AIISP, "aiisp->param_size[%d] = %d \r\n", i, aiisp->param_size[i]);
			PRINT_IQ(dbg_mode & IQ_DBG_P_AIISP, "aiisp->param_phyaddr[%d] = 0x%lX, value = %d \r\n", i, aiisp->param_phyaddr[i], iq_info->aiisp_final_param[i]);
		}
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_AIISP, aiisp->reserved_param_size);
		PRINT_IQ(dbg_mode & IQ_DBG_P_AIISP, "aiisp->reserved_param_viraddr = 0x%lX, value = %d \r\n", aiisp->reserved_param_viraddr, *(UINT8 *)aiisp->reserved_param_viraddr);
		PRINT_IQ(dbg_mode & IQ_DBG_P_AIISP, "\r\n");
	} else {
		PRINT_IQ(dbg_mode & IQ_DBG_P_AIISP, "aiisp does not need to update!! \r\n");
	}

	iq_dbg_clr_dbg_mode(iq_info->id, IQ_DBG_P_AIISP);
}
...
void iq_msg_ipe(IQALG_INFO *iq_info)
{
	...
	if (eext != NULL) {
		...
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_enable);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_symbol.static_symbol);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_symbol.transition_symbol);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_symbol.motion_symbol);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_str.static_str);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_str.transition_str);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->motion_str.motion_str);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->edge_ker_mode);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IPE, eext->edge_mode);
	...
	if (_3dcc->enable) {
		...
		PRINT_IQ_VAR((dbg_mode & IQ_DBG_P_IPE) || (dbg_mode & IQ_DBG_TABLE), _3dcc->mode);
		for (i = 0; i < 30; i++) {
...
void iq_msg_ime(IQALG_INFO *iq_info)
{
	...
	if (lca != NULL) {
		...
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, lca->edge_detect_en);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, lca->ref_center_en);
		PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, lca->motion_status_en);
	...
	if (dbcs != NULL) {
		...
		if (dbcs->enable) {
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, dbcs->proc_location);
	...
	if (tmnr != NULL) {
		...
		if (tmnr->enable) {
			...
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, tmnr->me_param.motion_en);
			...
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, tmnr->nr_param.tf_y_set_en);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, tmnr->nr_param.tf_c_set_en);
			...
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, tmnr->nr_param.dc_ratio0);
			PRINT_IQ_VAR(dbg_mode & IQ_DBG_P_IME, tmnr->nr_param.dc_ratio1);
...
#endif
{
	...
	PRINT_IQ_INFO(sfile, "    ai dgain :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->dg_ai);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "  ai dgain 2 :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->dg_ai2);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "          ob :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->final_sie.sie_ob.ob_ofs);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "       ai ob :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->ob_ai);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	PRINT_IQ_INFO(sfile, "  Stripe num :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->ipp_trig_obj.stripe_num);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	PRINT_IQ_INFO(sfile, "    AI AIDED :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", ((iq_info->ipp_trig_obj.func_en & ISP_FUNC_EN_AI_AIDED) ? 1 : 0));
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	PRINT_IQ_INFO(sfile, " AIISP DG_AI :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->dg_ai_enable);
		}
	}
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		...
		if (iq_flow_get_id_valid(id)) {
			...
			} else if (iq_info->dg_mode == IQ_DG_AI) {
				PRINT_IQ_INFO(sfile, "        AI");
			} else {
				PRINT_IQ_INFO(sfile, "    UNKNOW");
			}
	...
	PRINT_IQ_INFO(sfile, "aided_test_en: %9d \r\n", aiisp_aided_test_en);
	PRINT_IQ_INFO(sfile, "     clone_id: %9d \r\n", aiisp_aided_test_clone_id);
	PRINT_IQ_INFO(sfile, "       src_id: %9d \r\n", aiisp_aided_test_src_id);
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		...
		if (iq_flow_get_id_valid(id)) {
			...
			if ((iq_info->dpc_en == TRUE) && (iq_info->final_sie.sie_dpc != NULL)) {
				...
				PRINT_IQ_INFO(sfile, "    def_same_ch_only_en = %d \r\n", iq_info->final_sie.sie_dpc->def_same_ch_only_en);
			}
		}
	}
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		...
		if (iq_flow_get_id_valid(id)) {
			...
			if ((iq_info->_3dcc_en == TRUE) && (iq_info->final_ipp.ipe_3dcc != NULL)) {
				...
				PRINT_IQ_INFO(sfile, "      mode = %d \r\n", iq_info->final_ipp.ipe_3dcc->mode);
...
#endif
{
	...
	IQ_NR_EXT_TUNE_PARAM             *nr_ext_m, *nr_ext_a;
	...
	IQ_EDGE_EXT_TUNE_PARAM           *edge_ext_m, *edge_ext_a;
	IQ_3DNR_TUNE_PARAM               *_3dnr_m, *_3dnr_a;
	IQ_3DNR_EXT_TUNE_PARAM           *_3dnr_ext_m, *_3dnr_ext_a;
	...
	PRINT_IQ_INFO(sfile, "dbcs_proc_location = %d, \r\n", iq_param[iq_proc_id]->nr->dbcs_proc_location);

	nr_ext_m = &iq_param[iq_proc_id]->nr->ext_manual_param;
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_th_1 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_th_1[0], nr_ext_m->filter_th_1[1], nr_ext_m->filter_th_1[2], nr_ext_m->filter_th_1[3], nr_ext_m->filter_th_1[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_lut_1 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_lut_1[0], nr_ext_m->filter_lut_1[1], nr_ext_m->filter_lut_1[2], nr_ext_m->filter_lut_1[3], nr_ext_m->filter_lut_1[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_th_b_1 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_th_b_1[0], nr_ext_m->filter_th_b_1[1], nr_ext_m->filter_th_b_1[2], nr_ext_m->filter_th_b_1[3], nr_ext_m->filter_th_b_1[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_lut_b_1 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_lut_b_1[0], nr_ext_m->filter_lut_b_1[1], nr_ext_m->filter_lut_b_1[2], nr_ext_m->filter_lut_b_1[3], nr_ext_m->filter_lut_b_1[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_th_2 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_th_2[0], nr_ext_m->filter_th_2[1], nr_ext_m->filter_th_2[2], nr_ext_m->filter_th_2[3], nr_ext_m->filter_th_2[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_lut_2 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_lut_2[0], nr_ext_m->filter_lut_2[1], nr_ext_m->filter_lut_2[2], nr_ext_m->filter_lut_2[3], nr_ext_m->filter_lut_2[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_th_b_2 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_th_b_2[0], nr_ext_m->filter_th_b_2[1], nr_ext_m->filter_th_b_2[2], nr_ext_m->filter_th_b_2[3], nr_ext_m->filter_th_b_2[4]);
	PRINT_IQ_INFO(sfile, "ext_manual_param.filter_lut_b_2 = %d, %d, %d, %d, %d, \r\n"
		, nr_ext_m->filter_lut_b_2[0], nr_ext_m->filter_lut_b_2[1], nr_ext_m->filter_lut_b_2[2], nr_ext_m->filter_lut_b_2[3], nr_ext_m->filter_lut_b_2[4]);

	nr_ext_a = &iq_param[iq_proc_id]->nr->ext_auto_param[iq_proc_iso];
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_th_1 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_th_1[0], nr_ext_a->filter_th_1[1], nr_ext_a->filter_th_1[2], nr_ext_a->filter_th_1[3], nr_ext_a->filter_th_1[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_lut_1 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_lut_1[0], nr_ext_a->filter_lut_1[1], nr_ext_a->filter_lut_1[2], nr_ext_a->filter_lut_1[3], nr_ext_a->filter_lut_1[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_th_b_1 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_th_b_1[0], nr_ext_a->filter_th_b_1[1], nr_ext_a->filter_th_b_1[2], nr_ext_a->filter_th_b_1[3], nr_ext_a->filter_th_b_1[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_lut_b_1 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_lut_b_1[0], nr_ext_a->filter_lut_b_1[1], nr_ext_a->filter_lut_b_1[2], nr_ext_a->filter_lut_b_1[3], nr_ext_a->filter_lut_b_1[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_th_2 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_th_2[0], nr_ext_a->filter_th_2[1], nr_ext_a->filter_th_2[2], nr_ext_a->filter_th_2[3], nr_ext_a->filter_th_2[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_lut_2 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_lut_2[0], nr_ext_a->filter_lut_2[1], nr_ext_a->filter_lut_2[2], nr_ext_a->filter_lut_2[3], nr_ext_a->filter_lut_2[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_th_b_2 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_th_b_2[0], nr_ext_a->filter_th_b_2[1], nr_ext_a->filter_th_b_2[2], nr_ext_a->filter_th_b_2[3], nr_ext_a->filter_th_b_2[4]);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].filter_lut_b_2 = %d, %d, %d, %d, %d, \r\n", iq_proc_iso
		, nr_ext_a->filter_lut_b_2[0], nr_ext_a->filter_lut_b_2[1], nr_ext_a->filter_lut_b_2[2], nr_ext_a->filter_lut_b_2[3], nr_ext_a->filter_lut_b_2[4]);
	...
	edge_ext_m = &iq_param[iq_proc_id]->edge->ext_manual_param;
	PRINT_IQ_INFO(sfile, "ext_manual_param.motion_str = %d, %d, %d, \r\n", edge_ext_m->motion_str[0], edge_ext_m->motion_str[1], edge_ext_m->motion_str[2]);
	...
	edge_ext_a = &iq_param[iq_proc_id]->edge->ext_auto_param[iq_proc_iso];
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].motion_str = %d, %d, %d, \r\n", iq_proc_iso, edge_ext_a->motion_str[0], edge_ext_a->motion_str[1], edge_ext_a->motion_str[2]);
	...
	_3dnr_ext_m = &iq_param[iq_proc_id]->_3dnr->ext_manual_param;
	PRINT_IQ_INFO(sfile, "ext_manual_param.dc_ratio0 = %d, \r\n", _3dnr_ext_m->dc_ratio0);
	PRINT_IQ_INFO(sfile, "ext_manual_param.dc_ratio1 = %d, \r\n", _3dnr_ext_m->dc_ratio1);
	...
	_3dnr_ext_a = &iq_param[iq_proc_id]->_3dnr->ext_auto_param[iq_proc_iso];
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].dc_ratio0 = %d, \r\n", iq_proc_iso, _3dnr_ext_a->dc_ratio0);
	PRINT_IQ_INFO(sfile, "ext_auto_param[%d].dc_ratio1 = %d, \r\n", iq_proc_iso, _3dnr_ext_a->dc_ratio1);
	...
	PRINT_IQ_INFO(sfile, "fcurve_left_lut = %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,  \r\n"
		...
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[4], iq_param[iq_proc_id]->shdr->fcurve_left_lut[5]
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[6], iq_param[iq_proc_id]->shdr->fcurve_left_lut[7]
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[8], iq_param[iq_proc_id]->shdr->fcurve_left_lut[9]
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[10], iq_param[iq_proc_id]->shdr->fcurve_left_lut[11]
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[12], iq_param[iq_proc_id]->shdr->fcurve_left_lut[13]
		, iq_param[iq_proc_id]->shdr->fcurve_left_lut[14], iq_param[iq_proc_id]->shdr->fcurve_left_lut[15]);
	PRINT_IQ_INFO(sfile, "fcurve_right_lut = %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \r\n"
		...
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[4], iq_param[iq_proc_id]->shdr->fcurve_right_lut[5]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[6], iq_param[iq_proc_id]->shdr->fcurve_right_lut[7]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[8], iq_param[iq_proc_id]->shdr->fcurve_right_lut[9]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[10], iq_param[iq_proc_id]->shdr->fcurve_right_lut[11]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[12], iq_param[iq_proc_id]->shdr->fcurve_right_lut[13]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[14], iq_param[iq_proc_id]->shdr->fcurve_right_lut[15]
		, iq_param[iq_proc_id]->shdr->fcurve_right_lut[16]);
	PRINT_IQ_INFO(sfile, "fcurve_end_lut = %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,  \r\n"
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[0], iq_param[iq_proc_id]->shdr->fcurve_end_lut[1]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[2], iq_param[iq_proc_id]->shdr->fcurve_end_lut[3]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[4], iq_param[iq_proc_id]->shdr->fcurve_end_lut[5]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[6], iq_param[iq_proc_id]->shdr->fcurve_end_lut[7]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[8], iq_param[iq_proc_id]->shdr->fcurve_end_lut[9]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[10], iq_param[iq_proc_id]->shdr->fcurve_end_lut[11]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[12], iq_param[iq_proc_id]->shdr->fcurve_end_lut[13]
		, iq_param[iq_proc_id]->shdr->fcurve_end_lut[14], iq_param[iq_proc_id]->shdr->fcurve_end_lut[15]);
	...
	// iq_bnr_param
	PRINT_IQ_INFO(sfile, "====================id(%d) iq_bnr_param==================== \r\n", iq_proc_id);
	PRINT_IQ_INFO(sfile, "enable = %d, \r\n", iq_param[iq_proc_id]->bnr->enable);
	PRINT_IQ_INFO(sfile, "err_comp_en = %d, \r\n", iq_param[iq_proc_id]->bnr->err_comp_en);

	bnr_m = &iq_param[iq_proc_id]->bnr->manual_param;
	PRINT_IQ_INFO(sfile, "manual_param.prefilter_str = %d, \r\n", bnr_m->prefilter_str);
	PRINT_IQ_INFO(sfile, "manual_param.err_comp_l = %d, %d, %d, %d, %d, \r\n"
		, bnr_m->err_comp_l[0], bnr_m->err_comp_l[0]
		, bnr_m->err_comp_l[0], bnr_m->err_comp_l[0]
		, bnr_m->err_comp_l[0]);
	PRINT_IQ_INFO(sfile, "manual_param.err_comp_r = %d, %d, %d, %d, %d, \r\n"
		, bnr_m->err_comp_r[0], bnr_m->err_comp_r[0]
		, bnr_m->err_comp_r[0], bnr_m->err_comp_r[0]
		, bnr_m->err_comp_r[0]);
	PRINT_IQ_INFO(sfile, "manual_param.err_sft = %d, \r\n", bnr_m->err_sft);
	PRINT_IQ_INFO(sfile, "manual_param.coef_a = %d, \r\n", bnr_m->coef_a);
	PRINT_IQ_INFO(sfile, "manual_param.coef_b = %d, \r\n", bnr_m->coef_b);
	PRINT_IQ_INFO(sfile, "manual_param.downsample_th1 = %d, \r\n", bnr_m->downsample_th1);
	PRINT_IQ_INFO(sfile, "manual_param.downsample_th2 = %d, \r\n", bnr_m->downsample_th2);
	PRINT_IQ_INFO(sfile, "manual_param.static_region = %d, \r\n", bnr_m->static_region);
	PRINT_IQ_INFO(sfile, "manual_param.transision_region = %d, \r\n", bnr_m->transision_region);
	PRINT_IQ_INFO(sfile, "manual_param.motion_region = %d, \r\n", bnr_m->motion_region);
	PRINT_IQ_INFO(sfile, "manual_param.residue_th = %d, \r\n", bnr_m->residue_th);
	PRINT_IQ_INFO(sfile, "manual_param.md_th_l = %d, %d, %d, %d, %d, \r\n"
		, bnr_m->md_th_l[0], bnr_m->md_th_l[0]
		, bnr_m->md_th_l[0], bnr_m->md_th_l[0]
		, bnr_m->md_th_l[0]);
	PRINT_IQ_INFO(sfile, "manual_param.md_th_r = %d, %d, %d, %d, %d, \r\n"
		, bnr_m->md_th_r[0], bnr_m->md_th_r[0]
		, bnr_m->md_th_r[0], bnr_m->md_th_r[0]
		, bnr_m->md_th_r[0]);
	PRINT_IQ_INFO(sfile, "manual_param.md_base = %d, \r\n", bnr_m->md_base);
	PRINT_IQ_INFO(sfile, "manual_param.md_k1 = %d, \r\n", bnr_m->md_k1);
	PRINT_IQ_INFO(sfile, "manual_param.md_k2 = %d, \r\n", bnr_m->md_k2);

	bnr_a = &iq_param[iq_proc_id]->bnr->auto_param[iq_proc_iso];
	PRINT_IQ_INFO(sfile, "auto_param.prefilter_str = %d, \r\n", bnr_a->prefilter_str);
	PRINT_IQ_INFO(sfile, "auto_param.err_comp_l = %d, %d, %d, %d, %d, \r\n"
		, bnr_a->err_comp_l[0], bnr_a->err_comp_l[0]
		, bnr_a->err_comp_l[0], bnr_a->err_comp_l[0]
		, bnr_a->err_comp_l[0]);
	PRINT_IQ_INFO(sfile, "auto_param.err_comp_r = %d, %d, %d, %d, %d, \r\n"
		, bnr_a->err_comp_r[0], bnr_a->err_comp_r[0]
		, bnr_a->err_comp_r[0], bnr_a->err_comp_r[0]
		, bnr_a->err_comp_r[0]);
	PRINT_IQ_INFO(sfile, "auto_param.err_sft = %d, \r\n", bnr_a->err_sft);
	PRINT_IQ_INFO(sfile, "auto_param.coef_a = %d, \r\n", bnr_a->coef_a);
	PRINT_IQ_INFO(sfile, "auto_param.coef_b = %d, \r\n", bnr_a->coef_b);
	PRINT_IQ_INFO(sfile, "auto_param.downsample_th1 = %d, \r\n", bnr_a->downsample_th1);
	PRINT_IQ_INFO(sfile, "auto_param.downsample_th2 = %d, \r\n", bnr_a->downsample_th2);
	PRINT_IQ_INFO(sfile, "auto_param.static_region = %d, \r\n", bnr_a->static_region);
	PRINT_IQ_INFO(sfile, "auto_param.transision_region = %d, \r\n", bnr_a->transision_region);
	PRINT_IQ_INFO(sfile, "auto_param.motion_region = %d, \r\n", bnr_a->motion_region);
	PRINT_IQ_INFO(sfile, "auto_param.residue_th = %d, \r\n", bnr_a->residue_th);
	PRINT_IQ_INFO(sfile, "auto_param.md_th_l = %d, %d, %d, %d, %d, \r\n"
		, bnr_a->md_th_l[0], bnr_a->md_th_l[0]
		, bnr_a->md_th_l[0], bnr_a->md_th_l[0]
		, bnr_a->md_th_l[0]);
	PRINT_IQ_INFO(sfile, "auto_param.md_th_r = %d, %d, %d, %d, %d, \r\n"
		, bnr_a->md_th_r[0], bnr_a->md_th_r[0]
		, bnr_a->md_th_r[0], bnr_a->md_th_r[0]
		, bnr_a->md_th_r[0]);
	PRINT_IQ_INFO(sfile, "auto_param.md_base = %d, \r\n", bnr_a->md_base);
	PRINT_IQ_INFO(sfile, "auto_param.md_k1 = %d, \r\n", bnr_a->md_k1);
	PRINT_IQ_INFO(sfile, "auto_param.md_k2 = %d, \r\n", bnr_a->md_k2);

	// iq_aiisp_param
	PRINT_IQ_INFO(sfile, "====================id(%d) iq_aiisp_param==================== \r\n", iq_proc_id);
	PRINT_IQ_INFO(sfile, "path_id =               %d, \r\n", iq_param[iq_proc_id]->aiisp->path_id);
	PRINT_IQ_INFO(sfile, "version =               %d, \r\n", iq_param[iq_proc_id]->aiisp->version);
	PRINT_IQ_INFO(sfile, "param_num =             %d, \r\n", iq_param[iq_proc_id]->aiisp->param_num);
	for (i = 0; i < iq_param[iq_proc_id]->aiisp->param_num; i++) {
		PRINT_IQ_INFO(sfile, "param_size[%d] =         %d, \r\n", i, iq_param[iq_proc_id]->aiisp->param_size[i]);
		PRINT_IQ_INFO(sfile, "param_name[%d] =         %s, \r\n", i, iq_param[iq_proc_id]->aiisp->param_name[i]);
	}
	PRINT_IQ_INFO(sfile, "mode =                  %d, \r\n", iq_param[iq_proc_id]->aiisp->mode);
	
	PRINT_IQ_INFO(sfile, "manual_param.enable =   %d, \r\n", iq_param[iq_proc_id]->aiisp->manual_param.enable);
	PRINT_IQ_INFO(sfile, "manual_param.effect =   %d, \r\n", iq_param[iq_proc_id]->aiisp->manual_param.effect);
	for (i = 0; i < iq_param[iq_proc_id]->aiisp->param_num; i++) {
		PRINT_IQ_INFO(sfile, "manual_param.param[%d] = %d, \r\n", i, iq_param[iq_proc_id]->aiisp->manual_param.param[i]);
	}

	PRINT_IQ_INFO(sfile, "auto_param.enable =     %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \r\n", 
																iq_param[iq_proc_id]->aiisp->auto_param[0].enable, iq_param[iq_proc_id]->aiisp->auto_param[1].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[2].enable, iq_param[iq_proc_id]->aiisp->auto_param[3].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[4].enable, iq_param[iq_proc_id]->aiisp->auto_param[5].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[6].enable, iq_param[iq_proc_id]->aiisp->auto_param[7].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[8].enable, iq_param[iq_proc_id]->aiisp->auto_param[9].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[10].enable, iq_param[iq_proc_id]->aiisp->auto_param[11].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[12].enable, iq_param[iq_proc_id]->aiisp->auto_param[13].enable, 
																iq_param[iq_proc_id]->aiisp->auto_param[14].enable, iq_param[iq_proc_id]->aiisp->auto_param[15].enable);
	PRINT_IQ_INFO(sfile, "auto_param.effect =     %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \r\n", 
																iq_param[iq_proc_id]->aiisp->auto_param[0].effect, iq_param[iq_proc_id]->aiisp->auto_param[1].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[2].effect, iq_param[iq_proc_id]->aiisp->auto_param[3].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[4].effect, iq_param[iq_proc_id]->aiisp->auto_param[5].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[6].effect, iq_param[iq_proc_id]->aiisp->auto_param[7].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[8].effect, iq_param[iq_proc_id]->aiisp->auto_param[9].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[10].effect, iq_param[iq_proc_id]->aiisp->auto_param[11].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[12].effect, iq_param[iq_proc_id]->aiisp->auto_param[13].effect, 
																iq_param[iq_proc_id]->aiisp->auto_param[14].effect, iq_param[iq_proc_id]->aiisp->auto_param[15].effect);
	for (i = 0; i < iq_param[iq_proc_id]->aiisp->param_num; i++) {
		PRINT_IQ_INFO(sfile, "auto_param.param[%d] =   %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \r\n", 
																i, iq_param[iq_proc_id]->aiisp->auto_param[0].param[i], iq_param[iq_proc_id]->aiisp->auto_param[1].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[2].param[i], iq_param[iq_proc_id]->aiisp->auto_param[3].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[4].param[i], iq_param[iq_proc_id]->aiisp->auto_param[5].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[6].param[i], iq_param[iq_proc_id]->aiisp->auto_param[7].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[8].param[i], iq_param[iq_proc_id]->aiisp->auto_param[9].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[10].param[i], iq_param[iq_proc_id]->aiisp->auto_param[11].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[12].param[i], iq_param[iq_proc_id]->aiisp->auto_param[13].param[i], 
																	iq_param[iq_proc_id]->aiisp->auto_param[14].param[i], iq_param[iq_proc_id]->aiisp->auto_param[15].param[i]);
	}

	// iq_aiisp_custom_param
	PRINT_IQ_INFO(sfile, "====================id(%d) iq_aiisp_custom_param==================== \r\n", iq_proc_id);
	PRINT_IQ_INFO(sfile, "enable =                %d, \r\n", iq_param[iq_proc_id]->aiisp_custom->enable);
	PRINT_IQ_INFO(sfile, "path_id =               %d, \r\n", iq_param[iq_proc_id]->aiisp_custom->path_id);
	PRINT_IQ_INFO(sfile, "effect =                %d, \r\n", iq_param[iq_proc_id]->aiisp_custom->effect);
	PRINT_IQ_INFO(sfile, "param_num =             %d, \r\n", iq_param[iq_proc_id]->aiisp_custom->param_num);
	for (i = 0; i < iq_param[iq_proc_id]->aiisp_custom->param_num; i++) {
		PRINT_IQ_INFO(sfile, "param_size[%d] =    %d, \r\n", i, iq_param[iq_proc_id]->aiisp_custom->param_size[i]);
		PRINT_IQ_INFO(sfile, "param_phyaddr[%d] = 0x%lX, \r\n", i, iq_param[iq_proc_id]->aiisp_custom->param_phyaddr[i]);
	}
}
...
#endif
{
	...
	void __iomem *pre_reg_addr = NULL;
	...
	#endif
	//SIE
	T_R4_ENGINE_FUNCTION sie_eng_reg;
	//PRE
	T_PRE_CONTROL_REGISTER pre_ctrl_reg;
	T_PRE_SETTING_REGISTER pre_set_reg;
	T__3DNR_REGISTER _3dnr_reg;
	T_VA_REGISTER1 pre_va1_reg;
	T_VA_REGISTER10 pre_va10_reg;
	//IFE
	T_IFE_CONTROL_REGISTER ife_ctrl_reg;
	T_IFE_SETTING_REGISTER ife_set_reg;
	T_IFE_MOTION_CONTROL_REGISTER ife_motion_ctrl_reg;
	T_WDR_CONTROL_REGISTER0 wdr_ctrl_reg0;
	T_IFE_WDR_SUBIMAGE_REGISTER_0 wdr_subimg_reg;
	T_IFE_WDR_CONTROL_REGISTER1 wdr_ctrl_reg1;
	T_IFE_WDR_FBC_REGISTER_0 wdr_fbc_reg;
	T_IFE_WDR_TONE_MAPPING_REGISTER_0 wdr_tone_reg;
	//IPE
	T_IPE_MODE_REGISTER_1 ipe_mode_reg;
	T_IPE_POWER_SAVE_MODE_REGISTER ipe_power_save_reg;
	T_DEFOG_AIRLIGHT_REGISTER_1 defog_airlight_reg;
	T_PURPLE_FRINGE_REDUCTION_REGISTER0 pfr_control_reg;
	T_IPE_MODE_REGISTER_2 ipe_mode_reg2;
	//IME
	T_IME_FUNCTION_CONTROL_REGISTER0 ime_func_ctrl_reg;
	T_LOCAL_CHROMA_ADAPTATION_REGISTER0 local_chroma_reg;
	T_IME_TMNR_CONTROL_REGISTER0 tmnr_ctrl_reg;

	sie1_reg_addr = IOREMAP(SIE_BASE_ADDR, SIE_SIZE);
	pre_reg_addr = IOREMAP(PRE_BASE_ADDR, PRE_SIZE);
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->id);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "            low power lv :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_lv);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	// SIE
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "    SIE REGISTERS \r\n");
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	reg_addr = sie1_reg_addr;
	sie_eng_reg.reg = GETREG(reg_addr + R4_ENGINE_FUNCTION_OFS);
	PRINT_IQ_INFO(sfile, "                  dpc_en : %4d \r\n", sie_eng_reg.bit.DPC_EN);
	PRINT_IQ_INFO(sfile, "                  ecs_en : %4d \r\n", sie_eng_reg.bit.ECS_EN);

	// PRE
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "    PRE REGISTERS \r\n");
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	reg_addr = pre_reg_addr;
	pre_ctrl_reg.reg = GETREG(reg_addr + PRE_CONTROL_REGISTER_OFS);
	pre_set_reg.reg =  GETREG(reg_addr + PRE_SETTING_REGISTER_OFS);
	_3dnr_reg.reg =  GETREG(reg_addr + _3DNR_REGISTER_OFS);
	pre_va1_reg.reg = GETREG(reg_addr + VA_REGISTER1_OFS);
	pre_va10_reg.reg = GETREG(reg_addr + VA_REGISTER10_OFS);

	PRINT_IQ_INFO(sfile, "                 vacc_en : %4d \r\n", pre_va1_reg.bit.vacc_en);
	PRINT_IQ_INFO(sfile, "               win0_vaen : %4d \r\n", pre_va1_reg.bit.win0_vaen);
	PRINT_IQ_INFO(sfile, "               win1_vaen : %4d \r\n", pre_va1_reg.bit.win1_vaen);
	PRINT_IQ_INFO(sfile, "               win2_vaen : %4d \r\n", pre_va1_reg.bit.win2_vaen);
	PRINT_IQ_INFO(sfile, "               win3_vaen : %4d \r\n", pre_va1_reg.bit.win3_vaen);
	PRINT_IQ_INFO(sfile, "               win4_vaen : %4d \r\n", pre_va1_reg.bit.win4_vaen);
	PRINT_IQ_INFO(sfile, "      va_vertical_fir_en : %4d \r\n", pre_va1_reg.bit.va_vertical_fir_en);
	PRINT_IQ_INFO(sfile, "             vacc_outsel : %4d \r\n", pre_va10_reg.bit.vacc_outsel);
	PRINT_IQ_INFO(sfile, "     va_sram_shutdown_en : %4d \r\n", pre_set_reg.bit.pre_va_sram_shutdown_en);
	PRINT_IQ_INFO(sfile, "                 outl_en : %4d \r\n", pre_ctrl_reg.bit.outl_en);
	PRINT_IQ_INFO(sfile, "   outl_sram_shutdown_en : %4d \r\n", pre_set_reg.bit.pre_outl_sram_shutdown_en);
	PRINT_IQ_INFO(sfile, "                  bnr_en : %4d \r\n", _3dnr_reg.bit.pre_bnr_en);
	PRINT_IQ_INFO(sfile, "    bnr_sram_shutdown_en : %4d \r\n", pre_set_reg.bit.pre_bnr_sram_shutdown_en);
	PRINT_IQ_INFO(sfile, "    hdr_sram_shutdown_en : %4d \r\n", pre_set_reg.bit.pre_hdr_sram_shutdown_en);

	// IFE
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "    IFE REGISTERS \r\n");
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	reg_addr = ife_reg_addr;
	ife_ctrl_reg.reg = GETREG(reg_addr + IFE_CONTROL_REGISTER_OFS);
	ife_set_reg.reg = GETREG(reg_addr + IFE_SETTING_REGISTER_OFS);
	ife_motion_ctrl_reg.reg = GETREG(reg_addr + IFE_MOTION_CONTROL_REGISTER_OFS);
	wdr_ctrl_reg0.reg = GETREG(reg_addr + WDR_CONTROL_REGISTER0_OFS);
	wdr_subimg_reg.reg = GETREG(reg_addr + IFE_WDR_SUBIMAGE_REGISTER_0_OFS);
	wdr_ctrl_reg1.reg = GETREG(reg_addr + IFE_WDR_CONTROL_REGISTER1_OFS);
	wdr_fbc_reg.reg = GETREG(reg_addr + IFE_WDR_FBC_REGISTER_0_OFS);
	wdr_tone_reg.reg = GETREG(reg_addr + IFE_WDR_TONE_MAPPING_REGISTER_0_OFS);

	PRINT_IQ_INFO(sfile, "                 gbal_en : %4d \r\n", ife_ctrl_reg.bit.gbal_en);
	PRINT_IQ_INFO(sfile, "   gbal_sram_shutdown_en : %4d \r\n", ife_set_reg.bit.ife_gbal_sram_shutdown_en);
	PRINT_IQ_INFO(sfile, "               filter_en : %4d \r\n", ife_ctrl_reg.bit.filter_en);
	PRINT_IQ_INFO(sfile, "            filter_range : %4d \r\n", ife_ctrl_reg.bit.filter_range);
	PRINT_IQ_INFO(sfile, "               motion_en : %4d \r\n", ife_motion_ctrl_reg.bit.ife_motion_en);
	PRINT_IQ_INFO(sfile, "   2dnr_sram_shutdown_en : %4d \r\n", ife_set_reg.bit.ife_2dnr_sram_shutdown_en);
	PRINT_IQ_INFO(sfile, "                  wdr_en : %4d \r\n", wdr_ctrl_reg0.bit.ife_wdr_en);
	PRINT_IQ_INFO(sfile, "       wdr_subimg_out_en : %4d \r\n", wdr_ctrl_reg0.bit.ife_wdr_subimg_out_en);
	PRINT_IQ_INFO(sfile, "        wdr_histogram_en : %4d \r\n", wdr_ctrl_reg0.bit.ife_wdr_histogram_en);
	PRINT_IQ_INFO(sfile, "           wdr_tcurve_en : %4d \r\n", wdr_ctrl_reg0.bit.ife_wdr_tcurve_en);
	PRINT_IQ_INFO(sfile, "         wdr_bypass_mode : %4d \r\n", wdr_ctrl_reg0.bit.ife_wdr_bypass_mode);
	PRINT_IQ_INFO(sfile, "        wdr_dithering_en : %4d \r\n", wdr_subimg_reg.bit.ife_wdr_dithering_en);
	PRINT_IQ_INFO(sfile, "     wdr_outbld_table_en : %4d \r\n", wdr_ctrl_reg1.bit.ife_wdr_outbld_table_en);
	PRINT_IQ_INFO(sfile, "         wdr_gainctrl_en : %4d \r\n", wdr_ctrl_reg1.bit.ife_wdr_gainctrl_en);
	PRINT_IQ_INFO(sfile, "              wdr_fbc_en : %4d \r\n", wdr_fbc_reg.bit.ife_wdr_fbc_en);
	PRINT_IQ_INFO(sfile, "                wdr_mode : %4d \r\n", wdr_tone_reg.bit.ife_wdr_mode);
	PRINT_IQ_INFO(sfile, "    wdr_sram_shutdown_en : %4d \r\n", ife_set_reg.bit.ife_wdr_sram_shutdown_en);

	// IPE
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "    IPE REGISTERS \r\n");
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	reg_addr = ipe_reg_addr;
	ipe_mode_reg.reg = GETREG(reg_addr + IPE_MODE_REGISTER_1_OFS);
	ipe_power_save_reg.reg = GETREG(reg_addr + IPE_POWER_SAVE_MODE_REGISTER_OFS);
	defog_airlight_reg.reg = GETREG(reg_addr + DEFOG_AIRLIGHT_REGISTER_1_OFS);
	pfr_control_reg.reg = GETREG(reg_addr + PURPLE_FRINGE_REDUCTION_REGISTER0_OFS);
	ipe_mode_reg2.reg = GETREG(reg_addr + IPE_MODE_REGISTER_2_OFS);

	PRINT_IQ_INFO(sfile, "               rgblpf_en : %4d \r\n", ipe_mode_reg.bit.rgblpf_en);
	PRINT_IQ_INFO(sfile, "                  pfr_en : %4d \r\n", ipe_mode_reg.bit.pfr_en);
	PRINT_IQ_INFO(sfile, "                 pfr_set : %4d %4d %4d %4d \r\n", pfr_control_reg.bit.pfr_set0_en, pfr_control_reg.bit.pfr_set1_en, pfr_control_reg.bit.pfr_set2_en, pfr_control_reg.bit.pfr_set3_en);
	PRINT_IQ_INFO(sfile, "    power_save_mode_edge : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_edge);
	PRINT_IQ_INFO(sfile, "power_save_mode_edge_ker : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_edge_ker);
	PRINT_IQ_INFO(sfile, "      edge_ref_motion_en : %4d \r\n", ipe_mode_reg.bit.edge_ref_motion_en);
	PRINT_IQ_INFO(sfile, "                  cfa_en : %4d \r\n", ipe_mode_reg2.bit.cfa_en);
	PRINT_IQ_INFO(sfile, "       cfa_subimg_out_en : %4d \r\n", ipe_mode_reg2.bit.cfa_subimg_out_en);
	PRINT_IQ_INFO(sfile, "              cfa_fcs_en : %4d \r\n", ipe_mode_reg2.bit.cfa_fcs_en);
	PRINT_IQ_INFO(sfile, "          cfa_sram_sd_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_cfa_sram_sd_en);
	PRINT_IQ_INFO(sfile, "                 cc3d_en : %4d \r\n", ipe_mode_reg.bit.cc3d_en);
	PRINT_IQ_INFO(sfile, " power_save_mode_cc3d_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_cc3d_sram_sd_en);
	PRINT_IQ_INFO(sfile, "         cc3d_sram_sd_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_cc3d_sram_sd_en);
	PRINT_IQ_INFO(sfile, "                   cr_en : %4d \r\n", ipe_mode_reg.bit.cr_en);
	PRINT_IQ_INFO(sfile, "               ycurve_en : %4d \r\n", ipe_mode_reg.bit.ycurve_en);
	PRINT_IQ_INFO(sfile, "       ycurve_sram_sd_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_ycurve_sram_sd_en);
	PRINT_IQ_INFO(sfile, "                gamma_en : %4d \r\n", ipe_mode_reg.bit.rgbgamma_en);
	PRINT_IQ_INFO(sfile, "power_save_mode_gamma_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_gamma_sram_sd_en);
	PRINT_IQ_INFO(sfile, "        gamma_sram_sd_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_gamma_sram_sd_en);
	PRINT_IQ_INFO(sfile, "                defog_en : %4d \r\n", ipe_mode_reg.bit.defog_en);
	PRINT_IQ_INFO(sfile, "            defog_air_en : %4d \r\n", defog_airlight_reg.bit.defog_air_en);
	PRINT_IQ_INFO(sfile, "     defog_subimg_out_en : %4d \r\n", ipe_mode_reg.bit.defog_subimg_out_en);
	PRINT_IQ_INFO(sfile, "power_save_mode_defog_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_defog_sram_sd_en);
	PRINT_IQ_INFO(sfile, "        defog_sram_sd_en : %4d \r\n", ipe_power_save_reg.bit.ipe_power_save_mode_defog_sram_sd_en);
	PRINT_IQ_INFO(sfile, "                  lce_en : %4d \r\n", ipe_mode_reg.bit.lce_en);
	PRINT_IQ_INFO(sfile, "                  cst_en : %4d \r\n", ipe_mode_reg.bit.cst_en);
	PRINT_IQ_INFO(sfile, "           cstprotect_en : %4d \r\n", ipe_mode_reg.bit.cstprotect_en);
	PRINT_IQ_INFO(sfile, "                 ctrl_en : %4d \r\n", ipe_mode_reg.bit.ctrl_en);
	PRINT_IQ_INFO(sfile, "                 cadj_en : %4d \r\n", ipe_mode_reg.bit.cadj_en);

	// IME
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	PRINT_IQ_INFO(sfile, "    IME REGISTERS \r\n");
	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");
	reg_addr = ime_reg_addr;
	ime_func_ctrl_reg.reg = GETREG(reg_addr + IME_FUNCTION_CONTROL_REGISTER0_OFS);
	local_chroma_reg.reg = GETREG(reg_addr + LOCAL_CHROMA_ADAPTATION_REGISTER0_OFS);
	tmnr_ctrl_reg.reg = GETREG(reg_addr + IME_TMNR_CONTROL_REGISTER0_OFS);

	PRINT_IQ_INFO(sfile, "                  lca_en : %4d \r\n", ime_func_ctrl_reg.bit.ime_lca_en);
	PRINT_IQ_INFO(sfile, "      lca_sr_rf_ker_size : %4d \r\n", local_chroma_reg.bit.lca_sr_rf_ker_size);
	PRINT_IQ_INFO(sfile, "      lca_mr_rf_ker_size : %4d \r\n", local_chroma_reg.bit.lca_mr_rf_ker_size);
	PRINT_IQ_INFO(sfile, "      lca_edge_detect_en : %4d \r\n", local_chroma_reg.bit.lca_edge_detect_en);
	PRINT_IQ_INFO(sfile, "       lca_ref_center_en : %4d \r\n", local_chroma_reg.bit.lca_ref_center_en);
	PRINT_IQ_INFO(sfile, "    lca_motion_status_en : %4d \r\n", local_chroma_reg.bit.lca_motion_status_en);
	PRINT_IQ_INFO(sfile, "                 3dnr_en : %4d \r\n", ime_func_ctrl_reg.bit.ime_3dnr_en);
	PRINT_IQ_INFO(sfile, "         3dnr_ref_out_en : %4d \r\n", ime_func_ctrl_reg.bit.ime_3dnr_ref_out_en);
	PRINT_IQ_INFO(sfile, "         3dnr_nr_tf0_y_e : %4d \r\n", tmnr_ctrl_reg.bit.ime_3dnr_nr_tf0_y_e);
	PRINT_IQ_INFO(sfile, "         3dnr_nr_tf0_c_e : %4d \r\n", tmnr_ctrl_reg.bit.ime_3dnr_nr_tf0_c_e);
	PRINT_IQ_INFO(sfile, "         3dnr_mot_est_en : %4d \r\n", tmnr_ctrl_reg.bit.ime_3dnr_mot_est_en);
	PRINT_IQ_INFO(sfile, "             post_shp_en : %4d \r\n", ime_func_ctrl_reg.bit.ime_post_shp_en);

	PRINT_IQ_INFO(sfile, "---------------------------------------------------------------------------------------------- \r\n");

	IOUNMAP(sie1_reg_addr);
	IOUNMAP(pre_reg_addr);
...
#endif
{
	...
	PRINT_IQ_INFO(sfile, "         dpc :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.dpc);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         ecs :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.ecs);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "          va :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.va);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "     outlier :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.outlier);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         bnr :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.bnr);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "        gbal :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.gbal);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "          nr :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.nr);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.wdr);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "      rgblpf :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.rgblpf);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         pfr :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.pfr);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "        edge :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.edge);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         cfa :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cfa);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "          cc :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cc);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "     y_curve :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.y_curve);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "       gamma :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.gamma);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "   defog_lce :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.defog_lce);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         cst :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cst);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "        cstp :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cstp);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "       cctrl :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cctrl);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "     cadjust :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.cadjust);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "         lca :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.lca);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param._3dnr);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "post_sharpen :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %4d", iq_info->low_power_param.post_sharpen);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
}
...
#endif
{
	...
	PRINT_IQ_INFO(sfile, "    pre_f_en :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", ((iq_info->ob_mode_manual.pre_f_enable) ? 1 : 0));
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	for (i = 0; i < IQ_OB_LEN; i++) {
		PRINT_IQ_INFO(sfile, "pre_f_value%d :", i);
		for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
			iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
			if (iq_flow_get_id_valid(id)) {
				PRINT_IQ_INFO(sfile, "      %4d", iq_info->ob_mode_manual.pre_f_value[i]);
			}
		}
		PRINT_IQ_INFO(sfile, "\r\n");
	}

	PRINT_IQ_INFO(sfile, "      pre_en :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", ((iq_info->ob_mode_manual.pre_enable) ? 1 : 0));
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	for (i = 0; i < IQ_OB_LEN; i++) {
		PRINT_IQ_INFO(sfile, "  pre_value%d :", i);
		for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
			iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
			if (iq_flow_get_id_valid(id)) {
				PRINT_IQ_INFO(sfile, "      %4d", iq_info->ob_mode_manual.pre_value[i]);
			}
		}
		PRINT_IQ_INFO(sfile, "\r\n");
	}
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", ((iq_info->dg_mode_manual.pre_enable) ? 1 : 0));
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");

	PRINT_IQ_INFO(sfile, "   pre_value :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, "      %4d", iq_info->dg_mode_manual.pre_value);
		}
	}
	PRINT_IQ_INFO(sfile, "\r\n");
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		...
		if (iq_flow_get_id_valid(id)) {
			if (iq_info->dg_mode_manual.manual_mode == IQ_DG_OFF) {
				...
				PRINT_IQ_INFO(sfile, "       IFE");
			} else if (iq_info->dg_mode_manual.manual_mode == IQ_DG_AI) {
				PRINT_IQ_INFO(sfile, "        AI");
			} else {
				PRINT_IQ_INFO(sfile, "    UNKNOW");
			}
	...
	PRINT_IQ_INFO(sfile, "  sie_dg_max :");
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		iq_info = (IQALG_INFO *)((ULONG)iq_private + iq_info_buffer_size * iq_flow_get_info_map(id));
		if (iq_flow_get_id_valid(id)) {
			PRINT_IQ_INFO(sfile, " %9d", iq_info->dg_mode_manual.sie_dg_max);
		}
	}
	...
	for (id = IQ_ID_1; id < IQ_ID_MAX_NUM; id++) {
		...
		if (iq_flow_get_id_valid(id)) {
			if (iq_info->cg_mode_manual.manual_mode == IQ_CG_SIE) {
				PRINT_IQ_INFO(sfile, "       SIE");
			} else if (iq_info->cg_mode_manual.manual_mode == IQ_CG_PRE_F) {
				PRINT_IQ_INFO(sfile, "     PRE_F");
			} else if (iq_info->cg_mode_manual.manual_mode == IQ_CG_PRE) {
				PRINT_IQ_INFO(sfile, "       PRE");
