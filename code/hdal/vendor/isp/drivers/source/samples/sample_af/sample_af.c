...
static void af_api_reserve(ULONG addr);
static void af_api_get_test(ULONG addr);
static void af_api_set_test(ULONG addr);
...
typedef void (*sample_af_fp)(ULONG addr);
...
INT32 sample_af_ioctl(INT32 cmd, ULONG arg, ULONG *buf_addr)
{
	INT32 ret = 0;
	...
	if ((_IOC_DIR(cmd) == _IOC_READ) || (_IOC_DIR(cmd) == (_IOC_READ | _IOC_WRITE))) {
		...
		af_get_tab[item]((ULONG)buf_addr);
		...
		af_set_tab[item]((ULONG)buf_addr);
	} else {
...
static void sample_af_alg_process(UINT32 id)
{
	...
	INT32 set_fcs_pos;
	ISP_IPE_VA_RSLT *va_rslt = NULL;

	va_rslt = isp_dev_get_ipe_va(id);

	if (va_rslt == NULL) {
		printk("isp_dev_get_ipe_va(id = %d) NULL!\n", id);
		return;
	} else {
		set_fcs_pos = 60;
