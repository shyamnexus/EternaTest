...
MAIN(argc, argv)
{
	...
	if (argc == 2) {
		sscanf(argv[1], "%hu", &sobel_output_format);		
		printf("sobel output format is set to %d (0:8bit packed, 1:16bit packed)\n", sobel_output_format);
	}

	//ive_load_input();
#if defined(__FREERTOS)
	snprintf(in_file1, 64, "A:\\input\\y_only_512x376.bin");
#else
    snprintf(in_file1, 64, "//mnt//sd//input//y_only_512x376.bin");
#endif
	...
	if (sobel_output_format == 0) {
		dst_img_info1.enDataType = LIB_IVE_DATA_TYPE_S16C1;
		dst_img_info1.u32Stride  = 512*2;
	} else if (sobel_output_format == 1) {
		dst_img_info1.enDataType = LIB_IVE_DATA_TYPE_S32C1;
		dst_img_info1.u32Stride  = 512*4;
	}
	output_size1 = dst_img_info1.u32Stride * dst_img_info1.u32Height;
	
	sobel_ctrl.s8Mask[0] = 0;
	sobel_ctrl.s8Mask[1] = 0;
	sobel_ctrl.s8Mask[2] = 0;
	sobel_ctrl.s8Mask[3] = 0;
	sobel_ctrl.s8Mask[4] = 0;
	
	sobel_ctrl.s8Mask[5] = 0;
	sobel_ctrl.s8Mask[6] = -1;
	sobel_ctrl.s8Mask[7] = 0;
	sobel_ctrl.s8Mask[8] = 1;
	sobel_ctrl.s8Mask[9] = 0;
	
	sobel_ctrl.s8Mask[10] = 0;
	sobel_ctrl.s8Mask[11] = -2;
	sobel_ctrl.s8Mask[12] = 0;
	sobel_ctrl.s8Mask[13] = 2;
	sobel_ctrl.s8Mask[14] = 0;
	
	sobel_ctrl.s8Mask[15] = 0;
	sobel_ctrl.s8Mask[16] = -1;
	sobel_ctrl.s8Mask[17] = 0;
	sobel_ctrl.s8Mask[18] = 1;
	sobel_ctrl.s8Mask[19] = 0;
	
	sobel_ctrl.s8Mask[20] = 0;
	sobel_ctrl.s8Mask[21] = 0;
	sobel_ctrl.s8Mask[22] = 0;
	sobel_ctrl.s8Mask[23] = 0;
	sobel_ctrl.s8Mask[24] = 0;
	//sobel_ctrl.alpha_blending_en = 1;
	//sobel_ctrl.alpha_blending_factor = 128;
	sobel_ctrl.gradient_out_format = sobel_output_format;

	hd_common_mem_flush_cache((VOID *)src_img_info.u64Va, src_img_info.u32Width*src_img_info.u32Height);
    vos_perf_mark(&hw_tick_begin);
    ret = NVT_IVE_Sobel(&handle, &src_img_info, &dst_img_info1, &sobel_ctrl, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Sobel time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));   
    if (ret != HD_OK) {
		printf("err:NVT_IVE_Sobel error %d\r\n",ret);
		goto exit;
	}
    if(!instant) {
	...
	hd_common_mem_flush_cache((VOID *)dst_img_info1.u64Va, output_size1);
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\output\\sobel_output.bin");
#else
    if (sobel_output_format == 0) snprintf(out_file1, 64, "//mnt//sd//output//sobel_output8pack.bin");
    if (sobel_output_format == 1) snprintf(out_file1, 64, "//mnt//sd//output//sobel_output16pack.bin");
#endif
	...
	file_size = fwrite((const void *)share_mem[1].va,1,output_size1,fd);
