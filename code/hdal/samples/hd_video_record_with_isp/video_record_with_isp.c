...
// GC5603 uses RAW10, others use RAW12
#define RESOLUTION_SET  4 //0: 2M(IMX290), 1:5M(OS05A) 2: 2M (OS02K10) 3: 2M (AR0237IR) 4: 5M (GC5603)
...
static void *encode_thread(void *arg)
{
    ...
    snprintf(file_path_main, 64, "/mnt/sd/dump_bs_main_isp_%lu_%lu_%lu_%lu_%lu_%lu.dat", (unsigned long)p_stream0->enc_type,(unsigned long)g_shdr,(unsigned long)g_wdr,(unsigned long)g_3dnr,(unsigned long)g_colornr,(unsigned long)g_defog);
	//----- open output files -----
...
MAIN(argc, argv)
{
...
#if MD_INFO_ENABLE
		g_3dnr = 1;
		printf("md enable, force enable 3DNR! (%d)\r\n", g_3dnr);
#else
		g_3dnr = atoi(argv[4]);
#endif
		printf("3DNR %d\r\n", g_3dnr);
	...
	// Start RTSP server for live streaming
	printf("Starting RTSP server (nvtrtspd_ipc)...\n");
	system("nvtrtspd_ipc &");
	printf("RTSP stream available at rtsp://<device-ip>:554/live/main\n");
	...
	while (1) {
		...
		if (key == 'z') {
			...
			if ((ret = set_cap_param(stream[0].cap_path, &stream[0].cap_dim, 0)) != HD_OK)
				break;
			if ((ret = hd_videocap_open(HD_VIDEOCAP_IN(SEN1_VCAP_ID, 0), HD_VIDEOCAP_OUT(SEN1_VCAP_ID, 0), &(stream[0].cap_path))) != HD_OK)
				break;
			hd_videocap_start(stream[0].cap_path);
	...
	system("killall nvtrtspd_ipc 2>/dev/null");
