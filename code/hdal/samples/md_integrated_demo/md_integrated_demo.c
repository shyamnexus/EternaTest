...
static HD_RESULT setup_md(DEMO_CONTEXT* ctx)
{
    ...
    memset(&ctx->cross_ctrl, 0, sizeof(ctx->cross_ctrl));
    ctx->cross_ctrl.enCrossMode = LIB_MD_CROSS_LINE;
    ctx->cross_ctrl.u32OverlapTh = 10;
    ctx->cross_ctrl.u8PreObjNum = 0;  // No previous objects initially
    // Define a horizontal line in the middle of the 160x120 frame
    // Points p1(0,55) p2(159,55) p3(159,65) p4(0,65) define a corridor
    ctx->cross_ctrl.u32X1 = 0;   ctx->cross_ctrl.u32Y1 = 55;
    ctx->cross_ctrl.u32X2 = 159; ctx->cross_ctrl.u32Y2 = 55;
    ctx->cross_ctrl.u32X3 = 159; ctx->cross_ctrl.u32Y3 = 65;
    ctx->cross_ctrl.u32X4 = 0;   ctx->cross_ctrl.u32Y4 = 65;
...
static void* md_process_thread(void* arg)
{
    ...
    while (g_running && g_md_running) {
        ...
        if (md_ret == LIB_MD_OK && ctx->obj_info.u32ObjNum > 0) {
            ...
            // Run Cross-Line Detection based on detected objects
            // NOTE: libmd uses 1-based indexing for object arrays (SDK convention)
            ctx->cross_ctrl.u8ObjNum = (UINT8)ctx->obj_info.u32ObjNum;
            for (UINT32 i = 1; i <= ctx->obj_info.u32ObjNum && i < LIB_MD_MAX_OBJ_NUM; i++) {
                ctx->cross_objs[i].u32XStart = ctx->obj_info.stObjRst[i].u32XStart;
                ctx->cross_objs[i].u32YStart = ctx->obj_info.stObjRst[i].u32YStart;
                ctx->cross_objs[i].u32XEnd   = ctx->obj_info.stObjRst[i].u32XEnd;
                ctx->cross_objs[i].u32YEnd   = ctx->obj_info.stObjRst[i].u32YEnd;
                ctx->cross_objs[i].u8Valid   = 1;  // Mark as valid object
                ctx->cross_objs[i].u8Track   = 0;  // Reset tracking state
            }

            md_ret = NVT_MD_CrossLine(ctx->cross_objs, ctx->cross_results, &ctx->cross_ctrl);
            if (md_ret == LIB_MD_OK) {
                for (UINT32 i = 1; i <= ctx->cross_ctrl.u8ObjNum; i++) {
                    if (ctx->cross_results[i] == LIB_MD_CROSS_IN_TO_OUT || 
                        ...
                        g_stats.crossline_events++;
                        pthread_mutex_unlock(&g_stats_mutex);
                        printf(COLOR_CYAN "         [CROSS] Line crossed by Object %d! (%s)\n" COLOR_RESET, 
                               i, ctx->cross_results[i] == LIB_MD_CROSS_IN_TO_OUT ? "IN->OUT" : "OUT->IN");
                    }
                }
                
                // Save current objects as previous for next frame tracking
                ctx->cross_ctrl.u8PreObjNum = ctx->cross_ctrl.u8ObjNum;
                for (UINT32 i = 1; i <= ctx->cross_ctrl.u8ObjNum && i < LIB_MD_MAX_OBJ_NUM; i++) {
                    ctx->cross_ctrl.stPreObj[i] = ctx->cross_objs[i];
                }
            }
        } else {
            // No objects detected - clear previous tracking state
            ctx->cross_ctrl.u8PreObjNum = 0;
        }
