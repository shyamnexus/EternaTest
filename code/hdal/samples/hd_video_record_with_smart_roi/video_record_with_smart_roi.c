...
//CODEC FUNC
#define AQ_EN			0 //suggest enable AQ with smart ROI
#define RRC_EN			0
///////////////////////////////////////////////////////////////////////////////
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
...

		{
			...
			32 ~ 83 =>  qp:   0 ~  51
			*/
			smart_roi.fg_str[VENDOR_VIDEOENC_SMART_ROI_CLASS0] = 26; //dqp: -6
			smart_roi.fg_str[VENDOR_VIDEOENC_SMART_ROI_CLASS1] = 27; //dqp: -5
			smart_roi.fg_str[VENDOR_VIDEOENC_SMART_ROI_CLASS2] = 28; //dqp: -4
			smart_roi.fg_str[VENDOR_VIDEOENC_SMART_ROI_CLASS3] = 0;
			smart_roi.fg_str[VENDOR_VIDEOENC_SMART_ROI_CLASS4] = 0;
			smart_roi.mode = 0;
		...
		aq.p_str = 1;
		aq.depth = 2;
		ret = hd_videoenc_set(video_enc_path, HD_VIDEOENC_PARAM_OUT_AQ, &aq);
		if (ret != HD_OK) {
			printf("set vendor_enc_param_in = %d\r\n", ret);
			return ret;
		}

		// the parameter for Row RC //
...
static void *bbox_thread(void *arg)
{
	...
	while (p_stream0->bbox_exit == 0) {
		...
		// bbox info //
		smart_bbox.bbox[0].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_TOP_LEFT].x = 0;
		smart_bbox.bbox[0].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_TOP_LEFT].y = 0;
		smart_bbox.bbox[0].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].x = 200;
		smart_bbox.bbox[0].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].y = 200;
		smart_bbox.bbox[0].class_id = VENDOR_VIDEOENC_SMART_ROI_CLASS0;
		...
		smart_bbox.bbox[1].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].x = 300;
		smart_bbox.bbox[1].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].y = 300;
		smart_bbox.bbox[1].class_id = VENDOR_VIDEOENC_SMART_ROI_CLASS1;

		smart_bbox.bbox[2].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_TOP_LEFT].x = 200;
		smart_bbox.bbox[2].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_TOP_LEFT].y = 200;
		smart_bbox.bbox[2].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].x = 400;
		smart_bbox.bbox[2].positions[VENDOR_VIDEOENC_SMART_BBOX_POS_BOTTOM_RIGHT].y = 400;
		smart_bbox.bbox[2].class_id = VENDOR_VIDEOENC_SMART_ROI_CLASS2;
...
MAIN(argc, argv)
{
	...
	// destroy bbox thread
	pthread_join(stream[0].bbox_thread_id, NULL);
