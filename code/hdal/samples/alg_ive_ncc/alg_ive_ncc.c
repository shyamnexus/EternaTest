...
UINT64 test_pattern_golden[3] = {0x19e264565,0x19f5618ad,0x19e200d28};
...
MAIN(argc, argv)
{
    ...
    if (file_size == 0) {
		printf("load y_only_512x376.bin : %s\r\n", in_file1);
		return 0;
    }
	
	//ive_load_input2;
#if defined(__FREERTOS)
	snprintf(in_file2, 64, "A:\\IVEP\\input\\general_filter_y.bin");
#else
	snprintf(in_file2, 64, "//mnt//sd//IVEP//input//general_filter_y.bin");
#endif
	file_size = ive_load_file(in_file2, share_mem[1].va);
	if (file_size == 0) {
		printf("load general_filter_y.bin : %s\r\n", in_file2);
		return 0;
	}
    ...
    ncc_result.u64Pa 	 = share_mem[2].addr;
    ncc_result.u64Va 	 = share_mem[2].va;
	...
	lib_ive_ret = NVT_IVE_NCC(&handle, &src_img_info1, &src_img_info2, &ncc_result, instant);
	vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_NCC time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
	if (lib_ive_ret != LIB_IVE_OK) {
		printf("err:NVT_IVE_NCC error %d\r\n",lib_ive_ret);
		goto exit;
	}
    if(!instant) {
	...
	if(test_pattern_golden[0] != *(UINT64*)ncc_result.u64Va || 
	   test_pattern_golden[1] != *((UINT64*)ncc_result.u64Va+1)  || 
	   test_pattern_golden[2] != *((UINT64*)ncc_result.u64Va+2)){
	   	printf("NCC Compare failed.\r\n");
		printf("NCC Numerator result = %lu, golden result = %lu\r\n", *(UINT64*)ncc_result.u64Va,test_pattern_golden[0]);
		printf("NCC QuadSum1 result = %lu, golden result = %lu\r\n", *((UINT64*)ncc_result.u64Va+1),test_pattern_golden[1]);
		printf("NCC QuadSum2 result = %lu, golden result = %lu\r\n", *((UINT64*)ncc_result.u64Va+2),test_pattern_golden[2]);
	}else{
	   	printf("NCC Compare OK.\r\n");		
	}
