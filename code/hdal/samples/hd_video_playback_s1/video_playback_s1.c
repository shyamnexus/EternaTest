...
static HD_RESULT set_dec_param(HD_PATH_ID video_dec_path, UINT32 dec_type)
{
	...
	if (dec_type == HD_CODEC_TYPE_H264 || dec_type == HD_CODEC_TYPE_H265) {
		...
		parse_h26x_desc(dec_type, (UINTPTR)&desc_buf, bs_size, &is_desc);
		if (is_desc) {
...
static void *feed_bs_thread(void *arg)
{
	...
	while (p_stream0->dec_exit == 0) {
		...
		usleep(30000); // sleep 30 ms, to avoid pull_out write card too slow
	}
...
{
	printf("======version: %s\n",SAMPLE_VERSION);
	printf("Usage: ./hd_video_playback_s1 <dec_type> <suspend_sec>\n");
	printf("  <dec_type>   : 0 or empty (H265), 1(H264), 2(MJPG)\n");
...
EXAMFUNC_ENTRY(hd_video_playback, argc, argv)
{
	...
	UINT32 dec_type = 0, suspend_sec = 10;
	UINT32 out_type = 0;
	HD_DIM main_dim = {0};
	char rtc_cmd[128] = {0};
...
again:
	...
	if(key == 's'){
		...
		printf("hd_video_playback_s1 is terminating...\n");
	}
