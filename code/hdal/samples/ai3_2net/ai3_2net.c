...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(ai3_2net, argc, argv)
...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	if (p_net->net_cfg.p_share_model == NULL ) {
		...
		// setup p_cur_net
		p_cur_net = (NET_PROC*)p_net->net_cfg.p_share_model;
	}

	// load label
	ret = _load_label((UINTPTR)p_net->out_class_labels, LABEL_LEN, p_cur_net->net_cfg.label_filename);
	if (ret != HD_OK) {
	...
	// query model info for WORKBUF/RONLYBUF size , then alloc WORKBUF/RONLYBUF
	{
		...
		model_info.model_buf.pa   = p_cur_net->proc_mem.pa;
		model_info.model_buf.va   = p_cur_net->proc_mem.va;
		model_info.model_buf.size = p_cur_net->proc_mem.size;
#if DBG_OUT_DUMP
		...
		printf("net_path(%u) model_info get => workbuf size = %d, ronlybuf size = %d\r\n", net_path, model_info.proc_mem.buf[AI3_PROC_BUF_WORKBUF].size, model_info.proc_mem.buf[AI3_PROC_BUF_RONLYBUF].size);
	...
	// call open()
	{
		...
		proc_cfg.model_buf.pa   = p_cur_net->proc_mem.pa;
		proc_cfg.model_buf.va   = p_cur_net->proc_mem.va;
		proc_cfg.model_buf.size = p_cur_net->proc_mem.size;
...
static HD_RESULT network_dump_out_buf(NET_PATH_ID net_path, VENDOR_AI3_BUF *p_outbuf)
{
	...
	printf("[%d] Classification Results:\r\n", p_net->proc_id);
	for (i = 0; i < parm.top_n; i++) {
		printf("[%d] %d. no=%d, label=%s, score=%f\r\n", p_net->proc_id, i + 1, parm.classes[i].no,
...
MAIN(argc, argv)
{
	...
	if(argc < 2){
		printf("usage : ai3_2net (share_model)\n");
...
*/
	...
	// init mem
	{
		...
		for (idx = 0; idx < NN_RUN_NET_NUM; idx++) {
			// config common pool
			p_net_cfg = &net_cfg[idx];
			if (idx == 0 || p_net_cfg->p_share_model == NULL) {
				network_mem_config(stream[idx].net_path, &mem_cfg, p_net_cfg);
			}
		}
	}
#if defined(_BSP_NS02201_) || defined(_BSP_NS02302_)
	...
	for (i = 0; i < NN_RUN_NET_NUM; i++) {
		...
		allocate_buf_by_out_path_list(stream[i].net_path);
	}
...
exit:
	...
	for (i = NN_RUN_NET_NUM-1; i >=0; i--) { // stream[0] should be last to close, because other stream share model with stream[0]
