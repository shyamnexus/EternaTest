...
/* Type Definitions                                                                                                  */
/*-------------------------------------------------------------------------------------------------------------------*/
#define IVE_ERODE_USE_DDR           DDR_ID0
#define IVE_ERODE_VERBOSE           0
#define IVE_ERODE_MAX_STR_LEN       256
...
static HD_RESULT mem_init(IVE_SRC_IMAGE_S *img_src, IVE_SRC_IMAGE_S *img_dst)
{
	...
	mem_cfg.pool_info[0].ddr_id = IVE_ERODE_USE_DDR;
...
static HD_RESULT mem_alloc(MEM_RANGE *inout_buf, IVE_SRC_IMAGE_S *img_src, IVE_SRC_IMAGE_S *img_dst)
{
    ...
    for(i=0; i < 2; i++) {
        blk = hd_common_mem_get_block(HD_COMMON_MEM_USER_BLK, inout_buf[i].size, IVE_ERODE_USE_DDR);
...
{
    printf("Usage: alg_ive_erode out_file in_file datatype width height\n\r");
...
/* Main Function                                                                                   */
/*  Excution example:                                                                              */
/*   alg_ive_erode ./output/lena_316x316_U8_t_erode.bin ./input/lena_316x316_U8_t.bin 0 316 316    */
/*-------------------------------------------------------------------------------------------------*/
MAIN(argc, argv)
{
    UINT32 fsize;
    ...
    CHAR output_fname[256] = "//mnt//sd//output/lena_316x316_U8_t_erode.bin";
    ...
    if(argc>1 && argc!=6) {
        ...
        if(strlen(argv[1]) <= IVE_ERODE_MAX_STR_LEN) {
            strncpy(output_fname, argv[1], IVE_ERODE_MAX_STR_LEN);
        } else {
            printf("filename (%s) exceeds allowed length (%d)\r\n", argv[1], IVE_ERODE_MAX_STR_LEN);
            return 0;
        }
        output_fname[IVE_ERODE_MAX_STR_LEN-1] = '\0';

        if(strlen(argv[2]) <= IVE_ERODE_MAX_STR_LEN) {
            strncpy(input_info.filename, argv[2], IVE_ERODE_MAX_STR_LEN);
        } else {
            printf("filename (%s) exceeds allowed length (%d)\r\n", argv[2], IVE_ERODE_MAX_STR_LEN);
            return 0;
        }
        input_info.filename[IVE_ERODE_MAX_STR_LEN-1] = '\0';
    ...
    ret = NVT_IVE_Erode(&handle, &img_src, &img_dst, &kernel, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Erode time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end)); 
    // output result
    if(ret==HD_OK) {
