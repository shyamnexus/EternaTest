...
#if EXT_CODEC
static HD_RESULT set_out_cfg(HD_PATH_ID *p_audio_out_ctrl, HD_AUDIO_SR sample_rate, UINT32 i2s_num)
#else
...
static HD_RESULT open_module(AUDIO_OUTONLY *p_outonly)
{
	HD_RESULT ret;
#if EXT_CODEC
	ret = set_out_cfg(&p_outonly->out_ctrl, p_outonly->sample_rate_max, p_outonly->out_i2s_num);
#else
...
static void *playback_thread(void *arg)
{
	...
	/* read test pattern */
	snprintf(filename, sizeof(filename), "/mnt/sd/audio_bs_%d_%d_%d_pcm.dat", AUDOUT_BIT, AUDOUT_MODE, p_out_only->sample_rate);
	...
	snprintf(filename, sizeof(filename), "/mnt/sd/audio_bs_%d_%d_%d_pcm.len", AUDOUT_BIT, AUDOUT_MODE, p_out_only->sample_rate);
...
{
	UINT32 count = 0;
	AUDIO_OUTONLY *p_stream0 = (AUDIO_OUTONLY *)arg;
...
EXAMFUNC_ENTRY(hd_audio_output_only, argc, argv)
{
...
#if EXT_CODEC
	if (argc > 1){
		outonly.out_i2s_num = atoi(argv[1]);// 1: I2S, 0: EAC
	...
	while (1) {
		...
		if (key == 'n') {
			...
			printf("test signal\r\n");
			ret = pthread_create(&signal_thread_id, NULL, signal_thread, (void *)&outonly);
			if (ret < 0) {
