/**
 * @file ai_crowd_counting_demo.c
 * @brief Crowd Counting Demo - Estimates number of people/objects in scene
 * Usage: ./ai_crowd_counting_demo [duration]
...
static volatile int g_max_count = 0;
static volatile int g_avg_count = 0;
static volatile int g_samples = 0;
...
static HD_RESULT setup_counting(DEMO_CTX* ctx) {
    ...
    // Lower threshold to detect smaller objects (people)
    ctx->objdet_ctrl.u32ObjThres = 30;
    ctx->objdet_ctrl.enInRange = LIB_MD_IN_RANGE_0_1;
    ctx->objdet_ctrl.pstPtStk = ctx->obj_pt_stack;
    
    printf("[CROWD] Crowd counting initialized (max %d objects)\n", LIB_MD_MAX_OBJ_NUM);
    return HD_OK;
}
...
static void* counting_thread(void* arg) {
    ...
    while (g_running) {
        ...
        if (NVT_MD_MDBC(&src, &dst, &ctx->mdbc_ctrl) == LIB_MD_OK) {
            ...
            int count = ctx->obj_info.u32ObjNum;
            total_count += count;
            g_samples++;
            
            if (count > g_max_count) g_max_count = count;
            g_avg_count = total_count / g_samples;
            
            if (frame_cnt % 30 == 0) {
                printf("[CROWD] Frame %d: Current=%d, Max=%d, Avg=%d\n", 
                       frame_cnt, count, g_max_count, g_avg_count);
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    printf("\n=== NT98538 Crowd Counting Demo ===\n");
    printf("Features: Real-time object/person counting with statistics\n\n");
    ...
    printf("\n[DONE] Crowd Counting Statistics:\n");
    printf("  Max count: %d\n", g_max_count);
    printf("  Avg count: %d\n", g_avg_count);
    printf("  Samples: %d\n", g_samples);
    return 0;
