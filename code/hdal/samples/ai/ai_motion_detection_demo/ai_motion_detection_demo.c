/**
 * @file ai_motion_detection_demo.c
 * @brief Motion Detection Demo - MDBC, GlobalAlarm, SubAlarm, ObjectDet, CrossLine
 * Usage: ./ai_motion_detection_demo [duration] [sensitivity(0-3)]
...
typedef struct {
    ...
    int mdbc_initialized, sensitivity, duration;
} DEMO_CTX;

static void signal_handler(int sig) { g_running = 0; }

static HD_RESULT alloc_mem(MEM_BLOCK* b, UINT32 sz, const char* n) {
    b->size = sz;
...
static HD_RESULT setup_md(DEMO_CTX* ctx) {
    if (vendor_md_init() != HD_OK) return HD_ERR_NG;
    if (alloc_mem(&ctx->md_temp, MD_BUF_SIZE * 48, "MD_TEMP") != HD_OK) return HD_ERR_NG;
    if (alloc_mem(&ctx->md_dst, MD_BUF_SIZE, "MD_DST") != HD_OK) return HD_ERR_NG;
    ...
    ctx->galarm_ctrl.u8AlarmThres = 30;
    ctx->subalarm_ctrl.u8SubNum = NUM_SUB_REGIONS;
    ctx->subalarm_ctrl.pstSubParam = ctx->sub_params;
    for (int i = 0; i < NUM_SUB_REGIONS; i++) {
        ctx->sub_params[i].enSubRegion = 1;
        ctx->sub_params[i].u8SubAlarmThres = 20;
    }
    ctx->sub_params[0].u32XStart=0; ctx->sub_params[0].u32YStart=0; ctx->sub_params[0].u32XEnd=MD_WIDTH/2-1; ctx->sub_params[0].u32YEnd=MD_HEIGHT/2-1;
    ctx->sub_params[1].u32XStart=MD_WIDTH/2; ctx->sub_params[1].u32YStart=0; ctx->sub_params[1].u32XEnd=MD_WIDTH-1; ctx->sub_params[1].u32YEnd=MD_HEIGHT/2-1;
    ctx->sub_params[2].u32XStart=0; ctx->sub_params[2].u32YStart=MD_HEIGHT/2; ctx->sub_params[2].u32XEnd=MD_WIDTH/2-1; ctx->sub_params[2].u32YEnd=MD_HEIGHT-1;
    ctx->sub_params[3].u32XStart=MD_WIDTH/2; ctx->sub_params[3].u32YStart=MD_HEIGHT/2; ctx->sub_params[3].u32XEnd=MD_WIDTH-1; ctx->sub_params[3].u32YEnd=MD_HEIGHT-1;
    
    ctx->objdet_ctrl.u32ObjThres = 50;
    ctx->objdet_ctrl.enInRange = LIB_MD_IN_RANGE_0_1;
    ctx->objdet_ctrl.pstPtStk = ctx->obj_pt_stack;
    printf("[MD] Initialized, sensitivity=%d\n", ctx->sensitivity);
    return HD_OK;
}
...
static void* md_thread(void* arg) {
    ...
    while (g_running) {
        ...
        MD_SRC_IMAGE_S src = {0};
        src.u64Pa = frame.phy_addr[0];
        src.u64Va = (uintptr_t)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, frame.phy_addr[0], MD_BUF_SIZE*2);
        src.u32Width = MD_WIDTH; src.u32Height = MD_HEIGHT; src.u32Stride = MD_WIDTH;
        
        MD_DST_IMAGE_S dst = {0};
        dst.u64Pa = ctx->md_dst.pa; dst.u64Va = ctx->md_dst.va;
        dst.u32Width = MD_WIDTH; dst.u32Height = MD_HEIGHT; dst.u32Stride = MD_WIDTH;
        ...
        if (NVT_MD_MDBC(&src, &dst, &ctx->mdbc_ctrl) == LIB_MD_OK) {
            UINT8 galarm = 0, sub[4] = {0};
            NVT_MD_GlobalAlarm(&dst, &galarm, &ctx->galarm_ctrl);
            NVT_MD_SubAlarm(&dst, sub, &ctx->subalarm_ctrl);
            memset(&ctx->obj_info, 0, sizeof(ctx->obj_info));
            NVT_MD_ObjDet(&dst, &ctx->obj_info, &ctx->objdet_ctrl);
            if (galarm || ctx->obj_info.u32ObjNum > 0) {
                g_events++;
                if (frame_cnt % 30 == 0) printf("[MD] Frame %d: Alarm=%d Objs=%u\n", frame_cnt, galarm, ctx->obj_info.u32ObjNum);
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    ctx.sensitivity = argc > 2 ? atoi(argv[2]) : 2;
    
    printf("\n=== NT98538 Motion Detection Demo ===\n");
    ...
    // Setup video pipeline (simplified)
    ...
    HD_VIDEOCAP_IN ci = {.sen_mode = HD_VIDEOCAP_SEN_MODE_AUTO, .frc = HD_VIDEO_FRC_RATIO(MAIN_FPS, 1), .dim = {CAP_WIDTH, CAP_HEIGHT}, .pxlfmt = HD_VIDEO_PXLFMT_RAW10, .out_frame_num = HD_VIDEOCAP_SEN_FRAME_NUM_1};
    ...
    pthread_create(&tid, NULL, md_thread, &ctx);
