/**
 * @file ai_line_crossing_demo.c
 * @brief Line Crossing Detection Demo - Detects objects crossing virtual lines
 * Usage: ./ai_line_crossing_demo [duration]
...
static volatile int g_crossings = 0;
static volatile int g_in_count = 0, g_out_count = 0;
...
static HD_RESULT setup_crossline(DEMO_CTX* ctx) {
    ...
    // Cross-line: horizontal line at center of frame
    memset(&ctx->cross_ctrl, 0, sizeof(ctx->cross_ctrl));
    ctx->cross_ctrl.enCrossMode = LIB_MD_CROSS_LINE;
    ctx->cross_ctrl.u32OverlapTh = 50;
    ctx->cross_ctrl.u32X1 = 0;
    ctx->cross_ctrl.u32Y1 = MD_HEIGHT / 2;
    ctx->cross_ctrl.u32X2 = MD_WIDTH - 1;
    ctx->cross_ctrl.u32Y2 = MD_HEIGHT / 2;
    ctx->cross_ctrl.u32X3 = MD_WIDTH - 1;
    ctx->cross_ctrl.u32Y3 = MD_HEIGHT / 2 + 10;
    ctx->cross_ctrl.u32X4 = 0;
    ctx->cross_ctrl.u32Y4 = MD_HEIGHT / 2 + 10;
    
    printf("[LINE] Cross-line detection initialized (horizontal center)\n");
    return HD_OK;
}

static void* crossline_thread(void* arg) {
    ...
    while (g_running) {
        ...
        if (NVT_MD_MDBC(&src, &dst, &ctx->mdbc_ctrl) == LIB_MD_OK) {
            ...
            if (ctx->obj_info.u32ObjNum > 0) {
                // Convert object results for cross-line detection
                ...
                ctx->cross_ctrl.u8ObjNum = ctx->obj_info.u32ObjNum;
                
                LIB_MD_CROSS_RST cross_result = LIB_MD_CROSS_NONE;
                NVT_MD_CrossLine(obj_list, &cross_result, &ctx->cross_ctrl);
                
                if (cross_result != LIB_MD_CROSS_NONE) {
                    g_crossings++;
                    if (cross_result == LIB_MD_CROSS_IN_TO_OUT) {
                        g_out_count++;
                        printf("[LINE] Frame %d: Crossing IN->OUT (Total: %d out, %d in)\n", frame_cnt, g_out_count, g_in_count);
                    } else if (cross_result == LIB_MD_CROSS_OUT_TO_IN) {
                        g_in_count++;
                        printf("[LINE] Frame %d: Crossing OUT->IN (Total: %d out, %d in)\n", frame_cnt, g_out_count, g_in_count);
                    }
                }
                
                // Update previous objects for next frame
                memcpy(ctx->cross_ctrl.stPreObj, obj_list, sizeof(obj_list));
                ctx->cross_ctrl.u8PreObjNum = ctx->obj_info.u32ObjNum;
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    printf("\n=== NT98538 Line Crossing Detection Demo ===\n");
    printf("Features: Cross-line detection with in/out counting\n\n");
    ...
    pthread_create(&tid, NULL, crossline_thread, &ctx);
    ...
    printf("\n[DONE] Total crossings: %d (IN: %d, OUT: %d)\n", g_crossings, g_in_count, g_out_count);
    return 0;
}
