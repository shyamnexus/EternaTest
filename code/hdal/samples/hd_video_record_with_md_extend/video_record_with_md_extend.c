...
// GC5603 sensor supports RAW10 format (not RAW12)
...
static void *encode_thread(void *arg)
{
	...
	snprintf(file_path_main, 32, "/mnt/sd/dump_bs_main_%lu_%lu.dat", (unsigned long)p_stream0->enc_type, (unsigned long)p_stream0->ext_mode);
	snprintf(file_path_sub, 32, "/mnt/sd/dump_bs_sub_%lu_%lu.dat", (unsigned long)p_stream0->enc_type, (unsigned long)p_stream0->ext_mode);
	snprintf(file_path_sub2, 32, "/mnt/sd/dump_bs_sub2_%lu_%lu.dat", (unsigned long)p_stream0->enc_type, (unsigned long)p_stream0->ext_mode);
	//----- open output files -----
...
MAIN(argc, argv)
{
	...
	ret = set_enc_cfg(stream[0].enc_path, &stream[0].enc_max_dim, main_br);
	if (ret != HD_OK) {
	...
	ret = set_enc_cfg(stream[1].enc_path, &stream[1].enc_max_dim, sub_br);
	if (ret != HD_OK) {
		printf("set enc-cfg fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_enc_param(stream[1].enc_path, &stream[1].enc_dim, enc_type, sub_br);
	if (ret != HD_OK) {
		printf("set enc fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_enc_cfg(stream[2].enc_path, &stream[2].enc_max_dim, sub2_br);
	if (ret != HD_OK) {
		printf("set enc-cfg fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_enc_param(stream[2].enc_path, &stream[2].enc_dim, enc_type, sub2_br);
	if (ret != HD_OK) {
		printf("set enc fail=%d\n", ret);
		goto exit;
	}
