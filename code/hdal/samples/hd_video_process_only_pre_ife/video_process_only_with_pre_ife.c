/**
	...
	@file video_process_only_with_pipe.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_process_only_with_pipe, argc, argv)
...
#define NVX2_ALIGN    32

#define NVX2_LOFF(w)   (w % NVX2_ALIGN) ? (((w+NVX2_ALIGN)>>5<<5)*YUV_COMPRESS_RATIO/100):(w*YUV_COMPRESS_RATIO/100)
...
#define VDO_FMT_NAME       "yuv420"
#define VDO_FMT            HD_VIDEO_PXLFMT_YUV420
#else
#define VDO_SIZE_W         1920
#define VDO_SIZE_H         1080
#define VDO_FRM_NUM        1     // pattern file frame number
#define VDO_BLK_SIZE_IN    (VDO_RAW_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, VDO_FMT_IN))
#define VDO_FMT_IN_NAME    "raw"
#define VDO_FMT_IN         HD_VIDEO_PXLFMT_RAW12
#endif

#define VDO_BLK_SIZE_RAW   (DBGINFO_BUFSIZE()+VDO_RAW_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, HD_VIDEO_PXLFMT_RAW12))
#define VDO_FMT_NAME_RAW   "raw12"

#define VDO_BLK_SIZE_YUV   (DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, HD_VIDEO_PXLFMT_YUV420))
#define VDO_FMT_NAME_YUV   "yuv420"
...
///////////////////////////////////////////////////////////////////////////////
static HD_VIDEOPROC_PIPE    g_proc_pipe = HD_VIDEOPROC_PIPE_PRE;
static HD_VIDEOPROC_PIPE    g_proc_pipe2 = HD_VIDEOPROC_PIPE_RAWALL_LITE;
static HD_VIDEO_PXLFMT      g_vdo_fmt_in = HD_VIDEO_PXLFMT_RAW12;
static HD_VIDEO_PXLFMT      g_vdo_fmt_in2 = HD_VIDEO_PXLFMT_RAW12;
static HD_VIDEO_PXLFMT      g_vdo_fmt_out = HD_VIDEO_PXLFMT_YUV420;
static UINT32               g_vdo_size_out = VDO_BLK_SIZE_YUV;
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[0].blk_size = VDO_BLK_SIZE_IN;
	mem_cfg.pool_info[0].blk_cnt = 6;
	mem_cfg.pool_info[0].ddr_id = VDO_DDR_ID;
	mem_cfg.pool_info[1].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[1].blk_size = g_vdo_size_out;
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim, HD_CTRL_ID ctrl, HD_VIDEOPROC_PIPE pipe, HD_VIDEO_PXLFMT input_pxlfmt)
{
	HD_RESULT ret = HD_OK;
	...
	if (p_max_dim != NULL ) {
		...
		if (pipe == HD_VIDEOPROC_PIPE_PRE) {
			video_cfg_param.ctrl_max.func |= HD_VIDEOPROC_FUNC_BNR;
			video_cfg_param.ctrl_max.func |= HD_VIDEOPROC_FUNC_BNR_STA;
			video_cfg_param.ctrl_max.func |= HD_VIDEOPROC_FUNC_AIDED;
		}
		video_cfg_param.in_max.func = 0;
		video_cfg_param.in_max.dim.w = p_max_dim->w;
		video_cfg_param.in_max.dim.h = p_max_dim->h;
		video_cfg_param.in_max.pxlfmt = input_pxlfmt;
	...
	if (pipe == HD_VIDEOPROC_PIPE_PRE) {
		video_ctrl_param.func |= HD_VIDEOPROC_FUNC_BNR;
		video_ctrl_param.func |= HD_VIDEOPROC_FUNC_BNR_STA;
		video_ctrl_param.func |= HD_VIDEOPROC_FUNC_AIDED;
	}
...
static HD_RESULT open_module(VIDEO_PROCESS *p_stream, HD_DIM* p_proc_max_dim)
{
	...
	ret = set_proc_cfg(&p_stream->proc_ctrl, p_proc_max_dim, HD_VIDEOPROC_0_CTRL, g_proc_pipe, g_vdo_fmt_in);
	if (ret != HD_OK) {
	...
	// set videoproc config
	ret = set_proc_cfg(&p_stream->proc_ctrl2, p_proc_max_dim, HD_VIDEOPROC_1_CTRL, g_proc_pipe2, g_vdo_fmt_in2);
	if (ret != HD_OK) {
		printf("set proc-cfg fail=%d\n", ret);
		return HD_ERR_NG;
	}
...
static BOOL check_test_pattern(void)
{
	...
	sprintf(filename, "video_frm_%d_%d_%d_%s.dat", VDO_SIZE_W, VDO_SIZE_H, VDO_FRM_NUM, VDO_FMT_IN_NAME);
...
static void *feed_vdo_thread(void *arg)
{
	VIDEO_PROCESS* p_stream0 = (VIDEO_PROCESS *)arg;
	HD_RESULT ret = HD_OK;
	int i;
	HD_COMMON_MEM_DDR_ID ddr_id = VDO_DDR_ID;
	UINT32 blk_size = VDO_BLK_SIZE_IN;
	UINT32 vdo_size = VDO_BLK_SIZE_IN;
	...
	//------ [2] open input files ------
	sprintf(filepath_yuv_main, "/mnt/sd/video_frm_%d_%d_%d_%s.dat", VDO_SIZE_W, VDO_SIZE_H, VDO_FRM_NUM, VDO_FMT_IN_NAME);
	if ((f_in_main = fopen(filepath_yuv_main, "rb")) == NULL) {
		printf("open file (%s) fail !!....\r\nPlease copy test pattern to SD Card !!\r\n\r\n", filepath_yuv_main);
		return 0;
	}
    setvbuf(f_in_main, NULL, _IONBF, 0);
get_blk:
		//--- Get memory ---
		...
		//--- Read YUV from file ---
		read_len = fread((void *)va_yuv_main[blkidx], 1, vdo_size, f_in_main);
		if (read_len != vdo_size) {
			if (feof(f_in_main)) {
		...
		hd_common_mem_flush_cache((void *)va_yuv_main[blkidx], vdo_size);
...
static void *aquire_vdo_thread(void *arg)
{
	VIDEO_PROCESS* p_stream0 = (VIDEO_PROCESS *)arg;
	HD_RESULT ret = HD_OK;
	HD_VIDEO_FRAME video_frame = {0};
	...
	while (p_stream0->proc_exit == 0) {
		...
		if (1) {
			ret = hd_videoproc_pull_out_buf(p_stream0->proc_path2, &video_frame, p_stream0->wait_ms); // -1 = blocking mode, 0 = non-blocking mode, >0 = blocking-timeout mode
			if (ret != HD_OK) {
			...
			if (!p_stream0->do_snap) {
				ret = hd_videoproc_release_out_buf(p_stream0->proc_path2, &video_frame);
				if (ret != HD_OK) {
			...
			vdo_size = g_vdo_size_out;

			// mmap for frame buffer (just mmap one time only, calculate offset to virtual address later)
			vir_addr_main = (UINTPTR)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, phy_addr_main, vdo_size);
			if (vir_addr_main == 0) {
				printf("mmap error !!\r\n\r\n");
				goto skip;
			}
			...
			{
				snprintf(file_path_main, 32, "/mnt/sd/dump_frm_%s_%u.dat", VDO_FMT_NAME_YUV, p_stream0->shot_count);
			...
			ret = hd_common_mem_munmap((void *)vir_addr_main, vdo_size);
			if (ret != HD_OK) {
				printf("mnumap error !!\r\n\r\n");
				goto skip;
			}
...
//		stream[0].show_ret = 1; //show pull result
	...
	ret = set_proc_param(stream[0].proc_path, &stream[0].proc_dim, HD_VIDEO_PXLFMT_RAW12 | HD_VIDEO_PIX_RGGB_R);
	if (ret != HD_OK) {
	...
	// set videoproc parameter (main)
	ret = set_proc_param(stream[0].proc_path2, &stream[0].proc_dim, g_vdo_fmt_out);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	...
	ret = pthread_create(&stream[0].feed_thread_id, NULL, feed_vdo_thread, (void *)stream);
	if (ret < 0) {
	...
	ret = pthread_create(&stream[0].aquire_thread_id, NULL, aquire_vdo_thread, (void *)stream);
	if (ret < 0) {
	...
	while (1) {
		...
		if (key == 'm') {
			system("echo dump_hdl_all > /proc/kflow_ipp/cmd");
			system("echo dump_t > /proc/kflow_ipp/cmd");
		}
        ...
        if (key == 'n') {
    		...
    		printf("test signal\r\n");
    		ret = pthread_create(&signal_thread_id, NULL, signal_thread, (void *)stream);
    		if (ret < 0) {
	...
	hd_videoproc_stop(stream[0].proc_path2); // it could force abort pull_out with blocking mode
