/**
	@brief Sample code of video liveview with 4 windows.\n

	@file video_liveview_with_4_win.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_4_win, argc, argv)
...
UINT32 EXTEND_PATH[4] = {HD_VIDEOPROC_0_OUT_5, HD_VIDEOPROC_0_OUT_6, HD_VIDEOPROC_0_OUT_7, HD_VIDEOPROC_0_OUT_8}; //out 5~15 is extend path

#define PUSH_SIZE_W         352
#define PUSH_SIZE_H         288
#define PUSH_FMT            HD_VIDEO_PXLFMT_YUV420
#define MAX_PUSH_BLK_CNT    30
#define PUSH_BLK_SIZE       (DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(PUSH_SIZE_W, PUSH_SIZE_H, PUSH_FMT))
#define MAX_FILE_NAME		64
#define PATTEN_PATH         "/mnt/sd/video_frm_352_288_30_yuv420.dat"
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[0].blk_cnt = 12;
	...
	mem_cfg.pool_info[2].blk_cnt = 12;
	mem_cfg.pool_info[2].ddr_id = DDR_ID0;

	mem_cfg.pool_info[3].type     = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[3].blk_size = PUSH_BLK_SIZE;
	mem_cfg.pool_info[3].blk_cnt  = MAX_PUSH_BLK_CNT;
...
static HD_RESULT set_out_param(HD_PATH_ID video_out_path, HD_DIM *p_dim, HD_URECT* p_rect, HD_WIN_LAYER layer)
{
	HD_RESULT ret = HD_OK;
...
static int set_mask_param(HD_PATH_ID mask_path, int idx)
{
	...
	attr.position[0].x = 0 + idx*500;
	attr.position[0].y = 0 + idx*200;
	attr.position[1].x = 100 + idx*500;
	attr.position[1].y = 0 + idx*200;
	attr.position[2].x = 100 + idx*500;
	attr.position[2].y = 120 + idx*200;
	attr.position[3].x = 0 + idx*500;
	attr.position[3].y = 120 + idx*200;
	...
	if(idx == 0){
		...
		attr.color = 0x0000FF00;
	}else if(idx == 2){
		attr.color = 0x000000FF;
	}else{
		attr.color = 0x00FFFFFF;
	}
...
static HD_RESULT open_module(VIDEO_LIVEVIEW *p_stream, HD_DIM* p_proc_max_dim)
{
	...
	if ((ret = hd_videocap_open(HD_VIDEOCAP_IN(SEN1_VCAP_ID, 0), HD_VIDEOCAP_OUT(SEN1_VCAP_ID, 0), &p_stream->cap_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, SOURCE_PATH, &(p_stream->proc_path[0]))) != HD_OK)
		return ret;
...
static HD_RESULT open_module_2(VIDEO_LIVEVIEW *p_stream, HD_DIM* p_proc_max_dim, UINT32 out_type)
{
	...
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, EXTEND_PATH[0], &p_stream->proc_path[0])) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, EXTEND_PATH[1], &p_stream->proc_path[1])) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, EXTEND_PATH[2], &p_stream->proc_path[2])) != HD_OK)
		return ret;
    if(g_ide & VOUT_IDE1) {
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,0), HD_VIDEOOUT_OUT(0, 0), &p_stream->out_path[0])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,1), HD_VIDEOOUT_OUT(0, 1), &p_stream->out_path[1])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,2), HD_VIDEOOUT_OUT(0, 2), &p_stream->out_path[2])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,3), HD_VIDEOOUT_OUT(0, 3), &p_stream->out_path[3])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,0), HD_MASK_0, &p_stream->mask_path[0])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,1), HD_MASK_1, &p_stream->mask_path[1])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,2), HD_MASK_2, &p_stream->mask_path[2])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,3), HD_MASK_3, &p_stream->mask_path[3])) != HD_OK)
			return ret;
    }
    if(g_ide & VOUT_IDE2) {
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,0), HD_VIDEOOUT_OUT(1, 0), &p_stream->out_path[0])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,1), HD_VIDEOOUT_OUT(1, 1), &p_stream->out_path[1])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,2), HD_VIDEOOUT_OUT(1, 2), &p_stream->out_path[2])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,3), HD_VIDEOOUT_OUT(1, 3), &p_stream->out_path[3])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,0), HD_MASK_0, &p_stream->mask_path[0])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,1), HD_MASK_1, &p_stream->mask_path[1])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,2), HD_MASK_2, &p_stream->mask_path[2])) != HD_OK)
			return ret;
		if((ret = hd_videoout_open(HD_VIDEOOUT_IN(1,3), HD_MASK_3, &p_stream->mask_path[3])) != HD_OK)
			return ret;
    }
	return HD_OK;
}
...
static HD_RESULT close_module_2(VIDEO_LIVEVIEW *p_stream)
{
	...
	if ((ret = hd_videoproc_close(p_stream->proc_path[1])) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path[2])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->out_path[0])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->out_path[1])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->out_path[2])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->out_path[3])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->mask_path[0])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->mask_path[1])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->mask_path[2])) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(p_stream->mask_path[3])) != HD_OK)
		return ret;
	return HD_OK;
}
...
static void *aquire_yuv_thread(void *arg)
{
	...
	while (p_stream0->proc_exit == 0) {
		...
		ret = hd_videoproc_pull_out_buf(p_stream0->proc_path[0], &video_frame, -1); // -1 = blocking mode, 0 = non-blocking mode, >0 = blocking-timeout mode
		if (ret != HD_OK) {
		...
		//printf("release_out() ....\r\n");
		ret = hd_videoproc_release_out_buf(p_stream0->proc_path[0], &video_frame);
		if (ret != HD_OK) {
...
{
	static HD_COMMON_MEM_VB_BLK blk_registered[MAX_PUSH_BLK_CNT] = {0};
	INT32 i;
	for (i=0; i< MAX_PUSH_BLK_CNT; i++) {
...
{
    VIDEO_LIVEVIEW *p_vdoout_only = (VIDEO_LIVEVIEW *)arg;
	HD_RESULT ret = HD_OK;
	int i;
	HD_COMMON_MEM_DDR_ID ddr_id = DDR_ID0;
	UINT32 blk_size = PUSH_BLK_SIZE;
	UINT32 yuv_size = VDO_YUV_BUFSIZE(PUSH_SIZE_W, PUSH_SIZE_H, PUSH_FMT);
	char filepath_yuv_main[64];
	FILE *f_in_main = 0;
    UINT32 readbyte = 0 ;
	uintptr_t pa_yuv_main[MAX_PUSH_BLK_CNT] = {0};
	uintptr_t va_yuv_main[MAX_PUSH_BLK_CNT] = {0};
	INT32  blkidx;
	HD_COMMON_MEM_VB_BLK blk;

	//------ [2] open input files ------
   	sprintf(filepath_yuv_main, PATTEN_PATH);
	...
	//------ [3] feed yuv ------
	while (p_vdoout_only->proc_exit == 0) {
		...
		if (va_yuv_main[blkidx] == 0) { // if NOT mmap yet, mmap it
			va_yuv_main[blkidx] = (uintptr_t)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, pa_yuv_main[blkidx], blk_size); // Get virtual addr
...
#if 0
		memset((void *)va_yuv_main[blkidx], 0, yuv_size);
#else
		//--- Read YUV from file ---
		...
		//--- push_in ---
		{
			...
			video_frame.pxlfmt      = PUSH_FMT;
			video_frame.dim.w       = PUSH_SIZE_W;
			video_frame.dim.h       = PUSH_SIZE_H;
			video_frame.count       = 0;
			video_frame.timestamp   = hd_gettime_us();
			video_frame.loff[0]     = PUSH_SIZE_W; // Y
			video_frame.loff[1]     = PUSH_SIZE_W; // UV
			video_frame.phy_addr[0] = pa_yuv_main[blkidx];                          // Y
			video_frame.phy_addr[1] = pa_yuv_main[blkidx] + PUSH_SIZE_W*PUSH_SIZE_H;  // UV pack
			video_frame.blk         = blk;
			video_frame.pw[0]       = PUSH_SIZE_W;
			video_frame.ph[0]       = PUSH_SIZE_H;

			ret = hd_videoout_push_in_buf(p_vdoout_only->out_path[3], &video_frame, NULL, 0); // only support non-blocking mode now
			if (ret != HD_OK) {
...
MAIN(argc, argv)
{
	...
	ret = set_proc_param(stream[0].proc_path[0], &main_dim, HD_VIDEO_DIR_NONE);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}

	sub_dim.w = out_dim.w/2;
	sub_dim.h = out_dim.h/2;

	// set videoproc parameter (sub)
	ret = set_proc_param_extend(stream[1].proc_path[0], SOURCE_PATH, &sub_crop, &sub_dim, sub_dir);
	if (ret != HD_OK) {
		printf("set ext proc0 fail=%d\n", ret);
		goto exit;
	}
	ret = set_proc_param_extend(stream[1].proc_path[1], SOURCE_PATH, &sub_crop, &sub_dim, sub_dir);
	if (ret != HD_OK) {
		printf("set ext proc1 fail=%d\n", ret);
		goto exit;
	}
	ret = set_proc_param_extend(stream[1].proc_path[2], SOURCE_PATH, &sub_crop, &sub_dim, sub_dir);
	if (ret != HD_OK) {
		printf("set ext proc2 fail=%d\n", ret);
		goto exit;
	}
    ...
    sub_win.x = (0%2)*(out_dim.w/2);
    sub_win.y = (0/2)*(out_dim.h/2);
    sub_win.w = out_dim.w/2;
    sub_win.h = out_dim.h/2;

//	if(sub_win.w != PUSH_SIZE_W || sub_win.h != PUSH_SIZE_H){
//		printf("sub win w(%d) h(%d) != push w(%d) h(%d)\n", sub_win.w, sub_win.h, PUSH_SIZE_W, PUSH_SIZE_H);
//		goto exit;
//	}

	ret = set_out_param(stream[1].out_path[0], &stream[1].out_dim, &sub_win, HD_LAYER2);
	if (ret != HD_OK) {
		printf("set out fail=%d\n", ret);
		goto exit;
	}
    sub_win.x = (1%2)*(out_dim.w/2);
    sub_win.y = (1/2)*(out_dim.h/2);
	ret = set_out_param(stream[1].out_path[1], &stream[1].out_dim, &sub_win, HD_LAYER7);
	if (ret != HD_OK) {
		printf("set out fail=%d\n", ret);
		goto exit;
	}
    sub_win.x = (2%2)*(out_dim.w/2);
    sub_win.y = (2/2)*(out_dim.h/2);
	ret = set_out_param(stream[1].out_path[2], &stream[1].out_dim, &sub_win, HD_LAYER2);
	if (ret != HD_OK) {
		printf("set out fail=%d\n", ret);
		goto exit;
	}
    sub_win.x = (3%2)*(out_dim.w/2);
    sub_win.y = (3/2)*(out_dim.h/2);
//	ret = set_out_param(stream[1].out_path[3], &stream[1].out_dim, &sub_win, HD_LAYER2);
push_dim.w = 352;
push_dim.h = 288;
sub_win.w = 352;
sub_win.h = 288;
ret = set_out_param(stream[1].out_path[3], &push_dim, &sub_win, HD_LAYER2);
	if (ret != HD_OK) {
		printf("set out fail=%d\n", ret);
		goto exit;
	}
	...
	if(g_ide & VOUT_IDE1) {
		hd_videoproc_bind(EXTEND_PATH[0], HD_VIDEOOUT_IN(0,0));
		hd_videoproc_bind(EXTEND_PATH[1], HD_VIDEOOUT_IN(0,1));
		hd_videoproc_bind(EXTEND_PATH[2], HD_VIDEOOUT_IN(0,2));
	}
	if(g_ide & VOUT_IDE2) {
		hd_videoproc_bind(EXTEND_PATH[0], HD_VIDEOOUT_IN(1,0));
		hd_videoproc_bind(EXTEND_PATH[1], HD_VIDEOOUT_IN(1,1));
		hd_videoproc_bind(EXTEND_PATH[2], HD_VIDEOOUT_IN(1,2));
	}

	//setup mask parameter
	if(set_mask_param(stream[1].mask_path[0], 0)){
		printf("fail to set vo mask\r\n");
		goto exit;
	}
	ret = hd_videoout_start(stream[1].mask_path[0]);
	if (ret != HD_OK) {
		printf("fail to start vo mask\n");
		goto exit;
	}
	if(set_mask_param(stream[1].mask_path[1], 1)){
		printf("fail to set vo mask\r\n");
		goto exit;
	}
	ret = hd_videoout_start(stream[1].mask_path[1]);
	if (ret != HD_OK) {
		printf("fail to start vo mask\n");
		goto exit;
	}
	if(set_mask_param(stream[1].mask_path[2], 2)){
		printf("fail to set vo mask\r\n");
		goto exit;
	}
	ret = hd_videoout_start(stream[1].mask_path[2]);
	if (ret != HD_OK) {
		printf("fail to start vo mask\n");
		goto exit;
	}
	if(set_mask_param(stream[1].mask_path[3], 3)){
		printf("fail to set vo mask\r\n");
		goto exit;
	}
	ret = hd_videoout_start(stream[1].mask_path[3]);
	if (ret != HD_OK) {
		printf("fail to start vo mask\n");
		goto exit;
	}
	...
	ret = hd_videoproc_start(stream[0].proc_path[0]);
	if (ret) {
		printf("main to start main vproc");
		goto exit;
	}

	// start video_liveview modules (sub)
	ret = hd_videoproc_start(stream[1].proc_path[0]);
	if (ret) {
		printf("main to start sub vproc0");
		goto exit;
	}
	ret = hd_videoproc_start(stream[1].proc_path[1]);
	if (ret) {
		printf("main to start sub vproc1");
		goto exit;
	}
	ret = hd_videoproc_start(stream[1].proc_path[2]);
	if (ret) {
		printf("main to start sub vproc2");
		goto exit;
	}

	// just wait ae/awb stable for auto-test, if don't care, user can remove it
	sleep(1);
	hd_videoout_start(stream[1].out_path[0]);
	hd_videoout_start(stream[1].out_path[1]);
	hd_videoout_start(stream[1].out_path[2]);
	hd_videoout_start(stream[1].out_path[3]);

	ret = pthread_create(&yuv_thread_id, NULL, feed_yuv_thread, (void *)&(stream[1]));
	if (ret < 0) {
		printf("create encode thread failed");
		return -1;
	}

	video_out_param.rect.x = (1%2)*(out_dim.w/2);
	video_out_param.rect.y = (1/2)*(out_dim.h/2);
	video_out_param.rect.w = (out_dim.w/2);
	video_out_param.rect.h = (out_dim.h/2);

	// query user key
	printf("Enter q to exit\n");
	printf("Enter z to stop window 1\n");
	printf("Enter x to star window 1\n");
	printf("Enter c to stop window 4\n");
	printf("Enter v to star window 4\n");
	printf("Enter b to stop all windows\n");
	printf("Enter n to star all windows\n");
	printf("Enter m to move window 2 to center\n");
	printf("Enter s to scale window 2\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
			// let aquire_thread stop loop and exit
			stream[0].proc_exit = 1;
			stream[1].proc_exit = 1;
            usleep(300000); //delay 300 ms
			// quit program
			break;
		}else if(key == 'z'){
			hd_videoout_stop(stream[1].out_path[0]);
		}else if(key == 'x'){
			hd_videoout_start(stream[1].out_path[0]);
		}else if(key == 'c'){
			hd_videoout_stop(stream[1].out_path[3]);
		}else if(key == 'v'){
			hd_videoout_start(stream[1].out_path[3]);
		}else if(key == 'b'){
			hd_videoout_stop(stream[1].out_path[0]);
			hd_videoout_stop(stream[1].out_path[1]);
			hd_videoout_stop(stream[1].out_path[2]);
			hd_videoout_stop(stream[1].out_path[3]);
		}else if(key == 'n'){
			hd_videoout_start(stream[1].out_path[0]);
			hd_videoout_start(stream[1].out_path[1]);
			...
			if(pingpong){
				...
				video_out_param.rect.x = (out_dim.w/4);
				video_out_param.rect.y = (out_dim.h/4);
			}
			video_out_param.layer = HD_LAYER7;
			ret =  hd_videoout_set(stream[1].out_path[1], HD_VIDEOOUT_PARAM_IN_WIN_ATTR, &video_out_param);
			...
			if(pingpong){
				...
				video_out_param.rect.w = (out_dim.w/4);
				video_out_param.rect.h = (out_dim.h/4);
			}
			video_out_param.visible = 1;
			video_out_param.layer = HD_LAYER7;
			ret =  hd_videoout_set(stream[1].out_path[1], HD_VIDEOOUT_PARAM_IN_WIN_ATTR, &video_out_param);
	...
	hd_videoproc_stop(stream[1].proc_path[2]);
	hd_videoout_stop(stream[1].out_path[0]);
	hd_videoout_stop(stream[1].out_path[1]);
	hd_videoout_stop(stream[1].out_path[2]);
	hd_videoout_stop(stream[1].out_path[3]);
	...
	hd_videoproc_unbind(EXTEND_PATH[0]);
	hd_videoproc_unbind(EXTEND_PATH[1]);
	hd_videoproc_unbind(EXTEND_PATH[2]);
