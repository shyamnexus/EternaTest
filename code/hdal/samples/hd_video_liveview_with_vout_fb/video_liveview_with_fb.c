...
void FillColor16bit(UINT16 color,uintptr_t uiBufAddr,UINT32 uiBufSize)
{
	UINT32 i=0;
...
MAIN(argc, argv)
{
	...
	if(g_ide & VOUT_IDE2) {
		ret = vendor_videoout_set(VENDOR_VIDEOOUT_ID1, VENDOR_VIDEOOUT_ITEM_FB_INIT, &fb_init);
	}
	if(ret!= HD_OK)
		return ret;
    ...
    printf("screensize %x %d %d fbp %lx \r\n",screensize,screenW,screenH,(unsigned long)fbp);

	FillColor16bit(ARBG_4444_PINK,(uintptr_t)fbp,screenW*screenH*2);
	hd_common_mem_flush_cache((void *)fbp, screenW*screenH*2);
	usleep(WAIT_TIME);
	FillColor16bit(ARBG_4444_RED,(uintptr_t)fbp,screenW*screenH*2);
	hd_common_mem_flush_cache((void *)fbp, screenW*screenH*2);
	usleep(WAIT_TIME);
	FillColor16bit(ARBG_4444_GREEN,(uintptr_t)fbp,screenW*screenH*2);
	hd_common_mem_flush_cache((void *)fbp, screenW*screenH*2);
	usleep(WAIT_TIME);

	FillColor16bit(0,(uintptr_t)fbp,screenW*screenH*2);
	FillColor16bit(ARBG_4444_RED,(uintptr_t)fbp,screenW*screenH/2);
	FillColor16bit(ARBG_4444_GREEN,(uintptr_t)(fbp+screenW*screenH/2),screenW*screenH/2);
	FillColor16bit(ARBG_4444_BLUE,(uintptr_t)(fbp+screenW*screenH),screenW*screenH/2);
	hd_common_mem_flush_cache((void *)fbp, screenW*screenH*2);
...
exit:
	...
	if(g_ide == VOUT_IDE2){
		ret = vendor_videoout_set(VENDOR_VIDEOOUT_ID1, VENDOR_VIDEOOUT_ITEM_FB_UNINIT, &fb_uninit);
	}
	if(ret!= HD_OK)
