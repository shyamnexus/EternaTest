...
int main(int argc, char *argv[])
{
	...
	while (trig) {
		...
		case 13:
			...
			if (result == HD_OK) {
				printf("id = %d, stitch id1 = 0x%x, stitch id2 = 0x%x \n", stitch.id, stitch.stitch_id.stitch_id_1, stitch.stitch_id.stitch_id_2);
			} else {
		...
		case 34:
			...
			if (result == HD_OK) {
				printf("id : %d \n", target.id);
				printf("CT : 2300, 2800, 3700, 4700, 6500, 11000 \n");
				printf("cx : %4d, %4d, %4d, %4d, %4d, %4d \n", target.target.cx[0], target.target.cx[1], target.target.cx[2], target.target.cx[3], target.target.cx[4], target.target.cx[5]);
				printf("rg : %4d, %4d, %4d, %4d, %4d, %4d \n", target.target.rg_ratio[0], target.target.rg_ratio[1], target.target.rg_ratio[2], target.target.rg_ratio[3], target.target.rg_ratio[4], target.target.rg_ratio[5]);
				printf("bg : %4d, %4d, %4d, %4d, %4d, %4d \n", target.target.bg_ratio[0], target.target.bg_ratio[1], target.target.bg_ratio[2], target.target.bg_ratio[3], target.target.bg_ratio[4], target.target.bg_ratio[5]);
			} else {
		...
		case 63:
			printf("Set isp id (0, 1, 2, 3, 4)>> \n");
			stitch.id = (UINT32)get_choose_int();

			printf("Set stitch id0 >> \n");
			stitch.stitch_id.stitch_id_1 = (UINT32)get_choose_int();

			printf("Set stitch id1 >> \n");
			stitch.stitch_id.stitch_id_1  = stitch.stitch_id.stitch_id_1 + (UINT32)(get_choose_int() << 8);

			printf("Set stitch id2 >> \n");
			stitch.stitch_id.stitch_id_1  = stitch.stitch_id.stitch_id_1 + (UINT32)(get_choose_int() << 16);

			printf("Set stitch id3 >> \n");
			stitch.stitch_id.stitch_id_1  = stitch.stitch_id.stitch_id_1 + (UINT32)(get_choose_int() << 24);

			result = vendor_isp_set_awb(AWBT_ITEM_STITCH_ID, &stitch);
			if (result == HD_OK) {
				printf("id = %d, stitch id = 0x%x \n", stitch.id, stitch.stitch_id.stitch_id_1);
			} else {
