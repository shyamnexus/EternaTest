...
#define RESOLUTION_SET    5 //0: 2M(IMX290), 1:5M(OS05A) 2: 2M (OS02K10) 3: 2M (AR0237IR) 4: (PATTERN_GEN) 5: 8M (IMX415)
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
    ...
    if(RESOLUTION_SET == 0 || RESOLUTION_SET == 1 || RESOLUTION_SET == 2 || RESOLUTION_SET == 5) {
	...
	if (g_shdr == 1) {
		...
		#if (RESOLUTION_SET == 0 || RESOLUTION_SET == 5)
			cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[2] = 2;
			cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[3] = 3;
		#elif (RESOLUTION_SET == 1)
...
MAIN(argc, argv)
{
    ...
    snprintf(file_path_main, 64, "/mnt/sd/dump_bs_main_%lu_%lu_%x_%lu_%x_%lu.dat", (unsigned long)enc_type, (unsigned long)g_capbind,(unsigned int)g_capfmt,(unsigned long)g_prcbind,(unsigned int)g_prcfmt,(unsigned long)g_prccfg);
	...
	if (enc_type == 2) { // JPEG 4K
		ret = set_enc_cfg(stream[0].enc_path, &stream[0].enc_max_dim, 20 * 1024 * 1024);
	} else {
		ret = set_enc_cfg(stream[0].enc_path, &stream[0].enc_max_dim, 8 * 1024 * 1024);
	}
	if (ret != HD_OK) {
	...
	if (enc_type == 2) { // JPEG 4K
		ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, enc_type, 20 * 1024 * 1024);
	} else { // H26x 4K
		ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, enc_type, 8 * 1024 * 1024);
	}
	if (ret != HD_OK) {
