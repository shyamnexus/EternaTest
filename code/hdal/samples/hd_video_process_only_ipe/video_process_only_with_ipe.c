...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_process_only_with_ipe, argc, argv)
...
#define YUV_BLK_SIZE_IN       VDO_RAW_BUFSIZE(VDO_SIZE_W,VDO_SIZE_H,VDO_FMT_IN)//(DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, VDO_FMT))
#define VDO_FMT_IN_NAME       "raw"
#define VDO_FMT_IN            HD_VIDEO_PXLFMT_RAW12
#endif

#define YUV_BLK_SIZE       (DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, VDO_FMT_OUT))
#define VDO_FMT_OUT_NAME       "yuv420"
#define VDO_FMT_OUT           HD_VIDEO_PXLFMT_YUV420

#define MAX_YUV_BLK_CNT    3

#define SUB_VDO_SIZE_W	960
#define SUB_VDO_SIZE_H	540
#define SUB_YUV_BLK_SIZE     (DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(SUB_VDO_SIZE_W, SUB_VDO_SIZE_H, VDO_FMT_OUT))
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[1].blk_size = SUB_YUV_BLK_SIZE;
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		video_cfg_param.pipe = HD_VIDEOPROC_PIPE_RAWCOLOR; //HD_VIDEOPROC_PIPE_RAWALL
    ...
    if(video_cfg_param.pipe == HD_VIDEOPROC_PIPE_RAWALL)
    {
        uintptr_t ai_callback= 0x12345678;
        ret = vendor_videoproc_set(video_proc_ctrl, VENDOR_VIDEOPROC_PARAM_AI_CB, &ai_callback);
    }
    ...
    if(video_cfg_param.pipe == HD_VIDEOPROC_PIPE_RAWALL)
    {
        ...
        isp_ai.path_id = 0;
        isp_ai.proc_id = 1;
        ret = vendor_videoproc_set(video_proc_ctrl, VENDOR_VIDEOPROC_PARAM_ISP_AI_START, &isp_ai);
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	if (p_dim != NULL) { //if videoproc is already binding to dest module, not require to setting this!
		...
		video_out_param.pxlfmt = VDO_FMT_OUT;
...
static HD_RESULT open_module(VIDEO_PROCESS *p_stream, HD_DIM* p_proc_max_dim)
{
	...
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_0, &p_stream->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_1, &p_stream->proc_path1)) != HD_OK)
		return ret;
...
static HD_RESULT close_module(VIDEO_PROCESS *p_stream)
{
	...
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path1)) != HD_OK)
		return ret;
	return HD_OK;
}
...
static void *feed_yuv_thread(void *arg)
{
	...
	UINT32 blk_size = YUV_BLK_SIZE_IN;
	UINT32 yuv_size = YUV_BLK_SIZE_IN;
...
get_blk:
	...
	while (p_stream0->proc_exit == 0) {
		...
		//--- push_in ---
		{
			...
			video_frame.loff[0]     = NVX2_LOFF(video_frame.pw[0]); // Y
...
static void *aquire_yuv_thread(void *arg)
{
	...
	while (p_stream0->proc_exit == 0) {
		...
		if(1) {
			ret = hd_videoproc_pull_out_buf(p_stream0->proc_path1, &video_frame, p_stream0->wait_ms); // -1 = blocking mode, 0 = non-blocking mode, >0 = blocking-timeout mode
			if (ret != HD_OK) {
				if(p_stream0->show_ret) printf("pull_out(%d) error = %d!!\r\n", p_stream0->wait_ms, ret);
        			goto skip;
			}

			if(p_stream0->show_ret) printf("pull_out(%d) ok %llu!!\r\n", p_stream0->wait_ms,video_frame.timestamp);
			if(!p_stream0->do_snap) {
				ret = hd_videoproc_release_out_buf(p_stream0->proc_path1, &video_frame);
				if (ret != HD_OK) {
			...
			snprintf(file_path_main, 64, "/mnt/sd/dump_frm_%d_%d_%s_%u.dat",SUB_VDO_SIZE_W,SUB_VDO_SIZE_H,VDO_FMT_OUT_NAME ,p_stream0->shot_count);
            ...
            setvbuf(f_out_main, NULL, _IONBF, 0);
...
MAIN(argc, argv)
{
	...
	// set videoproc parameter (sub image)
	stream[0].proc_dim1.w = SUB_VDO_SIZE_W;
	stream[0].proc_dim1.h = SUB_VDO_SIZE_H;
	ret = set_proc_param(stream[0].proc_path1, &stream[0].proc_dim1);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
    ...
    // start video_process modules (sub image)
	hd_videoproc_start(stream[0].proc_path1);
	...
	while (1) {
		...
		if (key == 'g') {
            ...
            isp_ai.path_id = 2;
            isp_ai.proc_id = 5;
            ret = vendor_videoproc_set(stream[0].proc_ctrl, VENDOR_VIDEOPROC_PARAM_ISP_AI_START, &isp_ai);
		...
		if (key == 'h') {
            ...
            isp_ai.path_id = 2;
            isp_ai.proc_id = 1;
            ret = vendor_videoproc_set(stream[0].proc_ctrl, VENDOR_VIDEOPROC_PARAM_ISP_AI_STOP, &isp_ai);
	...
	hd_videoproc_stop(stream[0].proc_path1);
