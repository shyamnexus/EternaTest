...
/* Type Definitions                                                                                                  */
/*-------------------------------------------------------------------------------------------------------------------*/
#define IVE_LBP_USE_DDR          DDR_ID0
#define IVE_LBP_MAX_STR_LEN      256
...
static HD_RESULT mem_init(IVE_SRC_IMAGE_S *img_src, IVE_SRC_IMAGE_S *img_dst)
{
	...
	mem_cfg.pool_info[0].ddr_id = IVE_LBP_USE_DDR;
...
static HD_RESULT mem_alloc(MEM_RANGE *inout_buf, IVE_SRC_IMAGE_S *img_src, IVE_SRC_IMAGE_S *img_dst)
{
    ...
    for(i=0; i < 2; i++) {
        blk = hd_common_mem_get_block(HD_COMMON_MEM_USER_BLK, inout_buf[i].size, IVE_LBP_USE_DDR);
...
/* Main Function                                                                                                     */
/*  Excution example:                                                                                                */
/*   alg_ive_lbp ./output/lena_316x316_U8_t_lbp.bin ./input/lena_316x316_U8_t.bin 0 316 316                    */
/*-------------------------------------------------------------------------------------------------------------------*/
MAIN(argc, argv)
{
    UINT32 fsize;
    ...
    CHAR output_fname[IVE_LBP_MAX_STR_LEN] = "//mnt//sd//output//lbp_out.bin";

    IVE_IMG_FILE_INFO input_info = {
        .filename = "//mnt//sd//input//y_only_512x376.bin",
        .width = 512,
        .height = 376,
    ...
    if (argc == 2) {
		sscanf(argv[1], "%hu", &lbp_mode);		
		printf("lbp_mode:%d (0:NORMAL_MODE, 1:ABS_MODE)\n", lbp_mode);
	}
    snprintf(output_fname, 64, "//mnt//sd//output//lbp_out_mode_%d.bin", lbp_mode);

    IVE_LBP_S lbp_ctrl;
    lbp_ctrl.mode = lbp_mode;
    if (lbp_mode == 0) lbp_ctrl.threshold = 0;
    else lbp_ctrl.threshold = 4;
    ...
    ret = NVT_IVE_LBP(&handle, &img_src, &img_dst, &lbp_ctrl, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_LBP time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end)); 
