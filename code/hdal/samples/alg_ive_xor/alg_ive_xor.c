...
MAIN(argc, argv)
{
	...
	lib_ive_ret = NVT_IVE_Xor(&handle, &src_img_info1, &src_img_info2, &dst_img_info, instant);
	vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_XOr time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
	if (lib_ive_ret != LIB_IVE_OK) {
		printf("err:NVT_IVE_Xor error %d\r\n",lib_ive_ret);
		goto exit;
	}
    if(!instant) {
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\IVEP\\output\\xor_output.bin");
#else
    snprintf(out_file1, 64, "//mnt//sd//IVEP//output//xor_output.bin");
#endif
