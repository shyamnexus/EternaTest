...
MAIN(argc, argv)
{
	...
	MD_CROSS_OBJ_INFO_S stObjInfo[LIB_MD_MAX_OBJ_NUM] = {0};
	LIB_MD_CROSS_RST enDst[LIB_MD_MAX_OBJ_NUM] = {0};
	MD_CROSS_CTRL_S stCrossCtrl = {0};
	...
	for(frm_id=121;frm_id<131;frm_id++){
		/// load file
#if defined(__FREERTOS)
		snprintf(in_file1, 64, "A:\\MDBCP\\DI\\obj_detect_160x120_ID%d.bin",frm_id);
#else
		snprintf(in_file1, 64, "//mnt//sd//MDBCP//DI//obj_detect_160x120_ID%d.bin",frm_id);
#endif
		...
		stObjCtrl.u32ObjThres	= 150; // Obj Area threshold
		...
		if(obj_rst_info.u32ObjNum>0){
	    	for (obj_counter = 1; obj_counter <= obj_rst_info.u32ObjNum; obj_counter++) {
				...
				stObjInfo[obj_counter].u32XStart = obj_rst_info.stObjRst[obj_counter].u32XStart;
				stObjInfo[obj_counter].u32YStart = obj_rst_info.stObjRst[obj_counter].u32YStart;
				stObjInfo[obj_counter].u32XEnd 	= obj_rst_info.stObjRst[obj_counter].u32XEnd;
				stObjInfo[obj_counter].u32YEnd 	= obj_rst_info.stObjRst[obj_counter].u32YEnd;
			}
		}

		stCrossCtrl.enCrossMode = LIB_MD_CROSS_LINE;
		stCrossCtrl.u8ObjNum 	= obj_rst_info.u32ObjNum;
		stCrossCtrl.u32OverlapTh= 10;
		stCrossCtrl.u32X1		= 0;
		stCrossCtrl.u32Y1		= 35;
		stCrossCtrl.u32X2		= 159;
		stCrossCtrl.u32Y2		= 40;
		stCrossCtrl.u32X3		= 159;
		stCrossCtrl.u32Y3		= 45;
		stCrossCtrl.u32X4		= 0;
		stCrossCtrl.u32Y4		= 40;
		
		vos_perf_mark(&hw_tick_begin);
		lib_md_ret = NVT_MD_CrossLine(stObjInfo, enDst, &stCrossCtrl);
		vos_perf_mark(&hw_tick_end);
		printf("NVT_MD_CrossLine time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
		if (lib_md_ret != LIB_MD_OK) {
			printf("err:NVT_MD_ObjDet error %d\r\n",lib_md_ret);
			goto exit;
		}
		for(i=1;i<=stCrossCtrl.u8ObjNum;i++){
			if(enDst[i]==LIB_MD_CROSS_IN_TO_OUT)
				printf("[Frame %d]Cross Line. IN_TO_OUT\r\n",frm_id);
			if(enDst[i]==LIB_MD_CROSS_OUT_TO_IN)
				printf("[Frame %d]Cross Line. OUT_TO_IN\r\n",frm_id);
		}
	}
exit:
