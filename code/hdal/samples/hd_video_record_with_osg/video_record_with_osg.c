...
// Freetype support flag (controlled by Makefile)
#ifndef USE_FREETYPE
...
static int set_enc_stamp_param(HD_PATH_ID stamp_path, UINTPTR stamp_pa, UINT32 stamp_size, unsigned short *novatek_logo)
{
...
static int set_enc_mask_param(HD_PATH_ID mask_path)
{
	...
	if(is_mosaic){
		...
		attr.position[0].x = 1200;
		attr.position[0].y = 240;
		attr.position[1].x = 1400;
		attr.position[1].y = 240;
		attr.position[2].x = 1400;
		attr.position[2].y = 440;
		attr.position[3].x = 1200;
		attr.position[3].y = 440;
		attr.type          = HD_OSG_MASK_TYPE_SOLID;
		attr.alpha         = 255;
		attr.mosaic_blk_w  = 32;
		attr.mosaic_blk_h  = 32;

		return hd_videoenc_set(mask_path, HD_VIDEOENC_PARAM_IN_MOSAIC_ATTR, &attr);
	}else{
...
static HD_RESULT open_module(VIDEO_RECORD *p_stream, HD_DIM* p_proc_max_dim)
{
	...
	if((ret = hd_videoenc_open(HD_VIDEOENC_0_IN_0, HD_STAMP_0, &p_stream->enc_stamp_path)) != HD_OK)
				return ret;
	if((ret = hd_videoenc_open(HD_VIDEOENC_0_IN_0, HD_MASK_0, &p_stream->enc_mask_path)) != HD_OK)
			return ret;
...
static HD_RESULT close_module(VIDEO_RECORD *p_stream)
{
	...
	if((ret = hd_videoenc_close(p_stream->enc_stamp_path)) != HD_OK)
		return ret;
	if((ret = hd_videoenc_close(p_stream->enc_mask_path)) != HD_OK)
			return ret;
...
MAIN(argc, argv)
{
	...
	//setup enc mask parameter
	if(set_enc_mask_param(stream[0].enc_mask_path)){
		printf("fail to set enc mask\r\n");
		goto exit;
	}

	//render enc mask
	ret = hd_videoenc_start(stream[0].enc_mask_path);
	if (ret != HD_OK) {
	...
	while (1) {
		...
		if (key == 'q' || key == 0x3) {
			...
			img.dim.w      = STAMP_WIDTH;
			img.dim.h      = STAMP_HEIGHT;
			img.p_addr     = (uintptr_t)stream[0].novatek_logo;
			if(hd_videoenc_set(stream[0].enc_stamp_path, HD_VIDEOENC_PARAM_IN_STAMP_IMG, &img) != HD_OK){
				printf("fail to set stamp image\n");
			}else{
			...
			if(stamp_enable){
				hd_videoenc_start(stream[0].enc_stamp_path);
				printf("stamp is enabled\n");
			}else{
				hd_videoenc_stop(stream[0].enc_stamp_path);
				printf("stamp is disabled\n");
			}
		}else if(key == 'c'){
			mask_enable = (mask_enable ? 0 : 1);
			if(mask_enable){
				hd_videoenc_start(stream[0].enc_mask_path);
				printf("mask is enable\n");
			}else{
				hd_videoenc_stop(stream[0].enc_mask_path);
