...
#define SEN_SEL_OS02K10  0   //2M
#define SEN_SEL_IMX415   1   //8M
...
static VIDEO_LIVEVIEW stream[2] = {0}; //0: shdr main stream, 1: shdr sub stream

static UINT32 vdo_size_w = VDO_SIZE_W_2M;
static UINT32 vdo_size_h = VDO_SIZE_H_2M;
static UINT32 shdr_mode = 0;
static UINT32 g_capfmt = CAP_OUT_FMT;
static UINT32 resolution_set = SEN_SEL_IMX415;
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[1].blk_cnt = 10; //just a sample, not fine tuned
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	if (shdr_mode) {
		UINT32 id = SEN1_VCAP_ID;

		if (shdr_mode == SHDR_2FRAME) {
			shdr_vcap_map = (1 << id) | (1 << (id+1));
		}
		cap_cfg.sen_cfg.shdr_map = HD_VIDEOCAP_SHDR_MAP(HD_VIDEOCAP_HDR_SENSOR1, shdr_vcap_map);
	}
...
static HD_RESULT set_cap2_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	if (shdr_mode) {
		UINT32 id = SEN2_VCAP_ID;

		if (shdr_mode == SHDR_2FRAME) {
			shdr_vcap_map = (1 << id) | (1 << (id+1));
		}
		cap_cfg.sen_cfg.shdr_map = HD_VIDEOCAP_SHDR_MAP(HD_VIDEOCAP_HDR_SENSOR2, shdr_vcap_map);
	}
	ret |= hd_videocap_set(video_cap_ctrl, HD_VIDEOCAP_PARAM_DRV_CONFIG, &cap_cfg);
	iq_ctl.func = HD_VIDEOCAP_FUNC_AE | HD_VIDEOCAP_FUNC_AWB;
	if (shdr_mode) {
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
{
	...
	{//select sensor mode, manually or automatically
		...
		if ((resolution_set == SEN_SEL_PATGEN) && (stream[0].cap_path == video_cap_path)) {
			video_in_param.sen_mode = HD_VIDEOCAP_PATGEN_MODE(HD_VIDEOCAP_SEN_PAT_COLORBAR, 200);
			video_in_param.frc = HD_VIDEO_FRC_RATIO(30,1);
			video_in_param.dim.w = p_dim->w;
			video_in_param.dim.h = p_dim->h;
		} else if ((resolution_set == SEN_SEL_PATGEN) && (stream[1].cap_path == video_cap_path)) {
			...
			if (resolution_set == SEN_SEL_IMX415) {
...
MAIN(argc, argv)
{
	...
	if (argc == 1) {
		printf("Usage: <view type> <shdr>\r\n");
		printf("Help:\r\n");
		printf("  <view type> : 0(sensor1), 1(sensor2), 2(PIP view), 3(side by side)\r\n");
		printf("  <shdr> : 0(disable), 1(2-frame)\r\n");
		printf("  <sensor> :  0(os02k10), 1(imx415)\r\n");
		return 0;
	}

    arg_idx++;
	if (argc > arg_idx) {
        display = atoi(argv[arg_idx]);
        switch(display)
        {
            case 0:
                    //cap0 ->vout0
	...
	if (shdr_mode) {
		g_capfmt &= (~HD_VIDEO_PXLFMT_PLANE_MASK);
		if (shdr_mode == SHDR_2FRAME) {
			g_capfmt |= (2UL << 24);
		} else {
			g_capfmt |= (3UL << 24);
	...
	printf("g_capfmt 0x%X\r\n", g_capfmt);
	if (shdr_mode == SHDR_2FRAME) {
		printf("2-FRAME SHDR\r\n");
	} else if (shdr_mode > SHDR_2FRAME) {
		printf("Usupported SHDR mode!\r\n");
		goto exit;
	}
    ...
    if(!stream[0].copy_new_buf){
        ...
        stream[0].pip_rect.x = stream[0].out_max_dim.w/2;
        stream[0].pip_rect.y = 0;
    ...
    } else {//single sensor for VOUT
		main_dim.w = stream[0].out_max_dim.w;
		main_dim.h = stream[0].out_max_dim.h;
    }
    ...
    if (display > 1 ) {
		...
		sub_dim.w = stream[0].out_max_dim.w;
		sub_dim.h = stream[0].out_max_dim.h;
    }
