	...
	while (1) {
		...
		if (key == 'c') {
			...
			if(vp_in_crop_id > 0) {
				...
				vp_in_crop_id --;
				p_proc_cur_crop = cur_crop + vp_in_crop_id;
				if (p_proc_cur_crop->w == VDO_SIZE_W && p_proc_cur_crop->h == VDO_SIZE_H && video_in_param.win.rect.x == 0 && video_in_param.win.rect.y == 0) {
					printf("*** set videoproc.in crop = OFF ***\r\n", p_proc_cur_crop->x, p_proc_cur_crop->y, p_proc_cur_crop->w, p_proc_cur_crop->h);

					video_in_param.mode  = HD_CROP_OFF;
				} else {
					printf("*** set videoproc.in crop = (%d,%d,%d,%d) ***\r\n", p_proc_cur_crop->x, p_proc_cur_crop->y, p_proc_cur_crop->w, p_proc_cur_crop->h);
