/**
	...
	@file video_capture_only_with_lvds.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_capture_only_with_lvds, argc, argv)
...
static UINT32 vdo_size_w = 2048;
static UINT32 vdo_size_h = 1536;
...
static UINT32 sensor_sel_1 = SEN_SEL_IMX123;
static UINT32 data_lane = 8;
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl, HD_OUT_ID _out_id, UINT32 *p_cap_pin_map, char *p_driver_name)
{
	...
	cap_cfg.sen_cfg.sen_dev.if_type = HD_COMMON_VIDEO_IN_LVDS;
	cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux =  0;  //use @0 in peri-dev.dtsi
	cap_cfg.sen_cfg.sen_dev.pin_cfg.clk_lane_sel = HD_VIDEOCAP_SEN_CLANE_LVDS(0, 0);
...
static void *cap_raw_thread(void *arg)
{
	...
	FILE *f_out_main;
	UINTPTR phy_addr_main[2], vir_addr_main[2];
	UINT32 shot_count = 0;
	UINT32 index;
	UINT32 i;
	...
	while (p_stream0->cap_exit == 0) {
		...
		if(p_stream0->cap_snap) {
			...
			for (i = 0; i < p_stream0->cap_frame_num; i++) {
				...
				snprintf(file_path_main[i], 32, "/mnt/sd/vdocap%u_%u.raw", index, shot_count);
...
MAIN(argc, argv)
{
	...
	if (argc == 1) {
		printf("Usage: <sensor> <data_lane>\r\n");
		printf("Help:\r\n");
		printf("  <sensor> :  0(imx123),  1(imx305)\r\n");
		printf("  <data_lane>\r\n");
		return 0;
	}
	...
	if (argc > 2) {
		...
		printf("sensor_sel_1 %d\r\n", sensor_sel_1);
	}

	if (argc > 2) {
		data_lane = atoi(argv[2]);
		printf("data_lane %d\r\n", data_lane);
	}

	if (sensor_sel_1 == SEN_SEL_IMX123) {
		snprintf(stream[0].cap_drv_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_imx123");
		if (data_lane == 8) {
			//CTL_SEN_MODE_1
			VDO_SIZE_W = 2048;
			VDO_SIZE_H = 1536;
			...
			printf("unsupported data_lane(%d) while sensor_sel_1 = %d\n", data_lane, sensor_sel_1);
			goto exit;
		}
	} else {
		printf("unsupported sensor_sel_1(%d)\n", sensor_sel_1);
		goto exit;
	}
	...
	//unused pin map should init to HD_VIDEOCAP_SEN_IGNORE
	for (i = 0; i < HD_VIDEOCAP_SEN_SER_MAX_DATALANE; i++) {
		if (i < data_lane) {
			stream[0].cap_pin_map[i] = i;
		} else {
