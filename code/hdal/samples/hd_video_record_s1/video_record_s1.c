...
typedef enum {
	SENSOR_MODEL_PAT_GEN = 0,
	SENSOR_MODEL_OS02K10,
	SENSOR_MODEL_MAX
} SENSOR_MODEL;
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl, UINT32 sensor)
{
	HD_RESULT ret = HD_OK;
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim, UINT32 sensor)
{
	HD_RESULT ret = HD_OK;
...
static HD_RESULT open_module(VIDEO_RECORD *p_stream, HD_DIM* p_proc_max_dim, UINT32 sensor)
{
	HD_RESULT ret;
	// set videocap config
	ret = set_cap_cfg(&p_stream->cap_ctrl, sensor);
	if (ret != HD_OK) {
...
{
	printf("======version: %s\n",SAMPLE_VERSION);
	printf("Usage: ./hd_video_record_s1 <enc_type> <sensor> <suspend_sec>\n");
	printf("  <enc_type>   : 0 or empty (H265), 1(H264), 2(MJPG)\n");
	printf("  <sensor>     : 0 or empty (pattern gen), 1 (OS02K)\n");
	printf("  <suspend_sec>: empty (10 seconds), 1 ~ 100 (seconds)\n");
	printf("\n\n");
	sleep(1);  // for printf not interrupt by kernel log
	return;
}
...
MAIN(argc, argv)
{
	...
	UINT32 enc_type = 0, sensor = SENSOR_MODEL_PAT_GEN, suspend_sec = 10;
	HD_DIM main_dim;
	char rtc_cmd[128] = {0};
	...
	if (argc >= 3) {
		sensor = atoi(argv[2]);
		if(sensor > SENSOR_MODEL_MAX) {
			printf("error: not support sensor[%d]!\r\n", sensor);
			return 0;
		}
	}
	printf("sensor = %d\n", sensor);
	...
	if (argc >= 4) {
		suspend_sec = atoi(argv[3]);
		if(suspend_sec < 0) {
			printf("error: suspend_sec[%d] must be a interger bigger than zero!\r\n", suspend_sec);
			return 0;
		}
	}
	printf("suspend_sec = %d\n", suspend_sec);
	snprintf(&rtc_cmd[0], sizeof(rtc_cmd), "echo +%d > /sys/class/rtc/rtc0/wakealarm", suspend_sec);
	...
	ret = open_module(&stream[0], &stream[0].proc_max_dim, sensor);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_cap_param(stream[0].cap_path, &stream[0].cap_dim, sensor);
	if (ret != HD_OK) {
...
again:
	...
	// query user key
	printf("Enter s to suspend / resume\n");
	printf("Enter q to exit\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3 || key == 's') {
	...
	if(key == 's'){
		...
		printf("hd_video_record_s1 is terminating...\n");
	}
