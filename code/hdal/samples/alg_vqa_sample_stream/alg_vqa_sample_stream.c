...
#define BLUR_BLOCK_WIDTH           160
#define BLUR_BLOCK_HEIGHT          94
#define BLUR_BLOCK_BUF_SIZE        (BLUR_BLOCK_WIDTH *  BLUR_BLOCK_HEIGHT)
...
static INT32 share_memory_init(VQA_MEM_RANGE *p_share_mem)
{
	...
	UINT32 blur_blk_size = BLUR_BLOCK_BUF_SIZE;
	...
	for(i=0; i<5; i++){
		if(i==0)
			blk_size = IMG_BUF_SIZE; //YUV
		else if(i==1 || i==2)
			blk_size = IMG_BUF_SIZE * 2/3; //UV
		else if(i==3)
			blk_size = IMG_WIDTH * IMG_HEIGHT;
		else if(i==4)
			blk_size = blur_blk_size;
...
static INT32 share_memory_exit(VQA_MEM_RANGE *p_share_mem)
{
	...
	for(i=0;i<5;i++){
		if (p_share_mem[i].va != 0) {
			ret = hd_common_mem_munmap((void *)p_share_mem[i].va,
				       		   p_share_mem[i].size);
			if (HD_OK != ret) {
                		printf("mnumap error !!\r\n\r\n");
				ret = HD_ERR_NG;
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[3].blk_size = IMG_BUF_SIZE *2/3;
	mem_cfg.pool_info[3].blk_cnt = 2;
	mem_cfg.pool_info[3].ddr_id = DDR_ID0;

	mem_cfg.pool_info[4].type = HD_COMMON_MEM_VQA_POOL;
	mem_cfg.pool_info[4].blk_size = NVT_VQA_Calc_buf_size(320, 180) + BLUR_BLOCK_BUF_SIZE;
...
static HD_RESULT vqa_set_para(UINT8 en_blur, UINT32 blur_strength, UINT32 blur_cover_th, UINT32 blur_alarm_times)
{
	...
	// global parameters
	vqa_param.global_param.width 			= IMG_WIDTH;
	vqa_param.global_param.height 			= IMG_HEIGHT;
...
#endif
	vqa_res_t vqa_rst;
	vqa_rst.res_blur_block = (UINT8 *) p_share_mem[4].va;

	NVT_VQA_Init(0, p_share_mem[3].va);


	UINT32 en_blur=1, blur_strength=8, blur_cover_th=60, blur_alarm_times=5;
	vqa_set_para(en_blur, blur_strength, blur_cover_th, blur_alarm_times);

	while(g_endFlag==0) 
	{
		// -1 = blocking mode, 0 = non-blocking mode, >0 = blocking-timeout mode
		ret = hd_videoproc_pull_out_buf(g_stream[0].proc_alg_path,
			       		&video_frame, -1);
		if(ret != HD_OK) {
		...
		if(counter > 30) {	
			img_va = (UINTPTR) hd_common_mem_mmap(
			...
			if (img_va == 0) {
				...
				goto gen_init_fail;
                        ...
                        #if (DEBUG_FILE == 1)
				printf("dump file1 %s", out_file1);
                                snprintf(out_file1, 64, "//mnt//sd//output_y//vqa_y_input_%d.bin", frame_id);
                                fd = fopen(out_file1, "wb");
                                if (!fd) {
                                        printf("cannot open %s\r\n", out_file1);
                                        return 0;
                                }
                                file_size = fwrite((const void *) img_va, 1, IMG_WIDTH*IMG_HEIGHT, fd);
                                if (file_size == 0) {
...
#endif
			    ret = NVT_VQA_Run(0, (UINT8*) img_va, &vqa_rst); //img_va include yuv, but NVT_VQA_Run use y only acctually
#if VQA_PERF_EN
                vos_perf_mark(&hw_tick_end);
    	        printf("NVT_VQA_Run time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
#endif	
				if (ret != HD_OK) {
					printf("run vqa fail!\n");
				}
				printf(" overexposure:%d | underexposure:%d | \
					 blur:%d, blur_area:%d % \n", \
					(int)vqa_rst.res_too_light, \
					(int)vqa_rst.res_too_dark, \
					(int)vqa_rst.res_blur,
					(int)vqa_rst.res_blur_cover);
			}
			hd_common_mem_munmap((void *)img_va, IMG_BUF_SIZE);
...
MAIN(argc, argv)
{
	...
	VQA_MEM_RANGE share_mem[5];
	pthread_t vqa_thread_id[VQA_THREAD_NUM];

	UINT32 en_blur=1, blur_strength=8, blur_cover_th=60, blur_alarm_times=5;

	UINT32 autotest=0;
    UINTPTR vqa_y_buf, vqa_blur_buf;
	...
	if (argc >= 3) {
		autotest = atoi(argv[2]);
	}

	g_stream[0].hdmi_id = HD_VIDEOOUT_HDMI_1920X1080P60; //16
	...
	if(autotest == 1){
		...
		vqa_y_buf = (UINTPTR) malloc (IMG_BUF_SIZE + (IMG_BUF_SIZE/2));
		vqa_blur_buf = (UINTPTR) malloc (IMG_BUF_SIZE + (IMG_BUF_SIZE/2));
		...
		for(idx=1; idx < 10; ++idx){
			if(idx == 1){
				printf("VQA: blur test \n");
				frame_id = 1;
				load_img = 1;
			}
			if(idx == 2){
				printf("VQA: exposure test \n");
				frame_id = 2;
				load_img = 1;
			}
			if(load_img == 1){
				snprintf(in_file, 64, "//mnt//sd//input//vqa_y_input_%d.bin", frame_id);
				fd = fopen(in_file, "r");
				if (!fd) {
					printf("cannot read %s\r\n", in_file);
					return 0;
				}
				file_size = fread((void *) vqa_y_buf, 1, IMG_BUF_SIZE + (IMG_BUF_SIZE/2), fd);
				if (file_size == 0) {
					printf("file is zeros %s\r\n", in_file);
					return 0;
				}
				fclose(fd);
				load_img = 0;
			...
			vqa_rst.res_blur_block = (UINT8 *) vqa_blur_buf; 
			ret = NVT_VQA_Run(0, (UINT8*) vqa_y_buf, &vqa_rst);

			if (ret != HD_OK) {
				printf("VQA: runtime error\n");
				return 0;
			}
			
			if((((int)vqa_rst.res_blur_cover) <= 50) && idx == 1){
				printf("VQA: res_blur_cover %d \n", (int)vqa_rst.res_blur_cover);
				return 0;
			}
			if((((int)vqa_rst.res_too_light) != 1) && idx ==  10 ){
				printf("VQA: res_exposure %d \n", (int)vqa_rst.res_too_light);
				return 0;
			}
		}
		printf("VQA: Autotest pass \n");
		free((void *) vqa_y_buf);
                free((void *) vqa_blur_buf);
		return 0;
	}
	...
	g_stream[0].proc_max_dim.w = VDO_SIZE_W;
        g_stream[0].proc_max_dim.h = VDO_SIZE_H;
	...
	g_stream[0].proc_alg_max_dim.w = IMG_WIDTH;
	g_stream[0].proc_alg_max_dim.h = IMG_HEIGHT;
	...
	/* set vqa parameter before mem init */

	/* create thread (pull_out bitstream) */
	
	for (idx = 0; idx < VQA_THREAD_NUM; idx++) {
		ret = pthread_create(&vqa_thread_id[0], NULL, vqa_thread_api, (VOID*) share_mem);
		if (ret < 0) {
	...
	while (1) {
		...
		if (key == 'e') {
			if (en_blur == 0)
				...
				en_blur = 0;
        	vqa_set_para(en_blur, blur_strength,
			blur_cover_th, blur_alarm_times);
