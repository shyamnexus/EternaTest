...
static HD_RESULT open_module(VIDEOOUT_ONLY *p_vdoout_only,UINT32 out_type)
{
    ...
    if(g_ide & VOUT_IDE2) {
        if((ret = hd_videoout_open(HD_VIDEOOUT_1_IN_0, HD_VIDEOOUT_1_OUT_0, &p_vdoout_only->out_path)) != HD_OK)
            return ret;
    }
    return HD_OK;
}
...
#ifdef __BUILTIN_IMAGE
	extern unsigned char video_frm_320_240_3_yuv422[];
	int index = 0;
#else
	FILE *f_in_main = 0;
    UINT32 readbyte = 0 ;
#endif
	uintptr_t pa_yuv_main[MAX_YUV_BLK_CNT] = {0};
	uintptr_t va_yuv_main[MAX_YUV_BLK_CNT] = {0};
	INT32  blkidx;
...
#ifdef __BUILTIN_IMAGE
		memcpy((void *)va_yuv_main[blkidx], video_frm_320_240_3_yuv422 + index*yuv_size, yuv_size);
