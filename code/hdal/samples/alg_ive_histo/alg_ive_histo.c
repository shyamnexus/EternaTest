...
MAIN(argc, argv)
{
	...
	dst_img_info.enDataType = LIB_IVE_DATA_TYPE_U32C1;
	dst_img_info.u64Pa      = share_mem[1].addr;
	dst_img_info.u64Va      = share_mem[1].va;
	dst_img_info.u32Width   = 256;
	dst_img_info.u32Height  = 1;
    dst_img_info.u32Stride  = 256 * 4; // 0~255, U32
	...
	lib_ive_ret = NVT_IVE_Histo(&handle, &src_img_info, &dst_img_info, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Histo time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
	if (lib_ive_ret != LIB_IVE_OK) {
		printf("err:NVT_IVE_Histo error %d\r\n",lib_ive_ret);
		goto exit;
	}
    if(!instant) {
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\IVEP\\output\\histo_output.bin");
#else
    snprintf(out_file1, 64, "//mnt//sd//IVEP//output//histo_output.bin");
#endif
