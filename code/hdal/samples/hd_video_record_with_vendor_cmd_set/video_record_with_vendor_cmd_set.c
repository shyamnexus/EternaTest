...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_vendor_cmd_set, argc, argv)
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[1].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, HD_VIDEO_PXLFMT_YUV420)+md_buf_size;
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
...

		{
			...
			jnd.c_str = 10;
			jnd.t_str = 11;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_JND, &jnd);
		}

		{
			VENDOR_VIDEOENC_MIN_RATIO_CFG min_ratio = {0};
			min_ratio.min_i_ratio = 1200;
			min_ratio.min_p_ratio = 800;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_MIN_RATIO, &min_ratio);
		}

		{
			VENDOR_VIDEOENC_LONG_START_CODE long_start_code = {0};
			long_start_code.long_start_code_en = 1;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_LONG_START_CODE, &long_start_code);
		}

		{
			VENDOR_VIDEOENC_H26X_SVC_WEIGHT_MODE svc_weight_mode = {0};
			svc_weight_mode.h26x_svc_weight_mode = 1;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_H26X_SVC_WEIGHT_MODE, &svc_weight_mode);
		}

		{
			VENDOR_VIDEOENC_QUALITY_BASE_MODE q_base = {0};
			q_base.quality_base_en = 1;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_QUALITY_BASE, &q_base);
		}

		{
			VENDOR_VIDEOENC_BR_TOLERANCE br_tolerance = {0};
			br_tolerance.br_tolerance = 4;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_BR_TOLERANCE, &br_tolerance);
		}

		{
			VENDOR_VIDEOENC_H26X_MAQ_DIFF maq_diff = {0};
			maq_diff.b_enable = 1;
			maq_diff.str = 12;
			maq_diff.start_idx = 2;
			maq_diff.end_idx = 7;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_H26X_MAQ_DIFF, &maq_diff);
		}

		{
			VENDOR_VIDEOENC_BG_RDO_CFG bg_rdo = {0};
			bg_rdo.b_enable = 1;
			bg_rdo.avc_bg_skip_bias = 11;
			bg_rdo.hevc_bg_skip_bias = -12;
			bg_rdo.hevc_bg_merge_bias = 13;
			bg_rdo.bg_bias_shift = 2;
			bg_rdo.mode = 1;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_BG_RDO, &bg_rdo);
		}

		{
			VENDOR_VIDEOENC_SEI_SIGNED_VDO_CFG signed_vdo = {0};
			signed_vdo.enable = 1;
			signed_vdo.machine_id[VENDOR_VIDEOENC_SEI_SIGNED_VDO_ID_1] = 3;
			signed_vdo.machine_id[VENDOR_VIDEOENC_SEI_SIGNED_VDO_ID_2] = 4;
			signed_vdo.machine_id[VENDOR_VIDEOENC_SEI_SIGNED_VDO_ID_3] = 5;
			signed_vdo.machine_id[VENDOR_VIDEOENC_SEI_SIGNED_VDO_ID_4] = 6;
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_SEI_SIGNED_VDO, &signed_vdo);
		}

		if (enc_type == 0) {
			{
				VENDOR_VIDEOENC_RDO_CFG rdo = {0};
				rdo.rdo_codec = VENDOR_VIDEOENC_CODEC_265;
				rdo.rdo_param.rdo_265.hevc_inter_16x16_cost_bias = 3;
				rdo.rdo_param.rdo_265.hevc_intra_32x32_cost_bias = 4;
				rdo.rdo_param.rdo_265.hevc_intra_16x16_cost_bias = 5;
				rdo.rdo_param.rdo_265.hevc_intra_8x8_cost_bias = 6;
				rdo.rdo_param.rdo_265.hevc_inter_skip_cost_bias = -13;
				rdo.rdo_param.rdo_265.hevc_inter_merge_cost_bias = -14;
				rdo.rdo_param.rdo_265.hevc_inter_64x64_cost_bias = 7;
				rdo.rdo_param.rdo_265.hevc_inter_64x32_32x64_cost_bias = 20;
				rdo.rdo_param.rdo_265.hevc_inter_32x32_cost_bias = 21;
				rdo.rdo_param.rdo_265.hevc_inter_32x16_16x32_cost_bias = 22;
				rdo.rdo_param.rdo_265.hevc_inter_16x16_cost_bias = 23;
				vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_RDO, &rdo);
			}
			...
			{
				VENDOR_VIDEOENC_RDO_CFG rdo = {0};
				rdo.rdo_codec = VENDOR_VIDEOENC_CODEC_264;
				rdo.rdo_param.rdo_264.avc_intra_4x4_cost_bias = 7;
				rdo.rdo_param.rdo_264.avc_intra_8x8_cost_bias = 8;
				rdo.rdo_param.rdo_264.avc_intra_16x16_cost_bias = 9;
				rdo.rdo_param.rdo_264.avc_inter_tu4_cost_bias = 10;
				rdo.rdo_param.rdo_264.avc_inter_tu8_cost_bias = 11;
				rdo.rdo_param.rdo_264.avc_inter_skip_cost_bias = 12;
				vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_RDO, &rdo);
			}
			//--- HD_VIDEOENC_PARAM_OUT_ENC_PARAM ---
			...
			{
				VENDOR_VIDEOENC_JPG_COLOR_TO_GRAY_CFG gray = {0};
				gray.enable = 1;
				gray.color_to_gray = 1;
				gray.src_color_to_gray = 1;
				vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_JPG_COLOR_TO_GRAY, &gray);
			}

			{
				VENDOR_VIDEOENC_JPG_YUV_TRANS_CFG yuv_trans = {0};
				yuv_trans.jpg_yuv_trans_en = 1;
				vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_JPG_YUV_TRANS, &yuv_trans);
			}
...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == 'g') {
			VENDOR_VIDEOENC_H26X_ENC_GOP_CFG gop = {0};
			gop.h26x_enc_gop = 30;
			vendor_videoenc_set(stream[0].enc_path, VENDOR_VIDEOENC_PARAM_OUT_H26X_ENC_GOP, &gop);
			hd_videoenc_start(stream[0].enc_path);
		}
	}
