...
#if AUDDEC_PREBLK
	UINT32 block_size = 40000;

	// set audiodec raw block size [NOTE: need to set before path config]
	ret = vendor_audiodec_set(audio_dec_path, VENDOR_AUDIODEC_ITEM_RAW_BLOCK_SIZE, &block_size);
	if (ret != HD_OK) {
		printf("set_dec_raw_block_size = %d\r\n", ret);
	}
#endif
