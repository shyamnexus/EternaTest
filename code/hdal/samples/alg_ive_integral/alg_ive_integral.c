...
unsigned int IVE_INTEGRAL_INPUT_TEST_CASE  = 0; //0: 1ch, 1: 2ch, 2: 3ch
unsigned int IVE_INTEGRAL_OUTPUT_TEST_CASE = 0; //0: sum, 1: square sum, 2: combined format
...
MAIN(argc, argv)
{
	...
	IVE_INTEG_CTRL_S integral_ctrl = {0};
	...
	if (argc == 3) {
		sscanf(argv[1], "%hu", &IVE_INTEGRAL_INPUT_TEST_CASE);
		sscanf(argv[2], "%hu", &IVE_INTEGRAL_OUTPUT_TEST_CASE);
		printf("integral  input test case is set to %d\n", IVE_INTEGRAL_INPUT_TEST_CASE);
		printf("integral output test case is set to %d\n", IVE_INTEGRAL_OUTPUT_TEST_CASE);
	}

	//ive_load_input();
#if defined(__FREERTOS)
	snprintf(in_file1, 64, "A:\\input\\y_%dch.bin", IVE_INTEGRAL_INPUT_TEST_CASE+1);
#else
    snprintf(in_file1, 64, "//mnt//sd//input//y_%dch.bin", IVE_INTEGRAL_INPUT_TEST_CASE+1);
#endif
	...
	src_img_info.u32Stride  = 512 * (IVE_INTEGRAL_INPUT_TEST_CASE+1);
	src_img_info.u64Pa      = share_mem[0].addr;
	src_img_info.u64Va      = share_mem[0].va;
	src_img_info.u32Width   = 512;
	src_img_info.u32Height  = 512;
	
	dst_img_info.enDataType = (IVE_INTEGRAL_OUTPUT_TEST_CASE == 0) ? LIB_IVE_DATA_TYPE_U32C1 : LIB_IVE_DATA_TYPE_U64C1;
	dst_img_info.u32Stride  = (IVE_INTEGRAL_OUTPUT_TEST_CASE == 0) ? 512*(IVE_INTEGRAL_INPUT_TEST_CASE+1)*4 : 512*(IVE_INTEGRAL_INPUT_TEST_CASE+1)*8;
	dst_img_info.u64Pa      = share_mem[1].addr;
	dst_img_info.u64Va      = share_mem[1].va;
	dst_img_info.u32Width   = 512;
	dst_img_info.u32Height  = 512;

	output_size = dst_img_info.u32Stride * dst_img_info.u32Height;
	
	integral_ctrl.enInFormat  = IVE_INTEGRAL_INPUT_TEST_CASE;
	integral_ctrl.enOutFormat = IVE_INTEGRAL_OUTPUT_TEST_CASE;
    ...
    ret = NVT_IVE_Integ(&handle, &src_img_info, &dst_img_info, &integral_ctrl, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Integ time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
	if (ret != HD_OK) {
		printf("err:NVT_IVE_Integ error %d\r\n",ret);
		goto exit;
	}
    if(!instant) {
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\output\\integral_output_in%dout%d.bin", IVE_INTEGRAL_INPUT_TEST_CASE, IVE_INTEGRAL_OUTPUT_TEST_CASE);
#else
    snprintf(out_file1, 64, "//mnt//sd//output//integral_output_in%dout%d.bin", IVE_INTEGRAL_INPUT_TEST_CASE, IVE_INTEGRAL_OUTPUT_TEST_CASE);
#endif
    ...
    printf("[ive integral sample] write result done!\n");
