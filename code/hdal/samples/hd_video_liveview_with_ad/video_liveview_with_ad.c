...
#define OUT_TYPE 1// 1: LCD,  2: HDMI
#define HDMI_MODE HD_VIDEOOUT_HDMI_1920X1080P60
...
#define VDO_MEM_SIZE_W	3840
#define VDO_MEM_SIZE_H	2160
...
static HD_RESULT mem_init(UINT32 stream_num)
{
	...
	mem_cfg.pool_info[0].blk_cnt = 3*stream_num;
...
static HD_RESULT set_cap_cfg(UINT32 stream_id)
{
	...
	UINT32 ad_map, ad_type;
	HD_VIDEOCAP_SEN_CFG_EXT sen_cfg_ext[2] = {0};
	...
	if (p_stream->module_type == AD_MODULE_TYPE_NVP6124B) {
		...
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux = p_stream->chip_id;  //use @0 in peri-dev.dtsi
		...
		cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[7] = HD_VIDEOCAP_SEN_IGNORE;
		cap_cfg.sen_cfg.sen_dev.pin_cfg.ccir_vd_hd_pin = TRUE;
	} else if (p_stream->module_type == AD_MODULE_TYPE_TP2830) {
		printf("cap cfg tp2830");
		snprintf(cap_cfg.sen_cfg.sen_dev.driver_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_ad_tp2830");
		cap_cfg.sen_cfg.sen_dev.if_type = p_stream->module_info->bus_type;
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux = p_stream->chip_id;  //use @0 in peri-dev.dtsi
		cap_cfg.sen_cfg.sen_dev.pin_cfg.ccir_msblsb_switch = FALSE;
		cap_cfg.sen_cfg.sen_dev.pin_cfg.ccir_vd_hd_pin = TRUE;
	...
	// set vie pad a/b
	sen_cfg_ext[0].func = HD_VIDEOCAP_SEN_EXT_PAD_SEL;
	sen_cfg_ext[0].value = p_stream->pad; //referring to CTL_SEN_PAD_SEL

	// set single/dual edge
	sen_cfg_ext[1].func = HD_VIDEOCAP_SEN_EXT_SMP_EDGE;
	sen_cfg_ext[1].value = p_stream->smp_edge; //referring to CTL_SEN_SMP_EDGE
	...
	//set default ccir format
	vendor_videocap_set(video_cap_ctrl, VENDOR_VIDEOCAP_PARAM_DEFAULT_CCIR_FMT, &p_stream->ccir_info.fmt);

	// set preference camera standard
	ad_type = 0x1; // referring to AD_DET_TYPE
...
static HD_RESULT set_out_cfg(UINT32 stream_id)
{
	...
	UINT32 out_type = OUT_TYPE;
	HD_VIDEOOUT_HDMI_ID hdmi_id = HDMI_MODE;
...
static UINT32 get_input(int argc, char** argv)
{
	...
	if (argc == 0 || (argc % IN_ARG_NUM)) {
		...
		printf("module chip vin vcap mux_id lane_num det w h fps interlace pad ddr\r\n");
		printf("module - ");
		...
		printf("pad - 0~2 (0: PAD_A, 1: PAD_B, 2:PAD_AB)\r\n");
		printf("ddr - 0~1 (0: single edge, 1: dual edge)\r\n");
		return 0;
	}
	...
	while ((int)i*IN_ARG_NUM < argc) {
		...
		p_stream->pad 						= atoi(argv[(i*IN_ARG_NUM)+IN_ARG_PAD]);
		p_stream->smp_edge 					= atoi(argv[(i*IN_ARG_NUM)+IN_ARG_SMP]);
	...
	for (i = 0; i < stream_num; i++) {
		...
		for (j = 0; j < stream_num; j++) {
			...
			if (p_stream->module_type == p_stream_tmp->module_type && p_stream->chip_id == p_stream_tmp->chip_id) {
				p_stream->vcap_id_bit |= (1 << p_stream_tmp->vcap_id); // refer to HD_VIDEOCAP_0, HD_VIDEOCAP_1, ... theses define must be match
			}
		}
	}
	...
	for (i = 0; i < stream_num; i++) {
		...
		printf("Stream%u, module_type=%s chip_id=%u vin_id=%u vcap_id=%u ccir_mux_index=%u "
			...
			"ccir_fmt=%s vproc_id=%u vcap_id_bit=0x%x field_sel=%u pad=%s smp=%u\r\n",
			...
			pad_str[p_stream->pad], p_stream->smp_edge);
	}
