...
VENDOR_VIDEOPROC_FISHEYE_MASK fisheye = {
	.enable = 1,
	.cent_x = 960,
	.cent_y = 540,
	.valid_r = 100,
...
static int set_mask_param(HD_PATH_ID proc_path)
{
	int ret;

	ret = vendor_videoproc_set(proc_path, VENDOR_VIDEOPROC_PARAM_FISHEYE_MASK, &fisheye);
	if(ret){
		printf("VENDOR_VIDEOPROC_PARAM_FISHEYE_MASK fail\n");
	}
...
MAIN(argc, argv)
{
	...
	UINT32 pull_type = 0;
	HD_DIM main_dim;
	UINT8  enable = fisheye.enable;
	UINT32 cent_x = fisheye.cent_x;
	UINT32 cent_y = fisheye.cent_y;
	UINT32 valid_r = fisheye.valid_r;
	UINT8  decs_r = fisheye.decs_r;
	...
	if(set_mask_param(stream[0].proc_path)){
		printf("fail to set fisheye\r\n");
		goto exit;
	}
	...
	printf("Enter w to change radius\n");
	printf("Enter x to change sharpness\n");
	printf("Enter e to blink\n");
	printf("Enter z to move center\n");
	while (1) {
		key = GETCHAR();
		if (key == 's') {
			stream[0].do_snap = 1;
		...
		if (key == 'q' || key == 0x3) {
			...
			printf("x=%d , y=%d\n", cent_x, cent_x);
			fisheye.cent_x = cent_x;
			fisheye.cent_y = cent_y;
			set_mask_param(stream[0].proc_path);
		}else if(key == 'x'){
			if(decs_r == 0){
				decs_r = 128;
			}else if(decs_r == 128){
			...
			printf("sharpness = %u\n", decs_r);
			fisheye.decs_r = decs_r;
			set_mask_param(stream[0].proc_path);
		}else if(key == 'e'){
			enable = (enable ? 0 : 1);
			printf("fisheye is %s\n", enable ? "enable" : "disable");
			fisheye.enable = enable;
			set_mask_param(stream[0].proc_path);
		}else if(key == 'w'){
			if(valid_r == 100){
				valid_r = 540;
