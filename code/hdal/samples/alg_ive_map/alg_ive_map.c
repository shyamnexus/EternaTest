...
MAIN(argc, argv)
{
	...
	//ive_load_input2;
#if defined(__FREERTOS)
	snprintf(in_file2, 64, "A:\\input\\map_lut.bin");
#else
	snprintf(in_file2, 64, "//mnt//sd//input//map_lut.bin");
#endif
	file_size = ive_load_file(in_file2, share_mem[1].va);
	if (file_size == 0) {
		printf("load map_lut.bin : %s\r\n", in_file2);
		return 0;
	}
	...
	src_img_info2.u32Width   = 256;
	src_img_info2.u32Height  = 1;
    src_img_info2.u32Stride  = 256;
	...
	lib_ive_ret = NVT_IVE_Map(&handle, &src_img_info1, &src_img_info2, &dst_img_info, instant);
	vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Map time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
	if (lib_ive_ret != LIB_IVE_OK) {
		printf("err:NVT_IVE_Map error %d\r\n",lib_ive_ret);
		goto exit;
	}
    if(!instant) {
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\output\\map_output.bin");
#else
    snprintf(out_file1, 64, "//mnt//sd//output//map_output.bin");
#endif
