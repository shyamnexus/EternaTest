...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_process_only_nvx2, argc, argv)
...
#define YUV_BLK_SIZE_IN       VDO_NVX_BUFSIZE(VDO_SIZE_W,VDO_SIZE_H,VDO_FMT_IN)//(DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, VDO_FMT))
#define VDO_FMT_IN_NAME       "nvx2"
#define VDO_FMT_IN            HD_VIDEO_PXLFMT_YUV420_NVX2
#endif

#if OUT_NVX
#define YUV_BLK_SIZE       VDO_NVX_BUFSIZE(VDO_SIZE_W,VDO_SIZE_H,VDO_FMT_IN)
#define VDO_FMT_OUT_NAME       "nvx2"
#define VDO_FMT_OUT           HD_VIDEO_PXLFMT_YUV420_NVX2
...
static HD_VIDEOPROC_CROP _test_out_crop[7+2] = {
	...
	{HD_CROP_ON,  {{0, 0},{500,600,1920+700,1080+800}}},
	{HD_CROP_ON,  {{0, 0},{0,0,3840,2160}}},
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	#if 0  // ipp not supported
    {
    	HD_VIDEOPROC_CROP* p_crop = &_test_out_crop[2];

    	printf("*** set videoproc.out pre scale crop = (%d,%d,%d,%d) ***\r\n",
    		p_crop->win.rect.x, p_crop->win.rect.y, p_crop->win.rect.w, p_crop->win.rect.h);

    	ret = hd_videoproc_set(video_proc_path, HD_VIDEOPROC_PARAM_IN_CROP_PSR, p_crop);
    	if (ret != HD_OK) {
    		printf("run pre crop %d fail\r\n", ret);
    	}
	}
	#endif // ipp not supported
    {
    	HD_VIDEOPROC_CROP* p_crop = &_test_out_crop[1];

    	printf("*** set videoproc.out out crop = (%d,%d,%d,%d) ***\r\n",
    		p_crop->win.rect.x, p_crop->win.rect.y, p_crop->win.rect.w, p_crop->win.rect.h);

    	ret = hd_videoproc_set(video_proc_path, HD_VIDEOPROC_PARAM_OUT_CROP, p_crop);
    	if (ret != HD_OK) {
...
static void *aquire_yuv_thread(void *arg)
{
	...
	while (p_stream0->proc_exit == 0) {
		...
		if(1) {
			...
			snprintf(file_path_main, 32, "/mnt/sd/dump_frm_%s_%u.dat",VDO_FMT_OUT_NAME ,p_stream0->shot_count);
