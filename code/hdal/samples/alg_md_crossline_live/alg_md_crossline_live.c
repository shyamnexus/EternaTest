/**
 * @file md_integrated_demo.c
 * @brief Integrated Motion Detection Demo for NT98538
 * 
 * This sample demonstrates ALL motion detection features:
 * - MDBC (Background Comparison)
 * - Global Alarm
 * - Sub-Region Alarm (4 zones)
 * - Object Detection (bounding boxes)
 * - Cross-Line Detection
 * - Motion AQ integration with video encoder
 * 
 * Output:
 * - Console logs with motion events
 * - H.265 video file with Motion AQ enabled
 * - Motion statistics every second
 * 
 * Usage: ./md_integrated_demo [duration_seconds] [sensitivity]
...
#define MAIN(argc, argv)    EXAMFUNC_ENTRY(md_integrated_demo, argc, argv)
...
// Output files
#define OUTPUT_VIDEO_FILE   "/mnt/sd/md_demo_output.h265"
#define OUTPUT_LOG_FILE     "/mnt/sd/md_demo_log.txt"
...
static void print_banner(void)
{
    printf("\n");
    printf(COLOR_CYAN "╔══════════════════════════════════════════════════════════════╗\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_WHITE "       NT98538 Integrated Motion Detection Demo               " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "╠══════════════════════════════════════════════════════════════╣\n" COLOR_RESET);
    ...
    printf(COLOR_CYAN "║" COLOR_GREEN "    ✓ " COLOR_RESET "Global Motion Alarm                                     " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_GREEN "    ✓ " COLOR_RESET "4-Zone Sub-Region Detection                             " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_GREEN "    ✓ " COLOR_RESET "Object Detection (up to 32 objects)                     " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_GREEN "    ✓ " COLOR_RESET "Cross-Line Detection                                    " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_GREEN "    ✓ " COLOR_RESET "Motion AQ Video Encoding (EVBR)                         " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "╚══════════════════════════════════════════════════════════════╝\n" COLOR_RESET);
    printf("\n");
}
...
typedef struct {
    ...
    // Data paths
    HD_PATH_ID cap_path;
    HD_PATH_ID proc_path_main;    // Main stream to encoder (with MD enabled)
...
static HD_RESULT setup_videoproc(DEMO_CONTEXT* ctx)
{
    ...
    HD_VIDEOPROC_FUNC_CONFIG func_cfg = {0};
    ...
    // Open main output path (to encoder with MD enabled)
    ...
    // When binding to encoder, we can skip setting explicit output dimensions
    // The encoder input parameters will drive the scaling
    // But set output format for clarity
    proc_out.func = 0;
    ...
    // Enable MD output on main path - this enables hardware MD for Motion AQ
    func_cfg.out_func = HD_VIDEOPROC_OUTFUNC_MD;
    ret = hd_videoproc_set(ctx->proc_path_main, HD_VIDEOPROC_PARAM_FUNC_CONFIG, &func_cfg);
    if (ret != HD_OK) {
        printf("  Failed to enable MD on main path: %d\n", ret);
        return ret;
    }
    
    printf("[VIDEOPROC] MD output enabled on main path\n");
...
static HD_RESULT setup_md(DEMO_CONTEXT* ctx)
{
    ...
    ctx->objdet_ctrl.u32ObjThres = 150;  // Optimized: Higher threshold (was 50)
    ctx->objdet_ctrl.enInRange = LIB_MD_IN_RANGE_0_1;
    ctx->objdet_ctrl.pstPtStk = ctx->obj_pt_stack;

    // Configure Cross-Line Detection - Optimized Coordinates
    // Using a "thicker" slanted corridor similar to reference sample
    ctx->cross_ctrl.enCrossMode = LIB_MD_CROSS_LINE;
    ctx->cross_ctrl.u32OverlapTh = 10;
    
    // Coordinates modeled after alg_md_crossline sample (0,35)-(159,40)-(159,45)-(0,40)
    // Shifted (+20 Y) to center it around Y=60 for this demo
    ctx->cross_ctrl.u32X1 = 0;   ctx->cross_ctrl.u32Y1 = 55;
    ctx->cross_ctrl.u32X2 = 159; ctx->cross_ctrl.u32Y2 = 60;
    ctx->cross_ctrl.u32X3 = 159; ctx->cross_ctrl.u32Y3 = 65;
    ctx->cross_ctrl.u32X4 = 0;   ctx->cross_ctrl.u32Y4 = 60;
    ...
    while (g_running && g_md_running) {
        ...
        if (md_ret == LIB_MD_OK && ctx->obj_info.u32ObjNum > 0) {
            ...
            pthread_mutex_unlock(&g_stats_mutex);
                ...
                // Transfer detected objects to Cross-Line structure with SIZE FILTERING
                // NOTE: libmd uses 1-based indexing for object arrays (SDK convention)
                ctx->cross_ctrl.u8ObjNum = 0;
                
                for (UINT32 obj_idx = 1; obj_idx <= ctx->obj_info.u32ObjNum && ctx->cross_ctrl.u8ObjNum < LIB_MD_MAX_OBJ_NUM; obj_idx++) {
                    UINT32 w = ctx->obj_info.stObjRst[obj_idx].u32XEnd - ctx->obj_info.stObjRst[obj_idx].u32XStart;
                    UINT32 h = ctx->obj_info.stObjRst[obj_idx].u32YEnd - ctx->obj_info.stObjRst[obj_idx].u32YStart;
                    
                    // FILTER: Ignore small objects (Width < 5 or Height < 10)
                    if (w < 5 || h < 10) continue;
                    
                    // Use 1-based indexing for cross_objs array (per SDK convention)
                    UINT32 cross_idx = ctx->cross_ctrl.u8ObjNum + 1;
                    ctx->cross_objs[cross_idx].u32XStart = ctx->obj_info.stObjRst[obj_idx].u32XStart;
                    ctx->cross_objs[cross_idx].u32YStart = ctx->obj_info.stObjRst[obj_idx].u32YStart;
                    ctx->cross_objs[cross_idx].u32XEnd   = ctx->obj_info.stObjRst[obj_idx].u32XEnd;
                    ctx->cross_objs[cross_idx].u32YEnd   = ctx->obj_info.stObjRst[obj_idx].u32YEnd;
                    ctx->cross_ctrl.u8ObjNum++;
                }

                if (ctx->cross_ctrl.u8ObjNum > 0 && 
                    NVT_MD_CrossLine(ctx->cross_objs, ctx->cross_results, &ctx->cross_ctrl) == LIB_MD_OK) {
                    
                    // Check results using 1-based indexing (per SDK convention)
                    for (UINT32 i = 1; i <= ctx->cross_ctrl.u8ObjNum; i++) {
                        if (ctx->cross_results[i] == LIB_MD_CROSS_IN_TO_OUT || 
                            ctx->cross_results[i] == LIB_MD_CROSS_OUT_TO_IN) {
                            
                            pthread_mutex_lock(&g_stats_mutex);
                            g_stats.crossline_events++;
                            pthread_mutex_unlock(&g_stats_mutex);
                            
                            printf(COLOR_CYAN "\t [CROSS] Line crossed by Object %u! (%s)\n" COLOR_RESET, 
                                   i, 
                                   ctx->cross_results[i] == LIB_MD_CROSS_IN_TO_OUT ? "IN -> OUT" : "OUT -> IN");
                        }
                    }
            }
        }
...
static void* encode_thread(void* arg)
{
    ...
    printf(COLOR_GREEN "[Encoder Thread] Started, waiting for flow_start...\n" COLOR_RESET);
    ...
    printf("[Encoder Thread] Flow started, output: %s\n", OUTPUT_VIDEO_FILE);
    ...
    if (ret != HD_OK) {
        printf(COLOR_RED "[Encoder Thread] Failed to get buffer info: %d\n" COLOR_RESET, ret);
        return NULL;
    }
    
    printf("[Encoder Thread] Buffer info: PA=0x%lX, size=%u\n", 
    ...
    if (!ctx->enc_vir_addr) {
        printf(COLOR_RED "[Encoder Thread] Failed to mmap encoder buffer!\n" COLOR_RESET);
        return NULL;
    }
    
    // Setup poll list
    poll_list[0].path_id = ctx->enc_path;
    
    while (g_running) {
        // Use poll with blocking mode, then non-blocking pull
        ret = hd_videoenc_poll_list(poll_list, 1, 500);  // 500ms timeout
        if (ret == HD_OK) {
            if (poll_list[0].revent.event == TRUE) {
                ret = hd_videoenc_pull_out_buf(ctx->enc_path, &bs_data, 0);  // Non-blocking
                ...
                if (frame_count % 30 == 0) {
                    ...
                    printf(COLOR_WHITE "         Encoded: %u frames, %u I-frames, %.2f Mbps avg\n" COLOR_RESET,
                           frame_count, i_frames, mbps);
                }
            }
        }
    }
    ...
    printf(COLOR_GREEN "[Encoder Thread] Stopped\n" COLOR_RESET);
...
MAIN(argc, argv)
{
    ...
    printf("  Bitrate:     %d Mbps (EVBR with Motion AQ)\n", MAIN_BITRATE / 1000000);
    printf("  Output:      %s\n", OUTPUT_VIDEO_FILE);
    printf("\n");
    ...
    pthread_create(&enc_tid, NULL, encode_thread, &ctx);
    ...
    // Signal encode thread to start pulling
    g_flow_start = 1;
    
    printf("\n");
    printf(COLOR_CYAN "╔══════════════════════════════════════════════════════════════╗\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_GREEN "  Pipeline running! Press Ctrl+C to stop.                     " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "║" COLOR_RESET "  REAL libmd processing active, Motion AQ enabled             " COLOR_CYAN "║\n" COLOR_RESET);
    printf(COLOR_CYAN "╚══════════════════════════════════════════════════════════════╝\n" COLOR_RESET);
    printf("\n");
