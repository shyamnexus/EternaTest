...
static UINT32 id = 0;
static CA_DATA ca_data;

static UINT32 while_loop = TRUE;
static AWBT_CA awb_ca = {0};
static AET_STATUS_INFO ae_status = {0};
static AET_CURVE_GEN_MOVIE ae_curve_gen_movie = {0};
static AWBT_STATUS awb_sta;

static int light_info_get(LIGHT_INFO *light_info)
{
	HD_RESULT ret;
	awb_ca.id = id;
	...
	if ((awb_sta.status.mode != AWBALG_STATUS_FREEZE) && (awb_sta.status.mode != AWBALG_STATUS_NIGHTMODE)) {
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2 && strcmp(argv[1], "test") == 0) {
		...
		if (argc >= 3) {
			id = atoi(argv[2]);
			if (id > 5){
				printf("set %d, but arg[0] id : 0 1 2 3 4 5\n",id);
				return 1;
			}
	...
	// Init light_sensing
	item.night_ev = night_ev_th;
	item.day_ev = day_ev_th;
	item.sensitive = sensitive; 
	item.open_device = open_device;
	item.light_info_cb_fp = (LIGHT_INFO_CB_FP)&light_info_get;
	printf("addr = %x\n", item.light_info_cb_fp);
	if(sw_cds_init(id, item) < 0) {
