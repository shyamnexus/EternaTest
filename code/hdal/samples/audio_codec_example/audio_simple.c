...
EXAMFUNC_ENTRY(hd_audio_record, argc, argv)
{
	...
	UINT32 enc_type = 0;
	UINT32 duration = 0; // Auto-exit duration in seconds (0 = wait for 'q')
	...
	if (argc >= 3) {
		...
		if (duration > 0) {
			printf("Duration: %d seconds (auto-exit)\r\n", duration);
		}
	}
	...
	// query user key or wait for duration
	if (duration > 0) {
		printf("Recording for %d seconds... (press Ctrl+C to stop)\n", duration);
		sleep(duration);
