/**
 * @brief Combined Audio/Video Recording with G.711 μ-law Audio
 * 
 * This sample captures video using H.264/H.265 encoder and audio using G.711 μ-law
 * encoding. Both streams are saved to separate files that can be muxed later.
 * 
 * Usage: ./av_record_g711_ulaw [video_codec] [duration]
 ...
 *   - av_record_audio_ulaw.raw (G.711 μ-law audio, 48kHz stereo)
 * 
 * To mux with ffmpeg:
 *   ffmpeg -i av_record_video.h265 -f mulaw -ar 48000 -ac 2 -i av_record_audio_ulaw.raw \
 *          -c:v copy -c:a aac output.mp4
 * 
 * @file av_record_g711_ulaw.c
...
// ============================================================================
// G.711 μ-law Encoder Implementation
// ============================================================================
...
static void *video_encode_thread(void *arg)
{
    ...
    // Open output file
    snprintf(file_path, sizeof(file_path), "/mnt/sd/av_record_video.%s",
...
static void *audio_encode_thread(void *arg)
{
    ...
    UINTPTR vir_addr;
    char file_path[64] = "/mnt/sd/av_record_audio_ulaw.raw";
    FILE *f_out = NULL;
    time_t start_time;
    ...
    printf("Audio: recording G.711 μ-law to %s\n", file_path);
    start_time = time(NULL);
...
static void print_usage(const char *prog)
{
    ...
    printf("\nOutput files on /mnt/sd/:\n");
    printf("  - av_record_video.h265 or .h264\n");
    printf("  - av_record_audio_ulaw.raw (G.711 μ-law, 48kHz stereo)\n");
    printf("\nTo mux with ffmpeg:\n");
    printf("  ffmpeg -i av_record_video.h265 -f mulaw -ar 48000 -ac 2 -i av_record_audio_ulaw.raw \\\n");
...
cleanup_common:
    ...
    printf("Video: /mnt/sd/av_record_video.%s\n",
           stream.video_codec == 0 ? "h265" : "h264");
    printf("Audio: /mnt/sd/av_record_audio_ulaw.raw\n");
    printf("\nTo mux with ffmpeg:\n");
    printf("  ffmpeg -i /mnt/sd/av_record_video.%s -f mulaw -ar 48000 -ac 2 \\\n",
           stream.video_codec == 0 ? "h265" : "h264");
    printf("         -i /mnt/sd/av_record_audio_ulaw.raw -c:v copy -c:a aac /mnt/sd/output.mp4\n");
    printf("========================================\n");
