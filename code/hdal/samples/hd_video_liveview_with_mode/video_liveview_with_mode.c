...
#define RESOLUTION_SET  2 //0: 2M(IMX290), 1:5M(OS05A), 2: 2M (OS02K10), 3: 2M (AR0237IR), 4: 8M (IMX415)
#if ( RESOLUTION_SET == 0)
...
static HD_RESULT mem_init(void)
{
	...
	if ((g_prcbind == 0) || (g_prcbind == 0xff)) {
		...
		mem_cfg.pool_info[1].blk_cnt = 3; //vout release blk is fast
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	if (g_shdr == 1) {
		...
		#if (RESOLUTION_SET == 0 || RESOLUTION_SET == 4)
			cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[2] = 2;
			cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[3] = 3;
		#elif (RESOLUTION_SET == 1)
...
#if IGNORE_DTSI_PINMUX
	{
        //MUST be set prior to HD_VIDEOCAP_PARAM_DRV_CONFIG !!!
        HD_VIDEOCAP_SEN_CFG_EXT pinmux[3] = {0};
        HD_VIDEOCAP_SEN_CONFIG_EXT user_pinmux = {0};
		//the following setting is only for 690
		//referring to linux-kernel/include/dt-bindings/pinctrl/nvt-na51102-pinctrl.h
		#if (RESOLUTION_SET == 4)//IMX415
        pinmux[0].func = 11;        //PIN_FUNC_SENSORMISC;
        pinmux[0].value = 0x1;      //PIN_SENSORMISC_CFG_SN_MCLK_1;
        pinmux[1].func = 13;        //PIN_FUNC_MIPI_LVDS;
        pinmux[1].value = 0x100000; //PIN_MIPI_LVDS_CFG_HSI2HSI3_TO_CSI;
        pinmux[2].func = 3;         //PIN_FUNC_I2C;
        pinmux[2].value = 0x200;    //PIN_I2C_CFG_I2C3_2;
        #else // OS02K10
        pinmux[0].func = 11;        //PIN_FUNC_SENSORMISC;
        pinmux[0].value = 0x2;      //PIN_SENSORMISC_CFG_SN2_MCLK_1;
        pinmux[1].func = 13;        //PIN_FUNC_MIPI_LVDS;
        pinmux[1].value = 0x100000; //PIN_MIPI_LVDS_CFG_HSI2HSI3_TO_CSI;
        pinmux[2].func = 3;         //PIN_FUNC_I2C;
        pinmux[2].value = 0x2000;   //PIN_I2C_CFG_I2C4_2;
		#endif
        user_pinmux.cfg_num = 3;
        user_pinmux.p_sen_cfg_ext = pinmux;

        ret = vendor_videocap_set(video_cap_ctrl, VENDOR_VIDEOCAP_PARAM_USER_PINMUX, &user_pinmux);
        if (ret != HD_OK) {
            printf("VENDOR_VIDEOCAP_PARAM_USER_PINMUX failed!(%d)\r\n", ret);
        }
    }
#endif
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
{
	...
	if (g_capfmt == HD_VIDEO_PXLFMT_NRX12) {
		UINT32 enc_rate = VENDOR_VIDEOCAP_ENCODE_RATE(1, RAW_COMPRESS_RATIO);

		vendor_videocap_set(video_cap_path, VENDOR_VIDEOCAP_PARAM_ENC_RATE, &enc_rate);
	}
	return ret;
}
