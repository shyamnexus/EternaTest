...
static HD_VIDEO_PXLFMT      g_vdo_fmt_out = HD_VIDEO_PXLFMT_RAW12;
static UINT32               g_vdo_size_out = VDO_BLK_SIZE_RAW;
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		video_cfg_param.pipe = g_proc_pipe;
		video_cfg_param.isp_id = HD_ISP_DONT_CARE; //0
		video_cfg_param.ctrl_max.func = 0;
		video_cfg_param.in_max.func = 0;
		video_cfg_param.in_max.dim.w = p_max_dim->w;
		video_cfg_param.in_max.dim.h = p_max_dim->h;
		video_cfg_param.in_max.pxlfmt = g_vdo_fmt_in;
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	if (p_dim != NULL) { //if videoproc is already binding to dest module, not require to setting this!
		...
		video_out_param.pxlfmt = g_vdo_fmt_out;
...
static void *aquire_vdo_thread(void *arg)
{
	...
	while (p_stream0->proc_exit == 0) {
		...
		if (1) {
			...
			if (g_vdo_fmt_out == HD_VIDEO_PXLFMT_RAW12) {
				snprintf(file_path_main, 32, "/mnt/sd/dump_frm_%s_%u.dat", VDO_FMT_NAME_RAW, p_stream0->shot_count);
			...
			} else { //if (g_vdo_fmt_out == HD_VIDEO_PXLFMT_YUV420) {
...
//		stream[0].show_ret = 1; //show pull result
	...
	if (pipe_type == 0) {
		g_proc_pipe = HD_VIDEOPROC_PIPE_PRE;
		g_vdo_fmt_out = HD_VIDEO_PXLFMT_RAW12;
		g_vdo_size_out = VDO_BLK_SIZE_RAW;
		printf("run HD_VIDEOPROC_PIPE_PRE\r\n");
	} else if (pipe_type == 1) {
		g_proc_pipe = HD_VIDEOPROC_PIPE_RAWALL_LITE;
		g_vdo_fmt_out = HD_VIDEO_PXLFMT_YUV420;
		g_vdo_size_out = VDO_BLK_SIZE_YUV;
		printf("run HD_VIDEOPROC_PIPE_RAWALL_LITE\r\n");
	} else if (pipe_type == 2) {
		g_proc_pipe = HD_VIDEOPROC_PIPE_RAWCOLOR_LITE;
		g_vdo_fmt_out = HD_VIDEO_PXLFMT_YUV420;
		g_vdo_size_out = VDO_BLK_SIZE_YUV;
		printf("run HD_VIDEOPROC_PIPE_RAWCOLOR_LITE\r\n");
	}
