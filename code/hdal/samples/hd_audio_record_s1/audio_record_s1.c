...
{
	printf("======version: %s\n",SAMPLE_VERSION);
	printf("Usage: ./hd_audio_record_s1 <enc_type> <suspend_sec>\n");
	printf("  <enc_type>   : 0(AAC), 1(ULAW), 2(ALAW), 3 or empty (PCM)\n");
...
EXAMFUNC_ENTRY(hd_audio_record, argc, argv)
{
	...
	UINT32 enc_type = 3, suspend_sec = 10;
	char rtc_cmd[128] = {0};
	...
	if (enc_type == 0) {
		...
		stream[0].enc_type = HD_AUDIO_CODEC_PCM;
	}
...
again:
	...
	if(key == 's'){
		...
		printf("hd_audio_record_s1 is terminating...\n");
	}
