/**
	...
	@file video_snapshot_with_8path_srcout.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_snapshot_with_8path_srcout, argc, argv)
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[9].type = HD_COMMON_MEM_USER_POOL_BEGIN;
	mem_cfg.pool_info[9].blk_size = MAX_SNAPSHOT_SIZE; // for JPEG snapshot bs
	mem_cfg.pool_info[9].blk_cnt = 8;
...
static int video_file_open(VIDEO_FILE* fh, UINT32 enc_path, char* filename)
{
	...
	fh->vir_addr = (UINTPTR)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, fh->phy_buf.buf_info.phy_addr, fh->phy_buf.buf_info.buf_size);
...
for (i = 0; i < 8; i++) {
	...
	ret = hd_videoenc_set(p_stream0->enc_path[i], HD_VIDEOENC_PARAM_OUT_TRIG_SNAPSHOT, &snap_param);
	if (ret != HD_OK) {
		goto snapshot_fail;
	}
#if 0
	...
	{
		char file_path[32] = {0};
		snprintf(file_path, 32, "/mnt/sd/dump_bs%u_snap%u.jpg", i, p_stream0->shot_count);
