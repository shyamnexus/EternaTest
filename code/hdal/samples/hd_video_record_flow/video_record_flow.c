...
#if ( RESOLUTION_SET == 0 || RESOLUTION_SET == 2)
...
static void *save_thread(void *arg)
{
	...
	if (p_stream0->enc_type == 0) {
		snprintf(file_path_main, 64, "/mnt/sd/rec_%lux%lu_%lu_h265.dat",
			(unsigned long)p_stream0->record_dim.w, (unsigned long)p_stream0->record_dim.h,
			(unsigned long)(p_stream0->save_count+1));
	} else if (p_stream0->enc_type == 1) {
		snprintf(file_path_main, 64, "/mnt/sd/rec_%lux%lu_%lu_h264.dat",
			(unsigned long)p_stream0->record_dim.w, (unsigned long)p_stream0->record_dim.h,
			(unsigned long)(p_stream0->save_count+1));
	} else if (p_stream0->enc_type == 2) {
		snprintf(file_path_main, 64, "/mnt/sd/rec_%lux%lu_%lu_mjpg.dat",
			(unsigned long)p_stream0->record_dim.w, (unsigned long)p_stream0->record_dim.h,
			(unsigned long)(p_stream0->save_count+1));
	} else {
		printf("not support enc_type\r\n");
...
{
	VIDEO_LIVEVIEW* p_stream0 = (VIDEO_LIVEVIEW *)((ULONG*)arg)[0];
	VIDEO_RECORD* p_stream2 = (VIDEO_RECORD *)((ULONG*)arg)[1];
	UINT32 enc_type;
...
MAIN(argc, argv)
{
	...
	ULONG stream_list[2] = {((ULONG)&stream[0]), ((ULONG)&stream2[0])};
	UINT32 out_type = 1;
	UINT32 enc_type = 0;
