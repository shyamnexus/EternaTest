...
UINT32 vprc_out_fmt = HD_VIDEO_PXLFMT_YUV420;
UINT32 vprc_out_fmt_sub = HD_VIDEO_PXLFMT_YUV420;
///////////////////////////////////////////////////////////////////////////////
...
static HD_RESULT set_proc_cfg(HD_OUT_ID ctrl, HD_PATH_ID *p_video_proc_ctrl, HD_VIDEOPROC_PIPE pipe, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		...
		video_cfg_param.isp_id = (pipe == HD_VIDEOPROC_PIPE_SCALE) ? HD_ISP_DONT_CARE : SEN1_VCAP_ID;
		video_cfg_param.ctrl_max.func = 0;
		video_cfg_param.in_max.func = 0;
		video_cfg_param.in_max.dim.w = p_max_dim->w;
		video_cfg_param.in_max.dim.h = p_max_dim->h;
		video_cfg_param.in_max.pxlfmt = (pipe == HD_VIDEOPROC_PIPE_SCALE) ? vprc_out_fmt : CAP_OUT_FMT;
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim, HD_VIDEO_PXLFMT out_fmt)
{
	HD_RESULT ret = HD_OK;
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate, HD_VIDEO_PXLFMT in_fmt)
{
	HD_RESULT ret = HD_OK;
...
MAIN(argc, argv)
{
	...
	if (argc > 2) {
		UINT32 vpr_fmt = atoi(argv[2]);
		printf("vpr_fmt %d\r\n", vpr_fmt);
        if(vpr_fmt==1) {
            vprc_out_fmt = HD_VIDEO_PXLFMT_YUV420_NVX2;
			vprc_out_fmt_sub = HD_VIDEO_PXLFMT_YUV420;
        } else {
            vprc_out_fmt = HD_VIDEO_PXLFMT_YUV420;
			vprc_out_fmt_sub = HD_VIDEO_PXLFMT_YUV420;
        }
	...
	ret = set_proc_param(stream[0].proc0_path, &stream[0].proc1_dim, vprc_out_fmt);
	if (ret != HD_OK) {
	...
	ret = set_proc_param(stream[0].proc1_path, NULL, vprc_out_fmt);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_proc_param(stream[1].proc1_path, NULL, vprc_out_fmt_sub);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, enc_type, 2 * 1024 * 1024, vprc_out_fmt);
	if (ret != HD_OK) {
	...
	ret = set_enc_param(stream[1].enc_path, &stream[1].enc_dim, enc_type, 1 * 1024 * 1024, vprc_out_fmt_sub);
	if (ret != HD_OK) {
		printf("set enc fail=%d\n", ret);
		goto exit;
	}
