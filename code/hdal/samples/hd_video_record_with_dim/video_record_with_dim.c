...
static UINT32 g_vprc_fast_sync = 0;
///////////////////////////////////////////////////////////////////////////////
...
static void *encode_thread(void *arg)
{
	...
	printf("encode thread wait\r\n");
	//------ wait flow_start ------
	while (p_stream0->flow_start == 0) sleep(1);
	printf("encode thread in\r\n");
	...
	//----- open output files -----
	snprintf(file_path_main, 32, "/mnt/sd/dump_bs_main_%d.dat", p_stream0->bs_count);
...
MAIN(argc, argv)
{
	...
	if (argc == 3) {
		...
		g_vprc_fast_sync = atoi(argv[2]);
		printf("g_vprc_fast_sync %d\r\n", g_vprc_fast_sync);
		if(g_vprc_fast_sync > 1) {
			printf("error: not support vproc sync type!\r\n");
			return 0;
		}
	}
