...
//scale a 1920x1080 yuv422 image up to 960x540 and 3840x2160
static HD_RESULT test_422sp_scale(UINT32 buf_pa, UINT32 buf_size)
{
	int                 fd;
	HD_GFX_SCALE        param;
	void                *va;
	...
	frame.sign = MAKEFOURCC('V','F','R','M');
	frame.pxlfmt  = HD_VIDEO_PXLFMT_YUV422;
	frame.dim.h   = 1080;
	frame.loff[0] = 1920;
	frame.loff[1] = 1920;
	src_size = hd_common_mem_calc_buf_size(&frame);
	...
	//load yuv422 image from sd card
    fd = open("/mnt/sd/video_frm_1920_1080_1_yuv422sp.dat", O_RDONLY);
	if(fd == -1){
		printf("fail to open /mnt/sd/video_frm_1920_1080_1_yuv422sp.dat\n");
		ret = HD_ERR_NOT_FOUND;
		goto exit;
	}
	...
	close(fd);
	if(len != (int)src_size){
		printf("fail to read /mnt/sd/video_frm_1920_1080_1_yuv422sp.dat\n");
		ret = HD_ERR_SYS;
		goto exit;
	}

	//use gfx engine to scale up image
	memset(&param, 0, sizeof(HD_GFX_SCALE));
	param.src_img.dim.w            = 1920;
	param.src_img.dim.h            = 1080;
	param.src_img.format           = HD_VIDEO_PXLFMT_YUV422;
	param.src_img.p_phy_addr[0]    = src_pa;
	param.src_img.p_phy_addr[1]    = src_pa + 1920 * 1080;
	param.src_img.lineoffset[0]    = 1920;
	param.src_img.lineoffset[1]    = 1920;
	param.dst_img.dim.w            = 3840;
	param.dst_img.dim.h            = 2160;
	param.dst_img.format           = HD_VIDEO_PXLFMT_YUV422;
	param.dst_img.p_phy_addr[0]    = dst_pa;
	param.dst_img.p_phy_addr[1]    = dst_pa + 3840 * 2160;
	param.dst_img.lineoffset[0]    = 3840;
	param.dst_img.lineoffset[1]    = 3840;
	...
	//save the result image to sd card
    fd = open("/mnt/sd/hd_gfx_scale_3840_2160_1.yuv422", O_WRONLY | O_CREAT, 0644);
	if(fd == -1){
		printf("fail to open /mnt/sd/hd_gfx_scale_3840_2160_1.yuv422\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
	...
	close(fd);
	if(len != (int)dst_size){
		printf("fail to write /mnt/sd/hd_gfx_scale_3840_2160_1.yuv422\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
    printf("result is /mnt/sd/hd_gfx_scale_3840_2160_1.yuv422\n");
	...
	param.dst_img.dim.w            = 960;
	param.dst_img.dim.h            = 540;
	param.dst_img.p_phy_addr[1]    = dst_pa + 960 * 540;
	param.dst_img.lineoffset[0]    = 960;
	param.dst_img.lineoffset[1]    = 960;
	param.dst_region.w             = 960;
	param.dst_region.h             = 540;
	...
	//save the result image to sd card
    fd = open("/mnt/sd/hd_gfx_scale_960_540_1.yuv422", O_WRONLY | O_CREAT, 0644);
	if(fd == -1){
		printf("fail to open /mnt/sd/hd_gfx_scale_960_540_1.yuv422\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
	...
	close(fd);
	if(len != (int)dst_size){
		printf("fail to write /mnt/sd/hd_gfx_scale_960_540_1.yuv422\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
    printf("result is /mnt/sd/hd_gfx_scale_960_540_1.yuv422\n");
...
//transform a 1920x1080 yuv422 2 planar image to a yuyv 1 planar image
static HD_RESULT test_422SP_to_422ONE(UINT32 buf_pa, UINT32 buf_size)
{
	int                       fd;
	HD_GFX_COLOR_TRANSFORM    param;
	void                      *va;
	...
	//save the result image to sd card
	fd = open("/mnt/sd/hd_gfx_422sp_to_yuyv_1920_1080_1_yuyv.dat", O_WRONLY | O_CREAT, 0644);
	if(fd == -1){
		printf("fail to open /mnt/sd/hd_gfx_422sp_to_yuyv_1920_1080_1_yuyv.dat\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
	...
	if(len != (int)packed_size){
		printf("fail to write /mnt/sd/hd_gfx_422sp_to_yuyv_1920_1080_1_yuyv.dat\n");
		ret = HD_ERR_SYS;
		goto exit;
	}
	printf("result is /mnt/sd/hd_gfx_422sp_to_yuyv_1920_1080_1_yuyv.dat\n");
...
static HD_RESULT test_4203P_scale(UINT32 buf_pa, UINT32 buf_size)
{
	...
	UINTPTR                   file_pa, file_va, scale_pa, scale_va;
...
static HD_RESULT test_dma_copy(UINT32 buf_pa, UINT32 buf_size)
{
	...
	ret = memcmp((void*)src_va, (void*)dst_va, size);
	if(ret)
		printf("dma fails to copy %d bytes %d\n", size,ret);
	else{
...
#if 0
HD_RESULT test_yuv420_affine(UINT32 buf_pa, UINT32 buf_size)
{
	int                 fd;
	HD_GFX_AFFINE       param;
	void                *va;
...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		printf("Enter p to transform yuv422sp to yuyv\n");
		printf("Enter d to scale down yuv420 3p image\n");
		printf("Enter n to test dma memory copy by common\n");
		printf("Enter b to test scale yuv422sp down/up\n");
		...
		if (key == 'c') {
			...
			ret = test_422SP_to_422ONE(gfx.buf_pa, gfx.buf_size);
			if(ret != HD_OK)
				printf("422sp to yuyv fail=%d\n", ret);
			break;
		}
		else if (key == 'd') {
			...
			ret = test_422sp_scale(gfx.buf_pa, gfx.buf_size);
			if(ret != HD_OK)
				printf("scale fail=%d\n", ret);
