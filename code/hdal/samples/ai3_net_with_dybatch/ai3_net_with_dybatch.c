...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(ai3_net_with_dybatch, argc, argv)
...
static UINT32 set_batch_num[16] = {0};
///////////////////////////////////////////////////////////////////////////////
...
extern HD_RESULT _vendor_ai_net_force_not_check_input_align(VOID);

static HD_RESULT network_init(void)
{
	HD_RESULT ret = HD_OK;

    ret = _vendor_ai_net_force_not_check_input_align();
    if (ret != HD_OK) {
        printf("_vendor_ai_net_force_not_check_input_align fail=%d\n", ret);
        return ret;
    }
	...
	for (i = 0; i < p_stream->input_blob_num; i++) {
        ...
        for(j = 0; j < set_batch_num[i]; j++) {
            ...
            printf(" path_%d(0x%x) pa(0x%lx) va(0x%lx) size(%u) whcb(%d,%d,%d,%d)\n",
                    idx, p_net->net_info.in_path_list[idx], in_buf.pa, in_buf.va, in_buf.size, in_buf.width, in_buf.height, in_buf.channel, in_buf.batch_num);
            ret = vendor_ai3_net_set(proc_id, p_net->net_info.in_path_list[idx], &in_buf);
            if (HD_OK != ret) {
        ...
        idx = idx + p_in->in_cfg.b - set_batch_num[i];
...
MAIN(argc, argv)
{
	...
	//net_in
	NET_IN_CONFIG in_cfg = {
		.input_filename = "/mnt/sd/jpg/YUV420_SP_W512H376_batch16.bin",
		.w = 512,
		.h = 376,
		.c = 2,
		.b = 16,
	...
	NET_IN_CONFIG in_cfg2 = {
		.input_filename = "/mnt/sd/jpg/roi_blob_batch16.bin",
		.w = 1,
		.h = 1,
		.c = 5,
		.b = 16,
	...
	//net proc
	NET_PROC_CONFIG net_cfg = {
		.model_filename = "/mnt/sd/para/dynamic_batch/nvt_model_mblob.bin",
	...
	//net_in
	NET_IN_CONFIG in_cfg = {
		.input_filename = "/mnt/sd/jpg/YUV420_SP_W512H376_batch8.bin",
		.w = 512,
		.h = 376,
		.c = 2,
		.b = 6,
	...
	//net proc
	NET_PROC_CONFIG net_cfg = {
		.model_filename = "/mnt/sd/para/dynamic_batch/nvt_model.bin",
		.label_filename = "/mnt/sd/accuracy/labels.txt"
	};
#endif
...
*/
	...
	for (j=0; j < stream[0].input_blob_num; j++) {
		if (j == 0) {
            set_batch_num[j] = in_cfg.b;
			ret = input_set_config((stream[0].in_path + j), &in_cfg);
		} else {
            set_batch_num[j] = in_cfg2.b;
    ...
    printf("Enter z to set batch number (immediately)\n");
	...
	do {
        ...
        if (key == 'z') {
            ...
            scanf("%u", &batch_num);
            printf("set VENDOR_AI3_NET_PARAM_BATCH_N_IMM batch_num(%u)\n", batch_num);
            printf("update start (by number)\n");
            vendor_ai3_net_set(p_net->proc_id, VENDOR_AI3_NET_PARAM_BATCH_N_IMM, &batch_num);
            for(j=0; j < stream[0].input_blob_num; j++) {
				set_batch_num[j] = batch_num;
			}
            continue;
        }
		if (key == 'd') {
