...
#define FOR_SNAPSHOT_TOOL 1
#if FOR_SNAPSHOT_TOOL
#include "nvtsnapshot.h"
#endif
#if FOR_SNAPSHOT_TOOL
...
#if FOR_SNAPSHOT_TOOL
//Support stitching tool
static int g_reload_once = 0;   //reload settings
static int g_reloaded = 0;
static int g_rtsp_enabled = 0;
static int g_is_rtsp_enabled = 1;
static int g_vsp_command = -1;
...
/// @brief notify RTSP is ready after reload
void notify_reloaded(int is_reloaded)
{
	if(!is_reloaded)
	{
		g_reloaded = 0;
    ...
    g_reloaded = is_reloaded;
    pthread_cond_signal(&cond);
    //printf("notify_reloaded, is_reloaded=%d\r\n", is_reloaded);
    pthread_mutex_unlock(&mutex);
}

/// @brief wait flag event
/// @param pFlag
/// @param timeoutSec  timeout
/// @return 1: got event, 0: timeout
int wait_event(int *pFlag, int timeoutSec)
{
    int ret = 1;
    struct timeval now;
    ...
    while (*pFlag == 0) {
        ...
        if (result == ETIMEDOUT) {
            printf("ERR: wait_event timeout occurred\n");
            ret = 0;
            goto RELEASE;
        }
    }
...
int stitchingHandler(StitchingCommand nCmd, char *strParam)
{
	int enableVsp = -1;

	printf("Recevied cmd %d, strParam=%s\r\n", nCmd, strParam);

	switch(nCmd)
	{
		case StitchingCommand_Switch:
			enableVsp = atoi(strParam);
			if(g_vsp_mode == enableVsp)
			{
				//already at this vsp mode
				printf("WRN: already at vsp mode %d\r\n", g_vsp_mode);
				return 0;
			}
			g_vsp_command = enableVsp;
			return 0;
		break;
		case StitchingCommand_Distance:
		{
			char setting_path[1024] = {0};
			sprintf(setting_path, "%s/%s", VSP_DATA_PATH, strParam);
			//printf("copy files %s\r\n", setting_path);
			if(copy_files_to_parent_dir(setting_path))
			{
				//copy file fail
				return 2;
			}
			...
			g_reloaded = 0;
			g_reload_once = 1;  //reload for mem_init because the stitching size is different.
			if(wait_event(&g_reloaded, 15))
			{
				return 0;
			}
			else
			{
				printf("ERR: rtsp timeout when changing distance.\r\n");
				return 1;
			}
		}
		break;
		default:
			printf("ERR: Unknown cmd %d\r\n", nCmd);
		break;
	}
	return 1;
}

/// @brief Check if keyboard is hitted
/// @return true: hitted
int kbhit(int timeout_us)
{
#if defined(__LINUX)
	fd_set set;
...
void show_menu(void)
{
...
#if FOR_SNAPSHOT_TOOL
	printf("Enter r to reload settings\n");
#endif
	...
	if (g_vsp_mode == 0) {
		for (i = 0;i < vcap_num; i++) {
			printf("sensor%d:       rtsp://192.168.0.3//stream%d\r\n", i, i);
		}
	} else {
		printf("vsp main stream:      rtsp://192.168.0.3//stream%d\r\n", vcap_num);
		printf("vsp sub stream:       rtsp://192.168.0.3//stream%d\r\n", vcap_num + 1);
		printf("vsp sub stream2:      rtsp://192.168.0.3//stream%d\r\n", vcap_num + 2);
	}
	printf("================================================\r\n");
}
...
#if FOR_SNAPSHOT_TOOL
	//init stitching lib
	nvtsl_init( NVTSL_DEFAULT_PORT);
	nvtsl_setCallBack(&stitchingHandler);
...
#if FOR_SNAPSHOT_TOOL
			key = -1;
			if(!kbhit(10000)) {   //check if any key is pressed within 10 ms  (non blocking)
				// no key input within 10 ms, check flags
				if(g_reload_once) {
					usleep(300000);
					key = 'q';
				}
				else if(g_is_rtsp_enabled && !g_rtsp_enabled)
				{
					usleep(300000); //After a delay of 300 ms, rtsp will be activated.
					key = 'z';
				}
				else if (g_vsp_command != -1)
				{
					if(g_vsp_mode == g_vsp_command)
					{
						continue;
					}
					usleep(300000);
					switch(g_vsp_command)
					{
						case 0:
							key='0';
							break;
