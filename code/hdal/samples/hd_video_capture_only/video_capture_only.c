...
#define AUTO_DET_SENSOR  0

#define RAW_BUFFER_DDR_ID DDR_ID0

#define HD_VIDEO_MULTI_FRAME_SHIFT       8

#define SHDR_2FRAME 1
#define SHDR_3FRAME 2

static UINT32 vdo_size_w = VDO_SIZE_W_2M;
static UINT32 vdo_size_h = VDO_SIZE_H_2M;
static UINT32 resolution_set = SEN_SEL_OS02K10;
...
static HD_RESULT mem_init(UINT32 sen_hdr, UINT32 *p_blk_size)
{
	...
	if (sen_hdr == SHDR_2FRAME) {
		mem_cfg.pool_info[0].blk_cnt *= 2;
	} else if (sen_hdr == SHDR_3FRAME) {
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl, HD_OUT_ID _out_id, UINT32 *p_cap_pin_map, UINT32 cap_shdr_map, char *p_driver_name)
{
	...
	{
		UINT32 i;
		CHAR *sensor_probe_list[] =
		{
			"nvt_sen_imx290",
			"nvt_sen_os05a10",
			"nvt_sen_os02k10"
		};
		for (i = 0; i < sizeof(sensor_probe_list)/sizeof(CHAR *); i++) {
			snprintf(cap_cfg.sen_cfg.sen_dev.driver_name, HD_VIDEOCAP_SEN_NAME_LEN-1, sensor_probe_list[i]);
			ret = vendor_videocap_set(video_cap_ctrl, VENDOR_VIDEOCAP_PARAM_PROBE_SENSOR, &cap_cfg.sen_cfg.sen_dev);
			printf("probe %s: ret=%d\r\n", sensor_probe_list[i], ret);
			if (ret == HD_OK) {
				break;
			}
		}
	...
	#else
	if (strlen(p_driver_name) < (HD_VIDEOCAP_SEN_NAME_LEN)) {
        strncpy(cap_cfg.sen_cfg.sen_dev.driver_name, p_driver_name, HD_VIDEOCAP_SEN_NAME_LEN);
    } else {
        printf("driver name(%s) exceeds HD_VIDEOCAP_SEN_NAME_LEN-1 \r\n", p_driver_name);
    }
	#endif
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim, HD_VIDEOCAP_SEN_FRAME_NUM frame_num)
{
	...
	{//select sensor mode, manually or automatically
		...
		if (resolution_set == SEN_SEL_IMX415 && frame_num == 3) {
			video_in_param.frc = HD_VIDEO_FRC_RATIO(2995,100);
		} else {
...
static HD_RESULT open_module(VIDEO_LIVEVIEW *p_stream)
{
	HD_RESULT ret;
	// set videocap config
	ret = set_cap_cfg(&p_stream->cap_ctrl, HD_VIDEOCAP_CTRL(sen1_vcap_id), p_stream->cap_pin_map, p_stream->cap_shdr_map, p_stream->cap_drv_name);
	if (ret != HD_OK) {
...
static void *cap_raw_thread(void *arg)
{
	...
	HD_RESULT ret = HD_OK;
	HD_VIDEO_FRAME video_frame[3] = {0};
	char file_path_main[3][32] = {0};
	FILE *f_out_main;
	UINTPTR phy_addr_main[3], vir_addr_main[3];
	UINT32 shot_count = 0;
	UINT32 index;
	UINT32 i;
	...
	while (p_stream0->cap_exit == 0) {
		...
		if(p_stream0->cap_snap) {
			...
			for (i = 0; i < p_stream0->cap_frame_num; i++) {
				...
				vir_addr_main[i] = (UINTPTR)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, phy_addr_main[i], p_stream0->blk_size);
				...
				index = video_frame[i].ddr_id >> HD_VIDEO_MULTI_FRAME_SHIFT;
				snprintf(file_path_main[i], 32, "/mnt/sd/vdocap%lu_%lu.raw", (ULONG)index, (ULONG)shot_count);
...
MAIN(argc, argv)
{
	...
	if (argc > 2) {
		resolution_set = atoi(argv[2]);
		printf("resolution_set %d\r\n", resolution_set);
	}
	if (argc > 3) {
		pull_type = atoi(argv[3]);
		stream[0].show_ret = 1; //show pull result
		printf("pull_type %d\r\n", pull_type);
	}

	if (resolution_set == SEN_SEL_IMX290) {
		VDO_SIZE_W = VDO_SIZE_W_2M;
		VDO_SIZE_H = VDO_SIZE_H_2M;
	} else if (resolution_set == SEN_SEL_OS05A10) {
		VDO_SIZE_W = VDO_SIZE_W_5M;
		VDO_SIZE_H = VDO_SIZE_H_5M;
	} else if (resolution_set == SEN_SEL_OS02K10) {
		VDO_SIZE_W = VDO_SIZE_W_2M;
		VDO_SIZE_H = VDO_SIZE_H_2M;
	} else if (resolution_set == SEN_SEL_IMX415) {
	...
	if (resolution_set == SEN_SEL_IMX290) {
		...
		snprintf(stream[0].cap_drv_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_os05a10");
		...
		snprintf(stream[0].cap_drv_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_imx415");
	...
	if (sen_hdr) {
		UINT32 sen1_shdr_vcap_map = 0;

		if (sen_hdr == SHDR_2FRAME) {
			stream[0].cap_shdr_map = HD_VIDEOCAP_SHDR_MAP(HD_VIDEOCAP_HDR_SENSOR1, (HD_VIDEOCAP_0|HD_VIDEOCAP_1));
			if (sen1_vcap_id == 0) {
				sen1_shdr_vcap_map = (HD_VIDEOCAP_0|HD_VIDEOCAP_1);
			} else {
				sen1_shdr_vcap_map = (HD_VIDEOCAP_2|HD_VIDEOCAP_3);
			}
		} else {//if (sen_hdr == SHDR_3FRAME) {
			if (sen1_vcap_id == 0) {
				sen1_shdr_vcap_map = (HD_VIDEOCAP_0|HD_VIDEOCAP_1|HD_VIDEOCAP_2);
			} else {
				sen1_shdr_vcap_map = (HD_VIDEOCAP_2|HD_VIDEOCAP_3|HD_VIDEOCAP_4);
			}
		}
		stream[0].cap_shdr_map = HD_VIDEOCAP_SHDR_MAP(HD_VIDEOCAP_HDR_SENSOR1, sen1_shdr_vcap_map);
	...
	if (sen_hdr == SHDR_2FRAME) {
		...
		stream[0].cap_frame_num = HD_VIDEOCAP_SEN_FRAME_NUM_3;
	} else {
