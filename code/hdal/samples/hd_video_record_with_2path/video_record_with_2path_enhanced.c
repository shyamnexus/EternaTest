...
static HD_RESULT mem_init(void)
{
	...
	// config common pool (cap) - optimized to reduce SIE load
	mem_cfg.pool_info[0].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[0].blk_size = DBGINFO_BUFSIZE()+VDO_RAW_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, CAP_OUT_FMT)
														+VDO_CA_BUF_SIZE(CA_WIN_NUM_W, CA_WIN_NUM_H)
														+VDO_LA_BUF_SIZE(LA_WIN_NUM_W, LA_WIN_NUM_H)
														+0x800; // Add 2KB safety margin (reduced to lower memory pressure)
	mem_cfg.pool_info[0].blk_cnt = 3; // Keep original count to reduce SIE load
	mem_cfg.pool_info[0].ddr_id = DDR_ID0;
	
	// config common pool (main) - balanced configuration
	mem_cfg.pool_info[1].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[1].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, HD_VIDEO_PXLFMT_YUV420)
														+0x1000; // Add 4KB safety margin
	mem_cfg.pool_info[1].blk_cnt = 3; // Balanced buffer count
	...
	//config common pool (audio) - moderate increase
	mem_cfg.pool_info[5].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[5].blk_size = 0x6000; // 24KB for audio (moderate increase)
	mem_cfg.pool_info[5].blk_cnt = 2; // 2 buffers (was 1)
	mem_cfg.pool_info[5].ddr_id = DDR_ID0;

	// Validate memory configuration before initialization
	printf("Memory pool configuration (optimized for 4-stream stability):\n");
	for (int i = 0; i < 6; i++) {
		printf("  Pool[%d]: size=%u KB, count=%u\n", i, 
			mem_cfg.pool_info[i].blk_size / 1024, 
			mem_cfg.pool_info[i].blk_cnt);
	}

	ret = hd_common_mem_init(&mem_cfg);
	if (ret != HD_OK) {
		printf("ERROR: Memory initialization failed with error code: %d\n", ret);
		return ret;
	}
	
	printf("Memory pools initialized successfully\n");
	return ret;
}
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		...
		// Enable DIRECT mode to reduce SIE load and fix behavior errors
		video_cfg_param.ctrl_max.func = 0;
		video_cfg_param.in_max.func = HD_VIDEOPROC_INFUNC_DIRECT;  // Zero-buffer mode
		...
		if (ret != HD_OK) {
			printf("ERROR: Failed to set DIRECT mode: %d\n", ret);
			return HD_ERR_NG;
		}
		printf("Video proc configured in DIRECT mode (zero-buffer)\n");
	}

	// Disable ALL enhancements to minimize SIE processing load
	video_ctrl_param.func = 0;
	
	ret = hd_videoproc_set(video_proc_ctrl, HD_VIDEOPROC_PARAM_CTRL, &video_ctrl_param);
	if (ret != HD_OK) {
		printf("WARNING: Failed to disable enhancements: %d\n", ret);
	} else {
		printf("Video enhancements: DISABLED (for SIE stability)\n");
	}
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	if (p_dim != NULL) { //if videoproc is already binding to dest module, not require to setting this!
		...
		// Use direct mode instead of low latency to reduce SIE load
		video_out_param.func = 0;  // Disable extra processing to fix SIE behavior errors
		...
		if (ret != HD_OK) {
			printf("WARNING: Failed to set video proc output params: %d\n", ret);
			return ret;
		}
	}
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
		...
		printf("enc_type=%d, bitrate=%u, resolution=%dx%d\r\n", 
			enc_type, bitrate, p_dim->w, p_dim->h);
		...
		if (enc_type == 0) {
			...
			video_out_param.h26x.gop_num       = 30;  // Increased GOP for better compression
			...
			rc_param.cbr.init_i_qp       = 28;  // Adjusted for better quality
			rc_param.cbr.min_i_qp        = 15;  // Raised minimum QP
			rc_param.cbr.max_i_qp        = 48;  // Raised maximum QP
			rc_param.cbr.init_p_qp       = 28;
			rc_param.cbr.min_p_qp        = 15;
			rc_param.cbr.max_p_qp        = 48;
			...
			rc_param.cbr.frame_rate_base = 30;
			rc_param.cbr.frame_rate_incr = 1;
			rc_param.cbr.init_i_qp       = 28;
			rc_param.cbr.min_i_qp        = 15;
			rc_param.cbr.max_i_qp        = 48;
			rc_param.cbr.init_p_qp       = 28;
			rc_param.cbr.min_p_qp        = 15;
			rc_param.cbr.max_p_qp        = 48;
			rc_param.cbr.static_time     = 4;
			...
			video_out_param.jpeg.image_quality = 70;  // Increased quality from 50 to 70
			ret = hd_videoenc_set(video_enc_path, HD_VIDEOENC_PARAM_OUT_ENC_PARAM, &video_out_param);
			if (ret != HD_OK) {
				printf("set_enc_param_out = %d\r\n", ret);
				return ret;
...
static void *encode_thread(void *arg)
{
	...
	// Validate physical addresses before mapping
	if (phy_buf_main.buf_info.phy_addr == 0 || phy_buf_main.buf_info.buf_size == 0) {
		printf("ERROR: Invalid main buffer info\n");
		return (void *)-1;
	}
	...
	if (vir_addr_main == 0) {
		printf("ERROR: Failed to mmap main buffer\n");
		return (void *)-1;
	}
	
	if (phy_buf_sub.buf_info.phy_addr == 0 || phy_buf_sub.buf_info.buf_size == 0) {
		printf("ERROR: Invalid sub buffer info\n");
	...
	if (vir_addr_sub == 0) {
		printf("ERROR: Failed to mmap sub buffer\n");
		hd_common_mem_munmap((void *)vir_addr_main, phy_buf_main.buf_info.buf_size);
		return (void *)-1;
	}
	
	if (phy_buf_third.buf_info.phy_addr == 0 || phy_buf_third.buf_info.buf_size == 0) {
		printf("ERROR: Invalid third buffer info\n");
		hd_common_mem_munmap((void *)vir_addr_main, phy_buf_main.buf_info.buf_size);
	...
	if (vir_addr_third == 0) {
		printf("ERROR: Failed to mmap third buffer\n");
		hd_common_mem_munmap((void *)vir_addr_main, phy_buf_main.buf_info.buf_size);
		hd_common_mem_munmap((void *)vir_addr_sub, phy_buf_sub.buf_info.buf_size);
		return (void *)-1;
	}
	
	if (phy_buf_fourth.buf_info.phy_addr == 0 || phy_buf_fourth.buf_info.buf_size == 0) {
		printf("ERROR: Invalid fourth buffer info\n");
		hd_common_mem_munmap((void *)vir_addr_main, phy_buf_main.buf_info.buf_size);
		hd_common_mem_munmap((void *)vir_addr_sub, phy_buf_sub.buf_info.buf_size);
		hd_common_mem_munmap((void *)vir_addr_third, phy_buf_third.buf_info.buf_size);
		return (void *)-1;
	}
	...
	if (vir_addr_fourth == 0) {
		printf("ERROR: Failed to mmap fourth buffer\n");
		hd_common_mem_munmap((void *)vir_addr_main, phy_buf_main.buf_info.buf_size);
		hd_common_mem_munmap((void *)vir_addr_sub, phy_buf_sub.buf_info.buf_size);
		hd_common_mem_munmap((void *)vir_addr_third, phy_buf_third.buf_info.buf_size);
		return (void *)-1;
	...
	printf("Buffer mapping successful - Main: %p, Sub: %p, Third: %p, Fourth: %p\n",
		(void*)vir_addr_main, (void*)vir_addr_sub, (void*)vir_addr_third, (void*)vir_addr_fourth);
	//----- open output files -----
		...
		if (HD_OK == hd_videoenc_poll_list(poll_list, 2, -1)) {    // multi path poll_list , -1 = blocking mode
			if (TRUE == poll_list[0].revent.event) {
				...
				if (ret == HD_OK) {
					...
					if (data_pull.pack_num > MAX_BITSTREAM_NUM * 10) {
						printf("WARNING: Unexpected pack_num=%u, skipping frame\n", data_pull.pack_num);
						hd_videoenc_release_out_buf(p_stream0->enc_path, &data_pull);
					} else {
						for (j=0; j< data_pull.pack_num; j++) {
							...
							if (ptr == NULL || len == 0 || len > phy_buf_main.buf_info.buf_size) {
								printf("WARNING: Invalid buffer ptr=%p len=%u\n", ptr, len);
								continue;
							}
			...
			if (TRUE == poll_list[1].revent.event) {
				...
				if (ret == HD_OK) {
					// Validate data before processing
					if (data_pull.pack_num > MAX_BITSTREAM_NUM * 10) {
						printf("WARNING: Unexpected sub pack_num=%u, skipping frame\n", data_pull.pack_num);
						hd_videoenc_release_out_buf(p_stream1->enc_path, &data_pull);
					} else {
						for (j=0; j< data_pull.pack_num; j++) {
							...
							// Validate pointer and size
							if (ptr == NULL || len == 0 || len > phy_buf_sub.buf_info.buf_size) {
								printf("WARNING: Invalid sub buffer ptr=%p len=%u\n", ptr, len);
								continue;
							}
