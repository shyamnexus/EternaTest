...
MAIN(argc, argv)
{
    ...
    char out_file1[64],in_file1[64],in_file2[64],in_file3[64];
    FILE  *fd;
    UINT32 file_size = 0;
    int output_size = 512*376*2;
    MEM_RANGE share_mem[4] = {0};
	IVE_SRC_IMAGE_S  src_img_info1 = {0};
	IVE_SRC_IMAGE_S  src_img_info2 = {0};
    IVE_SRC_IMAGE_S  src_img_info3 = {0};
	...
	//ive_load_input1;
    snprintf(in_file1, 64, "//mnt//sd//input//rgb0_512x376.bin");
    file_size = ive_load_file(in_file1, share_mem[0].va);
    if (file_size == 0) {
		printf("load rgb0_512x376.bin : %s\r\n", in_file1);
		return 0;
    }
	
	//ive_load_input2;
	snprintf(in_file2, 64, "//mnt//sd//input//rgb1_512x376.bin");
	file_size = ive_load_file(in_file2, share_mem[1].va);
	if (file_size == 0) {
		printf("load rgb1_512x376.bin : %s\r\n", in_file2);
		return 0;
	}
    
	//ive_load_input3;
	snprintf(in_file3, 64, "//mnt//sd//input//rgb2_512x376.bin");
	file_size = ive_load_file(in_file3, share_mem[2].va);
	if (file_size == 0) {
		printf("load rgb2_512x376.bin : %s\r\n", in_file3);
		return 0;
	}
	...
	src_img_info1.enDataType = LIB_IVE_DATA_TYPE_U8C1;
	src_img_info1.u64Pa      = share_mem[0].addr;
	src_img_info1.u64Va      = share_mem[0].va;
	src_img_info1.u32Width   = 512;
	src_img_info1.u32Height  = 376;
    src_img_info1.u32Stride  = 512;

	src_img_info2.enDataType = LIB_IVE_DATA_TYPE_U8C1;
	src_img_info2.u64Pa 	 = share_mem[1].addr;
	src_img_info2.u64Va 	 = share_mem[1].va;
	src_img_info2.u32Width	 = 512;
	src_img_info2.u32Height  = 376;
	src_img_info2.u32Stride  = 512;

    src_img_info3.enDataType = LIB_IVE_DATA_TYPE_U8C1;
	src_img_info3.u64Pa 	 = share_mem[2].addr;
	src_img_info3.u64Va 	 = share_mem[2].va;
	src_img_info3.u32Width	 = 512;
	src_img_info3.u32Height  = 376;
	src_img_info3.u32Stride  = 512;

	dst_img_info.enDataType = LIB_IVE_DATA_TYPE_U8C1;
	dst_img_info.u64Pa      = share_mem[3].addr;
	dst_img_info.u64Va      = share_mem[3].va;
	dst_img_info.u32Width   = 512;
	dst_img_info.u32Height  = 376;
    dst_img_info.u32Stride  = 512*3;
	output_size = dst_img_info.u32Stride * dst_img_info.u32Height;
 
	hd_common_mem_flush_cache((VOID *)src_img_info1.u64Va, src_img_info1.u32Stride*src_img_info1.u32Height);
	hd_common_mem_flush_cache((VOID *)src_img_info2.u64Va, src_img_info2.u32Stride*src_img_info2.u32Height);
    hd_common_mem_flush_cache((VOID *)src_img_info3.u64Va, src_img_info3.u32Stride*src_img_info3.u32Height);
	vos_perf_mark(&hw_tick_begin);
	lib_ive_ret = NVT_IVE_Planar_To_Pack(&handle, &src_img_info1, &src_img_info2, &src_img_info3, &dst_img_info, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Planar_To_Pack time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end));
    if(!instant) {
        query_info.handle = handle;
    	query_info.instant = 0;
        query_info.job_finish = 0;
    	if (lib_ive_ret != LIB_IVE_OK) {
    		printf("err:NVT_IVE_Planar_To_Pack error %d\r\n",lib_ive_ret);
    		goto exit;
    	}
    	while (1) {
    ...
    snprintf(out_file1, 64, "//mnt//sd//output//planar_to_pack_output.bin");
	fd = fopen(out_file1, "wb");
	if (!fd) {
		printf("cannot open %s\r\n", out_file1);
		return 0;
	}
	file_size = fwrite((const void *)share_mem[3].va,1,output_size,fd);
	fclose(fd);
