...
static void *relay_yuv_thread(void *arg)
{
	...
	while (p_stream0->proc_exit == 0) {
		SEM_WAIT(p_stream0->sem_yuv_section);
		{
			//pull data
		...
		SEM_SIGNAL(p_stream0->sem_yuv_section);
...
MAIN(argc, argv)
{
	...
	// create sem_yuv_section (critical section to protect push_yuv & flow control)
	SEM_CREATE(stream[0].sem_yuv_section, 1);
	...
	SEM_WAIT(stream[0].sem_yuv_section);
	...
	SEM_SIGNAL(stream[0].sem_yuv_section);
	...
	SEM_DESTROY(stream[0].sem_yuv_section);
exit:
