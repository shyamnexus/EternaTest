...
MAIN(argc, argv)
{
	...
	filter_ctrl.u8Mask[0] = 0xc;
	filter_ctrl.u8Mask[1] = 0xa;
	filter_ctrl.u8Mask[2] = 0x2;
	filter_ctrl.u8Mask[3] = 0xc;
	filter_ctrl.u8Mask[4] = 0x8;	
	filter_ctrl.u8Mask[5] = 0x7;
	filter_ctrl.u8Mask[6] = 0xd;
	filter_ctrl.u8Mask[7] = 0x6;
	filter_ctrl.u8Mask[8] = 0x7;
	filter_ctrl.u8Mask[9] = 0x9;	
	filter_ctrl.in_fmt    = 0;
	...
	lib_ive_ret = NVT_IVE_Filter(&handle, &src_img_info, &dst_img_info, &filter_ctrl, instant);
    vos_perf_mark(&hw_tick_end);
	printf("NVT_IVE_Filter time = %d\r\n", vos_perf_duration(hw_tick_begin, hw_tick_end)); 
	if (lib_ive_ret != LIB_IVE_OK) {
		printf("err:NVT_IVE_Filter error %d\r\n",lib_ive_ret);
		goto exit;
	}
    if(!instant) {
...
#if defined(__FREERTOS)
	snprintf(out_file1, 64, "A:\\IVEP\\output\\filter_output.bin");
#else
    snprintf(out_file1, 64, "//mnt//sd//IVEP//output//filter_output.bin");
#endif
