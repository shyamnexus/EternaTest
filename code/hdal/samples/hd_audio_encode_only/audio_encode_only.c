...
static void *feed_frm_thread(void *arg)
{
	...
	UINTPTR pa[MAX_FRM_BLK_CNT] = {0}, va[MAX_FRM_BLK_CNT] = {0};
	...
	while (p_stream0->feed_exit == 0) {
		...
		if (va[blk_idx] == 0) { // if NOT mmap yet, mmap it
			va[blk_idx] = (UINTPTR)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, pa[blk_idx], blk_size); // Get virtual addr
			if (va[blk_idx] == 0) {
