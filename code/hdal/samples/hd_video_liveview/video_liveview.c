...
static void display_edid(HD_PATH_ID video_out_ctrl)
{
	...
	if(ret != 0)
	{
		printf("fail to query edid\n");
		return;
	}

	printf("hdmi edid count=%d\n", hdmi_ability.len);
	for (i = 0; i < hdmi_ability.len; i++) {
		printf("Video Ability %d: %d\r\n", (int)((UINT32)i), (int)((UINT32)hdmi_ability.video_id[i]));
	}
}
...
MAIN(argc, argv)
{
	...
	printf("Enter e to show HDMI EDID\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
			// quit program
			...
			display_edid(stream[0].out_ctrl);
		}
