...
MAIN(argc, argv)
{
	...
	if (argc > 2) {
		...
		if (argv[1][0] == '?') {
			printf("Usage: <pat_mode> <pat_value> <out_type>\r\n");
			printf("Help:\r\n");
			printf("  <pat_mode>   : 1(COLOR BAR), 2(RANDOM), 3(FIXED), 4(HINCREASE), 5(HVINCREASE)\r\n");
			printf("  <pat_value>\r\n");
			printf("  <out_type>   : 0(CVBS), 1(LCD), 34(HDMI 1080P30), 16(HDMI 1080P60), 5(HDMI 1080I60)\r\n");
			return 0;
		}
		pat_mode = atoi(argv[1]);
		pat_value = 0;
