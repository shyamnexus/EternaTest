...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	// query model info for WORKBUF/RONLYBUF size , then alloc WORKBUF/RONLYBUF
	{
		...
		if (ret != HD_OK) {
			printf("WARNING: net_path(%u) vendor_ai3_dev_get(MODEL_INFO) fail=%d (chip ID check disabled, continuing anyway)\n", net_path, ret);
			// Bypass chip ID check - continue execution despite error
			ret = HD_OK;
		}
