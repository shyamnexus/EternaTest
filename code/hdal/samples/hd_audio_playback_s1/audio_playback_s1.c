...
static HD_RESULT set_out_cfg(HD_PATH_ID *p_audio_out_ctrl)
{
	...
	/*set audio out power always ON control*/
	pwr_en = TRUE;
...
static void *playback_thread(void *arg)
{
	...
	UINTPTR pa, va, bs_buf_start, bs_buf_curr, bs_buf_end;
...
{
	printf("======version: %s\n",SAMPLE_VERSION);
	printf("Usage: ./hd_audio_playback_s1 <dec_type> <suspend_sec>\n");
	printf("  <dec_type>   : 0(AAC), 1(ULAW), 2(ALAW), 3 or empty (PCM)\n");
...
EXAMFUNC_ENTRY(hd_audio_playback, argc, argv)
{
	...
	UINT32 dec_type = 3, suspend_sec = 10;
	char rtc_cmd[128] = {0};
...
again:
	...
	if(key == 's'){
		...
		printf("hd_audio_playback_s1 is terminating...\n");
	}
