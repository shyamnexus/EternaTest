...
#if !defined(__FREERTOS)
unsigned short arb_chksum(DDR_ARB id, uintptr_t addr, unsigned long len)
{
	ARB_CHKSUM chksum = {
...
{
	uintptr_t phy_addr = 0x100000; // must be word alignment
	unsigned long len = 0x100000; // must be word alignment
	unsigned short sum = arb_chksum(0, phy_addr, len);
	printf("the sum value on phy_addr(0x%08lX) with len(0x%08lX) is 0x%08X.\n", (unsigned long)phy_addr, (unsigned long)len, sum);
	return 0;
}
