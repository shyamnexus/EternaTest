...
#if EXT_CODEC
static HD_RESULT set_out_cfg(HD_CTRL_ID ctrl_id, HD_PATH_ID *p_audio_out_ctrl, HD_AUDIO_SR sample_rate, UINT32 extcodec)
#else
...
static HD_RESULT open_module(AUDIO_OUTONLY *p_outonly)
{
	HD_RESULT ret;
#if EXT_CODEC
	ret = set_out_cfg(HD_AUDIOOUT_0_CTRL, &p_outonly->out_ctrl, p_outonly->sample_rate_max, p_outonly->extcodec);
#else
...
#if EXT_CODEC
	if (p_outonly->extcodec) {
		ret = set_out_cfg(HD_AUDIOOUT_2_CTRL, &p_outonly->out_ctrl2, p_outonly->sample_rate_max, p_outonly->extcodec);
		if (ret != HD_OK) {
			printf("set out-cfg fail\n");
			return HD_ERR_NG;
		}
		if((ret = hd_audioout_open(HD_AUDIOOUT_2_IN_0, HD_AUDIOOUT_2_OUT_0, &p_outonly->out_path2)) != HD_OK)
			return ret;
	} else {
		ret = set_out_cfg(HD_AUDIOOUT_1_CTRL, &p_outonly->out_ctrl2, p_outonly->sample_rate_max, p_outonly->extcodec);
		if (ret != HD_OK) {
			printf("set out-cfg fail\n");
			return HD_ERR_NG;
		}
...
#if EXT_CODEC
	if (argc > 1){
		outonly.extcodec = atoi(argv[1]);
		if (outonly.extcodec)
			printf("test extcodec\n");
	}
#endif
