...
********************************************************************/
#define HD_VIDEOENC_MAX_IN         128   ///< max count of input of this device (interface)
#define HD_VIDEOENC_MAX_OUT        128   ///< max count of output of this device (interface)
...
typedef enum _HD_VIDEOENC_RC_MODE {
	...
	HD_RC_MODE_CVBR,        ///< Constrained VariableBit Rate
	HD_RC_MAX,
...
typedef struct _HD_H26XENC_CBR3 {
	UINT32          bitrate;            ///< Bit rate (bits per second)
	UINT32          frame_rate_base;    ///< Frame rate
	UINT32          frame_rate_incr;    ///< Frame rate = uiFrameRateBase / uiFrameRateIncr
	UINT8           init_i_qp;          ///< Rate control's init I qp.        default: 26, range: 0~51
	...
	INT32           motion_aq_str;      ///< Motion aq strength for smart ROI. default: 0, range: -15~15
	UINT32          max_frame_size;     ///< max frame size. Default: 0, 0: limited by buffer size. Others: max size limited (byte)
	UINT32          br_tolerance;       ///< Bitrate tolerance. default 2: range: 0~6
} HD_H26XENC_CBR3;
...
typedef struct _HD_H26XENC_VBR3 {
	UINT32          bitrate;            ///< Bit rate (bits per second)
	UINT32          frame_rate_base;    ///< Frame rate
	UINT32          frame_rate_incr;    ///< Frame rate = uiFrameRateBase / uiFrameRateIncr
	UINT8           init_i_qp;          ///< Rate control's init I qp.        default: 26, range: 0~51
...
typedef struct _HD_H26XENC_EVBR3 {
	UINT32          bitrate;            ///< Bit rate (bits per second)
	UINT32          frame_rate_base;    ///< Frame rate
	UINT32          frame_rate_incr;    ///< Frame rate = uiFrameRateBase / uiFrameRateIncr
	UINT8           init_i_qp;          ///< Rate control's init I qp.        default: 26, range: 0~51
...
typedef struct _HD_H26XENC_CVBR {
	UINT32          bitrate;            ///< Bit rate (bits per second)
	UINT32          frame_rate_base;    ///< Frame rate
	UINT32          frame_rate_incr;    ///< Frame rate = uiFrameRateBase / uiFrameRateIncr
	UINT8           init_i_qp;          ///< Rate control's init I qp.        default: 26, range: 0~51
	...
	UINT32          max_frame_size;     ///< Max frame size. Default: 0, 0: limited by buffer size. Others: max size limited (byte)
	UINT32          br_tolerance;       ///< Bitrate tolerance. default 2: range: 0~6
	UINT32          lt_min_bitrate;     ///< Long term minimal bitrate
	UINT32          st_max_bitrate;     ///< Short term maximal bitrate
	UINT32          lt_measure_period;  ///< Long term measure period (unit minute)
} HD_H26XENC_CVBR;
...
typedef struct _HD_H26XENC_RATE_CONTROL3 {
	HD_VIDEOENC_RC_MODE    rc_mode;      ///< rate control mode. default: 1, range: 1~4 (1: CBR, 2: VBR, 3: FixQP, 4: EVBR)
	union {
		HD_H26XENC_CBR3    cbr;          ///< parameter of rate control mode CBR
		HD_H26XENC_VBR3    vbr;          ///< parameter of rate control mode VBR
		HD_H26XENC_FIXQP   fixqp;        ///< parameter of rate control mode FixQP
		HD_H26XENC_EVBR3   evbr;         ///< parameter of rate control mode EVBR
		HD_H26XENC_CVBR    cvbr;         ///< parameter of rate control mode CVBR
	};
} HD_H26XENC_RATE_CONTROL3;

typedef struct _HD_JPEGENC_RATE_CONTROL {
	HD_VIDEOENC_RC_MODE    rc_mode;         ///< rate control mode. default: 1, range: 1~4 (1: CBR, 2: VBR, 3: FixQP, 4: EVBR)
	UINT32                 base_quality;    ///< JPEG image quality. default: 50, range 1~100
	UINT32                 min_quality;     ///< JPEG min image quality. default: 1, range 1~100
	UINT32                 max_quality;     ///< JPEG max image quality. default: 100, range 1~100
	UINT32                 bitrate;         ///< Bit rate (bits per second)
	UINT32                 frame_rate_base; ///< Frame rate
	UINT32                 frame_rate_incr; ///< Frame rate = uiFrameRateBase / uiFrameRateIncr
} HD_JPEGENC_RATE_CONTROL;
...
/************ jpeg roi ************/
#define HD_JPEGENC_ROI_WIN_COUNT  11
typedef struct _HD_JPEGENC_ROI_WIN {
	BOOL                enable;         ///< enable jpeg roi. default: 0, range: 0~1 (0: disable, 1: enable)
	HD_URECT            rect;           ///< rectangle of jpeg roi
	UINT32              roi_threshold;  ///< threshold of jpeg roi. default: 0, range: 0~15.
} HD_JPEGENC_ROI_WIN;

typedef struct _HD_JPEGENC_ROI {
	HD_JPEGENC_ROI_WIN  st_roi[HD_JPEGENC_ROI_WIN_COUNT];   ///< jpeg roi window settings.
} HD_JPEGENC_ROI;
...
typedef struct _HD_H26XENC_AQ3 {
	BOOL            enable;             ///< AQ enable. default: 0, range: 0~1 (0: disable, 1: enable)
	UINT8           i_str;              ///< aq strength of I frame. default: 3, range: 1~8
	UINT8           p_str;              ///< aq strength of P frame. default: 1, range: 1~8
	INT8            max_delta_qp;       ///< max delta qp of aq.
	...
	INT16           thd_table[HD_H26XENC_AQ_MAP_TABLE_NUM];
										///< non-linear AQ mapping table. range: -512~511, default: {-120,-112,-104, -96, -88, -80, -72, -64, -56, -48, -40, -32, -24, -16, -8, 7, 15, 23, 31, 39,47, 55, 63, 71, 79, 87, 95, 103, 111, 119}
										///<           for ( dqp = -15; dqp < 15; dqp++ ) if ( Cu.RelativeTextureComplexity(x_str) <= thd_table[dqp+15] ) break; Cu.DeltaQP_AQ = MIN ( MAX (min_delta_qp, dqp), max_delta_qp );
	UINT8           i_str1;             /// < aq strength of I frame of CU32. default: 3(same as i_str), range: 1~8, HEVC only
	UINT8           p_str1;             ///<  aq strength of P frame of CU32. default: 3(same as i_str), range: 1~8, HEVC only
	UINT8           i_str2;             ///<  aq strength of I frame of CU16. default: 1, range: 1~8, HEVC only
	UINT8           p_str2;             ///<  aq strength of P frame of CU16. default: 1, range: 1~8, HEVC only
} HD_H26XENC_AQ3;
...
typedef enum _HD_VIDEOENC_PARAM_ID {
	...
	HD_VIDEOENC_PARAM_OUT_RATE_CONTROL3,  ///< support get/set with i/o path, using HD_H26XENC_RATE_CONTROL3 struct
	HD_VIDEOENC_PARAM_OUT_AQ3,            ///< support get/set with i/o path, using HD_H26XENC_AQ3 struct
	HD_VIDEOENC_PARAM_OUT_JPEG_RATE_CONTROL, ///< support get/set with i/o path, using HD_JPEGENC_RATE_CONTROL struct
	HD_VIDEOENC_PARAM_OUT_JPEG_ROI,       ///< support get/set with i/o path, using HD_JPEGENC_ROI struct
