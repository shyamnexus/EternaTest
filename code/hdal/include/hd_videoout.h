...
********************************************************************/
#define HD_VIDEOOUT_MAX_IN  128 ///< max count of input of this device (interface)
...
typedef enum _HD_VIDEOOUT_HDMI_ID {
	...
	HD_VIDEOOUT_HDMI_3840X2160P60       = 101,  ///< HDMI VIDEO FORMAT IS 3840X2160 & PROGRESSIVE 60FPS
...
typedef struct _HD_VIDEOOUT_MODE3 {
	HD_COMMON_VIDEO_OUT_TYPE output_type;    ///< select lcd output device
	HD_VIDEOOUT_INPUT_DIM input_dim;         ///< NVR only.set input dim(IPC set input dim by HD_VIDEOOUT_PARAM_IN)
	union {
		HD_VIDEOOUT_LCD_ID lcd;    ///< set lcd  output resolution
	...
	INT hdmi_id;                              ///< select hdmi controller id
} HD_VIDEOOUT_MODE3;

typedef struct _HD_VIDEOOUT_WB_INFO {
	UINT32 wb_en;
	UINT32 wb_uvswap;  /* 0: VU, 1: UV */
	UINT32 wb_one_shot;/* 0: free-run, 1: one-shot */
	UINT32 wb_sel;     /* 0: ch0~ch2+cursor, 1: ch0~ch2, 2: ch0 */
    UINT16 wb_lx;   	/* write-back x start pixel (two-pixel alignment), Left x, for SDO1 */
    UINT16 wb_ty;   	/* write-back y start line, Top Y, for SDO1 */
    UINT16 wb_rx;   	/* write-back x start pixel (two-pixel alignment), Right x, for SDO1 */
    UINT16 wb_by;   	/* write-back y start line, Bottom Y, for SDO1 */
    UINT16 wb_width;	/* outer size, the inner size is SDO1 destination size */
    UINT16 wb_height;	/* outer size, the inner size is SDO1 destination size */
	UINT32 wb_fmt;     /* 0: 422, 1: 420, 2: 420SCE */
    uintptr_t y_addr;   	/* physical base address */
    uintptr_t uv_addr;   	/* physical base address */
	UINT32 rev[4];
} HD_VIDEOOUT_WB_INFO;
...
typedef struct _HD_FB_ATTR3 {
	HD_FB_ID   fb_id;      ///< select fb layer
	UINT8   alpha_blend;   ///< 0~255,except ARBG format,ARBG format would refer pixel alpha,lcd0:support fb0~2,lcd_1/lcd_2:fb0~1
	UINT8   alpha_1555;    ///< 0~255,only for ARBG1555 format,the pixel alpha 1 would refer the value
	BOOL    colorkey_en;   ///< 0:disable/1:enable
	UINT16  y_ckey_l;      ///< y value, low, keying value = low (y, u, v) to high (y, u, v)
	UINT16  y_ckey_h;      ///< y value, high
	UINT16  u_ckey_l;      ///< u value, low
	UINT16  u_ckey_h;      ///< u value, high
	UINT16  v_ckey_l;      ///< v value, low
	UINT16  v_ckey_h;      ///< v value, high
} HD_FB_ATTR3;
...
typedef enum _HD_VIDEOOUT_REG_MODE {
	HD_SINGLE_REG = 0,		///< videoout displayed to the hardware according to the single area configuration
	HD_MULTI_REG = 1,		///< videoout displayed to the hardware according to the multi-region configuration
	ENUM_DUMMY4WORD(HD_VIDEOOUT_REG_MODE)
} HD_VIDEOOUT_REG_MODE;

typedef struct _HD_VIDEOOUT_MULTI_REGION {
	HD_VIDEOOUT_REG_MODE mode;
} HD_VIDEOOUT_MULTI_REGION;
...
typedef enum _HD_VIDEOOUT_PARAM_ID {
	...
	HD_VIDEOOUT_PARAM_MODE3,                  ///< support set with ctrl path, using HD_VIDEOOUT_MODE3 struct
	HD_VIDEOOUT_PARAM_FB_ATTR3,               ///< support get/set with ctrl path,  using HD_FB_ATTR3 struct
	HD_VIDEOOUT_PARAM_WRITEBACK,              ///< support get/set with i path,  using HD_VIDEOOUT_WB_INFO struct
	HD_VIDEOOUT_PARAM_MULTI_REGION,           ///< support get/set with ctrl path, using HD_VIDEOOUT_MULTI_REGION struct
	///Common End//
