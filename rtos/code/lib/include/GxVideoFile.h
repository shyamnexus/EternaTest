...
typedef struct _IMG_BUF {
	...
	uintptr_t           PxlAddr[MAX_PLANE_NUM];    ///< pixel address
...
typedef struct _GXVIDEO_INFO {
	...
	uintptr_t        uiH264DescAddr;         ///<[out] H264 SPS/PPS data address
...
/**
    ...
    @return ER
*/
typedef  ER(* FPGXVIDEO_READCB)(UINT32 pos, UINT32 size, uintptr_t addr);

typedef  ER(* FPGXVIDEO_READCB64)(UINT64 pos, UINT32 size, uintptr_t addr);
...
*/
typedef  ER(* FPGXVIDEO_GETHUMBCB)(uintptr_t userdata_buf, UINT32 userdata_size, UINT64 *thumb_pos, UINT32 *thumb_size);
