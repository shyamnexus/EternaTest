...
*/
typedef void (MediaRecDrawStr)(UINT32 uiVidEncId, ULONG yAddr, ULONG cbAddr, ULONG crAddr, UINT32 lineoftY, UINT32 drawWidth);//2010/09/07 Meg Lin
...
*/
typedef INT32(*FSWRITECB)(ULONG Addr, UINT32 Size, UINT32 Fmt, UINT32 uiPathID);
...
*/
typedef struct {
	ULONG               uiQVMemAddr;      ///< [in]  quick view output addr
...
typedef struct {
	...
	ULONG               uiUserDataBuf;    ///< user data buffer
...
*/
typedef struct {
	ULONG  dataoutAddr;         ///< [in] address to put nidx data
	...
	ULONG  thisVideoFrameData;  ///< [in] this second 1st video frame entry addr
	...
	ULONG  h264descData;        ///< [in] h264 desc data
...
typedef struct {
	...
	ULONG  thisVideoFrameData;  ///< [in] this second 1st video frame entry addr
	UINT32 thisVideoFrmDataLen; ///< [in] this second video frame info len
	ULONG  thisAudioFrameData;  ///< [in] this second audio data starting fileoffset
...
typedef struct {
	...
	ULONG  thisSubVideoFrameData;  ///< [in] this second 1st video frame entry addr
	...
	ULONG  Subh264descData;        ///< [in] h264 desc data
...
*/
typedef struct {
	ULONG  outAddr;                     ///< [in] output address
...
typedef struct {
	ULONG  bufAddr;             ///< [in] address to put gps data
	UINT32 clusterSize;         ///< [in] this tag size
	ULONG  gpsDataAddr;         ///< [in] gpsData addr
...
typedef struct {
	...
	ULONG               uiFre1Addr;         ///< thumbnail bit-stream starting address
	...
	ULONG               uiUdataAddr;        ///< [AVI] user data starting address
...
typedef struct {
	...
	ER(*SetMemBuf)(UINT32 idnum, ULONG startAddr, UINT32 size);   ///< set memory to fileformat container
	ER(*MakeHeader)(ULONG hdrAddr, UINT32 *pSize, MEDIAREC_HDR_OBJ *pobj);  ///< make front-header using fileformat maker
	ER(*UpdateHeader)(UINT32 idnum, void *pobj);      ///< make end-header using fileformat maker
	ER(*GetInfo)(MEDIAWRITE_GETINFO_TYPE type, VOID *pparam1, VOID *pparam2, VOID *pparam3);  ///< get information from media fileformat container.
	ER(*SetInfo)(MEDIAWRITE_SETINFO_TYPE type, ULONG param1, ULONG param2, ULONG param3);        ///< set information to media fileformat container.
	ER(*CustomizeFunc)(UINT32 type, void *pobj);     ///< custimized function if neeeded
	ER(*cbWriteFile)(UINT8 *type, UINT64 pos, UINT32 size, ULONG addr);  ///< a file-writing function for fileformat maker
	ER(*cbReadFile)(UINT8 *type, UINT64 pos, UINT32 size, ULONG addr);   ///< a file-reading function for fileformat maker
...
typedef struct {
	...
	ER(*CustomFunc)(UINT32 filetype, ULONG p1, ULONG p2, ULONG p3);
	void (*SetInfo)(MEDIANAMING_SETINFO_TYPE type, ULONG p1, ULONG p2, ULONG p3);
	UINT32 checkID;                                  ///< checkID for fileformat maker to check valid type
