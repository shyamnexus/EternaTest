...
typedef enum _FWSRV_ER {
	...
	FWSRV_ER_DECRYPT           = -37, ///< failed to decrypt
...
typedef enum _FWSRV_CMD_IDX {
    ...
    FWSRV_CMD_IDX_DECRYPT,          ///< Input: MEM_RANGE (encrypted data)
...
typedef struct _FWSRV_BIN_UPDATE_ALL_IN_ONE {
	uintptr_t uiSrcBufAddr;    ///< all-in-one bin context
...
typedef struct _FWSRV_FASTLOAD {
	...
	UINT32    DummyPrefixLen;///< normal is zero. for some case, custom add some bytes in the front of gz-linux, fwsrv need to skip it
} FWSRV_FASTLOAD;

typedef enum _FWSRV_DECRYPT_SIGNATURE {
	FWSRV_DECRYPT_SIGNATURE_NONE  = 0, ///< no signature
	FWSRV_DECRYPT_SIGNATURE_AES128,    ///< signature by aes128
	FWSRV_DECRYPT_SIGNATURE_RSA2048,   ///< signature by rsa2048
	FWSRV_DECRYPT_SIGNATURE_ECDSA,   ///< signature by ecdsa
	ENUM_DUMMY4WORD(FWSRV_DECRYPT_SIGNATURE)
} FWSRV_DECRYPT_SIGNATURE;

typedef struct _FWSRV_DECRYPT {
	FWSRV_DECRYPT_SIGNATURE Signature;
	MEM_RANGE EncryptedMem; ///< temp buffer to encrypted data
} FWSRV_DECRYPT;
