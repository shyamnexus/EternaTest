...
INT32 vendor_dma_reset_data_count(int dram)
{
	CNT_INFO cnt_info = {0};
	INT32 mau_fd = -1;
	INT32 ret;
	...
	ret = ioctl(mau_fd, DDR_IOC_RESET_CNT, &cnt_info);
	if (ret < 0) {
		printf("[VENDOR_MAU] reset count fail!\n");
		return ret;
	}
...
UINT64 vendor_dma_get_data_count(int dram)
{
	CNT_INFO cnt_info = {0};
	INT32 ret;
	INT32 mau_fd = -1;
	...
	ret = ioctl(mau_fd, DDR_IOC_GET_CNT, &cnt_info);
	if (ret < 0) {
		printf("[VENDOR_MAU] get count fail!\n");
		return ret;
	}
