...
typedef struct _AE_SHDR_METER {
	UINT32 shdr_meter_mode;                        ///0: mix mode, 1: fusion mode
	UINT32 mix_weight;                             ///for mix mode only, range : 0~100
	UINT32 fusion_adjust_ratio;                    ////for Fusion mode only, range: 0~255
	UINT32 la_fusion_curve[65];                    // for Fusion mode only
	UINT32 tab_reserved0[AEALG_DYNAMIC_LV_NUM];
...
typedef struct _AE_STATUS_INFO {
	...
	UINT32 aperture;
	UINT32 sensor_gain[ISP_SEN_MFRAME_MAX_NUM];    ///< range : 100~3276800
	UINT32 dgain[ISP_SEN_MFRAME_MAX_NUM];          ///< range : 128~32767
} AE_STATUS_INFO;
