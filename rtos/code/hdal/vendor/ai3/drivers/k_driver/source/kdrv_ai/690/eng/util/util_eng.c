...
{
    p_eng->p_util_reg_st = (NT98690_UTIL_REGISTER_STRUCT *)reg_base_addr;
...
VOID util_eng_trig_ll_hw_reg(UTIL_ENG_HANDLE *p_eng, uintptr_t ll_addr_msb, uintptr_t ll_addr_lsb)
{
    T_UTIL_LL_JOB_REGISTER_0 ll_dma_lsb;
    T_UTIL_LL_JOB_REGISTER_1 ll_dma_msb;
	...
	UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_LL_JOB_REGISTER_0_OFS, ll_dma_lsb.reg);
    UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_LL_JOB_REGISTER_1_OFS, ll_dma_msb.reg);
	//UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_INTERRUPT_ENABLE_REGISTER_OFS, UTIL_ENG_INTERRUPT_LL_ERR); //set ll_error enable befor trigger, and the int_en will update by ll_cmd
...
VOID util_eng_isr_hw_reg(UTIL_ENG_HANDLE *p_eng)
{
	...
	if (status != 0) {
		...
		if (status_inte & UTIL_ENG_INTERRUPT_LL_END) {
			flag |= FLGPTN_UTIL_LL_END;
		}
		if (status & UTIL_ENG_INTERRUPT_LL_ERR) {
			DBG_ERR("%s: UTIL linked-list error\r\n", __func__);
		}
		if (status & UTIL_ENG_INTERRUPT_LL_END) {
...
VOID util_eng_wait_framend_ll(UTIL_ENG_HANDLE *p_eng)
{
    FLGPTN uiflag;
    ER er_return = E_OK;

    er_return = util_eng_platform_flg_wait(p_eng, &uiflag, FLGPTN_UTIL_LLEND);
    if(er_return != E_OK) {
...
UINT32 util_eng_get_ll_cycle(UTIL_ENG_HANDLE *p_eng)
{
