...
{
    p_eng->p_ppu_reg_st = (NT98690_PPU_REGISTER_STRUCT *)reg_base_addr;
...
static INT32 ppu_eng_int_dma_enable_hw_reg(PPU_ENG_HANDLE *p_eng, BOOL set_en)
{
...
INT32 ppu_eng_reset(PPU_ENG_HANDLE *p_eng)
{
    ...
    // DMA disable
    ppu_eng_int_dma_enable_hw_reg(p_eng, FALSE);
    ...
    if(ppu_eng_platform_dma_idle(p_eng)) {
        ...
        // DMA enable
        ppu_eng_int_dma_enable_hw_reg(p_eng, TRUE);
...
VOID ppu_eng_trig_ll_hw_reg(PPU_ENG_HANDLE *p_eng, uintptr_t ll_addr_msb, uintptr_t ll_addr_lsb)
{
...
VOID ppu_eng_wait_framend_ll(PPU_ENG_HANDLE *p_eng)
{
    FLGPTN uiflag;
    ER er_return = E_OK;
...
UINT32 ppu_eng_get_ll_cycle(PPU_ENG_HANDLE *p_eng)
{
