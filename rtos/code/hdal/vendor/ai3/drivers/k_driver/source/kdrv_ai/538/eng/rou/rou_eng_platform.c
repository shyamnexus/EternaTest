...
{
    const UINT32 ROU_MAX_CLK_FREQ = 600;
	...
	if ((rou_eng_platform_get_chip_id()==CHIP_NS02302) || (rou_eng_platform_get_chip_id()==CHIP_NS02402)) {
        if(source_clk == 480) {
			...
			if (nvt_get_chip_id() == CHIP_NS02302) {
				if(source_clk > ROU_MAX_CLK_FREQ) {
					DBG_ERR("%s: input frequency %d convnd to 600\r\n", __func__, source_clk);
					source_clk = ROU_MAX_CLK_FREQ;
				}
			}else{
				if(source_clk > 650) {
	...
	// select clock
	source_clk = p_eng->clock_rate;
	if (rou_eng_platform_get_chip_id() == CHIP_NS02302) {
        if(source_clk >= ROU_MAX_CLK_FREQ) {
            selected_clock = PLL_CLKSEL_NUE30_PLL10;
            if(source_clk > ROU_MAX_CLK_FREQ) {
                DBG_ERR("%s: input frequency %d round to %d\r\n", __func__, source_clk, ROU_MAX_CLK_FREQ);
            }
...
UINT32 rou_eng_platform_get_busy_count(ROU_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    if(rou_eng_platform_get_chip_id()==CHIP_NS02302) {
        DBG_ERR("ROU does not support busy count\n\r");
    } else if(rou_eng_platform_get_chip_id()==CHIP_NS02402) {
        if(p_eng==NULL) {
            DBG_ERR("ROU handle null\r\n");
            return 0;
        }
        if(ll_idx > 1) {
            DBG_ERR("ROU does not support ll_idx %d\n\r", ll_idx);
            return 0;
        }
        if(ll_idx==0) return ROU_ENG_GETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_1_OFS);
        else return ROU_ENG_GETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_2_OFS);
    } 
    return 0;
}

VOID rou_eng_platform_clr_busy_count(ROU_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    T_ROU_BUSY_COUNT_REGISTER_0 ctrl_busy_clr;
    ...
    if(rou_eng_platform_get_chip_id()==CHIP_NS02302) {
        ...
        if(ll_idx==0) {
            ctrl_busy_clr.bit.ROU_LL0_BUSY_COUNT_CLR = 1;
            ROU_ENG_SETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.ROU_LL0_BUSY_COUNT_CLR);
            ctrl_busy_clr.bit.ROU_LL0_BUSY_COUNT_CLR = 0;
            ROU_ENG_SETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.ROU_LL0_BUSY_COUNT_CLR);
        } else {
            ctrl_busy_clr.bit.ROU_LL1_BUSY_COUNT_CLR = 1;
            ROU_ENG_SETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.ROU_LL1_BUSY_COUNT_CLR);
            ctrl_busy_clr.bit.ROU_LL1_BUSY_COUNT_CLR = 0;
            ROU_ENG_SETREG(p_eng->reg_io_base + ROU_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.ROU_LL1_BUSY_COUNT_CLR);
        }
    } 
}
