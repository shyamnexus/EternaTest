...
{
    p_eng->p_cal_reg_st = (NT98635_CAL_REGISTER_STRUCT *)reg_base_addr;
...
VOID cal_eng_trig_ll_hw_reg(CAL_ENG_HANDLE *p_eng, uintptr_t ll_addr_msb, uintptr_t ll_addr_lsb, UINT32 ll_idx)
{
    T_CAL_CONTROL_REGISTER ctl;
    T_CAL_DRAMUB_JOB0_REGISTER_0 ll0_dma_lsb;
    T_CAL_DRAMUB_JOB0_REGISTER_1 ll0_dma_msb;
    T_CAL_DRAMUB_JOB1_REGISTER_0 ll1_dma_lsb;
    T_CAL_DRAMUB_JOB1_REGISTER_1 ll1_dma_msb;
    ...
    case 0:
    	...
    	CAL_ENG_SETREG(p_eng->reg_io_base + CAL_DRAMUB_JOB0_REGISTER_0_OFS, ll0_dma_lsb.reg);
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_DRAMUB_JOB0_REGISTER_1_OFS, ll0_dma_msb.reg);
    ...
    case 1:
    	...
    	CAL_ENG_SETREG(p_eng->reg_io_base + CAL_DRAMUB_JOB1_REGISTER_0_OFS, ll1_dma_lsb.reg);
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_DRAMUB_JOB1_REGISTER_1_OFS, ll1_dma_msb.reg);
...
VOID cal_eng_isr_hw_reg(CAL_ENG_HANDLE *p_eng)
{
	...
	if (status != 0) {
		...
		if (status_inte & CAL_ENG_INTERRUPT_LL0_END) {
			flag |= FLGPTN_CAL_LL0_END;
			status_ll0 = 1;
		}
		if (status & CAL_ENG_INTERRUPT_LL0_ERR) {
			DBG_ERR("CAL linked-list error\r\n");
		}
        if (status_inte & CAL_ENG_INTERRUPT_LL1_END) {
			flag |= FLGPTN_CAL_LL1_END;
			status_ll1 = 1;
		}
		if (status & CAL_ENG_INTERRUPT_LL1_ERR) {
			DBG_ERR("CAL linked-list error\r\n");
		}
		if ((status & CAL_ENG_INTERRUPT_LL0_END) || (status & CAL_ENG_INTERRUPT_LL1_END)) {
...
VOID cal_eng_wait_framend_ll(CAL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    FLGPTN uiflag;
    ER er_return = E_OK;
    ...
    switch(ll_idx) {
    case 0:
        er_return = cal_eng_platform_flg_wait(p_eng, &uiflag, FLGPTN_CAL_LL0END);
        break;
    case 1:
        er_return = cal_eng_platform_flg_wait(p_eng, &uiflag, FLGPTN_CAL_LL1END);
        break;
    default:
        DBG_ERR("CAL wrong LL wait framend option (%d)\n\r", ll_idx);
        return;
    }
    if(er_return != E_OK) {
...
UINT32 cal_eng_get_ll_cycle(CAL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    switch(ll_idx) {
    case 0:
        return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_CYCLE_COUNT_REGISTER_1_OFS);
    case 1:
        return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_CYCLE_COUNT_REGISTER_3_OFS);
    default:
...
UINT32 cal_eng_get_ll_sta_acc_cycle(CAL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    switch(ll_idx) {
    case 0:
        return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_STATISTIC_QOS_REGISTER_0_OFS);
    case 1:
        return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_STATISTIC_QOS_REGISTER_1_OFS);
    default:
        DBG_ERR("CAL wrong get stastic LL acc cycle option (%d)\n\r", ll_idx);
        return 0;
    }
}
...
INT32 cal_eng_set_ll_setting(CAL_ENG_HANDLE *p_eng, CAL_LL_SETTING *ll_param)
{
    T_CAL_JOB_PRIORITY_REGISTER_0 ctl_priority;
    T_CAL_QOS_REGISTER_0 ctl_qos0;
    T_CAL_QOS_REGISTER_1 ctl_qos1;
    T_CAL_STATISTIC_QOS_REGISTER_2 ctl_sta_qos2;
    ...
    CAL_ENG_SETREG(p_eng->reg_io_base + CAL_JOB_PRIORITY_REGISTER_0_OFS, ctl_priority.reg);
    CAL_ENG_SETREG(p_eng->reg_io_base + CAL_QOS_REGISTER_0_OFS, ctl_qos0.reg);
    CAL_ENG_SETREG(p_eng->reg_io_base + CAL_QOS_REGISTER_1_OFS, ctl_qos1.reg);
    CAL_ENG_SETREG(p_eng->reg_io_base + CAL_STATISTIC_QOS_REGISTER_2_OFS, ctl_sta_qos2.reg);

    return TRUE;
}

UINT32 cal_eng_get_qos_setting(CAL_ENG_HANDLE *p_eng)
{
    return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_QOS_REGISTER_0_OFS);
}

UINT32 cal_eng_get_busy_count(CAL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    if(p_eng == NULL) {
        DBG_ERR("CAL handle null\r\n");
		return FALSE;
    ...
    if(ll_idx > 1) {
        DBG_ERR("CAL does not support ll_idx %d\n\r", ll_idx);
        return FALSE;
    }

    if(ll_idx==0) return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_1_OFS);
    else return CAL_ENG_GETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_2_OFS);
}

VOID cal_eng_clr_busy_count(CAL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    T_CAL_BUSY_COUNT_REGISTER_0 ctrl_busy_clr;
    ...
    if(ll_idx==0) {
        ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR = 1;
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR);
        ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR = 0;
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR);
    } else {
        ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR = 1;
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR);
        ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR = 0;
        CAL_ENG_SETREG(p_eng->reg_io_base + CAL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR);
    }
}
