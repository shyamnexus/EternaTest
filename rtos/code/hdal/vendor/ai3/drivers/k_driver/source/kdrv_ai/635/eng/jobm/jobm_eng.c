...
UINT32 jobm_eng_get_arb_status(JOBM_ENG_HANDLE *p_eng)
{
	...
	arb_status = (JOBM_ENG_GETREG(p_eng->reg_io_base + JOBM_SUB_MODE_REGISTER2_OFS) >> 16) & 0xFF;
	if ((arb_status >> 5) > 0) {
		nue_unit_sel = (JOBM_ENG_GETREG(p_eng->reg_io_base + UTIL_ROU_CAL_ARBITER_REGISTER0_OFS) >> 20) & 0xF;
		if (nue_unit_sel == 2) {
			arb_status = (arb_status & 0x1F) | (1 << 5);
		} else if (nue_unit_sel == 3) {
			arb_status = (arb_status & 0x1F) | (1 << 6);
		} else if (nue_unit_sel == 4) {
