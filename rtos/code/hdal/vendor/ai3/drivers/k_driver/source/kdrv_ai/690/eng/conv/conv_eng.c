...
{
    p_eng->p_conv_reg_st = (NT98690_CONV_REGISTER_STRUCT *)reg_base_addr;
...
void conv_eng_trig_ll_hw_reg(CONV_ENG_HANDLE *p_eng, uintptr_t ll_addr_msb, uintptr_t ll_addr_lsb)
{
    T_DMA_TO_CONV_JOB_CHANNEL_REGISTER0 ll_dma_lsb;
    T_DMA_TO_CONV_JOB_CHANNEL_REGISTER1 ll_dma_msb;
	T_CONV_CONTROL_REGISTER ctl;
	...
	CONV_ENG_SETREG(p_eng->reg_io_base + DMA_TO_CONV_JOB_CHANNEL_REGISTER0_OFS, ll_dma_lsb.reg);
    CONV_ENG_SETREG(p_eng->reg_io_base + DMA_TO_CONV_JOB_CHANNEL_REGISTER1_OFS, ll_dma_msb.reg);
...
void conv_eng_isr_hw_reg(CONV_ENG_HANDLE *p_eng)
{
	...
	if (status != 0) {
		...
		if (status_inte & CONV_ENG_INTERRUPT_LL_END) {
			flag |= FLGPTN_CONV_LL_END;
		}
		if (status & CONV_ENG_INTERRUPT_LL_ERR) {
			//flg |= FLGPTN_IME_LL_ERR;
			DBG_ERR("%s: linked-list error...\r\n", __func__);
		}
		if (status & CONV_ENG_INTERRUPT_LL_END) {
...
void conv_eng_wait_framend_ll(CONV_ENG_HANDLE *p_eng)
{
    FLGPTN uiflag;
    ER er_return = E_OK;

    er_return = conv_eng_platform_flg_wait(p_eng, &uiflag, FLGPTN_CONV_LL_END);
    if(er_return != E_OK) {
...
UINT32 conv_eng_get_ll_cycle(CONV_ENG_HANDLE *p_eng)
{
...
{
    return CONV_ENG_GETREG(p_eng->reg_io_base + CYCLE_COUNT_REGISTER2_OFS);
}

UINT32 conv_eng_get_dram_bw(CONV_ENG_HANDLE *p_eng)
{
    return CONV_ENG_GETREG(p_eng->reg_io_base + RESERVED66_OFS);
}

UINT32 conv_eng_get_ub_bw(CONV_ENG_HANDLE *p_eng)
{
    return CONV_ENG_GETREG(p_eng->reg_io_base + RESERVED67_OFS);
}
...
UINT32 conv_eng_get_out_chksum(CONV_ENG_HANDLE *p_eng)
{
	...
	return CONV_ENG_GETREG(p_eng->reg_io_base + 0x474);
}
