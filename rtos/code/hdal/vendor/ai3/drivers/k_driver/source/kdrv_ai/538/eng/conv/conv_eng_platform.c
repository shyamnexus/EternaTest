...
INT32 conv_eng_platform_set_clk_rate(CONV_ENG_HANDLE *p_eng)
{
	...
	if (conv_eng_platform_get_chip_id() == CHIP_NS02302) {
		if(source_clk == 480) {
...
#elif defined (__FREERTOS )
	UINT32 selected_clock = PLL_CLKSEL_CONV_PLL10;
	UINT32 source_clk;
	...
	if (conv_eng_platform_get_chip_id() == CHIP_NS02302) {
        if(source_clk >= 600) {
            selected_clock = PLL_CLKSEL_CONV_PLL10;
            if(source_clk > 600) {
                DBG_ERR("%s: input frequency %d convnd to 600\r\n", __func__, source_clk);
            }
            ...
            selected_clock = PLL_CLKSEL_CONV_320;
            if(source_clk > 320) {
                DBG_ERR("%s: input frequency %d convnd to 320\r\n", __func__, source_clk);
            }
        } else if(source_clk >= 240) {
            selected_clock = PLL_CLKSEL_CONV_240;
            if(source_clk > 240) {
                DBG_ERR("%s: input frequency %d convnd to 240\r\n", __func__, source_clk);
            }
        } else {
            DBG_ERR("%s: illegal input frequency %u, set as 600\r\n", __func__, source_clk);
            selected_clock = PLL_CLKSEL_CONV_PLL10;
        }
	} else {
		selected_clock = PLL_CLKSEL_CONV_PLL10;
