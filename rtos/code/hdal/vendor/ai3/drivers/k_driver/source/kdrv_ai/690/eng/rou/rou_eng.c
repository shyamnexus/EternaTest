...
{
    p_eng->p_rou_reg_st = (NT98690_ROU_REGISTER_STRUCT *)reg_base_addr;
...
VOID rou_eng_trig_ll_hw_reg(ROU_ENG_HANDLE *p_eng, uintptr_t ll_addr_msb, uintptr_t ll_addr_lsb)
{
    T_ROU_DRAMUB_JOB_REGISTER_0 ll_dma_lsb;
    T_ROU_DRAMUB_JOB_REGISTER_1 ll_dma_msb;
	T_ROU_CONTROL_REGISTER ctl;
	...
	ROU_ENG_SETREG(p_eng->reg_io_base + ROU_DRAMUB_JOB_REGISTER_0_OFS, ll_dma_lsb.reg);
    ROU_ENG_SETREG(p_eng->reg_io_base + ROU_DRAMUB_JOB_REGISTER_1_OFS, ll_dma_msb.reg);
...
VOID rou_eng_isr_hw_reg(ROU_ENG_HANDLE *p_eng)
{
	...
	if (status != 0) {
		...
		if (status_inte & ROU_ENG_INTERRUPT_LL_END) {
			flag |= FLGPTN_ROU_LL_END;
		}
		if (status & ROU_ENG_INTERRUPT_LL_ERR) {
			DBG_ERR("ROU linked-list error\r\n");
		}
		if (status & ROU_ENG_INTERRUPT_LL_END) {
...
VOID rou_eng_wait_framend_ll(ROU_ENG_HANDLE *p_eng)
{
    FLGPTN uiflag;
    ER er_return = E_OK;

    er_return = rou_eng_platform_flg_wait(p_eng, &uiflag, FLGPTN_ROU_LLEND);
    if(er_return != E_OK) {
...
UINT32 rou_eng_get_ll_cycle(ROU_ENG_HANDLE *p_eng)
{
