...
{
    p_eng->p_util_reg_st = (NT98635_UTIL_REGISTER_STRUCT *)reg_base_addr;
...
UINT32 util_eng_get_busy_count(UTIL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    if(p_eng == NULL) {
        DBG_ERR("UTIL handle null\r\n");
		return FALSE;
    ...
    if(ll_idx > 1) {
        DBG_ERR("UTIL does not support ll_idx %d\n\r", ll_idx);
        return FALSE;
    }

    if(ll_idx==0) return UTIL_ENG_GETREG(p_eng->reg_io_base + UTIL_BUSY_COUNT_REGISTER_1_OFS);
    else return UTIL_ENG_GETREG(p_eng->reg_io_base + UTIL_BUSY_COUNT_REGISTER_2_OFS);
}

VOID util_eng_clr_busy_count(UTIL_ENG_HANDLE *p_eng, UINT32 ll_idx)
{
    T_UTIL_BUSY_COUNT_REGISTER_0 ctrl_busy_clr;
    ...
    if(ll_idx==0) {
        ...
        UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR);
        ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR = 0;
        UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL0_BUSY_COUNT_CLR);
    } else {
        ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR = 1;
        UTIL_ENG_SETREG(p_eng->reg_io_base + UTIL_BUSY_COUNT_REGISTER_0_OFS, ctrl_busy_clr.bit.LL1_BUSY_COUNT_CLR);
