...
{
    p_eng->p_jobm_reg_st = (NT98690_JOBM_REGISTER_STRUCT *)reg_base_addr;
...
INT32 jobm_eng_set_arb_dbg_en(JOBM_ENG_HANDLE *p_eng, UINT32 enable_eng)
{
	...
	LocalReg.bit.ARB_DBG_EN_UTIL  = (enable_eng >> 5) & 0x1;
	LocalReg.bit.ARB_DBG_EN_ROU   = (enable_eng >> 6) & 0x1;
	LocalReg.bit.ARB_DBG_EN_CAL   = (enable_eng >> 7) & 0x1;
...
INT32 jobm_eng_run_arb_dbg(JOBM_ENG_HANDLE *p_eng, UINT32 enable_eng)
{
	...
	if (dbg_en_reg.bit.ARB_DBG_EN_CONV1) {
		LocalReg.bit.ARB_DBG_NEXT_CONV1 = (enable_eng >> 1) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_CONV2) {
		LocalReg.bit.ARB_DBG_NEXT_CONV2 = (enable_eng >> 2) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_CONV3) {
		LocalReg.bit.ARB_DBG_NEXT_CONV3 = (enable_eng >> 3) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_LSU) {
		LocalReg.bit.ARB_DBG_NEXT_LSU   = (enable_eng >> 4) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_UTIL) {
		LocalReg.bit.ARB_DBG_NEXT_UTIL  = (enable_eng >> 5) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_ROU) {
		LocalReg.bit.ARB_DBG_NEXT_ROU   = (enable_eng >> 6) & 0x1;
	}
	if (dbg_en_reg.bit.ARB_DBG_EN_CAL) {
		LocalReg.bit.ARB_DBG_NEXT_CAL   = (enable_eng >> 7) & 0x1;
	}
...
INT32 jobm_eng_set_priority(JOBM_ENG_HANDLE *p_eng, UINT32 jl_id, UINT32 no_dispatch_cnt, UINT32 priority)
{
	...
	LocalReg.bit.JLA_PRIORITY		 = priority & 0x1;
...
UINT32 jobm_eng_set_cnt_clr(JOBM_ENG_HANDLE *p_eng, UINT32 clr_eng)
{
	...
	LocalReg.bit.CONV1_BUSY_COUNT_CLR = (clr_eng >> 1) & 0x1;
	LocalReg.bit.CONV2_BUSY_COUNT_CLR = (clr_eng >> 2) & 0x1;
	LocalReg.bit.CONV3_BUSY_COUNT_CLR = (clr_eng >> 3) & 0x1;
	LocalReg.bit.LSU_BUSY_COUNT_CLR   = (clr_eng >> 4) & 0x1;
	LocalReg.bit.UTIL_BUSY_COUNT_CLR  = (clr_eng >> 5) & 0x1;
	LocalReg.bit.ROU_BUSY_COUNT_CLR   = (clr_eng >> 6) & 0x1;
	LocalReg.bit.CAL_BUSY_COUNT_CLR   = (clr_eng >> 7) & 0x1;
	
	JOBM_ENG_SETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER0_OFS, LocalReg.reg);
	return 0;
}

UINT32 jobm_eng_get_conv0_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER1_OFS);
}

UINT32 jobm_eng_get_conv1_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER2_OFS);
}

UINT32 jobm_eng_get_conv2_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER3_OFS);
}

UINT32 jobm_eng_get_conv3_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER4_OFS);
}
...
UINT32 jobm_eng_get_util_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER6_OFS);
}

UINT32 jobm_eng_get_rou_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER7_OFS);
}

UINT32 jobm_eng_get_cal_busy_cnt(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return JOBM_ENG_GETREG(p_eng->reg_io_base + BUSY_DEBUG_REGISTER8_OFS);
}

UINT32 jobm_eng_get_arb_status(JOBM_ENG_HANDLE *p_eng)
{
	if (p_eng == NULL) {
		DBG_ERR("JOBM handle null\r\n");
		return 0;
	...
	return (JOBM_ENG_GETREG(p_eng->reg_io_base + JOBM_SUB_MODE_REGISTER2_OFS) >> 16) & 0xFF;
}

UINT32 jobm_eng_get_eng_arb_info(JOBM_ENG_HANDLE *p_eng, UINT32 eng, UINT32 *jl_idx, UINT32 *dispatch_sel, UINT32 *net_id, UINT64 *job_addr)
{
	...
	LocalReg.reg  = JOBM_ENG_GETREG(p_eng->reg_io_base + 0x10*eng + CONV0_ARBITER_REGISTER0_OFS);
	...
	*net_id       = LocalReg.bit.CONV0_ARB_NET_ID;
	addr          = JOBM_ENG_GETREG(p_eng->reg_io_base + 0x10*eng + CONV0_ARBITER_REGISTER2_OFS);
	addr_msb      = JOBM_ENG_GETREG(p_eng->reg_io_base + 0x10*eng + CONV0_ARBITER_REGISTER3_OFS);
