...
static int kdrv_ai_get_clk_info(KDRV_AI_CLK_INFO* clk_info)
{
	...
	switch (clk_info->eng) {
		...
		case KDRV_AI_ENG_CONV2:
		case KDRV_AI_ENG_CONV3:
		case KDRV_AI_ENG_CONV4:{
...
static void kdrv_ai_rou_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	KDRV_AI_HANDLE *p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_ROU);
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) { 
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = rou_eng_get_ll_cycle(p_eng_handle);
...
static void kdrv_ai_nue2_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = nue2_eng_get_ll_cycle(p_eng_handle);
...
static void kdrv_ai_conv1_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	KDRV_AI_HANDLE *p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_CONV1);
...
static void kdrv_ai_lsu_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = lsu_eng_get_ll_cycle(p_eng_handle);
...
static void kdrv_ai_util_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	KDRV_AI_HANDLE *p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_UTIL);
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = util_eng_get_ll_cycle(p_eng_handle);
...
static void kdrv_ai_cal_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	KDRV_AI_HANDLE *p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_CAL);
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = cal_eng_get_ll_cycle(p_eng_handle);
...
static void kdrv_ai_ppu_isr_cb(void* handle, UINT32 intstatus, void* data)
{
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#else
				exec_cycle_LL  = ppu_eng_get_ll_cycle(p_eng_handle);
...
INT32 kdrv_ai_engine_reset(KDRV_AI_ENG ai_eng)
{
	...
	{// Disable autogating
		...
		clk_set_phase(ai_clk[4], 0);
		clk_set_phase(ai_clk[5], 0);
		clk_set_phase(ai_clk[7], 0);
		clk_set_phase(ai_clk[8], 0);
		clk_set_phase(ai_clk[9], 0);
		clk_set_phase(ai_clk[10], 0);
	#else
		pll_clear_clk_auto_gating(ROU_M_GCLK);
		pll_clear_clk_auto_gating(NUE2_M_GCLK);
		pll_clear_clk_auto_gating(CONV_M_GCLK);
		pll_clear_clk_auto_gating(CONV2_M_GCLK);
		pll_clear_clk_auto_gating(CONV3_M_GCLK);
		pll_clear_clk_auto_gating(CONV4_M_GCLK);
		pll_clear_clk_auto_gating(LSU_M_GCLK);
		pll_clear_clk_auto_gating(UTIL_M_GCLK);
		pll_clear_clk_auto_gating(CAL_M_GCLK);
		pll_clear_clk_auto_gating(PPU_M_GCLK);
	#endif
	}
	
	if (ai_eng == KDRV_AI_ENG_CONV1 || ai_eng == KDRV_AI_ENG_CONV2 ||
		ai_eng == KDRV_AI_ENG_CONV3 || ai_eng == KDRV_AI_ENG_CONV4) {
		p_conv_handle = conv_eng_get_handle(0, ai_eng - KDRV_AI_ENG_CONV1);
		...
		p_conv_handle = conv_eng_get_handle(0, KDRV_AI_ENG_CONV1);
		rt = conv_eng_reset(p_conv_handle);
		if (rt) {
	...
	{// Enable autogating
		...
		clk_set_phase(ai_clk[4], 1);
		clk_set_phase(ai_clk[5], 1);
		clk_set_phase(ai_clk[7], 1);
		clk_set_phase(ai_clk[8], 1);
		clk_set_phase(ai_clk[9], 1);
		clk_set_phase(ai_clk[10], 1);
	#else
		pll_set_clk_auto_gating(ROU_M_GCLK);
		pll_set_clk_auto_gating(NUE2_M_GCLK);
		pll_set_clk_auto_gating(CONV_M_GCLK);
		pll_set_clk_auto_gating(CONV2_M_GCLK);
		pll_set_clk_auto_gating(CONV3_M_GCLK);
		pll_set_clk_auto_gating(CONV4_M_GCLK);
		pll_set_clk_auto_gating(LSU_M_GCLK);
		pll_set_clk_auto_gating(UTIL_M_GCLK);
		pll_set_clk_auto_gating(CAL_M_GCLK);
...
INT32 kdrv_ai_set(KDRV_AI_PARAM_ID param_id, VOID *p_param)
{
	...
	switch (param_id) {
		...
		case KDRV_AI_PARAM_BASE_ADDR: {
			...
			if (p_base_addr_parm->eng == KDRV_AI_ENG_ROU) {
				...
				rou_eng_set_dma_base_addr(p_handle, (p_base_addr_parm->pa >> 32) & 0xF,
											p_base_addr_parm->pa & 0xFFFFFFFF, p_base_addr_parm->base_id);
			} else if (p_base_addr_parm->eng == KDRV_AI_ENG_NUE2) {
				NUE2_ENG_HANDLE *p_handle = nue2_eng_get_handle(0, 0);
				nue2_eng_set_dma_base_addr(p_handle, (p_base_addr_parm->pa >> 32) & 0xF,
											p_base_addr_parm->pa & 0xFFFFFFFF, p_base_addr_parm->base_id);
			} else if (p_base_addr_parm->eng == KDRV_AI_ENG_CONV1 || p_base_addr_parm->eng == KDRV_AI_ENG_CONV2 || 
					   p_base_addr_parm->eng == KDRV_AI_ENG_CONV3 || p_base_addr_parm->eng == KDRV_AI_ENG_CONV4) {
				CONV_ENG_HANDLE *p_handle = conv_eng_get_handle(0, p_base_addr_parm->eng - KDRV_AI_ENG_CONV1);
		...
		case KDRV_AI_PARAM_JOBM_DBG: {
			...
			jobm_eng_set_arb_dbg_en(p_handle, 0);
			...
			if (dbg_param->dbg_mode == KDRV_AI_JM_DBG_MODE_NONE) {
				...
				jobm_eng_set_arb_dbg_en(p_handle, dbg_param->arb_eng);
...
INT32 kdrv_ai_get(KDRV_AI_PARAM_ID param_id, VOID *p_param)
{
	...
	switch (param_id) {
		...
		case KDRV_AI_PARAM_JOBM_BUSY_CNT: {
			JOBM_ENG_HANDLE *p_handle = jobm_eng_get_handle(0, 0);
			CONV_ENG_HANDLE *p_conv0_handle = conv_eng_get_handle(0, 0);
			CONV_ENG_HANDLE *p_conv1_handle = conv_eng_get_handle(0, 1);
			CONV_ENG_HANDLE *p_conv2_handle = conv_eng_get_handle(0, 2);
			CONV_ENG_HANDLE *p_conv3_handle = conv_eng_get_handle(0, 3);
			...
			cnt_param->busy_cnt[KDRV_AI_ENG_CONV1] = (UINT32)((UINT64)jobm_eng_get_conv0_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv0_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_CONV2] = (UINT32)((UINT64)jobm_eng_get_conv1_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv1_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_CONV3] = (UINT32)((UINT64)jobm_eng_get_conv2_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv2_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_CONV4] = (UINT32)((UINT64)jobm_eng_get_conv3_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv3_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_LSU]   = (UINT32)((UINT64)jobm_eng_get_lsu_busy_cnt(p_handle)*lsu_eng_get_clk_rate(p_lsu_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_UTIL]  = (UINT32)((UINT64)jobm_eng_get_util_busy_cnt(p_handle)*util_eng_get_clk_rate(p_util_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_ROU]   = (UINT32)((UINT64)jobm_eng_get_rou_busy_cnt(p_handle)*rou_eng_get_clk_rate(p_rou_handle)/12000000);
			cnt_param->busy_cnt[KDRV_AI_ENG_CAL]   = (UINT32)((UINT64)jobm_eng_get_cal_busy_cnt(p_handle)*cal_eng_get_clk_rate(p_cal_handle)/12000000);
		}
			break;
		case KDRV_AI_PARAM_JMISP_INT: {
			KDRV_AI_JMISP_INT_INFO* jmisp_int_info = (KDRV_AI_JMISP_INT_INFO*)p_param;
		...
		case KDRV_AI_PARAM_JOBM_ENG_BUSY_CNT: {
			...
			if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV0) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv0_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv0_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV1) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv1_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv1_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV2) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv2_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv2_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV3) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv3_busy_cnt(p_handle)*conv_eng_get_clk_rate(p_conv3_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_LSU) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_lsu_busy_cnt(p_handle)*lsu_eng_get_clk_rate(p_lsu_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_UTIL) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_util_busy_cnt(p_handle)*util_eng_get_clk_rate(p_util_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_ROU) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_rou_busy_cnt(p_handle)*rou_eng_get_clk_rate(p_rou_handle)/12000000);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CAL) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_cal_busy_cnt(p_handle)*cal_eng_get_clk_rate(p_cal_handle)/12000000);
			} else {
		...
		case KDRV_AI_PARAM_JOBM_ARB_STATUS: {
			...
			UINT32 arb_status = jobm_eng_get_arb_status(p_handle);
			p_info->arb_status[KDRV_AI_ENG_CONV1] = arb_status & 0x1;
			p_info->arb_status[KDRV_AI_ENG_CONV2] = (arb_status >> 1) & 0x1;
			p_info->arb_status[KDRV_AI_ENG_CONV3] = (arb_status >> 2) & 0x1;
			p_info->arb_status[KDRV_AI_ENG_CONV4] = (arb_status >> 3) & 0x1;
		...
		case KDRV_AI_PARAM_JOBM_ARB_INFO: {
			...
			jobm_eng_get_eng_arb_info(p_handle, p_info->eng, &p_info->arb_jl_idx, &p_info->arb_dispatch_sel, 
...
INT32 kdrv_ai_init(VOID)
{
	...
	p_rou_handle->pclk    = ai_clk[0];
	p_rou_handle->reg_io_base = ai_eng_io_addr[0];
	p_rou_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_rou_handle->isr_cb = kdrv_ai_rou_isr_cb;
	p_rou_handle->clock_rate = 800;
	...
	conv_eng_init(1, 4);
	for (i = 0; i < 4; i++) {
		p_conv_handle = conv_eng_get_handle(0, i);
		if (p_conv_handle == NULL) {
			DBG_ERR("CONV%d init fail\r\n", i);
			return -1;
		}
		p_conv_handle->chip_id = 0;
		p_conv_handle->eng_id  = i;
		p_conv_handle->pclk    = ai_clk[2+i];
		p_conv_handle->reg_io_base = ai_eng_io_addr[2+i];
		...
		p_conv_handle->clock_rate = kdrv_ai_get_conv_clk_rate();
		snprintf(&p_conv_handle->name[0], sizeof(p_conv_handle->name), "conv%d", i+1);
	...
	p_jobm_handle->reg_io_base = ai_eng_io_addr[6];
	...
	p_lsu_handle->pclk    = ai_clk[7];
	p_lsu_handle->reg_io_base = ai_eng_io_addr[7];
	p_lsu_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_lsu_handle->isr_cb = kdrv_ai_lsu_isr_cb;
	p_lsu_handle->clock_rate = 800;
	...
	p_util_handle->pclk    = ai_clk[8];
	p_util_handle->reg_io_base = ai_eng_io_addr[8];
	p_util_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_util_handle->isr_cb = kdrv_ai_util_isr_cb;
	p_util_handle->clock_rate = 800;
	...
	p_cal_handle->pclk    = ai_clk[9];
	p_cal_handle->reg_io_base = ai_eng_io_addr[9];
	p_cal_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_cal_handle->isr_cb = kdrv_ai_cal_isr_cb;
	p_cal_handle->clock_rate = 800;
	...
	p_ppu_handle->pclk    = ai_clk[10];
	p_ppu_handle->reg_io_base = ai_eng_io_addr[10];
	p_ppu_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_ppu_handle->isr_cb = kdrv_ai_ppu_isr_cb;
	p_ppu_handle->clock_rate = 800;
	...
	p_jmisp_handle->reg_io_base = ai_eng_io_addr[11];
...
INT32 kdrv_ai_trigger(KDRV_AI_ENG eng, uintptr_t pa)
{
	...
	if (eng == KDRV_AI_ENG_ROU) {
		...
		rou_eng_trig_ll_hw_reg(p_rou_handle, pa >> 32, pa & 0xFFFFFFFF);
		...
		nue2_eng_trig_ll_hw_reg(p_nue2_handle, pa >> 32, pa & 0xFFFFFFFF);
	} else if (eng == KDRV_AI_ENG_CONV1 || eng == KDRV_AI_ENG_CONV2 ||
				eng == KDRV_AI_ENG_CONV3 || eng == KDRV_AI_ENG_CONV4) {
		CONV_ENG_HANDLE* p_conv_handle = conv_eng_get_handle(0, eng - KDRV_AI_ENG_CONV1);
		conv_eng_set_intrpt_en(p_conv_handle, 0x1000000);
		conv_eng_trig_ll_hw_reg(p_conv_handle, pa >> 32, pa & 0xFFFFFFFF);
		...
		lsu_eng_trig_ll_hw_reg(p_handle, pa >> 32, pa & 0xFFFFFFFF);
	} else if (eng == KDRV_AI_ENG_UTIL) {
		UTIL_ENG_HANDLE* p_handle = util_eng_get_handle(0, 0);
		util_eng_set_intrpt_en(p_handle, 0x1000000);
		util_eng_trig_ll_hw_reg(p_handle, pa >> 32, pa & 0xFFFFFFFF);
	} else if (eng == KDRV_AI_ENG_CAL) {
		CAL_ENG_HANDLE* p_handle = cal_eng_get_handle(0, 0);
		cal_eng_set_intrpt_en(p_handle, 0x1000000);
		cal_eng_trig_ll_hw_reg(p_handle, pa >> 32, pa & 0xFFFFFFFF);
	} else if (eng == KDRV_AI_ENG_PPU) {
		PPU_ENG_HANDLE* p_handle = ppu_eng_get_handle(0, 0);
		ppu_eng_set_intrpt_en(p_handle, 0x1000000);
		ppu_eng_trig_ll_hw_reg(p_handle, pa >> 32, pa & 0xFFFFFFFF);
...
INT32 kdrv_ai_jm_dbg_run_next(KDRV_AI_JOBM_DBG_PARAM jm_dbg_param)
{
	...
	if (jm_dbg_param.dbg_mode == KDRV_AI_JM_DBG_MODE_NONE) {
		...
		jobm_eng_run_arb_dbg(p_handle, jm_dbg_param.arb_eng);
	} 
