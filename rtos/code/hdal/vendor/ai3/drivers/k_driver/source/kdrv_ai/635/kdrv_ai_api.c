...
static void kdrv_ai_rou2_isr_cb(void* handle, UINT32 intstatus, void* data, UINT32 ll_idx)
{
	KDRV_AI_HANDLE *p_handle;
	ROU_ENG_HANDLE *p_eng_handle = rou_eng_get_handle(0, 1);
	UINT32 exec_cycle 		= 0;
	UINT32 exec_cycle_LL 	= 0;
	UINT32 exec_cycle_dma 	= 0;
	
	if(ll_idx==1){
		p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_ROU2_LL1);
	}else{
		p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_ROU3);
	}
	if (p_handle == NULL || p_eng_handle == NULL) {
		DBG_ERR("kdrv_ai_get_handle fail\r\n");
	} else {
		if (intstatus & 0x1) { 
			if (p_handle->isrcb_fp != NULL) {
				...
				#endif
				if(ll_idx==1)
					p_handle->isrcb_fp(exec_cycle, exec_cycle_LL, exec_cycle_dma, intstatus, KDRV_AI_ENG_ROU4, NULL);
				else
					p_handle->isrcb_fp(exec_cycle, exec_cycle_LL, exec_cycle_dma, intstatus, KDRV_AI_ENG_ROU3, NULL);
			}
		}
	}
}

static void kdrv_ai_lsu2_isr_cb(void* handle, UINT32 intstatus, void* data, UINT32 ll_idx)
{
	KDRV_AI_HANDLE *p_handle = kdrv_ai_get_handle(KDRV_AI_ENG_LSU2);
	LSU_ENG_HANDLE *p_eng_handle = lsu_eng_get_handle(0, 1);
	UINT32 exec_cycle 		= 0;
	UINT32 exec_cycle_LL 	= 0;
	UINT32 exec_cycle_dma 	= 0;
	...
	if (p_handle == NULL || p_eng_handle == NULL) {
		...
		if (intstatus & 0x1) {
			if (p_handle->isrcb_fp != NULL) {
				...
				#endif
				p_handle->isrcb_fp(exec_cycle, exec_cycle_LL, exec_cycle_dma, intstatus, KDRV_AI_ENG_LSU2, NULL);
			}
		}
	}
}
...
INT32 kdrv_ai_engine_reset(KDRV_AI_ENG ai_eng)
{
	...
	} else if (ai_eng == KDRV_AI_ENG_LSU2) {
		p_lsu_handle = lsu_eng_get_handle(0, 1);
		rt = lsu_eng_reset(p_lsu_handle);
	...
	} else if (ai_eng == KDRV_AI_ENG_ROU3 || ai_eng == KDRV_AI_ENG_ROU2_LL1) {
		p_rou_handle = rou_eng_get_handle(0, 1);
		rt = rou_eng_reset(p_rou_handle);
...
INT32 kdrv_ai_set(KDRV_AI_PARAM_ID param_id, VOID *p_param)
{
	...
	switch (param_id) {
		...
		case KDRV_AI_PARAM_BASE_ADDR: {
			...
			if (p_base_addr_parm->eng == KDRV_AI_ENG_ROU) {
			...
			} else if (p_base_addr_parm->eng == KDRV_AI_ENG_ROU3) {
				ROU_ENG_HANDLE *p_handle = rou_eng_get_handle(0, 1);
				rou_eng_set_dma_base_addr(p_handle, (p_base_addr_parm->pa >> 32) & 0xF,
											p_base_addr_parm->pa & 0xFFFFFFFF, p_base_addr_parm->base_id,0);
			} else if (p_base_addr_parm->eng == KDRV_AI_ENG_ROU2_LL1) {
				ROU_ENG_HANDLE *p_handle = rou_eng_get_handle(0, 1);
				rou_eng_set_dma_base_addr(p_handle, (p_base_addr_parm->pa >> 32) & 0xF,
											p_base_addr_parm->pa & 0xFFFFFFFF, p_base_addr_parm->base_id,1);
				...
				lsu_eng_set_dma_base_addr(p_handle, (p_base_addr_parm->pa >> 32) & 0xF,
											p_base_addr_parm->pa & 0xFFFFFFFF, p_base_addr_parm->base_id,0);
			} else if (p_base_addr_parm->eng == KDRV_AI_ENG_LSU2) {
		...
		case KDRV_AI_PARAM_LL_SETTING: {
			...
			}else if(p_ll_setting_param->eng == KDRV_AI_ENG_ROU3 || p_ll_setting_param->eng == KDRV_AI_ENG_ROU2_LL1){
				ROU_ENG_HANDLE *p_handle = rou_eng_get_handle(0, 1);
				ROU_LL_SETTING ll_param;
				ll_param.unlock_cycle = p_ll_setting_param->unlock_cycle;
				ll_param.priority_mode = p_ll_setting_param->priority_mode;
	...
	switch (param_id) {
		...
		case KDRV_AI_PARAM_JOBM_BUSY_CNT: {
			...
			if(conv_eng_get_qos_setting(p_conv_handle) == 0){
				cnt_param->busy_cnt[KDRV_AI_ENG_CONV1] = (UINT32)((UINT64)jobm_eng_get_conv0_busy_cnt(p_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_CONV1_LL1] = (UINT32)((UINT64)jobm_eng_get_conv0_busy_cnt(p_handle, 1)*600/240);
			}else{
				cnt_param->busy_cnt[KDRV_AI_ENG_CONV1] = (UINT32)((UINT64)conv_eng_get_ll_sta_acc_cycle(p_conv_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_CONV1_LL1] = (UINT32)((UINT64)conv_eng_get_ll_sta_acc_cycle(p_conv_handle, 1)*600/240);
			}
			
			//cnt_param->busy_cnt[KDRV_AI_ENG_CONV2] = (UINT32)((UINT64)jobm_eng_get_conv1_busy_cnt(p_handle)*600/12);
			//cnt_param->busy_cnt[KDRV_AI_ENG_CONV3] = (UINT32)((UINT64)jobm_eng_get_conv2_busy_cnt(p_handle)*600/12);
			//cnt_param->busy_cnt[KDRV_AI_ENG_CONV4] = (UINT32)((UINT64)jobm_eng_get_conv3_busy_cnt(p_handle)*600/12);
			cnt_param->busy_cnt[KDRV_AI_ENG_LSU]   = (UINT32)((UINT64)jobm_eng_get_lsu_busy_cnt(p_handle)*600/240);
			
			p_util_handle = util_eng_get_handle(0, 0);
			if(util_eng_get_qos_setting(p_util_handle) == 0){
				cnt_param->busy_cnt[KDRV_AI_ENG_UTIL]  = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_UTIL_LL1]  = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 1)*600/240);
			}else{
				cnt_param->busy_cnt[KDRV_AI_ENG_UTIL] = (UINT32)((UINT64)util_eng_get_ll_sta_acc_cycle(p_util_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_UTIL_LL1] = (UINT32)((UINT64)util_eng_get_ll_sta_acc_cycle(p_util_handle, 1)*600/240);
			}
			
			if(rou_eng_get_qos_setting(p_rou_handle) == 0){
				cnt_param->busy_cnt[KDRV_AI_ENG_ROU]   = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_ROU_LL1] = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 1)*600/240);
			}else{
				cnt_param->busy_cnt[KDRV_AI_ENG_ROU] = (UINT32)((UINT64)rou_eng_get_ll_sta_acc_cycle(p_rou_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_ROU_LL1] = (UINT32)((UINT64)rou_eng_get_ll_sta_acc_cycle(p_rou_handle, 1)*600/240);
			}
			if(cal_eng_get_qos_setting(p_cal_handle) == 0){
				cnt_param->busy_cnt[KDRV_AI_ENG_CAL]   = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_CAL_LL1] = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 1)*600/240);
			}else{
				cnt_param->busy_cnt[KDRV_AI_ENG_CAL] = (UINT32)((UINT64)cal_eng_get_ll_sta_acc_cycle(p_cal_handle, 0)*600/240);
				cnt_param->busy_cnt[KDRV_AI_ENG_CAL_LL1] = (UINT32)((UINT64)cal_eng_get_ll_sta_acc_cycle(p_cal_handle, 1)*600/240);
			}			
		}
			break;
		case KDRV_AI_PARAM_JMISP_INT: {
		...
		case KDRV_AI_PARAM_JOBM_ENG_BUSY_CNT: {
			...
			if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV0) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv0_busy_cnt(p_handle, 0)*600/240);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV1) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv1_busy_cnt(p_handle, 0)*600/240);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV2) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv2_busy_cnt(p_handle, 0)*600/240);
			} else if (p_info->eng == KDRV_AI_JM_DBG_ENG_CONV3) {
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_conv3_busy_cnt(p_handle, 0)*600/240);
				...
				p_info->busy_cnt = (UINT32)((UINT64)jobm_eng_get_util_rou_cal_busy_cnt(p_handle, 0)*600/240);
		...
		case KDRV_AI_PARAM_JOBM_ARB_INFO: {
			...
			jobm_eng_get_eng_arb_info(p_handle, p_info->eng, &p_info->arb_jl_idx, &p_info->arb_dispatch_sel, &p_info->arb_net_id, &p_info->arb_job_addr);		
		}
			break;
		case KDRV_AI_PARAM_JMISP_FRM_UPD_PARAM: {
			KDRV_AI_JMISP_FRM_UPD_PATH_INFO* jmisp_param = (KDRV_AI_JMISP_FRM_UPD_PATH_INFO*)p_param;
...
INT32 kdrv_ai_init(VOID)
{
	...
	p_rou_handle = rou_eng_get_handle(0, 1);
	if (p_rou_handle == NULL) {
		DBG_ERR("ROU2 init fail\r\n");
		return -1;
	}
	p_rou_handle->chip_id = 0;
	p_rou_handle->eng_id  = 1;
	p_rou_handle->pclk    = ai_clk[7];
	
	p_rou_handle->reg_io_base = ai_eng_io_addr[9];
	p_rou_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_rou_handle->isr_cb = kdrv_ai_rou2_isr_cb;
	p_rou_handle->clock_rate = 800;
	snprintf(&p_rou_handle->name[0], sizeof(p_rou_handle->name), "rou2");
	rou_eng_init_resource(p_rou_handle);
	p_rou_handle_reg_base = (UINT8*)kdrv_ai_alloc_mem(rou_eng_get_reg_base_buf_size(0)*sizeof(UINT8)); 
	p_rou_handle_reg_flag = (UINT8*)kdrv_ai_alloc_mem(rou_eng_get_reg_flag_buf_size(0)*sizeof(UINT8)); 
	rou_eng_set_reg_buf(p_rou_handle, (uintptr_t)p_rou_handle_reg_base, (uintptr_t)p_rou_handle_reg_flag);
	...
	p_lsu_handle->chip_id = 0;
	p_lsu_handle->eng_id  = 1;
	p_lsu_handle->pclk    = ai_clk[8];
	p_lsu_handle->reg_io_base = ai_eng_io_addr[10];
	p_lsu_handle->irq_id = 0;//pive_drv_eng_info[i].irq_no;
	p_lsu_handle->isr_cb = kdrv_ai_lsu2_isr_cb;
	p_lsu_handle->clock_rate = 800;
	snprintf(&p_lsu_handle->name[0], sizeof(p_lsu_handle->name), "lsu2");
	lsu_eng_init_resource(p_lsu_handle);
	p_lsu_handle_reg_base = (UINT8*)kdrv_ai_alloc_mem(lsu_eng_get_reg_base_buf_size(0)*sizeof(UINT8)); 
	p_lsu_handle_reg_flag = (UINT8*)kdrv_ai_alloc_mem(lsu_eng_get_reg_flag_buf_size(0)*sizeof(UINT8)); 
	lsu_eng_set_reg_buf(p_lsu_handle, (uintptr_t)p_lsu_handle_reg_base, (uintptr_t)p_lsu_handle_reg_flag);
...
INT32 kdrv_ai_trigger(KDRV_AI_ENG eng, uintptr_t pa)
{
	...
	} else if (eng == KDRV_AI_ENG_ROU3) {
		ROU_ENG_HANDLE* p_rou_handle = rou_eng_get_handle(0, 1);
		rou_eng_set_intrpt_en(p_rou_handle, 0x1000000);
		rou_eng_trig_ll_hw_reg(p_rou_handle, pa >> 32, pa & 0xFFFFFFFF,0);
	} else if (eng == KDRV_AI_ENG_ROU2_LL1) {
		ROU_ENG_HANDLE* p_rou_handle = rou_eng_get_handle(0, 1);
		rou_eng_set_intrpt_en(p_rou_handle, 0x1000000);
		rou_eng_trig_ll_hw_reg(p_rou_handle, pa >> 32, pa & 0xFFFFFFFF,1);
