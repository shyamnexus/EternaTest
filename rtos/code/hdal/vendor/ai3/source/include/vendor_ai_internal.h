...
#define VENDOR_AI_NET_PARAM_EXT_LIST(layer_id, out_id)   (0x70000000 | (((out_id) & 0xfff) << 16) | ((layer_id) & 0xffff)) ///< ext_id input buffer
#define VENDOR_AI_NET_PARAM_OUT_LIST(layer_id, out_id)   (0xb0000000 | (((out_id) & 0xfff) << 16) | ((layer_id) & 0xffff)) ///< ext_id output buffer
#define VENDOR_AI_NET_PARAM_BUF_LIST(layer_id, out_id)   (0x60000000 | (((out_id) & 0xfff) << 16) | ((layer_id) & 0xffff)) ///<  buffer id input buffer
...
typedef struct _VENDOR_AI_PERF_TIMELINE {
    UINT32 enable;
    UINT32 timeline_func;
    UINT32 timeline_buf_size;
} VENDOR_AI_PERF_TIMELINE;
...
typedef enum _VENDOR_AI_CFG_ID {
	...
	//VENDOR_AI_CFG_MULTI_PROCESS        =  8,  ///< set    , support multi process (default 0, set to 0 or 1)    ENUM_DUMMY4WORD(VENDOR_AI_CFG_ID)
	...
	VENDOR_AI_CFG_KFLOW_ISP_IMPL_VERSION = 13,  ///< get    , kflow isp implement version
    VENDOR_AI_CFG_TIMELINE_ALL           = 14,  ///< set    , support perf timeline_all by user api
...
typedef struct _VENDOR_AI_NET_CFG_BUF_CTRL {
    UINT32 ctrl;                                ///< buf ctrl, bitwise mask using CTRL_XXXX
} VENDOR_AI_NET_CFG_BUF_CTRL;
...
typedef enum {
	...
	VENDOR_AI_NET_BUF_OPT_SHRINK_O3   = 4,  ///< shrink buffer space with TCM (ONLY ONE proc_id can enable TCM at the same time)
	VENDOR_AI_NET_BUF_OPT_SHRINK_O4   = 5,  ///< shrink buffer space with TCM optimize level 1
...
typedef enum _VENDOR_AI_NET_PARAM_ID {
	...
	VENDOR_AI_NET_PARAM_CFG_BUF_CTRL            = 30, ///< set/get, set buffer mode & ctrl ()
    ...
    VENDOR_AI_NET_PARAM_CFG_WORKBUF_IMM         = 54, ///< set	  , set work memory, using VENDOR_AI_NET_CFG_WORKBUF struct, MUST set after START
	VENDOR_AI_NET_PARAM_UBUF_MASK               = 55, ///      set, set ub mask
    VENDOR_AI_NET_PARAM_JOB_PRI                 = 60, ///<     set, set job priority
    VENDOR_AI_NET_PARAM_CORE_MASK               = 61, ///<     set, set core mask
	              
    VENDOR_AI_NET_PARAM_PLUGIN_ENGINE           = 62, ///<     set, set plugin engine
...
typedef enum _VENDOR_AI_OP {
    ...
    VENDOR_AI_OP_MAU                            = 3,  ///< matrix arithmetic unit
    VENDOR_AI_OP_FLOAT2FIXED                    = 4,  ///< float2fixed
    VENDOR_AI_OP_FIXED2FLOAT                    = 5,  ///< fixed2float
	VENDOR_AI_OP_TOPN                           = 6,  ///< topn
	VENDOR_AI_OP_POSTPROC                       = 7,  ///< postproc
	VENDOR_AI_OP_MATRIX                         = 8,  ///< matrix
    ENUM_DUMMY4WORD(VENDOR_AI_OP)
} VENDOR_AI_OP;

typedef enum _AI_EOP {
    FC                             =  0,
	PREPROC_YUV2RGB                =  1,  
	PREPROC_YUV2RGB_SCALE          =  2,
	PREPROC_YUV2RGB_MEANSUB_PLANE  =  3,
	PREPROC_YUV2RGB_MEANSUB_DC     =  4,
	PREPROC_Y2Y_UV2UV			   =  5,
	FC_LL_MODE                     =  6,
	TOPN_SORT					   =  7,
	PREPROC_RGB_SCALE              = 14,
	PREPROC_Y8_SCALE               = 15,
	ENUM_DUMMY4WORD(AI_EOP)
} AI_EOP;
...
typedef VENDOR_AI_OP_CFG_WORKBUF VENDOR_AI_OP_MAU_CMDBUF;
...
#define AI_CSC_YUV2RGB                     0x0000  ///< default YUV convert to RGB
#define AI_CSC_BT601_YUV2RGB_LIMIT         0x1000  ///<  BT601 YUV convert to RGB Limit Range [16, 235]
#define AI_CSC_BT709_YUV2RGB_LIMIT         0x2000  ///<  BT709 YUV convert to RGB Limit Range [16, 235]
#define AI_CSC_BT601_YUV2RGB_FULL          0x3000  ///<  BT601 YUV convert to RGB Full Range [0, 255]
#define AI_CSC_BT709_YUV2RGB_FULL          0x4000  ///<  BT709 YUV convert to RGB Full Range [0, 255]

/**
    MAU cal mode
*/
typedef enum {
	VENDOR_AI_MAU_MAT_MUL              = 0,              ///< Matrix multiplication, C(mxn) = A(mxw) x B(nxw)
	VENDOR_AI_MAU_MAT_ADD  	           = 1,              ///< Matrix addition, C(mxn) = A(mxn) + B(mxn)
	VENDOR_AI_MAU_MAT_SUB          	   = 2,              ///< Matrix subtraction, C(mxn) = A(mxn) - B(mxn)
	VENDOR_AI_MAU_L2_NORM  	           = 3,              ///< L2 norm
	VENDOR_AI_MAU_L2_NORM_INV          = 4,              ///< L2 norm inverse
	VENDOR_AI_MAU_VECTOR_NORMALIZAION  = 5,              ///< Vector Normalization
	VENDOR_AI_MAU_TOPN_SORT            = 6,              ///< Topn Sorting
	ENUM_DUMMY4WORD(VENDOR_AI_MAU_CAL_MODE)
} VENDOR_AI_MAU_CAL_MODE;

/**
    MAU sort operation
*/    
typedef enum {
	VENDOR_AI_MAU_SORT_DESC   	= 0,            ///< Descending order
	VENDOR_AI_MAU_SORT_ASC  	= 1,            ///< Ascending order
	ENUM_DUMMY4WORD(VENDOR_AI_MAU_SORT_OP)
} VENDOR_AI_MAU_SORT_OP;

/**
    MAU Mat B input mode
*/   
typedef enum {
	VENDOR_AI_MAU_IN_B_BUF  = 0,              	///< MAU input B read from VENDOR_AI_BUF
	VENDOR_AI_MAU_IN_B_DC  	= 1,              	///< MAU input B read from mau_mat_dc
	ENUM_DUMMY4WORD(VENDOR_AI_MAU_IN_B_MODE)
} VENDOR_AI_MAU_IN_B_MODE;

typedef struct _VENDOR_AI_OP_MAU_PARAM {
	...
	UINT32 mau_mat_outid;                       ///< return active out id
    VENDOR_AI_BUF* mau_mat_out;                 ///< return active out buffer
} VENDOR_AI_OP_MAU_PARAM;

/*TopN*/
typedef enum {
	VENDOR_AI_SORT_DESC   	= 0,              	///< Descending order
	VENDOR_AI_SORT_ASC  	= 1,              	///< Ascending order
	ENUM_DUMMY4WORD(VENDOR_AI_TOPNSORT_OP)
} VENDOR_AI_TOPNSORT_OP;

typedef struct _VENDOR_AI_OP_TOPNSORT_PARAM {
	VENDOR_AI_TOPNSORT_OP sort_op;				///< sort operation, range [0, 1]
	UINT32 sort_n;								///< top n number to sort, range [1, 128]
} VENDOR_AI_OP_TOPNSORT_PARAM;
...
extern HD_RESULT vendor_ai_net_push_isp_frame (UINT32 proc_id, VENDOR_AI3_BUF* in_buf, VENDOR_AI3_BUF* out_buf) ; 
...
extern HD_RESULT vendor_ai_op_open2 (UINT32 proc_id);
extern HD_RESULT vendor_ai_op_close (UINT32 proc_id);
extern HD_RESULT vendor_ai_op_close2 (UINT32 proc_id);
extern HD_RESULT vendor_ai_op_start (UINT32 proc_id);
extern HD_RESULT vendor_ai_op_stop (UINT32 proc_id);
extern HD_RESULT vendor_ai_op_proc (UINT32 proc_id, VENDOR_AI_OP op_id, void* p_op_param, UINT32 in_cnt, VENDOR_AI3_BUF* p_in_buf, UINT32 out_cnt, VENDOR_AI3_BUF* p_out_buf);
extern HD_RESULT _vendor_ai_get_cal_max_buf_size(UINT32 *buf_size) ;
extern HD_RESULT _vendor_ai_get_ppu_max_buf_size(UINT32 *buf_size) ;
...
extern HD_RESULT vendor_ai3_op_get_cal_cmd_size(UINT32* req_size);
extern HD_RESULT vendor_ai_op_util_float2fixed(UINT32 proc_id, uintptr_t in_pa, uintptr_t out_pa, HD_VIDEO_PXLFMT out_fmt, UINT32 w, UINT32 h, UINT32 c, INT32 zero_point);
extern HD_RESULT vendor_ai_op_util_fixed2float(UINT32 proc_id, uintptr_t in_pa, uintptr_t out_pa, HD_VIDEO_PXLFMT in_fmt, UINT32 w, UINT32 h, UINT32 c, UINT32 b, INT32 zero_point, FLOAT scale_ratio);
#ifdef __cplusplus
}
#endif
