...
typedef enum _KDRV_GRPH_PARAM_ID {
	...
	KDRV_GRPH_PARAM_ABORT,  ///< [set] Abort current graphic operation (force kdrv_grph_trigger abort)
				///<	valid setting: refer to KDRV_GRPH_EXEC_MODE
	KDRV_GRPH_PARAM_USAGE,  ///< [set/get] usage monitor
} KDRV_GRPH_PARAM_ID;
...
typedef enum _GRPH_CMD {
	...
	///< - Destination image: GRPH_IMG_ID_C
	GRPH_CMD_Draw_line = 9,                      ///< Draw Line
	///< - (GRPH_ID_2)
	///< - Format: GRPH_FORMAT_8BITS,GRPH_FORMAT_8BITS_YPACK, GRPH_FORMAT_16BITS,GRPH_FORMAT_16BITS_UVPACK,GRPH_FORMAT_16BITS_RGB565,GRPH_FORMAT_16BITS_ARGB1555_ARGB
	///< - GRPH_FORMAT_16BITS_ARGB1555_RGB,GRPH_FORMAT_16BITS_ARGB4444_ARGB,GRPH_FORMAT_16BITS_ARGB4444_RGB,GRPH_FORMAT_32BITS,GRPH_FORMAT_32BITS_ARGB8888_ARGB
	///< - GRPH_FORMAT_32BITS_ARGB8888_RGB
	///< - Source image: GRPH_IMG_ID_A
	///< - Destination image: GRPH_IMG_ID_A (Draw on same image )
	...
	///< - Source/Destination image B: GRPH_IMG_ID_B (Y)
	///< - Source/Destination image: GRPH_IMG_ID_C (UV pack)
	GRPH_CMD_YCC_ENC,                    	///< YCC encode (compress)
	///< - (GRPH_ID_1 only)
	///< - Format: GRPH_FORMAT_8BITS, GRPH_FORMAT_16BITS_UVPACK
	///< - Source image A: GRPH_IMG_ID_A
	///< - Destination image: GRPH_IMG_ID_C
	GRPH_CMD_YCC_DEC,                    	///< YCC decode (decompress)
	///< - (GRPH_ID_1 only)
	///< - Format: GRPH_FORMAT_8BITS, GRPH_FORMAT_16BITS_UVPACK
	///< - Source image A: GRPH_IMG_ID_A
	///< - Destination image: GRPH_IMG_ID_C
	GRPH_CMD_Non_uni_correction,
	///< - (GRPH_ID_2 only)
	///< - Format: GRPH_FORMAT_16BITS
	///< - Source image A: GRPH_IMG_ID_A
	///< - Destination image: GRPH_IMG_ID_C
...
typedef enum _GRPH_FORMAT {
	...
	GRPH_FORMAT_32BITS_ARGB8888_ARGB,       ///< 32 bits ARGB8888 pack data
	...
	GRPH_FORMAT_16BITS_ARGB1555_ARGB,       ///< 16 bits ARGB1555 pack data
	...
	GRPH_FORMAT_16BITS_ARGB4444_ARGB,       ///< 16 bits ARGB4444 pack data
...
typedef enum _GRPH_UV_SUBSAMPLE {
	...
	GRPH_YUV_CENTERED,			///< UV center (YUV averaged)
...
typedef enum _GRPH_PROPERTY_ID {
	...
	GRPH_PROPERTY_ID_Y,                     ///< property for Y plane
	...
	GRPH_PROPERTY_ID_YCC_EN,		///< Image format is YCC codec . Valid setting can be:
						///< - @b GRPH_YCC_PROPTY(TRUE, byte_ofs)
						///< - @b FALSE: disable

	GRPH_PROPERTY_ID_MB_EN,			///< Image is MB formatted . Valid setting can be:
						///< - @b TRUE: MB mode
						///< - @b FALSE: normal
 ...
 * YCC info property
 *
 * @param[in] enable       Enable YCC
 * @param[in] ofs    	   crop byte offset from YCC decompress unit
 *			   (each YCC decompress unit is 16 bytes)
 *                          - valid setting: 0~30 (must be 2 alignment)
 *
 * @note For operations support YCC.
 */
#define GRPH_YCC_PROPTY(enable, ofs)  (((enable)&0x01) | (((ofs)&0x1F)<<8))
...
typedef struct _GRPH_IMG {
	...
	uintptr_t dram_addr;               ///< Image DRAM address (unit: byte)
...
typedef struct _GRPH_PROPERTY {
	...
	uintptr_t property;                ///< Property value
...
typedef struct _GRPH_LINE_DESC {
	...
	//UINT32 slope_int;
	//UINT32 slope_float;		// slope_float is Q13 fixed_point
	//UINT32 scan_range;	//set default value 0x6 in driver,if the line slope is 45degree and linewidth is more than 13
	//UINT32 scan_extend;	//set default value 0x6 in driver,if the line slop is less than 45degree and linewidth is more than 13
	/*GRPH_POINT_DESC point0;       ///< point 0 (left/top)
	GRPH_POINT_DESC top_right;      ///< point 1 (right/top)*/


	//struct GRPH_LINE_DESC *p_next;
} GRPH_LINE_DESC, *PGRPH_LINE_DESC;
 ...
 *			- @b KDRV_GFX2D_GRPH2: graphic engine 2
