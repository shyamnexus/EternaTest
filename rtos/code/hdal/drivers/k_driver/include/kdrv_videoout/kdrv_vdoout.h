...
typedef enum {
	KDRV_VDDO_HDMI_AVI_RGB_DEFAULT = 0,    ///< HDMI AVI RGB Quantization range select defualt
	KDRV_VDDO_HDMI_AVI_RGB_LIMITED,        ///< HDMI AVI RGB Quantization range select limited
	KDRV_VDDO_HDMI_AVI_RGB_FULL,           ///< HDMI AVI RGB Quantization range select full ranged


	ENUM_DUMMY4WORD(KDRV_VDDO_HDMI_AVI_RGB_SEL)
} KDRV_VDDO_HDMI_AVI_RGB_SEL;
...
typedef struct {
	union {
		...
		*/
		...
		struct { // VDDO_DISPCTRL_LINE_MON
			UINT32            line_idx1;            ///< LINE IDX
		} KDRV_VDDO_LINE_MON;
		struct { // VDDO_DISPCTRL_LINE_MON
			UINT32            line_idx2;            ///< LINE IDX
		} KDRV_VDDO_LINE_MON2;
	...
	union {
		...
		*/
		struct { // VDDO_DISPDEV_OUTPUT_DRAM
			uintptr_t      y_addr;                  ///< Output Y address
			uintptr_t      uv_addr;                 ///< Output UV address
        ...
        */
		...
		/**
			get hdmi dev hot plug status
			This structure is used for display device control operation code "VDDO_DISPDEV_HDMI_HOTPLUG_STATUS"
		*/
		struct {
			BOOL		  hplg_status;				///< HDMI device hot plug status
		} KDRV_VDDO_HDMI_HP_STATUS;

		/**
			Set hdmi dev AVI RGB quantization select
			This structure is used for display device control operation code "VDDO_DISPDEV_HDMI_AVI_RGB_SEL"
		*/
		struct {
			KDRV_VDDO_HDMI_AVI_RGB_SEL		  rgb_sel;				///< HDMI AVI RGB Quantization range select
		} KDRV_VDDO_HDMI_AVI_RGB_SEL;		
	...
	union {
		...
		*/
		struct { // VDDO_DISPLAYER_BUFADDR
			uintptr_t              addr_y;       ///< Buffer Starting Address for  Y(or RGB) plane
			uintptr_t              addr_cbcr;    ///< Buffer Starting Address for  CbCR(or A)  plane
			KDRV_VDDO_DISPLAYER    layer;
		} KDRV_VDDO_BUFADDR;
		...
		*/
		...
		struct {
			KDRV_VDDO_DISPLAYER layer;			 ///< the dispaly layer
			UINT32				format; 		 ///< the bit definition: KDRV_VDDO_DISPBUFFORMAT
			BOOL				rld;			 ///< support rld or not
			BOOL				ycc;			 ///< support ycc or not
			BOOL				afbc;			 ///< support afbc or not
		} KDRV_VDDO_CAP;
...
typedef enum {
	...
	*/
	...
	VDDO_DISPCTRL_LINE_MON,						 ///< [set] Set LINE monitor.
	VDDO_DISPCTRL_LINE_MON2,					 ///< [set] Set LINE monitor2.
	/*
	...
	*/
	...
	VDDO_DISPDEV_HDMI_EDID_RAW,                  ///< [get] Get hdmi edid raw.
	VDDO_DISPDEV_HDMI_HOTPLUG_STATUS,            ///< [get] Get hdmi hot plug status. 1: plugged 0:un-plugged
	VDDO_DISPDEV_HDMI_AVI_RGB_SEL,				 ///< [set] Set hdmi AVI info frame RGB Quantization Range sel, please refer KDRV_VDDO_HDMI_AVI_RGB_SEL as input  
	...
	*/
	...
	VDDO_DISPLAYER_CAP, 						 ///< [get] Get display layer capability
