...
//0x200 PAD Power Control Register 0
#define PAD_PWR1_REG_OFS                     0x200
union PAD_PWR1_REG {
	uint32_t reg;
	struct {
...
//0x204 PAD Power Control Register 1
#define PAD_PWR2_REG_OFS                     0x204
union PAD_PWR2_REG {
	uint32_t reg;
	struct {
...
//0x208 PAD Power Control Register 2
#define PAD_PWR3_REG_OFS                     0x208
union PAD_PWR3_REG {
	uint32_t reg;
	struct {
...
//0x20C PAD Power Control Register 3
#define PAD_PWR4_REG_OFS                     0x20C
union PAD_PWR4_REG {
	uint32_t reg;
	struct {
	unsigned int BG_EN:1;		// DDR LDO Bandgap enable
	unsigned int LDO_EN:1;      // DDR LDO power enable
	unsigned int LEVEL_CTL:2;   // DDR LDO Output voltage select
	unsigned int BG_SEL:1;    	// Select Reference Voltage where come from
	unsigned int SUSPEN_0V9:1;	// Suspend Enable
