...
typedef enum {
	...
	///< - @b 0: Issue SPI_GYRO_INT_SYNC_END at last gyro transfer (default)
	///< - @b 1: Issue SPI_GYRO_INT_SYNC_END at second-last gyro transfer
	///< - @b 2: Issue SPI_GYRO_INT_SYNC_END at third-last gyro transfer
	///< - etc...
	SPI_CONFIG_ID_GYRO_BUF_0_ADDR,  ///< Buffer address 0 for gyro usage
	///< must be word align
	SPI_CONFIG_ID_GYRO_BUF_1_ADDR,  ///< Buffer address 1 for gyro usage
	///< must be word align
	...
	SPI_CONFIG_ID_ENG_GYRO_INTMSK_2,//< engineer mode: gyro interrupt mask 2
	SPI_CONFIG_ID_ENG_GYRO_INTMSK_3,//< engineer mode: gyro interrupt mask 3
...
*/
typedef enum {
	SPI_BUS_WIDTH_1_BIT,        //< Bus width is 1 bit (full or half duplex, D0 always ouput, D1 always input)
	SPI_BUS_WIDTH_2_BITS,       //< Bus width is 2 bits (only half duplex. D0,D1 both are input or output direction)
...
typedef enum {
	...
	SPI_GYRO_INT_STOPPED            = 0x00000040,   ///< Gyro has been stopped.
...
typedef enum {
	...
	SPI_VD_SRC_VIE1,                    ///< VD source is VIE1
	SPI_VD_SRC_VIE2,                    ///< VD source is VIE2
	SPI_VD_SRC_VIE3,                    ///< VD source is VIE3
	SPI_VD_SRC_VIE4,                    ///< VD source is VIE4
	ENUM_DUMMY4WORD(SPI_VD_SRC)
} SPI_VD_SRC;

/**
    SPI gyro data path select
*/
typedef enum {
	SPI_GYRO_PATH_SEL_PIO,
	SPI_GYRO_PATH_SEL_DMA,

	ENUM_DUMMY4WORD(SPI_GYRO_PATH_SEL)
} SPI_GYRO_PATH_SEL;
...
typedef struct {
	...
	SPI_GYRO_PATH_SEL gyroPath; ///< Gyro data path select

	UINT32 uiOpInterval;        ///< Delay between 2 OP (unit: us)
	UINT32 uiTransferInterval;   ///< Delay between 2 transfer (unit: us)
	UINT32 uiTransferCount;     ///< total transfer in single run. valid value: 1~256(96 for PIO mode)
	...
	UINT32 uiFirstTransferDelay;///< first transfer delay offset (unit: us). valid value: 0x0~0x1FFFFF

	UINT32 uiOpInterval_odd;        ///< Delay between 2 OP (unit: us)
	UINT32 uiTransferInterval_odd;   ///< Delay between 2 transfer (unit: us)
	UINT32 uiTransferCount_odd;     ///< total transfer in single run. valid value: 1~256(96 for PIO mode)
	UINT32 uiTransferLen_odd;       ///< length of Transer: unit: OP. valid value: 1~4
	UINT32 uiOp0Length_odd;         ///< length of OP0. unit: byte. valid value: 1~8
	UINT32 uiOp1Length_odd;         ///< length of OP1. unit: byte. valid value: 1~8
	UINT32 uiOp2Length_odd;         ///< length of OP2. unit: byte. valid value: 1~8
	UINT32 uiOp3Length_odd;         ///< length of OP3. unit: byte. valid value: 1~8
	UINT32 uiFirstTransferDelay_odd;///< first transfer delay offset (unit: us). valid value: 0x0~0x1FFFFF
...
extern ER   spi_writeReadData(SPI_ID spiID, UINT32 uiWordCount, VOS_ADDR *pTxBuf, VOS_ADDR *pRxBuf, BOOL bDmaMode);
...
extern void spi_setGyroBufAddr(SPI_ID spiID, SPI_CONFIG_ID configID, UINT32 addr);
extern void spi_clrGyroTrsIntEn(SPI_ID spiID);

extern ER   spi_setConfig(SPI_ID spiID, SPI_CONFIG_ID configID, UINT32 configContext);
extern ER   spi_getConfig(SPI_ID spiID, SPI_CONFIG_ID configID, UINT32 *pConfigContext);
