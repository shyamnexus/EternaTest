...
#define DCS_CMD_GET_PIXEL_FMT                   0x0C    ///< get current pixel format

//@}

/**
    @name   Time Gen parameter

*/
//@{
#define TIME_GEN_WIDTH_DUMMY_PIXEL              64      ///< Suitable frame width cause fifo underrun, add dummy pixel to ensure fifo not empty
//@}
...
typedef enum {
	...
	ENUM_DUMMY4WORD(DSI_ID)
} DSI_ID;

#if (defined __FREERTOS)
#if defined(__aarch64__)
#define DSIDATA_TYPE	UINT64
#else
#define DSIDATA_TYPE	UINT32
#endif
#else
#define DSIDATA_TYPE	UINT64
#endif
...
typedef enum {
	...
	DSI_MODE_AUTO_MODE4,        ///< Auto Mode 2: FRM1 + FRM2 + ...
	DSI_MODE_AUTO_MODE5,        ///< Auto Mode 3: CMD(s) + FRM1 + CMD(s) + FRM2 + ...
...
typedef enum {
	...
	DSI_CONFIG_ID_PHY_HS_CLK_INV,   ///< Configure DSI HS clock inverse
...
extern void 	dsi_set_base_addr(DSIDATA_TYPE addr);
...
extern ER       dsi_set_cmd_rw_ctrl(DSI_CFG_CMD_RW_CTRL cmd, DSIDATA_TYPE param);
...
extern void     dsi_set_time_gen_parameter(UINT32 enable, UINT32 data_fmt);

extern void     dsi_dump_info(void);
extern void dsi_set_reg(UINT32 offset, UINT32 value);
extern UINT32 dsi_get_reg(UINT32 offset);
/* dsi int*/
#if defined(__FREERTOS)
