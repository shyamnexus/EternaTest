...
// Typedefs
//------------------------------------------------------------------------------
typedef signed long         NVTMPP_VB_POOL;     ///< Signed long data type
typedef signed long         NVTMPP_VB_BLK;      ///< Signed long data type
...
#define NVTMPP_VB_INVALID_POOL     (signed long)-1
#define NVTMPP_VB_INVALID_BLK      (signed long)-1
...
typedef struct {
	...
	uintptr_t  start_addr;              ///< the starting address of this pool
} NVT_VB_CPOOL_S;

typedef struct {
	uintptr_t virt_addr;    				///< Memory buffer virtual starting address
	uintptr_t phys_addr;    				///< Memory buffer physical starting address
...
*/
uintptr_t  nvtmpp_sys_pa2va(uintptr_t phys_addr);
...
/**
    ...
    @code
    {
		...
		if (addr == 0) {
			DBG_ERR("block2vaddr fail, blk = 0x%lx\r\n", blk);
			return FALSE;
		}
    }
    @endcode
*/
uintptr_t         nvtmpp_vb_blk2va(NVTMPP_VB_BLK blk);
...
/**
    ...
    @endcode
*/
uintptr_t         nvtmpp_vb_blk2pa(NVTMPP_VB_BLK blk);
...
/**
    ...
    @endcode
*/
NVTMPP_VB_BLK  nvtmpp_vb_va2blk(uintptr_t blk_vaddr);
...
/**
    ...
    @endcode
*/
NVTMPP_VB_POOL nvtmpp_vb_addr2pool(uintptr_t blk_addr);
...
*/
NVTMPP_VB_BLK  nvtmpp_sys_get_blk_by_pa(uintptr_t phys_addr);
...
/**
    Get video buffer block size by block handle.

    @param blk: the block handle.

    @return the block buffer size, return 0 when some error happened,
    ...
    @code
    {
		...
		blk_size = nvtmpp_vb_blk2pa(blk);
		if (blk_size == 0) {
			DBG_ERR("get block size fail, blk = 0x%x\r\n", blk);
		}
    }
    @endcode
*/
UINT32  nvtmpp_vb_get_block_size(NVTMPP_VB_BLK blk);
