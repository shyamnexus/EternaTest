...
typedef struct {
    ...
    unsigned long value;            ///< [in]parameter value.
...
typedef struct {
    ...
    unsigned long addr;             ///< [out]data addr. (for new)
...
typedef struct {
	unsigned int vdocap_active_list[16];
	...
	unsigned int vdoprc_vpe_max_handle;
	unsigned int vdoprc_vpe_max_2dlut_sz;
	unsigned int vdoprc_aiisp_maxdevice;
