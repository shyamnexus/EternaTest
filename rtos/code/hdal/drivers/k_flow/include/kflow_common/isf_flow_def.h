...
#define ISF_DATA_DESC_SIZE  64 ///< size of data desc in WORDS
#else
#define ISF_DATA_DESC_SIZE  46 ///< size of data desc in WORDS
#endif

#ifdef ISF_TS
#if defined (__aarch64__)
#define ISF_DATA_SIZE       (74+8+8+8) ///< size of data in WORDS
#else
#define ISF_DATA_SIZE       (52+8+8+8) ///< size of data in WORDS
#endif
#else
#if defined (__aarch64__)
#define ISF_DATA_SIZE       74 ///< size of data in WORDS
#else
#define ISF_DATA_SIZE       52 ///< size of data in WORDS
#endif
#endif
...
typedef struct _ISF_DATA {
	...
	ULONG  h_data;  ///< for common blk = "nvtmpp blk_id"; for private blk = "custom data handle";
	UINT32 src; ///< [private blk] src port, [common blk] for in, it is src's oport, for out, it is this oport
	ULONG  func; ///< [private blk] pool id, [common blk] ;
