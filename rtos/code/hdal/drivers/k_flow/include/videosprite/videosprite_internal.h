...
typedef enum _VDS_INTERNAL_MASK_TYPE {
	...
	VDS_INTERNAL_MASK_TYPE_POLYGON,
...
extern void* vds_lock_context(VDS_QUERY_STAGE stage, UINT32 id, unsigned long *ime_flag);
extern int   vds_unlock_context(VDS_QUERY_STAGE stage, UINT32 id, unsigned long ime_flag);
...
extern UINT32* vds_get_venc_mosaic_blk_size(int vid);
extern UINT32  vds_get_venc_overlap_type(int vid);
