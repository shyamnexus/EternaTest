...
#define VDS_MAX_POLYGON_POSITION 10
...
typedef enum {
	...
	VDS_USER_CMD_OPEN,
	VDS_USER_CMD_SET_VPE_MASK,
	VDS_USER_CMD_SET_VPE_MOSAIC,
	VDS_USER_CMD_SET_IME_FISHEYE,
	VDS_USER_CMD_SET_IME_POLYGON_MASK,
	VDS_USER_CMD_SET_VPE_POLYGON_MASK,
	VDS_USER_CMD_SET_ENCODER_BUILTIN_MOSAIC,
	VDS_USER_CMD_SET_MOSAIC_BLK_SIZE,
	VDS_USER_CMD_SET_STAMP_OVERLAP,
	VDS_USER_CMD_SET_VENC_QP_MOSAIC,
	VDS_USER_CMD_MAX,
...
typedef struct _VDS_MASK_ATTR {
	...
	union{
		...
		struct{
			...
			UINT32     num;
			UINT32     alpha;
			UINT32     x[VDS_MAX_POLYGON_POSITION];
			UINT32     y[VDS_MAX_POLYGON_POSITION];
			UINT32     is_mosaic;
			UINT32     mosaic_blk_w;
			UINT32     mosaic_blk_h;
		}polygon;
...
typedef struct _VDS_VPE_MOSAIC_ATTR {
	UINT8               enable;
	UINT32    		    x[4];
	UINT32    		    y[4];
	UINT32       		mosaic_blk_w;
...
typedef struct _VDS_COLOR_OVERLAP {
	UINT32              type;
} VDS_OVERLAP;

typedef struct __VDS_COLOR_OVERLAP {
	UINT32              magic1;
	UINT32              magic2;
	VDS_OVERLAP         data;
} _VDS_OVERLAP;
...
typedef struct _VDS_ENC_MOSAIC {
	UINT32           x;
	UINT32           y;
	UINT32           w;
	UINT32           h;
...
typedef struct __VDS_ENC_MOSAIC {
	UINT32          magic1;
	UINT32          magic2;
	VDS_ENC_MOSAIC  data;
} _VDS_ENC_MOSAIC;

typedef struct _VDS_MOSAIC_BLK_SIZE {
	UINT32          mosaic_blk_w;
	UINT32          mosaic_blk_h;
} VDS_MOSAIC_BLK_SIZE;

typedef struct __VDS_MOSAIC_BLK_SIZE {
	UINT32               magic1;
	UINT32               magic2;
	VDS_MOSAIC_BLK_SIZE  data;
} _VDS_MOSAIC_BLK_SIZE;
...
typedef struct _VDS_QP_MOSAIC {
	int                qp_mosaic;
} VDS_QP_MOSAIC;

typedef struct __VDS_QP_MOSAIC {
	UINT32             magic1;
	UINT32             magic2;
	VDS_QP_MOSAIC      data;
} _VDS_QP_MOSAIC;
...
typedef struct _VDS_USER_DATA {
	...
	union {
		...
		VDS_ENC_MOSAIC      enc_mosaic;
		VDS_MOSAIC_BLK_SIZE mosaic_blk_size;
		VDS_VPE_MOSAIC_ATTR vpe_mosaic;
		VDS_OVERLAP         overlap;
		VDS_QP_MOSAIC       qp_mosaic;
	} data;  
} VDS_USER_DATA;
