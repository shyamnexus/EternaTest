...
#define CTL_DCE_ISP_WDR_INPUT_YV_BLD_NUM    9
#define CTL_DCE_ISP_WDR_NEQ_TABLE_L_NUM     65
#define CTL_DCE_ISP_WDR_NEQ_TABLE_R_NUM     17
#define CTL_DCE_ISP_WDR_FBC_TH_NUM          6
...
typedef enum {
	CTL_DCE_ISP_WDR_NORMAL_MODE = 0,	///< wdr normal mode
	CTL_DCE_ISP_WDR_ANTI_HALO_MODE = 1,	///< wdr anti halo mode
	ENUM_DUMMY4WORD(CTL_DCE_ISP_WDR_MODE)
} CTL_DCE_ISP_WDR_MODE;

typedef enum {
	CTL_DCE_ISP_ANTI_HALO_BLENDING = 0,	///< blending anti halo method
	CTL_DCE_ISP_ANTI_HALO_ADAPTIVE = 1,	///< adaptive anti halo method
	ENUM_DUMMY4WORD(CTL_DCE_ISP_ANTI_HALO_OPT)
} CTL_DCE_ISP_ANTI_HALO_OPT;
...
typedef struct {
	...
	UINT32 fov_bnd_r;                   ///< Replace value, range [0~255]
	UINT32 fov_bnd_g;                   ///< Replace value, range [0~255]
	UINT32 fov_bnd_b;                   ///< Replace value, range [0~255]
} CTL_DCE_ISP_FOV;
...
*/
typedef struct {
	UINT16 left_table[CTL_DCE_ISP_WDR_NEQ_TABLE_L_NUM];     ///< non equatable table value [0~4095]
	UINT16 right_table[CTL_DCE_ISP_WDR_NEQ_TABLE_R_NUM];    ///< non equatable table value [0~4095]
} CTL_WDR_NEQ_TABLE;
...
typedef struct {
	...
	UINT32 in_yv_blend_lut[CTL_DCE_ISP_WDR_INPUT_YV_BLD_NUM];	///< lut of YV blending [0~63]
} CTL_WDR_IN_BLD;
...
typedef struct {
	...
	CTL_DCE_ISP_WDR_MODE wdr_mode;              ///< wdr mode, 0:Normal mode, 1:Anti-halo mode
	CTL_DCE_ISP_ANTI_HALO_OPT wdr_anti_halo_opt;///< wdr anti halo option, 0:blending, 1:adaptive
...
/**
    DCE structure - WDR foreground brightness compensation parameters
*/
typedef struct {
	BOOL    fbc_en;                             ///< wdr fbc enable, 0:disable, 1:enable
	UINT8   fbc_ratio;                          ///< wdr fbc ratio [0~255]
	UINT16  fbc_th[CTL_DCE_ISP_WDR_FBC_TH_NUM]; ///< wdr th [0~8191]
} CTL_WDR_FBC;
...
typedef struct {
	...
	CTL_WDR_FBC        fbc;              ///< wdr foreground brightness compensation
} CTL_DCE_ISP_WDR;
...
*/
typedef struct {
	UINT32           subimg_size_h;            ///< wdr sub-image size [8~48]
	UINT32           subimg_size_v;            ///< wdr sub-image size [8~48]
