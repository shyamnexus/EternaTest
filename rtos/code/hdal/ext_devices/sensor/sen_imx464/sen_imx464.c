...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx464, 1, 01, 000, 00);
...
static void sen_get_hdr_rhs_imx464(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data);
//=============================================================================
// global variable
//=============================================================================
...
static ER sen_get_info_imx464(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_MFR_OUTPUT_TIMING:
		sen_get_hdr_rhs_imx464(id, (CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *)(data));
		break;
	default:
		rt = E_NOSPT;
	}
...
static void sen_get_hdr_rhs_imx464(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data)
{
	UINT32 fcs =0, temp_vd = 0, temp_max_short_line = 0, t_row = 0;
	UINT32 rhs1 = 0;
	...
	if (mode_basic_param[data->mode].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (((mode_basic_param[data->mode].frame_num) == data->num)  && ( mode_basic_param[data->mode].mode == (data->mode))) {
			if (2 == data->num) { 
				...
				t_row = sen_calc_rowtime_imx464(id, data->mode);
				temp_max_short_line = (100000000 / (data->frame_rate)) * 10 / 17 / t_row; //max support 1:16
				rhs1 = (fcs - (BRL_ALL_PIXEL * 2)) - 2;
				if (rhs1 >= ((BRL_ALL_PIXEL * 2) - 1)) {
					rhs1 =  BRL_ALL_PIXEL * 2 - 1;
				} else if (rhs1 > (9 + temp_max_short_line)) {
				...
				data->diff_row_vd[0] = rhs1;//when VC mode,frame1 and frame2 diff time same as diff_row
			}
		}
	}
}
