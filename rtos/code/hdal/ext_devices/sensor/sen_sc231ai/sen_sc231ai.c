...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_sc231ai, 1, 01, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_SC231AI_MODULE_NAME      "sen_sc231ai"
...
#define NON_HDR_LONG_EXPOSURE_LINE  21
#define NON_HDR_SHORT_EXPOSURE_LINE 19  
#define MAX_SHORT_EXPOSURE_LINE     140 //0x3e23 0x3e24

#define SEN_I2C_ADDR 0x60>>1
#define SEN_I2C_COMPATIBLE "nvt,sen_sc231ai"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc231ai(void);
static void sen_pwr_ctrl_sc231ai(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_sc231ai(CTL_SEN_ID id);
static ER sen_close_sc231ai(CTL_SEN_ID id);
static ER sen_sleep_sc231ai(CTL_SEN_ID id);
static ER sen_wakeup_sc231ai(CTL_SEN_ID id);
static ER sen_write_reg_sc231ai(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_sc231ai(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_sc231ai(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_sc231ai(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_sc231ai(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_sc231ai(CTL_SEN_ID id, void *param);
static void sen_set_expt_sc231ai(CTL_SEN_ID id, void *param);
static void sen_set_preset_sc231ai(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_sc231ai(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_sc231ai(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_sc231ai(CTL_SEN_ID id, void *param);
void sen_get_expt_sc231ai(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_sc231ai(CTL_SEN_ID id, void *param);
static void sen_get_expt_sc231ai(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_sc231ai(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_sc231ai(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_sc231ai(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_sc231ai(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_sc231ai(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_sc231ai(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_sc231ai(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_sc231ai(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_sc231ai(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_sc231ai(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_sc231ai(CTL_SEN_ID id);
static void sen_set_chgmode_fps_sc231ai(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_sc231ai(CTL_SEN_ID id);
...
static CTL_SEN_CMD sc231ai_mode_1[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
	{0x37f9, 1, {0x80, 0x0}},
...
static CTL_SEN_CMD sc231ai_mode_2[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
	{0x37f9, 1, {0x80, 0x0}},
...
static CTL_SEN_CMD sc231ai_mode_3[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
	{0x37f9, 1, {0x80, 0x0}},
...
static CTL_SEN_DRV_TAB sc231ai_sen_drv_tab = {
	sen_open_sc231ai,
	sen_close_sc231ai,
	sen_sleep_sc231ai,
	sen_wakeup_sc231ai,
	sen_write_reg_sc231ai,
	sen_read_reg_sc231ai,
	sen_chg_mode_sc231ai,
	sen_chg_fps_sc231ai,
	sen_set_info_sc231ai,
	sen_get_info_sc231ai,
};

static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc231ai(void)
{
	return &sc231ai_sen_drv_tab;
}
...
static CTL_SEN_CMD sen_set_cmd_info_sc231ai(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_sc231ai(struct device_node *of_node)
{
...
static ER sen_open_sc231ai(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_sc231ai);
	#endif
...
static UINT32 sen_get_cmd_tab_sc231ai(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = sc231ai_mode_1;
		return sizeof(sc231ai_mode_1) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_2:
		*cmd_tab = sc231ai_mode_2;
		return sizeof(sc231ai_mode_2) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_3:
		*cmd_tab = sc231ai_mode_3;
		return sizeof(sc231ai_mode_3) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_sc231ai(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_sc231ai(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_sc231ai(id, &sensor_ctrl);
		sen_set_expt_sc231ai(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_sc231ai(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc231ai(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc231ai(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc231ai(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_sc231ai(0x320E, 1, (sensor_vd >> 8)& 0x7F, 0x0);
			rt |= sen_write_reg_sc231ai(id, &cmd);
			cmd = sen_set_cmd_info_sc231ai(0x320F, 1, sensor_vd & 0xFF, 0x0);
			rt |= sen_write_reg_sc231ai(id, &cmd);
			...
			sen_set_flip_sc231ai(id, &flip);
		} else {
			cmd = sen_set_cmd_info_sc231ai(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_sc231ai(id, &cmd);
		}
	}
...
static ER sen_chg_fps_sc231ai(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc231ai(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc231ai(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc231ai(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_sc231ai(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_sc231ai(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_sc231ai(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_sc231ai(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_sc231ai(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_sc231ai(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_sc231ai(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_sc231ai((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_sc231ai((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_sc231ai(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_sc231ai((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_sc231ai(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_sc231ai(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_sc231ai((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_sc231ai((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_sc231ai((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_sc231ai(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_sc231ai(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_sc231ai(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_sc231ai(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_sc231ai(id, fps);
	sen_set_cur_fps_sc231ai(id, fps);
...
static void sen_set_gain_sc231ai(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if (116550 >= (sensor_ctrl->gain_ratio[frame_cnt])) {
			if (2000 > (sensor_ctrl->gain_ratio[frame_cnt])) { 
				a_gain_h = 0x00;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 1000;
			} else if ( 3700 > (sensor_ctrl->gain_ratio[frame_cnt]) ){
				a_gain_h = 0x01;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 2000;
			} else if ( 7400 > (sensor_ctrl->gain_ratio[frame_cnt]) ){
				a_gain_h = 0x80;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 3700;
			} else if ( 14800 > (sensor_ctrl->gain_ratio[frame_cnt]) ){
				a_gain_h = 0x81;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 7400;
			} else if ( 29600 > (sensor_ctrl->gain_ratio[frame_cnt]) ){
				a_gain_h = 0x83;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 14800;
			} else if ( 59200 > (sensor_ctrl->gain_ratio[frame_cnt]) ){
				a_gain_h = 0x87;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 29600;
			} else {
				a_gain_h = 0x8F;
				a_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 32 / 59200;
			}
			d_gain_h = 0x00;
			d_gain_l = 0x80;		
		} else {
			if (233000 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				d_gain_h = 0x00;
				d_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 128 / 116500;				
			} else if (466000 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				d_gain_h = 0x01;
				d_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 128 / 233000;						
			} else if (932000 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				d_gain_h = 0x03;
				d_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 128 / 466000;						
			} else {
				d_gain_h = 0x07;
				d_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 128 / 932000;						
			}
			a_gain_h = 0x8F;
			a_gain_l = 0x3F;			
		}
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		//Set analog gain
		cmd = sen_set_cmd_info_sc231ai(0x3E08, 1, data1[0] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E09, 1, data2[0] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		//Set digital gain
		cmd = sen_set_cmd_info_sc231ai(0x3E06, 1, data3[0] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E07, 1, data4[0] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		...
		//Set S analog gain
		cmd = sen_set_cmd_info_sc231ai(0x3E12, 1, data1[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E13, 1, data2[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		//Set S digital gain
		cmd = sen_set_cmd_info_sc231ai(0x3E10, 1, data3[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E11, 1, data4[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc231ai(id, &cmd);			
	}
...
static void sen_set_expt_sc231ai(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_sc231ai(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Write exposure line
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_sc231ai(id);
	...
	sen_set_cur_fps_sc231ai(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_sc231ai(id, cur_fps);
	...
	// Set vts to sensor
	cmd = sen_set_cmd_info_sc231ai(0x320E, 1,( sensor_vd >> 8 ) & 0xFF, 0);
	rt |= sen_write_reg_sc231ai(id, &cmd);
	cmd = sen_set_cmd_info_sc231ai(0x320F, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_sc231ai(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
    	...
    	cmd = sen_set_cmd_info_sc231ai(0x3E00, 1, ( line[0] >> 12 ) & 0x0F, 0);
    	rt |= sen_write_reg_sc231ai(id, &cmd);
    	cmd = sen_set_cmd_info_sc231ai(0x3E01, 1, ( line[0] >> 4 ) & 0xFF, 0);
    	rt |= sen_write_reg_sc231ai(id, &cmd);
    	cmd = sen_set_cmd_info_sc231ai(0x3E02, 1, ( line[0] & 0x0F ) << 4, 0);
    	rt |= sen_write_reg_sc231ai(id, &cmd);
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		//Check max exp line reg
		if (line[0] > (MAX_EXPOSURE_LINE)) {
		...
		if (0 != (NON_HDR_SHORT_EXPOSURE_LINE % 2)) {
			max_short_expline = (max_short_expline / 2) + (NON_HDR_SHORT_EXPOSURE_LINE / 2) + 1;
		} else {
			max_short_expline = (max_short_expline / 2) + (NON_HDR_SHORT_EXPOSURE_LINE / 2);
		}
		...
		//Check max exp line
		if ((line[1]) > ((2 * max_short_expline) - NON_HDR_SHORT_EXPOSURE_LINE)) {
			line[1] = (2 * max_short_expline) - NON_HDR_SHORT_EXPOSURE_LINE;
		}	
		
		//Check max exp line
		if ((line[0]) > ((sensor_vd * 2) - (2 * max_short_expline) - NON_HDR_LONG_EXPOSURE_LINE)) {
			line[0] = (sensor_vd * 2) - (2 * max_short_expline) - NON_HDR_LONG_EXPOSURE_LINE;
		}
		...
		cmd = sen_set_cmd_info_sc231ai(0x3E02, 1, (line[0] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E22, 1, (line[1] >> 12) & 0x0F, 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E04, 1, (line[1] >> 4) & 0xFF , 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E05, 1, (line[1] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		
		cmd = sen_set_cmd_info_sc231ai(0x3E23, 1, (max_short_expline >> 8) & 0xFF , 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);
		cmd = sen_set_cmd_info_sc231ai(0x3E24, 1, max_short_expline & 0xFF, 0);
		rt |= sen_write_reg_sc231ai(id, &cmd);					
	}
...
static void sen_set_flip_sc231ai(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_sc231ai(0x3221, 1, 0x0, 0x0);
	rt |= sen_read_reg_sc231ai(id, &cmd);
	...
	cmd = sen_set_cmd_info_sc231ai(0x3221, 1, temp_reg, 0x0);	
	rt |= sen_write_reg_sc231ai(id, &cmd);
...
static void sen_get_fps_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_sc231ai(id);
	data->chg_fps = sen_get_chgmode_fps_sc231ai(id);
}
...
static void sen_get_rowtime_sc231ai(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_sc231ai(id, data->mode);
	data->row_time = sen_calc_rowtime_sc231ai(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_sc231ai(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_sc231ai(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}
int sen_init_sc231ai(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_sc231ai");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_sc231ai");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_sc231ai;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_sc231ai();
	rt = ctl_sen_reg_sendrv("nvt_sen_sc231ai", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_sc231ai(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc231ai");
}

#else
static int __init sen_init_sc231ai(void)
{
...
static void __exit sen_exit_sc231ai(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc231ai");
}

module_init(sen_init_sc231ai);
module_exit(sen_exit_sc231ai);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_SC231AI_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
