...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_os02k10, 1, 01, 000, 00);
...
#define NON_HDR_L_EXPOSURE_LINE    8
#define NON_HDR_S_EXPOSURE_LINE    2
#define HCG_ENABLE                 1
...
static void sen_set_expt_os02k10(CTL_SEN_ID id, void *param)
{
	...
	UINT32 l_expline = 0, s_expline = 0;
	...
	if ((mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) || (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_BUILTIN_HDR)) {
		...
		if (l_expline > (sensor_vd - NON_HDR_L_EXPOSURE_LINE)) {
			l_expline = sensor_vd - NON_HDR_L_EXPOSURE_LINE;
		}

		if (s_expline > (sensor_vd - l_expline - NON_HDR_S_EXPOSURE_LINE)) {
			s_expline = sensor_vd - l_expline - NON_HDR_S_EXPOSURE_LINE;
		}
		
		compensation_ratio[id][1] = 100 * temp_line[1] / s_expline;
		compensation_ratio[id][0] = 100 * temp_line[0] / l_expline;
		
		// set exposure line to sensor
		//L exp
		cmd = sen_set_cmd_info_os02k10(0x3501, 1, (l_expline >> 8 ) & 0xFF, 0);
		rt |= sen_write_reg_os02k10(id, &cmd);
		cmd = sen_set_cmd_info_os02k10(0x3502, 1, l_expline & 0xFF , 0);
		rt |= sen_write_reg_os02k10(id, &cmd);

		//S exp
		cmd = sen_set_cmd_info_os02k10(0x3541, 1, ( s_expline >> 8 ) & 0xFF, 0);
		rt |= sen_write_reg_os02k10(id, &cmd);
		cmd = sen_set_cmd_info_os02k10(0x3542, 1, s_expline & 0xFF , 0);
		rt |= sen_write_reg_os02k10(id, &cmd);
	}
