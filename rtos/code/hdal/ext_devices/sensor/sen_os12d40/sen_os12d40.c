...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_os12d40, 1, 01, 000, 00);
...
static CTL_SEN_CMD shdr_noflip_table[] = {	
	{0x3684, 1, {0x07, 0x0}},
	{0x3687, 1, {0xe2, 0x0}},
	{0x3713, 1, {0x16, 0x0}},
	{0x3720, 1, {0x08, 0x0}},
...
static CTL_SEN_CMD shdr_flip_table[] = {
	{0x3684, 1, {0x27, 0x0}},
	{0x3687, 1, {0x62, 0x0}},
	{0x3713, 1, {0x86, 0x0}},
	{0x3720, 1, {0x01, 0x0}},
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_FLIP) {
		if (*flip & CTL_SEN_FLIP_V) {
			...
			if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_BUILTIN_HDR) {
				p_cmd_list = shdr_flip_table;
				cmd_num =  sizeof(shdr_flip_table) / sizeof(CTL_SEN_CMD);
				for (idx = 0; idx < cmd_num; idx++) {
					cmd = sen_set_cmd_info_os12d40(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
					rt |= sen_write_reg_os12d40(id, &cmd);
				}
			...
			if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_BUILTIN_HDR) {
				p_cmd_list = shdr_noflip_table;
				cmd_num =  sizeof(shdr_noflip_table) / sizeof(CTL_SEN_CMD);
				for (idx = 0; idx < cmd_num; idx++) {
					cmd = sen_set_cmd_info_os12d40(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
					rt |= sen_write_reg_os12d40(id, &cmd);
				}
