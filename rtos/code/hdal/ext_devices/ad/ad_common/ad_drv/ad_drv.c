...
#define AD_DRV_WAIT_TIMEOUT_MS (1000*10) // 10s
...
static SEM_HANDLE g_ad_drv_sem[AD_DRV_CHIP_MAX];

ER ad_drv_sem_wait(UINT32 chip_id)
{
	if (chip_id >= AD_DRV_CHIP_MAX) {
		AD_ERR("fail. chip_id (%u) >= max (%u)\r\n", chip_id, AD_DRV_CHIP_MAX);
		return E_PAR;
	}

	if (SEM_WAIT_TIMEOUT(g_ad_drv_sem[chip_id], vos_util_msec_to_tick(AD_DRV_WAIT_TIMEOUT_MS)) != E_OK) {
		AD_WRN("timeout\r\n");
		return E_TMOUT;
	}
...
void ad_drv_sem_signal(UINT32 chip_id)
{
	if (chip_id >= AD_DRV_CHIP_MAX) {
		AD_ERR("fail. chip_id (%u) >= max (%u)\r\n", chip_id, AD_DRV_CHIP_MAX);
		return;
	}
	SEM_SIGNAL(g_ad_drv_sem[chip_id]);
}
...
ER ad_drv_open(UINT32 chip_id, AD_DRV_OPEN_INFO *open_info)
{
	...
	SEM_CREATE(g_ad_drv_sem[chip_id], 1);
...
ER ad_drv_close(UINT32 chip_id)
{
	...
	SEM_DESTROY(g_ad_drv_sem[chip_id]);
...
#define GET_STR(strary, idx) ((UINT32)(idx) < sizeof(strary)/sizeof(typeof(strary[0])) ? strary[idx] : "Unknown")
	...
	AD_DUMP("%4s | %6s  %7s  %7s  %8s  %7s  %3s  %7s  %9s  %9s  %5s  %6s\r\n", "vout", "active", "bus", "fmt", "protocal", "depth", "mux", "dr_mode", "clk_rate", "data_rate", "dlane", "vir_ch");
	AD_DUMP("==================================================================================================\r\n");
	for (i_vout = 0; i_vout < ad_info->vout_max; i_vout++) {
		vout = &ad_info->vout[i_vout];

		AD_DUMP("%4u | %6u  %7s  %7s  %8s  %7s  %3u  %7s  %9u  %9u  %5d  %6u\r\n",
			i_vout, vout->active, GET_STR(bus_type_str, vout->bus_type), GET_STR(fmt_str, vout->fmt), GET_STR(protocal_str, vout->protocal),
			GET_STR(depth_str, vout->depth), vout->mux_num, GET_STR(dr_mode_str, vout->dr_mode), vout->clk_rate, vout->data_rate, (INT32)vout->data_lane_num, vout->virtual_ch);
	}
...
static ER ad_drv_dbg_dump_bank_reg(UINT32 chip_id)
{
	...
	for (i_bank = 0; i_bank < reg_info->bank_num; i_bank++) {
		...
		// show register number at first line
		AD_DUMP("%*s  ", (int)ad_drv_info->i2c_addr_len*2, " ");
		for (j_addr = 0; j_addr < reg_info->dump_per_row; j_addr+=ad_drv_info->i2c_data_len) {
			AD_DUMP("%0*x ", (int)ad_drv_info->i2c_addr_len*2, j_addr);
		}
		AD_DUMP("\r\n");
		...
		for (i_addr = 0; i_addr < reg_info->bank[i_bank].len; i_addr+=reg_info->dump_per_row) {
			...
			for (j_addr = 0; j_addr < reg_info->dump_per_row; j_addr+=ad_drv_info->i2c_data_len) {
				if (unlikely(ad_drv_i2c_read(chip_id, i_addr+j_addr, &val, ad_drv_info->i2c_data_len))) {
					AD_DUMP("%*s ", (int)ad_drv_info->i2c_data_len*2, "X");
				} else {
