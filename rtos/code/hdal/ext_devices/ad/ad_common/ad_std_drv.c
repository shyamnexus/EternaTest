...
static UINT32 ad_dbg_set_item[CTL_SEN_NUM_SEN_ID][MAX_DBG_SET_ITEM];
//Function prototype
#if 0
#endif
...
static UINT8 fastboot_stage = 0xff;
#else
...
static AD_ID_MAP ad_map[CTL_SEN_NUM_SEN_ID] = {
	[0] = {
		.chip_id = 0,
		.vin_id = 0,
		.vout_id = 0,
...
//=============================================================================
/*
static SEN_I2C sen_i2c[CTL_SEN_NUM_SEN_ID] = {
	{AD_I2C_ID_1, 0x0},
};
*/
static CTL_SENDRV_OPEN_CFG_PARAM opencfg_param[CTL_SEN_NUM_SEN_ID] = {
	[0 ... (CTL_SEN_NUM_SEN_ID-1)] =
	{
		CTL_SEN_FMT_SEL_CCIR656,
		CTL_SEN_SMP_SINGLE_EDGE,
	},
};
...
static CTL_SENDRV_GET_SPEED_PARAM speed_param[CTL_SEN_NUM_SEN_ID] = { //get from driver
    [0 ... (CTL_SEN_NUM_SEN_ID-1)] =
    {
		CTL_SEN_MODE_1,
...
static CTL_SENDRV_GET_MODE_MIPI_PARAM mipi_param[CTL_SEN_NUM_SEN_ID] = { //get from driver
    [0 ... (CTL_SEN_NUM_SEN_ID-1)] =
	{
		CTL_SEN_MODE_1,
...
static CTL_SENDRV_GET_MODE_PARA_PARAM parallel_param[CTL_SEN_NUM_SEN_ID] = {
    [0 ... (CTL_SEN_NUM_SEN_ID-1)] =
	{CTL_SEN_MODE_1, {1}}
};

static CTL_SENDRV_GET_MODE_DVI_PARAM dvi_param[CTL_SEN_NUM_SEN_ID] = { //get from driver
    [0 ... (CTL_SEN_NUM_SEN_ID-1)] =
	{
		CTL_SEN_MODE_1,
		CTL_SEN_DVI_CCIR601,
...
static CTL_SENDRV_GET_MODE_BASIC_PARAM mode_basic_param[CTL_SEN_NUM_SEN_ID] = { //get from driver
    [0 ... (CTL_SEN_NUM_SEN_ID-1)] =
	{
		// 1920x1080p30
		CTL_SEN_MODE_1,
...
static CTL_SEN_SMP_EDGE smp_param[CTL_SEN_NUM_SEN_ID] = { //get from driver
	[0 ... (CTL_SEN_NUM_SEN_ID-1)] = CTL_SEN_SMP_SINGLE_EDGE,
};

static CTL_SEN_CSI_EN_CB ad_std_csi_en_cb = NULL;
static UINT32 ad_std_csi_en_cnt[AD_CHIP_MAX] = {0};
...
static void sen_get_speed_ad_std(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data)
{
    ...
    switch (ad_info[ad_map[id].chip_id].pwr.mclk_sel) {
        ...
        case CTL_SEN_CLK_SEL_SIEMCLK_IGNORE:
            ...
            break;
        default:
            DBG_ERR("id%d, clk src error(%d)!\r\n", (int)id, ad_info[ad_map[id].chip_id].pwr.mclk_sel);
            data->mclk_src = CTL_SEN_SIEMCLK_SRC_MCLK;
            break;
    }
}
...
static void ad_update_mode_info(CTL_SEN_ID id, AD_MODE_INFO mode_info)
{
	...
	switch(mode_info.dr_mode) {
		case AD_DATA_RATE_MODE_SDR:
			smp_param[id] = CTL_SEN_SMP_SINGLE_EDGE;
			break;
		case AD_DATA_RATE_MODE_DDR:
			smp_param[id] = CTL_SEN_SMP_DUAL_EDGE;
			break;
		default:
			DBG_ERR("AD smp type error(%d)\r\n", mode_info.dr_mode);
	}
}
...
static void reset_dbg_default_value(void)
{
    ...
    for (sen_id = 0; sen_id < CTL_SEN_NUM_SEN_ID; sen_id ++) {
...
static ER init_ad_std_drv(CTL_SEN_ID id)
{
	...
	// for 530 VIE set default CHx_FMT
	{
		switch (opencfg_param[id].fmt) {
		case CTL_SEN_FMT_SEL_CCIR601:
			dvi_param[id].fmt = CTL_SEN_DVI_CCIR601;
			break;
		case CTL_SEN_FMT_SEL_CCIR656:
			dvi_param[id].fmt = CTL_SEN_DVI_CCIR656_EAV;
			break;
		case CTL_SEN_FMT_SEL_CCIR709:
			dvi_param[id].fmt = CTL_SEN_DVI_CCIR709;
			break;
		case CTL_SEN_FMT_SEL_CCIR1120:
			dvi_param[id].fmt = CTL_SEN_DVI_CCIR656_1120_EAV;
			break;
		default:
			DBG_ERR("id%u Unknown dvi format %u\r\n", id, opencfg_param[id].fmt);
			dvi_param[id].fmt = CTL_SEN_DVI_CCIR601;
		}

		mode_basic_param[id].if_type = opencfg_param[id].if_type;
		parallel_param[id].mux_info.mux_data_num = g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].mux_num;
	}

	switch (opencfg_param[id].smp_edge) {
	case CTL_SEN_SMP_SINGLE_EDGE:
		g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].dr_mode = AD_DATA_RATE_MODE_SDR;
		break;
	case CTL_SEN_SMP_DUAL_EDGE:
		g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].dr_mode = AD_DATA_RATE_MODE_DDR;
		break;
	default:
		DBG_ERR("id%u Unknown smp_edge %u\r\n", id, opencfg_param[id].smp_edge);
		g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].dr_mode = AD_DATA_RATE_MODE_SDR;
	}
...
static ER chg_mode_ad_std_drv(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	// only enable csi when not enable before
    if ((mode_basic_param[id].if_type == CTL_SEN_IF_TYPE_MIPI) && ad_std_csi_en_cb && ad_std_csi_en_cnt[ad_map[id].chip_id] == 0) {
		ad_sig_sem(id); // unlock before cb csi en
		...
		ad_wait_sem(id); // lock for change ad_std_csi_en_cnt
		ad_std_csi_en_cnt[ad_map[id].chip_id] = 1; // set en cnt to notify next power off can do csi disable cb, to prevent cb csi disable in case of power on -> (without chgmode) -> power off
    }
	ad_sig_sem(id);
...
static ER set_info_ad_std_drv(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
    ...
    switch (drv_cfg_id) {
        case CTL_SENDRV_CFGID_AD_ID_MAP:
            ...
            break;
        default:
            if (ad_map[id].inited == FALSE) {
                DBG_ERR("Sensor id%d : AD map is not set yet!!!\r\n", id);
                return E_NOEXS;
            }
            break;
    }
	...
	switch (drv_cfg_id) {
		...
		case CTL_SENDRV_CFGID_FLIP_TYPE:
			...
			break;
		case CTL_SENDRV_CFGID_SET_OPEN_CFG:
			opencfg_param[id] = *(CTL_SENDRV_OPEN_CFG_PARAM *)data;
			break;
		case CTL_SENDRV_CFGID_AD_INIT:
			g_ad_dev->op.init(ad_map[id].chip_id, NULL);
			break;
	...
	switch (drv_cfg_id) {
		...
		case CTL_SENDRV_CFGID_GET_MODESEL:
			...
			//} else {
				...
				g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].protocal = mode_setting->ccir.fmt;
                g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].data_lane_num = mode_setting->data_lane;
				g_ad_dev->chip[ad_map[id].chip_id].vout[ad_map[id].vout_id].dr_mode = mode_setting->smp_edge;
...
static void sen_pwr_ctrl_ad_std_drv(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb)
{
    ...
    switch (flag) {
        ...
        case CTL_SEN_PWR_CTRL_TURN_OFF:
			// do csi disable cb depend on ad_std_csi_en_cnt, to prevent cb csi disable in case of power on -> (without chgmode) -> power off
			if ((mode_basic_param[id].if_type == CTL_SEN_IF_TYPE_MIPI) && ad_std_csi_en_cb && ad_std_csi_en_cnt[chip_id]) {
				ad_std_csi_en_cb(id, FALSE);
				ad_wait_sem(id);
				ad_std_csi_en_cnt[chip_id] = 0;
				ad_sig_sem(id);
			}
            break;
        default:
...
static BOOL nvt_ad_proc_ad_mapinfo(unsigned char argc, char **pargv)
{
	...
	if (sen_id >= CTL_SEN_NUM_SEN_ID) {
		DBG_ERR("sen id = %d over than max(%d)\r\n", sen_id, CTL_SEN_NUM_SEN_ID-1);
		return -1;
	}
...
static BOOL nvt_ad_proc_dumpinfo(unsigned char argc, char **pargv)
{
    ...
    DBG_DUMP("vout   : mux_num[%d] / contain vin[0x%x](bit mapping) / edge[%s/%s]\r\n", g_ad_dev->chip[chip_id].vout[ad_map[sen_id].vout_id].mux_num, g_ad_dev->chip[chip_id].vout[ad_map[sen_id].vout_id].vout2vin_map, smp_name[smp_param[sen_id]], smp_name[g_ad_dev->chip[chip_id].vout[ad_map[sen_id].vout_id].dr_mode]);
	...
	DBG_DUMP("dvi    : format[CCIR%s/CCIR%s] / field[%s] / pixel array[%s]\r\n", fmt_name[dvi_param[sen_id].fmt], prot_name[g_ad_dev->chip[chip_id].vout[ad_map[sen_id].vout_id].protocal], mode_basic_param[sen_id].mode_type == CTL_SEN_MODE_CCIR_INTERLACE ? "interlace" : "progressive", st_px_name[mode_basic_param[sen_id].stpix-CTL_SEN_STPIX_YUV_YUYV]);
...
void AD_UTIL_DECLARE_FUNC(sen_exit_ad)(void)
{
	...
	ctl_sen_unreg_sendrv(AD_UTIL_STR(nvt_sen_ad));
}
#else
...
static struct proc_ops proc_ad_std_fops = {
	.proc_open    = nvt_ad_proc_std_open,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
	.proc_write   = nvt_ad_proc_std
};
static struct proc_ops proc_ad_drv_fops = {
	.proc_open    = nvt_ad_proc_drv_open,
	.proc_read    = seq_read,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
	.proc_write   = nvt_ad_proc_drv
};
...
static int __init sen_init_ad_std_drv(void)
{
    ...
    if (fastboot_stage == 0xff) {
        fastboot_stage = kdrv_builtin_is_fastboot();
    }
