...
//@{
/*Used in DSI*/
const T_PANEL_CMD t_cmd_mode_dsi2[] = {
#if 1
	// SW reset
	{DSICMD_CMD,     0x01},
	{CMDDELAY_MS,    50},
...
const T_PANEL_CMD t_cmd_standby_dsi2[] = {
...
const T_LCD_PARAM t_mode_dsi2[] = {
	/***********       MI Serial Format 1      *************/
	{
		...
		(T_PANEL_CMD *)t_cmd_mode_dsi2,                 //!< p_cmd_queue
		sizeof(t_cmd_mode_dsi2) / sizeof(T_PANEL_CMD),  //!< n_cmd
	}
};

const T_LCD_ROT *t_rot_dsi2 = NULL;

//@}

T_LCD_ROT *dispdev_get_lcd2_rotate_dsi_cmd(UINT32 *mode_number)
{
#if 0
	if (t_rot_dsi2 != NULL) {
		*mode_number = sizeof(t_rot_dsi2) / sizeof(T_LCD_ROT);
	} else
#endif
	{
		*mode_number = 0;
	}
	return (T_LCD_ROT *)t_rot_dsi2;
}

T_LCD_PARAM *dispdev_get_lcd2_config_mode_dsi(UINT32 *mode_number)
{
	*mode_number = sizeof(t_mode_dsi2) / sizeof(T_LCD_PARAM);
	return (T_LCD_PARAM *)t_mode_dsi2;
}

T_PANEL_CMD *dispdev_get_lcd2_standby_cmd_dsi(UINT32 *cmd_number)
{
	*cmd_number = sizeof(t_cmd_standby_dsi2) / sizeof(T_PANEL_CMD);
	return (T_PANEL_CMD *)t_cmd_standby_dsi2;
}


void dispdev_set_lcd2_dsi_config(DSI_CONFIG *p_dsi_config)
{
#if 0
	...
	/*if(dsi_set_config(DSI_CONFIG_ID_TLPX, 1!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_GO, 7){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}

	if(dsi_set_config(DSI_CONFIG_ID_THS_PREPARE, 1){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_ZERO, 4){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_TRAIL, 2){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_EXIT, 3){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}

	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PREPARE, 1){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_ZERO, 7){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_POST, 8){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PRE, 2){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_TRAIL, 1){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	*/
	if(dsi_set_config(DSI_CONFIG_ID_TLPX, 1)!= E_OK){
	...
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_GO, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_SURE, 1)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_GET, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}

	if(dsi_set_config(DSI_CONFIG_ID_THS_PREPARE, 0)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_ZERO, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_TRAIL, 9)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_EXIT, 4)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}

	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PREPARE, 1)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_ZERO, 3)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_POST, 9)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PRE, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_TRAIL, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	...
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_GO, 21)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_SURE, 0)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_BTA_TA_GET, 20)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}

	if(dsi_set_config(DSI_CONFIG_ID_THS_PREPARE, 3)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_ZERO, 6)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_TRAIL, 4)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	...
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_ZERO, 0x10)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_POST, 0x20)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PRE, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_TRAIL, 3)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	...
	if(dsi_set_config(DSI_CONFIG_ID_THS_PREPARE, 4)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_ZERO, 6)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_TRAIL, 7)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_THS_EXIT, 6)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	...
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_ZERO, 16)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_POST, 16)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
	}
	if(dsi_set_config(DSI_CONFIG_ID_TCLK_PRE, 2)!= E_OK){
		DBG_DUMP("dsi_set_config, line %d fail\r\n", __LINE__);
