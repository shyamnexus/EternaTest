...
UINT32 dispdev_readDsiReg2(UINT32 addr);
UINT32 dispdev_readDsiReg2(UINT32 addr)
{
	UINT32 test;
...
#endif
	p_config_mode = dispdev_get_lcd2_config_mode_dsi(&mode_number);
...
//#if _FPGA_EMULATION_
...
#endif
#endif
	dispdev_set_lcd2_dsi_config(&dsi_config);
	...
	if ((p_mode->panel.lcd_mode >= PINMUX_DSI_1_LANE_CMD_MODE_RGB565) && (p_mode->panel.lcd_mode <= PINMUX_DSI_1_LANE_VDO_SYNC_EVENT_RGB888)) {
		...
		if(dsi_set_config(DSI_CONFIG_ID_DATALANE_NO, DSI_DATA_LANE_0) != E_OK) return E_SYS;
		ui_lane_cnt = 1;
	}
	...
	if ((p_mode->panel.lcd_mode >= PINMUX_DSI_2_LANE_CMD_MODE_RGB565) && (p_mode->panel.lcd_mode <= PINMUX_DSI_2_LANE_VDO_SYNC_EVENT_RGB888)) {
		...
		if(dsi_set_config(DSI_CONFIG_ID_DATALANE_NO, DSI_DATA_LANE_1) != E_OK) return E_SYS;
		ui_lane_cnt = 2;
	}
	...
	if ((p_mode->panel.lcd_mode >= PINMUX_DSI_4_LANE_CMD_MODE_RGB565) && (p_mode->panel.lcd_mode <= PINMUX_DSI_4_LANE_VDO_SYNC_EVENT_RGB888)) {
		...
		if(dsi_set_config(DSI_CONFIG_ID_DATALANE_NO, DSI_DATA_LANE_3) != E_OK) return E_SYS;
		ui_lane_cnt = 4;
	}
	...
	if (ui_pixel_pkt_mode_sel == DSI_PIXMODE_COMMAND_MODE) {
		if (dsi_set_config(DSI_CONFIG_ID_BLANK_CTRL, TRUE) != E_OK)
			DSI_ERR_MSG(" Configure ID blank fail\r\n");
		if (dsi_set_config(DSI_CONFIG_ID_INTER_PKT_LP, TRUE) != E_OK)
			DSI_ERR_MSG(" Configure inter pkt lp fail\r\n");
#if 0
		...
		if ((p_mode->panel.lcd_mode >= PINMUX_DSI_4_LANE_VDO_SYNC_PULSE_RGB565) &&
			...
			if(dsi_set_config(DSI_CONFIG_ID_BLANK_CTRL, TRUE) != E_OK){
				DSI_ERR_MSG(" Configure DSI_CONFIG_ID_BLANK_CTRL error\r\n");
			}
		} else {
		...
		if(dsi_set_config(DSI_CONFIG_ID_INTER_PKT_LP, FALSE)!= E_OK){
			DSI_ERR_MSG(" Configure DSI_CONFIG_ID_INTER_PKT_LP error\r\n");
		}
...
//          uiVIDEData = (uiVtotal_Sub_Vactive) * p_mode->panel.ui_hsync_total_period * 3 / 2;
		...
		if (ui_video_sync_mode == DSI_VIDEOPKT_TYPE_SYNC_PULSE) {
			...
			if (dsi_set_config(DSI_CONFIG_ID_SYNCEVT_NULL_LEN, 0x20) != E_OK) {
				DSI_CFG_MSG(" DSI_CONFIG_ID_SYNCEVT_NULL_LEN fail\r\n");
			}
			if (dsi_set_config(DSI_CONFIG_ID_SYNCEVT_SLICE_NO, 0) != E_OK) {
				DSI_CFG_MSG(" DSI_CONFIG_ID_SYNCEVT_SLICE_NO fail\r\n");
			}
	...
	/*case DISPCTRL_SRCCLK_IDE2_PLL10: */{
			src_pll = PLL_ID_4;
		}
		break;
...
//#endif
	...
	p_standby = dispdev_get_lcd2_standby_cmd_dsi(&cmd_number);
