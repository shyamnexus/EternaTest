...
static ER dispdev_open_if8bits(void)
{
	...
	if (lcd_mode == PINMUX_LCDMODE_CCIR656) {
		...
		*/
		...
		DBG_IND("ui_vsync_sync_width------------------->[0x%02x]\r\n", p_mode->panel.ui_vsync_sync_width);
		...
		if (p_mode->ide.interlace == TRUE) {
			...
			dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_start  = ((p_mode->panel.ui_vsync_sync_width + 1) >> 1) + 1; //0x04;//((p_mode->panel.ui_vsync_sync_width + 1) >> 1) + 2;
			dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_end    = (p_mode->panel.ui_vsync_total_period >> 1) + dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_start; //0x010A;//p_mode->panel.ui_vsync_total_period + dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_field_start;
		} else {
			...
			dev_io_ctrl.SEL.SET_CCIR656_SYNCCODE.ui_even_end     = (p_mode->panel.ui_vsync_active_period) + (p_mode->panel.ui_vsync_back_porch_odd) + ((p_mode->panel.ui_vsync_total_period - p_mode->panel.ui_vsync_active_period ));
	...
	//dispdev_set_display(p_disp_dev_control, T_LCD_INF_PARALLEL_16BITS, p_mode, pin_func_id); //for test 16-bit
