...
#ifdef _NVT_FPGA_
	mi_config.mi_src_clk = (float)12.0;
#else	
	//mi_config.mi_src_clk = (float)192.0;
#endif	
	dispdev_set_mi_config(&mi_config);
	mi_setManual(TRUE);
...
static void dispdev_write_lcd_reg(UINT32 ui_addr, UINT32 ui_value)
{
	dispdev_write_lcd_mi(ui_addr, ui_value);
	//return E_OK;
}
...
static UINT32  dispdev_read_lcd_reg(UINT32 ui_addr)
{
	...
	dispdev_read_lcd_mi(ui_addr, &ui_value);
	return ui_value;
}
