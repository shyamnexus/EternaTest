...
*/
//@{
const T_PANEL_CMD t_cmd_standby2[] = {
	//{0x2B, 0x00}
};

const T_PANEL_CMD t_cmd_mode_rgb3202[] = {
    {IFCMD_CMD,0x11},
...
const T_LCD_PARAM t_mode2[] = {
	/***********       RGB 320 mode         *************/
	{
		...
		(T_PANEL_CMD *)t_cmd_mode_rgb3202,                 //!< p_cmd_queue
		sizeof(t_cmd_mode_rgb3202) / sizeof(T_PANEL_CMD),  //!< n_cmd
	}

};
const T_PANEL_CMD t_cmd_rotate_none2[] = {
	//{0x04,0x18}
};
const T_PANEL_CMD t_cmd_rotate_1802[] = {
	//{0x04,0x1b}
};
const T_LCD_ROT t_rotate2[] = {
	//{DISPDEV_LCD_ROTATE_NONE, (T_PANEL_CMD *)t_cmd_rotate_none, 1},
	//{DISPDEV_LCD_ROTATE_180, (T_PANEL_CMD *)t_cmd_rotate_180, 1}
};

const T_LCD_SIF_PARAM t_sif_param2[] = {
	{
	// SIF mode 00
	0,
	//SIF_MODE_00,
...
T_LCD_ROT *dispdev_get_lcd_rotate_cmd2(UINT32 *mode_number)
{
	...
	*mode_number = sizeof(t_rotate2) / sizeof(T_LCD_ROT);

	return (T_LCD_ROT *)t_rotate2;
}

T_LCD_PARAM *dispdev_get_config_mode2(UINT32 *mode_number)
{
	*mode_number = sizeof(t_mode2) / sizeof(T_LCD_PARAM);
	return (T_LCD_PARAM *)t_mode2;
}

T_PANEL_CMD *dispdev_get_standby_cmd2(UINT32 *cmd_number)
{
	*cmd_number = sizeof(t_cmd_standby2) / sizeof(T_PANEL_CMD);
	return (T_PANEL_CMD *)t_cmd_standby2;
}
...
static void dispdev_writeToLcdGpio_Cmd2(DISPDEV_IOCTRL p_disp_dev_control, UINT32 addr, UINT32 value)
{
...
static void dispdev_writeToLcdGpio_data2(DISPDEV_IOCTRL p_disp_dev_control, UINT32 addr, UINT32 value)
{
 	UINT32                  ui_sif_data, j;
	UINT32                  sif_clk, sif_sen, sif_data;
	DISPDEV_IOCTRL_PARAM    dev_io_ctrl;
...
void dispdev_write_to_lcd_gpio2(DISPDEV_IOCTRL p_disp_dev_control, UINT32 addr, UINT32 value)
{
    ...
    if(addr & IFCMD_DATA)
            dispdev_writeToLcdGpio_data2(p_disp_dev_control,addr,value);
    else
            dispdev_writeToLcdGpio_Cmd2(p_disp_dev_control,addr,value);

}

void dispdev_read_from_lcd_gpio2(DISPDEV_IOCTRL p_disp_dev_control, UINT32 addr, UINT32 *p_value)
{
...
{
	return (T_LCD_SIF_PARAM *)t_sif_param2;
}
...
int panel_init2(void)
{
	...
	nodeoffset = fdt_path_offset((const void*)fdt_addr, "/display2");
	if (nodeoffset >= 0) {
...
static int __init panel_init2(void)
{
	...
	panel = of_find_node_by_name(NULL, "display2");
	if (panel) {
