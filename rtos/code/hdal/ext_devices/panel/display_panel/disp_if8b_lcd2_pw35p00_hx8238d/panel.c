...
const T_LCD_ROT t_rotate2[] = {
	{DISPDEV_LCD_ROTATE_NONE, (T_PANEL_CMD *)t_cmd_rotate_none2, 1},
	{DISPDEV_LCD_ROTATE_180, (T_PANEL_CMD *)t_cmd_rotate_1802, 1}
};
...
int panel_init2(void)
{
	...
	display_read_cfg(&gamma_config, &color_control_config, DISP_2);
...
static int __init panel_init2(void)
{
	...
	if ((strstr(panel_cfg_path, "null")) || (strstr(panel_cfg_path, "NULL"))) {
		...
		display_read_cfg(cfg_path, DISP_2);
	}	
	p_disp_obj->dev_callback = &dispdev_get_lcd2_dev_obj;
	p_disp_obj->close();
