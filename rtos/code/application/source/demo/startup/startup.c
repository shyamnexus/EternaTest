...
void vApplicationIdleHook(void)
{
	...
	cur_time = hrtimer_get_target_count();
...
{
	prev_wdt_timer_time = hrtimer_get_target_count();
...
static void os_init(void)
{
	...
	init_parm.total.size = (ULONG)_libc_heap_end;
	init_parm.resv.addr = 0;
	init_parm.resv.size = (ULONG)_section_01_addr;
	init_parm.code.addr = (ULONG)_section_01_addr;
	init_parm.code.size = (ULONG)_image_general_zi_zi_base - (ULONG)_section_01_addr;
	init_parm.zi.addr   = (ULONG)_image_general_zi_zi_base;
	init_parm.zi.size   = (ULONG)_image_general_zi_zi_limit - (ULONG)_image_general_zi_zi_base;
	init_parm.os_heap.addr = (ULONG)_rtos_heap_start;
	init_parm.os_heap.size = (ULONG)_rtos_heap_end - (ULONG)_rtos_heap_start;
	init_parm.libc_heap.addr = (ULONG)_libc_heap_start;
	init_parm.libc_heap.size = (ULONG)_libc_heap_end - (ULONG)_libc_heap_start;
...
static void rtos_main_tsk(void *p_param)
{
	...
	vos_task_resume(vos_task_create(env_init, NULL, "env_init", 28, 2048));
...
int main(void)
{
	...
	// vTaskStartScheduler will enable cpu timer cause timer interrupts should be handle by gic
	arm_gic_init();
	// start the scheduler.
	vTaskStartScheduler();
