...
#define ECP_POINTS1_NUM 16
#define ECP_POINTS2_NUM 16
static mbedtls_ecp_point points1[ECP_POINTS1_NUM];
static mbedtls_mpi       points2[ECP_POINTS2_NUM];
static int points1_allocated = 0;
static int points2_allocated = 0;
                                   ...
                                   mbedtls_ecp_point *T[], size_t T_size )
{
	...
	if( points2_allocated ){
		DBG_ERR("ecp points#2 is already allocated\r\n");
		return( MBEDTLS_ERR_ECP_ALLOC_FAILED );
	}
	if( T_size > ECP_POINTS2_NUM){
		DBG_ERR("T_size(%d) > ECP_POINTS2_NUM(%d)\r\n", T_size, ECP_POINTS2_NUM);
		return( MBEDTLS_ERR_ECP_ALLOC_FAILED );
	}
	memset(points2, 0, sizeof(points2));
	c = points2;
                         ...
                         mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    /* Allocate table if we didn't have any */
    {
        ...
        //    goto cleanup;
        //}
		if( points1_allocated ){
			DBG_ERR("ecp points#1 is already allocated\r\n");
            ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
            goto cleanup;
		}
		if( T_size > ECP_POINTS1_NUM){
			DBG_ERR("T_size(%d) > ECP_POINTS1_NUM(%d)\r\n", T_size, ECP_POINTS1_NUM);
            ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
            goto cleanup;
		}
		memset(points1, 0, sizeof(points1));
		T = points1;
