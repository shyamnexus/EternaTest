...
function make_gcov() {
	...
	echo -e "alias gcov_tar=\"mkdir -p \${GCOV_PREFIX};cp -rf /sys/kernel/debug/gcov/home \${GCOV_PREFIX}/../../; cd /mnt/sd; tar -jcf gcov.tar.bz2 gcov; sync\"" >> ${ROOTFS_DIR}/rootfs/etc/profile
...
function gen_report_user() {
	rm -rf report_u
	${BUILD_DIR}/lcov/lcov -c -d $1 -t ${MODEL} -o ${MODEL}.user.info --gcov-tool ${CROSS_COMPILE}gcov
	${BUILD_DIR}/lcov/genhtml -o report_u ${MODEL}.user.info
}

function gen_report_kernel() {
	rm -rf report_k
	${BUILD_DIR}/lcov/lcov -c -d $1 -b ${KERNELDIR} -t ${MODEL} -o ${MODEL}.kernel.info --gcov-tool ${CROSS_COMPILE}gcov
	${BUILD_DIR}/lcov/genhtml -o report_k ${MODEL}.kernel.info
}
