declare -A toolchains=(
	...
	# aarch64-ca53-linux-gnueabihf-10.4.0
	[aarch64-ca53-linux-gnueabihf-10.4.0, host]="aarch64-ca53-linux-gnu"
	[aarch64-ca53-linux-gnueabihf-10.4.0, sysroot]="usr/aarch64-ca53-linux-gnu/sysroot"
	[aarch64-ca53-linux-gnueabihf-10.4.0, compile]="usr/bin/aarch64-ca53-linux-gnu-"
	...
	# aarch64-ca53-elf-newlibc-8.5
	[aarch64-ca53-elf-newlibc-8.5, host]="arm-eabihf"
	[aarch64-ca53-elf-newlibc-8.5, sysroot]=""
	[aarch64-ca53-elf-newlibc-8.5, compile]="bin/aarch-eabi-"
	# aarch64-ca53-elf-newlibc-10.3.0
	[aarch64-ca53-elf-newlibc-10.3.0, host]="aarch"
	[aarch64-ca53-elf-newlibc-10.3.0, sysroot]=""
	[aarch64-ca53-elf-newlibc-10.3.0, compile]="bin/aarch-"
)
...
declare -A compiler_flags=(
	...
	# [rtos, cortex-a53x64
	[rtos, cortex-a53x64, c]="-march=armv8-a -mtune=cortex-a53 -nostartfiles -Wall -Winline -Wno-missing-braces  -Wpointer-arith -Wsign-compare -Wstrict-prototypes -Wundef -Werror -fno-exceptions -fno-common -fno-optimize-sibling-calls  -fno-strict-aliasing -fshort-wchar -O3 -fPIC -ffunction-sections -fdata-sections -fno-omit-frame-pointer"
	[rtos, cortex-a53x64, cxx]="-march=armv8-a -mtune=cortex-a53 -nostartfiles -Wall -Winline -Wno-missing-braces  -Wpointer-arith -Wsign-compare -Wundef -Werror -fno-exceptions -fno-common -fno-optimize-sibling-calls  -fno-strict-aliasing -fshort-wchar -O3 -fPIC -ffunction-sections -fdata-sections -fno-omit-frame-pointer"
	[rtos, cortex-a53x64, s]="-march=armv8-a -mtune=cortex-a53 -nostartfiles -Werror"
)
...
function add_lunch_toolchain_menu()
{
	...
	if [ $NVT_PRJCFG_CFG == "Linux" ]; then
		if [[ $LINUX_CPU_TYPE == *"x64" ]]; then
			add_lunch_toolchain_combo aarch64-ca53-linux-gnueabihf-10.4.0
		else
			add_lunch_toolchain_combo arm-ca53-linux-gnueabihf-8.4
		fi
	elif [ $NVT_PRJCFG_CFG == "rtos" ]; then
		if [[ $RTOS_CPU_TYPE == *"x64" ]]; then
			add_lunch_toolchain_combo aarch64-ca53-elf-newlibc-10.3.0
		else
			add_lunch_toolchain_combo gcc-linaro-6.4.1-2018.05-x86_64_arm-eabi
		fi
	else
...
{
	export KERNELDIR=${LINUX_BUILD_BSP}/linux-5.10.y
	export NVT_LINUX_VER="5.10.168"
}
...
function set_stuff_for_environment()
{
	...
	if [ $NVT_PRJCFG_CFG == "Linux" ]; then
		...
		export KERNELDIR=${LINUX_BUILD_BSP}/rtos/${RTOS_TYPE}
	fi
	export BUSYBOX_DIR=${LINUX_BUILD_BSP}/busybox
	export TOYBOX_DIR=${LINUX_BUILD_BSP}/toybox
	export ATF_DIR=${LINUX_BUILD_BSP}/trusted-firmware-a
...
function setpath_toolchain_config()
{
	...
	if [ -z $CROSS_TOOLCHAIN_DIR ]; then
		if [ -d /opt/ivot/$NVT_CROSS ]; then
			...
			export CROSS_TOOLCHAIN_DIR=/opt/arm
		fi
	fi
	...
	if [ $NVT_PRJCFG_CFG == "Linux" ]; then
		...
		if [ $RTOS_TYPE == "amazon-freertos" ]; then
			export FREERTOS_INC_DIR="-I${KERNELDIR}/lib/include -I${KERNELDIR}/lib/include/private -I${KERNELDIR}/include"
		elif [ $RTOS_TYPE == "freertos" ]; then
			export FREERTOS_INC_DIR="-I${KERNELDIR}/include -I${KERNELDIR}/include/freertos -I${KERNELDIR}/include/freertos-port -I${KERNELDIR}/include/lwip -I${KERNELDIR}/include/lwip-port -I${KERNELDIR}/include/posix -I${KERNELDIR}/include/posix/private -I${KERNELDIR}/include/posix-port -I${KERNELDIR}/include/tcp -I${KERNELDIR}/include/tcp-port"
		fi
		export PLATFORM_CFLAGS="${compiler_flags[${NVT_PRJCFG_CFG}, ${RTOS_CPU_TYPE}, c]} -D_BSP_${SDK_CODENAME^^}_ -D__FREERTOS ${FREERTOS_INC_DIR}"
		export PLATFORM_CXXFLAGS="${compiler_flags[${NVT_PRJCFG_CFG}, ${RTOS_CPU_TYPE}, cxx]} -D_BSP_${SDK_CODENAME^^}_ -D__FREERTOS ${FREERTOS_INC_DIR}"
		export PLATFORM_AFLAGS="${compiler_flags[${NVT_PRJCFG_CFG}, ${RTOS_CPU_TYPE}, s]} -D_BSP_${SDK_CODENAME^^}_ -D__FREERTOS ${FREERTOS_INC_DIR}"
		if [[ $RTOS_CPU_TYPE == *"x64" ]]; then
			export UBOOT_CROSS_COMPILE_PATH=${CROSS_TOOLCHAIN_DIR}/${DEFAULT_NVT_UBOOT_CROSS_FOR_RTOS64}
			export UBOOT_CROSS_COMPILE=${UBOOT_CROSS_COMPILE_PATH}/${toolchains[${DEFAULT_NVT_UBOOT_CROSS_FOR_RTOS64}, compile]}
		else
	...
	#
	# if _NVTTIMER_DISABLE_ -> use SW timer delay
	# rtos/freertos/novatek/ns02201/include/FreeRTOSConfig.h
	# configTICK_RATE_HZ
	#
	if [ "$RTOS_NVTTIMER" == "OFF" ]; then
		export PLATFORM_CFLAGS="${PLATFORM_CFLAGS} -D_NVTTIMER_DISABLE_"
		export PLATFORM_AFLAGS="${PLATFORM_AFLAGS} -D_NVTTIMER_DISABLE_"
	fi
	export PLATFORM_CFLAGS="${PLATFORM_CFLAGS} -D_${NVT_SDK_TYPE}_"
	export PLATFORM_AFLAGS="${PLATFORM_AFLAGS} -D_${NVT_SDK_TYPE}_"
}
